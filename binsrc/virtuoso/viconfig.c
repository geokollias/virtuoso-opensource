...
extern dk_cpu_set_t wi_cl_listener_affinity;
extern int wi_is_cl_listener_affinity;
extern int32 cp_unremap_quota;
extern int correct_parent_links;
extern long repl_queue_max;
extern int32 repl_write_timeout;
...
cfg_setup (void)
{
  ...
  f_config_g_file = s_strdup (setext (f_config_file, "", EXT_REMOVE));
  f_config_g_file = s_strdup (setext (f_config_g_file, "global.ini", EXT_SET));
  ...
  cfg_init (&pconfig_g, f_config_g_file);
#undef cfg_getlong
#undef cfg_getstring
#define cfg_getlong(pc, sect, name, ret) cfg2_getlong (pc, pconfig_g, sect, name, ret)
#define cfg_getstring(pc, sect, name, ret) cfg2_getstring (pc, pconfig_g, sect, name, ret)
#define cfg_getsize(pc, sect, name, ret) cfg2_getsize (pc, pconfig_g, sect, name, ret)
#ifndef WIN32
  {
  ...
  {
    char * affinity;
    if (0 == cfg_getstring (pconfig, section,  "Affinity", &affinity))
      {
	if (0 == thread_parse_cpu_set (&wi_affinity, affinity))
	  {
	    thread_set_default_affinity (&wi_affinity);
	    thread_set_affinity (&wi_affinity);
	  }
	else
	  log_error ("Bad cpu affinity spec %s", affinity);
      }
    if (0 == cfg_getstring (pconfig, section,  "ListenerAffinity", &affinity))
      {
	if (0 != thread_parse_cpu_set (&wi_cl_listener_affinity, affinity))
	  log_error ("Bad cpu affinity spec %s", affinity);
	else
	  wi_is_cl_listener_affinity = 1;
      }
  }
...
/*    c_sqlo_enable = 1;*/
  ...
  */
  ...
  if (cfg_getlong (pconfig, section, "SimpleRdfNumbers", &simple_rdf_numbers) == -1)
    simple_rdf_numbers = 0;
...
int
cfg2_getsize (PCONFIG pc, PCONFIG pc_g, char * sec, char * attr, size_t * sz)
{
  char * str;
  if (-1 == cfg2_getstring (pc, pc_g, sec, attr, &str))
    return -1;
  if (-1 == cfg_parse_size_with_modifier (str, sz, NULL, NULL))
    return -1;
  return 0;
