...
const char *wsse_uris[] = { WSS_WSS_URI, WSS_WSS_URI_0204, WSS_WSS_URI_0207, WSS_WSS_URI_OASIS, NULL };
const char *wsu_uris[] = { WSS_WSU_URI, WSS_WSU_URI_OASIS, NULL };
...
caddr_t *xenc_generate_security_tags (query_instance_t * qi, xpath_keyinst_t ** arr,
    dsig_signature_t * dsig, int gen_ref_list, caddr_t * err_ret, wsse_ser_ctx_t * sctx);

static void xenc_security_token_id_format (char *buf, int maxlen, xenc_id_t id, int is_ref);
...
static void
check_ents (xml_tree_ent_t ** ents, int arg, const char *funname)
{
  int inx;
...
static void
check_key_instance (xenc_key_inst_t * kei, int arg, char *func)
{
  if (!kei)
  ...
  while (kei)
    {
	  ...
	  (BOX_ELEMENTS (kei) != 3) ||
	  ((DV_TYPE_OF (kei->xeki_key_name) != DV_C_STRING) && (DV_TYPE_OF (kei->xeki_key_name) != DV_STRING)))
	sqlr_new_error ("42000", "XENC03", "Argument %d of %s is not key instance (%s)", arg + 1, func, kei->xeki_key_name);
...
caddr_t
xenc_get_option (caddr_t * options, const char *opt, char *def)
{
  int ix;
  if (!options)
    return def;
...
caddr_t
xenc_encrypt (caddr_t src, xenc_key_inst_t * key)
{
...
void
xml_doc_subst_free (xml_doc_subst_t * xs)
{
  dk_free (xs, sizeof (xml_doc_subst_t));
}


close_tag_t *bx_pop_ct (caddr_t * qst, dk_session_t * out, close_tag_t * ct, wcharset_t * src_charset, int child_num);

caddr_t *
xenc_get_namespaces (caddr_t * curr, id_hash_t * namespaces)
{
...
void
xenc_set_namespaces (caddr_t * curr, caddr_t * nss, id_hash_t * h)
{
  id_hash_set (h, (caddr_t) & curr, (caddr_t) & nss);
}

void
xenc_nss_add_namespace_prefix (id_hash_t * namespaces, caddr_t * tag, const char *uri, const char *prefix)
{
 ...
 */
static int
ns_is_in_ancestor (dk_set_t nss1, caddr_t pref, caddr_t uri)
{
...
void
xenc_bx_out_q_name (caddr_t * qst, dk_session_t * out, close_tag_t * ct, caddr_t name, int is_attr, wcharset_t * src_charset,
    dk_set_t namespaces)
{
  caddr_t pref = NULL;
  int ns_len;
...
#endif
#if 1
      if (xml_is_sch_qname (tag[0], tag[inx]) || xml_is_soap_qname (tag[0], tag[inx]) || xml_is_wsdl_qname (tag[0], tag[inx]))
  ...
  if (DV_ARRAY_OF_POINTER == dtp)
	  ...
	  if (uname__disable_output_escaping == name)
	    {
	      if (len > 1)
		dks_esc_write (ses,
		    (char *) current[1], box_length ((caddr_t) current[1]) - 1, xsst->xsst_charset, CHARSET_UTF8, DKS_ESC_NONE);
	      return;
	    }
	  if (uname__attr == name)
	    return;
      ...
      if (!is_root)
	{
	  xenc_bx_tree_start_tag (xsst->xsst_qst, ses, (caddr_t *) current[0], &(xsst->xsst_ct),
	      len - 1, xsst->xsst_out_method, &curr_tag, 0, xsst->xsst_charset, CHARSET_UTF8, xs->xs_parent_link);
	}
      if (content)
...
caddr_t
xml_doc_subst (xml_doc_subst_t * xs)
{
  dk_session_t *ses;
  ...
  if (xs->xs_envelope && xs->xs_new_child_tags)
    {
      ...
      if (header)
	{
	  new_header = (caddr_t *) dk_alloc_box (box_length (header) + box_length (xs->xs_new_child_tags), DV_ARRAY_OF_POINTER);
...
static int
xenc_base64_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
...
static int
xenc_base64_decryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  int read_b;
  int len;
...
xenc_persist_key (xenc_key_t * k, caddr_t * qst, int store, caddr_t * err_ret)
{
  ...
  err = qr_rec_exec (qr[idx], qi->qi_client, NULL, qi, NULL, 1, ":0", k->xek_name, QRP_STR);
err:
  if (SQL_SUCCESS != err)
    {
      if (err_ret)
...
#if 0
xenc_algo_t *
xenc_algo_copy (xenc_algo_t * algo)
{
...
xenc_algo_t *
xenc_algorithms_get (const char *name)
{
...
int
xenc_algorithms_create (const char *ns0, const char *name, xenc_encryptor_f enc, xenc_decryptor_f dect, DSIG_KEY_TYPE key_type)
{
  ptrlong idx;
  xenc_algo_t *algo;
...
xenc_doc_t *
xenc_doc_create (xml_tree_ent_t * doc_ent)
{
...
xenc_key_inst_t *
xenc_create_key_instance (const char *name, xenc_key_inst_t * super_key_inst)
{
...
caddr_t
bif_xenc_key_inst_create (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (super && (DV_TYPE_OF (super) != DV_ARRAY_OF_POINTER || BOX_ELEMENTS (super) != 3))
    sqlr_new_error ("42000", "XENC05", "Argument 2 of xenc_key_inst_create is not key instance");
...
xenc_key_t *
xenc_get_key_by_name (const char *name, int protect)
{
  xenc_key_t **key_ptr;
...
int
xenc_store_key (xenc_key_t * key, int protect)
{
  if (protect)
    mutex_enter (xenc_keys_mtx);
...
*/
xenc_key_t *
xenc_key_create (const char *key_name, const char *enc_type /* algorithm */ ,
...
void
xenc_key_remove (xenc_key_t * key, int lock)
{
  if (lock)
    mutex_enter (xenc_keys_mtx);
...
void
xenc_certificates_hash_add (caddr_t keyidentifier, xenc_key_t * k, int lock)
{
  if (lock)
    mutex_enter (xenc_keys_mtx);
...
static caddr_t
bif_delete_temp_keys (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  int c = 0;
  mutex_enter (xenc_keys_mtx);
...
static caddr_t
bif_xenc_set_primary_key (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
xenc_key_t *
xenc_get_key_by_keyidentifier (caddr_t keyident, int lock)
{
  if (lock)
    mutex_enter (xenc_keys_mtx);
  if (keyident)
...
xenc_key_t *
xenc_key_create_from_x509_cert (char *name, char *certificate, char *private_key_str,
...
#else
      x509 = (X509 *) PEM_ASN1_read_bio ((char *(*)()) d2i_X509, PEM_STRING_X509, b, NULL, NULL, NULL);
#endif
    }
  ...
  if (b_priv)
    {
	  ...
	  PEM_STRING_EVP_PKEY, b_priv, NULL, pass_cb, (void *) private_key_passwd);
#endif
      if (!private_key)
	{
#if 0
...
static				/*xenc_key_DSA_create */
    caddr_t
bif_xenc_key_dsa_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  xenc_key_t *key;
...
static caddr_t
bif_xenc_key_DH_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  xenc_key_t *key;
  ...
  if (!dh)
    {
      mutex_leave (xenc_keys_mtx);
      sqlr_new_error ("42000", "XENC11", "DH parameters generation error");
    }
  if (!dh || !DH_generate_key (dh))
    {
      mutex_leave (xenc_keys_mtx);
      sqlr_new_error ("42000", "XENC12", "Can't generate the DH private key");
    }
  key->ki.dh.dh_st = dh;
...
static caddr_t
bif_xenc_DH_get_params (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  xenc_key_t *key;
...
static caddr_t
bif_xenc_DH_compute_key (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  xenc_key_t *key;
...
static caddr_t
bif_xenc_xor (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
caddr_t
bif_xenc_bn2dec (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
#ifdef _KERBEROS
int _krb_init_srv_ctx (caddr_t service_name, caddr_t tkt, gss_ctx_id_t * context);
#endif

#define XENC_SERVICE_NAME "host"

xenc_key_t *
xenc_key_create_from_kerberos_tgs_cert (const char *name, caddr_t decoded_cert)
{
#if 0
...
static xenc_cert_type_t xenc_cert_types[] = {
  {"Kerberosv5TGT"},
  {"Kerberosv5ST"},
  {"X.509"}
};
...
static caddr_t
bif_key_name_arg (caddr_t * qst, state_slot_t ** args, int arg, const char *funcname)
{
...
static				/*xenc_key_create_cert */
    caddr_t
bif_xenc_key_create_cert (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  ...
  long type = BOX_ELEMENTS (args) > 3 ? bif_long_arg (qst, args, 3, "xenc_key_create_cert") : CERT_TYPE_PEM_FORMAT;
  xenc_key_t *k;
  ...
  if (cert_type_idx != xenc_cert_X509_idx)
    sqlr_new_error ("42000", "XENC34", "%s certificates are still not supported", xenc_cert_types[cert_type_idx].xcert_name);
...
static				/* xenc_key_remove */
    caddr_t
bif_xenc_key_remove (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_key_exists (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
int
__xenc_key_dsa_init (char *name, int lock, int num)
{
  DSA *dsa;
  /*int num=512; */
  ...
  if (dsa == NULL)
    {
      sqlr_new_error ("42000", "XENC11", "DSA parameters generation error");
    }
  if (!DSA_generate_key (dsa))
    {
      sqlr_new_error ("42000", "XENC12", "Can't generate the DSA private key");
    }
  pkey->xek_dsa = dsa;
...
int
__xenc_key_dh_init (char *name, int lock)
{
  DH *dh;
  int num = 512, g = 2;
  xenc_key_t *pkey = xenc_get_key_by_name (name, lock);
...
#if 0
static caddr_t
bif_xenc_dsa_sha1_sign (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  caddr_t name = bif_string_arg (qst, args, 0, "xenc_key_DSA_create");
  xenc_key_t *pkey = xenc_get_key_by_name (name, 1);
  if (NULL == pkey)
...
static caddr_t
bif_xenc_dsa_sha1_verify (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  caddr_t name = bif_string_arg (qst, args, 0, "xenc_key_DSA_create");
  xenc_key_t *pkey = xenc_get_key_by_name (name, 1);
  if (NULL == pkey)
...
static int
__xenc_key_3des_init (char *name, char *pwd, int lock)
{
  char _key[KEYSIZB + 1];
  des_cblock key[3];
...
/*  RAND_pseudo_bytes(pkey->ki.triple_des.salt, PKCS5_SALT_LEN); - nosalt */
  ...
  EVP_BytesToKey (EVP_des_ede3_cbc (), EVP_md5 (), NULL /*pkey->ki.triple_des.salt - nosalt */ ,
      (unsigned char *) _key, strlen (_key), 1, (unsigned char *) &key[0], pkey->ki.triple_des.iv);
...
void
xenc_key_3des_init (xenc_key_t * pkey, unsigned char *k1, unsigned char *k2, unsigned char *k3)
{
...
static caddr_t
bif_xenc_key_3des_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_key_3des_rand_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  ...
  if ((des_set_key_checked (&k1, ks1) < 0) || (des_set_key_checked (&k2, ks2) < 0) || (des_set_key_checked (&k3, ks3) < 0))
...
static caddr_t
bif_xenc_key_3des_read (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  ...
  EVP_BytesToKey (EVP_des_ede3_cbc (), EVP_md5 (),
      NULL, (unsigned char *) key_base64, 24, 1, (unsigned char *) _key, k->ki.triple_des.iv);
...
{
  static const char *me = "xenc_key_RSA_construct";
...
static caddr_t
bif_xenc_key_raw_read (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_key_raw_rand_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_rand_bytes (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
#ifdef DEBUG
static caddr_t
bif_xenc_key_3des_test_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
  ...
  if (DV_TYPE_OF (name) == DV_STRING)
    ...
    sqlr_new_error ("....", "....", "type of key name must be either string or NULL not %s", dv_type_title (DV_TYPE_OF (name)));
...
static caddr_t
bif_xenc_key_serialize (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_x509_cert_serialize (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
#ifdef AES_ENC_ENABLE
xenc_key_t *
xenc_key_aes_create (const char *name, int keylen, const char *pwd)
{
  char _key[KEYSIZB + 1];
  xenc_key_t *k;
  const char *algoname;
  ...
  switch (keylen)
    ...
    default:
      ...
      algoname = NULL;
      cipher = NULL;		/* To keep gcc 4.0 happy */
    }
  ...
  EVP_BytesToKey (cipher, EVP_md5 (), NULL, (unsigned char *) _key, strlen (_key), 1, (unsigned char *) k->ki.aes.k, k->ki.aes.iv);

  return k;
}

static caddr_t
bif_xenc_key_aes_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
static caddr_t
bif_xenc_key_aes_rand_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
void krb_init_ctx (char *service_name, gss_ctx_id_t * context, caddr_t * tkt);

/* can throw an error!!! */
xenc_key_t *
xenc_key_kerberos_create (const char *name, const char *service)
{
  ...
  mutex_enter (xenc_keys_mtx);
  k = xenc_key_create (name, XENC_TRIPLEDES_ALGO, DSIG_HMAC_SHA1_ALGO, 0);
  if (!k)
    {
      mutex_leave (xenc_keys_mtx);
      return 0;
...
static caddr_t
bif_xenc_key_kerberos_create (caddr_t * qst, caddr_t * err_r, state_slot_t ** args)
{
...
xenc_id_t
xenc_encode_by_key (xenc_key_t * key, dk_session_t * ses, long seslen,
    ptrlong type_idx, dk_session_t * oses, xenc_key_inst_t * superkey, xenc_try_block_t * t, wsse_ser_ctx_t * sctx)
{
  char buf[1024];
...
caddr_t *
xenc_generate_enc_texts (xenc_key_inst_t * keyins, ptrlong type_idx,
    xml_tree_ent_t ** ents, id_hash_t * nss, caddr_t * err_ret, wsse_ser_ctx_t * sctx)
{
  int inx;
  dk_session_t *ses = strses_allocate ();
  ...
  XENC_TRY (&t)
  {
	...
	{
	  if ((DV_TYPE_OF (ent->xte_current) == DV_ARRAY_OF_POINTER) && (BOX_ELEMENTS (ent->xte_current) > 1))
	    {
	      int inx;
  ...
  XENC_CATCH
  {
    ...
    if (err_ret)
      err_ret[0] = box_dv_short_string (buf);
    err = 1;
  }
  XENC_TRY_END (&t);
...
*/
int
xenc_check_ents_encryptability (xml_tree_ent_t ** ents, ptrlong type_idx)
{
  int inx;
...
static void
xpath_keyinst_free (xpath_keyinst_t * xpkei)
{
...
xml_tree_ent_t *
xenc_get_entity_arg (query_instance_t * qi, state_slot_t ** args, int inx, char *func, id_hash_t ** _nss)
{
...
bif_xmlenc_encrypt (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  qr = sql_compile ("select xpath_eval (? , ?, 0)", qi->qi_client, (caddr_t *) & err, SQLC_DEFAULT);
  if (err)
    {
#ifdef DEBUG
  ...
  while (xpath_arg_pointer < BOX_ELEMENTS_INT (args) - 2)
    {
      ...
      err = (caddr_t *) qr_rec_exec (qr, qi->qi_client, &lc, qi, NULL, 2, ":0", xpath_expr, QRP_STR, ":1", doc, QRP_RAW);
  ...
  security_tags =
      xenc_generate_security_tags ((query_instance_t *) qst, xp_keys_arr, dsig, generate_ref_list, &err_ret_sec_tags, &sctx);
  if (err_ret_sec_tags)
    {
...
bif_xml_sign (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (elem)
	...
	if (child == elem)
	  {
	    dk_free_box ((caddr_t) child);
	    ((caddr_t **) top)[inx] = new_elem;
	  }
      }
      END_DO_BOX;
      xenc_set_namespaces (new_elem, (caddr_t *) box_copy_tree (curr_nss), _nss);
    }
  else
    {
...
void
dsig_fullnames_free (dk_set_t names)
{
...
/* zero means error */
int
dsig_add_reference_1 (dsig_signature_t * dsig, const char *uri, const char *name, caddr_t * curr)
{
...
/* zero means error */
int
dsig_add_reference (dsig_signature_t * dsig, caddr_t * curr, dk_set_t names, char **error_tag)
{
...
id_hash_t *
nss_allocate (caddr_t * curr, const char *prefix, const char *uri,
    const char *pr2, const char *uri2, const char *pr3, const char *uri3)
{
...
*/
caddr_t
bif_dsig_template_ext (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  xs->xs_doc = dsig_xte;
  xs->xs_namespaces = nss_allocate (dsig_xte->xte_current, "ds", DSIG_URI, "wsse", WSSE_URI (&sctx), "wsu", WSU_URI (&sctx));
  ret_text = xml_doc_subst (xs);
  dk_free_tree ((box_t) ret);
...
caddr_t *
fuse_arrays (caddr_t ** parr, caddr_t * arr2, dtp_t dtp)
{
...
static void
xenc_security_token_id_format (char *buf, int maxlen, xenc_id_t id, int is_ref)
{
  if (maxlen < 250)
    {
...
void
xenc_write_key_info_tag (dk_session_t * ses, const char *name)
{
...
xenc_tag_t *
xenc_tag_create (const char *uri, const char *name)
{
...
void
xenc_tag_add_att (xenc_tag_t * t, const char *name, char *val)
{
...
void
xenc_tag_add_att_ns (xenc_tag_t * t, const char *ns, const char *name, const char *val)
{
...
xenc_tag_t *
xenc_tag_add_child (xenc_tag_t * t, caddr_t * child)
{
#ifdef DEBUG
...
caddr_t *
xenc_tag_finalize (xenc_tag_t * t)
{
...
xenc_tag_t *
xenc_tag_add_child_BN (xenc_tag_t * tag, BIGNUM * bn)
{
...
caddr_t **
xenc_generate_ext_info (xenc_key_t * key)
{
  dk_set_t l = 0;
  caddr_t **array;
...
#if 0
void
xenc_serialize_key (query_instance_t * qi, xenc_key_t * key, dk_session_t * ses)
{
...
caddr_t
certificate_encode (BIO * b, const char *encoding_type)
{
...
decode_box (caddr_t encoded_cert, const char *encoding_type)
{
  if (!encoded_cert)
    return 0;
  if (!strcmp (encoding_type, WSSE_BASE64_ENCODING_TYPE) || !strcmp (encoding_type, WSSE_OASIS_BASE64_ENCODING_TYPE))
    {
...
xenc_key_t *
certificate_decode (caddr_t encoded_cert, const char *value_type, const char *encoding_type)
{
...
void
xenc_generate_certificate_tag (query_instance_t * qi, xenc_key_t * k, dk_set_t * l, xenc_id_t * ids, wsse_ser_ctx_t * sctx)
{
  if (k)
    {
      caddr_t encoded_cert = 0;
...
caddr_t
xenc_generate_encrypted_key_tag (query_instance_t * qi, xenc_key_inst_t * kei, xenc_key_inst_t * superi, caddr_t * err_ret,
    wsse_ser_ctx_t * sctx)
{
...
caddr_t *
xenc_generate_ref_list (query_instance_t * qi, xenc_id_t * ids)
{
  xenc_tag_t *reflist = xenc_tag_create (XENC_URI, ":ReferenceList");
  int inx;
  caddr_t *ret;
  DO_BOX (xenc_id_t, id, inx, ids)
  {
    ...
    memset (id_str, 0, 200);
    stpcpy (id_str, "#Id-");
    uuid_unparse ((const unsigned char *) ((uuid_t *) id), id_str + strlen (id_str));
...
void
xenc_generate_key_taglist (query_instance_t * qi, xenc_key_inst_t * xki, dk_set_t * tags,
...
xenc_generate_security_tags (query_instance_t * qi, xpath_keyinst_t ** arr,
    dsig_signature_t * dsig, int generate_ref_list, caddr_t * err_ret, wsse_ser_ctx_t * sctx)
{
  int inx;
  dk_set_t l = 0;
  xenc_tag_t *security;
...
caddr_t *
xenc_sel_tree_get (caddr_t * tree, xenc_reference_t ref, id_hash_t * id_cache)
{
...
xenc_err_code_t
xenc_id_repl_text_get (xenc_enc_key_t * ek, xenc_replace_t * repl, xenc_err_code_t * c, char **err)
{
  ...
  if (!val)
	...
	c[0] = cc;
      if (err)
	err[0] = box_dv_short_string ("EncryptedData without data");
      return cc;
    }
...
xenc_err_code_t
xenc_enc_key_check (xenc_enc_key_t * ek, xenc_err_code_t * c, char **err)
{
  xenc_key_t *skey;
  xenc_err_code_t cc = 0;
  ...
  if (!ek->xeke_super_key)
	...
	c[0] = cc;
      if (err)
	err[0] = box_dv_short_string ("not encrypted keys are not allowed");
      return cc;
    }
#endif
  if (!ek->xeke_super_key)
  ...
  if (!skey)
	...
	c[0] = cc;
      if (err)
	err[0] = box_dv_short_string (ek->xeke_super_key);
    }
#if 0
...
xenc_err_code_t
xenc_decrypt_key (query_instance_t * qi, caddr_t enc, lang_handler_t * lh,
    xenc_enc_key_t * enc_key, id_hash_t * id_cache, xenc_err_code_t * c, char **err)
{
  xenc_key_t *key = 0;
  dk_session_t *in, *out;
  xenc_try_block_t t;
  ...
  if (!algo)
	...
	c[0] = cc;
      if (err)
	err[0] = box_dv_short_string (err_str);
      return cc;
    }
  ...
  if (is_unenc || enc_key->xeke_is_raw)	/* no super key */
	...
	{
	  if (c)
	    c[0] = XENC_UNKNOWN_KEY_ERR;
	  if (err)
	    err[0] = box_dv_short_string ("NULL");
      ...
      if (!enc_key->xeke_encrypted_key)
	{
	  if (c)
	    c[0] = XENC_UNKNOWN_KEY_ERR;
	  if (err)
	    err[0] = box_dv_short_string (enc_key->xeke_name);
	  return XENC_UNKNOWN_KEY_ERR;
	}
      if (0 != strcmp (enc_key->xeke_encrypted_key->xek_enc_algo->xea_ns, algo))
	{
	  if (c)
	    c[0] = XENC_DIFF_KEYS_ALGO_ERR;
	  if (err)
	    err[0] = box_dv_short_string (enc_key->xeke_name);
...
void
xenc_repls_free (dk_set_t repls)
{
...
void
xenc_build_ids_hash (caddr_t * curr, id_hash_t ** id_hash, int only_encrypted_data)
{
  int inx;
  if (DV_TYPE_OF (curr) != DV_ARRAY_OF_POINTER)
    return;
  ...
  if (!only_encrypted_data || !strcmp ((((caddr_t **) (curr))[0][0]), XENC_URI ":EncryptedData"))
      ...
      if (Id)
	{
	  char idbuf[128];
	  if (!id_hash[0])
	    id_hash[0] = id_hash_allocate (31, sizeof (caddr_t), sizeof (caddr_t *), strhash, strhashcmp);
...
void
xenc_ids_hash_free (id_hash_t * ids)
{
  id_hash_iterator_t hit;
  char **id;
  caddr_t **curr;

  for (id_hash_iterator (&hit, ids); hit_next (&hit, (char **) &id, (char **) &curr);
      /* */ )
    {
      if (id)
	dk_free_box (id[0]);
...
xenc_err_code_t
xenc_decrypt_xml (query_instance_t * qi, xenc_dec_t * enc,
    caddr_t in_xml, caddr_t encode, lang_handler_t * lh, dk_session_t * out_xml, xenc_err_code_t * c, char **err)
{
  id_hash_t *nss = 0;
  xml_tree_ent_t *doc;
  xenc_err_code_t cc = 0;
  ...
  if (repls)
    {
      int inx = 0;
      subst_items = (subst_item_t *) dk_alloc_box (sizeof (subst_item_t) * dk_set_length (repls), DV_ARRAY_OF_POINTER);
  ...
  FAILED
  {
    ...
    if (c)
      c[0] = cc;
    if (err)
      err[0] = box_dv_short_string ("in xenc_decrypt_xml internal error");
    goto failed;
  }
...
*/
xenc_err_code_t
dsig_signature_serialize (query_instance_t * qi, dsig_signature_t * dsig,
    dk_session_t * xml_doc, long xml_ses_len, caddr_t ** out_tag, xenc_err_code_t * code, char **err, wsse_ser_ctx_t * sctx)
{
  xenc_err_code_t c;
...
caddr_t *
dsig_ref_tag_create (dsig_reference_t * ref)
{
...
caddr_t *
dsig_signinfo_tag_create (dsig_signature_t * dsig)
{
...
caddr_t *
dsig_signval_tag (dsig_signature_t * dsig)
{
...
void
dsig_generate_signedinfo (query_instance_t * qst, dsig_signature_t * dsig, dk_session_t * ses, xenc_try_block_t * t)
{
...
caddr_t *
signature_serialize_1 (dsig_signature_t * dsig, wsse_ser_ctx_t * sctx)
{
...
dsig_signature_t *
dsig_template_xpath (char *signature_method, char *xpath_sel)
{
  NEW_VAR (dsig_signature_t, dsig);
  NEW_VAR (dsig_transform_t, tr);
  NEW_VARZ (dsig_reference_t, ref);
...
xenc_err_code_t
dsig_initialize (query_instance_t * qi, dk_session_t * xml_doc, long xml_ses_len,
    dsig_signature_t * dsig, xenc_err_code_t * c, char **err)
{
  ...
  XENC_TRY (&t)
  {
	...
	if (!(tr_func) (qi, ses_in, strses_length (ses_in)
		...
		ses_out, tr->dst_data))
	  {
		...
		xenc_report_error (&t, 500 + strlen (tr->dst_name),
		    XENC_ALGO_ERR, "Transform error at %s [%s]", tr->dst_name, tr->dst_data);
	      }
	    else
	      xenc_report_error (&t, 500 + strlen (tr->dst_name), XENC_ALGO_ERR, "Transform error at %s", tr->dst_name);
	  }
	ses = ses_out;
	ses_out = ses_in;
	ses_in = ses;
	...
	xenc_report_error (&t, 500 + strlen (ref->dsr_digest_method), XENC_ALGO_ERR, "Digest error at %s", ref->dsr_digest_method);
    }
    END_DO_SET ();
...
int
base64_strcmp (char *_s1, char *_s2)
{
...
*/
int
dsig_compare (dsig_signature_t * d1, dsig_signature_t * d2, dsig_compare_t * cmp)
{
...
dsig_signature_t *
dsig_copy_draft (dsig_signature_t * d1)
{
...
void
xenc_make_error (char *buf, long maxlen, xenc_err_code_t c, const char *err)
{
  char *buf_ptr = buf;
  int len;
  memset (buf, 0, maxlen);
...
void
xenc_report_error (xenc_try_block_t * t, long buflen, xenc_err_code_t c, const char *errbuf, ...)
{
  if (t)
    {
      int res;
...
caddr_t
bif_dsig_a_test (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  caddr_t *dsig_tag;
  wsse_ser_ctx_t sctx;
  NEW_VAR (dsig_signature_t, dsig);
...
caddr_t
bif_dsig_b_test (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  caddr_t *dsig_tag;
  caddr_t xml_text;
...
xmlenc_base64_test ()
{
  ...
  XENC_TRY (&t)
  {
    ...
    strses_flush (ses_in);
    xenc_base64_decryptor (ses_out, strses_length (ses_out), ses_in, xenc_get_key_by_name ("virtdev@localhost", 1), &t);
  }
  XENC_CATCH
  {
    xenc_assert (0);
...
xmlenc_des3_test ()
{
  ...
  XENC_TRY (&t)
  {
    ...
    xenc_des3_decryptor (ses_out, strses_length (ses_out), ses_in, xenc_get_key_by_name ("virtdev3@localhost", 1), &t);
  }
  XENC_CATCH
  {
    xenc_assert (0);
  ...
  XENC_TRY (&t)
  {
    ...
    xenc_des3_encryptor (ses_in, ses_in->dks_in_fill, ses_out, xenc_get_key_by_name ("virtdev3@localhost", 1), &t);
    {
...
xmlenc_rsa_test ()
{
  ...
  XENC_TRY (&t)
  {
    ...
    xenc_rsa_decryptor (ses_out, strses_length (ses_out), ses_in, xenc_get_key_by_name ("virtdev5@localhost", 1), &t);
  }
  XENC_CATCH
  {
    xenc_assert (0);
...
caddr_t
bif_xenc_test (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
xenc_kt_test ()
{
  ...
  XENC_TRY (&t)
  {
    ...
    if (memcmp (new_key->ki.triple_des.k1, key->ki.triple_des.k1, sizeof (des_cblock)))
      xenc_assert (0);
    if (memcmp (new_key->ki.triple_des.k2, key->ki.triple_des.k2, sizeof (des_cblock)))
      xenc_assert (0);
    if (memcmp (new_key->ki.triple_des.k3, key->ki.triple_des.k3, sizeof (des_cblock)))
      xenc_assert (0);
...
caddr_t
xenc_x509_get_key_identifier (X509 * cert)
{
...
{
  const char *me = "xenc_rsa_sha1_digest";
...
{
  const char *me = "xenc_dsig_signature";
...
{
  const char *me = "xenc_dsig_verify";
  caddr_t text = bif_string_arg (qst, args, 0, me);
  caddr_t name = bif_string_arg (qst, args, 1, me);
  caddr_t signature_method = bif_string_arg (qst, args, 2, me);
...
static int
x509_add_ext (X509 * cert, int nid, char *value)
{
  X509_EXTENSION *ex;
  X509V3_CTX ctx;
...
#if 0
static void
x509_add_custom_bin (X509 * x, ccaddr_t n, ccaddr_t v)
{
  ...
  if (!os)
    goto err;
  ASN1_OCTET_STRING_set (os, v, box_length (v) - 1);
  if (!(obj = OBJ_txt2obj (n, 1)))
    goto err;
  if (!X509_EXTENSION_create_by_OBJ (&ext, obj, 0 /* not critical */ , os))
    goto err;
...
{
  static const char *ssh_header = "\x00\x00\x00\x07ssh-rsa";
  caddr_t ret;
  int len, pos;
  caddr_t n = BN2binbox (x->n);	/* modulus */
  caddr_t e = BN2binbox (x->e);	/* public exponent */
...
{
  const char *me = "x509_verify_array";
  caddr_t cert_name = bif_string_arg (qst, args, 0, me);
...
{
  const char *me = "x509_cert_verify_array";
...
{
  const char *me = "x509_ca_cert_add";
  caddr_t cert_text = bif_string_arg (qst, args, 0, me);
  char err_buf[512];
...
{
  const char *me = "x509_ca_certs_remove";
...
{
  const char *me = "x509_ca_certs_list";
...
bif_xmlenc_init ()
{
  ...
  __xenc_keys = id_hash_allocate (231, sizeof (caddr_t), sizeof (caddr_t), strhash, strhashcmp);
  __xenc_certificates = id_hash_allocate (231, sizeof (caddr_t), sizeof (caddr_t), strhash, strhashcmp);

  algo_stores_init ();
  dsig_sec_init ();

  /* test keys */
  xenc_algorithms_create (XENC_DES3_ALGO, "tripledes encoding algorithm", xenc_des3_encryptor, xenc_des3_decryptor, DSIG_KEY_3DES);

  xenc_algorithms_create (DSIG_DSA_SHA1_ALGO, "dsa sha1 algorithm", xenc_signature_wrapper, xenc_signature_wrapper_1, DSIG_KEY_DSA);

  xenc_algorithms_create (DSIG_RSA_SHA1_ALGO, "rsa sha1 algorithm", xenc_signature_wrapper, xenc_signature_wrapper_1, DSIG_KEY_RSA);

  xenc_algorithms_create (XENC_BASE64_ALGO, "base64 encoding algorithm",
      xenc_base64_encryptor, xenc_base64_decryptor, DSIG_KEY_RAW);

  xenc_algorithms_create (XENC_RSA_ALGO, "rsa encoding algorithm", xenc_rsa_encryptor, xenc_rsa_decryptor, DSIG_KEY_RSA);
  xenc_algorithms_create (XENC_DSA_ALGO, "dsa encoding algorithm", xenc_dsa_encryptor, xenc_dsa_decryptor, DSIG_KEY_DSA);

#ifdef AES_ENC_ENABLE
  xenc_algorithms_create (XENC_AES128_ALGO, "aes 128 cbc encoding algorithm", xenc_aes_encryptor, xenc_aes_decryptor, DSIG_KEY_AES);
  xenc_algorithms_create (XENC_AES192_ALGO, "aes 192 cbc encoding algorithm", xenc_aes_encryptor, xenc_aes_decryptor, DSIG_KEY_AES);
  xenc_algorithms_create (XENC_AES256_ALGO, "aes 256 cbc encoding algorithm", xenc_aes_encryptor, xenc_aes_decryptor, DSIG_KEY_AES);
#endif

  xenc_algorithms_create (XENC_DH_ALGO, "dh encoding algorithm", xenc_dh_encryptor, xenc_dh_decryptor, DSIG_KEY_DH);

  xenc_algorithms_create (DSIG_DH_SHA1_ALGO, "dsa sha1 algorithm", xenc_signature_wrapper, xenc_signature_wrapper_1, DSIG_KEY_DH);
  ...
  xenc_cert_X509_idx = ecm_find_name ("X.509", (void *) xenc_cert_types, xenc_cert_types_len, sizeof (xenc_cert_type_t));
  xenc_cert_KERB5TGT_idx = ecm_find_name ("Kerberosv5TGT", (void *) xenc_cert_types, xenc_cert_types_len,
      sizeof (xenc_cert_type_t));
  xenc_cert_KERB5ST_idx = ecm_find_name ("Kerberosv5ST", (void *) xenc_cert_types, xenc_cert_types_len, sizeof (xenc_cert_type_t));
...
#ifdef DEBUG
void
print_hash (id_hash_t * h)
{
  id_hash_iterator_t iter;
  caddr_t *tag;
  caddr_t *nss;
  for (id_hash_iterator (&iter, h); hit_next (&iter, (caddr_t *) & tag, (caddr_t *) & nss);
      /* */ )
    {
      printf ("*********************\n");
