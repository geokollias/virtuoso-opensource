...
bif_date_arg_rb_type (caddr_t * qst, state_slot_t ** args, int nth, char *func, int *rb_type_ret)
{
  ...
  if ((DV_RDF == DV_TYPE_OF (src_rdf_box))
	  ...
	  || (rb_type__xsd_gYear == src_rdf_box->rb_type) || (rb_type__xsd_gYearMonth == src_rdf_box->rb_type)))
...
{
  if (rb_type__xsd_gDay == rb_type)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y | DT_PRINT_MODE_NO_M;
  if (rb_type__xsd_gMonth == rb_type)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y | DT_PRINT_MODE_NO_D;
  if (rb_type__xsd_gMonthDay == rb_type)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y;
  if (rb_type__xsd_gYear == rb_type)
...
{
  if (uname_xmlschema_ns_uri_hash_gDay == xsd_type_uname)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y | DT_PRINT_MODE_NO_M;
  if (uname_xmlschema_ns_uri_hash_gMonth == xsd_type_uname)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y | DT_PRINT_MODE_NO_D;
  if (uname_xmlschema_ns_uri_hash_gMonthDay == xsd_type_uname)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_Y;
  if (uname_xmlschema_ns_uri_hash_gYear == xsd_type_uname)
    return DT_PRINT_MODE_YMD | DT_PRINT_MODE_NO_M | DT_PRINT_MODE_NO_D;
...
dt_scan_from_buffer (const char *buf, int mode, caddr_t * dt_ret, const char **err_msg_ret)
{
  ...
  fld_len = 0;
  acc = 0;
  while (isdigit (tail[0]) && (4 > fld_len))
    {
      acc = acc * 10 + (tail++)[0] - '0';
      fld_len++;
    }
  if ('-' == tail[0])
      ...
      if (4 != fld_len)
	{
      ...
      fld_len = 0;
      acc = 0;
      while (isdigit (tail[0]) && (2 > fld_len))
	{
	  acc = acc * 10 + (tail++)[0] - '0';
	  fld_len++;
	}
...
bif_x_name (day, "%A") bif_x_name (month, "%B") caddr_t
...
bif_nasa_tjd_fraction (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  double f =
      (((DT_HOUR (arg) * (boxint) 60 + DT_MINUTE (arg)) * (boxint) 60 + DT_SECOND (arg)) * (boxint) 1000000 +
      DT_FRACTION (arg)) / (60 * 60 * 24 * 1000000.0);
  return box_double (f);
}
...
bif_dateadd (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DT_TYPE_DATE == dt_type && (0 == stricmp (part, "year") || 0 == stricmp (part, "month") || 0 == stricmp (part, "day")))
...
interval_odbc_to_text (ptrlong odbcInterval, char *func_name)
{
  ...
  switch (odbcInterval)
    ...
    case SQL_TSI_YEAR:
      ...
      break;
    default:
      sqlr_new_error ("22015", "DT003", "Interval not supported in %s: %ld", func_name, (long) odbcInterval);
    }
  return szpart;
}
...
generic_err:
  err_ret[0] =
      srv_make_new_error ("22003", "SR087",
      "Wrong arguments for datetime arithmetic, can not add values of type %d (%s) and type %d (%s).", dtp1, dv_type_title (dtp1),
      dtp2, dv_type_title (dtp2));
  return NULL;
}
...
generic_err:
  err_ret[0] =
      srv_make_new_error ("22003", "SR087",
      "Wrong arguments for datetime arithmetic, can not subtract value of type %d (%s) from value type %d (%s).", dtp2,
      dv_type_title (dtp2), dtp1, dv_type_title (dtp1));
  return NULL;
}
...
bif_date_init ()
{
  ...
  bif_define_ex ("now", bif_timestamp, BMD_ALIAS, "getdate", BMD_ALIAS, "rdf_now_impl", BMD_ALIAS, "get_timestamp", BMD_ALIAS, "current_timestamp", BMD_RET_TYPE, &bt_timestamp, /*BMD_IS_PURE, */ BMD_DONE);	/* This is standard name */
