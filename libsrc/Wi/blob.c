...
static int wide_blob_buffered_read (void *ses_from, unsigned char *to, int req_chars, blob_state_t * state, int *page_end,
    int *preadchars, get_data_generic_proc get_data_proc);

static int
bh_fill_page_generic (blob_handle_t * bh,
    wcharset_t * charset,
    blob_cvt_type_t in,
    blob_cvt_type_t out,
    unsigned char *buffer,
    int maxin, int maxout, int *nin, int *noutbytes, int *noutchars, get_data_generic_proc get_data_proc, void *proc_arg, int pos)
{
  assert (vWIDE != out);
  if (0 == maxin)
    {
	...
	switch (out)
	  ...
	  case vNARROW:
	    {
	      int len = 0;
	      if ((NULL == charset) ||
		  (NULL == default_charset) || (charset == default_charset) || (vBINARY == out) || (vBINARY == in))
		{
	  ...
	  case vUTF:
	      ...
	      while (maxout >= VIRT_MB_CUR_MAX && maxin > 0)
		{
		  ...
		  len =
		      (int) cli_narrow_to_utf8 ((vBINARY == out) ? NULL : charset, &cin, 1, (unsigned char *) (buffer + *noutbytes),
		      VIRT_MB_CUR_MAX);
		  assert (len > 0);
		  maxout -= len;
		  (*noutbytes) += len;
	  ...
	  case vUTF:
	    {
	      ...
	      ret = wide_blob_buffered_read (proc_arg, buffer, ilen, &bh->bh_state, &page_end, &len, get_data_proc);
	      if (ret < 1 && maxin < VIRT_MB_CUR_MAX)
		{
		  log_error ("Incomplete UTF-8 char found in filling up a BLOB." " The wide blob data may be garbaled.");
		  memset (buffer, '?', ilen);
		  ret = ilen;
		}
	      *nin = ret;
...
#ifdef BL_DEBUG
void
dbg_blob_layout_free (const char *file, int line, blob_layout_t * bl)
#else
void
...
{
#ifdef DEBUG
  if (DV_CUSTOM != DV_TYPE_OF (bl) || sizeof (blob_layout_t) != box_length (bl))
...
static void bh_fill_pagedir_buffer (blob_handle_t * bh, buffer_desc_t * buf, it_cursor_t * itc_from, int *at_end,
    size_t * position);
static long bh_fill_data_buffer (blob_handle_t * bh, buffer_desc_t * buf, it_cursor_t * itc_from, int *status_ret,
    size_t * data_len_in_bytes);
...
cli_end_blob_read (client_connection_t * cli)
{
  ...
  if (cli_is_interactive (cli) && !cli->cli_ws)
      ...
      if (cli->cli_session)
	{
	  ...
	  is_burst = !cli->cli_session->dks_fixed_thread && cli->cli_session->dks_thread_state == DKST_BURST ? 1 : 0;
...
{
  int is_burst;
  caddr_t *req = (caddr_t *) dk_alloc_box (2 * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
  mutex_enter (thread_mtx);
  is_burst = !cli->cli_session->dks_fixed_thread && cli->cli_session->dks_thread_state == DKST_BURST ? 1 : 0;
  mutex_leave (thread_mtx);
  if (!is_burst)
...
int
page_wait_blob_access (it_cursor_t * itc, dp_addr_t dp_to, buffer_desc_t ** buf_ret, int mode, blob_handle_t * bh,
    int itc_in_map_wrap)
{
  short flag;
  if (itc_in_map_wrap)
  ...
  if (PF_OF_DELETED == buf_ret[0])
    {
      log_info ("Attempt to read deleted blob dp = %lu start = %lu.", (unsigned long) dp_to,
	  (unsigned long) ((NULL != bh) ? bh->bh_page : 0));
      goto errexit;
    }
  if (NULL == buf_ret[0])
    {
  ...
  if ((DPF_BLOB != flag) && (DPF_BLOB_DIR != flag))
	...
	GPF_T1 ("page_wait_blob_access with PA_WRITE and non-BLOB buffer");
      log_info ("Attempt to read (deleted?) blob dp = %lu start = %lu: non-blob page found", (unsigned long) dp_to,
	  (unsigned long) ((NULL != bh) ? bh->bh_page : 0));
  ...
  if ((*buf_ret)->bd_page != (*buf_ret)->bd_physical_page)
    {
      ...
      log_error ("a blob page cannot have different bd_page and bd_physical_page L=%d P=%d", (*buf_ret)->bd_page,
	  (*buf_ret)->bd_physical_page);
      goto errexit;
    }
#if 0
  if (rand () < (RAND_MAX >> 8))
  ...
  if (bh->bh_state.bufpos < bh->bh_state.buflen)
    {
	  ...
	  bh->bh_state.bufpos <= PAGE_DATA_SZ && bh->bh_state.bufpos <= bh->bh_state.buflen && len <= PAGE_DATA_SZ);
      len -= l;
      copybytes += l;
    }
...
long
bh_get_data_from_user (blob_handle_t * bh, client_connection_t * cli, db_buf_t to, int max_bytes)
{
	...
	if (bh->bh_bytes_coming)
	  {
	    ...
	    page_end = bh_fill_page_generic (bh,
		cli->cli_charset,
		flin, flout, to + n_in, bh->bh_bytes_coming, to_go, &nin, &nout, &ncout, get_data_from_client, ses_from, 0);
  ...
  if (NULL != bh->bh_pages)
    {
      ...
      memcpy (buf->bd_buffer + DP_DATA, ((char *) bh->bh_pages) + (*position), copybytes);
      (*position) += copybytes;
...
bh_fill_data_buffer (blob_handle_t * bh, buffer_desc_t * buf, it_cursor_t * itc_from, int *status_ret, size_t * data_len_in_bytes)
{
  ...
  if (bh->bh_string)
	...
	default:
	  ...
	  flin,
	  flout, buf->bd_buffer + DP_DATA, len - bh->bh_position, PAGE_DATA_SZ, &nin, &nout, &ncout, proc, arg, bh->bh_position);
      bh->bh_position += nin;
  ...
  if (bh->bh_current_page && !BH_FROM_CLUSTER (bh))
	...
	default:
	  ...
	  (int) (bh->bh_diskbytes - bh->bh_position), PAGE_DATA_SZ, &nin, &nout, &ncout, get_data_from_blob, &bl, 0);
      bh->bh_position += nin;
  ...
  if (bh->bh_ask_from_client || BH_FROM_CLUSTER (bh))
      ...
      if ((BLOB_ALL_RECEIVED == bh->bh_all_received) || (BLOB_NULL_RECEIVED == bh->bh_all_received))
	{
      ...
      *data_len_in_bytes = n_in;
	  ...
	  (long) wide_char_length_of_utf8_string (buf->bd_buffer + DP_DATA, n_in) : n_in);
    }
...
blob_schedule_delayed_delete (it_cursor_t * itc, blob_layout_t * bl, int add_jobs)
{
  ...
  if (0 && itc->itc_ltrx->lt_is_excl)	/* no immediate delete in atomic mode */
    ...
    /* if (REPL_NO_LOG != itc->itc_ltrx->lt_replicate) GK: nothing to do w/ the log mode */
    {
      ...
      if ((main_lt->lt_has_branches || IS_MT_BRANCH (main_lt) || cl_run_local_only == CL_RUN_CLUSTER)
	  && main_lt->lt_status == LT_PENDING)
	{
	  IN_TXN;
	  in_mtx = 1;
	}
    ...
    oid_t col_id, char *table_name)
{
  ...
  if (REPL_NO_LOG == it->itc_ltrx->lt_replicate || cl_non_logged_write_mode)
  ...
  else
    {
      ...
      it->itc_ltrx->lt_blob_log = dk_set_conc (it->itc_ltrx->lt_blob_log, dk_set_cons ((caddr_t) blob_log, NULL));
    }
}
...
static int
blob_new_page (it_cursor_t * row_itc, buffer_desc_t ** blob_buf, int page_flag)
{
  *blob_buf = it_new_page (row_itc->itc_tree, row_itc->itc_page, page_flag, 0, 0);
  if (!*blob_buf)
    {
...
bl_from_dv_it (dtp_t * col, index_tree_t * it)
{
  ...
  slice_id_t slice;
  int inx, n_pages;
  blob_layout_t *bl = (blob_layout_t *) blob_layout_ctor (DV_BLOB_HANDLE_DTP_FOR_BLOB_DTP (*col),	/* bl_blob_handle_dtp */
...
bh_from_dv (dtp_t * col, it_cursor_t * itc)
{
  ...
  if (DV_COL_BLOB_SERIAL == dtp)
    dtp = DV_BLOB;
  bh = (blob_handle_t *) dk_alloc_box_zero (sizeof (blob_handle_t), DV_BLOB_HANDLE_DTP_FOR_BLOB_DTP (dtp));
...
bh_to_dv (blob_handle_t * bh, dtp_t * col, dtp_t dtp)
{
  ...
  for (inx = 0; inx < BL_DPS_ON_ROW; inx++)
    LONG_SET_NA (col + BL_DP + sizeof (dp_addr_t) * inx, inx < n_pages ? bh->bh_pages[inx] : 0);
  LONG_SET_NA (col + BL_PAGE_DIR, bh->bh_dir_page);
  LONG_SET_NA (col + BL_TS, bh->bh_timestamp);
}

static int
itc_set_xper_col (it_cursor_t * row_itc, db_buf_t col, xper_entity_t * data, dp_addr_t first_page,
    blob_layout_t ** replaced_version_ptr, blob_handle_t ** source_bh_ptr)
{
  int rc;
  ...
  if (XPD_NEW == cut->xe_doc.xpd->xpd_state
      /*&& cut_bh->bh_length > MAX_ROW_BYTES */ )
    {
      ...
      tmp_itc = itc_create (NULL, cut->xe_doc.xd->xd_qi->qi_trx);
...
itc_set_blob_col (it_cursor_t * row_itc, db_buf_t col,
    caddr_t data, blob_layout_t * replaced_version, int log_as_insert, sql_type_t * col_sqt)
{
...
maybe_rfwd:
  if (data && !IS_BOX_POINTER (data))
    {
      rc = LTE_SQL_ERROR;
      LT_ERROR_DETAIL_SET (row_itc->itc_ltrx, box_dv_short_string ("Invalid data type (integer) when making a blob"));
  ...
  switch (dtp)
	...
	LT_ERROR_DETAIL_SET (row_itc->itc_ltrx,
	    box_sprintf (60, "Invalid data type (%.30s) when making a blob", dv_type_title (dtp)));
	goto resource_cleanup;
      }
    }
  if (IS_BLOB_HANDLE_DTP (dtp))
...
bh_is_ready:
  ...
  ITC_FAIL (blob_itc)
  {
    ...
    if ((!row_is_temporary) && (dtp == DV_BLOB_HANDLE || dtp == DV_BLOB_WIDE_HANDLE
#ifdef BIF_XML
...
long
blob_write_crash_recover_log_1 (dk_session_t * log, blob_log_t * bl, dp_addr_t start, long should_be_len)
{
  dp_addr_t page;
  int inx = 0, bit = 0;
  uint32 *array = NULL;
  ...
  if (no_free_set || (0 != (array[inx] & (1 << bit)) && !gethash (DP_ADDR2VOID (start), storage->dbs_cpt_remap)))
    {
  ...
  while (dir_start)
      ...
      if (no_free_set || (0 != (array[inx] & (1 << bit)) && !gethash (DP_ADDR2VOID (start), storage->dbs_cpt_remap)))
	{
	  ...
	  if (WI_ERROR == buf_disk_read (buf))
		...
		{
		  log_error ("Non-blob-dir page %ld remap %ld in logging blob. %d", buf->bd_page, buf->bd_physical_page,
		      SHORT_REF (buf->bd_buffer + DP_FLAGS));
		  if (is_crash)
		    {
		      buf->bd_readers--;
		      is_crash++;
...
fin:
  if (is_crash > 1)
	...
	log_error ("Blob %s (table %s) is reduced to %ld length (should be %ld)",
	    col ? col->col_name : NULL, bl->bl_table_name, fulllen, bl->bl_diskbytes);
      }
    }
  CATCH_WRITE_FAIL (log) session_buffered_write_char (0, log);
  END_WRITE_FAIL (log);
  return 0;
      ...
      if (no_free_set || (0 != (array[inx] & (1 << bit)) && !gethash (DP_ADDR2VOID (start), storage->dbs_cpt_remap)))
	{
	  ...
	  if (WI_ERROR == buf_disk_read (buf))
	  ...
	  else
	    {
	      ...
	      if (DPF_BLOB != SHORT_REF (buf->bd_buffer + DP_FLAGS))
		{
		  log_error ("Non-blob page %ld remap %ld in logging blob. %d", buf->bd_page, buf->bd_physical_page,
		      SHORT_REF (buf->bd_buffer + DP_FLAGS));
		  if (is_crash)
		    {
		      buf->bd_readers--;
      ...
      if (bytes_on_page)
	{
	  if (!omit)
	      ...
	      if (DK_MEM_RESERVE)
		{
	      ...
	      memcpy (page_string, buf->bd_buffer + DP_DATA + from_byte, bytes_on_page);
...
#if 0
int
bh_fill_buffer_from_blob (index_space_t * isp, lock_trx_t * lt, blob_handle_t * bh, caddr_t outbuf, long get_bytes)
{
  ...
  while (start)
	...
	break;
      ...
      if (bytes_on_page)
	{
	  memcpy (ptr, buf->bd_buffer + DP_DATA + from_byte, bytes_on_page);
	  ptr += bytes_on_page;
	  bytes_filled += bytes_on_page;
	  get_bytes -= bytes_on_page;
	  from_byte += bytes_on_page;
...
bh_read_ahead (lock_trx_t * lt, blob_handle_t * bh, unsigned from, unsigned to)
{
  ...
  if (from != to)
      ...
      if (!bh->bh_page_dir_complete)
	{
      ...
      memcpy (pra->ra_dp, bh->bh_pages + pidx, pra->ra_fill * sizeof (dp_addr_t));
      TC (tc_blob_ra);
...
bl_check (blob_layout_t * bl)
{
  ...
  if (bl->bl_pages)
	...
	{
	  error = 1;
	  log_info ("Blob disk bytes and page dir length disagree L=%d  bytes= %d dir pages=" BOXINT_FMT, bl->bl_start, n,
	      bl->bl_diskbytes);
	}
      if (!dp && n > BL_DPS_ON_ROW)
	{
...
bh_write_out (lock_trx_t * lt, blob_handle_t * bh, dk_session_t * ses)
{
  ...
  it_cursor_t *tmp_itc;
  tmp_itc = itc_create (NULL, lt);
  if (bh->bh_page == bh->bh_current_page && bh->bh_diskbytes > 2 * PAGE_DATA_SZ)
    {
...
void
blob_send_bytes (lock_trx_t * lt, caddr_t bhp, long get_bytes, int send_position)
{
...
blob_subseq (lock_trx_t * lt, caddr_t bhp, size_t from, size_t to)
{
  ...
  if (!isWide)
	...
	{
	  sqlr_new_error ("22023", "SR099", "Reading deleted blob in subseq.");
	}
      if (pidx)
	{
	  if (bh->bh_pages)
...
blob_to_string_isp (lock_trx_t * lt, caddr_t bhp)
{
  ...
  if (bytes > 10000000)		/* if no such error further we will try to alloc larger box and so on, bad error message */
    sqlr_new_error ("22023", "SR102", "BLOB larger than 10mb cannot be converted into a string.");
  ...
  if (!BH_FROM_CLUSTER (bh))	/* do this if local blob */
      ...
      if (BLOB_FREE == bh_fetch_dir (lt, bh))
	{
	  ...
	  sqlr_new_error ("22023", "SR099", "Reading deleted blob in blob_to_string");
	}
      bh_read_ahead (lt, bh, 0, (unsigned) bh->bh_length);
    }

  string_list = bh_string_list (lt, bh, 10000000, 0);	/* up to 10MB as varchar */
  bh->bh_current_page = bh->bh_page;
  bh->bh_position = 0;

  if (!string_list || (BH_DIRTYREAD == string_list))
    sqlr_new_error ("22023", "SR469", "Dirty read is detected, wrong BLOB HANDLE value.");
...
static int
wide_blob_buffered_read (void *ses_from,
    unsigned char *to, int req_chars, blob_state_t * state, int *page_end, int *preadchars, get_data_generic_proc get_data_proc)
{
  ...
  while (req_chars)
      ...
      else
	{
	  ...
	  /*return -1; */
	  state->count = 0;
	  log_error ("Invalid UTF-8 char (%02X) read in filling up a BLOB. The wide blob data may be garbaled.", utf8_char);
	  utf8_char = '?';
	}
...
blob_fill_buffer_from_wide_string (caddr_t _bh, caddr_t _buf, int *at_end, long *char_len)
{
  ...
  long wchars = (box_length (bh->bh_string) / sizeof (wchar_t)) - bh->bh_position - 1;
  long utf8len;
