  ...
  for (ctr = 0; ctr < sub_gp_count; ctr++)
    {
	  ...
	  gp_in_cbk, gp_out_cbk, expn_in_cbk, expn_out_cbk, expn_subq_cbk, literal_cbk);
      if (retcode & SPAR_GPT_COMPLETED)
	return SPAR_GPT_COMPLETED;
    }
  if (sub_expn_count && ((NULL != expn_in_cbk) || (NULL != expn_out_cbk) || (NULL != expn_subq_cbk) || (NULL != literal_cbk)))
    {
...
void
sparp_gp_trav_resume (sparp_t * sparp)
{
...
sparp_continue_gp_trav_in_sub (sparp_t * sparp, SPART * subq_gp_wrapper, void *common_env, int trav_in_out_clauses)
{
  ...
  if (trav_in_out_clauses)
    sparp_trav_out_clauses (sparp, subq_gp_wrapper->_.gp.subquery, common_env, SPARP_GP_TRAV_CALLBACK_ARGS (stp[0]));
...
spar_macro_xlate_selid (sparp_t * sparp, caddr_t * selid_ptr, spar_mproc_ctx_t * ctx)
{
  ...
  selid_ptr[0] =
      t_box_sprintf (100, "%.70s%.20s%.10s", ctx->smpc_mcall->_.macrocall.mname, ctx->smpc_defbody_currselid, selid_ptr[0]);
}
...
param_index_found:
  ...
  if (SPAR_VARIABLE != SPART_TYPE (argtree))
    spar_error (sparp,
	"The argument #%d (?%.100s) of macro '%.100s' should be a plain variable, because it is used as an alias name",
...
spar_macroprocess_tree (sparp_t * sparp, SPART * tree, spar_mproc_ctx_t * ctx)
{
  ...
  switch (SPART_TYPE (tree))
	  ...
	  default:
	      ...
	      for (memb_ctr = 0; memb_ctr < memb_count; memb_ctr++)
		{
		  ...
		  switch (SPART_TYPE (memb))
			...
			if (defm->_.defmacro.subtype)
			  {
			    ...
			    if (defm->_.defmacro.subtype != context_graph_type)
			      spar_error (sparp,
				  "Macro <%.200s> should be used in context of %s graph but is placed into %s graph pattern", mname,
			...
			tree->_.gp.members = membs =
			    t_modify_spartlist (membs, memb_ctr, 1, ctx->smpc_ins_membs, BOX_ELEMENTS_0 (ctx->smpc_ins_membs));
			...
			tree->_.gp.members = membs =
			    t_modify_spartlist (membs, memb_ctr, 1, arg_copy->_.gp.members,
			    BOX_ELEMENTS_0 (arg_copy->_.gp.members));
			memb_count = BOX_ELEMENTS (membs);
			memb_ctr--;
	...
	if (NULL != ctx->smpc_context_gp)
	  {
	    ctx->smpc_context_gp->_.gp.members =
		t_modify_spartlist (context_membs, context_memb_count, 0, ctx->smpc_ins_membs,
		BOX_ELEMENTS_0 (ctx->smpc_ins_membs));
	    ctx->smpc_ins_membs = NULL;
	    if (NULL != ctx->smpc_ins_filts)
	      {
...
sparp_equiv_alloc (sparp_t * sparp)
{
  ...
  if (eqcount >= 0x10000)
	...
	spar_internal_error (sparp,
	    "The SPARQL optimizer has failed to process the query with reasonable quality. The resulting SQL query is abnormally long. Please paraphrase the SPARQL query.");
      else
	spar_error (sparp,
	    "The query is prohibitively inefficient if it should be executed on storage <%s>. Please optimize your mappings of relational data to RDF or paraphrase the SPARQL query.",
...
sparp_equiv_dbg_gp_print (sparp_t * sparp, SPART * tree)
{
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, tree, eq_ctr, eq)
  {
    ...
    spar_dbg_printf ((" ( %d subv (%d bindings, %d nest.opt.), %d recv, %d gspo, %d const, %d opt, %d subq:",
	    BOX_ELEMENTS_INT_0 (eq->e_subvalue_idxs), (int) (eq->e_nested_bindings), (int) (eq->e_nested_optionals),
	    BOX_ELEMENTS_INT_0 (eq->e_receiver_idxs), (int) (eq->e_gspo_uses), (int) (eq->e_const_reads),
	    (int) (eq->e_optional_reads), (int) (eq->e_subquery_uses)));
...
sparp_equiv_get (sparp_t * sparp, SPART * haystack_gp, SPART * needle_var, int flags)
{
  ...
  switch (SPART_TYPE (needle_var))
	...
	spar_internal_error (sparp, "sparp_" "equiv_get() with SPARP_EQUIV_INS_VARIABLE needs SPART * as needle_var");
      if ((flags & SPARP_EQUIV_GET_NAMESAKES) && ((DV_STRING == DV_TYPE_OF (needle_var)) || (DV_UNAME == DV_TYPE_OF (needle_var))))
	break;
    default:
      spar_internal_error (sparp, "sparp_" "equiv_get() with non-variable SPART *needle_var");
      break;
    }
...
sparp_equiv_get_ro (sparp_equiv_t ** equivs, ptrlong equiv_count, SPART * haystack_gp, SPART * needle_var, int flags)
{
  ...
  switch (SPART_TYPE (needle_var))
	...
	break;
    default:
      spar_internal_error (NULL, "sparp_equiv_get_ro() with non-variable SPART *needle_var");
      break;
    }
#endif
  needle_var_name = (
...
sparp_equiv_connect_outer_to_inner (sparp_t * sparp, sparp_equiv_t * outer, sparp_equiv_t * inner, int add_if_missing)
{
  ...
  if ((OPTIONAL_L == inner->e_gp->_.gp.subtype) || (0 == o_count))
      ...
      for (o_ctr = o_count; o_ctr--; /* no step */ )
	{
      ...
      outer->e_subvalue_idxs =
	  (ptrlong *) t_list_insert_before_nth ((caddr_t) (outer->e_subvalue_idxs), (caddr_t) ((ptrlong) (inner->e_own_idx)),
	  o_ctr + 1);
    }
...
sparp_equiv_connect_param_to_external (sparp_t * sparp, sparp_equiv_t * param, sparp_equiv_t * external)
{
  ...
  external->e_uses_as_params =
      (ptrlong *) t_list_concat_tail ((caddr_t) (external->e_uses_as_params), 1, (ptrlong) (param->e_own_idx));
  ...
  if (1 == namesakes_count)
      ...
      for (ctr = count; ctr--; /* no step */ )
	{
	  if (strcmp (eq->e_varnames[ctr], var->_.var.vname))
	    continue;
	  while (++ctr < count)
	    eq->e_varnames[ctr - 1] = eq->e_varnames[ctr];
...
sparp_equiv_remove (sparp_t * sparp, sparp_equiv_t * eq)
{
  ...
  if ((0 != eq->e_const_reads) ||
      ...
      (0 != BOX_ELEMENTS_0 (eq->e_uses_as_params)) || (SPART_VARR_EXPORTED & eq->e_rvr.rvrRestrictions))
...
#endif
  ret =
      sparp_equiv_restrict_by_constant (sparp, pri, sec->e_rvr.rvrDatatype, (SPART *) (sec->e_rvr.rvrFixedValue),
      sec->e_rvr.rvrFixedOrigText);
...
int
sparp_values_equal (sparp_t * sparp, ccaddr_t first, ccaddr_t first_dt, ccaddr_t first_lang, ccaddr_t second, ccaddr_t second_dt,
    ccaddr_t second_lang)
{
...
rvr_can_be_tightened (sparp_t * sparp, rdf_val_range_t * dest, rdf_val_range_t * add_on, int assume_binv_var_and_eq)
{
  ...
  if (add_on->rvrRestrictions & ~(dest->rvrRestrictions) &
	  ...
	  SPART_VARR_TYPED | SPART_VARR_FIXED | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL |
	  SPART_VARR_ALWAYS_NULL))
    return 1;
  ...
  if ((add_on->rvrRestrictions & SPART_VARR_FIXED)
      && (DVC_MATCH != cmp_boxes_safe (dest->rvrFixedValue, add_on->rvrFixedValue, NULL, NULL)))
  ...
  if (add_on->rvrRestrictions & SPART_VARR_IRI_CALC)
	...
	{
	  if ((add_on->rvrSprintffCount != dest->rvrSprintffCount)
	      || memcmp (add_on->rvrSprintffs, dest->rvrSprintffs, add_on->rvrSprintffCount * sizeof (ccaddr_t)))
	    return 1;
	}
    }
  if (add_on->rvrRedCutCount)
	...
	{
	  if ((add_on->rvrRedCutCount != dest->rvrRedCutCount)
	      || memcmp (add_on->rvrRedCuts, dest->rvrRedCuts, add_on->rvrRedCutCount * sizeof (ccaddr_t)))
	    return 1;
	}
    }
  return 0;
...
sparp_rvr_add_iri_classes (sparp_t * sparp, rdf_val_range_t * rvr, ccaddr_t * add_classes, ptrlong add_count)
{
  ...
  for (addctr = 0; addctr < add_count; addctr++)
      ...
      for (ctr = 0; ctr < len; ctr++)
	{
      ...
      addon_superclasses =
	  jso_triple_get_objs ((caddr_t *) (sparp->sparp_sparqre->sparqre_qi), (caddr_t) addon,
      ...
      for (ctr = 0; ctr < len; ctr++)
	{
      ...
      dk_free_tree ((caddr_t) addon_superclasses);
      addon_subclasses =
	  jso_triple_get_subjs ((caddr_t *) (sparp->sparp_sparqre->sparqre_qi), (caddr_t) uname_virtrdf_ns_uri_isSubclassOf,
	  (caddr_t) addon);
...
sparp_rvr_intersect_iri_classes (sparp_t * sparp, rdf_val_range_t * rvr, ccaddr_t * isect_classes, ptrlong isect_count)
{
  ...
  for (ctr = 0; ctr < len; ctr++)
      ...
      for (isectctr = 0; isectctr < isect_count; isectctr++)
	{
      ...
      old_superclasses =
	  jso_triple_get_objs ((caddr_t *) (sparp->sparp_sparqre->sparqre_qi), (caddr_t) old,
	  (caddr_t) uname_virtrdf_ns_uri_isSubclassOf);
	  ...
	  if (DV_ARRAY_OF_POINTER == valtype)
	      ...
	      if ((NULL == dest->rvrDatatype) && (NULL == dest->rvrLanguage) && (DV_STRING != valtype))
		{
		  ...
		  if (uname_xmlschema_ns_uri_hash_string == dest->rvrDatatype)
		    dest->rvrDatatype = NULL;
		  else if ((dest->rvrDatatype == value->_.lit.datatype) && ((DV_LONG_INT == valtype) || (DV_DOUBLE_FLOAT == valtype)
			  || (DV_SINGLE_FLOAT == valtype) || (DV_DATETIME == valtype)))
	  ...
	  else
	    {
	      ...
	      if (uname_xmlschema_ns_uri_hash_string == dest->rvrDatatype)
		dest->rvrDatatype = NULL;
	      else if ((DV_LONG_INT == valtype) || (DV_DOUBLE_FLOAT == valtype) || (DV_SINGLE_FLOAT == valtype)
		  || (DV_DATETIME == valtype))
		dest->rvrRestrictions &= ~SPART_VARR_LONG_EQ_SQL;
	      dest->rvrLanguage = NULL;
	    }
...
sparp_rvr_add_restrictions (sparp_t * sparp, rdf_val_range_t * dest, ptrlong addon_restrictions)
{
  ...
  if (((new_restr & SPART_VARR_IS_REF) && (new_restr & SPART_VARR_IS_LIT)) ||
      ...
      ((new_restr & SPART_VARR_ALWAYS_NULL) && (new_restr & (SPART_VARR_NOT_NULL | SPART_VARR_IS_LIT | SPART_VARR_IS_REF))))
    goto conflict;		/* see below */
  ...
  if (addon->rvrRestrictions & changeable_flags & SPART_VARR_FIXED)
      ...
      if (dest->rvrRestrictions & SPART_VARR_FIXED)
	{
	  if (DVC_MATCH != cmp_boxes_safe (dest->rvrFixedValue, addon->rvrFixedValue, NULL, NULL))
	    goto conflict;	/* see below */
	  if ((NULL != dest->rvrFixedOrigText) && (NULL != addon->rvrFixedOrigText)
	      && strcmp (dest->rvrFixedOrigText, addon->rvrFixedOrigText))
	    goto conflict;	/* see below */
	}
      else
	{
...
sparp_rvr_loose (sparp_t * sparp, rdf_val_range_t * dest, rdf_val_range_t * addon, int changeable_flags)
{
  ...
  changeable_flags &= ((ptrlong) (SPART_VARR__ALL)) & ~(SPART_VARR_EXPORTED | SPART_VARR_GLOBAL | SPART_VARR_EXTERNAL);
  new_restr = dest->rvrRestrictions & (addon->rvrRestrictions | ~changeable_flags);
  if (dest->rvrDatatype != addon->rvrDatatype)
    {
  ...
  if (!(new_restr & changeable_flags & SPART_VARR_FIXED))
      ...
      if (dest->rvrRestrictions & changeable_flags & SPART_VARR_FIXED)
	{
      ...
      if ((addon->rvrRestrictions & changeable_flags & SPART_VARR_FIXED) && (dest->rvrRestrictions & SPART_VARR_SPRINTFF))
	{
  ...
  else if ((NULL == dest->rvrFixedOrigText) || (NULL == addon->rvrFixedOrigText)
      || strcmp (dest->rvrFixedOrigText, addon->rvrFixedOrigText))
    dest->rvrFixedOrigText = NULL;
  ...
  if (orig_len > 20)
    {
	  ...
	  'A' + ((hash >> 8) & 0x10), 'A' + ((hash >> 12) & 0x10), (int) (sparp->sparp_sg->sg_cloning_serial));
    }
  else
...
SPART **sparp_treelist_full_clone_int (sparp_t * sparp, SPART ** origs, SPART * parent_gp);
  ...
  switch (SPART_TYPE (orig))
      ...
      if (SPART_BAD_EQUIV_IDX != eq_idx)
	{
	  ...
	  if (BOX_ELEMENTS (cloned_eq->e_vars) <= cloned_eq->e_var_count)
	    spar_internal_error (sparp,
		"sparp_"
		"tree_full_clone_int(): mismatch of lengths of buffer for variables in clone equiv and the number of variables");
      ...
      for (eq_ctr = 0; eq_ctr < orig->_.gp.equiv_count; eq_ctr++)
	{
	  ...
	  if (NULL != parent_gp)
		...
		spar_internal_error (sparp,
		    "sparp_" "tree_full_clone_int(): nonempty recv list without parent, maybe clone source is not detached");
...
#endif
	tgt->_.req_top.expanded_orig_retvals =
	    sparp_treelist_full_clone_int (sparp, orig->_.req_top.expanded_orig_retvals, orig_pattern);
...
/* Add more cases right above this line when introducing more SPAR_nnn constants */
    default:
      break;			/* No need to copy names and literals because we will never change them in-place. */
    }
	  ...
	  if (SPAR_IS_BLANK_OR_VAR (field))
	    {
	      ...
	      /*parent_eq = */ sparp_equiv_get (sparp, parent_gp, field,
		  SPARP_EQUIV_INS_VARIABLE | SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_ADD_GSPO_USE);
	    }
	}
      if (NULL != memb->_.triple.options)
	{
...
void
sparp_gp_attach_member (sparp_t * sparp, SPART * parent_gp, SPART * memb, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr)
{
  dk_set_t touched_equivs_set = NULL;
  dk_set_t *touched_equivs_set_ptr = ((NULL != touched_equivs_ptr) ? &touched_equivs_set : NULL);
  SPART **old_members = parent_gp->_.gp.members;
...
void
sparp_gp_attach_many_members (sparp_t * sparp, SPART * parent_gp, SPART ** new_members, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr)
{
  dk_set_t touched_equivs_set = NULL;
  dk_set_t *touched_equivs_set_ptr = ((NULL != touched_equivs_ptr) ? &touched_equivs_set : NULL);
  ...
  if (0 < ins_count)
    {
      parent_gp->_.gp.members =
	  (SPART **) t_list_insert_many_before_nth ((caddr_t) old_members, (caddr_t *) new_members, ins_count, insert_before_idx);
...
sparp_find_triple_with_var_obj_of_freetext (sparp_t * sparp, SPART * gp, SPART * filt, int make_ft_type_check)
{
  ...
  DO_BOX_FAST_REV (SPART *, memb, memb_ctr, gp->_.gp.members)
  {
    ...
    if ((SPAR_TRIPLE_SHOULD_HAVE_NO_FT_TYPE & make_ft_type_check) && (NULL != memb->_.triple.ft_type))
      spar_error (sparp,
	  "Two special predicates, %.500s() and %.500s() are used for same variable $%.500s in same scope, please rephrase the query",
	  memb->_.triple.ft_type, ft_pred_name, var_name);
    if ((SPAR_TRIPLE_SHOULD_HAVE_FT_TYPE & make_ft_type_check) && (NULL == memb->_.triple.ft_type))
      spar_internal_error (sparp,
	  "sparp_" "find_triple_with_var_obj_of_freetext(): lost connection between triple pattern and an ft predicate");
  ...
  if ((SPAR_TRIPLE_FOR_FT_SHOULD_EXIST & make_ft_type_check) && (NULL == triple_with_var_obj))
    spar_error (sparp,
	"No suitable triple pattern is found for a variable $%.500s in special predicate %.500s() at line %ld of query", var_name,
	ft_pred_name, (long) unbox (filt->srcline));
...
sparp_distinct_varnames_of_tree (sparp_t * sparp, SPART * tree, dk_set_t * acc)
{
  ...
  sparp_gp_trav_int (sparp, tree, stss + 1, acc,
      NULL, NULL, sparp_gp_distinct_varnames_expn_in_cbk, NULL, sparp_gp_distinct_varnames_expn_subq_cbk, NULL);
}
...
sparp_gp_detach_filter (sparp_t * sparp, SPART * parent_gp, int filter_idx, sparp_equiv_t *** touched_equivs_ptr)
{
  ...
  if (NULL != ft_type)
    {
	  ...
	  sparp_find_triple_with_var_obj_of_freetext (sparp, parent_gp, filt,
	  SPAR_TRIPLE_FOR_FT_SHOULD_EXIST | SPAR_TRIPLE_SHOULD_HAVE_FT_TYPE);
  ...
  sparp_gp_trav_int (sparp, filt, stss + 1, touched_equivs_set_ptr,
      NULL, NULL, sparp_gp_detach_filter_expn_in_cbk, NULL, sparp_gp_detach_filter_expn_subq_cbk, NULL);
  if (NULL != touched_equivs_ptr)
    touched_equivs_ptr[0] = (sparp_equiv_t **) (t_revlist_to_array (touched_equivs_set));
...
sparp_extract_filters_replaced_by_equiv (sparp_t * sparp, sparp_equiv_t * eq, dk_set_t * filts_from_equiv_ret)
{
  ...
  if (repl_bits & SPART_VARR_IS_REF)
    t_set_push (filts_from_equiv_ret, sparp_make_builtin_call (sparp, SPAR_BIF_ISREF, (SPART **) t_list (1,
		spar_make_variable (sparp, sample_varname))));
  if (repl_bits & SPART_VARR_IS_IRI)
    t_set_push (filts_from_equiv_ret, sparp_make_builtin_call (sparp, SPAR_BIF_ISIRI, (SPART **) t_list (1,
		spar_make_variable (sparp, sample_varname))));
  if (repl_bits & SPART_VARR_IS_BLANK)
    t_set_push (filts_from_equiv_ret, sparp_make_builtin_call (sparp, SPAR_BIF_ISBLANK, (SPART **) t_list (1,
		spar_make_variable (sparp, sample_varname))));
  if (repl_bits & SPART_VARR_IS_LIT)
    t_set_push (filts_from_equiv_ret, sparp_make_builtin_call (sparp, SPAR_BIF_ISLITERAL, (SPART **) t_list (1,
		spar_make_variable (sparp, sample_varname))));
  ...
  if (repl_bits & SPART_VARR_NOT_NULL)
    t_set_push (filts_from_equiv_ret, sparp_make_builtin_call (sparp, BOUND_L, (SPART **) t_list (1, spar_make_variable (sparp,
		    sample_varname))));
  if (repl_bits & SPART_VARR_ALWAYS_NULL)
    t_set_push (filts_from_equiv_ret, spartlist (sparp, 3, BOP_NOT,
	    sparp_make_builtin_call (sparp, BOUND_L, (SPART **) t_list (1, spar_make_variable (sparp, sample_varname))), NULL));
  if (repl_bits & SPART_VARR_EQ_VAR)
	...
	t_set_push (filts_from_equiv_ret, spartlist (sparp, 3, BOP_EQ,
		spar_make_variable (sparp, sample_varname), spar_make_variable (sparp, eq->e_varnames[varname_ctr])));
    }
}

SPART **
sparp_gp_detach_all_filters (sparp_t * sparp, SPART * parent_gp, int extract_filters_replaced_by_equivs,
    sparp_equiv_t *** touched_equivs_ptr)
{
  sparp_trav_state_t stss[SPARP_MAX_SYNTDEPTH + 2];
  ...
  if (0 != parent_gp->_.gp.glued_filters_count)
    spar_internal_error (sparp,
	"sparp_" "gp_detach_all_filters(): optimization tries to break the semantics of LEFT OUTER JOIN for OPTIONAL clause");
...
void
sparp_gp_attach_filter (sparp_t * sparp, SPART * parent_gp, SPART * new_filt, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr)
{
  sparp_trav_state_t stss[SPARP_MAX_SYNTDEPTH + 2];
  SPART **old_filters = parent_gp->_.gp.filters;
  ...
  if (NULL != ft_type)
    {
      SPART *triple_with_var_obj =
	  sparp_find_triple_with_var_obj_of_freetext (sparp, parent_gp, new_filt,
  ...
  sparp_gp_trav_int (sparp, new_filt, stss + 1, touched_equivs_set_ptr, NULL, NULL, sparp_gp_attach_filter_cbk, NULL, NULL, NULL);
  if (NULL != touched_equivs_ptr)
    touched_equivs_ptr[0] = (sparp_equiv_t **) (t_revlist_to_array (touched_equivs_set));
  sparp_equiv_audit_all (sparp, 0);
}
...
void
sparp_gp_attach_many_filters (sparp_t * sparp, SPART * parent_gp, SPART ** new_filters, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr)
{
  sparp_trav_state_t stss[SPARP_MAX_SYNTDEPTH + 2];
  SPART **old_filters = parent_gp->_.gp.filters;
  ...
  parent_gp->_.gp.filters =
      (SPART **) t_list_insert_many_before_nth ((caddr_t) old_filters, (caddr_t *) new_filters, ins_count, insert_before_idx);
  if (insert_before_idx > (old_len - parent_gp->_.gp.glued_filters_count))
  ...
  for (filt_ctr = ins_count; filt_ctr--; /*no step */ )
      ...
      if (NULL != ft_type)
	{
	  ...
	  NULL, NULL, sparp_gp_attach_filter_cbk, NULL, NULL, NULL);
    }
  if (NULL != touched_equivs_ptr)
    touched_equivs_ptr[0] = (sparp_equiv_t **) (t_revlist_to_array (touched_equivs_set));
  sparp_equiv_audit_all (sparp, 0);
...
sparp_gp_tighten_by_eq_replaced_filters (sparp_t * sparp, SPART * dest, SPART * orig, int remove_origin)
{
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, orig, eq_ctr, orig_eq)
  {
	...
	DO_BOX_FAST (caddr_t, varname, varname_ctr, orig_eq->e_varnames)
	{
	  ...
	  sparp_equiv_t *dest_eq =
	      sparp_equiv_get (sparp, dest, (SPART *) varname, SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_INS_CLASS);
	  if (orig_eq->e_rvr.rvrRestrictions & SPART_VARR_FIXED)
	    changeable_flags |=
		(SPART_VARR_TYPED | SPART_VARR_IS_LIT | SPART_VARR_IS_REF | SPART_VARR_IS_BLANK | SPART_VARR_IS_IRI |
		SPART_VARR_NOT_NULL);
...
sparp_gp_deprecate (sparp_t * sparp, SPART * gp, int suppress_eq_check)
{
  ...
  DO_BOX_FAST (SPART *, filt, filt_ctr, gp->_.gp.members)
  {
    ...
    sparp_gp_trav_int (sparp, filt, &fake_stack, NULL, NULL, NULL, NULL, NULL, sparp_gp_trav_gp_deprecate_expn_subq, NULL);
  }
  END_DO_BOX_FAST;
...
sparp_req_top_deprecate (sparp_t * sparp, SPART * top)
{
  ...
  sparp_trav_out_clauses_int (sparp, top, &fake_stack, NULL, NULL, NULL, NULL, NULL, sparp_gp_trav_gp_deprecate_expn_subq, NULL);
}
...
SPART *
sparp_find_origin_of_external_varname_in_eq (sparp_t * sparp, sparp_equiv_t * eq, caddr_t varname, int find_exact_specimen,
    int null_result_allowed)
{
...
sparp_find_origin_of_some_external_varname_in_eq (sparp_t * sparp, sparp_equiv_t * eq, int find_exact_specimen)
{
  ...
  END_DO_BOX_FAST_REV;
  spar_internal_error (sparp,
      "sparp_" "find_origin_of_some_external_varname_in_eq(): external source equiv share no common names with the give equiv");
...
sparp_get_options_of_tree (sparp_t * sparp, SPART * tree)
{
  switch (SPART_TYPE (tree))
    {
    case SPAR_GP:
      return tree->_.gp.options;
    case SPAR_TRIPLE:
      return tree->_.triple.options;
    }
  return NULL;
}
...
sparp_validate_options_of_tree (sparp_t * sparp, SPART * tree, SPART ** options)
{
  ...
  for (idx = BOX_ELEMENTS_0 (options) - 2; idx >= 0; idx -= 2)
	...
	case T_OUT_L:
	  {
	    ...
	    if (NULL == subq_orig_retvals)
	      spar_error (sparp,
		  "T_IN and T_OUT options can be used only for SELECT subquery because they should refer to result-set of a subquery");
		...
		subq_orig_retvals[pos1_ret - 1] =
		    spartlist (sparp, 6, SPAR_ALIAS, ret, v->_.var.vname, SSG_VALMODE_AUTO, (ptrlong) 0, (ptrlong) 0);
	    }
	    END_DO_BOX_FAST;
	    continue;
	  }
	case T_STEP_L:
	  {
		...
		if (0 == sparp_subexpn_position1_in_retlist (sparp, vname, subq_orig_retvals))
		  spar_error (sparp, "Variable ?%.100s is used in T_STEP option but not in the result-set of a subquery", vname);
	      }
	    else
	      {
	...
	case SERVICE_L:
	  ...
	  continue;
	default:
	  spar_internal_error (sparp, "sparp_" "validate_options_of_tree(): unsupported option");
	}
    }
  switch (ttype)
    {
...
sparp_set_option (sparp_t * sparp, SPART *** options_ptr, ptrlong key, SPART * value, ptrlong mode)
{
  ...
  for (idx = BOX_ELEMENTS_0 (options) - 2; idx >= 0; idx -= 2)
	...
	case SPARP_SET_OPTION_APPEND1:
	  ...
	  options[idx + 1]->_.list.items =
	      (SPART **) t_list_concat_tail ((caddr_t) (options[idx + 1]->_.list.items), 1, (caddr_t) value);
	  return options[idx + 1];
	default:
	  spar_internal_error (sparp, "sparp_set_option(): bad mode");
	}
    }
  switch (mode)
    {
...
spar_var_name_of_ret_column (SPART * tree)
{
  switch (DV_TYPE_OF (tree))
	...
	{
	case SPAR_ALIAS:
	  return spar_var_name_of_ret_column (tree->_.alias.arg);
	case SPAR_VARIABLE:
	case SPAR_BLANK_NODE_LABEL:
	  return tree->_.var.vname;
	}
      return NULL;
/*
...
{
  if ((DV_ARRAY_OF_POINTER == DV_TYPE_OF (tree)) && (SPAR_ALIAS == tree->type))
...
spart_dump_opname (ptrlong opname, int is_op)
{
  ...
  if (is_op)
	...
	return "boolean operation '='";
      case SPAR_BOP_EQ_NONOPT:
	return "special nonoptimizable equality";
      case SPAR_BOP_EQNAMES:
	return "declaration of equivalence of names";
      case SPAR_UNION_WO_ALL:
	return "UNION (but not UNION ALL) gp";
      case BOP_NEQ:
	return "boolean operation '!='";
      case BOP_LT:
	return "boolean operation '<'";
  ...
  switch (opname)
    {
    case _LBRA:
      return "quad mapping parent group name";
    case ASC_L:
      return "ascending order";
    case ASK_L:
      return "ASK result-mode";
    case BOUND_L:
    ...
    case DEFAULT_L:
      return "default graph context";
    case DEFMACRO_L:
      return "DEFMACRO";
    case DESC_L:
      return "descending";
    case DESCRIBE_L:
      return "DESCRIBE result-mode";
    case DISTINCT_L:
      return "SELECT DISTINCT result-mode";
    case false_L:
...
void
spart_dump_long (void *addr, dk_session_t * ses, int is_op)
{
...
void
spart_dump_varr_bits (dk_session_t * ses, int varr_bits, int hot_bits, char hot_sign)
{
  char buf[200];
  char *tail = buf;
...
void
spart_dump_rvr (dk_session_t * ses, rdf_val_range_t * rvr, int hot_bits, char hot_sign)
{
  char buf[300];
  char *tail = buf;
  int len;
...
spart_dump_eq (sparp_equiv_t * eq, dk_session_t * ses)
{
  ...
  sprintf (buf, " %s( %d subv (%d bindings, %d nest.opt.), %d recv, %d gspo, %d const, %d opt, %d subq:",
      ...
      BOX_ELEMENTS_INT_0 (eq->e_subvalue_idxs), (int) (eq->e_nested_bindings), (int) (eq->e_nested_optionals),
      BOX_ELEMENTS_INT_0 (eq->e_receiver_idxs), (int) (eq->e_gspo_uses), (int) (eq->e_const_reads), (int) (eq->e_optional_reads),
      (int) (eq->e_subquery_uses));
  SES_PRINT (ses, buf);
  varname_count = BOX_ELEMENTS (eq->e_varnames);
  for (varname_ctr = 0; varname_ctr < varname_count; varname_ctr++)
    {
  ...
  for (var_ctr = 0; var_ctr < eq->e_var_count; var_ctr++)
    {
      ...
      SES_PRINT (ses, " ");
      SES_PRINT (ses,
	  ((NULL != var->_.var.tabid) ? var->_.var.tabid : (NULL != var->_.var.selid) ? var->_.var.selid : "[no selid]"));
    }
  SES_PRINT (ses, ";");
  spart_dump_rvr (ses, &(eq->e_rvr), eq->e_replaces_filter, '!');
  if (SPART_BAD_EQUIV_IDX != eq->e_external_src_idx)
    {
...
spart_dump (void *tree_arg, dk_session_t * ses, int indent, const char *title, int hint)
{
  ...
  switch (hint)
	...
	switch (tree->type)
	  ...
	  case SPAR_MACROPU:
	    {
	      sprintf (buf, "MACRO PARAM %s (#%ld), type %ld\n", tree->_.macropu.pname, (long) (tree->_.macropu.pindex),
		  (long) (tree->_.macropu.pumode));
	      SES_PRINT (ses, buf);
	      break;
	    }
	  case SPAR_PPATH:
		...
		strcpy (subt_buf, "\\0");
		  ...
		  (long) (unbox (tree->_.ppath.minrepeat)), (long) (unbox (tree->_.ppath.maxrepeat)),
		  (long) (tree->_.ppath.num_of_invs));
	      SES_PRINT (ses, buf);
	      switch (tree->_.ppath.subtype)
		{
		case '/':
		  part_title = "SEQUENCE STEP";
		  break;
		case '*':
		  part_title = "TRANSITIVE BODY";
		  break;
		case '!':
		  part_title = "NEGATIVE";
		  break;
...
int
sparp_valmode_is_correct (ssg_valmode_t fmt)
{
  jso_rtti_t *rtti;
...
void
sparp_jso_validate_format (sparp_t * sparp, ssg_valmode_t fmt)
{
...
#if 0
void
ssg_jso_validate_format (spar_sqlgen_t * ssg, ssg_valmode_t fmt)
{
  if (!sparp_valmode_is_correct (fmt))
