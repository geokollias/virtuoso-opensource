...
extern int64 sqlc_cum_memory;
extern long tc_sqlo_no_mem;
extern int sq_cache_hit, sq_cache_miss, sq_hash_fill_hit, sq_hash_fill_miss;
extern long tc_cached_subq_no_reuse_for_card;
...
wi_storage_report ()
{
  DO_SET (wi_db_t *, wd, &wi_inst.wi_dbs)
  {
	...
	continue;
	  ...
	  dbs->dbs_n_pages, dbs_count_free_pages (dbs), dbs->dbs_cpt_remap->ht_count, dbs_mapped_back (dbs));
    }
    END_DO_SET ();
  }
  END_DO_SET ();
...
dbms_status_report (void)
{
  ...
  if (1)
	...
	mutex_leave (bp->bp_mtx);
	  ...
	  st_db_free_pages,
	  n_buffers, n_used, n_dirty, n_wired, bp_replace_count ? (int) (bp_replace_age / bp_replace_count) : 0, n_io, n_crsr);
	...
	w_rate[0] = 0;
      if (ac_col_pages_in)
	snprintf (col_ac_str, sizeof (col_ac_str), " col ac: %ld in %ld%% saved", ac_col_pages_in,
	    100 * (ac_col_pages_in - ac_col_pages_out) / (1 + ac_col_pages_in));
      else
	col_ac_str[0] = 0;
      rep_printf
	  ("  Disk Usage: %ld reads avg %ld msec, %d%% r %d%% w last  %ld s, %ld writes %s,\n    %ld read ahead, batch = %ld.  Autocompact %ld in %ld out, %ld%% saved%s.\n",
	  disk_reads, read_cum_time / (disk_reads ? disk_reads : 1), read_percent, write_percent, interval_msec / 1000, disk_writes,
	  w_rate, ra_count, ra_pages / (ra_count + 1), ac_pages_in, ac_pages_out,
	  100 * (ac_pages_in - ac_pages_out) / (1 + ac_pages_in), col_ac_str);
  ...
  rep_printf ("Gate:  %ld 2nd in reads, %ld gate write waits, %ld in while read %ld busy scrap. %s\n",
      second_reads, 0, in_while_read, busy_pre_image_scrap, disk_no_mt_write ? "no mt write" : "");
  ...
  rep_printf ("Log = %s, " OFF_T_PRINTF_FMT " bytes\n",
      dbs->dbs_log_name ? dbs->dbs_log_name : "none", (OFF_T_PRINTF_DTP) dbs->dbs_log_length);
  ...
  snprintf (st_db_log_length, sizeof (st_db_log_length_buffer), OFF_T_PRINTF_FMT, (OFF_T_PRINTF_DTP) dbs->dbs_log_length);
...
trx_status_report (lock_trx_t * lt)
{
  ...
  IN_TXN;
  {
	...
	lt->lt_status == LT_PENDING ? "PENDING" : (lt->lt_status == LT_BLOWN_OFF_C ? "BLOWN OFF" : "ROLLED BACK"), lt->lt_threads);
	...
	DO_HT (page_lock_t *, pl, void *, ign, &lt->lt_lock)
	{
	  ...
	  while (waiting)
		...
		{
		  wait = 1;
		  rep_printf ("   %ld: W%s, ", pl->pl_page, waiting->itc_lock_mode == PL_SHARED ? "S" : "E");
		  break;
		}
	  ...
	  if (!wait)
	    {
	      rep_printf ("%ld: I%s, ", pl->pl_page, pl->pl_type == PL_SHARED ? "S" : "E");
	    }
	}
	END_DO_HT;
	LEAVE_LT_LOCKS (lt);
...
cli_status_report (dk_session_t * ses)
{
  ...
  rep_printf ("\nClient %s:  Account: %s, " OFF_T_PRINTF_FMT " bytes in, "
      ...
      (OFF_T_PRINTF_DTP) ses->dks_bytes_sent, cli->cli_statements->ht_inserts - cli->cli_statements->ht_deletes);
  if (ARRAYP (cli->cli_info) && BOX_ELEMENTS (cli->cli_info) > 5)
    {
	  ...
	  unbox (cli->cli_info[LGID_PID]), cli->cli_info[LGID_OS], app_name[0] ? app_name : "unknown", from);
    }
  if (!cli->cli_trx)
    return;
...
lt_short_name (lock_trx_t * lt)
{
  ...
  name = lt->lt_client->cli_session ?
      (lt->lt_client->cli_session->dks_peer_name ? lt->lt_client->cli_session->dks_peer_name : "NO_CONN") : "INTERNAL";
  last = strchr (name, ':');
  if (last)
    return (last + 1);
  return name;
...
gen_lock_status (gen_lock_t * pl, char *indent, long id)
{
  ...
  rep_printf ("%s%ld: I%s%s ", indent, id, PL_TYPE (pl) == PL_EXCLUSIVE ? "E" : "S", PL_IS_PAGE (pl) ? "P" : "R");
  if (NULL == pl->pl_owner)
    {
      rep_printf ("NO OWNER ");
    }
...
hic_status ()
{
  ...
  for (it = hash_index_cache.hic_first; it; it = it->it_hic_next)
      ...
      for (inx = 0; inx < n_cols; inx++)
	{
	  ...
	  ((unsigned int) it->it_hi->hi_lock_mode), ((unsigned int) it->it_hi->hi_isolation));
#else
      rep_printf ("\n     %d pages %d entries %d reuses %d busy %s \n",
	  0,
	  it->it_hi->hi_count,
	  it->it_hi_reuses, it->it_ref_count, it->it_shared == HI_OK ? "OK" : it->it_shared == HI_FILL ? "FILL" : "BUST");
#endif
    }
  rep_printf ("\n");
...
key_status (const void *k_id, void *k_key)
{
  ...
  rep_printf ("%-18s %-19s %7ld %7ld %4ld%% %7ld %7ld %3ld%% %ld\n",
      ...
      key->key_lock_set, key->key_lock_wait, (key->key_lock_wait * 100) / (key->key_lock_set + 1), key->key_deadlocks);
}
...
product_version_string ()
{
  ...
  if ('\0' == buf[0])
    snprintf (buf, sizeof (buf), "%s%.500s Server (%s%s Edition) Version %s%s as of %s", PRODUCT_DBMS, build_special_server_model,
#ifdef OEM_BUILD
	"OEM ",
#else
	"",
	...
	((build_thread_model[0] == '-' && build_thread_model[1] == 'f') ?
	    "Lite" : "Enterprise"), DBMS_SRV_VER, build_thread_model, build_date);
  return buf;
}
...
status_report (const char *mode, query_instance_t * qi)
{
  ...
  rep_printf ("Checkpoint Remap %ld pages, %ld mapped back. %ld s atomic time.\n",
      wi_inst.wi_master->dbs_cpt_remap->ht_count, st_chkp_mapback_pages, atomic_cp_msecs / 1000);
...
stat_desc_t stat_descs[] = {
  ...
  {"mp_mmap_clocks", (long *) &mp_mmap_clocks, NULL},
  ...
  {"rdf_create_graph_keywords", (long *) &rdf_create_graph_keywords, SD_INT32},
  {"rdf_query_graph_keywords", (long *) &rdf_query_graph_keywords, SD_INT32},
...
/*    {"dbf_no_raw_exit", &dbf_no_raw_exit, SD_INT32},*/
  ...
  {"mp_large_reserved", (long *) &mp_large_reserved},
  {"mp_max_large_reserved", (long *) &mp_max_large_reserved},
  {"mp_large_reserve_limit", (long *) &mp_large_reserve_limit},
  {"mp_large_soft_cap", (long *) &mp_large_soft_cap},
  {"mp_large_hard_cap", (long *) &mp_large_hard_cap},
  ...
  {"enable_qr_comment", (long *) &enable_qr_comment, SD_INT32},
  {"timezoneless_datetimes", (long *) &timezoneless_datetimes, SD_INT32},
  {NULL, NULL, NULL}
};
...
sys_stat_impl (const char *name)
{
  ...
  if (!st_dbms_name_buffer[0])
    snprintf (st_dbms_name_buffer, sizeof (st_dbms_name_buffer), "%s %.500s Server", PRODUCT_DBMS, build_special_server_model);
...
bif_identify_self (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  return (caddr_t) (list (4,
	  box_dv_short_string (host), box_dv_short_string (pid), box_dv_short_string (ip_addr), box_dv_short_string ("")));
}
...
key_n_buffers (dbe_key_t * key, int dirty)
{
  ...
  for (inx = 0; inx < wi_inst.wi_n_bps; inx++)
      ...
      for (inx2 = 0; inx2 < bp->bp_n_bufs; inx2++)
	{
	  ...
	  if (buf->bd_tree && buf->bd_tree->it_key == key && (!dirty || buf->bd_is_dirty))
	    ct++;
	}
    }
  return ct;
...
prof_report (void)
{
  ...
  if (out)
      ...
      if (prof_stat_table == 1)
	{
	  ...
	  fprintf (out, "</table>\n\n");
	  fprintf (out,
	      "<table id='qprof_t' border=\"1\" cellpadding=\"3\" cellspacing=\"0\"><tr><th colspan=\"5\">Query Profile (msec)</th></tr><tr><th>Real</th><th>client wait</th><th>avg conc</th><th>n_execs</th><th>avg exec</th></tr><tr><td>%ld</td><td>%ld</td><td>%4f</td><td>%ld</td><td>%ld</td></tr></table>\n\n",
	      real, prof_exec_time, (float) prof_exec_time / (float) real, prof_n_exec, prof_avg_exec);
	  ...
	  fprintf (out, "</table>\n\n");
	  fprintf (out,
	      "<table id='stmts_t' border=\"1\" cellpadding=\"3\" cellspacing=\"0\"><tr><th>stmts compiled</th><th>Time (msec)</th><th>prepared reused</th></tr>");
	  fprintf (out, "<tr><td>%ld</td><td>%ld</td><td>%ld %%</td></tr></table>\n", prof_n_compile, prof_compile_time,
	      prof_n_reused);

	  fprintf (out,
	      "<table border=\"1\" cellpadding=\"3\" cellspacing=\"0\"><tr><th>%%</th><th>total</th><th>n-times</th><th>n-errors</th></tr>\n");
	  for (inx = 0; inx < fill; inx++)
	    {
	      fprintf (out, "<tr><td>%-3ld%%</td><td>%-8ld</td><td>%-8ld</td><td>%-5ld</td><td>%s</td></tr>\n",
		  (long) (((float) times[inx]->qt_cum * 100) / (float) prof_exec_time),
		  (long) times[inx]->qt_cum, (long) times[inx]->qt_n, (long) times[inx]->qt_n_error, times[inx]->qt_title);
      ...
      else
	{
	  ...
	  for (inx = 0; inx < TIME_N_SLOTS; inx++)
	    {
	      fprintf (out, "%ld %% under %d s\n", (100 * prof_under[inx].tr_n) / prof_n_exec, prof_under[inx].tr_max / 1000);
	      prof_under[inx].tr_n = 0;
	    }
...
bif_profile_enable (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!prof_stat)
    prof_stat = id_hash_allocate (PROF_MAX_DISTINCT / 2, sizeof (void *), sizeof (qr_time_t), strhash, strhashcmp);
...
caddr_t dbg_print_itcs (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args);
	...
	case DV_ARRAY_OF_XQVAL:
	  {
		...
		if IS_POINTER
		  (elt) dbg_print_box_aux (elt, out, known);
		else
	...
	case DV_TEXT_SEARCH:
	  {
		...
		if ((length >= (n + (32 / sizeof (char *)))) && (
			...
			((27 + 2 * (32 / sizeof (char *))) == n) || ((19 + 1 * (32 / sizeof (char *))) == n) || (1 == n)))
		  {
		    dbg_print_d_id_aux (out, (unsigned char *) (((caddr_t *) object) + n));
		    continue;
		  }
		if ((length >= (31 + 6 * (32 / sizeof (char *)))) && (((23 + 2 * (32 / sizeof (char *))) == n)))
		  {
	...
	case DV_RDF:
	  {
	    ...
	    fprintf (out, ",%ld,%ld,%ld,%ld", (long) (rb->rb_type), (long) (rb->rb_lang), (long) (rb->rb_ro_id),
		(long) (rb->rb_is_complete));
...
col_ref_print (FILE * out, db_buf_t xx, short vl1)
{
  ...
  fprintf (out, "[C %d ces, start %d dps ", n_ces, (int) SHORT_REF_NA (xx + CPP_FIRST_CE));
...
dbg_page_map_f (buffer_desc_t * buf, FILE * out)
{
  ...
  fprintf (out, "Page %ld %s, child of %ld remap %ld   Key %s: \n",
      ...
      (long) buf->bd_physical_page, page_key_id == KI_TEMP ? "temp key" : page_key ? page_key->key_name : "<no key>");
  if (DPF_BLOB == SHORT_REF (buf->bd_buffer + DP_FLAGS))
    {
      fprintf (out, "  Blob, extend = %ld, bytes = %ld\n",
	  (long) LONG_REF (buf->bd_buffer + DP_OVERFLOW), (long) LONG_REF (buf->bd_buffer + DP_BLOB_LEN));
      return;
    }
  ...
  DO_ROWS (buf, map_pos, row, NULL)
  {
    ...
    fprintf (out, "    %d: %ldB  Key %ld: %s", map_pos, l, (long) kv, flag_str);
...
dbg_hash_page_f (buffer_desc_t * buf, FILE * out)
{
  ...
  fprintf (out, "Page %ld, next %ld : \n", (long) buf->bd_page, (long) LONG_REF (buf->bd_buffer + DP_OVERFLOW));
  if (DPF_BLOB == SHORT_REF (buf->bd_buffer + DP_FLAGS))
    {
      fprintf (out, "  Blob, extend = %ld, bytes = %ld\n",
	  (long) LONG_REF (buf->bd_buffer + DP_OVERFLOW), (long) LONG_REF (buf->bd_buffer + DP_BLOB_LEN));
  ...
  if (buf)
    {
	  ...
	  buf, buf->bd_page, buf->bd_physical_page, (long) (ptr - buf->bd_buffer));
...
print_registry ()
{
  ...
  for (id_hash_iterator (&hit, registry); hit_next (&hit, (char **) &name, (char **) &place);
      /* */ )
    {
      printf ("registry entry: %s [%s]\n", name ? name[0] : "NULL", place ? place[0] : "NULL");
    }
}
	...
	for (page_no = 2; page_no < storage->dbs_n_pages; page_no++)
	  {
	    ...
	    if (0 != alloc && !gethash (DP_ADDR2VOID (page_no), storage->dbs_cpt_remap))
	      {
...
sys_em_stat_entry (dk_set_t * set, dbe_key_t * key, extent_map_t * em)
{
  ...
  dk_set_push (set, list (11,
	  ...
	  box_num (em->em_n_free_blob_pages), box_num (n_free), box_num (n_free_blob), box_num (n_free_remap)));
}
...
bif_sys_em_stat (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  END_DO_SET ()LEAVE_DBS (wi_inst.wi_master);
...
real_cv_print_proc (query_t * qr, ptrlong hist[], ptrlong * gl, ptrlong * gcnt)
{
  ...
  DO_INSTR (ins, 0, qr->qr_head_node->src_pre_code)
  {
    switch (ins->ins_type)
      ...
      case IN_ARTM_FPTR:
	lhist[_in_artm_fptr]++;
	real_len += sizeof (ins->_.artm_fptr);
	break;
      case IN_PRED:
	lhist[_in_pred]++;
	real_len += sizeof (ins->_.pred);
	break;
      case IN_COMPARE:
	lhist[_in_compare]++;
	real_len += sizeof (ins->_.pred);
	...
	break;
      case INS_SUBQ:
	lhist[_ins_subq]++;
	real_len += sizeof (ins->_.subq);
	break;
      case INS_QNODE:
	lhist[_ins_qnode]++;
	real_len += sizeof (ins->_.qnode);
	break;
      case INS_OPEN:
	lhist[_ins_open]++;
	real_len += sizeof (ins->_.open);
	break;
      case INS_FETCH:
	lhist[_ins_fetch]++;
	real_len += sizeof (ins->_.fetch);
	break;
      case INS_CLOSE:
	lhist[_ins_close]++;
	real_len += sizeof (ins->_.close);
	break;
      case INS_HANDLER:
	lhist[_ins_handler]++;
	real_len += sizeof (ins->_.handler);
	break;
      case INS_HANDLER_END:
	lhist[_ins_handler_end]++;
	real_len += sizeof (ins->_.handler_end);
	break;
      case IN_BRET:
	lhist[_ins_bret]++;
	real_len += sizeof (ins->_.bret);
	break;
      case IN_VRET:
	lhist[_ins_vret]++;
	real_len += sizeof (ins->_.vret);
	break;
      case IN_JUMP:
	lhist[_ins_jump]++;
	real_len += sizeof (ins->_.label);
	break;
      case INS_BREAKPOINT:
	lhist[_ins_breakpoint]++;
	real_len += sizeof (ins->_.breakpoint);
	break;
      case IN_ARTM_PLUS:
	lhist[_ins_artm]++;
	real_len += sizeof (ins->_.artm);
	break;
      case IN_ARTM_MINUS:
	lhist[_ins_artm]++;
	real_len += sizeof (ins->_.artm);
	...
	lhist[_ins_compound_start]++;
	break;
      case INS_COMPOUND_END:
	lhist[_ins_compound_end]++;
	break;
      default:
	lhist[_ins_other]++;
	real_len += INS_LEN (ins);
	break;
      }
    cnt++;
  }
  ...
  bif_result_inside_bif (24,
      ...
      tmp[20] = box_num (lhist[_ins_other]), tmp[21] = box_num (lhist[_ins_artm]), tmp[22] = box_num (cnt));
  *gl += l;
  *gcnt += cnt;
...
bif_real_cv_size (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  bif_result_inside_bif (24,
      ...
      tmp[21] = box_num (hist[_ins_other]), tmp[22] = box_num (hist[_ins_artm]), tmp[23] = box_num (total_cnt));
...
int
key_stat_to_spec (caddr_t data, dbe_key_t * key, int nth, search_spec_t * sp, it_cursor_t * itc, int *v_fill)
{
  int res = 0;
...
found:
  ...
  for (inx = 0; inx < n_parts; inx++)
    {
      res = key_stat_to_spec (bif_arg (qst, args, inx + 2, "key_estimate"), key, inx, &specs[inx], itc, &v_fill);
      if (KS_CAST_OK != res)
	{
	  itc_free (itc);
...
double
rep_num_scale (double n, const char **scale_ret, int is_base_2)
{
  ...
  static const char *letter[] = { "K", "M", "G", "T", "" };
  int inx;
  for (inx = 0; inx < 4; inx++)
    {
      if (n < scale[inx])
...
{
  const char *rans, *seqs, *rs, *bs, *ms, *same_segs, *same_pages, *same_pars, *specs, *qps;
  ...
  snprintf (out, len,
      "%6.4g%s rnd %6.4g%s seq %6.4g%s same seg  %6.4g%s same pg %6.4g%s same par %6.4g%s disk %6.4g%s spec disk %6.4g%sB / %6.4g%s messages %6.4g%s fork",
      ran, rans, seq, seqs, same_seg, same_segs, same_page, same_pages, same_par, same_pars, reads, rs, spec_reads, specs, bytes,
      bs, msgs, ms, qp, qps);
}
...
bif_db_activity (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  ccaddr_t acct = BOX_ELEMENTS (args) > 1 ? bif_string_arg (qst, args, 1, "db_activity") : "client";
...
bif_stat_export (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  DO_HT (ptrlong, id, dbe_column_t *, col, sc->sc_id_to_col)
  {
    dk_set_push (&cols, list (6, box_string (col->col_defined_in->tb_name), box_string (col->col_name),
	    box_num (col->col_n_distinct), box_num (col->col_count), sc_data_to_ext (qi, col->col_min), sc_data_to_ext (qi,
		col->col_max)));
  }
  END_DO_HT;
	...
	while (hit_next (&hit, (caddr_t *) & id, (caddr_t *) & arr))
	  {
	    dk_set_push (&psts, list (5, sc_data_to_ext (qi, box_iri_id (*(iri_id_t *) id)), box_float (arr[0]), box_float (arr[1]),
		    box_float (arr[2]), box_float (arr[3])));
	  }
	p_arr = list_to_array (psts);
      }
    dk_set_push (&keys, list (4, box_string (key->key_table->tb_name), box_string (key->key_name),
	    box_num (key->key_table->tb_count_estimate), p_arr));
  }
  END_DO_HT;
  DO_IDHASH (caddr_t, name, rdf_inf_ctx_t *, ric, rdf_name_to_ric)
  {
    ...
    while (hit_next (&hit, (caddr_t *) & s_key, (caddr_t *) & smp))
      {
	caddr_t *ent = (caddr_t *) sc_data_to_ext (qi, *(caddr_t *) s_key);
