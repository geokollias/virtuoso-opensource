...
void
apc_free (ap_class_t * apc, int mode)
{
...
void
aps_free (ap_set_t * aps, int mode)
{
  if (mode & AP_FREE_LOCK)
...
ap_class_t *
apc_register (query_instance_t * qst, ptrlong id, int allow_overwrite)
{
...
void
apc_unregister (query_instance_t * qst, ptrlong id)
{
  ap_class_t *apc;
  caddr_t err = NULL;
  local_cursor_t *lc;
...
void
apb_prepare (ap_bitarrays_t * apb, ptrlong scale)
{
  apb->apb_scale = scale;
...
ap_set_t *
aps_register (query_instance_t * qst, ptrlong id, int allow_overwrite)
{
...
/* First of all, we try to read the table, before entering any mutexes */
  ...
  aps_tmp.aps_class = NULL;
  apc_id = unbox (lc_nth_col (lc, 3));	/* to be completed inside mutex */
...
/* Now we can calculate bitmap's data */
  ...
  if (NULL == elh_get_handler (&eh__UTF8, aps_tmp.aps_lh))
    sqlr_new_error ("42000", "APS07",
	"Annotation phrase set #%ld refers to an language '%.300s' that has no accelerated UTF-8 support", (long) id,
	aps_tmp.aps_lh->lh_ISO639_id);
/* Now aps_tmp is OK */
  mutex_enter (ap_globals.apg_mutex);
  ...
  if (NULL == aps_tmp.aps_class)
    {
      ...
      sqlr_new_error ("42000", "APS03", "Annotation phrase set #%ld refers to a phrase class #%ld that is not registered",
	  (long) id, (long) apc_id);
    }
...
/* The pair of id_hash_remove() and id_hash_set() is here to prevent the use of deleted string aps->aps_name as a key */
	...
	{
	  aps_tmp.aps_bitarrays.apb_arrayX = aps->aps_bitarrays.apb_arrayX;
	  aps->aps_bitarrays.apb_arrayX = NULL;
	  aps_tmp.aps_bitarrays.apb_arrayY = aps->aps_bitarrays.apb_arrayY;
	  aps->aps_bitarrays.apb_arrayY = NULL;
...
void
aps_unregister (query_instance_t * qst, ptrlong id)
{
  ap_set_t *aps;
  caddr_t err = NULL;
  local_cursor_t *lc;
...
static const char *ap_select_chksum_and_aps_id__text = "select AP_CHKSUM from DB.DBA.SYS_ANN_PHRASE where AP_APS_ID = ?";

/*! This creates all bitarrays in all phrase sets in a single pass through SYS_ANN_PHRASE. */
void
aps_load_phrases (query_instance_t * qst, ap_set_t * aps)
{
  caddr_t err = NULL;
  local_cursor_t *lc;
  ...
  APB_ALLOC (apb[0]);
  err = qr_quick_exec (ap_select_chksum_and_aps_id__qr, qst->qi_client, "", &lc, 1, ":0", (ptrlong) (aps->aps_id), QRP_INT);
  if ((caddr_t) SQL_SUCCESS != err)
    sqlr_resignal (err);
  while (lc_next (lc))
    {
...
ap_class_t *
apc_get (ptrlong id, int lock_mode /* 0 = none, 1 = rdlock, 2 = wrlock */ )
{
  ap_class_t *res;
  if (!lock_mode)
    mutex_enter (ap_globals.apg_mutex);
  ...
  switch (lock_mode)
    ...
    case 0:
      break;
    case 1:
      rwlock_rdlock (res->apc_rwlock);
      break;
    case 2:
      rwlock_wrlock (res->apc_rwlock);
      break;
    default:
      GPF_T;
    }
...
ap_set_t *
aps_get (ptrlong id, int lock_mode /* 0 = none, 1 = rdlock, 2 = wrlock */ )
{
  ap_set_t *res;
  if (!lock_mode)
    mutex_enter (ap_globals.apg_mutex);
  ...
  switch (lock_mode)
    ...
    case 0:
      break;
    case 1:
      rwlock_rdlock (res->aps_rwlock);
      break;
    case 2:
      rwlock_wrlock (res->aps_rwlock);
      break;
    default:
      GPF_T;
    }
...
ap_set_t *
aps_get_byname (caddr_t name, int lock_mode /* 0 = none, 1 = rdlock, 2 = wrlock */ )
{
...
ap_set_t **
aps_tryrdlock_array (caddr_t * set_ids, int load_phrases, query_instance_t * qst /* can be NULL if no need to load phrases */ )
{
  ap_set_t **sets;
...
void
aps_unlock_array (ap_set_t ** sets, int set_count, int free_array)
{
  int set_ctr;
  for (set_ctr = 0; set_ctr < set_count; set_ctr++)
    {
...
void
ap_phrase_chksum_callback (const utf8char * buf, size_t bufsize, void *appdata)
{
...
uint32
ap_phrase_chksum (caddr_t ptext, encodedlang_handler_t * elh__UTF8, int *errcode_ret)
{
  uint32 chksum_acc[2];
  chksum_acc[0] = chksum_acc[1] = 0;
  elh__UTF8->elh_iterate_patched_words (ptext, box_length (ptext) - 1,
      elh__UTF8->elh_unicoded_language->lh_is_vtb_word,
      elh__UTF8->elh_unicoded_language->lh_toupper_word, ap_phrase_chksum_callback, (void *) chksum_acc);
  if (0 == chksum_acc[1])
    {
...
int
box_serialization_length_est (caddr_t box, int threshold)
{
  dtp_t box_dtp;
  int len;
...
static const char *ap_delete1__text = "delete from DB.DBA.SYS_ANN_PHRASE where AP_APS_ID = ? and AP_CHKSUM = ? and AP_TEXT = ?";
...
void
aps_add_phrases (query_instance_t * qst, ap_set_t * aps, caddr_t ** descrs)
{
  ...
  for (ctr = ap_count; ctr--; /* no step */ )
    {
	  ...
	  (BOX_ELEMENTS (descrs[ctr]) < 1) || (BOX_ELEMENTS (descrs[ctr]) > 2) || (DV_STRING != DV_TYPE_OF (descrs[ctr][0])))
	{
	  ap->app_text = NULL;
  ...
  for (ctr = 0; ctr < ap_count; ctr++)	/* This is incremental to process in ascending order of checksums */
      ...
      if (AP_DELETE_IT == ap->app_link_data)
	{
	  ...
	  err = qr_quick_exec (ap_delete1__qr, qst->qi_client, "", NULL, 3,
	      ":0", (ptrlong) ap->app_set->aps_id, QRP_INT, ":1", (ptrlong) ap->app_chksum, QRP_INT, ":2", ap->app_text, QRP_STR);
	  if ((caddr_t) SQL_SUCCESS != err)
	    goto errexit;
	  ...
	  err = qr_quick_exec (ap_find_bitX_sample__qr, qst->qi_client, "", &lc, 3,
	      ":0", (ptrlong) ap->app_set->aps_id, QRP_INT, ":1", (ptrlong) minchksum, QRP_INT, ":2", (ptrlong) maxchksum, QRP_INT);
	  if ((caddr_t) SQL_SUCCESS != err)
	    goto errexit;
	  if (!lc_next (lc))
      ...
      else
	{
	  ...
	  ins_qr = (
		  ...
		  (BOX_SERIALIZATION_IMPOSSIBLE == est_len)) ? ap_insert_ext__qr : ap_insert_short__qr);
	  err = qr_quick_exec (ins_qr, qst->qi_client, "", NULL, 4,
	      ":0", (ptrlong) ap->app_set->aps_id, QRP_INT,
	      ":1", (ptrlong) ap->app_chksum, QRP_INT,
	      ":2", (ptrlong) ap->app_text, QRP_STR, ":3", box_copy_tree (ap->app_link_data), QRP_RAW);
	  if ((caddr_t) SQL_SUCCESS != err)
	    goto errexit;
	}
    }
...
ap_proc_inst_t *
appi_create (query_instance_t * qi, caddr_t source_UTF8, ap_set_t ** sets, int set_count, lang_handler_t * lh)
{
  int set_ctr;
...
ap_arrow_t *
appi_add_arrow (int type, unsigned apa_start, unsigned apa_end, unsigned htmltm_bits, int innermost_tag, ap_proc_inst_t * appi)
{
  ap_arrow_t *arr;
...
void
appi_add_word_arrow (const utf8char * buf, size_t bufsize, unsigned apa_start, unsigned apa_end, unsigned htmltm_bits,
    int innermost_tag, ap_proc_inst_t * appi)
{
  ap_arrow_t *arr;
  ...
  uint32 word_hash;
  check_buf_length =
      eh_decode_buffer__UTF8 (check_buf, WORD_MAX_CHARS, (__constcharptr *) (&check_src_begin), (const char *) (buf + bufsize));
  ...
  chksum_end =
      (utf8char *) eh_encode_buffer__UTF8 (patch_buf, patch_buf + patch_buf_length, (char *) chksum_buf,
      (char *) (chksum_buf + BUFSIZEOF__UTF8_WORD));
  ...
  for (word_idx = 0; word_idx < max_phrase_len; word_idx++)
      ...
      for (aps_ctr = appi->appi_set_count; aps_ctr--; /* no step */ )
	{
	  ...
	  if (!(aps->aps_bitarrays.apb_arrayX[idxX >> 5] & (1 << (idxX & 0x1F))))
	    {
	      dbg_printf (("   failed in X that is 0x%08lx, ofs %ld\n", (unsigned long) (aps->aps_bitarrays.apb_arrayX[idxX >> 5]),
		      (long) (idxX >> 5)));
	      continue;
	    }
	  ...
	  aph->aph_first_idx =
	      appi->appi_place_idx_last[(appi->appi_place_last_ffree_idx + APPI_MAX_PHRASE_NONNOISE - (word_idx +
		      1)) % APPI_MAX_PHRASE_NONNOISE];
	  aph->aph_last_idx =
	      appi->appi_place_idx_last[(appi->appi_place_last_ffree_idx + APPI_MAX_PHRASE_NONNOISE -
		  1) % APPI_MAX_PHRASE_NONNOISE];
	  ...
	  do
	    {
	      ...
	      caddr_t fragm =
		  box_dv_short_nchars (appi->appi_source_UTF8 + start_word->apa_start, end_word->apa_end - start_word->apa_start);
...
void
appi_prepare_to_class_callbacks (ap_proc_inst_t * appi)
{
  ...
  while (candidate_hit_ctr < candidate_hit_count)
      ...
      for (next_diff_hit_ctr = candidate_hit_ctr + 1; next_diff_hit_ctr < candidate_hit_count; next_diff_hit_ctr++)
	{
	  ...
	  ":0", (ptrlong) aps_id, QRP_INT, ":1", (ptrlong) chksum, QRP_INT);
      if ((caddr_t) SQL_SUCCESS == err)
	{
	  while (lc_next (lc))
	    {
...
caddr_t
appi_prepare_match_list (ap_proc_inst_t * appi, int collapse_flags)
{
  ...
  for (aps_ctr = 0; aps_ctr < aps_count; aps_ctr++)
      ...
      if (NULL == m_apc[apc_idx])
	{
	  ...
	  m_apc[apc_idx] = list (3,
	      box_dv_short_string (apc->apc_name), box_dv_short_string (apc->apc_callback), box_copy_tree (apc->apc_app_env));
	}
      m_aps[aps_ctr * 2] = box_num (aps->aps_id);
      m_aps[aps_ctr * 2 + 1] = list (4,
	  box_dv_short_string (aps->aps_name),
	  box_num (aps->aps_class->apc_id), box_num (apc_idx), box_copy_tree (aps->aps_app_env));
  ...
  for (app_ctr = 0; app_ctr < app_count; app_ctr++)
    {
	  ...
	  box_dv_short_string (app->app_text), box_copy_tree (app->app_link_data));
  ...
  for (apa_fctr = 0; apa_fctr < apa_fcount; apa_fctr++)
      ...
      if (NULL != hits)
	{
	  apa_itm = list (6,
	      ...
	      box_num (apa->apa_end), box_num (apa->apa_htmltm_bits), box_num (apa->apa_innermost_tag), hits);
	}
      else
	{
	  apa_itm = list (5,
	      box_num (apa->apa_is_markup),
	      box_num (apa->apa_start), box_num (apa->apa_end), box_num (apa->apa_htmltm_bits), box_num (apa->apa_innermost_tag));
	}
  ...
  while (aph_ctr--)
    {
	  ...
	  box_num ((ptrlong) gethash (aph->aph_.aph_confirmed.aph_phrase, app_hash)), box_num (aph->aph_.aph_confirmed.aph_prev));
...
void
appi_word_cbk_ptext (const utf8char * buf, size_t bufsize, void *userdata)
{
  ...
  if (NULL != appi->appi_vtb)
    {
      ...
      lh_iterate_patched_words (vtb->vtb_default_eh,
	  appi->appi_lh,
	  (const char *) buf, bufsize,
	  appi->appi_lh->lh_is_vtb_word,
	  appi->appi_lh->lh_normalize_word, (lh_word_callback_t *) vtb_hash_string_ins_callback, (void *) vtb);
    }
}


void
appi_markup_ptext (ap_proc_inst_t * appi)
{
  appi->appi_elh_UTF8->elh_iterate_words (appi->appi_source_UTF8, box_length (appi->appi_source_UTF8) - 1,
...
void
appi_element (void *userdata, const char *name, vxml_parser_attrdata_t * attrdata)
{
  ...
  curr_apa = appi_add_arrow (APA_OPENING_TAG, start, end, htmltm_bits, ah->ah_arrow_idx[ah->ah_depth - 1], appi);
...
void
appi_element_end (void *userdata, const char *name)
{
  appi_html_ctx_t *ah = (appi_html_ctx_t *) userdata;
  ap_proc_inst_t *appi = ah->ah_appi;
  ap_arrow_t *curr_apa;
  ...
  curr_apa = appi_add_arrow (APA_CLOSING_TAG, start, end, htmltm_bits, ah->ah_arrow_idx[ah->ah_depth - 1], appi);
  ah->ah_depth -= 1;
}

void
appi_id (void *userdata, const char *name)
{
}

void
appi_word_cbk_html (const utf8char * buf, size_t bufsize, void *userdata)
{
  appi_html_ctx_t *ah = (appi_html_ctx_t *) userdata;
  ap_proc_inst_t *appi = ah->ah_appi;
  ...
  appi_add_word_arrow (buf, bufsize,
      apa_start, apa_end, ah->ah_tags[ah->ah_depth - 1]->apa_htmltm_bits, ah->ah_arrow_idx[ah->ah_depth - 1], ah->ah_appi);
  if (NULL != appi->appi_vtb)
    {
      vt_batch_t *vtb = appi->appi_vtb;
      lh_iterate_patched_words (vtb->vtb_default_eh,
...
void
appi_character (void *userdata, const char *s, size_t len)
{
  appi_html_ctx_t *ah = (appi_html_ctx_t *) userdata;
  ap_proc_inst_t *appi = ah->ah_appi;
  ...
  appi->appi_elh_UTF8->elh_iterate_words (s, len, appi->appi_lh->lh_is_vtb_word, appi_word_cbk_html, ah);
...
appi_other (void *userdata)
{
  ...
  int end = ah->ah_prev_end = VXmlGetCurrentByteNumber (ah->ah_parser);
  unsigned htmltm_bits = ah->ah_tags[ah->ah_depth - 1]->apa_htmltm_bits;
  curr_apa = appi_add_arrow (APA_OTHER, start, end, htmltm_bits, ah->ah_arrow_idx[ah->ah_depth - 1], appi);
}

void
appi_pi (void *userdata, const char *target, const char *data)
{
  appi_other (userdata);
}

void
appi_comment (void *userdata, const char *text)
{
  appi_other (userdata);
}

void
appi_entity (void *userdata, const char *refname, size_t reflen, int isparam, const xml_def_4_entity_t * edef)
{
  appi_other (userdata);
}

void
appi_markup_html (ap_proc_inst_t * appi, int is_html, caddr_t * err_ret)
{
  ...
  config.dtd_config =
      mp_box_string (appi->appi_mp, "Validation=DISABLE ErrorContext=DISABLE BuildStandalone=DISABLE Include=DISABLE");
...
static const char *apc_select_max_id__text = "select max (APC_ID) from DB.DBA.SYS_ANN_PHRASE_CLASS";

static query_t *aps_select_max_id__qr = NULL;
static const char *aps_select_max_id__text = "select max (APS_ID) from DB.DBA.SYS_ANN_PHRASE_SET";
...
ap_global_init (query_instance_t * qst)
{
  ...
  sql_compile_many (10, 1,
      ...
      ap_find_bitX_sample__text, &ap_find_bitX_sample__qr, app_find_by_hit__text, &app_find_by_hit__qr, NULL);
...
bif_ap_class_status (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (new_status)
    {
    case 0:
      apc_unregister ((query_instance_t *) qst, apc_id);
      return box_num (0);
    case 1:
...
bif_ap_set_status (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (new_status)
    {
    case 0:
      aps_unregister ((query_instance_t *) qst, aps_id);
      return box_num (0);
    case 1:
...
bif_ap_phrase_chksum (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL == elh)
    sqlr_new_error ("42000", "APG08",
	"Language '%.300s' has no accelerated UTF-8 support and can not be used for text annotation routines", lang_name);
...
bif_ap_build_match_list (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (is_html < 0 || is_html > DEAD_HTML)
    sqlr_new_error ("22023", "APG11",
	"The flag for HTML mode must be between 0 and 2, use 0 to indicate plain text 1 or 2 for HTML mode");
