...
int itc_page_search (it_cursor_t * it, buffer_desc_t ** buf_ret, dp_addr_t * leaf_ret, int skip_first_key_cmp);
...
void
itc_sqlr_error (it_cursor_t * itc, buffer_desc_t * buf, const char *code, const char *msg, ...)
#ifdef __GNUC__
...
#define RAB_SPECULATIVE 1	/* to indicate speculative read ahead with random lookup, buffers should be inited fairly old and no touch for ones already in */
     ...
     int em_ext_ra_pages (extent_map_t * em, it_cursor_t * itc, dp_addr_t ext_dp, dp_addr_t * leaves, int max, dp_addr_t except_dp,
    dk_hash_t * except);
...
#define DBGP_ARGS_0
    ...
    buffer_desc_t * buf_from, buffer_desc_t ** buf_ret, int mode, int max_change);
     ...
     buffer_desc_t *page_try_transit (it_cursor_t * it, buffer_desc_t * from, dp_addr_t dp, int mode);
...
/* read.c */
     ...
     void itc_check_col_prefetch (it_cursor_t * itc, buffer_desc_t * buf);
...
/* disk.c */
     ...
     void gen_qsort (int *in, int *left, int n_in, int depth, sort_cmp_func_t cmp, void *cd);
     void gen_qmsort (int *in, int *left, int n_in, sort_cmp_func_t cmp, void *cd, int key_bytes);
...
#define dbs_locate_incbackup_bit(dbs, near_dp, array, page_no, inx, bit) \
  dbs_locate_page_bit(dbs, &(dbs)->dbs_incbackup_set, near_dp, array, page_no, inx, bit, V_EXT_OFFSET_INCB_SET, 1)
     ...
     int dbs_locate_page_bit (dbe_storage_t * dbs, buffer_desc_t ** free_set, dp_addr_t near_dp,
...
#define pg_check_map(buf)
     ...
     void itc_insert_dv (it_cursor_t * it, buffer_desc_t ** buf, row_delta_t * rd, int is_recursive, row_lock_t * new_rl);
...
/* page.c */
     ...
     short pfh_var (pf_hash_t * pfh, dbe_col_loc_t * cl, db_buf_t str, int len, unsigned short *prefix_bytes,
    unsigned short *prefix_ref, dtp_t * extra, int mode);
...
#define key_col_in_layout_seq(key,col) key_col_in_layout_seq_1(key, col, 1)
     void kc_var_col (dbe_key_t * key, buffer_desc_t * buf, db_buf_t row, dbe_col_loc_t * cl, db_buf_t * p1, row_size_t * len1,
    db_buf_t * buf2, row_size_t * len2, unsigned short *offset);
...
/* row.c */
     ...
     void row_insert_cast (row_delta_t * rd, dbe_col_loc_t * cl, caddr_t data, caddr_t * err_ret, db_buf_t old_blob);
     void row_insert_cast_temp (row_delta_t * rd, dbe_col_loc_t * cl, caddr_t data, caddr_t * err_ret, db_buf_t old_blob);
    ...
    dtp_t extra);
     ...
     int itc_row_insert (it_cursor_t * it, row_delta_t * rd, buffer_desc_t ** unq_buf, int blobs_in_place, int pk_only);
...
#define IS_REMOTE_ROUTINE_QR(qr) ((qr) && (qr)->qr_is_remote_proc)
     ...
     void row_print_blob (it_cursor_t * itc, caddr_t thing, dk_session_t * ses, dbe_column_t * col, caddr_t * err_ret);
...
#define LOG_TRAIL_CL_FLT 2	/* history as far as needed for cluster fault tolerance */
     ...
     extern int32 pop3s_port;
     extern int imap_port;
     extern int32 imaps_port;
...
/* ddlrun.c */
     ...
     caddr_t it_read_object_schema (index_tree_t * it, lock_trx_t * lt, dbe_schema_t * sp);
...
#define blob_layout_free(b) dbg_blob_layout_free (__FILE__, __LINE__, b)
     ...
     box_t blob_layout_ctor (dtp_t blob_handle_dtp, dp_addr_t start, dp_addr_t dir_start, int64 length, int64 diskbytes,
    index_tree_t * it);
     ...
     int bh_fill_buffer_from_blob (index_tree_t * it, lock_trx_t * lt, blob_handle_t * bh, caddr_t outbuf, long get_bytes);
...
/* Low-level read or write of a blob page, e.g., for random access */
     extern int page_wait_blob_access (it_cursor_t * itc, dp_addr_t dp_to, buffer_desc_t ** buf_ret, int mode, blob_handle_t * bh,
    int itc_map_wrap);
...
#define VD_ARRAY_PARAMS_ALL  2
     ...
     typedef enum
     { SQW_OFF, SQW_ON, SQW_ERROR } sqw_mode;
