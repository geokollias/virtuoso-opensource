...
#define HT_ENTER(h) rwlock_wrlock (h->ht_rwlock)
#define HT_LEAVE(h) rwlock_unlock (h->ht_rwlock)
...
get_compiled_regexp (id_hash_t * c_r, const char *pattern, int options, caddr_t * err_ret)
{
  ...
  if (NULL == tmp.code)
    {
      if (error)
	err_ret[0] = srv_make_new_error ("2201B", "SR098", "regexp error at \'%s\' column %d (%s)", pattern, erroff, error);
      else
	err_ret[0] = srv_make_new_error ("2201B", "SR098", "regexp error at \'%s\' column %d", pattern, erroff);
      return NULL;
    }
