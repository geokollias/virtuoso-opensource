...
void bif_to_xml_array_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, dk_set_t * ret_set, dk_set_t * head_set);
...
xn_box_attrs (xp_node_t * xn, caddr_t name, vxml_parser_attrdata_t * attrdata, int reserve_attrs)
{
  ...
  fill = 0;
  xn->xn_attrs = box =
      (caddr_t *) dk_alloc_box (((n_attrs + ((NULL != save_ns) ? 1 : 0) + reserve_attrs) * 2 + 1) * sizeof (caddr_t),
      DV_ARRAY_OF_POINTER);
  ...
  for (inx = 0; inx < n_attrs; inx++)
    {
      ...
      if (xml_is_sch_qname (box[0], nsname) || xml_is_soap_qname (box[0], nsname) || xml_is_wsdl_qname (box[0], nsname))
...
void *
xmlap_xpath (void *userData, const char *elname, const char *attrname, const char *attrvalue)
{
  GPF_T;
  return NULL;
}
...
void *
xmlap_qname (void *userData, const char *elname, const char *attrname, const char *attrvalue)
{
  GPF_T;
  return NULL;
}
...
nss_free (id_hash_t * nss)
{
  ...
  if (!nss)
    return;
  for (id_hash_iterator (&hit, nss); hit_next (&hit, (caddr_t *) & curr, (caddr_t *) & namespaces);
      /* */ )
    {
      if (namespaces)
...
xp_entity (vxml_parser_t * parser, const char *refname, int reflen, int isparam, const xml_def_4_entity_t * edef)
{
  ...
  head = ((NULL != uri) ?
	  ...
	  uname__bang_uri,
	  box_dv_short_string (uri)) : list (3, uname__ref, uname__bang_name, box_dv_short_nchars (refname, reflen)));
...
xp_pi (vxml_parser_t * parser, const char *target, const char *data)
{
  ...
  dk_set_push (&xp->xp_current->xn_children,
      (void *) ((NULL != data) ? list (2, head, box_dv_short_string (data)) : list (1, head)));
  xp->xp_current->xn_n_children++;
  if (xp->xp_current->xn_n_children >= MAX_BOX_ELEMENTS)
    xn_error (xp->xp_current, "The number of children elements is over the limits");
}
...
xp_comment (vxml_parser_t * parser, const char *text)
{
  ...
  XP_STRSES_FLUSH (xp);
  if ('\0' != text[0])
    dk_set_push (&xp->xp_current->xn_children, (void *) list (2, list (1, uname__comment), box_dv_short_string (text)));
  else
  ...
  if (xp->xp_namespaces)
    {
      ...
      caddr_t *tag;
      caddr_t *nss;
      for (id_hash_iterator (&iter, xp->xp_namespaces); hit_next (&iter, (caddr_t *) & tag, (caddr_t *) & nss);
	  /* */ )
	{
	  dk_free_tree (nss[0]);
	}
...
caddr_t
xml_make_tree_with_ns (query_instance_t * qi, caddr_t text, caddr_t * err_ret, const char *enc, lang_handler_t * lh,
...
make_tree:
  ...
  context.xp_strses = strses_allocate ();
  context.xp_top = xn;
  context.xp_namespaces = id_hash_allocate (31, sizeof (caddr_t *), sizeof (caddr_t *), voidptrhash, voidptrhashcmp);
  context.xp_namespaces_are_valid = 1;
	...
	if (!strcmp (attr, "id") || !strcmp (attr, SOAP_ENC_SCHEMA12 ":id"))
	...
	else if (!strcmp (attr, "href") || !strcmp (attr, SOAP_ENC_SCHEMA12 ":ref"))
	  {
		...
		if (!strcmp (value, id) && !*err_ret)
		  {
		    *err_ret = srv_make_new_error ("22023", "A ref attribute item and an id item on the same element", "SR363");
		  }
	      }
	    n_hrefs++;
	  }
		  ...
		  if (val_inx % 2 == 0 && nxt && nxt->data && !strcmp (value, id))
		    {
		      ...
		      caddr_t *new_tree = (caddr_t *) dk_alloc_box_zero ((old_child_len + new_child_len - 1) * sizeof (caddr_t),
			  DV_ARRAY_OF_POINTER);
		      caddr_t *new_attrs = (caddr_t *) dk_alloc_box_zero ((new_attr_len - 2) * sizeof (caddr_t),
			  DV_ARRAY_OF_POINTER);
		...
		if (!found)
		  {
		    if (!*err_ret)
		      {
			*err_ret = srv_make_new_error ("22023", "Missing reference in source XML tree", "SR362");
		      }
		  }
	      }
	    else
  ...
  ( !strncmp ((strg), net_loc, net_loc_len) && \
   (('/' == (strg)[net_loc_len]) || ('\0' == (strg)[net_loc_len])) )
	  ...
	  if (tmp[0] == '/')
	      ...
	      if (tmp[1] == '/')	/* '//' indicates relative URI with new network address */
		{
		  if (IS_NET_LOC_MATCHES (tmp + 2))
		  ...
		  else
		    {
		      ...
		      strcpy_size_ck (box[fill + 2] + schema_len, tmp + 2, box_length (box[fill + 2]) - schema_len);
		    }
		}
	    ...
	    box[fill + 2] = local_to_relative (tmp, path);
	  else if ((0 == strncasecmp (tmp, schema, schema_len)) && IS_NET_LOC_MATCHES (tmp + schema_len))
...
caddr_t
xml_make_mod_tree (query_instance_t * qi, caddr_t text, caddr_t * err_ret, long html_mode, caddr_t uri, const char *enc,
    lang_handler_t * lh, caddr_t dtd_config, dtd_t ** ret_dtd, id_hash_t ** ret_id_cache, xml_ns_2dict_t * ret_ns_2dict)
{
  int dtp_of_text = box_tag (text);
  dk_set_t top;
...
_bif_xml_tree_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int mode)
{
  ...
  do
    {
	  ...
	  (dtp_of_text_arg == DV_LONG_STRING) || (dtp_of_text_arg == DV_C_STRING) || (dtp_of_text_arg == DV_BIN))
      ...
      if (dtp_of_text_arg == DV_STRING_SESSION)
	{
	  ...
	  if (XE_XPER_SERIALIZATION == ses_sort)
	    sqlr_error ("42000",
		"A string session with persistent XML data passed to function %s() as argument 1; try xml_persistent function instead",
		bif_name);
	  if (XE_XPACK_SERIALIZATION == ses_sort)
		...
		sqlr_error ("42000", "The BLOB passed to a function %s() contains corrupted packed XML serialization data",
		    bif_name);
  ...
  tree =
      xml_make_mod_tree ((query_instance_t *) qst, text_arg, (caddr_t *) & err, parser_mode, uri, enc, lh, dtd_config, dtd_ptr,
      id_cache_ptr, &ns_2dict);
  if (NULL == tree)
    {
...
void
xp_insert_text (xparse_ctx_t * xp, caddr_t text, int level, caddr_t * err_ret)
{
  GPF_T;
}
...
tb_make_iq (dbe_table_t * tb, query_instance_t * qi)
{
...

  {
    ...
    snprintf (text, sizeof (text), "insert into \"%s\" (E_ID, E_LEVEL, E_NAME, E_MISC, E_WHITESPACE, E_LEADING, E_TRAILING ",
	tb->tb_name);
...
void
xp_insert_elt (xparse_ctx_t * xp, caddr_t * tree, int level, caddr_t * err_ret)
{
  GPF_T;
}
...
bx_out_nsdecls_of_2dict (caddr_t * qst, dk_session_t * out, close_tag_t * ct, xml_ns_2dict_t * xd_ns_2dict, caddr_t excl_res_prefx)
{
  ...
  while (tail < token_buf_end)
    {
      char *pfx;
      while (((unsigned char) (' ') >= (unsigned char) (tail[0])) && (tail < token_buf_end))
	tail++;
      pfx = tail;
      while (((unsigned char) (' ') < (unsigned char) (tail[0])) && (tail < token_buf_end))
	tail++;
...
bx_out_value (caddr_t * qst, dk_session_t * out, db_buf_t val, wcharset_t * tgt_charset, wcharset_t * src_charset, int dks_esc_mode)
{
  if (DV_STRINGP (val))
	...
	text = ((caddr_t *) text)[0];
      dks_esc_write (out, text, box_length (text) - 1, tgt_charset, src_charset, dks_esc_mode);
    }
	...
	varchar = (caddr_t) list (3, (ptrlong) DV_LONG_STRING, 0, 0);
      ...
      dks_esc_write (out, string, box_length (string) - 1, tgt_charset, src_charset, dks_esc_mode);
      dk_free_box (string);
    }
}
...
/* Now we know we have a namespace and we need a prefix */
  ...
  if (!is_attr && (NULL != ct->ct_all_default_ns))
    {
      ...
      if ((NULL != dflt_ns) && (strlen (dflt_ns) == (size_t) ns_len) && 0 == memcmp (dflt_ns, name, ns_len))
	{
	  pref = NULL;
	  goto pref_is_set;	/* see below */
	}
  ...
  if (0 != xsst->xsst_ns_2dict.xn2_size)
    {
      ptrlong idx =
	  ecm_find_name (uri, xsst->xsst_ns_2dict.xn2_uri2prefix, xsst->xsst_ns_2dict.xn2_size, sizeof (xml_name_assoc_t));
  ...
  for (inx = 1; inx < len; inx += 2)
      ...
      if (' ' == tag[inx][0])
	{
	  if (!strcmp (tag[inx], uname__bang_exclude_result_prefixes))
	    bx_out_nsdecls_of_2dict (xsst->xsst_qst, ses, xsst->xsst_ct, &(xsst->xsst_entity->xe_doc.xtd->xd_ns_2dict),
		tag[inx + 1]);
	  continue;
	}
      SES_PRINT (ses, " ");
  ...
  if (DV_ARRAY_OF_POINTER == dtp)
      ...
      if (!is_root)
	{
	  if (xsst->xsst_do_indent)
	    {
	      if (xsst->xsst_in_block == WSP_EXPLICIT || (curr_tag.htmltd_is_block && xsst->xsst_in_block == WSP_COND))
      ...
      if (!is_root)
	{
	  ...
	  if (xsst->xsst_do_indent)	/* xsl:output indent=yes */
		...
		xsst->xsst_indent_depth--;
		  ...
		  (xsst->xsst_in_block == WSP_EXPLICIT || (curr_tag.htmltd_is_block && xsst->xsst_in_block == WSP_COND)))
...
bif_number (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (dtp == DV_LONG_INT || dtp == DV_SINGLE_FLOAT || dtp == DV_DOUBLE_FLOAT || dtp == DV_NUMERIC)
    return (box_copy (n));
...
xml_uri_get (query_instance_t * qi, caddr_t * err_ret, caddr_t * options, ccaddr_t base_uri, ccaddr_t rel_uri, int mode)
{
  ...
  static const char *pl_call_text[3] =
      { "DB.DBA.XML_URI_GET (?, ?)", "DB.DBA.XML_URI_GET_STRING (?, ?)", "DB.DBA.XML_URI_GET_STRING_OR_ENT (?, ?)" };
  caddr_t err = NULL;
  ...
  err = qr_rec_exec (qr[query_idx], qi->qi_client, &lc, qi, NULL, 2,
      ":0", ((NULL == base_uri) ? box_dv_short_nchars ("", 0) : box_copy (base_uri)), QRP_RAW, ":1", box_copy (rel_uri), QRP_RAW);
  if (SQL_SUCCESS != err)
    {
      LC_FREE (lc);
      if (err_ret)
...
bif_xml_validate_dtd (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  do
    {
      if ((dtp_of_text_arg == DV_SHORT_STRING) || (dtp_of_text_arg == DV_LONG_STRING) || (dtp_of_text_arg == DV_C_STRING))
	{			/* Note DV_TIMESTAMP_OBJ is not enumerated in if(...), unlike bif_string_arg)_ */
	  text = text_arg;
	  break;
	}
      ...
      if (dtp_of_text_arg == DV_STRING_SESSION)
	{
	  ...
	  if (MAX_XML_STRING_LENGTH < len)
	    sqlr_error ("42000", "Unable to process STRING SESSION by function xml_validate_dtd, it's length (%ld) is too long",
		(long) len);
...
shuric_t *
shuric_alloc__xmlschema (void *env)
{
...
caddr_t
shuric_uri_to_text__xmlschema (caddr_t uri, query_instance_t * qi, void *env, caddr_t * err_ret)
{
  caddr_t resource_text;
  if (NULL == qi)
    {
      err_ret[0] =
	  srv_make_new_error ("37XQR", "SQ197", "Unable to retrieve '%.1000s' from SQL compiler due to danger of fatal deadlock",
	  uri);
    }
...
void
shuric_parse_text__xmlschema (shuric_t * shuric, caddr_t uri_text_content, query_instance_t * qi, void *env, caddr_t * err_ret)
{
  ...
  QR_RESET_CTX
  {
	...
	err_ret[0] =
	    srv_make_new_error ("42000", "SQ199",
	    "Unable to compile schema '%.300s'.\n%.1000s\nFunction xml_load_schema_decl() can return full list of these errors.",
	    shuric->shuric_uri, msg);
	dk_free_box (msg);
      }
    else
      {
...
void
shuric_destroy_data__xmlschema (shuric_t * shuric)
{
...
bif_xml_load_schema_decl_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, const char *fname, int mode)
{
  ...
  if (err)
    {
      ...
      caddr_t res;
      snprintf (restmp, restmp_len, "Unable to resolve reference URI '%.300s' with base URI '%.300s'", schema_decl_uri,
	  schema_decl_base);
  ...
  if ((RELOAD_SCHEMA_DECL != mode) && (RELOAD_MAPPING_SCHEMA_DECL != mode))
    {
      schema_decl_text = xml_uri_get ((query_instance_t *) qst, &err, NULL, schema_decl_base, schema_decl_uri, 1);
    }
  if (err)
    {
      size_t restmp_len = strlen (schema_decl_base) + strlen (schema_decl_uri) + 100;
  ...
  QR_RESET_CTX
  {
	...
	if ((LOAD_SCHEMA_DECL != mode) && (RELOAD_SCHEMA_DECL != mode) && (TABLES_FROM_MAPPING_SCHEMA_DECL != mode))
	  {
	    ...
	    if (NULL == xml_mapping_view)
	      sqlr_new_error ("42000", "SQ202", "The XML schema '%s' is valid but can not be used as a mapping schema.",
		  schema_decl_path);
	  }
    ...
    if ((LOAD_SCHEMA_DECL != mode) && (RELOAD_SCHEMA_DECL != mode) && !xmlparser_is_ok (parser))
      sqlr_new_error ("42000", "SQ176",
	  "Unable to compile mapping schema '%s'. Function xml_load_schema_decl() can return full list of these errors.",
	  schema_decl_path);
    if (err)
      sqlr_resignal (err);
    log = VXmlValidationLog (parser);
  ...
  if ((LOAD_SCHEMA_DECL != mode) && (RELOAD_SCHEMA_DECL != mode) && (TABLES_FROM_MAPPING_SCHEMA_DECL != mode))
    {
      ...
      if (NULL != err)
	{
	  sqlr_new_error ("42000", "SQ176", "Unable to declare an XML view '%s' from '%s': %s", raw_view_name, schema_decl_path,
	      err);
	}
    }
  if (LOAD_MAPPING_SCHEMA_DECL == mode)
    {
      caddr_t reload_text;
      int buf_len = 100 + 2 * (box_length (schema_decl_base) + box_length (schema_decl_uri) +
	  box_length (enc) + box_length (lang) +
	  box_length (schema_decl_text) + ((xsd_prefix && xsd_postfix) ? (box_length (xsd_prefix) + box_length (xsd_prefix)) : 0));
...
bif_xml_validate_schema (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  do
      ...
      if (dtp_of_text_arg == DV_STRING_SESSION)
	{
	  ...
	  long len = strses_length (ses_arg);
	  if (MAX_XML_STRING_LENGTH < len)
	    sqlr_error ("42000", "Unable to process STRING SESSION by function xml_validate_schema, it's length (%ld) is too long",
		(long) len);
	  text = strses_string (ses_arg);
	  text_is_temporary = 1;
	  break;
...
caddr_t DBG_NAME (box_cast_to_UTF8) (DBG_PARAMS caddr_t * qst, caddr_t data)
{
...
box_cast_to_UTF8_xsd (caddr_t * qst, caddr_t data)
{
  ...
  switch (DV_TYPE_OF (data))
    ...
    case DV_SINGLE_FLOAT:
      boxdbl = (double) (unbox_float (data));
      goto make_double;		/* see below */
    case DV_DOUBLE_FLOAT:
      boxdbl = unbox_double (data);
      goto make_double;		/* see below */
    default:
      return box_cast_to_UTF8 (qst, data);
    }
make_double:
  ...
  if ((NULL == strchr (tmpbuf, '.')) && (NULL == strchr (tmpbuf, 'E')) && (NULL == strchr (tmpbuf, 'e')))
      ...
      if (isalpha (tmpbuf[1 + 1]))
	{
	  ...
	  if (myPOSINF_d == boxdbl)
	    return box_dv_short_string ("INF");
	  else if (myNEGINF_d == boxdbl)
	    return box_dv_short_string ("-INF");
	  else
	    return box_dv_short_string ("NAN");
	}
      else
	{
...
box_cast_to_UTF8_uname (caddr_t * qst, caddr_t raw_name)
{
  switch (DV_TYPE_OF (raw_name))
    {
    case DV_UNAME:
      return box_copy (raw_name);
    case DV_STRING:
      return box_dv_uname_nchars (raw_name, box_length (raw_name) - 1);
    case DV_WIDE:
    case DV_LONG_WIDE:
      {
...
caddr_t
string_concat (caddr_t first, caddr_t second)
{
  caddr_t result;
...
void
dk_check_xte_nodebld_acc (caddr_t * acc)
{
...
/* The following 'if' must not appear here, but this is a workaround for a weird error in aggr in nested select. */
  ...
  if (new_acc_length > MAX_BOX_ELEMENTS)
    sqlr_new_error ("22003", "SR346",
	"Out of memory allocation limits: the composed XML contains a node that have too many children");
	...
	case DV_OBJECT:
	  {
		...
		for (new_acc_length = acc_length; (new_filled_count) >= new_acc_length;
		    new_acc_length += (new_acc_length + 1)) /* do nothing */ ;
		if (acc_length != new_acc_length)
		  {
		    caddr_t new_acc;
		    if (new_acc_length > MAX_BOX_ELEMENTS)
  ...
  if (2 <= arg_ctr)
    {
      ...
      if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (head)) || (1 != (BOX_ELEMENTS (head) % 2))
	  || (DV_UNAME != DV_TYPE_OF (((caddr_t *) head)[0])))
	{
	  dk_free_tree ((caddr_t) head);
	  sqlr_new_error ("42000", "XTE04",
	      "The second argument of function xte_nodebld_final() should be a value returned by xte_head()");
	}
    }
  ...
  for (idx = filled_size / sizeof (caddr_t); --idx > 0; /* no step */ )
      ...
      if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (item))
	{
	  ...
	  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (head))
	    ...
	    {
	      dk_free_tree ((caddr_t) acc);
	      sqlr_new_error ("22003", "SR621",
		  "Attribute can not appear directly in an XML entity aggregate, it should be part of an XML element");
	    }
/*
            {
...
*/
	    ...
	    {
	      dk_free_tree ((caddr_t) acc);
	      sqlr_new_error ("22003", "SR620",
		  "Inappropriate value is found in XML entity aggregate (neither vector of XML data nor a literal)");
	    }
	}
    }
...
bif_vectorbld_concat_acc_impl (caddr_t * qst, state_slot_t ** args, int preserve_args, caddr_t ** acc_ptr)
{
  ...
  for (arg_inx = 1; arg_inx < argcount; arg_inx++)
	...
	case DV_DB_NULL:
	  break;
	default:
	  sqlr_new_error ("22003", "SR475", "The argument %d of vectorbld_concat_acc() is of type %d (%s), should be array or null",
	      arg_inx + 1, addon_dtp, dv_type_title (addon_dtp));
	}
    }
/* The following 'if' must not appear here, but this is a workaround for a weird error in aggr in nested select. */
  if (NULL == acc)
...
bif_vectorbld_crop (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t *res;
  if (from > to)
    sqlr_new_error ("22003", "SR477",
	"Argument #2 (value %ld) is greater than argument #3 (value %ld) in call of vectorbld_crop ()", from, to);
  if (from < 0)
  ...
  if (to > old_len)
    sqlr_new_error ("22003", "SR477", "Argument #3 (value %ld) is greater than the length of vector in call of vectorbld_crop ()",
	to);
  if (from == to)
    {
...
{
  return bif_vectorbld_acc_impl (qst, args, bif_vectorbld_acc_impl_BIT_PRESERVE_ARGS | bif_vectorbld_acc_impl_BIT_IGNORE_NULLS,
      ((caddr_t **) (QST_GET_ADDR (qst, args[0]))));
}
...
static int
xq_box_recursive_length (caddr_t box)
{
	...
	for (ctr = 0; ctr < len; ctr++)
	  {
		...
		continue;
	      case DV_ARRAY_OF_POINTER:
		res += xq_box_recursive_length (((caddr_t *) box)[ctr]);
		continue;
	      default:
		res++;
	      }
...
static void
xq_box_flatten (caddr_t * qst, caddr_t ** box_ptr, caddr_t ** dst_ptr, int boxes_are_utf8)
{
  int ctr, len;
	...
	for (ctr = 0; ctr < len; ctr++)
	  {
		...
		continue;
	      case DV_ARRAY_OF_XQVAL:
		xq_box_flatten (qst, (caddr_t **) (box_ptr[0] + ctr), dst_ptr, 1);
		continue;
	      case DV_ARRAY_OF_POINTER:
		xq_box_flatten (qst, (caddr_t **) (box_ptr[0] + ctr), dst_ptr, 0);
		continue;
	      case DV_WIDE:
	      case DV_LONG_WIDE:
...
bif_xq_sequencebld_final_impl (caddr_t * qst, state_slot_t ** args, int plain_return)
{
  ...
  if (NODEBLD_ACC_IS_BAD (acc))
	...
	{
	  dk_free_tree ((caddr_t) acc);
	  sqlr_new_error ("22003", "SR348",
	      "The first argument of xq_sequencebld_final is not made by xq_sequencebld_init() function");
	}
    }
  filled_size = sizeof (caddr_t) * (unbox (acc[0]));
  dk_check_vectorbld_acc (acc);
  ...
  if (NODEBLD_ACC_IS_BAD (acc))
    {
      dk_free_tree (tag);
      dk_free_tree ((caddr_t) acc);
      sqlr_new_error ("22003", "SR350",
	  "The second argument of xte_node_from_nodebld() is not made by xte_nodebld_init() function");
    }
...
caddr_t
sql_xml_var_transform (caddr_t box)
{
  int lastcolon = -1, inx;
  int length;
  int new_length;
  ...
  if ((NULL == wide_box) && (new_length == length))
    return box_dv_uname_nchars (box, length);
...
static int
bif_to_xml_array_push_new_attr (dk_set_t * head_set, caddr_t attr_name, int copy_name, caddr_t attr_value)
{
...
void
bif_to_xml_array_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, dk_set_t * ret_set, dk_set_t * head_set)
{
  ...
  switch (dtp)
	...
	if (xte->xe_attr_name)	/*attribute */
	  {
	    bif_to_xml_array_push_new_attr (head_set, xte->xe_attr_name, 1, xte->_->xe_currattrvalue ((xml_entity_t *) xte));
	  }
    ...
    default:
      ...
      *head_set = NULL;
	  ...
	  "Invalid argument type %s (%d) for arg %d to %s", dv_type_title (dtp), (int) dtp, nth + 1, func);
      break;
    }
}
...
bif_xmlelement (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (arg_inx = 1; arg_inx < argcount; arg_inx++)
      ...
      if (DV_ARRAY_OF_POINTER != raw_value_dtp && DV_XML_ENTITY != raw_value_dtp && DV_XML_ENTITY != raw_value_dtp)
	{
      ...
      else if (DV_ARRAY_OF_POINTER == raw_value_dtp && (((caddr_t *) raw_value)[0]) == XMLATTRIBUTE_FLAG && arg_inx > 1)
	{
	  sqlr_new_error ("22003", "SR345", "XMLATTRIBUTES() must be the second argument of XMLELEMENT()");
	}
      else
...
bif_xml_init (void)
{
  xmlschema_dflt_config =
      box_dv_short_string
      ("Validation=RIGOROUS FsaBadWs=IGNORE Fsa=ERROR BuildStandalone=ENABLE SchemaDecl=ENABLE MaxErrors=2000 MaxWarnings=10000");
