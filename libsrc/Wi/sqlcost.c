...
{ \
  ...
  void* __ic_save = THR_ATTR (self, TA_IC);

#define IC_RESTORE SET_THR_ATTR (self, TA_IC, __ic_save); }
...
tb_distinct_key_counts (dbe_table_t * tb)
{
  ...
  {
    if (key->key_distinct || (tb_is_rdf_quad (tb) && !key->key_is_primary))
...
dbe_key_count (dbe_key_t * key)
{
  ...
  if (tb->tb_closest_cset)
    return dbe_key_count (tb->tb_closest_cset->cset_table->tb_primary_key);
  if (key->key_distinct || (tb_is_rdf_quad (tb) && !key->key_is_primary))
    {
  ...
  if (key->key_table->tb_count != DBE_NO_STAT_DATA)
	...
	return 10000;		/* if you know nothing, assume a remote table is 10K rows */
      tb->tb_count_estimate = key_count_estimate (tb->tb_primary_key, tb->tb_primary_key->key_is_elastic ? 1 : 1, 1);
      tb->tb_count_delta = 0;
...
dbe_key_row_cost (dbe_key_t * key, float *rpp, int *key_len)
{
  ...
  DO_SET (dbe_column_t *, k_col, &key->key_parts)
  {
	...
	k_col->col_sqt.sqt_dtp == DV_WIDE || k_col->col_sqt.sqt_dtp == DV_BIN || k_col->col_sqt.sqt_dtp == DV_ANY)
...
dfe_scan_card (df_elt_t * dfe)
{
  ...
  DO_SET (df_elt_t *, pred, &dfe->_.table.col_preds)
  {
	...
	&& 'P' == toupper (pred->_.bin.left->_.col.col->col_name[0]) && (p = pred_rhs_iri (pred)))
      break;
  }
  END_DO_SET ();
  if (!p)
    return dbe_key_count (key);
  found = ric_p_stat_from_cache (dfe_ric (dfe), key, unbox_iri_id (p), p_stat);
  if (found)
    card = p_stat[0];
  if (!found)
    return dbe_key_count (key);
...
dfe_is_index_ord_subq (df_elt_t * dfe)
{
  ...
  for (dfe = dfe; dfe; dfe = dfe->dfe_next)
    {
      if (DFE_TABLE == dfe->dfe_type && !dfe->_.table.hash_role && dfe->_.table.in_order)
	return 1;
    }
  return 0;
}
...
pred_has_inx_subq (df_elt_t ** body)
{
  ...
  if (DV_ARRAY_OF_POINTER == dtp)
      ...
      for (inx = 1; inx < BOX_ELEMENTS (body); inx++)
	{
	  ...
	  if ((DFE_EXISTS == elt->dfe_type || DFE_VALUE_SUBQ == elt->dfe_type) && dfe_is_index_ord_subq ((df_elt_t *) elt))
	    return 1;
...
dfe_prev_tb (df_elt_t * dfe, float *card_between_ret, int stop_on_new_order)
{
  ...
  while (pt)
	...
	{
	  if (DFE_DT == pt->dfe_type || (DFE_VALUE_SUBQ == pt->dfe_type && dfe_is_index_ord_subq (pt)))
	    return NULL;
...
dfe_key_nth_dfe (df_elt_t * dfe, int nth)
{
  ...
  ST *st = t_listst (3, COL_DOTTED, dfe->_.table.ot->ot_new_prefix, col_name);
...
int
dfe_n_in_order (df_elt_t * dfe, df_elt_t * prev_tb, df_elt_t ** prev_ret, float *card_between, int *eq_on_ordering,
    int *cl_colocated)
{
...
dfe_vec_index_unit (df_elt_t * dfe, float spacing)
{
  ...
  if (key->key_is_col)
      ...
      else
	{
	  ...
	  return inx_cost * n_from_top + n_sib * dfe_key_next_cost (dfe, n_in_sibs, 1) + n_page * dfe_key_next_cost (dfe,
	      rows_per_page, 0) + dfe_key_seg_next_cost (dfe, spacing);
	}
    }
  else
    {
      ...
      else
	{
	  ...
	  n_sib = (1 - n_from_top) * (spacing / rows_per_page);
	  n_page = (1 - n_from_top - n_sib);
	  return inx_cost * n_from_top + n_sib * dfe_key_next_cost (dfe, n_in_sibs, 1) + n_page * dfe_key_next_cost (dfe, spacing,
	      0);
	}
    }
}
...
int
dfe_preds_rhs_unq (df_elt_t ** preds, int n_preds)
{
  /* true if right hand side cols in eq predsjoining to the same table  are collectively unique in their table */
  op_table_t *rhs_tb = (op_table_t *) preds[0]->_.bin.right->dfe_tables->data;
  if (!rhs_tb->ot_table)
    return 0;
  DO_SET (dbe_key_t *, key, &rhs_tb->ot_table->tb_keys)
  {
	...
	{
	  int cinx;
	  for (cinx = 0; cinx < n_preds; cinx++)
	    {
	      if (part == preds[cinx]->_.bin.right->_.col.col)
		goto in_key;
	    }
	  goto next_key;
	in_key:
...
{
  /* see if multiple cols of referencing table are unique */
  df_elt_t *preds[10];
  int fill;
  op_table_t *rhs_tb;
  SET_THR_ATTR (THREAD_CURRENT_THREAD, TA_IC, ic);
  DO_SET (df_elt_t *, pred, &dfe->_.table.col_preds)
  {
    if (dfe_is_eq_pred (pred))
      pred->_.bin.is_used = pred->_.bin.is_pk_fk_ref = 0;
  }
  END_DO_SET ();

  DO_SET (df_elt_t *, pred, &dfe->_.table.col_preds)
  {
    if (!dfe_is_eq_pred (pred) || DFE_COLUMN != pred->_.bin.right->dfe_type)
      continue;
    rhs_tb = (op_table_t *) pred->_.bin.right->dfe_tables->data;
    preds[0] = pred;
    fill = 1;
    DO_SET (df_elt_t *, other_pred, &nxt)
    {
      if (!dfe_is_eq_pred (other_pred) || DFE_COLUMN != other_pred->_.bin.right->dfe_type)
	continue;
      if (fill == sizeof (preds) / sizeof (caddr_t))
	break;
      if (rhs_tb == (op_table_t *) other_pred->_.bin.right->dfe_tables->data)
	preds[fill++] = other_pred;
    }
    END_DO_SET ();
    if (fill > 1 && dfe_preds_rhs_unq (preds, fill))
      {
	int cinx;
	df_elt_t **pred_arr = (df_elt_t **) t_alloc_box (fill * sizeof (caddr_t), DV_BIN);
	for (cinx = 0; cinx < fill; cinx++)
	  {
	    pred_arr[cinx] = preds[cinx];
	    preds[cinx]->_.bin.is_pk_fk_ref = 1;
	  }
	t_set_push (&ic->ic_pk_fk_refs, (void *) pred_arr);
      }
  }
  END_DO_SET ();
}
...
int
dfe_pk_fk_ref_card (df_elt_t * pred, float *a1)
{
  index_choice_t *ic = (index_choice_t *) THR_ATTR (THREAD_CURRENT_THREAD, TA_IC);
  if (pred->_.bin.is_pk_fk_ref)
    {
      DO_SET (df_elt_t **, pk, &ic->ic_pk_fk_refs)
      {
	if (-1 != box_position_no_tag ((caddr_t *) pk, (caddr_t) pred))
	  {
	    dbe_table_t *ref_tb = pred->_.bin.right->_.col.col->col_defined_in;
	    dbe_column_t *left_col = pred->_.bin.left->_.col.col;
	    dbe_column_t *right_col = pred->_.bin.right->_.col.col;
	    float this_tb_card = dbe_key_count (left_col->col_defined_in->tb_primary_key);
	    float ref_tb_card = dbe_key_count (ref_tb->tb_primary_key);
	    float cum_card = this_tb_card;
	    float target_card = this_tb_card / ref_tb_card, this_sel;
	    int inx;
	    DO_BOX (df_elt_t *, other_pred, inx, pk)
	    {
	      if (!other_pred->_.bin.is_used)
		continue;
	      cum_card *= other_pred->dfe_arity;
	    }
	    END_DO_BOX;
	    pred->_.bin.is_used = 1;
	    this_sel = 1.0 / MAX (left_col->col_n_distinct, right_col->col_n_distinct);
	    if (cum_card * this_sel >= target_card)
	      return 0;
	    *a1 = target_card / cum_card;
	    return 1;
	  }
      }
      END_DO_SET ();
...
int
col_believe_card (dbe_column_t * col)
{
  /* an int col card that is equal to max - min is CREDIBLE */
  if (DV_LONG_INT == dtp_canonical[col->col_sqt.sqt_dtp] && (col->col_min || col->col_max))
    {
      int64 min = unbox (col->col_min);
      int64 max = unbox (col->col_max);
      return (max - min) == col->col_n_distinct;
    }
  return 0;
}
  ...
  if (DFE_IS_CONST (right) &&
      ...
      if (left_col->col_n_distinct > 0)
	{
	  ...
	  if (!is_rdf_col && right_col && COL_KP_UNQ != left_col->col_is_key_part && COL_KP_UNQ == right_col->col_is_key_part
	      && right_col->col_n_distinct > 0 && !col_believe_card (left_col))
	    n_dist = right_col->col_n_distinct;
...
sqlo_in_list_unit (df_elt_t * in_tb, df_elt_t * pred, float *u1, float *a1)
{
  ...
  if (-1 == n_items && pred->_.bin.is_on_index)
    {
  ...
  if (!pred->_.bin.is_on_index)
    {
      int inx2, n = BOX_ELEMENTS (in_list);
      for (inx2 = 2; inx2 < n; inx2++)
	{
	  float a = 0;
	  sqlo_eq_cost (in_list[0]->_.col.col, in_list[inx2], NULL, &a);
	  *a1 += a;
	}
    }
  return 1;
...
void
sqlo_in_pred_index (df_elt_t * dfe, index_choice_t * ic, df_elt_t * lower, dbe_column_t * col, int inx_const_fill,
    int *is_indexed_ret, float tb_count, float inx_arity)
{
...
int
dfe_has_const (df_elt_t * dfe, caddr_t * const_ret)
{
  if (DFE_CONST == dfe->dfe_type)
    {
      if (const_ret)
	*const_ret = (caddr_t) dfe->dfe_tree;
      return 1;
    }
  if (dfe->dfe_dc && 1 == dfe->dfe_dc->dc_n_values)
    {
      if (const_ret)
	{
	  caddr_t box = dc_box (dfe->dfe_dc, 0);
	  *const_ret = t_box_copy_tree (box);
	  dk_free_tree (box);
	}
      return 1;
    }
...
#define IS_LOWER_CONST(dfe, cnst) ((BOP_GT == dfe->_.bin.op || BOP_GTE == dfe->_.bin.op) && dfe_has_const (dfe->_.bin.right, cnst))
#define IS_UPPER_CONST(dfe, cnst) ((BOP_LT == dfe->_.bin.op || BOP_LTE == dfe->_.bin.op) && dfe_has_const (dfe->_.bin.right, cnst))

int
col_is_comparable (dbe_column_t * col, caddr_t cnst)
{
  dtp_t dtp;
  if (DV_ANY == col->col_sqt.sqt_dtp)
    return 1;
  dtp = DV_TYPE_OF (cnst);
  if (IS_NUM_DTP (dtp) && IS_NUM_DTP (col->col_sqt.sqt_dtp))
    return 1;
  return dtp_canonical[dtp] == dtp_canonical[col->col_sqt.sqt_dtp];
}
...
col_may_use_hist (dbe_column_t * col, df_elt_t * lower, df_elt_t * upper)
{
  ...
  if (lower && upper && IS_LOWER_CONST (lower, &cnst) && IS_UPPER_CONST (upper, &cnst2))
    return col_is_comparable (col, cnst) && col_is_comparable (col, cnst2);
  if (IS_LOWER_CONST (lower, &cnst) || IS_UPPER_CONST (lower, &cnst))
    return col_is_comparable (col, cnst);
  if (upper && IS_UPPER_CONST (upper, &cnst))
    return col_is_comparable (col, cnst);
  return 0;
}

double
hist_diff (caddr_t lower, caddr_t upper)
{
  /*  a metric of distance between points.  If number, this is the difference, if dates this is seconds between, if strings, undefined (-1) */
  dtp_t dtp1 = DV_TYPE_OF (lower);
  dtp_t dtp2 = DV_TYPE_OF (upper);
  if (IS_NUM_DTP (dtp1) && IS_NUM_DTP (dtp2))
    {
      caddr_t diff = box_sub (upper, lower, NULL, NULL);
      double d = box_to_double (diff, DV_TYPE_OF (diff));
      dk_free_box (diff);
      return d;
    }
  if (DV_DATETIME == dtp1 && DV_DATETIME == dtp2)
    {
      return dt_seconds (upper) - dt_seconds (lower);
    }
  return 0;
}

uint64
hist_rows (caddr_t ** hist, int lower, int upper)
{
  if (lower >= upper)
    return 0;
  return unbox (hist[upper][0]) - unbox (hist[lower][0]);
}

int
col_hist_bucket (dbe_column_t * col, caddr_t value, int is_upper, double *dist_ret, double *width_ret)
{
  /* returns the bucket where the value falls.  -1 if below first, len + 1 if above last, dist is set to distance of value from
   * start of bucket, width is set to distance between start and end of bucket, set to -1 if value below first or abobve last  */
  caddr_t **hist = (caddr_t **) col->col_hist;
  int len = BOX_ELEMENTS (hist), rc;
  int below = len, at_or_above = 0, guess;
  for (;;)
    {
      if (below - at_or_above <= 1)
	{
	  rc = cmp_boxes (value, hist[at_or_above][1], col->col_collation, col->col_collation);
	  if (DVC_LESS == rc)
	    {
	      if (0 == at_or_above)
		return -1;
	      *dist_ret = hist_diff (hist[at_or_above - 1][1], value);
	      *width_ret = hist_diff (hist[at_or_above - 1][1], hist[at_or_above][1]);
	  ...
	  if (at_or_above == len - 1)
	    return len;
	  *dist_ret = hist_diff (hist[at_or_above][1], value);
	  *width_ret = hist_diff (hist[at_or_above][1], hist[at_or_above + 1][1]);
	  return at_or_above;
	}
      guess = (at_or_above + below) / 2;
      rc = cmp_boxes (value, hist[guess][1], col->col_collation, col->col_collation);
      if (DVC_MATCH == rc)
	{
	  if (!is_upper)
	    below = guess;
...
float
col_hist_card (df_elt_t * col_dfe, df_elt_t * lower, df_elt_t * upper)
{
  double lower_width = 1, lower_dist = 0, upper_width = 1, upper_dist = 1, mpy = 1;
  int lower_pos, upper_pos;
  df_elt_t *pred = lower ? lower : upper;
  dbe_column_t *col = pred->_.bin.left->_.col.col;
  caddr_t **hist = (caddr_t **) col->col_hist;
  int len = BOX_ELEMENTS (hist);
  double rows, tb_card = unbox (hist[len - 1][0]), lower_closed = 0, upper_closed = 0;
  if (!upper && IS_UPPER_CONST (lower, NULL))
    {
      upper = lower;
      lower = NULL;
    }
  if (upper && lower)
      ...
      if (!DFE_IS_CONST (lower->_.bin.right))
	{
      ...
      if (!DFE_IS_CONST (upper->_.bin.right))
	{
	  upper = NULL;
	  mpy = 0.3;
	}
  ...
  if (lower)
    {
      lower_pos = col_hist_bucket (col, (caddr_t) lower->_.bin.right->dfe_tree, 0, &lower_dist, &lower_width);
      lower->_.bin.rng_by_hist = 1;
    }
  if (upper)
    {
      upper_pos = col_hist_bucket (col, (caddr_t) upper->_.bin.right->dfe_tree, 1, &upper_dist, &upper_width);
      upper->_.bin.rng_by_hist = 1;
    }
  if (upper && upper_pos >= len)
    upper = NULL;
  else if (upper && upper_pos < 0)
    goto empty;
  if (lower && lower_pos < 0)
    lower = NULL;
  else if (lower && lower_pos >= len - 1)
    goto empty;
  if (!upper && !lower)
    return 1;
  if (lower && BOP_GTE == lower->_.bin.op)
    lower_closed = (double) tb_card / col->col_n_distinct;
  if (upper && BOP_LTE == upper->_.bin.op)
    upper_closed = (double) tb_card / col->col_n_distinct;
  if (lower && upper)
    {
...
sqlo_pred_unit_1 (df_elt_t * lower, df_elt_t * upper, df_elt_t * in_tb, float *u1, float *a1)
{
  ...
  if (BOP_EQ == lower->_.bin.op)
    {
      *a1 = (float) 0.03;
      if (lower->_.bin.left == lower->_.bin.right && DFE_COLUMN != lower->_.bin.left->dfe_type)
	{
  ...
  if (lower->dfe_type != DFE_TEXT_PRED &&
      ...
      lower->_.bin.left->_.col.col && lower->_.bin.left->_.col.col->col_count != DBE_NO_STAT_DATA)
    {
      ...
      if (lower->_.bin.op == BOP_EQ)
	{
      ...
      else if (col_may_use_hist (left_col, lower, upper))
	{
	  *a1 = col_hist_card (lower, lower, upper);
	  return;
	}
      ...
      else if (lower->_.bin.op == BOP_LIKE)
	{
      ...
      lower->_.bin.left->dfe_type == DFE_COLUMN && lower->_.bin.left->_.col.col && lower->_.bin.op == BOP_LIKE)
    {
      dbe_column_t *left_col = lower->_.bin.left->_.col.col;
...
{				/* calculates the overhead of changing locus */
  ...
  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (dfe))
	...
	if ((DFE_TABLE == elt->dfe_type || DFE_DT == elt->dfe_type) && elt->dfe_locus)
	  {
      ...
      while (dfe)
	{
	  ...
	  if ((DFE_TABLE == dfe->dfe_type || DFE_DT == dfe->dfe_type) && dfe->dfe_locus)
	    {
...
rdf_inf_ctx_t *
dfe_ric (df_elt_t * dfe)
{
  rdf_inf_ctx_t *ric;
  if (DFE_TABLE != dfe->dfe_type)
    return empty_ric;
  ric = rdf_name_to_ctx (sqlo_opt_value (dfe->_.table.ot->ot_opts, OPT_RDF_INFERENCE));
  return ric ? ric : empty_ric;
}


void
sqlo_cset_p_stat (iri_id_t p, float *p_stat, int is_posg)
{
  int o_pos = is_posg ? 1 : 2;
  int s_pos = is_posg ? 2 : 1;
  dk_set_t csetp_list = p_to_csetp_list.ht_count ? (dk_set_t) gethash ((void *) p, &p_to_csetp_list) : NULL;
  DO_SET (cset_p_t *, csetp, &csetp_list)
  {
    dbe_table_t *tb = csetp->csetp_cset->cset_table;
    int64 tb_count = dbe_key_count (tb->tb_primary_key);
    dbe_column_t *s_col = tb_name_to_column (tb, "S");
    dbe_column_t *g_col = tb_name_to_column (tb, "G");
    if (!s_col || !g_col)
      continue;
    p_stat[0] += csetp->csetp_col->col_count;
    p_stat[o_pos] += csetp->csetp_col->col_n_distinct;
    p_stat[s_pos] += s_col->col_n_distinct;
    p_stat[3] += g_col->col_n_distinct;
  }
  END_DO_SET ();
}
...
void
ric_set_p_stat (rdf_inf_ctx_t * ric, dbe_key_t * key, caddr_t p, float *p_stat)
{
  id_hash_t *p_hash;
  mutex_enter (ric->ric_mtx);
  p_hash = (id_hash_t *) gethash ((void *) (ptrlong) key->key_id, ric->ric_p_stat);
  if (!p_hash)
    {
      p_hash = id_hash_allocate (201, sizeof (iri_id_t), 4 * sizeof (float), boxint_hash, boxint_hashcmp);
      sethash ((void *) (ptrlong) key->key_id, ric->ric_p_stat, (void *) p_hash);
      id_hash_set_rehash_pct (p_hash, 200);
    }
  id_hash_set (p_hash, (caddr_t) & ((iri_id_t *) p)[0], (caddr_t) p_stat);
  mutex_leave (ric->ric_mtx);
}


int
ric_p_stat_from_cache (rdf_inf_ctx_t * ric, dbe_key_t * key, iri_id_t id, float *p_stat)
{
  id_hash_t *p_hash;
  int found = 0;
  mutex_enter (ric->ric_mtx);
  ...
  if (p_hash)
    {
      float *place = (float *) id_hash_get (p_hash, (caddr_t) & id);
      if (place)
	{
	  found = 1;
...
err:
  ...
  sqlo_cset_p_stat (unbox_iri_id (p), p_stat, 0);
  ...
  p_stat[2] = s_cnt;
  p_stat[3] = MIN (p_stat[0], n_g);
  sqlo_cset_p_stat (unbox_iri_id (p), p_stat, 1);
  ric_set_p_stat (empty_ric, key, p, p_stat);
  ...
  *is_lit = 1;
  switch (DV_TYPE_OF (tree))
    ...
    case DV_SINGLE_FLOAT:
      return unbox_float ((caddr_t) tree);
    case DV_DOUBLE_FLOAT:
      return unbox_double ((caddr_t) tree);
    case DV_NUMERIC:
      {
	double d;
...
key_one_slice (dbe_key_t * key)
{
  if (key->key_is_elastic)
    {
      DO_LOCAL_CSL (csl, key->key_partition->kpd_map) return csl->csl_id;
      END_DO_LOCAL_CSL;
    }
  return QI_NO_SLICE;
}
...
sqlo_eval_text_count (dbe_table_t * tb, caddr_t str, caddr_t ext_fti)
{
  ...
  if (NULL != ext_fti)
    err = qr_rec_exec (call2, cli, &lc, CALLER_LOCAL, NULL, 3, ":0", cn, QRP_STR, ":1", str, QRP_STR, ":2", ext_fti, QRP_STR);
  else
    err = qr_rec_exec (call, cli, &lc, CALLER_LOCAL, NULL, 2, ":0", cn, QRP_STR, ":1", str, QRP_STR);
  if (err)
    goto err;
  ct = unbox (lc_ret (lc));
  lc_free (lc);
...
dfe_non_text_id_refd (df_elt_t * dfe, dbe_column_t * text_id_col)
{
  ...
  END_DO_SET ()DO_SET (df_elt_t *, pred, &dfe->_.table.all_preds)
  {
...
sqlo_is_text_after_test (df_elt_t * tb_dfe, df_elt_t * text_pred)
{
  ...
  cset_t *cset = tb_dfe->_.table.ot->ot_table->tb_closest_cset;
  dbe_key_t *id_key;
  if (cset)
    return !tb_dfe->_.table.is_text_order;
  id_key = tb_text_key (tb_dfe->_.table.ot->ot_table);
  ...
  DO_SET (df_elt_t *, pred, &tb_dfe->_.table.col_preds)
  {
	...
	&& BOP_EQ == pred->_.bin.op && pred->_.bin.left->dfe_type == DFE_COLUMN
	&& pred->_.bin.left->_.col.col == (dbe_column_t *) id_key->key_parts->data)
      return 1;
  }
  END_DO_SET ();
  return 0;
...
dfe_text_cost (df_elt_t * dfe, float *u1, float *a1, int text_order_anyway)
{
  ...
  if (text_pred)
	...
	{
	  int gtype;
	  ST **args = sqlc_geo_args (text_pred->dfe_tree, &gtype);
	  text_key_cost = dbe_key_unit_cost (sqlg_geo_index_table (text_key, args)->tb_primary_key);
	}
      else
	{
...
sqlo_rdf_obj_const_value (ST * tree, caddr_t * val_ret, caddr_t * lang_ret)
{
  ...
  if (ST_P (tree, CALL_STMT) && 3 == BOX_ELEMENTS (tree->_.call.params)
      ...
      && DV_STRINGP (tree->_.call.params[0]) && DV_STRINGP (tree->_.call.params[2]))
    {
      dtp_t dtp = DV_TYPE_OF (tree->_.call.params[0]);
      if (DV_SYMBOL == dtp)
	return 0;
...
rdf_obj_of_typed_sqlval (caddr_t val, caddr_t vtype, caddr_t lang, caddr_t * data_ret)
{
  ...
  if (DV_STRING == dtp)
    {
      int lang_id = DV_DB_NULL == DV_TYPE_OF (lang) ? RDF_BOX_DEFAULT_LANG : key_rdf_lang_id (lang);
      caddr_t r;
      rdf_box_t *rb;
      if (!lang_id)
	return 0;
...
void
itc_record_sample_param (it_cursor_t * itc, df_elt_t * pred, df_elt_t * left, df_elt_t * right)
{
  index_choice_t *ic;
  dbe_key_t *key;
  if (!right->dfe_nth_param || !pred->dfe_sqlo->so_stl)
    return;
  key = itc->itc_insert_key;
  if (DFE_COLUMN != left->dfe_type)
    return;
  if (col_is_rdf (left->_.col.col, 'P'))
    return;
  if (itc->itc_search_par_fill && col_is_rdf (key->key_parts->data, 'P') && box_equal (rdfs_type, itc->itc_search_params[0]))
    {
      if (col_is_rdf (left->_.col.col, 'O'))
	return;
    }
  /* record which lit param was cast.  first exclude rdf p and po if p is rdfs type */
  ic = itc->itc_sample_ic;
  if (ic)
    t_set_push (&ic->ic_lit_param_nos, (void *) ((ptrlong) ((itc->itc_search_par_fill - 1) << 16) + right->dfe_nth_param));
}
...
dfe_const_rhs (search_spec_t * sp, df_elt_t * pred, it_cursor_t * itc, int *v_fill)
{
  ...
  if (CMP_NULL == sp->sp_min_op || CMP_NON_NULL == sp->sp_min_op)
    return KS_CAST_OK;
  if (right->dfe_dc && 1 == right->dfe_dc->dc_n_values)
    {
      caddr_t box = dc_box (right->dfe_dc, 0);
      int res = sample_search_param_cast (itc, sp, mp_full_box_copy_tree (THR_TMP_POOL, box));
      dk_free_tree (box);
  ...
  if (DFE_CONST == right->dfe_type
      || sqlo_iri_constant_name (right->dfe_tree) || sqlo_rdf_obj_const_value (right->dfe_tree, NULL, NULL))
    {
      ...
      *v_fill = itc->itc_search_par_fill;
      if (KS_CAST_OK == res)
	itc_record_sample_param (itc, pred, left, right);
      return res;
    }
  return KS_CAST_UNDEF;
}

#define KS_CAST_NA 99

int
dfe_const_to_spec (df_elt_t * lower, df_elt_t * upper, dbe_key_t * key, search_spec_t * sp, it_cursor_t * itc, int *v_fill)
{
  int res = 0;
  dbe_col_loc_t *cl;
  df_elt_t **in_list = sqlo_in_list (lower, NULL, NULL);
  ...
  if (left_col == (dbe_column_t *) CI_ROW)
    SQL_GPF_T (NULL);
  cl = key_find_cl (key, left_col->col_id);
  if (!cl)
    return KS_CAST_NA;
  sp->sp_cl = *cl;
  sp->sp_col = left_col;
...
{
  int inx, fill = 0, n_dep = 0;
  char buf[1000];
  sample_cache_key_t *sck = (sample_cache_key_t *) & buf;
  dtp_t *data = (dtp_t *) & sck->sck_data;
  caddr_t *box = (caddr_t *) dk_alloc_box (sizeof (caddr_t) * (1 + itc->itc_search_par_fill), DV_ARRAY_OF_POINTER);
  search_spec_t *sp;
  sck->sck_key = itc->itc_insert_key->key_id;
  for (sp = itc->itc_key_spec.ksp_spec_array; sp; sp = sp->sp_next)
    {
      int conds = sp->sp_min_op;
      conds |= sp->sp_max_op << 4;
      data[fill++] = conds;
    }
  sck->sck_n_key = fill;
  for (sp = itc->itc_row_specs; sp; sp = sp->sp_next)
    {
      LONG_SET_NA (data + fill, sp->sp_cl.cl_col_id);
      data[fill + 4] = sp->sp_min_op;
      data[fill + 5] = sp->sp_max_op;
      fill += 6;
      n_dep++;
    }
  if (itc->itc_v_out_map)
    {
      int n_out = box_length (itc->itc_v_out_map) / sizeof (v_out_map_t), inx;
      for (inx = 0; inx < n_out; inx++)
	{
	  v_out_map_t *om = &itc->itc_v_out_map[inx];
	  LONG_SET_NA (data + fill, om->om_cl.cl_col_id);
	  fill += 4;
	}
      sck->sck_n_out = n_out;
    }
  ...
  for (inx = 0; inx < itc->itc_search_par_fill; inx++)
    box[inx + 1] = box_copy_tree (itc->itc_search_params[inx]);
  return (caddr_t) box;
}
...
itc_row_selectivity (it_cursor_t * itc, int64 inx_est)
{
  if (itc->itc_row_specs && itc->itc_st.n_rows_sampled)
	...
	{
	  /* in case of cluster, no recursive sample here, do not access tb count since can trigger sampling inside sampling */
	  float cnt =
	      CL_RUN_CLUSTER == cl_run_local_only ? MAX (10, 10 * itc->itc_st.n_rows_sampled) : dbe_key_count (itc->itc_insert_key);
	  return MAX (0.5 / cnt, 0.7 / itc->itc_st.n_rows_sampled);
	}
    }
  return 1;
}
...
float sqlo_inx_sample_1 (df_elt_t * tb_dfe, dbe_key_t * key, df_elt_t ** lowers, df_elt_t ** uppers, int n_parts,
...
dbe_column_t *
cset_col_by_rq (df_elt_t * dfe, cset_p_t * csetp, df_elt_t * rq_col)
{
  dbe_key_t *cset_pk = csetp->csetp_cset->cset_table->tb_primary_key;
  dbe_column_t *col = rq_col->_.col.col;
  switch (col->col_name[0])
    ...
    case 'S':
    case 's':
      return (dbe_column_t *) cset_pk->key_parts->data;
    case 'O':
    case 'o':
      return csetp->csetp_col;
    case 'G':
    case 'g':
      return (dbe_column_t *) cset_pk->key_parts->next->data;
    }
  return NULL;
}
...
int64
dfe_rq_cset_extra (df_elt_t * dfe, dbe_key_t * key, df_elt_t ** lowers, df_elt_t ** uppers, int n_parts,
    sample_opt_t * sop, index_choice_t * ic)
{
  df_elt_t *cs_lowers[2];
  df_elt_t *cs_uppers[2];
  df_elt_t *cs1;
  int cs_n_parts = 0;
  int64 sample = 0, c;
  iri_id_t p;
  index_choice_t cs_ic;
  char by_o;
  dbe_key_t *save_key = dfe->_.table.key;
  df_elt_t *r;
  dk_set_t csetp_list;
  rq_cols_t rq;
  if (!enable_cset || !tb_is_rdf_quad (key->key_table) || ic->ic_no_dep_sample)
    return 0;
  dfe->_.table.key = key;
  rq_cols_init (dfe, &rq);
  ...
  by_o = !dfe->_.table.key->key_is_primary;
  r = dfe_right (dfe, rq.rq_p.rqp_lower);
  if (DV_IRI_ID != DV_TYPE_OF (r->dfe_tree))
    {
      dfe->_.table.key = save_key;
      return 0;
    }
  p = unbox_iri_id (r->dfe_tree);
  csetp_list = (dk_set_t) gethash ((void *) p, &p_to_csetp_list);
  if (RQ_BOUND_EQ == rq.rq_s.rqp_op && RQ_UNBOUND == rq.rq_o.rqp_op)
    {
      dfe->_.table.key = save_key;
      return csetp_list ? 1 : 0;
    }
  dfe->_.table.key = save_key;
  cs1 = sqlo_key_part_best (rq.rq_s_col, dfe->_.table.col_preds, 0);
  if (cs1 && PRED_IS_EQ (cs1))
    {
      cs_n_parts = 1;
      cs_lowers[0] = cs1;
      cs1 = sqlo_key_part_best (rq.rq_g_col, dfe->_.table.col_preds, 0);
      if (cs1 && PRED_IS_EQ (cs1))
	{
	  cs_lowers[1] = cs1;
	  cs_n_parts = 2;
  ...
  memzero (cs_uppers, sizeof (cs_uppers));
  DO_SET (cset_p_t *, csetp, &csetp_list)
  {
    cset_t *cset = csetp->csetp_cset;
    dk_hash_t col_bk;
    dbe_key_t *save_key = dfe->_.table.key;
    if (by_o && csetp->csetp_index_o)
      continue;			/* stat in the rq sample */
	...
	{
	  dbe_column_t *cs_col = cset_col_by_rq (dfe, csetp, left_col);
	  if (!cs_col)
	    continue;
	  sethash ((void *) left_col, &col_bk, left_col->_.col.col);
	  left_col->_.col.col = cs_col;
	}
    }
    END_DO_SET ();
    memzero (&cs_ic, sizeof (cs_ic));
    c = sqlo_inx_sample_1 (dfe, dfe->_.table.key, cs_lowers, cs_uppers, cs_n_parts, sop, &cs_ic);
    sample += c;
    if (!(sop && SMPL_QUEUE == sop->sop_is_cl))
      {
	ic->ic_unit += cs_ic.ic_unit;
	ic->ic_arity += c;
	ic->ic_inx_card += cs_ic.ic_inx_card;
	ic->ic_n_rows_sampled += cs_ic.ic_n_rows_sampled;
	ic->ic_n_row_spec_matches += cs_ic.ic_n_row_spec_matches;
    ...
    sample_opt_t * sop, index_choice_t * ic)
{
  ...
  for (inx = 0; inx < n_parts; inx++)
    {
      res = dfe_const_to_spec (lowers[inx], uppers[inx], key, &specs[inx], itc, &v_fill);
      if (KS_CAST_NA == res)
	continue;		/* the col is not in this cset */
      if (KS_CAST_OK != res)
	{
	  itc_free (itc);
  ...
  sqlo_sample_out_cols (so, tb_dfe, itc);
  ...
  {
    res = itc_sample (itc);
    ic->ic_n_rows_sampled = itc->itc_st.n_rows_sampled;
    ic->ic_n_row_spec_matches = itc->itc_st.n_row_spec_matches;
    res += from_cset = dfe_rq_cset_extra (tb_dfe, key, lowers, uppers, n_parts, sop, ic);
    itc->itc_st.n_rows_sampled = ic->ic_n_rows_sampled;
    itc->itc_st.n_row_spec_matches = ic->ic_n_row_spec_matches;
...
int64
sqlo_inx_inf_sample (df_elt_t * tb_dfe, dbe_key_t * key, df_elt_t ** lowers, df_elt_t ** uppers, int n_parts, rdf_inf_ctx_t * ctx,
    rdf_sub_t * sub, caddr_t * variable, index_choice_t * ic, sample_opt_t * super_sop)
{
  sample_opt_t sop;
  int is_first = 1;
  ...
  super_sop->sop_n_sample_rows = sop.sop_n_sample_rows;
  dk_free_box ((caddr_t) rit);
  *variable = org_o;
  ic->ic_n_lookups = sub->rs_n_subs;
...
int
sqlo_is_rdf_p (df_elt_t * dfe, dbe_key_t * key, caddr_t p_const, float *prev_est)
{
  iri_id_t p;
  if (enable_p_stat)
    {
	...
	return RDF_NO_P_STAT;
      ...
      found = ric_p_stat_from_cache (dfe_ric (dfe), key, p, place);
      if (!found)
...
sqlo_record_rdf_p (sample_opt_t * sop, dbe_key_t * key, caddr_t p_const, int64 est, float prev_est, int *is_rdf_p)
{
  ...
  int fill = 1, completed = 0, is_posg;
  col_stat_t *cs;
  ...
  memzero (&distincts, sizeof (distincts));
  if (0 == est)
    est = 1;
  ...
  hash_table_free (sop->sop_cols);
  sop->sop_cols = NULL;
  is_posg = 'O' == ((dbe_column_t *) key->key_parts->next->data)->col_name[0];
  prev_p_cnt = distincts[0];
  sqlo_cset_p_stat (p, distincts, is_posg);
  if (completed || prev_p_cnt != distincts[0])
    ric_set_p_stat (empty_ric, key, p_const, distincts);
  return 0;
}
  ...
  if (ctx && tb_is_rdf_quad (tb_dfe->_.table.ot->ot_table))
      ...
      for (inx = 0; inx < ic->ic_leading_constants; inx++)
	{
	  ...
	  switch (left_col->col_name[0])
	    {
	    case 'P':
	      p_dfe = lowers[inx]->_.bin.right;
	      org_p = p_dfe->dfe_tree;
	      break;
	    case 'O':
	      o_dfe = lowers[inx]->_.bin.right;
	      org_o = o_dfe->dfe_tree;
	      break;
	    }
	}
      if (o_dfe && p_dfe)
	...
	switch (left_col->col_name[0])
      ...
      if (box_equal (rdfs_type, p_const) && o_const && (sub = ric_iri_to_sub (ctx, o_const, RI_SUBCLASS, 0)) && sub->rs_sub)
	{
      ...
      if (p_const && (sub = ric_iri_to_sub (ctx, p_const, RI_SUBPROPERTY, 0)) && sub->rs_sub)
	{
...
sqlo_try_inf_filter (df_elt_t * tb_dfe, index_choice_t * ic)
{
  ...
  END_DO_SET ();
  if (dk_set_length (no_inf_cp) < 2)
    return;			/* if only one indexable cond is left after putting inferred as after test, it is not worthwhile */
...
void
sqlo_record_lit_param_sample (df_elt_t * tb_dfe, caddr_t sc_key, index_choice_t * ic, int64 c)
{
  sqlo_t *so = tb_dfe->dfe_sqlo;
  ...
  {
    qces.qces_params[fill++] = n;
  }
  END_DO_SET ();
  qces.qces_card = c;
  qces.qces_n_params = fill;
  qces.qces_ric = ic->ic_ric ? ic->ic_ric : empty_ric;
  pqces = &qces;
  if (id_hash_get (stl->stl_samples, (caddr_t) & pqces))
    return;
  qces2 = (qce_sample_t *) dk_alloc_box (QCES_HEAD + fill * sizeof (int32), DV_BIN);
  memcpy_16 (qces2, &qces, box_length (qces2));
  t_id_hash_set (stl->stl_samples, (caddr_t) & qces2, NULL);
}
...
sqlo_inx_sample_inner (df_elt_t * tb_dfe, dbe_key_t * key, df_elt_t ** lowers, df_elt_t ** uppers, int n_parts, index_choice_t * ic)
{
  ...
  if (tb_is_rdf_quad (tb_dfe->_.table.ot->ot_table))
      ...
      for (inx = 0; inx < n_parts; inx++)
	{
      ...
      if (is_p)
	is_p = sqlo_is_rdf_p (tb_dfe, key, p_const, &prev_est);
...
sqlo_rdf_col_card (sqlo_t * so, df_elt_t * tb_dfe, df_elt_t * dfe)
{
  ...
  if (!p || DV_IRI_ID != DV_TYPE_OF (p))
    return;
  found = ric_p_stat_from_cache (dfe_ric (dfe), key, unbox_iri_id (p), place);
  dk_free_box (p);
  if (!found)
    {
      return;
...
dfe_hash_fill_cond_card (df_elt_t * tb_dfe)
{
  ...
  if (DFE_DT == tb_dfe->dfe_type)
    return tb_dfe->_.sub.hash_filler->_.sub.hash_fill_imp_sel;
  if (DFE_DT == tb_dfe->_.table.hash_filler->dfe_type)
    {
      index_choice_t ic;
      df_elt_t tb_dfe_c;
      memcpy (&tb_dfe_c, tb_dfe, DF_ELT_HEAD_SZ + sizeof (tb_dfe->_.table));
      tb_dfe_c.dfe_unit = 0;
...
caddr_t
pred_other_s_prefix (df_elt_t * pred, caddr_t pref, dbe_column_t * s_col)
{
  /* if pref is a join s to s from pref to another, then return the other's pref */
  if (DFE_COLUMN == pred->_.bin.left->dfe_type
      && DFE_COLUMN == pred->_.bin.right->dfe_type && s_col == pred->_.bin.left->_.col.col && s_col == pred->_.bin.right->_.col.col)
    {
      if (box_equal (pred->_.bin.left->dfe_tree->_.col_ref.prefix, pref))
	return pred->_.bin.right->dfe_tree->_.col_ref.prefix;
      if (box_equal (pred->_.bin.right->dfe_tree->_.col_ref.prefix, pref))
	return pred->_.bin.left->dfe_tree->_.col_ref.prefix;
      return NULL;
    }
  return NULL;
}
...
dk_set_t
ot_s_eqs (op_table_t * ot, caddr_t pref, dbe_column_t * s_col)
{
  dk_set_t res = NULL;
  DO_SET (df_elt_t *, pred, &ot->ot_preds)
  {
    caddr_t other;
    if (PRED_IS_EQ (pred) && (other = pred_other_s_prefix (pred, pref, s_col)))
      t_set_push (&res, other);
  }
  END_DO_SET ();
  return res;
}
...
df_elt_t *
ot_pref_p_eq (op_table_t * ot, caddr_t pref)
{
  /* given a prefix, return a pref.p = const dfe if any */
  DO_SET (df_elt_t *, pred, &ot->ot_preds)
  {
    if (PRED_IS_EQ (pred)
	&& DFE_COLUMN == pred->_.bin.left->dfe_type && DFE_CONST == pred->_.bin.right->dfe_type
	&& box_equal (pred->_.bin.left->dfe_tree->_.col_ref.prefix, pref) && 'P' == pred->_.bin.left->_.col.col->col_name[0])
      return pred;
  }
  END_DO_SET ();
  return NULL;
...
{
  DO_SET (dbe_column_t *, col, &key->key_parts) if (!CASEMODESTRCMP (col->col_name, name))
    return col;
  END_DO_SET ();
  return NULL;
}
...
rq_sample (df_elt_t * dfe, rq_cols_t * rq, index_choice_t * ic)
{
  ...
  ic->ic_no_dep_sample = (non_index_in || fill > 1) ? 0 : 1;
  if (non_index_in)
    {
...
int64
rq_sample_subp (df_elt_t * dfe, rq_cols_t * rq, index_choice_t * ic)
{
  int64 sum = 0;
  caddr_t *variable;
  float p_stat[4];
  float p_sum[4];
  index_choice_t ic2 = *ic;
  rdf_inf_ctx_t *ric = dfe_ric (dfe);
  caddr_t *save_opts = dfe->_.table.ot->ot_opts;
  caddr_t save;
  rdf_sub_t *sub;
  ri_iterator_t *rit;
  int found;
  ...
  if (!sub || (!sub->rs_sub && !sub->rs_equiv))
      ...
      if (!found)
	{
      ...
      if (found)
	{
	  ric_set_p_stat (ric, dfe->_.table.key, *variable, p_stat);
	  return p_stat[0];
	}
      return 0;
    }
  rit = ri_iterator (sub, RI_SUBPROPERTY, 1);
  memzero (p_sum, sizeof (p_sum));
  dfe->_.table.ot->ot_opts = NULL;
  ic2.ic_ric = empty_ric;
  while ((sub_iri = rit_next (rit)))
    {
      *variable = sub_iri->rs_iri;
      found = ric_p_stat_from_cache (empty_ric, dfe->_.table.key, unbox_iri_id (*variable), p_stat);
      ...
      if (found)
	{
	  int inx;
	  for (inx = 0; inx < 4; inx++)
	    p_sum[inx] += p_stat[inx];
	}
    }
  *variable = save;
  dk_free_box ((caddr_t) rit);
  ric_set_p_stat (ric, dfe->_.table.key, save, p_sum);
  dfe->_.table.ot->ot_opts = save_opts;
  return p_sum[0];
}
...
p_stat_again:
  ...
  if (!p)
    return -1;
  found = ric_p_stat_from_cache (ric, key, unbox_iri_id (p), p_stat);
  dk_free_box (p);
  if (!found)
    {
      rq_cols_t rq2 = *rq;
      ...
      dfe->_.table.key = key;
      sample = rq_sample_subp (dfe, &rq2, ic);
      dfe->_.table.key = save_key;
      if (sample <= 0)
	return -1;
      if (checked)
...
sqlo_pg_sample (rq_cols_t * rq)
{
  ...
  smp = sqlo_inx_sample (dfe, dfe->_.table.key, dfe_rq_p_v_card (df_elt_t * tb_dfe, df_elt_t * pred, float *inx_card));
}
#endif


float
sqlo_p_stat_default (float c, float *p_stat)
{
  int inx;
  for (inx = 0; inx < 4; inx++)
    {
...
int
sqlo_var_p_card (df_elt_t * tb_dfe, rq_cols_t * rq, float *inx_card)
{
  float card2 = 0;
  float n_ps = rq->rq_p_col->col_n_distinct;
  df_elt_t *pred = rq->rq_p.rqp_lower;
  if (!pred || !PRED_IS_EQ (pred))
    return 0;
  dfe_rq_p_v_card (tb_dfe, pred, &card2);
  if (card2)
    {
      /* guess based on tb count / n_ps is corrected based on other ps joined with the s, if any. card2 is better estimate than tb count / n_ps */
      float tb_count = dbe_key_count (tb_dfe->_.table.ot->ot_table->tb_primary_key);
      float p_corr = (tb_count / n_ps) / card2;
      *inx_card = *inx_card / p_corr;
      return 1;
    }
  return 0;
}
...
sqlo_use_p_stat_2 (df_elt_t * dfe, float *inx_card, float *col_card, index_choice_t * ic, float *sample_ret)
{
  ...
  if (RQ_CONST_EQ != rq.rq_p.rqp_op)
    {
      return sqlo_var_p_card (dfe, &rq, inx_card);
    }
  if ((dbe_column_t *) key->key_parts->data != rq.rq_p_col)
    return 0;
  if ((dbe_column_t *) key->key_parts->next->data == rq.rq_g_col)
    return 0;
  ...
  *sample_ret = p_card;
  if (-1 == p_card)
    return 0;
  p_card = sqlo_p_stat_default (p_card, p_stat);
...
{
  /* if there is a sample with leading constant p and the rest variable, consult the p stat for the p in question */
  caddr_t p;
  float place[4];
  dbe_key_t *key = dfe->_.table.key;
  ...
  if (!enable_p_stat || !inx_const_fill)
    return 0;
  if (0 != strcmp (((dbe_column_t *) key->key_parts->data)->col_name, "P") || !strstr (key->key_table->tb_name, "RDF_QUAD"))
    return 0;
...
pred_const_rhs (df_elt_t * pred)
{
  ...
  if (!r)
    return 0;
  if (r->dfe_dc && 1 == r->dfe_dc->dc_n_values)
    return 1;
  ...
  if (sqlo_iri_constant_name (r->dfe_tree) || sqlo_rdf_obj_const_value (r->dfe_tree, NULL, NULL))
    return 1;
  return 0;
}
...
void
dfe_table_narrow_fk (df_elt_t * dfe, index_choice_t * ic, float tb_card, float *inx_card_ret, df_elt_t ** eq_preds, int eq_fill,
    float *col_card_ret)
{
  /* fk with low card refs pk with high card, like tpc ds fact tables to date_dim.  Adjust the col selectivity to be as if the refd table had fk card worth rows */
  int inx;
  float rcard, tcard;
  dbe_table_t *tb = dfe->_.table.ot->ot_table;
  if (tb_is_rdf_quad (tb))
  ...
  for (inx = 0; inx < eq_fill; inx++)
    {
      df_elt_t *pred = eq_preds[inx], *left, *right;
      if (!pred)
	continue;
      left = pred->_.bin.left;
      if (left->_.col.col != (dbe_column_t *) dfe->_.table.ot->ot_table->tb_primary_key->key_parts->data)
	continue;		/* consider only eq with a single pk column */
      right = pred->_.bin.right;
      if (!right || !right->dfe_tables || dk_set_member (right->dfe_tables, dfe->_.table.ot))
	continue;
      rcard = dfe_exp_card (dfe->dfe_sqlo, right);
      tcard = tb_card * *inx_card_ret * *col_card_ret;
      if (rcard < tb_card * 0.9)
	{
	  /* the right side (fk) is  lower card than the pk. */
	  if (tcard > rcard)
	    {
	      /* more rows on the dim table are selected than distinct fks in the fact table.  All fk's are expected to hit */
	      return;
	    }
	  *col_card_ret *= tb_card / rcard;
	  return;
	}
    }
}
...
void
dfe_table_unq_card (df_elt_t * dfe, index_choice_t * ic, float tb_card, float *inx_card_ret, df_elt_t ** eq_preds, int eq_fill,
    float *col_card_ret)
{
  ...
  if (1 == key->key_n_significant)
    goto independent;
  for (inx = 0; inx < MIN (unq_limit, eq_fill); inx++)
    {
...
independent:
  ...
  *inx_card_ret = inx_card;
  dfe_table_narrow_fk (dfe, ic, tb_card, inx_card_ret, eq_preds, eq_fill, col_card_ret);
}
...
dfe_table_cost_ic_2 (df_elt_t * dfe, index_choice_t * ic, int inx_only)
{
  ...
  DO_SET (dbe_column_t *, part, &dfe->_.table.key->key_parts)
  {
    ...
    if (lower || upper)
      {
	if (lower && DFE_BOP_PRED == lower->dfe_type)
	  lower->_.bin.is_on_index = is_indexed;
	sqlo_pred_unit (lower, upper, dfe, &p_cost, &p_arity);
	if (lower && 1 == lower->_.bin.is_in_list && is_indexed)
	  sqlo_in_pred_index (dfe, ic, lower, part, inx_const_fill, &is_indexed, tb_count, inx_arity);
	if (is_indexed && !sqlo_pred_indexable (lower, upper))
	  is_indexed = 0;
	if (is_indexed)
	  {
	    inx_cost += (float) COL_PRED_COST *log (inx_arity) / log (2);	/*cost of compare * log2 of inx count */
	    inx_arity *= p_arity;
	    if (is_inx_const && inx_const_fill < 4 && (lower ? pred_const_rhs (lower) : 1) && (upper ? pred_const_rhs (upper) : 1))
	      {
	...
	else
	  {
	    ...
	    if (!(sqlo_sample_dep_cols &&
		    (lower ? !in_list && pred_const_rhs (lower) : 1) &&
    ...
    col_already_eq = lower && BOP_EQ == lower->_.bin.op;
    ...
    DO_SET (df_elt_t *, pred, &dfe->_.table.col_preds)
    {
      ...
      if (DFE_TEXT_PRED == pred->dfe_type)
	continue;
      left_col = in_list ? in_list[0]->_.col.col : (pred->_.bin.left->dfe_type == DFE_COLUMN ? pred->_.bin.left->_.col.col : NULL);
  ...
  if (dfe_rdfs_type_check_card (dfe, ic, eq_preds, eq_fill, &inx_cost))
  ...
  else if (2 == enable_p_stat && tb_is_rdf_quad (dfe->_.table.ot->ot_table)
      && sqlo_use_p_stat_2 (dfe, &inx_arity, &col_arity, ic, &inx_sample))
    {
      p_stat = 2;
      ic->ic_inx_card = inx_arity;
      ...
      else
	{
	  if (inx_const_fill)
	    inx_arity =
		inx_sample * inx_arity / (inx_arity_guess_for_const_parts != -1 ? inx_arity_guess_for_const_parts : inx_arity);
  ...
  ic->ic_joins_pk = 0;
  if ((!cset && sqlo_table_any_nonkey_refd (dfe))
      || (dfe->dfe_sqlo->so_sc->sc_is_update && 0 == strcmp (dfe->_.table.ot->ot_new_prefix, "t1")))
    {
	...
	if (DFE_BOP_PRED == pred->dfe_type &&
	    !dk_set_member (key->key_parts, (void *) left_col) && !dk_set_member (ic->ic_inx_sample_cols, (void *) left_col))
	  {
	    dfe->_.table.key = pk;
	    sqlo_pred_unit (pred, NULL, dfe, &p_cost, &p_arity);
	    dfe->_.table.key = key;
  ...
  if (cset || tb_text_key (dfe->_.table.ot->ot_table))
  ...
  if (!inx_only && dfe->dfe_unit > 0)
    {
      ...
      ic->ic_inx_card = dfe->_.table.inx_card;
      if (dfe->_.table.hash_role == HR_REF)
	{
	  float fu1, fa1, fo1;
	  IC_SAVE;
...
float
sqlo_func_max_card (caddr_t name)
{
  if (!SINV_DV_STRINGP (name))
    return -1;
  ...
  if (!stricmp (name, "hour"))
    return 24;
  if (!stricmp (name, "minute"))
    return 60;
  if (!stricmp (name, "second"))
    return 60;
  return -1;
}
...
dfe_exp_card (sqlo_t * so, df_elt_t * dfe)
{
  ...
  switch (dfe->dfe_type)
	...
	END_DO_BOX;
	mx = sqlo_func_max_card (dfe->_.call.func_name);
	return -1 != mx ? MIN (c, mx) : c;
      }
    default:
      return 1;
    }
...
void
dfe_top_tb_dfe_cost (df_elt_t * dfe, float *u1, float *a1)
{
  if (*a1 < 1 && (DFE_TABLE == dfe->dfe_type || DFE_DT == dfe->dfe_type))
    {
      df_elt_t *prev;
      for (prev = dfe->dfe_prev; prev;)
	{
	  if (DFE_TABLE == prev->dfe_type || DFE_DT == prev->dfe_type)
	    break;
	  if (prev->dfe_prev)
	    prev = prev->dfe_prev;
	  else
...
dfe_unit_cost (df_elt_t * dfe, float input_arity, float *u1, float *a1, float *overhead_ret)
{
  ...
  switch (dfe->dfe_type)
      ...
      else
	{
      ...
      if (dfe->dfe_type == DFE_DT && dfe->_.sub.ot->ot_is_outer)
	...
	*a1 = 1;
	...
	{
	  float n_ins, n_lookup, mem_cost;
	  dfe->_.setp.gb_card = dfe_group_by_card (dfe);
	  n_ins = MIN (input_arity, dfe->_.setp.gb_card);
...
dfe_list_cost (df_elt_t * dfe, float *unit_ret, float *arity_ret, float *overhead_ret, locus_t * loc)
{
  ...
  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (dfe))
	...
	dfe_top_tb_dfe_cost (dfe, &u1, &a1);
	if (IS_OV (a1 * arity) || IS_OV (u1 + cum))
	  bing ();
	if ((DFE_TABLE == elt->dfe_type || DFE_DT == elt->dfe_type) && elt->dfe_locus)
	  {
		...
		if (elt->dfe_locus != LOC_LOCAL && (DFE_TABLE != elt->dfe_type || HR_REF != elt->_.table.hash_role))
      ...
      while (dfe)
	{
	  ...
	  if ((DFE_TABLE == dfe->dfe_type || DFE_DT == dfe->dfe_type) && dfe->dfe_locus)
	      ...
	      if (loc && loc != dfe->dfe_locus)
		{
		  ...
		  if (dfe->dfe_locus != LOC_LOCAL && (DFE_TABLE != dfe->dfe_type || HR_REF != dfe->_.table.hash_role))
		    {
...
/*  if (sqlo_print_debug_output)
  ...
  dfe->dfe_unit = u1 + overhead / MAX (in_arity, 1);
  if (DFE_DT == dfe->dfe_type)
    dfe->_.sub.hash_fill_ov = overhead;
  return (in_arity * u1 + (overhead / in_arity));
}


int
sqlo_pred_indexable (df_elt_t * lower, df_elt_t * upper)
{
  /* looping over key parts, return 0 if the best pred for the part is not indexable, e.g. is a like, neq or in of a subrange */
  if (!lower)
    return 1;
