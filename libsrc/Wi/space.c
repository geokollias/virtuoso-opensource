  ...
  for (inx = 0; inx < IT_N_MAPS; inx++)
      ...
      while (dk_hit_next (&hit, (void **) &dp, (void **) &buf))
	{
	  ...
	  if (IT_CHECK_POST != mode && dbs_is_free_page (it->it_storage, (dp_addr_t) dp))
		...
		{
		  log_error
		      ("***  In cpt, suspect to have buffer with free dp L=%ld buf=%p  while it is neither blob or blob dir, which may be free at this point if uncommitted.\n",
		      dp, (void *) buf);
		  error = 1;
		}
	    }
	  if (((dp_addr_t) dp) != buf->bd_physical_page && dbs_is_free_page (it->it_storage, buf->bd_physical_page))
	    {
...
{
  if (DPF_COLUMN == SHORT_REF (buf->bd_buffer + DP_FLAGS) && buf->bd_tree->it_col_extent_maps)
    {
      extent_map_t *em =
	  (extent_map_t *) gethash ((void *) (ptrlong) LONG_REF (buf->bd_buffer + DP_PARENT), buf->bd_tree->it_col_extent_maps);
      if (em)
	return em;
    }
...
itc_delta_this_buffer (it_cursor_t * itc, buffer_desc_t * buf, int stay_in_map)
{
  ...
  if (!remap_to)
	...
	{
	  log_error
	      ("Out if disk during commit.  The transaction is in effect and will be replayed from the log at restart.  Exiting due to no disk space, thus cannot maintain separation of checkpoint and commit space and transactional semantic."
	...
	GPF_T1
	    ("The database is out of disk during an insert.  The insert has exceeded its space safety margin.  This does not normally happen.  This is a planned exit and not a corruption. Make more disk space available.  If this occurs continuously or without in fact running out of space, this may be reported to support.");
...
buffer_desc_t *
it_new_page (index_tree_t * it, dp_addr_t addr, int type, oid_t col_id, it_cursor_t * has_hold)
{
  it_map_t *itm;
  ...
  sethash (DP_ADDR2VOID (physical_dp), &itm->itm_remap, DP_ADDR2VOID (physical_dp));
...
it_free_page (index_tree_t * it, buffer_desc_t * buf)
{
  ...
  l = SHORT_REF (buf->bd_buffer + DP_FLAGS);
  if (!(l == DPF_BLOB || l == DPF_BLOB_DIR || l == DPF_COLUMN) && !remap)
...
/* Note that the the buf is not passed to page_wait_access.  This is because of 'being read' possibility. page_fault will detect this and sync. */
  ...
  DBG_PT_PRINTF (("Free absent blob  L=%d \n", dp));
  {
	...
	if (remap == phys_dp)
      ...
      {
	if (DPF_HASH == dp_type)
	  GPF_T1 ("a hash temp page is not supposed to be in cpt s[space");
