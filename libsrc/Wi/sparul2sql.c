...
spar_compose_report_flag (sparp_t * sparp)
{
  ...
  if ((NULL == fmtname) && (NULL == sparp->sparp_env->spare_parent_env) && ssg_is_odbc_cli ())
    {
      if (ssg_is_odbc_msaccess_cli ())
	fmtname = sparp->sparp_sg->sg_output_format_name = t_box_dv_short_string ("_MSACCESS_");
      else
	fmtname = sparp->sparp_sg->sg_output_format_name = t_box_dv_short_string ("_UDBC_");
    }
  res = t_box_num_nonull (((NULL != fmtname) && (!strcmp (fmtname, "_JAVA_") || !strcmp (fmtname, "_UDBC_")
	      || !strcmp (fmtname, "_MSACCESS_"))) ? 0 : 1);
  sparp->sparp_sg->sg_output_compose_report = res;
  return res;
}

extern int sparp_ctor_fields_are_disjoin_with_where_fields (sparp_t * sparp, SPART * top, SPART ** ctor_fields,
    SPART ** where_fields);
extern int sparp_ctor_fields_are_disjoin_with_data_gathering (sparp_t * sparp, SPART ** ctor_fields, SPART * req,
    int the_query_is_topmost);

int
sparp_ctor_fields_are_disjoin_with_where_fields (sparp_t * sparp, SPART * top, SPART ** ctor_fields, SPART ** where_fields)
{
  sparp_equiv_t **top_eqs = sparp->sparp_sg->sg_equivs;
  int top_eq_count = sparp->sparp_sg->sg_equiv_count;
  SPART *top_gp = top->_.req_top.pattern;
  int fld_ctr;
...
sparp_gp_trav_find_isect_with_ctor (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  switch (curr->type)
    ...
    case SPAR_TRIPLE:
	  ...
	  sparp_ctor_fields_are_disjoin_with_where_fields (sparp, sparp->sparp_stp->stp_trav_req_top, (SPART **) common_env,
	      curr->_.triple.tr_fields))
	...
	case SELECT_L:
	  {
	    ...
	    isect_res =
		sparp_ctor_fields_are_disjoin_with_data_gathering (sub_sparp, (SPART **) common_env, curr->_.gp.subquery, 1);
...
sparp_ctor_fields_are_disjoin_with_data_gathering (sparp_t * sparp, SPART ** ctor_fields, SPART * req, int the_query_is_topmost)
{
  ...
  res = sparp_gp_trav_top_pattern (sparp, req, ctor_fields,
      sparp_gp_trav_find_isect_with_ctor, NULL, NULL, NULL, sparp_gp_trav_find_isect_with_ctor, NULL);
...
{				/* This rewrites variables that are nested into backquoted expressions in ctor template when the query has limit or offset clause */
  ...
  if (SPAR_BLANK_NODE_LABEL == curr_type)
    spar_error (sparp,
	"Blank nodes can not be used in backquoted expressions of constructor template, consider using variables instead");
...
void
spar_compose_retvals_of_ctor (sparp_t * sparp, SPART * ctor_gp, const char *funname, sql_comp_t * sc_for_big_ssl_const,
    SPART * arg0, SPART * arglast, SPART *** retvals, ctor_var_enumerator_t * cve, const char *formatter, const char *agg_formatter,
    const char *agg_mdata, int use_limits)
{
  ...
  if (NULL != sc_for_big_ssl_const)
      ...
      for (triple_ctr = BOX_ELEMENTS_INT (ctor_gp->_.gp.members); triple_ctr--; /* no step */ )
	{
	  ...
	  args = (g_is_default ?
		  ...
		  (ptrlong) CTOR_OPCODE_CONST_OR_EXPN, NULL,
		  (ptrlong) CTOR_OPCODE_CONST_OR_EXPN, NULL,
		  (ptrlong) CTOR_OPCODE_CONST_OR_EXPN, NULL, (ptrlong) CTOR_OPCODE_CONST_OR_EXPN, NULL));
	  for (fld_ctr = g_is_default ? 1 : 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
		...
		case SPAR_QNAME:
		  ...
		  break;
		default:
		  spar_internal_error (sparp, "Non-const in big ssl const mode constructor pattern");
		}
	    }
  ...
  for (triple_ctr = BOX_ELEMENTS_INT (ctor_gp->_.gp.members); triple_ctr--; /* no step */ )
      ...
      for (fld_ctr = g_is_default ? 1 : 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
	{
	  ...
	  switch (fld_type)
		...
		sparp_gp_trav (sparp, NULL /*unused */ , fld, cve, NULL, NULL, sparp_gp_trav_ctor_var_to_limofs_aref, NULL, NULL,
		    NULL);
		opcode_arg_ptr[0] = t_box_num_nonull (CTOR_OPCODE_CONST_OR_EXPN);
		val_arg_ptr[0] = fld;
...
args_ready:
  ...
  if (NULL != arglast)
    ctor_call = spar_make_funcall (sparp, 1, funname, (SPART **) t_list (5, arg0, arg1, var_vector_arg, arg3, arglast));
  else if (NULL != arg0)
    ctor_call = spar_make_funcall (sparp, 1, funname, (SPART **) t_list (4, arg0, arg1, var_vector_arg, arg3));
  else
  ...
  if (cve->cve_limofs_var_alias)
    {
      SPART *alias =
	  spartlist (sparp, 6, SPAR_ALIAS, var_vector_expn, cve->cve_limofs_var_alias, SSG_VALMODE_AUTO, (ptrlong) 0, (ptrlong) 0);
    ...
    const char *formatter, const char *agg_formatter, const char *agg_mdata)
{
  ...
  if ((NULL == sparp->sparp_env->spare_storage_name) || ('\0' != sparp->sparp_env->spare_storage_name))
    use_limits = 1;
...
spar_find_sc_for_big_ssl_const (sparp_t * sparp, sql_comp_t ** sc_ret)
{
  ...
  if (NULL == sc_ret[0])
    {
      spar_error (sparp,
	  "The query can be compiled and executed but not translated to an accurate SQL text, add 'define sql:big-data-const 0' for workaround");
    }
  else
    {
...
spar_compose_retvals_of_insert_or_delete (sparp_t * sparp, SPART * top, SPART * graph_to_patch, SPART * ctor_gp)
{
  ...
  memset (&cve, 0, sizeof (ctor_var_enumerator_t));
  log_mode = sparp->sparp_sg->sg_sparul_log_mode;
  if (NULL == log_mode)
  ...
  if (NULL != sparp->sparp_sg->sg_output_route_name)
    {
      top_fname = t_box_sprintf (200, "sql:SPARQL_ROUTE_DICT_CONTENT_%.100s", sparp->sparp_sg->sg_output_route_name);
      rv[0] = spar_make_funcall (sparp, 0, top_fname,
	  (SPART **) t_list (11, graph_to_patch,
	      t_box_dv_short_string ((NULL !=
		      multigraph) ? (top_subtype_is_insert ? "INSERT_QUAD" : "DELETE_QUAD") : (top_subtype_is_insert ? "INSERT" :
		      "DELETE")),
	      ...
	      ((INSERT_L == top->_.req_top.subtype) ? (caddr_t) (rv[0]) : t_NEW_DB_NULL), t_NEW_DB_NULL,
...
spar_compose_retvals_of_modify (sparp_t * sparp, SPART * top, SPART * graph_to_patch, SPART * del_ctor_gp, SPART * ins_ctor_gp)
{
  ...
  if (NULL != sparp->sparp_sg->sg_output_route_name)
	...
	(SPART **) t_list (11, graph_to_patch,
	    ...
	    ((sparp->sparp_env->spare_disable_output_formatting
		    || NULL == sparp->sparp_sg->sg_output_format_name) ? t_NEW_DB_NULL : sparp->sparp_sg->sg_output_format_name),
	    rv[0], ins[0], t_NEW_DB_NULL, spar_exec_uid_and_gs_cbk (sparp), log_mode, spar_compose_report_flag (sparp)));
  else
    rv[0] =
	spar_make_funcall (sparp, 0,
	(g_may_vary ? "sql:SPARQL_MODIFY_BY_QUAD_DICT_CONTENTS" : "sql:SPARQL_MODIFY_BY_DICT_CONTENTS"), (SPART **) t_list (6,
	    graph_to_patch, rv[0], ins[0], spar_exec_uid_and_gs_cbk (sparp), log_mode, spar_compose_report_flag (sparp)));
}
...
spar_compose_ctor_triples_from_where_gp (sparp_t * sparp, int subtype, SPART * gp, SPART * g, dk_set_t * ret_tmpls)
{
  ...
  DO_BOX_FAST (SPART *, memb, memb_ctr, gp->_.gp.members)
  {
	...
	if (0 < BOX_ELEMENTS_0 (memb->_.triple.options))
	  spar_error (sparp,
	      "DELETE WHERE does not support OPTION () clauses of triples and related features, such as transitivity");
...
spar_compose_ctor_gp_from_where_gp (sparp_t * sparp, int subtype, SPART * where_gp, SPART * gtp)
{
  ...
  END_DO_SET ()tmpl_gp = (SPART *) (t_box_copy ((caddr_t) where_gp));	/* Dirty hack here, indeed, however we need only a box with type and proper length, nothing else */
...
spar_emulate_ctor_field (sparp_t * sparp, SPART * opcode, SPART * oparg, SPART ** vars)
{
  ...
  switch (unbox ((caddr_t) opcode))
    ...
    case CTOR_OPCODE_CONST_OR_EXPN:
      return oparg;
    }
...
spar_tr_fields_are_similar (sparp_t * sparp, SPART * fld1, SPART * fld2)
{
  ...
  switch (fldtype)
	...
	if ((DV_TYPE_OF (v1) != DV_TYPE_OF (v2)) || (DVC_MATCH != cmp_boxes_safe (v1, v2, NULL, NULL)))
	  return 0;
	if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (fld1)) || (DV_ARRAY_OF_POINTER != DV_TYPE_OF (fld2)))
	  return 1;
  ...
  if (g_is_set)
    {
      ...
      if (!spar_tr_fields_are_similar (sparp, emu, triple->_.triple.tr_fields[SPART_TRIPLE_GRAPH_IDX]))
	return 0;
    }
  else
    {
      if (!spar_tr_fields_are_similar (sparp, graph_expn, triple->_.triple.tr_fields[SPART_TRIPLE_GRAPH_IDX]))
	return 0;
    }
  emu = spar_emulate_ctor_field (sparp, args[0], args[1], known_vars);
  if (!spar_tr_fields_are_similar (sparp, emu, triple->_.triple.tr_fields[SPART_TRIPLE_SUBJECT_IDX]))
    return 0;
  emu = spar_emulate_ctor_field (sparp, args[2], args[3], known_vars);
  if (!spar_tr_fields_are_similar (sparp, emu, triple->_.triple.tr_fields[SPART_TRIPLE_PREDICATE_IDX]))
    return 0;
  emu = spar_emulate_ctor_field (sparp, args[4], args[5], known_vars);
  if (!spar_tr_fields_are_similar (sparp, emu, triple->_.triple.tr_fields[SPART_TRIPLE_OBJECT_IDX]))
    return 0;
  good_ctor_call = spar_make_funcall (sparp, 1, "sql:SPARQL_DELETE_CTOR",
      (SPART **) t_list (5,
	  graph_expn,
	  spar_make_funcall (sparp, 0, "bif:vector", var_triples), ctor->_.funcall.argtrees[1], uid_expn, log_mode_expn));
  ctor->_.funcall.argtrees[0] = spar_make_funcall (sparp, 0, "bif:vector", (SPART **) t_list (0));
...
spar_optimize_retvals_of_insert_or_delete (sparp_t * sparp, SPART * top)
{
  ...
  for (tctr = all_triple_count; tctr--; /* no step */ )
	...
	t_set_push (&positions_with_bnodes, ((void *) ((ptrlong) tctr)));
      if (CTOR_DISJOIN_WHERE == sparp_ctor_fields_are_disjoin_with_data_gathering (sparp, quad_fields, top, 1))
	{
  ...
  good_ctor_call = spar_make_funcall (sparp, 1, fname,
	  ...
	  spar_make_funcall (sparp, 0, "bif:vector",
	      (SPART **) t_list_to_array (good_triples)), ctor->_.funcall.argtrees[1], uid_expn, log_mode_expn));
  ctor->_.funcall.argtrees[0] = spar_make_funcall (sparp, 0, "bif:vector", (SPART **) t_list_to_array (bad_triples));
  retvals[0]->_.funcall.argtrees[0] = good_ctor_call;
}
...
/* Part 1. Collecting optimized data for DELETE ctor */
  ...
  for (ins_tctr = all_ins_triple_count; ins_tctr--; /* no step */ )
      ...
      for (del_tctr = all_del_triple_count; del_tctr--; /* no step */ )
	{
	  ...
	  if (CTOR_DISJOIN_WHERE != sparp_ctor_fields_are_disjoin_with_where_fields (sparp, top, quad_fields, del_fields))
	    goto ins_is_bad;	/* see below */
	}
  ...
  good_ctor_call = spar_make_funcall (sparp, 1, "sql:SPARQL_MODIFY_CTOR",
	  ...
	  spar_make_funcall (sparp, 0, "bif:vector",
	      (SPART **) t_list_to_array (good_ins_triples)), ins_ctor->_.funcall.argtrees[1], uid_expn, log_mode_expn));
  if (NULL == bad_del_triples)
    retvals[0]->_.funcall.argtrees[1] = (SPART *) t_NEW_DB_NULL;
  else
    del_ctor->_.funcall.argtrees[0] = spar_make_funcall (sparp, 0, "bif:vector", (SPART **) t_list_to_array (bad_del_triples));
  if (NULL == bad_ins_triples)
    retvals[0]->_.funcall.argtrees[2] = (SPART *) t_NEW_DB_NULL;
  else
    ins_ctor->_.funcall.argtrees[0] = spar_make_funcall (sparp, 0, "bif:vector", (SPART **) t_list_to_array (bad_ins_triples));
  retvals[0]->_.funcall.argtrees[0] = good_ctor_call;
}
