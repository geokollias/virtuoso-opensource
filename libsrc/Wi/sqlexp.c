...
sqlc_trans_funcs (sql_comp_t * sc, ST * tree, state_slot_t * ret, dk_set_t * code)
{
  ...
  if (!stricmp (tree->_.call.name, "T_STEP"))
      ...
      if (DV_LONG_INT == dtp)
	{
	  ...
	  if (tn_step_refers_to_input)
		...
		wanted_pos = box_position ((caddr_t *) (sc->sc_trans->tn_input_pos), (caddr_t) pos);
	      if (-1 == wanted_pos)
		sqlc_new_error (sc->sc_cc, "37000", "TR...",
		    "T_STEP argument refers to an index %d of a column %.200s that is not in %s list (T_DIRECTION is set to %d)",
		    pos + 1, wanted->ssl_name, ((TRANS_LR == sc->sc_trans->tn_direction) ? "T_IN" : "T_OUT"),
		    sc->sc_trans->tn_direction);
	...
	sqlc_new_error (sc->sc_cc, "37000", "TR...",
	    "the argument of T_STEP must be a 1 based column index in the selection or 'step_no' or 'path_id'");
      return ret;
    }
  if (!stricmp (tree->_.call.name, "T_STATE"))
	...
	sc->sc_trans->tn_state_ssl = ssl_new_variable (sc->sc_cc, "tn_state", DV_ANY);
      tree->_.call.params =
	  (ST **) t_box_append_1 ((caddr_t) tree->_.call.params, t_box_num ((ptrlong) sc->sc_trans->tn_state_ssl));
      return NULL;
    }
  return NULL;
}
...
/*                     012345678901 */
  ...
  if (proc)
	...
	if (formal)
	  {
	    snprintf (tm, sizeof (tm), "%s() %dth arg", proc->qr_proc_name ? proc->qr_proc_name : "<unnamed>", inx + 1);
...
cv_bop_params (state_slot_t * l, state_slot_t * r, const char *op)
{
  if (!r || !l)
    return;
  if (!SSL_IS_UNTYPED_PARAM (l) && !SSL_IS_UNTYPED_PARAM (r) && SQW_DTP_COLIDE (l->ssl_dtp, l->ssl_class, r->ssl_dtp, r->ssl_class))
    {
...
cv_asg_broader_type (instruction_t * ins)
{
  ...
  if (dtp_canonical[res->ssl_dtp] == dtp_canonical[l->ssl_dtp] || DV_OBJECT == res->ssl_dtp || DV_REFERENCE == res->ssl_dtp)
    return;
      ...
      if (ins->_.artm.right)
	{
	  switch (ins->ins_type)
	      ...
	      if (DV_DATETIME == ins->_.artm.left->ssl_dtp)
		{
		  ...
		  if ((DV_LONG_INT == ins->_.artm.right->ssl_dtp) || (DV_INT64 == ins->_.artm.right->ssl_dtp)
		      || (DV_DOUBLE_FLOAT == ins->_.artm.right->ssl_dtp))
		    {
		      ins->_.artm.result->ssl_dtp = DV_DATETIME;
		      goto result_dtp_is_set;
		    }
	  ...
	  if (DV_LONG_INT == ins->_.artm.result->ssl_sqt.sqt_dtp && (DV_INT64 == ins->_.artm.left->ssl_sqt.sqt_dtp
		  || DV_INT64 == ins->_.artm.right->ssl_sqt.sqt_dtp))
	    ins->_.artm.result->ssl_sqt.sqt_dtp = DV_INT64;
	  ins->_.artm.result->ssl_sqt.sqt_non_null = ins->_.artm.left->ssl_sqt.sqt_non_null
	      && ins->_.artm.right->ssl_sqt.sqt_non_null;
	result_dtp_is_set:
	  ...
	  cv_bop_params (ins->_.artm.left, ins->_.artm.right, ins_type_to_artm_name (ins->ins_type));
	}
      else
	{
	  cv_bop_params (ins->_.artm.result, ins->_.artm.left, ins_type_to_artm_name (ins->ins_type));
	}
    }
  else
    {
...
void
cv_artm (dk_set_t * code, ao_func_t f, state_slot_t * res, state_slot_t * l, state_slot_t * r)
{
...
void
cv_agg (dk_set_t * code, int op, state_slot_t * res, state_slot_t * arg, state_slot_t * set_no, void *distinct, sql_comp_t * sc)
{
...
void
cv_compare (dk_set_t * code, int bop, state_slot_t * l, state_slot_t * r, jmp_label_t succ, jmp_label_t fail, jmp_label_t unkn)
{
...
cv_vret (dk_set_t * code, state_slot_t * ssl)
{
  ...
  if (top_sc && top_sc->sc_cc->cc_query && top_sc->sc_cc->cc_query->qr_proc_ret_type)
    {
      state_slot_t lsl;
  ...
  if (top_sc && top_sc->sc_cc->cc_query && top_sc->sc_cc->cc_query->qr_proc_ret_type)
      ...
      if (SQW_DTP_COLIDE (dtp, NULL, DV_LONG_INT, NULL))
	{
	  sqlc_warning ("01V01", "QW005",
	      ...
	      MAX_NAME_LEN, dv_type_title (DV_LONG_INT), (int) DV_LONG_INT, MAX_NAME_LEN, dv_type_title (dtp), (int) dtp);
	}
    }
#endif
}
...
void
cv_call (dk_set_t * code, state_slot_t * fun_exp, caddr_t proc, state_slot_t * ret, state_slot_t ** params)
{
  bif_t bif = NULL;
...
subq_pred_t *
cmp_subq_call (sql_comp_t * sc, ST * tree, dk_set_t * code, state_slot_t * left)
{
...
pred_gen_1 (sql_comp_t * sc, ST * tree, dk_set_t * code, int succ, int fail, int unkn)
{
  ...
  if (BIN_EXP_P (tree))
    ...
    {
      state_slot_t *left = tree->_.subq.left ? scalar_exp_generate (sc, tree->_.subq.left, code) : NULL;
      NEW_INSTR (ins, IN_PRED, code);
      ins->_.pred.fail = fail;
      ins->_.pred.succ = succ;
      ins->_.pred.unkn = unkn;
...
sqlo_cl_locatable (ST * tree, int level)
{
  ...
  if (ST_P (tree, TABLE_DOTTED))
	...
	return !bif_is_no_cluster (bif) && sqlo_array_cl_locatable ((ST *) tree->_.call.params, level);
      return sqlo_array_cl_locatable ((ST *) tree->_.call.params, level) && sqlo_proc_cl_locatable (name, level, NULL);
    }
...
qr_is_local (query_t * qr, int is_cluster)
{
  ...
  DO_SET (data_source_t *, src, &qr->qr_nodes)
  {
	...
	|| !cv_is_local_1 (src->src_after_test, is_cluster) || !cv_is_local_1 (src->src_pre_code, is_cluster))
      return 0;
...
ins_assigned (instruction_t * ins, dk_set_t * res)
{
  ...
  switch (ins->ins_type)
	...
	dk_set_push (res, (void *) ins->_.artm.result);
      break;
    case INS_SUBQ:
      if (ins->_.subq.query && ins->_.subq.query->qr_select_node && ((out = ins->_.subq.query->qr_select_node->sel_out_slots)))
	{
...
qn_refd_slots (sql_comp_t * sc, data_source_t * qn, dk_hash_t * res, dk_hash_t * all_res, int *non_cl_local)
{
  ...
  if (IS_TS ((table_source_t *) qn) || IS_QN (qn, chash_read_input) || IS_QN (qn, sort_read_input))
    {
      ...
      if (ts->ts_csm || ts->ts_csts || ts->ts_csq)
...
code_to_cv_1 (sql_comp_t * sc, dk_set_t code, int trim_one_long_cv)
{
  ...
  if (code)
	...
	if (ins->ins_type == IN_LABEL)
	  {
	    sethash ((void *) (ptrlong) ins->_.label.label, lblhash, (void *) (ptrlong) (INSTR_OFS (dins, cv) + 1));
	  }
	if (ins_len)
	  {
...
cv_free (code_vec_t cv)
{
  ...
  DO_INSTR (ins, 0, cv)
  {
    if (ins->ins_type == IN_PRED && ins->_.pred.cmp)
      {
    ...
    else if (ins->ins_type == INS_CALL || ins->ins_type == INS_CALL_IND)
      {
...
state_slot_t *
sqlg_agg_ins (sql_comp_t * sc, ST * tree, dk_set_t * code, dk_set_t * fun_ref_code)
{
...
state_slot_t *
select_ref_generate (sql_comp_t * sc, ST * tree, dk_set_t * code, dk_set_t * fun_ref_code, int *is_fun_ref)
{
  if (ST_P (tree, FUN_REF) && AMMSC_USER != tree->_.fn_ref.fn_code)
	...
	case AMMSC_MAX:
	  {
	    state_slot_t *best =
		ssl_new_inst_variable (sc->sc_cc, AMMSC_MAX == tree->_.fn_ref.fn_code ? "best" : "min", DV_UNKNOWN);
	    ...
	    cv_compare (fun_ref_code, tree->_.fn_ref.fn_code == AMMSC_MAX ? BOP_LT : BOP_GT, best, arg, better, end, unk);
	...
	case AMMSC_USER:
	  {
		...
		(state_slot_t **) dk_alloc_box (sizeof (state_slot_t *) * (1 + BOX_ELEMENTS (tree->_.fn_ref.fn_arglist)),
		DV_ARRAY_OF_POINTER);
	    env->ssl_qr_global = 1;
	    ...
	    cv_compare (fun_ref_code, BOP_EQ, flag, const_short_1, setenv_end, setenv_begin, setenv_begin);
