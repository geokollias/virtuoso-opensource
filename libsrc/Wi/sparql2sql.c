  ...
  if (SELECT_L == curr->_.gp.subtype)
      ...
      for (ctr = BOX_ELEMENTS_0 (options); 1 < ctr; ctr -= 2)
	{
	  ...
	  switch (key)
	    ...
	    case OFFBAND_L:
	    case SCORE_L:
	      name = val->_.var.vname;
	      break;
	    case T_STEP_L:
	      name = val->_.alias.aname;
	      break;
	    }
	  if (SPART_VARNAME_IS_NICE_RETVAL (name, ((dk_set_t *) (common_env))[0]))
	    t_set_push_new_string ((dk_set_t *) (common_env), name);
...
sparp_gp_trav_list_nonaggregate_retvals (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  switch (curr->type)
	...
	END_DO_SET ()t_set_push (&(((list_nonaggregate_retvals_t *) (common_env))->names), varname);
	return SPAR_GPT_NODOWN;
      }
    case SPAR_FUNCALL:
...
sparp_preprocess_obys (sparp_t * sparp, SPART * root)
{
  ...
  DO_BOX_FAST (SPART *, oby, oby_ctr, root->_.req_top.order)
  {
	...
	if (0 >= rv_count)
	  spar_error (sparp,
	      "SELECT query should contain explicit list of returned columns if ORDER BY refers to indexes of that columns");
	if ((0 >= i) || (rv_count < i))
...
sparp_expand_top_retvals (sparp_t * sparp, SPART * query, int safely_copy_all_vars, dk_set_t binds_revlist)
{
  ...
  if (IS_BOX_POINTER (retvals))
	...
	return;
	  ...
	  NULL, &lnar, NULL, NULL, sparp_gp_trav_list_nonaggregate_retvals, NULL, sparp_gp_trav_list_nonaggregate_retvals, NULL);
      if (NULL != query->_.req_top.having)
	{
	  ...
	  sparp_gp_trav_int (sparp, query->_.req_top.having, stss + 1, &lnar,
	      NULL, NULL, sparp_gp_trav_list_nonaggregate_retvals, NULL, sparp_gp_trav_list_nonaggregate_retvals, NULL);
	}
      ...
      if (NULL != query->_.req_top.groupings)
	{
	  ...
	  sparp_gp_localtrav_treelist (sparp, query->_.req_top.groupings,
	      ...
	      sparp_gp_trav_list_subquery_retval_names, NULL, sparp_gp_trav_list_expn_retval_names, NULL, NULL, NULL);
...
sparp_wpar_retvars_in_max (sparp_t * sparp, SPART * query)
{
  ...
  if (((SELECT_L == query->_.req_top.subtype) ||
	  (DISTINCT_L == query->_.req_top.subtype)) && (NULL == formatter) && (NULL == agg_formatter))
    {
  ...
  sparp_gp_localtrav_treelist (sparp, retvals, NULL, NULL, NULL, NULL, sparp_gp_trav_wrap_vars_in_max, NULL, NULL, NULL);
}
...
sparp_expand_binds_like_macro (sparp_t * sparp, SPART ** expr_ptr, dk_set_t binds, SPART * parent_gp)
{
  switch (SPART_TYPE (expr_ptr[0]))
	...
	if (NULL == prev_bind)
	  return;
	expr_ptr[0] =
	    spartlist (sparp, 6, SPAR_ALIAS, sparp_tree_full_copy (sparp, prev_bind->_.alias.arg, parent_gp), vname,
	    SSG_VALMODE_AUTO, (ptrlong) 0, (ptrlong) 1);
	return;
      }
    case SPAR_GP:
      {
	...
	sparp_gp_trav (sparp, NULL /*unused */ , expr_ptr[0], &e,
	    ...
	    sparp_gp_trav_expand_binds_expn_in, NULL, sparp_gp_trav_expand_binds_expn_subq, NULL);
	return;
      }
    case SPAR_ALIAS:
      {
...
sparp_rewrite_retvals (sparp_t * sparp, SPART * req_top, int safely_copy_retvals)
{
  ...
  if (safely_copy_retvals)
    req_top->_.req_top.expanded_orig_retvals =
	sparp_treelist_full_copy (sparp, req_top->_.req_top.retvals, req_top->_.req_top.pattern);
  else
  ...
  if (DESCRIBE_L == req_top->_.req_top.subtype)
    {
	  ...
	  req_top->_.req_top.retvals, req_top->_.req_top.limit, req_top->_.req_top.offset);
    }
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_preopt_in_gp, NULL, NULL, NULL, sparp_gp_trav_preopt_expn_subq, NULL);
	...
	case SCORE_L:
	  {
	    if (SPART_VARR_GLOBAL & val->_.var.rvr.rvrRestrictions)
	      spar_error (sparp, "Only plain variables can be used in OFFBAND_L or SCORE_L options, not parameters like ?%.50s",
		  val->_.var.vname);
	...
	case T_STEP_L:
	  {
	    ...
	    sparp_equiv_get (sparp, gp, (SPART *) name,
		SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_ADD_SUBQUERY_USE);
	    break;
	  }
	case SAME_AS_L:
	case SAME_AS_O_L:
	...
	case T_OUT_L:
	  {
	    ...
	    {
	      sparp_equiv_t *eq =
		  sparp_equiv_get (sparp, gp, (SPART *) (v->_.var.vname), SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_GET_ASSERT);
      ...
      if ((SELECT_L == curr->_.gp.subtype) && (0 == curr->_.gp.equiv_count))
	{
	  ...
	  DO_BOX_FAST (SPART *, retval, ctr, curr->_.gp.subquery->_.req_top. /*orig_ */ retvals)
	  {
	    ...
	    if (SPART_VARNAME_IS_GLOB (name))
	      goto ignore_retval_name;	/* see below */
	    sparp_equiv_get (sparp, curr, (SPART *) name,
		SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_ADD_SUBQUERY_USE);
	  ignore_retval_name:;
	  }
	  END_DO_BOX_FAST;
...
sparp_gp_trav_cu_in_expns (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  switch (SPART_TYPE (curr))
    ...
    case SPAR_BOP_EQNAMES:
      {
	sparp_equiv_t *eq_l =
	    sparp_equiv_get (sparp, gp, (SPART *) (curr->_.bin_exp.left->_.var.vname),
	    SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES);
	sparp_equiv_t *eq_r =
	    sparp_equiv_get (sparp, gp, (SPART *) (curr->_.bin_exp.right->_.var.vname),
	    SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES);
...
sparp_gp_trav_cu_in_subq (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  {
    SPART *rel_gp;
    sparp_equiv_get (sparp, curr, (SPART *) varname,
	SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_ADD_OPTIONAL_READ);
    if (NULL != sts_this->sts_ancestor_gp)
      rel_gp = sts_this->sts_ancestor_gp;
    else
      rel_gp = sparp->sparp_stp->stp_trav_req_top->_.req_top.pattern;
    sparp_equiv_get (sparp, rel_gp, (SPART *) varname,
	SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_ADD_OPTIONAL_READ);
  }
  END_DO_SET ()sparp_gp_trav_cu_out_triples_1 (sparp, curr, sts_this, common_env);
  return 0;
}
...
sparp_gp_trav_cu_in_retvals (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  curr->_.var.rvr.rvrRestrictions |=
      SPART_VARR_EXPORTED
      /* This is redundand: if these bits are set, why set them again: | (curr->_.var.rvr.rvrRestrictions & (SPART_VARR_GLOBAL | SPART_VARR_EXTERNAL)) */
      ;
...
sparp_count_usages (sparp_t * sparp, SPART * req_top, dk_set_t * optvars_ret)
{
  sparp_gp_trav_top_pattern (sparp, req_top, optvars_ret,
      sparp_gp_trav_cu_in_triples, sparp_gp_trav_cu_out_triples_1, sparp_gp_trav_cu_in_expns, NULL, sparp_gp_trav_cu_in_subq, NULL);
  sparp_trav_out_clauses (sparp, req_top, optvars_ret,
      NULL, NULL, sparp_gp_trav_cu_in_retvals, NULL, sparp_gp_trav_cu_in_subq, NULL);
  if (NULL != optvars_ret)
    sparp_gp_trav_top_pattern (sparp, req_top, optvars_ret,
	NULL, sparp_gp_trav_cu_out_triples_2, NULL, NULL, sparp_gp_trav_cu_in_subq, NULL);
}
...
sparp_use_assume_eq (sparp_t * sparp, SPART * curr, SPART ** stmt_ptr, SPART * left, SPART * right)
{
  ...
  if (SPAR_ASSUME_IS_CONTRADICTION != stmt_ptr[0])
      ...
      if (mix_rvr.rvrRestrictions & SPART_VARR_CONFLICT)
	{
      ...
      addon_restrs =
	  (mix_rvr.rvrRestrictions & ~left_rvr.
	  rvrRestrictions & ~(SPART_VARR_FIXED | SPART_VARR_TYPED | SPART_VARR_SPRINTFF | SPART_VARR_IRI_CALC));
      if (0x0 != addon_restrs)
	sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, left, addon_restrs);
      addon_restrs =
	  (mix_rvr.rvrRestrictions & ~right_rvr.
	  rvrRestrictions & ~(SPART_VARR_FIXED | SPART_VARR_TYPED | SPART_VARR_SPRINTFF | SPART_VARR_IRI_CALC));
      if (0x0 != addon_restrs)
	sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, left, addon_restrs);
    }
...
sparp_use_assume (sparp_t * sparp, SPART * curr, SPART ** stmt_ptr)
{
  ...
  switch (stmt_type)
    ...
    case SPAR_QNAME:
    case SPAR_LIT:
      spar_error (sparp, "Constant expression in ASSUME is formally valid but too suspicious");
    case SPAR_BUILT_IN_CALL:
      {
	SPART *arg;
	...
	switch (stmt->_.builtin.btype)
	  ...
	  case SPAR_BIF_ISBLANK:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL | SPART_VARR_IS_REF | SPART_VARR_IS_BLANK);
	    return;
	  case SPAR_BIF_ISIRI:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL | SPART_VARR_IS_REF | SPART_VARR_IS_IRI);
	    return;
	  case SPAR_BIF_ISREF:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL | SPART_VARR_IS_REF);
	    return;
	  case SPAR_BIF_ISLITERAL:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL | SPART_VARR_IS_LIT);
	    return;
	  case SPAR_BIF_ISNUMERIC:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg,
		SPART_VARR_NOT_NULL | SPART_VARR_IS_LIT | SPART_VARR_LONG_EQ_SQL);
	    return;
	  case BOUND_L:
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL);
	    return;
	...
	if (!strcmp (qname, "bif:isnull"))
	  {
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_ALWAYS_NULL);
	    return;
	  }
	if (!strcmp (qname, "bif:isnotnull"))
	  {
	    sparp_use_assume_rvr_restr (sparp, curr, stmt_ptr, arg, SPART_VARR_NOT_NULL);
	    return;
	  }
...
int
sparp_merge_BOP_OR_of_INs_prep (SPART * tree, so_BOP_OR_filter_ctx_t * ctx, SPART ** var_ret, SPART *** vals_ret,
    int *val_count_ret)
{
  switch (SPART_TYPE (tree))
    {
...
sparp_optimize_BOP_OR_filter_walk (SPART ** filt_ptr, SPART ** filt_parent_ptr, so_BOP_OR_filter_ctx_t * ctx)
{
  ...
  switch (filt_type)
    ...
    case BOP_EQ:
    case SPAR_BOP_EQNAMES:	/* No case for SPAR_BOP_EQ_NONOPT ! */
...
sparp_optimize_BOP_OR_filter (sparp_t * sparp, SPART * parent_gp, SPART * gp, int gp_idx, int filt_idx)
{
  ...
  if (NULL != ctx.bofc_reason_for_union)
	...
	spar_error (sparp,
	    "A special predicate, like bif:contains or bif:st_intersects, can not be used as an argument of '||' operator in a \"joining\" FILTER at line %ld of query; please re-phrase the query",
	    (long) unbox (filt_ptr[0]->srcline));
      ...
      if (ctx.bofc_BOP_OR_of_reason_for_union == filt_ptr)
	{
	  ...
	  if (NULL != ft_type)
	    {
	      SPART *triple_with_var_obj =
		  sparp_find_triple_with_var_obj_of_freetext (sparp, case_with_rest, alt_for_reason,
		  SPAR_TRIPLE_FOR_FT_SHOULD_EXIST | SPAR_TRIPLE_SHOULD_HAVE_NO_FT_TYPE);
      ...
      if (ctx.bofc_BOP_OR_of_reason_for_union == filt_ptr)
	{
	  ...
	  if (NULL != ft_type)
	    {
	      SPART *triple_with_var_obj =
		  sparp_find_triple_with_var_obj_of_freetext (sparp, gp, ctx.bofc_reason_for_union,
		  SPAR_TRIPLE_FOR_FT_SHOULD_EXIST | SPAR_TRIPLE_SHOULD_HAVE_NO_FT_TYPE);
	      triple_with_var_obj->_.triple.ft_type = ft_type;
	    }
	}
  ...
  if (ctx.bofc_not_optimizable)
    {
      while (NULL != ctx.bofc_strings)
	dk_set_pop (&(ctx.bofc_strings));
      return 0;
    }
...
spar_var_eq_to_equiv (sparp_t * sparp, SPART * curr, sparp_equiv_t * eq_l, SPART * r)
{
  ...
  switch (SPART_TYPE (r))
	...
	if ((SPARP_EQUIV_MERGE_OK != ret) && (SPARP_EQUIV_MERGE_CONFLICT != ret) && (SPARP_EQUIV_MERGE_DUPE != ret))
	  return 0;
	...
	switch (r->_.builtin.btype)
	  {
	  case SPAR_BIF_STR:
	    eq_l->e_rvr.rvrRestrictions |= SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL;
	    break;
	  case IRI_L:
	    eq_l->e_rvr.rvrRestrictions |= SPART_VARR_IS_REF | SPART_VARR_NOT_NULL;
	    break;
	  default:
	    {
...
spar_var_bangeq_to_equiv (sparp_t * sparp, SPART * curr, sparp_equiv_t * eq_l, SPART * r)
{
  ...
  switch (SPART_TYPE (r))
    ...
    case SPAR_QNAME:
      {
	if (sparp_values_equal (sparp, eq_l->e_rvr.rvrFixedValue, eq_l->e_rvr.rvrDatatype, eq_l->e_rvr.rvrLanguage, (ccaddr_t) r,
		NULL, NULL))
	  {
	    SPARP_DEBUG_WEIRD (sparp, "conflict");
	    eq_l->e_rvr.rvrRestrictions |= SPART_VARR_CONFLICT;
...
/* Now filters can be processed */
  switch (SPART_TYPE (filt))
    {
    case BOP_EQ:
    case SPAR_BOP_EQNAMES:	/* No case for SPAR_BOP_EQ_NONOPT ! Indeed, this is the main reason for introducing SPAR_BOP_EQ_NONOPT at all */
      {
	SPART *l = filt->_.bin_exp.left;
	SPART *r = filt->_.bin_exp.right;
	switch (SPART_TYPE (l))
	  ...
	  case SPAR_QNAME:
	    {
	      ...
	      if ((SPAR_BUILT_IN_CALL == SPART_TYPE (r)) && (DATATYPE_L == r->_.builtin.btype))
		{
		  ...
		  if (SPAR_IS_BLANK_OR_VAR (rarg1))
		    {
		      ...
		      if ((lval == uname_xmlschema_ns_uri_hash_boolean) || (lval == uname_xmlschema_ns_uri_hash_string))
			{
			  ...
			  (lval == uname_xmlschema_ns_uri_hash_integer) || (lval == uname_xmlschema_ns_uri_hash_time))
			{
	      ...
	      if ((NULL != str_lval) && (SPAR_BUILT_IN_CALL == SPART_TYPE (r)) && (SPAR_BIF_STR == r->_.builtin.btype))
		{
		  ...
		  if (SPAR_IS_BLANK_OR_VAR (rarg1))
		      ...
		      if (SPART_VARR_IS_REF & rarg1_eq->e_rvr.rvrRestrictions)
			{
			  ...
			  int restr_ret;
			  SPART *lval_tmp_qname =
			      spartlist (sparp, 2, SPAR_QNAME, t_box_dv_uname_nchars (str_lval, box_length (str_lval) - 1));
			  restr_ret = sparp_equiv_restrict_by_constant (sparp, rarg1_eq, NULL, lval_tmp_qname, NULL);
			  if ((SPARP_EQUIV_MERGE_OK != restr_ret) &&
			      (SPARP_EQUIV_MERGE_CONFLICT != restr_ret) && (SPARP_EQUIV_MERGE_DUPE != restr_ret))
			    return 0;
...
{
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_restrict_by_simple_filters_gp_in, NULL, NULL, NULL, sparp_gp_trav_restrict_by_simple_filters_expn_subq, NULL);
}
...
sparp_make_common_eqs (sparp_t * sparp, SPART * req_top)
{
  ...
  sparp_trav_out_clauses (sparp, req_top, &top_vars_to_propagate,
      ...
      NULL, NULL, sparp_gp_trav_make_common_eqs_expn_subq, NULL);
...
void
sparp_gp_add_chain_aliases (sparp_t * sparp, SPART * inner_var, sparp_equiv_t * inner_eq, sparp_trav_state_t * sts_this,
    SPART * top_gp)
{
...
sparp_gp_trav_remove_unused_aliases (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  SPARP_REVFOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
  {
	...
	(0 != eq->e_var_count) || (SPART_VARR_EXPORTED & eq->e_rvr.rvrRestrictions) || eq->e_replaces_filter)
...
sparp_make_aliases (sparp_t * sparp, SPART * req_top)
{
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, retvars,
      sparp_gp_trav_make_retval_aliases, sparp_gp_trav_cu_out_triples_1, NULL, NULL, NULL, NULL);
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
...
sparp_find_external_namesake_eq_of_varname (sparp_t * sparp, caddr_t varname, dk_set_t parent_gps)
{
  DO_SET (SPART *, parent, &parent_gps)
  {
    ...
    if ((NULL != parent_eq) &&
	((0 < parent_eq->e_gspo_uses) || (0 < parent_eq->e_subquery_uses) || (0 < parent_eq->e_nested_bindings)))
      {
	/*
...
sparp_gp_trav_label_external_vars_gp_in (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, curr, eqctr, eq)
  {
    ...
    {
      sparp_equiv_t *external_namesake_eq =
	  sparp_find_external_namesake_eq_of_varname (sparp, varname, sleve->parent_gps_for_var_search);
  ...
  if (SPAR_IS_BLANK_OR_VAR (curr))
    {
      ...
      sparp_equiv_t *external_namesake_eq =
	  sparp_find_external_namesake_eq_of_varname (sparp, curr->_.var.vname, sleve->parent_gps_for_var_search);
      if (NULL != external_namesake_eq)
	{
...
sparp_label_external_vars (sparp_t * sparp, SPART * req_top, sparp_label_external_vars_env_t * sleve)
{
  ...
  sparp_trav_out_clauses (sparp, req_top, sleve,
	  ...
	  sparp_gp_trav_label_external_vars_expn_in : NULL), NULL, sparp_gp_trav_label_external_vars_expn_subq, NULL);
  sparp_gp_trav_top_pattern (sparp, req_top, sleve,
      sparp_gp_trav_label_external_vars_gp_in, NULL, NULL, NULL, sparp_gp_trav_label_external_vars_expn_subq, NULL);
}
...
sparp_remove_redundant_connections (sparp_t * sparp, SPART * req_top, ptrlong flags)
{
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      ...
      NULL, NULL, sparp_gp_trav_remove_redundant_connections, NULL);
  sparp_trav_out_clauses (sparp, req_top, NULL, NULL, NULL /* was sparp_gp_trav_cu_out_triples_1 */ ,
      NULL, NULL, sparp_gp_trav_remove_redundant_connections, NULL);
  ...
  for (eq_ctr = sparp->sparp_sg->sg_equiv_count; eq_first_idx < eq_ctr--; /*no step */ )
      ...
      for (sub_ctr = BOX_ELEMENTS_INT_0 (eq->e_subvalue_idxs); sub_ctr--; /*no step */ )
	{
	  ...
	  if (SPARP_EQ_IS_ASSIGNED_BY_CONTEXT (eq) &&
	      ...
	      (1 == BOX_ELEMENTS (sub_eq->e_varnames)) && !strcmp (eq->e_varnames[0], sub_eq->e_varnames[0]))
	    can_unlink = 1;
...
skip_ext_ext_unlinks:
  sparp_gp_trav_top_pattern (sparp, req_top, NULL, sparp_gp_trav_remove_unused_aliases, NULL, NULL, NULL, NULL, NULL);
}
...
sparp_restr_of_union_eq_from_connected_subvalues (sparp_t * sparp, sparp_equiv_t * eq)
{
  ...
  if ((acc.rvrRestrictions & SPART_VARR_NOT_NULL) && (!(eq->e_rvr.rvrRestrictions & SPART_VARR_NOT_NULL)))
    dbg_printf (("sparp_" "restr_of_union_eq_from_connected_subvalues(): strong optimization on ?%s (was 0x%x) by 0x%x acc",
	    eq->e_varnames[0], (unsigned) (eq->e_rvr.rvrRestrictions), (unsigned) (acc.rvrRestrictions)));
#endif
...
void
sparp_find_best_join_eq_for_optional (sparp_t * sparp, SPART * parent, int pos_of_curr_memb, sparp_equiv_t * eq,
    sparp_equiv_t ** ret_parent_eq, SPART ** ret_tree_in_parent, SPART ** ret_source_in_parent)
{
  SPART *prev = NULL;
  int varname_ctr, ctr;
  ...
  for (pos_of_prev_memb = 0; pos_of_prev_memb < pos_of_curr_memb; pos_of_prev_memb++)
      ...
      if (SPAR_GP == prev->type)
	{
	  DO_BOX_FAST (caddr_t, varname, varname_ctr, eq->e_varnames)
	  {
	    ...
	    parent_eq =
		sparp_equiv_get_ro (sparp->sparp_sg->sg_equivs, sparp->sparp_sg->sg_equiv_count, parent, (SPART *) varname,
		SPARP_EQUIV_GET_NAMESAKES);
	    if (NULL == parent_eq)
	      continue;
	    prev_eq =
		sparp_equiv_get_ro (sparp->sparp_sg->sg_equivs, sparp->sparp_sg->sg_equiv_count, prev, (SPART *) varname,
		SPARP_EQUIV_GET_NAMESAKES);
	    if (NULL == prev_eq)
	      continue;
	    good_prev = prev;
...
int
sparp_gp_trav_eq_restr_from_connected_subvalues_gp_out (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this,
    void *common_env)
{
  int eq_ctr;
  if (SPAR_GP != SPART_TYPE (curr))
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
  {
    switch (curr->_.gp.subtype)
      {
      case SELECT_L:
	sparp_restr_of_select_eq_from_connected_subvalues (sparp, eq);
	break;
      case UNION_L:
...
int
sparp_gp_trav_eq_restr_from_connected_subvalues_expn_subq (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this,
    void *common_env)
{
  sparp_continue_gp_trav_in_sub (sparp, curr, common_env, 0);
  return 0;
...
int
sparp_gp_trav_eq_restr_from_connected_receivers_gp_in (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this,
    void *common_env)
{
  sparp_equiv_t **equivs = sparp->sparp_sg->sg_equivs;
  int eq_ctr;
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
  {
	...
	for (sts_iter = sts_this;
	    ...
	    (SPAR_GP == SPART_TYPE (sts_iter->sts_parent)) && (SELECT_L != sts_iter->sts_parent->_.gp.subtype); sts_iter--)
	  {
	    sparp_equiv_t *outer_eq =
		sparp_equiv_get_ro (equivs, sparp->sparp_sg->sg_equiv_count, sts_iter->sts_parent, (SPART *) (var->_.var.vname),
		SPARP_EQUIV_GET_NAMESAKES);
	    if (NULL != outer_eq)
  ...
  if (VALUES_L == curr->_.gp.subtype)
    {
      ...
      SPART *parent_gp = (((NULL != sts_this->sts_parent)
	      && (SPAR_GP == SPART_TYPE (sts_this->sts_parent))) ? sts_this->sts_parent : NULL);
...
int
sparp_gp_trav_eq_restr_from_connected_receivers_expn_subq (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this,
    void *common_env)
{
  sparp_continue_gp_trav_in_sub (sparp, curr, common_env, 0);
  return 0;
...
sparp_eq_restr_from_connected (sparp_t * sparp, SPART * req_top)
{
  ...
  if (NULL != binv)
      ...
      for (varctr = varcount; varctr--; /* no step */ )
	{
	  ...
	  if (NULL == ret_expn)
		...
		spar_error (sparp, "Variable name '%.100s' is used in the BINDINGS clause but not in the query result set",
		    var->_.var.vname);
	      SPARP_DEBUG_WEIRD (sparp, "conflict");
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      ...
      NULL, NULL, sparp_gp_trav_eq_restr_from_connected_subvalues_expn_subq, NULL);
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_eq_restr_from_connected_receivers_gp_in, NULL,
      NULL, NULL, sparp_gp_trav_eq_restr_from_connected_receivers_expn_subq, NULL);
}
...
sparp_eq_restr_to_vars (sparp_t * sparp, SPART * req_top)
{
  /*sparp_env_t *env = sparp->sparp_env; */
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_eq_restr_to_vars_gp_in, NULL, NULL, NULL, sparp_gp_trav_eq_restr_to_vars_expn_subq, NULL);
}
...
sparp_gp_trav_equiv_audit_inner_vars (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  switch (SPART_TYPE (curr))
      ...
      if (SPAR_REQ_TOP == SPART_TYPE (curr->_.gp.subquery))
	{
	  ...
	  sparp_gp_trav_int (sparp, curr->_.gp.subquery->_.req_top.pattern, stss + 1, common_env,
	      ...
	      sparp_gp_trav_equiv_audit_inner_vars, NULL, sparp_gp_trav_equiv_audit_inner_vars, NULL);
  ...
  if (SPART_BAD_EQUIV_IDX != curr->_.var.equiv_idx)
      ...
      if (NULL == eq_by_id)
	{
      ...
      if (eq_by_id->e_gp != gp)
	spar_audit_error (sparp,
	    "sparp_" "gp_trav_equiv_audit_inner_vars(): e_gp of eq_by_id does not match gp, gp %s var %s/%s/%s", gp->_.gp.selid,
	    curr->_.var.selid, curr->_.var.tabid, curr->_.var.vname);
      eq = sparp_equiv_get (sparp, gp, curr, 0);
      if (NULL == eq)
	{
	  spar_audit_error (sparp, "sparp_" "gp_trav_equiv_audit_inner_vars(): eq not found, gp %s var %s/%s/%s", gp->_.gp.selid,
	      curr->_.var.selid, curr->_.var.tabid, curr->_.var.vname);
	  return 0;
	}
      if (eq->e_own_idx != curr->_.var.equiv_idx)
	spar_audit_error (sparp, "sparp_" "gp_trav_equiv_audit_inner_vars(): eq idx mismatch, gp %s var %s/%s/%s", gp->_.gp.selid,
	    curr->_.var.selid, curr->_.var.tabid, curr->_.var.vname);
      if (strcmp (gp->_.gp.selid, curr->_.var.selid))
	spar_audit_error (sparp, "sparp_" "gp_trav_equiv_audit_inner_vars(): var selid differs from gp selid, gp %s var %s/%s/%s",
	    gp->_.gp.selid, curr->_.var.selid, curr->_.var.tabid, curr->_.var.vname);
    }
...
sparp_equiv_audit_retvals (sparp_t * sparp, SPART * top)
{
  ...
  DO_BOX_FAST (SPART *, expn, ctr, top->_.req_top.retvals)
  {
	...
	NULL, NULL, sparp_gp_trav_equiv_audit_retvals, NULL, NULL, NULL);
  }
  END_DO_BOX_FAST;
...
sparp_equiv_audit_all (sparp_t * sparp, int flags)
{
  ...
  for (eq_ctr = sparp->sparp_first_equiv_idx; eq_ctr < eq_count; eq_ctr++)
	...
	spar_audit_error (sparp,
	    "sparp_" "equiv_audit_all(): No vars with global names, but SPART_VARR_GLOBAL of var is set, equiv_idx %d in %s",
	    eq_ctr, gp->_.gp.selid);
...
#if 0
      if (!SPARP_EQ_IS_ASSIGNED_LOCALLY (eq) && !SPARP_EQ_IS_ASSIGNED_EXTERNALLY (eq) && ((0 != eq->e_const_reads) ||	/* no check for (0 != eq->e_optional_reads) */
...
int
sparp_gp_trav_check_if_local (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
...
sparp_tree_is_global_expn (sparp_t * sparp, SPART * tree)
{
  ...
  res = sparp_gp_trav (sparp, NULL /* unused */ , tree, NULL,
      NULL, NULL, sparp_gp_trav_check_if_local, NULL, NULL /*!!!TBD add */ ,
      NULL);
  if (sparp_inside_gp_trav)
    sparp_gp_trav_resume (sparp);
...
int
sparp_gp_trav_expn_reads_equiv (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
...
{
  bif_define_ex ("SPECIAL::bif:COUNT", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_special_bif_agg, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_RET_TYPE, &bt_integer, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("SPECIAL::bif:MAX", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_special_bif_agg, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("SPECIAL::bif:MIN", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_special_bif_agg, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("SPECIAL::bif:AVG", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_special_bif_agg, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("SPECIAL::bif:SUM", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_special_bif_agg, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("sparql_only:args_in_same_eq", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_args_in_same_eq, BMD_MIN_ARGCOUNT, 2,
      BMD_MAX_ARGCOUNT, 2, BMD_RET_TYPE, &bt_integer, BMD_SPARQL_ONLY, BMD_DONE);
  bif_define_ex ("sparql_only:arg_is_local_var", NULL, BMD_SPARQL_OPTIMIZER_IMPL, bifsparqlopt_arg_is_local_var, BMD_MIN_ARGCOUNT,
      1, BMD_MAX_ARGCOUNT, 1, BMD_RET_TYPE, &bt_integer, BMD_SPARQL_ONLY, BMD_DONE);
}
...
sparp_literal_is_xsd_valid (sparp_t * sparp, ccaddr_t sqlval, ccaddr_t dt_iri, ccaddr_t lang)
{
  ...
  if (!strncmp (dt_iri, XMLSCHEMA_NS_URI "#", XMLSCHEMA_NS_URI_LEN + 1 /* +1 is for '#' */ ))
      ...
      else
	{
	  ...
	  END_QR_RESET dk_free_tree (parsed_value);
	  return 1;
	}
    }
  return 1;
...
sparp_simplify_builtin (sparp_t * sparp, SPART * tree, int *trouble_ret)
{
  ...
  switch (tree->_.builtin.btype)
    ...
    case SPAR_BIF_REGEX:
      break;
    case SPAR_BIF_REMOVE_UNICODE3_ACCENTS:
      break;
    ...
    case SPAR_BIF_STR:
      break;
    case SPAR_BIF_STRAFTER:
      break;
    case SPAR_BIF_STRBEFORE:
      break;
    case SPAR_BIF_STRDT:
      break;
    case SPAR_BIF_STRENDS:
      break;
    case SPAR_BIF_STRLANG:
      break;
    case SPAR_BIF_STRLEN:
      break;
    case SPAR_BIF_STRSTARTS:
      break;
    case SPAR_BIF_STRUUID:
      break;
    case SPAR_BIF_SUBSTR:
      break;
    case SPAR_BIF_TIMEZONE:
      break;
    case SPAR_BIF_TZ:
      break;
    case SPAR_BIF_UCASE:
...
sparp_gp_trav_simplify_expn_out (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  switch (curr_type)
    ...
    case SPAR_MACROPU:
	  ...
	  "sparp_" "gp_trav_simplify_expn_out(): the expn_out callback should not get this type of subtree");
      return 0;
    case SPAR_BUILT_IN_CALL:
      {
	SPART *arg1;
	...
	if (NULL != bmd)
	  {
		...
		int trouble = 0;
		res =
		    spar_run_pure_bif_in_sandbox (sparp, qname, curr->_.funcall.argtrees, BOX_ELEMENTS (curr->_.funcall.argtrees),
		    bmd, &trouble);
		if (!trouble)
		  goto res_done;	/* see below */
		if (2 == trouble)
...
sparp_simplify_expns (sparp_t * sparp, SPART * req_top)
{
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      NULL, NULL, NULL, sparp_gp_trav_simplify_expn_out, sparp_gp_trav_simplify_expn_subq, NULL);
...
spar_binv_is_convertible_to_filter (sparp_t * sparp, SPART * parent_gp, SPART * member_gp, SPART * member_binv)
{
  ...
  eq = sparp_equiv_get_ro (sparp->sparp_sg->sg_equivs, sparp->sparp_sg->sg_equiv_count, parent_gp,
...
sparp_check_field_mapping_of_cvalue (sparp_t * sparp, SPART * cvalue, rdf_val_range_t * qmv_or_fmt_rvr, rdf_val_range_t * rvr)
{
  ...
  if (NULL != qmv_or_fmt_rvr)
	...
	{
	  if (!sparp_values_equal (sparp, (ccaddr_t) cvalue, NULL, NULL, qmv_or_fmt_rvr->rvrFixedValue, qmv_or_fmt_rvr->rvrDatatype,
		  qmv_or_fmt_rvr->rvrLanguage))
	    return SSG_QM_NO_MATCH;
	  return SSG_QM_PROVEN_MATCH;
	}
    }
    ...
    rdf_val_range_t * qmv_or_fmt_rvr, rdf_val_range_t * rvr, int invalidation_level)
{
  ...
  DO_BOX_FAST (SPART *, source, source_ctr, tcc->tcc_sources)
  {
	...
	if ((NULL != qmv_or_fmt_rvr) && (SPART_VARR_FIXED & qmv_or_fmt_rvr->rvrRestrictions) &&
	    sparp_values_equal (sparp, source->_.graph.iri, NULL, NULL, qmv_or_fmt_rvr->rvrFixedValue, qmv_or_fmt_rvr->rvrDatatype,
		qmv_or_fmt_rvr->rvrLanguage))
	  return SSG_QM_NO_MATCH;
	continue;
      }
    ...
    quad_map_t * qm, qm_value_t * qmv, rdf_val_range_t * rvr, int invalidation_level)
{
  ...
  if (!tcc->tcc_check_source_graphs && (qm != tcc->tcc_qs->qsDefaultMap) &&
      (tcc->tcc_qs->qsMatchingFlags & SPART_QS_NO_IMPLICIT_USER_QM) && (SPAR_BLANK_NODE_LABEL == SPART_TYPE (field)))
    return SSG_QM_NO_MATCH;
...
#ifdef DEBUG
	      GPF_T1
		  ("sparp_check_field_mapping_g(): non-UNAME fixed value of variable used as graph of a triple. Legal but strange");
#else
	      return SSG_QM_NO_MATCH;
#endif
	    }
...
/* First pass is optimistic and tries to find an exact equality */
		...
		for (qmv_ctr = qmv_or_fmt_rvr->rvrSprintffCount; qmv_ctr--; /* no step */ )
		  if (!strcmp (field->_.var.rvr.rvrSprintffs[fld_ctr], qmv_or_fmt_rvr->rvrSprintffs[qmv_ctr]))
		    goto field_sff_isects_qmv_sff;
/* Second pass checks everything, slowly */
	      for (fld_ctr = field->_.var.rvr.rvrSprintffCount; fld_ctr--; /* no step */ )
		for (qmv_ctr = qmv_or_fmt_rvr->rvrSprintffCount; qmv_ctr--; /* no step */ )
		  if (NULL != sprintff_intersect (field->_.var.rvr.rvrSprintffs[fld_ctr], qmv_or_fmt_rvr->rvrSprintffs[qmv_ctr], 1))
		    goto field_sff_isects_qmv_sff;
	      return SSG_QM_NO_MATCH;	/* reached if no match found */
    ...
    quad_map_t * qm, qm_value_t * qmv, rdf_val_range_t * rvr)
{
  ...
  if (NULL != qmv)
	...
	{
	  if ((SPART_VARR_SPRINTFF | SPART_VARR_FIXED | SPART_VARR_IS_LIT | SPART_VARR_IS_REF) & qmv->qmvFormat->qmfValRange.
	      rvrRestrictions)
	    qmv_or_fmt_rvr = &(qmv->qmvFormat->qmfValRange);
	}
    }
...
sparp_check_triple_case (sparp_t * sparp, tc_context_t * tcc, quad_map_t * qm, int invalidation_level)
{
  ...
  int g_match = SSG_QM_UNSET, s_match = SSG_QM_UNSET, p_match = SSG_QM_UNSET, o_match = SSG_QM_UNSET;
  if (NULL == qm)
    return SSG_QM_NO_MATCH;
...
sparp_find_triple_cases (sparp_t * sparp, SPART * triple, SPART ** sources, int required_source_type)
{
  ...
  DO_BOX_FAST (SPART *, source, source_ctr, tmp_tcc.tcc_sources)
  {
    ...
    else if (!((SPART_GRAPH_NOT_FROM == source->_.graph.subtype) || (SPART_GRAPH_NOT_NAMED == source->_.graph.subtype)))
  ...
  if ((SPART *) ((ptrlong) _STAR) == sinv_idx_and_qms[1])
      ...
      for (ctr = BOX_ELEMENTS (tmp_tcc.tcc_top_allowed_qms); ctr--; /* no step */ )
	{
	  ...
	  if (((caddr_t) DEFAULT_L) == triple_qm_iri)
		...
		spar_error (sparp, "QUAD MAP DEFAULT group pattern is used in RDF storage '%.200s' that has no default quad map",
		    triple_storage_iri);
  ...
  DO_BOX_FAST (quad_map_t *, qm, ctr, triple_storage->qsMjvMaps)
  {
    ...
    if (0 != BOX_ELEMENTS_0 (qm->qmUserSubMaps))
      spar_error (sparp,
	  "RDF quad mapping metadata are corrupted: MJV has submaps; the quad storage '%.200s' used in the query should be configured again",
	  triple_storage_iri);
    if (SPART_QM_EMPTY & qm->qmMatchingFlags)
      spar_error (sparp,
	  "RDF quad mapping metadata are corrupted: MJV is declared as empty; the quad storage '%.200s' used in the query should be configured again",
	  triple_storage_iri);
...
full_exclusive_match_detected:
  ...
  if (NULL == tmp_tcc.tcc_found_cases)
    {
      ...
      printf ("\nStorage : ");
      dbg_print_box (triple_storage_iri, stdout);
      printf ("\nGraph   : ");
      dbg_print_box ((caddr_t) (triple->_.triple.tr_graph), stdout);
      printf ("\nSubj    : ");
      dbg_print_box ((caddr_t) (triple->_.triple.tr_subject), stdout);
      printf ("\nPred    : ");
      dbg_print_box ((caddr_t) (triple->_.triple.tr_predicate), stdout);
      printf ("\nObj     : ");
      dbg_print_box ((caddr_t) (triple->_.triple.tr_object), stdout);
      printf ("\n");
    }
#endif
#endif
  ...
  mp_box_tag_modify (res_list, DV_ARRAY_OF_LONG);
  while (tmp_tcc.tcc_found_cases)
    dk_set_pop (&(tmp_tcc.tcc_found_cases));
  for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
    {
...
sparp_refresh_triple_cases (sparp_t * sparp, SPART ** sources, SPART * triple)
{
  ...
  if ((NULL == triple->_.triple.tc_list) && (0 == new_cases_count) && sparp->sparp_sg->sg_signal_void_variables)
	...
	spar_dbg_string_of_triple_field (sparp, triple->_.triple.tr_object), (long) unbox (triple->srcline));
      ...
      for (ctr = 0; ctr < new_cases_count; ctr++)
	{
	  ...
	  if (NULL != qmv)
	      ...
	      if (SPART_VARR_SPRINTFF & qmv_fmt->qmfValRange.rvrRestrictions)
		{
		  ...
		  sparp_rvr_add_sprintffs (sparp, &qmv_rvr, qmv_fmt->qmfValRange.rvrSprintffs,
		      qmv_fmt->qmfValRange.rvrSprintffCount);
		}
	  ...
	  else if (fld_const_rvr->rvrRestrictions & SPART_VARR_FIXED)
	    {
	      if (NULL != qmv)
		spar_internal_error (sparp,
		    "Invalid quad map storage metadata: quad map has set both quad map value and a constant for same field.");
#ifdef DEBUG
	    ...
	    spar_internal_error (sparp,
		"Invalid quad map storage metadata: neither quad map value nor a constant is set for a field of a quad map.");
	  if (NULL != qmv)
	    {
  ...
  for (memb_ctr = BOX_ELEMENTS (parent_gp->_.gp.members); memb_ctr--; /*no step */ )
    {
      ...
      if ((SPAR_GP == SPART_TYPE (memb)) && (0 == BOX_ELEMENTS_0 (memb->_.gp.filters)) &&	/* This condition might be commented out if memb_filters and memb_filters_count below are uncommented */
	  (NULL == memb->_.gp.options) &&
	  ((parent_gp->_.gp.subtype == memb->_.gp.subtype) ||
	      ((0 == memb->_.gp.subtype) &&
		  (1 == BOX_ELEMENTS (memb->_.gp.members)) && (SPAR_GP == SPART_TYPE (memb->_.gp.members[0])))))
	{
...
sparp_flatten_join (sparp_t * sparp, SPART * parent_gp)
{
  ...
  if ((UNION_L == parent_gp->_.gp.subtype) || (SPAR_UNION_WO_ALL == parent_gp->_.gp.subtype)
      || (SELECT_L == parent_gp->_.gp.subtype) || (VALUES_L == parent_gp->_.gp.subtype))
...
*/
      ...
      for (sub_ctr = sub_count; sub_ctr--; /* no step */ )
	{
      ...
      if (0 != memb_filters_count)
	sparp_gp_attach_many_filters (sparp, parent_gp,
	    memb_filters /*!!! should it be sparp_treelist_full_copy (sparp, memb_filters, NULL) ? */ , 0, NULL);
      memb_ctr += sub_count;
...
{
  int field_ctr;
  if (!strcmp (triple->_.triple.tabid, new_tabid) && !strcmp (triple->_.triple.selid, new_selid))
    return;
  for (field_ctr = SPART_TRIPLE_FIELDS_COUNT; field_ctr--; /*no step */ )
    {
...
/* Add more cases right above this line when introducing more SPAR_nnn constants that may appear inside expression */
    default:
      spar_internal_error (sparp, "sparp_" "expns_are_equal () get expression of unsupported type");
    }
  GPF_T;
  return 0;
}
	...
	if (SPAR_IS_BLANK_OR_VAR (fld_expn))
	  {
	    ...
	    eq = sparp_equiv_get (sparp, qm_case_gp, new_fld_expn,
		SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_INS_VARIABLE | SPARP_EQUIV_ADD_GSPO_USE);
...
#if 0
	    sparp_rvr_tighten (sparp, (&new_fld_expn->_.var.rvr), &(eq->e_rvr),
		~0 /* not (SPART_VARR_EXTERNAL | SPART_VARR_GLOBAL) */ );
#else
...
sparp_gp_produce_nothing (sparp_t * sparp, SPART * curr)
{
  ...
  SPARP_REVFOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
  {
    ...
    DO_BOX_FAST (ptrlong, recv_eq_idx, recv_eq_ctr, eq->e_receiver_idxs)
    {
      ...
      if ((UNION_L != recv_eq->e_gp->_.gp.subtype) && (SPAR_UNION_WO_ALL != recv_eq->e_gp->_.gp.subtype)
	  && (OPTIONAL_L != curr->_.gp.subtype))
	{
	  SPARP_DEBUG_WEIRD (sparp, "conflict");
...
int
sparp_gp_trav_refresh_triple_cases (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  switch (curr->type)
    {
    case SPAR_TRIPLE:
...
int
sparp_gp_trav_multiqm_to_unions (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  {				/* countdown direction of 'for' is important due to possible insertions/removals */
	...
	if (NULL == ft_cond_to_relocate)
	  spar_error (sparp,
	      "optimizer can not process a combination of quad map patterns and free-text condition for variable ?%.200s",
...
int
sparp_gp_trav_detach_conflicts_out (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  if (SPAR_GP != curr->type)	/* Not a gp ? -- nothing to do */
    return 0;
  if (sparp_detach_conflicts (sparp, curr))
...
int
sparp_gp_trav_1var (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  for (filt_ctr = filt_count; filt_ctr--; /* no step */ )
    {
      ...
      sparp_gp_trav_int (sparp, filt, sts_this, &(single_var), NULL, NULL, sparp_gp_trav_1var, NULL, NULL, NULL);
	...
	switch (sub_gp->_.gp.subtype)
	  ...
	  case SELECT_L:
	    subval_count--;
	    break;		/*!!!TBD now HAVING is supported so filter can be moved inside subselect if there's no LIMIT/OFFSET */
	  case SERVICE_L:
	    subval_count--;
	    break;
	  case VALUES_L:
	    subval_count--;
	    break;		/* can't localize inside procedure view */
	  case OPTIONAL_L:
	    {
	      if (!(SPART_VARR_NOT_NULL & sv_eq->e_rvr.rvrRestrictions))
	...
	switch (sub_gp->_.gp.subtype)
	  ...
	  case SPAR_UNION_WO_ALL:
	    ...
	    break;
	  case SELECT_L:
	    continue;		/*!!!TBD see comment above */
	  case SERVICE_L:
	    continue;
	  case VALUES_L:
	    continue;
...
sparp_calc_importance_of_eq (sparp_t * sparp, sparp_equiv_t * eq)
{
  int res = 16 * eq->e_subquery_uses +
      4 * eq->e_gspo_uses +
      4 * eq->e_nested_bindings + 3 * eq->e_optional_reads + 2 * eq->e_const_reads + 2 * BOX_ELEMENTS_0 (eq->e_receiver_idxs);
...
sparp_gp_trav_union_of_joins_out (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  if ((0 == curr->_.gp.subtype) || (WHERE_L == curr->_.gp.subtype))
      ...
      for (case_ctr = 0; case_ctr < case_count; case_ctr++)
	{
	  ...
	  sparp_gp_attach_many_filters (sparp, new_join, new_filts_u,
	      BOX_ELEMENTS (new_join->_.gp.filters) - new_join->_.gp.glued_filters_count, NULL);
	  new_join->_.gp.glued_filters_count += union_glued_filters_count;
	  sparp_gp_attach_many_filters (sparp, new_join, new_filts_j,
	      BOX_ELEMENTS (new_join->_.gp.filters) - new_join->_.gp.glued_filters_count, NULL);
...
static void
sparp_collect_single_atable_use (sparp_t * sparp_or_null, ccaddr_t alias, ccaddr_t tablename, qm_atable_use_t * uses,
    ptrlong * use_count_ptr)
{
  ...
  if (ECM_MEM_NOT_FOUND == old_qmatu_idx)
      ...
      if (strcmp (tablename, qmatu->qmatu_tablename))
	{
	  if (NULL == sparp_or_null)
	    sqlr_new_error ("22023", "SR640", "internal error in processing table \"%.200s\" (alias \"%.200s\") in some RDF View",
		tablename, alias);
	  else
...
static void
sparp_collect_atable_uses (sparp_t * sparp_or_null, ccaddr_t singletablename, qm_atable_array_t qmatables, qm_atable_use_t * uses,
    ptrlong * use_count_ptr)
{
  int ata_ctr;
...
sparp_collect_all_atable_uses (sparp_t * sparp_or_null, quad_map_t * qm)
{
  ...
  for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
	...
	if ((NULL == col->qmvcAlias) || ('\0' == col->qmvcAlias[0]))
	  ...
	  (default_qm_val_table_used ? qmv->qmvTableName : NULL), qmv->qmvATables, uses, &use_count);
    }
  sparp_collect_atable_uses (sparp_or_null, (default_qm_table_used ? qm->qmTableName : NULL), qm->qmATables, uses, &use_count);
...
#if 0
static int
sparp_atable_uses_match (qm_atable_array_t qmatables, qm_atable_use_t * uses, int use_count)
{
...
sparp_gp_may_reuse_tabids_in_union (sparp_t * sparp, SPART * gp, int expected_triples_count)
{
  ...
  if ((SPAR_GP != gp->type) || (0 != gp->_.gp.subtype))
    return NULL;
...
sparp_bitmask_fields_with_equations (sparp_t * sparp, SPART * triple)
{
  ...
  for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
      ...
      if (!SPAR_IS_BLANK_OR_VAR (fld))
	{
	  ...
	  (restr & (SPART_VARR_GLOBAL | SPART_VARR_EXTERNAL)) || ((restr & SPART_VARR_FIXED) && (NULL != fld_qmv)));
...
sparp_try_reuse_tabid_in_union (sparp_t * sparp, SPART * curr, int base_idx)
{
  ...
  for (dep_idx = base_idx + 1;	/* breakup optimization is symmetrical so the case of two triples should be considered only once, not base_triple...dep then dep...base_triple */
      ...
      for (bt_ctr = base_triples_count; bt_ctr--; /* no step */ )
	{
	  ...
	  if ((base_bitmask_of_equations & (1 << SPART_TRIPLE_OBJECT_IDX)) && !(base_bitmask_of_equations & (1 << SPART_TRIPLE_SUBJECT_IDX)))	/* If chances on good breakup are low but cost of wrong decision may be high... */
	    goto next_dep;	/* see below */
	  dep_bitmask_of_equations = sparp_bitmask_fields_with_equations (sparp, dep_triple);
	  if ((dep_bitmask_of_equations & (1 << SPART_TRIPLE_OBJECT_IDX)) && !(dep_bitmask_of_equations & (1 << SPART_TRIPLE_SUBJECT_IDX)))	/* If chances on good breakup are low but cost of wrong decision may be high... */
	    goto next_dep;	/* see below */
	  ...
	  for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
		...
		spar_internal_error (sparp,
		    "sparp_" "try_reuse_tabid_in_union(): different field types in triples with same src_serial");
	  ...
	  for (bt_ctr = base_triples_count; bt_ctr--; /* no step */ )
	    {
	      ...
	      if (!sparp_quad_maps_eq_for_breakup (sparp, base_triple->_.triple.tc_list[0]->tc_qm,
		      chk_triple->_.triple.tc_list[0]->tc_qm))
  ...
  for (dep_triple_idx = base_idx; dep_triple_idx < memb_count; dep_triple_idx++)
      ...
      for (dep_field_idx = 0; dep_field_idx < SPART_TRIPLE_FIELDS_COUNT; dep_field_idx++)
	{
	  ...
	  if (SPAR_QNAME == dep_field_type)
	    ...
	    {
	      if (((SPART_VARR_FIXED | SPART_VARR_IS_IRI) !=
		      ((SPART_VARR_FIXED | SPART_VARR_IS_IRI) & dep_field->_.var.rvr.rvrRestrictions))
		  ...
		  if (NULL == outer_eq)
		    {
		      ...
		      if (eq->e_rvr.rvrRestrictions & SPART_VARR_EXPORTED)
			spar_internal_error (sparp,
			    "sparp_" "gp_trav_flatten_and_reuse_tabids(): exported eq in base without eq in curr");
#endif
		      continue;
		    }
...
there was no receiver equiv outside and hence no propagation took place. So let's propagate now without running the whole optimization loop. */
		...
		{
		  sparp_equiv_t *outer_eq =
		      sparp_equiv_get (sparp, curr, (SPART *) (eq->e_varnames[0]),
		      SPARP_EQUIV_INS_CLASS | SPARP_EQUIV_GET_NAMESAKES);
...
/* After flattening, we check for possible reuse of tabids for self-joins. */
	...
	if ((0 != BOX_ELEMENTS_0 (curr->_.gp.filters)) ||
	    (1 != BOX_ELEMENTS_0 (curr->_.gp.members)) || (SPAR_TRIPLE != SPART_TYPE (curr->_.gp.members[0])))
	  break;
	if (NULL != curr->_.gp.options)
	  break;
	SPARP_FOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
...
static int
sparp_try_reduce_trivial_optional_via_eq (sparp_t * sparp, SPART * opt, SPART * key_field, qm_value_t * key_qmv,
    sparp_equiv_t * key_recv_eq, SPART * key_asc_or_self)
...
sparp_gp_trav_restore_filters_for_weird_subq (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  SPARP_FOREACH_GP_EQUIV (sparp, curr, eq_ctr, eq)
  {
	...
	curr->_.gp.subquery->_.req_top.pattern, (SPART *) (eq->e_varnames[0]), SPARP_EQUIV_GET_NAMESAKES);
    if (NULL == subq_eq)
      continue;
	...
	l = spartlist (sparp, 7 + (sizeof (rdf_val_range_t) / sizeof (caddr_t)),
	    ...
	    parent_gp->_.gp.selid, NULL, (ptrlong) (0), SPART_BAD_EQUIV_IDX, SPART_RVR_LIST_OF_NULLS, (ptrlong) (0x0));
	...
	if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (eq->e_rvr.rvrFixedValue))
	  ...
	  r = spartlist (sparp, 5, SPAR_LIT, eq->e_rvr.rvrFixedValue, eq->e_rvr.rvrDatatype, eq->e_rvr.rvrLanguage,
...
sparp_gp_trav_add_graph_perm_read_filters (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  for (membctr = 0; membctr < membcount; membctr++)
      ...
      else
	{
	  ...
	  DO_BOX_FAST (SPART *, src, ctr, sources)
	  {
	    if (src->_.graph.use_expn_in_gs_checks
		&& ((SPART_IS_DEFAULT_GRAPH_BLANK (g_norm_expn) ? SPART_GRAPH_FROM : SPART_GRAPH_NAMED) == src->_.graph.subtype))
  ...
  if ((OPTIONAL_L != curr->_.gp.subtype) &&
      ...
      (UNION_L != sts_this[0].sts_parent->_.gp.subtype) && (SPAR_UNION_WO_ALL != sts_this[0].sts_parent->_.gp.subtype))
    {
...
sparp_dig_and_glue_loj_filter_for_eq (sparp_t * sparp, sparp_equiv_t * eq)
{
  ...
  for (filter_ctr = BOX_ELEMENTS_0 (gp->_.gp.filters) - gp->_.gp.glued_filters_count; filter_ctr--; /* no step */ )
	...
	spar_error (sparp,
	    "Variable '%.100s' is used in OPTIONAL inside UNION but not assigned in OPTIONAL, please rephrase the query",
	    eq->e_varnames[0]);
      sparp_gp_detach_filter (sparp, gp, filter_ctr, NULL);
      sparp_gp_attach_filter (sparp, good_recv_gp, filt,
	  BOX_ELEMENTS_0 (good_recv_gp->_.gp.filters) - good_recv_gp->_.gp.glued_filters_count, NULL);
...
/* Building qm_list for every triple in the tree. */
  sparp_gp_trav_top_pattern (sparp, req_top, NULL, sparp_gp_trav_refresh_triple_cases, NULL, NULL, NULL, NULL, NULL);
  sparp_equiv_audit_all (sparp, 0);
  sparp_rewrite_basic (sparp, req_top);
}
...
#if 0
	  spar_internal_error (sparp,
	      "SPARQL optimizer performed too many rounds of query rewriting, this looks like endless loop. Please rephrase the query.");
#else
...
/* Converting to GP_UNION of every triple such that many quad maps contains triples that matches the mapping pattern */
  sparp_gp_trav_top_pattern (sparp, req_top, (void *) ((ptrlong) opt_ctr),
      sparp_gp_trav_refresh_triple_cases, sparp_gp_trav_multiqm_to_unions, NULL, NULL, sparp_gp_trav_rewrite_qm_optloop, NULL);
  sparp_trav_out_clauses (sparp, req_top, (void *) ((ptrlong) opt_ctr),
      NULL, NULL, NULL, NULL, sparp_gp_trav_rewrite_qm_optloop, NULL);
  sparp_equiv_audit_all (sparp, 0);
  sparp_rewrite_basic (sparp, req_top);
  sparp_equiv_audit_all (sparp, SPARP_EQUIV_AUDIT_NOBAD);
/* Converting join with a union into a union of joins with parts of union */
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_union_of_joins_in, sparp_gp_trav_union_of_joins_out, NULL, NULL, NULL, NULL);
  sparp_equiv_audit_all (sparp, 0);
  sparp_rewrite_basic (sparp, req_top);
/* Removal of gps that can not produce results */
  sparp_gp_trav_top_pattern (sparp, req_top, NULL, NULL, sparp_gp_trav_detach_conflicts_out, NULL, NULL, NULL, NULL);
  sparp_equiv_audit_all (sparp, 0);
  sparp_rewrite_basic (sparp, req_top);
  sparp_gp_trav_top_pattern (sparp, req_top, NULL, NULL, sparp_gp_trav_detach_conflicts_out, NULL, NULL, NULL, NULL);
  sparp_equiv_audit_all (sparp, 0);
  ...
  if (!(opt_ctr % 2))		/* Do this not in every loop because it's costly and it almost never give a result after first loop */
    {
      sparp_gp_trav_top_pattern (sparp, req_top, NULL, sparp_gp_trav_localize_filters, NULL, NULL, NULL, NULL, NULL);
      sparp_equiv_audit_all (sparp, 0);
      sparp_rewrite_basic (sparp, req_top);
      sparp_equiv_audit_all (sparp, 0);
    }
...
sparp_list_external_vars (sparp_t * sparp, SPART * tree, dk_set_t * set_ret)
{
  ...
  if (NULL != top)
	...
	NULL, NULL, sparp_gp_trav_list_external_vars_expn_in, NULL, sparp_gp_trav_list_external_vars_expn_subq, NULL);
  sparp_gp_trav (sparp, NULL /* unused */ , top_pattern, set_ret,
      sparp_gp_trav_list_external_vars_gp_in, NULL,
      sparp_gp_trav_list_external_vars_expn_in, NULL, sparp_gp_trav_list_external_vars_expn_subq, NULL);
}
  ...
  DO_SET (SPART *, gp, &(all_sinvs))
  {
      ...
      if ((NULL == param_xfer_equiv) || (0 == BOX_ELEMENTS_0 (param_xfer_equiv->e_receiver_idxs)))
	{
	  ...
	  if (0 > dk_set_position_of_string (used_globals, param_var_name))
		...
		spar_error (sparp,
		    "%.300s declares IN ?%.200s variable but an IN variable should be used both inside and outside the SERVICE clause",
    ...
    END_DO_SET ()if (NULL != new_used_globals_as_params)
      sinv->_.sinv.param_varnames =
	  t_list_concat ((caddr_t) (sinv->_.sinv.param_varnames), (caddr_t) t_revlist_to_array (new_used_globals_as_params));
...
sparp_cut_inner_limit_with_outer_limit (sparp_t * sparp, SPART * inner_limit, SPART * inner_offset, SPART * outer_limit)
{
  if (NULL == outer_limit)
    return inner_limit;
  if ((DV_LONG_INT == DV_TYPE_OF (outer_limit)) && (DV_LONG_INT == DV_TYPE_OF (inner_offset))
      && (DV_LONG_INT == DV_TYPE_OF (inner_limit)))
    {
...
spar_propagate_limit_as_option (sparp_t * sparp, SPART * tree, SPART * outer_limit)
{
  switch (SPART_TYPE (tree))
	...
	return;
      tree->_.req_top.limit =
	  sparp_cut_inner_limit_with_outer_limit (sparp, tree->_.req_top.limit, tree->_.req_top.offset, outer_limit);
      if ((NULL != tree->_.req_top.limit) && (DISTINCT_L != tree->_.req_top.subtype) && (NULL == tree->_.req_top.groupings)
	  && (NULL == tree->_.req_top.having))
	spar_propagate_limit_as_option (sparp, tree->_.req_top.pattern, sparp_limit_for_cutting_inner_limits (sparp,
		tree->_.req_top.limit, tree->_.req_top.offset));
      return;
    case SPAR_GP:
      {
	int eq_ctr;
...
retry_after_reducing_optionals:
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, &optionals_to_reduce,
      sparp_gp_trav_rewrite_qm_postopt, sparp_gp_trav_flatten_and_reuse_tabids, NULL, NULL, sparp_gp_trav_rewrite_qm_postopt, NULL);
  ...
  sparp_audit_mem (sparp);
  sparp_trav_out_clauses (sparp, req_top, NULL, NULL, NULL, NULL, NULL, sparp_gp_trav_rewrite_qm_postopt, NULL);
  ...
  sparp_gp_trav_top_pattern (sparp, req_top, NULL,
      sparp_gp_trav_restore_filters_for_weird_subq, security_cbk, NULL, NULL, NULL, NULL);
/* Final processing: */
  ...
  for (equiv_ctr = equiv_count; equiv_ctr--; /* no step */ )
	...
	continue;
      if (eq->e_deprecated)
	continue;
      if (!SPARP_EQ_IS_ASSIGNED_LOCALLY (eq) && ((0 != eq->e_const_reads) || (0 != eq->e_replaces_filter) ||	/* note: no check for (0 != eq->e_optional_reads) */
	      (0 != BOX_ELEMENTS_0 (eq->e_receiver_idxs))) &&
	  !(eq->e_rvr.rvrRestrictions & (SPART_VARR_FIXED | SPART_VARR_GLOBAL | SPART_VARR_EXTERNAL)))
	{
	  ...
	  if (!sparp->sparp_sg->sg_signal_void_variables)
	    ...
	    spar_error (sparp, "Variable '%.100s' can not be bound due to mutually exclusive restrictions on its value",
		eq->e_varnames[0]);
	  else
	    spar_error (sparp, "Variable '%.100s' is used in subexpressions of the query but can not be assigned",
		eq->e_varnames[0]);
	}
#ifdef DEBUG
...
sparp_rewrite_grab (sparp_t * sparp, SPART * req_top)
{
  ...
  END_DO_SET ()if (NULL != rgc->rgc_destination)
...
/* Making subqueries: iter */
  ...
  if (NULL != sparp_of_iter->sparp_entire_query->_.req_top.order)
    sparp_tweak_order_of_iter (sparp_of_iter, sparp_of_iter->sparp_entire_query,
	sparp_of_iter->sparp_entire_query->_.req_top.order);
...
#define PUSH_GRAB_PARAM(n,v) do { t_set_push (&grab_params, t_box_dv_short_string ((n))); t_set_push (&grab_params, (v)); } while (0)
  ...
  END_DO_SET ()ret_limit_expn = req_top->_.req_top.limit;
  ...
  rewritten_req_top = spartlist (sparp, 13, SPAR_CODEGEN,	/* #0 */
      ...
      ret_limit_expn,		/* #8 */
      ((NULL == rgc->rgc_consts) ? NULL : spar_make_vector_qm_sql (sparp, (SPART **) (t_revlist_to_array (rgc->rgc_consts)))),	/* #9 */
...
/* The order of declarations is important: side effect on init */
  ...
  if (use_plain_return)
  ...
  else
    {
      ssg_puts ("SELECT ");
      if (0 == varcount)
	ssg_puts ("TOP 1 1 as __ask_retval ");	/* IvAn/Bug12961/071109 Added special case for ASK */
      for (varctr = 0; varctr < varcount; varctr++)
	{
      ...
      ssg_prin_function_name (ssg, procedure_name);
      ssg_puts
	  (" (_grabber_app_params, _grabber_params, _grabber_seed, _grabber_iter, _grabber_final, _grabber_ret_limit, _grabber_consts, _grabber_depth, _plain_ret, _uid) (rset any) ");
      ssg_prin_id (ssg, call_alias);
      ssg_newline (0);
