...
qi_qn_stat_1 (query_instance_t * qi, query_t * qr)
{
  ...
  END_DO_SET ();
  DO_SET (query_t *, sq, &qr->qr_subq_queries) qi_qn_stat_1 (qi, sq);
  END_DO_SET ();
}
...
#if 0
  DO_SET (query_t *, sq, &qr->qr_subq_queries) sum += qr_qn_total (sq);
  END_DO_SET ();
#endif
  return sum;
}
...
ssl_print (state_slot_t * ssl)
{
  ...
  switch (ssl->ssl_type)
	...
	stmt_printf (("<r $%d %s via ", sslr->sslr_index, sslr->sslr_ssl
		&& sslr->sslr_ssl->ssl_name ? sslr->sslr_ssl->ssl_name : ""));
    ...
    default:
      stmt_printf (("<$%d \"%s\" spec %d>", ssl->ssl_index, ssl->ssl_name ? ssl->ssl_name : "-", ssl->ssl_type));
      break;
    }
}
...
code_vec_print (code_vec_t cv)
{
  ...
  DO_INSTR (in, 0, cv)
  {
      ...
      case IN_AGG:
	{
	  int op = in->_.agg.op;
	  char *name =
	      AMMSC_SUM == op ? "sum" : AMMSC_COUNTSUM == op ? "countsum" : AMMSC_COUNT == op ? "count" : AMMSC_MIN ==
	      op ? "min" : AMMSC_MAX == op ? "max" : AMMSC_ONE == op ? "subq_value" : "unknown ";
	  stmt_printf ((" %s ", name));
      ...
      case IN_PRED:
	{
	  ...
	  if (in->_.pred.end)
	    stmt_printf ((") then %d else %d unkn %d merge %d", in->_.pred.succ, in->_.pred.fail, in->_.pred.unkn, in->_.pred.end));
	  else
	    stmt_printf ((") then %d else %d unkn %d", in->_.pred.succ, in->_.pred.fail, in->_.pred.unkn));
	  break;
	}
      ...
      case IN_COMPARE:
	{
	  ...
	  if (in->_.cmp.end)
	    stmt_printf ((") then %d else %d unkn %d merge %d", in->_.cmp.succ, in->_.cmp.fail, in->_.cmp.unkn, in->_.cmp.end));
	  else
	    stmt_printf ((") then %d else %d unkn %d", in->_.cmp.succ, in->_.cmp.fail, in->_.cmp.unkn));
	  break;
	}
      case INS_SUBQ:
	{
	...
	stmt_printf (("end_handler code Type=%s", in->_.handler_end.type == HANDT_EXIT ? "EXIT" : "CONTINUE"));
	break;
	...
	stmt_printf (("Comp start (level=%d, line=%d, src=%s:%d)",
		...
		in->_.compound_start.file_name ? in->_.compound_start.file_name : "", in->_.compound_start.l_line_no));
	break;
...
ks_is_column_derived (key_source_t * ks)
{
  ...
  DO_BOX (state_slot_t *, ssl, inx, ks->ks_vec_source) if (ssl_is_column_derived (ssl))
    return 1;
  END_DO_BOX;

  DO_BOX (state_slot_t *, ssl, inx, ks->ks_vec_cast) if (ssl_is_column_derived (ssl))
    return 1;
  END_DO_BOX;
  return 0;
}
...
ks_print (key_source_t * ks)
{
  ...
  if (ks->ks_from_temp_tree || !ks->ks_key)
    ...
    stmt_printf (("Key %s  %s %s %s", ks->ks_key->key_name,
	    ks->ks_descending ? "DESC" : "ASC", ks->ks_is_deleting ? "deleting" : "", ks->ks_oby_order
	    && !ks->ks_vec_asc_eq ? "no vec sort" : ""));
  ssl_list_print (ks->ks_out_slots);
  if (ks->ks_v_out_map)
    {
      int inx;
  ...
  if (ks->ks_hash_spec)
    {
      ...
      DO_SET (search_spec_t *, sp, &ks->ks_hash_spec) hrng_print ((hash_range_spec_t *) sp->sp_min_ssl, sp);
      END_DO_SET ();
      stmt_printf (("\n"));
    }
  if (ks->ks_vec_cast)
  ...
  if (IS_QN (qn, query_frag_input) && ctx_inst)
    {
      ...
      stmt_printf (("wait time %9.2g%% of exec real time, fanout %9.6g\n", (float) QST_INT (ctx_inst,
		  qf->qf_wait_clocks) * 100 / (float) rt, srs->srs_n_in ? srs->srs_n_out / (float) srs->srs_n_in : 0.0));
    }
  else
    stmt_printf (("time %9.2g%% fanout %9.6g input %9.6g rows\n", (float) srs->srs_cum_time * 100 / (float) total,
	    srs->srs_n_in ? srs->srs_n_out / (float) srs->srs_n_in : 0.0, (float) srs->srs_n_in));
...
ts_print_0 (table_source_t * ts)
{
  ...
  if (!ts->ts_order_ks->ks_from_temp_tree && ts->ts_order_ks->ks_key)
	...
	snprintf (card, sizeof (card), "%9.2g rows", ts->ts_cardinality);
      stmt_printf (("%s %s", ts->ts_order_ks->ks_key->key_name, card));
    }
  ...
  if (ts->ts_inx_op)
    {
      ...
      {
	stmt_printf (("from %s by %s\n", term->iop_ks->ks_key->key_table->tb_name, term->iop_ks->ks_key->key_name));
      ...
      if (ts->ts_branch_ssls && dbf_explain_level > 2)
	{
	  ...
	  DO_BOX (state_slot_t *, ssl, inx, ts->ts_branch_ssls) stmt_printf ((" $%d", ssl ? ssl->ssl_index : 0));
	  END_DO_BOX;
	  stmt_printf ((" sets: "));
...
node_print_0 (data_source_t * node)
{
  ...
  if (IS_TS (node))
      ...
      if (del->del_keys)
	{
	  int inx;
	  stmt_printf (("keys: "));
	  DO_BOX (ins_key_t *, ik, inx, del->del_keys) if (ik)
	    ik_print (ik);
	  END_DO_BOX;
	}
      stmt_printf (("\n"));
  ...
  else if (in == (qn_input_fn) hash_source_input)
    {
      ...
      stmt_printf (("Hash source %d %s %s %9.2g rows", hs->hs_ha->ha_tree->ssl_index,
	      hs->hs_merged_into_ts ? "merged into ts" : hs->hs_is_outer ? "outer" : "",
...
node_print (data_source_t * node)
{
  ...
  if (IS_TS (node))
	...
	max_rows[0] = 0;
      stmt_printf (("  { Cluster location fragment %d %d %s %s\n   Params: ", node->src_in_state, (int) qf->qf_nth,
	      qf->qf_order ? "" : "unordered", max_rows));
    ...
    {
      fun_ref_node_t *fref = (fun_ref_node_t *) node;
      stmt_printf (("Fork %d \n{  %s\n", node->src_in_state, fref->fnr_hi_signature ? " shareable hash fill " : ""));
      ...
      if (sctr->sctr_hash_spec)
	{
	  ...
	  DO_SET (search_spec_t *, sp, &sctr->sctr_hash_spec) hrng_print ((hash_range_spec_t *) sp->sp_min_ssl, sp);
	  END_DO_SET ();
	  stmt_printf (("\n"));
	}
      stmt_printf (("\n"));
...
code_vec_print_xml (QI * qi, code_vec_t cv, dk_session_t * s)
{
  ...
  DO_INSTR (in, 0, cv)
  {
	...
	ses_sprintf (s, "<comp level='%d' line='%d' src='%s'>", compound_level,
	    in->_.compound_start.line_no, in->_.compound_start.file_name ? in->_.compound_start.file_name : "");
	break;
      case INS_COMPOUND_END:
	SES_PRINT (s, "</comp>");
	break;
  ...
  if (IS_QN (qn, table_source_input) || IS_QN (qn, table_source_input_unique))
      ...
      if (ks->ks_hash_spec)
	{
	  ...
	  DO_SET (search_spec_t *, sp, &ks->ks_hash_spec)
	  {
	    ...
	    if (hrng->hrng_hs || hrng->hrng_ht_id || hrng->hrng_ht)
	      {
		ses_sprintf (s, "<hrng hs='%d' flags='%d' />", hrng->hrng_hs ? hrng->hrng_hs->src_gen.src_sets : 0,
		    hrng->hrng_flags);
	      }
	  }
	  END_DO_SET ();
  ...
  else if (IS_QN (qn, setp_node_input))
    {
	  ...
	  setp->setp_ha && setp->setp_ha->ha_op == HA_FILL ? setp->setp_ha->ha_tree->ssl_index : 0, qn->src_sets);
      if (setp->setp_ha)
	{
	  int inx;
	  SES_PRINT (s, "<key>");
  ...
  else if (IS_QN (qn, delete_node_input))
    {
      ...
      ses_sprintf (s, "<del tb='%s' key='%s'>", del->del_table->tb_name, del->del_key_only ? del->del_key_only->key_name : "");
      node_print_code_xml (qi, s, qn);
      SES_PRINT (s, "<place>");
  ...
  else if (IS_QN (qn, trans_node_input))
    {
	  ...
	  tn->tn_prepared_step && tn->tn_ifp_ctx_name ? tn->tn_ifp_ctx_name : "", tn->tn_direction);
      node_print_code_xml (qi, s, qn);
      SES_PRINT (s, "<in>");
...
bif_procedure_cols (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!proc_name)
    return NULL;
  full_name = sch_full_proc_name (isp_schema (qi->qi_space), proc_name, qi->qi_client->cli_qualifier, CLI_OWNER (qi->qi_client));
  if (!full_name)
    return NULL;
  ...
  DO_SET (state_slot_t *, ssl, &qr->qr_parms)
  {
    caddr_t *col = proc_result_col_from_ssl (inx++, ssl,
...
xsql_print_stmt (sql_comp_t * sc, comp_table_t * ct, ST * tree, char *text, size_t tlen, int *fill)
{
  ...
  if (ST_COLUMN (tree, COL_DOTTED))
      ...
      if (ST_P (tree->_.insert.vals, INSERT_VALUES))
	{
	  ...
	  sqlc_insert_commalist (sc, ct, tree, NULL, text, tlen, fill, 0);
	  sprintf_more (text, tlen, fill, ")");
	}
      else
	{
...
qnw_hash (query_instance_t * qi, data_source_t * qn, qnw_cd_t * qnw)
{
  ...
  if (IS_TS (((table_source_t *) qn)))
  ...
  else if ((qn_input_fn) setp_node_input == qn->src_input)
    {
      ...
      if (setp->setp_ha)
	{
	  DO_BOX (state_slot_t *, ssl, inx, setp->setp_keys_box) MHASH_STEP (qnw->qnw_hash, ssl->ssl_index);
	  END_DO_BOX;
	  DO_BOX (state_slot_t *, ssl, inx, setp->setp_dependent_box) MHASH_STEP (qnw->qnw_hash, ssl->ssl_index);
	  END_DO_BOX;
	}
    }
  else if ((qn_input_fn) select_node_input_subq == qn->src_input)
    {
      ...
      DO_BOX (state_slot_t *, ssl, inx, sel->sel_out_slots) if (ssl)
  ...
  if (IS_CL_TXS (qn))
    ...
    {
      QNCAST (dpipe_node_t, dp, qn);
      DO_BOX_0 (state_slot_t *, ssl, inx, dp->dp_inputs) MHASH_STEP (qnw->qnw_hash, dp->dp_inputs[inx]->ssl_index);
      END_DO_BOX;
    }
}
...
qn_walk (query_instance_t * qi, query_t * qr, qnw_cb_t cb, qnw_cd_t * cd)
{
  ...
  END_DO_SET ();
  DO_SET (query_t *, sq, &qr->qr_subq_queries) qn_walk (qi, sq, cb, cd);
  END_DO_SET ();
}
...
qi_array_mem (QI ** qis)
{
  uint64 mem = 0;
  int inx;
  DO_BOX (QI *, slice_qi, inx, qis) if (slice_qi)
...
{
  return IS_QN (qn, query_frag_input) || IS_QN (qn, stage_node_input) || IS_QN (qn, trans_node_input)
...
qi_log_stats_1 (query_instance_t * qi, caddr_t err, caddr_t ext_text)
{
  ...
  if (!ext_text)
      ...
      if (comm)
	{
      ...
      DO_SET (caddr_t *, r, &res) r_len += box_length (r[0]);
      END_DO_SET ();
