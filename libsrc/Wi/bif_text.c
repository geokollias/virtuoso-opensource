...
vt_string_words (char *string, char *extra)
{
  ...
  for (;;)
	...
	{
	  str_asciiupc_copy (temp, w_start, MIN (sizeof (temp) - 2, ((long) w_end - (long) w_start) + 1));
...
vt_noise_word_init (char *file, id_hash_t ** noise_ht)
{
  ...
  *noise_ht = id_hash_allocate (7, sizeof (lenmem_t), sizeof (caddr_t), lenmemhash, lenmemhashcmp);
  ...
  while (fgets (nw, sizeof (nw), noise))
	...
	continue;
      res =
	  lh_iterate_patched_words (eh, lh, nw, tail - nw, lh->lh_is_vtb_word, NULL, noise_word_init_callback,
	  (void *) (future_noise_words));
      res |=
	  lh_iterate_patched_words (eh, lh, nw, tail - nw, lh->lh_is_vtb_word, lh->lh_tocapital_word, noise_word_init_callback,
	  (void *) (future_noise_words));
      res |=
	  lh_iterate_patched_words (eh, lh, nw, tail - nw, lh->lh_is_vtb_word, lh->lh_toupper_word, noise_word_init_callback,
	  (void *) (future_noise_words));
      res |=
	  lh_iterate_patched_words (eh, lh, nw, tail - nw, lh->lh_is_vtb_word, lh->lh_tolower_word, noise_word_init_callback,
	  (void *) (future_noise_words));
      res |=
	  lh_iterate_patched_words (eh, lh, nw, tail - nw, lh->lh_is_vtb_word, lh->lh_normalize_word, noise_word_init_callback,
	  (void *) (future_noise_words));
      if (res)
  ...
  switch (BOX_ELEMENTS (args))
      ...
      if ((sz < VT_BATCH_SIZE_MIN) || (VT_BATCH_SIZE_MAX < sz))
	{
	  if (0 == sz)
	    ...
	    sqlr_new_error ("42000", "FT038", "Invalid batch size argument '%ld' is specified by an argument of vt_batch()",
		(long) sz);
	}
      /* no break */
    case 0:
...
bif_vt_batch_d_id (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!D_AT_END (&vtb->vtb_d_id) && IS_LT (d_id_cmp (&nid, &vtb->vtb_d_id)))
    sqlr_new_error ("22023", "FT009", "vt_batch_d_id id's not in ascending order");

  if (D_AT_END (&nid) || D_INITIAL (&nid) || D_PRESET (&nid) || (DV_LONG_INT == DV_TYPE_OF (new_d_id) && unbox (new_d_id) < 0))
...
void
vtb_qsort (caddr_t * in, caddr_t * left, int n_in, int depth)
{
  if (n_in < 3)
    return;
  if (n_in < 5)
      ...
      for (inx = 0; inx < n_in; inx += 2)
	{
      ...
      vtb_qsort (left + n_right + 1, in + n_right + 1, (n_in - n_right) - 1, depth + 1);
      memcpy (in, left, n_left * sizeof (caddr_t));
      in[n_left] = split;
      in[n_left + 1] = mid_buf;
      memcpy (in + n_left + 2, left + n_left + 2, ((n_in - n_right) - 1) * sizeof (caddr_t));
...
vtb_hash_string_ins_callback (const utf8char * buf, size_t bufsize, void *userdata)
{
  ...
  if (NULL != place)
      ...
      if (DVC_MATCH != d_id_cmp (&vtb->vtb_d_id, &place->wb_d_id))
	{
      ...
      wbp_add_pos (
	  (IS_ATTR_WORD_POS (vtb->vtb_word_pos) ? &(place->wb_attr_positions) : &(place->wb_main_positions)), vtb->vtb_word_pos);
    }
  else
    {
      lenmem_t lm;
      ...
      wbp_add_pos ((IS_ATTR_WORD_POS (vtb->vtb_word_pos) ? &(wb.wb_attr_positions) : &(wb.wb_main_positions)), vtb->vtb_word_pos);
  ...
  if (is_string_type (dtp))
    {
      ...
      lh_iterate_patched_words (&eh__UTF8, lh,
	  (caddr_t) xte, box_length ((caddr_t) xte), lh->lh_is_vtb_word, lh->lh_normalize_word, cbk, vtb);
      ...
      if (0 != attr_idx_max)
	{
	  ...
	  for (attr_idx = 1; attr_idx < attr_idx_max; attr_idx += 2)
		...
		continue;
	      ...
	      lh_iterate_patched_words (&eh__UTF8, lh,
		  attr_value, box_length (attr_value), lh->lh_is_vtb_word, lh->lh_normalize_word, cbk, vtb);
	      snprintf (textbufptr[0], box_length (textbufptr[0]), "}%s", attr_name);
	      cbk ((const utf8char *) textbufptr[0], 1 + namelen, vtb);
	    }
  ...
  if (place)
    {
      if (DVC_MATCH != d_id_cmp (&vtb->vtb_d_id, &place->wb_d_id))
	{
	  dk_set_push (&place->wb_word_recs, (void *) wb_offband_string (&place->wb_d_id, place->wb_main_positions.wbp_buf));
	  place->wb_d_id = vtb->vtb_d_id;
...
bif_vt_batch_feed (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  lang_handler_t *lh = ((argc > 4) ? lh_get_handler (bif_string_arg (qst, args, 4, "vt_batch_feed")) : vtb->vtb_default_lh);
  ...
  if (vtb->vtb_strings_taken)
    sqlr_new_error ("42000", "FT014",
	"The vt_batch object can't be used in vt_batch_feed() if vt_batch_strings() or vt_batch_strings_array() has been called.");
...
process_string:
  ...
  if (is_xml)
	...
	temp_tree =
	    xml_make_mod_tree (qi, str, err_ret, GE_XML, NULL, NULL /*(is_wide ? NULL : vtb->vtb_default_eh->eh_names[0]) */ , lh,
	    dtd_config, NULL /* do not save DTD */ , NULL /* do not cache IDs */ , NULL /* no namespace 2way dict */ );
      }
...
process_as_text:
  if (store_result)
      ...
      else
	{
      ...
      lh_iterate_patched_words (eh, lh, str, box_length (str), lh->lh_is_vtb_word, lh->lh_normalize_word, cbk, vtb);
    }
  goto done;
...
bif_vt_batch_feed_wordump (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (vtb->vtb_strings_taken)
    sqlr_new_error ("42000", "FT034",
	"The vt_batch object can't be used in vt_batch_feed_wordump() after vt_batch_strings_array() has been called.");
  if (dtp == DV_DB_NULL)
    return NULL;
  if (dtp == DV_STRING_SESSION)
    {
...
bif_wb_apply (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (;;)
      ...
      if (mid < org_len)
	{
      ...
      if (mid >= org_len && !ins)
	break;
      if (ins && (mid >= org_len || IS_LTE (d_id_cmp (&ins_id, &d_id))))
	{
...
static char *exestr_text = "create procedure execstr (in str varchar)\n" "{\n" "  declare st, msg varchar;\n" "  st := \'00000\';\n"
/*"  dbg_obj_print (str); \n"*/
...
static char *vt_create_update_log_text =
    ...
    "  declare tb_suff, vtlog_suff, id_col_type, key_name, cl_part, cl_opts varchar;" "  declare _lang_enc_args varchar;"
/*"  tablename := complete_table_name (fix_identifier_case (tablename), 1); "*/
    ...
    "    of_cols_d := concat (of_cols_d, \'\"\', ofcol, \'\",\');" "    of_cols_o := concat (of_cols_o, \'O.\"\', ofcol, \'\",\');"
...
ddl_text_index_upgrade (void)
{
  query_t *proc = NULL;
  char *full_name =
      sch_full_proc_name ( /*isp_schema (db_main_tree->it_commit_space) */ wi_inst.wi_schema, "DB.DBA.vt_create_text_index",
  ...
  if (proc != NULL)
    return;
  ddl_sel_for_effect
      ("select count (*)  from SYS_VT_INDEX where 0 = __vt_index (VI_TABLE, VI_INDEX, VI_COL, VI_ID_COL, VI_INDEX_TABLE, deserialize (VI_OFFBAND_COLS), VI_LANGUAGE, VI_ENCODING, deserialize (VI_ID_CONSTR))");
