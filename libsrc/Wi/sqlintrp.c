    ...
    int *any_out, code_vec_t code_vec, state_slot_t ** vec_defaults)
{
  ...
  for (inx = n_ret_param; inx < (ins->_.call.params ? BOX_ELEMENTS_INT (ins->_.call.params) : 0); inx++)
    {
      ...
      if (param_ssl && IS_SSL_REF_PARAMETER (param_ssl->ssl_type))
	{
	  ...
	  if (actual_ssl->ssl_type == SSL_CONSTANT)
	    {
	      err =
		  srv_make_new_error ("HY105", "SR181", "Cannot pass literal as reference parameter \"%.100s\"",
		  param_ssl->ssl_name);
	      goto err_end;
	    }
	  if (SSL_VEC == actual_ssl->ssl_type || SSL_REF == actual_ssl->ssl_type)
		...
		{
		  err =
		      srv_make_new_error ("42000", "VEC..",
		      "In vectored code calling non-vectored inout parameter mode is supported only if caller variable is a boxed vector, e.g. type any array (caller variable \"%.100s\", calling parameter \"%.100s\")",
  ...
  DO_SET (state_slot_t *, formal, &proc->qr_parms)
  {
    if (0x7fffffff == (ptrlong) params[inx])
      {
	if (IS_SSL_REF_PARAMETER (formal->ssl_type) || formal->ssl_vec_param >= SSL_VP_OUT)
	  {
	...
	if (proc->qr_parm_default && BOX_ELEMENTS (proc->qr_parm_default) > inx && proc->qr_parm_default[inx])
	  params[inx] =
	      is_vec_call ? ins_call_vec_deflt (vec_defaults, (caddr_t) proc->qr_parm_default[inx], n_param_box,
	      &deflt_ctr) : box_copy_tree (proc->qr_parm_default[inx]);
	else
	  {
...
sqlr_run_bif_in_sandbox (bif_metadata_t * bmd, caddr_t * args, caddr_t * err_ret)
{
  ...
  END_QR_RESET dk_free_box ((caddr_t) qi_stub);
  ...
  if (ins->_.bif.ret == CV_CALL_PROC_TABLE)
    {
	  ...
	  "function in a procedure view.  " "Define an intermediate PL function to call the bif.");
    }
...
ins_call (instruction_t * ins, caddr_t * qst, code_vec_t code_vec)
{
  ...
  if (ins->_.call.pn && (proc = ins->_.call.pn->pn_query))
  ...
  else if (DV_TYPE_OF (proc_name) == DV_ARRAY_OF_POINTER)
    {
      ...
      ptrlong mtd_inx = -1;
      if (BOX_ELEMENTS (proc_name) != 2 || !DV_STRINGP (proc_mtd_call[0]) || !DV_LONG_INT == DV_TYPE_OF (proc_mtd_call[1]))
	{
      ...
      if (!udt)
	{
	  err = srv_make_new_error ("22023", "UD005", "Non-existent user defined type %.200s", proc_mtd_call[0]);
	  goto report_error;
	}
      if (!udt->scl_method_map || mtd_inx < 0 || mtd_inx >= BOX_ELEMENTS_INT (udt->scl_method_map))
	{
	  err = srv_make_new_error ("22023", "UD006", "No such method in user defined type %.200s", proc_mtd_call[0]);
	  goto report_error;
	}
	...
	{			/* should call the bif - no kwd_params */
	  ...
	  ret = udt_method_call (qst, udt, udi, mtd, ins->_.call.params, BOX_ELEMENTS (ins->_.call.params));
	...
	{
	  proc = sch_partial_proc_def (isp_schema (qi->qi_space), proc_name, qi->qi_query->qr_qualifier, CLI_OWNER (qi->qi_client));
  ...
  if (!proc || IS_REMOTE_ROUTINE_QR (proc))
      ...
      if (QR_IS_MODULE_PROC (qi->qi_query))
	{
	  ...
	  sch_split_name (qi->qi_query->qr_qualifier, qi->qi_query->qr_module->qr_proc_name, rq, ro, rn);
	  proc_name_2 = sch_full_proc_name_1 (isp_schema (qi->qi_space), proc_name, rq, CLI_OWNER (qi->qi_client), rn);
	}
      if (!proc_name_2)
  ...
  if (!ret && (qi->qi_caller == CALLER_CLIENT || qi->qi_lc))
    {
  ...
  if (!proc->qr_proc_vectored)
	...
	dk_free_tree (value);
      ...
      else if (qi->qi_caller != CALLER_CLIENT && !qi->qi_lc)
	dk_free_tree (value);
#endif
    }
}
...
qn_is_flushable (data_source_t * qn)
{
  if (IS_RTS (((remote_table_source_t *) qn))
      || (qn_input_fn) insert_node_input == qn->src_input || (qn_input_fn) query_frag_input == qn->src_input)
    return 1;
  return 0;
}
...
qn_init (table_source_t * ts, caddr_t * inst)
{
  ...
  if (IS_TS (ts))
	...
	QST_INT (inst, ts->ts_in_sdfg) = 0;
      if (ts->ts_csq && ts->ts_csq->csq_nth_step)
	QST_INT (inst, ts->ts_csq->csq_nth_step) = 0;
      if (ts->ts_csq && ts->ts_csq->csq_at_end)
	QST_INT (inst, ts->ts_csq->csq_at_end) = 0;
...
subq_init (query_t * subq, caddr_t * inst)
{
  ...
  DO_SET (table_source_t *, ts, &nodes)
  {
    ...
    else if ((ts->src_gen.src_input == (qn_input_fn) table_source_input || ts->src_gen.src_input == (qn_input_fn) table_source_input_unique) && ts->ts_order_ks	/* not set if inx op */
	&& ts->ts_order_ks->ks_key->key_id == KI_TEMP && ts->ts_order_cursor)
      {
	/* if there is a read from sort temp and there is a cursor for the space, delete it here, so as not to reuse the cursor on a different temp space later */
...
ins_subq (instruction_t * ins, caddr_t * qst)
{
  ...
  if (err == (caddr_t) SQL_NO_DATA_FOUND
      ...
      && BOX_ELEMENTS (ins->_.subq.query->qr_select_node->sel_out_slots) > 0 && ins->_.subq.query->qr_select_node->sel_out_slots[0])
...
ins_break (instruction_t * ins, caddr_t * qst)
{
  ...
  if ((ins->_.breakpoint.brk_set & 2) ||
      ...
      (qi->qi_step > 0 && cli->cli_pldbg->pd_session && cli->cli_pldbg->pd_step))

    {
      cli->cli_pldbg->pd_is_step = 1;

      if (cli->cli_pldbg->pd_session && (cli->cli_pldbg->pd_step & (PLDS_STEP | PLDS_NEXT)) && cli->cli_pldbg->pd_send)
...
handler_has_state (instruction_t * handler, caddr_t err_state, long *star_pos)
{
  if (IS_BOX_POINTER (handler->_.handler.states))
	...
	if (err_state == (caddr_t) state ||
		...
		unbox (state[1]) >= *star_pos && !strncmp (state[0], err_state, unbox (state[1]))))
	  {
...
void
err_append_callstack_procname (caddr_t err, caddr_t proc_name, caddr_t file_name, int line_no)
{
  ...
  if (DV_TYPE_OF (err) == DV_ARRAY_OF_POINTER)
	...
	line[0] = 0;
      ...
      if (file_name)
	new_msg = box_sprintf (box_length (err_msg) +
	    strlen (in_msg) +
	    strlen (msg_proc_name) +
	    strlen (file_name) + strlen (line) + 2, "%s%s%s(%s%s)", err_msg, in_msg, msg_proc_name, file_name, line);
      else
	new_msg = box_sprintf (box_length (err_msg) +
	    strlen (in_msg) + strlen (msg_proc_name) + strlen (line), "%s%s%s%s", err_msg, in_msg, msg_proc_name, line);
...
box_err_print_box (caddr_t param_value, int call_depth)
{
  ...
  switch (DV_TYPE_OF (param_value))
    ...
    case DV_STRING:
	  ...
	  (((box_length (param_value) > 100) || (box_length (param_value) != (strlen (param_value) + 1))) ? " (truncated)" : ""));
    case DV_UNAME:
	...
	if (rbb->rbb_base.rb_chksum_tail)
	  {
		...
		(int) (rbb->rbb_base.rb_ro_id), (int) (rbb->rbb_base.rb_is_complete), printed_rbb_chksum, (int) (rbb->rbb_box_dtp));
...
void
err_append_callstack_param (caddr_t err, caddr_t param_name, caddr_t param_value)
{
...
ins_agg (instruction_t * ins, caddr_t * inst)
{
  ...
  switch (ins->_.agg.op)
    ...
    case AMMSC_MAX:
      ...
      if (DVC_UNKNOWN == rc || (rc == DVC_LESS && AMMSC_MIN == ins->_.agg.op) || (rc == DVC_GREATER && AMMSC_MAX == ins->_.agg.op))
	{
	  qst_set (inst, ins->_.agg.result, box_copy_tree (arg));
...
qi_check_trx_error (query_instance_t * qi, int flags)
{
  ...
  if (cli->cli_start_time &&
      time_now_msec - cli->cli_start_time > BURST_STOP_TIMEOUT && cli->cli_session && cli_is_interactive (cli) && !cli->cli_ws)
    {
...
instruction_t *
code_vec_handle_error (code_vec_t code_vec, query_instance_t * qi, instruction_t * iins, int *pbreak_nesting_level, caddr_t err)
{
  ...
  DO_INSTR (ins, 0, code_vec)
  {
	...
	(nesting_level < break_nesting_level || (nesting_level == break_nesting_level && ins > start_ins)))
      {
  ...
  if (curr)
	...
	qst_set ((caddr_t *) qi, curr->_.handler.message,
	    DV_TYPE_OF (err) == DV_ARRAY_OF_POINTER ? box_copy (ERR_MESSAGE (err)) : dk_alloc_box (0, DV_DB_NULL));
#ifndef ROLLBACK_XQ
...
qi_check_buf_writers (void)
{
  ...
  DO_BOX (buffer_pool_t *, bp, inx, wi_inst.wi_bps)
  {
	...
	if (self == buf->bd_writer && buf->bd_space &&
	    buf->bd_space->isp_tree && buf->bd_space->isp_tree->it_key && buf->bd_space->isp_tree->it_key->key_id != KI_TEMP)
	  {
  ...
  QR_RESET_CTX_T (qi->qi_thread)
  {
	...
	switch (ins->ins_type)
	  ...
	  case IN_ARTM_IDENTITY:
	    HANDLE_ARTM (box_identity);
	  case IN_ARTM_FPTR:
	    HANDLE_ARTM_FPTR (ins->_.artm_fptr.func);
	  case IN_AGG:
	    ins_agg (ins, qst);
		  ...
		  if (ins != unsafe_ins)
		    {
			  ...
			  __LINE__, unsafe_flag, ((unsafe_ins == INSTR_ADD_OFS (code_vec,
				      prev_ins->_.cmp.succ)) ? "succ" : "not succ"), flag, ((ins == INSTR_ADD_OFS (code_vec,
				      prev_ins->_.cmp.succ)) ? "succ" : "not succ"), (int) prev_ins->_.cmp.op);
	...
	if (ins->ins_type == INS_CALL_BIF)
	  {
		...
		{
		  char name[20];
		  snprintf (name, sizeof (name), "__%02d", argidx + 1);
...
code_vec_run_no_catch (code_vec_t code_vec, it_cursor_t * itc)
{
  ...
  for (;;)
	...
	case IN_VRET:
	  {
	    caddr_t value = ins->_.vret.value ? box_copy_tree (qst_get (qst, ins->_.vret.value)) : NULL;
	    qi->qi_thread->thr_func_value = value;
	    return value;
	  }
	case IN_BRET:
...
bop_comp_func (caddr_t * qst, void *_bop)
{
  ...
  switch (op)
    ...
    default:
	  ...
	  QST_GET (qst, bop->cmp_right), bop->cmp_left->ssl_sqt.sqt_collation, bop->cmp_right->ssl_sqt.sqt_collation);
	...
	for (;;)
	  {
		...
		subp->subp_left->ssl_sqt.sqt_collation, subp->subp_query->qr_select_node->sel_out_slots[0]->ssl_sqt.sqt_collation);
	    successp = 0;
	    ...
	    if ((successp && subp->subp_type == SOME_PRED) || (successp && subp->subp_type == ONE_PRED))
	      {
		pred_succ = 1;	/*success */
		break;
	      }
	    if ((!successp && subp->subp_type == ALL_PRED) || (!successp && subp->subp_type == ONE_PRED))
	      {
		pred_succ = -1;	/* fail */
		break;
	      }
...
ins_vec_agg (instruction_t * ins, caddr_t * inst)
{
  ...
  if (!set_mask)
    {
	  ...
	  && ((SSL_IS_VEC_OR_REF (ins->_.agg.arg)
		  && DV_LONG_INT == QST_BOX (data_col_t *, inst, ins->_.agg.arg->ssl_index)->dc_sqt.sqt_dtp
		  && !QST_BOX (data_col_t *, inst, ins->_.agg.arg->ssl_index)->dc_any_null)
  ...
  SET_LOOP
  {
	...
	if (DVC_UNKNOWN == rc
	    || (rc == DVC_LESS && AMMSC_MIN == ins->_.agg.op) || (rc == DVC_GREATER && AMMSC_MAX == ins->_.agg.op))
	  {
	    qst_set (inst, ins->_.agg.result, box_copy_tree (arg));
	    if (DV_DB_NULL != DV_TYPE_OF (arg))
	      {
    ...
    ssl_index_t bool_ret_fill)
{
  ...
  for (;;)
	...
	{
	case IN_ARTM_PLUS:
	  V_HANDLE_ARTM (box_add, vec_adds);
	case IN_ARTM_MINUS:
	  V_HANDLE_ARTM (box_sub, vec_subs);
	case IN_ARTM_TIMES:
	  V_HANDLE_ARTM (box_mpy, vec_mpys);
	case IN_ARTM_DIV:
	  V_HANDLE_ARTM (box_div, vec_divs);
	case IN_ARTM_IDENTITY:
	...
	case IN_ARTM_FPTR:
	  V_HANDLE_ARTM_FPTR (ins->_.artm_fptr.func);
	...
	case IN_PRED:
	  {
		...
		SET_MASK_NEG (next_mask, save_mask, n_sets) qi->qi_set_mask = next_mask;
	...
	case IN_VRET:
	  {
		...
		if (SSL_VEC != ret_ssl->ssl_type)
		  {
		    qi->qi_set = 0;
		    qst_set ((caddr_t *) qi->qi_caller, (state_slot_t *) qi->qi_proc_ret, box_copy_tree (qst_get (qst,
				ins->_.vret.value)));
		    return;
		  }
