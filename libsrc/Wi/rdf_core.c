...
tf_free (triple_feed_t * tf)
{
  ...
  for (id_hash_iterator (&dict_hit, dict); hit_next (&dict_hit, (char **) (&dict_key), (char **) (&dict_val));
      /*no step */ )
    {
    ...
    char **full_name_ret, query_t ** proc_ret, caddr_t * err_ret)
{
  ...
  if (NULL != cli->cli_user && !sec_proc_check (proc_ret[0], cli->cli_user->usr_id, cli->cli_user->usr_g_id))
    {
      err_ret[0] =
	  srv_make_new_error ("42000", "SR575:SECURITY",
	  "No permission to execute %.300s as callback of %.100s() with user ID %d, group ID %d", full_name, funname,
	  (int) (cli->cli_user->usr_id), (int) (cli->cli_user->usr_g_id));
      return;
    }
  ...
  DO_SET (state_slot_t *, ssl, &(proc_ret[0]->qr_parms))
  {
	...
	err_ret[0] = srv_make_new_error ("42000", "SR577",
	    ...
	    (long) (param_type_iter - cbk_param_types) + 1, ssl->ssl_name, full_name, funname, problem);
	return;
      }
    param_type_iter++;
  }
...
tf_new_base (triple_feed_t * tf, caddr_t new_base)
{
  ...
  dk_free_box (tf->tf_base_uri);
  tf->tf_base_uri = box_dv_short_string (new_base);
  do
    {
  ...
  do
      ...
      if (dtp_of_text_arg == DV_STRING_SESSION)
	{
	  ...
	  if (XE_XPACK_SERIALIZATION == ses_sort)
	    {
#if 1
	      sqlr_error ("42000", "Function %.200s() does not support loading from string session with packed XML data", bifname);
#else
		...
		sqlr_error ("42000",
		    "The BLOB passed to a function rdf_load_rdfxml() contains corrupted packed XML serialization data");
	...
	sqlr_error ("42000", "Function %.200s() does not support loading from persistent XML objects", bifname);
      if ((DV_BLOB_HANDLE == dtp_of_text_arg) || (DV_BLOB_WIDE_HANDLE == dtp_of_text_arg))
	{
	  ...
	  if (XE_XPER_SERIALIZATION == blob_sort)
	    sqlr_error ("42000", "Function %.200s() does not support loading from BLOBs with persistent XML data", bifname);
	  if (XE_XPACK_SERIALIZATION == blob_sort)
	    {
#if 1
	      sqlr_error ("42000", "Function %.200s() does not support loading from BLOBs with packed XML data", bifname);
#else
	      caddr_t *tree_tmp = NULL;	/* Solely to avoid dummy warning C4090: 'function' : different 'volatile' qualifiers */
		...
		dtd_addref (dtd, 0);
	      strses_free (ses);
	      if ((NULL == tree) && (DEAD_HTML != (parser_mode & ~(FINE_XSLT | GE_XML | WEBIMPORT_HTML | FINE_XML_SRCPOS))))
		sqlr_error ("42000", "The BLOB passed to a function %.200s() contains corrupted packed XML serialization data",
		    bifname);
	      goto tree_complete;	/* see below */
#endif
	    }
...
ttlyyerror_impl (ttlp_t * ttlp_arg, const char *raw_text, const char *strg)
{
  if (NULL != ttlp_arg)
	...
	raw_text = ttlp_arg[0].ttlp_raw_text;
	  ...
	  ttlp_arg[0].ttlp_err_hdr, lineno, strg, ((NULL == raw_text) ? "" : " at "), ((NULL == raw_text) ? "" : raw_text));
    }
...
ttlyyerror_impl_1 (ttlp_t * ttlp_arg, const char *raw_text, int yystate, short *yyssa, short *yyssp, const char *strg)
{
  if (NULL != ttlp_arg)
	...
	raw_text = ttlp_arg[0].ttlp_raw_text;
	  ...
	  ((NULL == raw_text) ? "" : " at '"), ((NULL == raw_text) ? "" : raw_text), ((NULL == raw_text) ? "" : "'"));
    }
  sqlr_new_error ("37000", "SP029", "Parser error: %s; please report the parsed document", strg);
}
...
caddr_t
ttlp_strliteral (ttlp_t * ttlp_arg, const char *strg, int mode, char delimiter)
{
  caddr_t tmp_buf;
  caddr_t res;
  const char *err_msg;
...
{
  if (!strcmp (qname, "a"))
    return TTLP_ALLOW_QNAME_A;
  if (!strcmp (qname, "has"))
    return TTLP_ALLOW_QNAME_HAS;
  if (!strcmp (qname, "is"))
    return TTLP_ALLOW_QNAME_IS;
  if (!strcmp (qname, "of"))
    return TTLP_ALLOW_QNAME_OF;
  if (!strcmp (qname, "this"))
    return TTLP_ALLOW_QNAME_THIS;
  return 0;
}
...
void
tf_triple_l (triple_feed_t * tf, caddr_t s_uri, caddr_t p_uri, caddr_t obj_sqlval, caddr_t obj_datatype, caddr_t obj_language)
{
  ...
  switch (DV_TYPE_OF (obj_sqlval))
    {
    case DV_LONG_INT:
      rdf_dbg_printf (("\ntf_triple_l (%ld)", (long) (obj_sqlval)));
      break;
    case DV_STRING:
    case DV_UNAME:
      rdf_dbg_printf (("\ntf_triple_l (%s, %s, %s)", obj_sqlval, obj_datatype, obj_language));
      break;
    default:
      rdf_dbg_printf (("\ntf_triple_l (..., %s, %s)", obj_datatype, obj_language));
      break;
    }
#endif
...
bif_rdf_load_turtle (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  res = rdf_load_turtle (str, 0, base_uri, graph_uri, flags,
      (ccaddr_t *) cbk_names, app_env, (query_instance_t *) qst, QST_CHARSET (qst), &err);
  if (NULL != err)
    {
      dk_free_tree (res);
      sqlr_resignal (err);
...
dk_hash_t rdf_iri_always_cached;
...
caddr_t DBG_NAME (nic_id_name_n) (DBG_PARAMS name_id_cache_t * nic, boxint id)
{
  caddr_t ret;
  boxint r;
  int nth_id;
...
/*  id_hashed_key_t hi = iristrhash (&name);
  ...
  while (nic->nic_ni_array[nth_name]->ht_count > nic->nic_size / nic->nic_n_ways)
	...
	{
	  if (gethash ((void *) el.nicel_id, &rdf_iri_always_cached))
	    {
	      id_hash_set (nic->nic_ni_array[nth_name], (caddr_t) & el.nicel_name, (caddr_t) & el.nicel_id);
	      if (!cachelet_mutex_locked)
		mutex_leave (&nic->nic_ni_mtx[nth_name]);
	      return;
	    }
...
nic_remove_some_elements (name_id_cache_t * nic, char cache_mutex_locked)
{
  ...
  while (nic->nic_id_to_name->ht_count > nic->nic_size)
      ...
      if (id_hash_remove_rnd (nic->nic_name_to_id, rnd, (caddr_t) & el.nicel_name, (caddr_t) & el.nicel_id))
	{
	  if (gethash ((void *) el.nicel_id, &rdf_iri_always_cached))
	    {
	      id_hash_set (nic->nic_name_to_id, (caddr_t) & el.nicel_name, (caddr_t) & el.nicel_id);
	      if (!cache_mutex_locked)
...
{
  name_id_cache_t *lt_nic = lt->lt_client->cli_row_autocommit ? NULL : nic == iri_name_cache ? lt->lt_rdf_iri : lt->lt_rdf_prefix;
  if (lt_nic)
    {
...
caddr_t DBG_NAME (lt_nic_id_name) (DBG_PARAMS lock_trx_t * lt, name_id_cache_t * nic, boxint id)
{
  name_id_cache_t *lt_nic = lt->lt_client->cli_row_autocommit ? NULL : nic == iri_name_cache ? lt->lt_rdf_iri : lt->lt_rdf_prefix;
  if (lt_nic)
    {
...
tb_string_and_id_check (dbe_table_t * tb, dbe_column_t ** str_col, dbe_column_t ** id_col)
{
  ...
  if (!col2
      ...
      || (!IS_INT_LIKE (col2->col_sqt.sqt_dtp)) || !tb->tb_keys->next || tb->tb_keys->next->next)
    return 0;
  *str_col = col1;
  *id_col = col2;
  return 1;
  ...
  if (-1 == nth)
    {
      ...
      nth = (((uptrlong) self) ^ (((uptrlong) self) >> 11)) % N_IRI_SEQS_USED;
    }
  else
  ...
  if (!in_srv_global_init)
    {
	  ...
	  box_dv_short_string (iri_seq[nth]), box_dv_short_string (iri_seq_max[nth]), box_num (id), box_num (id + IRI_RANGE_SZ));
...
tb_new_id_and_name (lock_trx_t * lt, it_cursor_t * itc, dbe_table_t * tb, caddr_t name, char *value_seq_name)
{
  ...
  dbe_column_t *id_col;
  caddr_t res_box;
  caddr_t seq_box = NULL;
  if (box_flags (name) >= 4)
    {
      query_instance_t qi_tmp;
      uint32 seq_no = box_flags (name) / 4;
      int64 log_id;
      memzero (&qi_tmp, sizeof (qi_tmp));
      qi_tmp.qi_trx = lt;
      res = ir_new_id ((caddr_t *) & qi_tmp, seq_no, &log_id);
    ...
    {
      seq_box = box_dv_short_string (value_seq_name);
  ...
  if (itc->itc_insert_key->key_partition)
    {
	  ...
	  "tb_cl_name_to_id is not to be run. Problem of initializing partitioned db with local only mode.");
...
/*                                             01234567 */
  ...
  if (CL_RUN_SINGLE_CLUSTER == cl_run_local_only)
    return NULL;
  if ((by_ip = iri_from_ip (&name, NULL, &name_to_del)))
    {
      return box_iri_id (by_ip);
    }
  flags = box_flags (name);
  if ((BF_N_IRI & flags))
    {
      GPF_T1 ("rdf n iri not supported for single server");
    }
  ...
  box_flags (local) = flags;
  iri_id = tb_name_to_id (lt, "DB.DBA.RDF_IRI", local,
      (make_new ? ((('_' == name[0]) && (':' == name[1])) ? "RDF_URL_IID_NAMED_BLANK" : "RDF_URL_IID_NAMED") : NULL));
  if (!iri_id)
    {
...
again:
  switch (dtp)
    ...
    default:
      err_ret[0] = srv_make_new_error ("RDFXX", ".....", "Bad argument to iri_to_id (), type %d", (unsigned int) orig_dtp);
      goto return_error;	/* see below */
    }
...
/*                     0123456789 */
  if (!strncmp (name, "nodeID://", 9))
	...
	acc = acc * 10 + ((tail++)[0] - '0');
      if ('\0' != tail[0])
	{
	  err_ret[0] = srv_make_new_error ("RDFXX", ".....", "Bad argument to iri_to_id (), '%.100s' is not valid bnode IRI", name);
	  goto return_error;	/* see below */
	}
      acc += prefix_base;
...
bif_id_to_iri (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (DV_TYPE_OF (iid_box))
	...
	else if ((iri = iri_to_string (iid, NULL)))
	  return iri;
	else
	  {
...
bif_rdf_cache_id_to_name (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  name_id_cache_t *cache = mode[0] == 'p' ? iri_prefix_cache
      : mode[0] == 'l' ? rdf_lang_cache : mode[0] == 'i' ? iri_name_cache : mode[0] == 't' ? rdf_type_cache : NULL;
  if (!cache)
...
char *rdf_n_iri_text = "create table DB.DBA.RDF_N_IRI (RIN_EXT_ID iri_id_8 not null primary key, RIN_INT_ID IRI_ID_8 not null)"
    "alter index RDF_N_IRI on RDF_N_IRI partition (RIN_EXT_ID int (0hexffff00))"
    "create column index RI_N_REV on RDF_N_IRI (RIN_INT_ID) partition (RIN_INT_ID int (0hexffff00))";
...
int
rdf_obj_ft_rule_iri_hkey_cmp (caddr_t d1, caddr_t d2)
{
  ...
  if (NULL == known_reasons_ptr[0])
    {
      ...
      {
	rdf_obj_ft_rule_iri_hkey_t *old_iri_hkey =
	    (rdf_obj_ft_rule_iri_hkey_t *) id_hash_get_key (rdf_obj_ft_rules_by_iris, (caddr_t) (&iri_hkey));
...
bif_rdf_obj_set_is_text_if_ft_rule_check (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (DV_TYPE_OF (obj))
	...
	if (obj_rb->rb_is_text_index
	    || (obj_rb->rb_is_outlined && (DV_STRING != ((rdf_bigbox_t *) obj_rb)->rbb_box_dtp)
		&& (DV_XML_ENTITY != ((rdf_bigbox_t *) obj_rb)->rbb_box_dtp)))
	  return box_num (1);
...
caddr_t
bif_rdf_iri_always_cached (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  caddr_t arg = bif_arg (qst, args, 0, "rdf_iri_always_cached");
  iri_id_t i;
  ...
  if (DV_LONG_INT == dtp)
    ...
    return NULL;
  sethash ((void *) i, &rdf_iri_always_cached, (void *) 1);
  return NULL;
}

int always_cached_inited = 0;

caddr_t
bif_rdf_iri_always_cached_init (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  QNCAST (QI, qi, qst);
  if (always_cached_inited)
    return NULL;
  DO_HT (iri_id_t, id, void *, ign, &rdf_iri_always_cached)
  {
    caddr_t box = key_id_to_canonicalized_iri (qi, id);
    dk_free_box (box);
  }
  END_DO_HT;
  always_cached_inited = 1;
  return NULL;
}
...
void
jso__rdf_val_range_inst_validation (rdf_val_range_t * rvr, const char *structname, jso_rtti_t * inst_rtti,
    dk_set_t * warnings_log_ptr)
{
  ...
  if (NULL != rvr->rvrFixedValue)
      ...
      else
	{
	  ...
	  if ((NULL == dt)
		  ...
		  && !((SPART_VARR_IS_LIT & saved_orig_rvr.rvrRestrictions)
		      && (SPART_VARR_FIXED & saved_orig_rvr.rvrRestrictions))))
	    {
    ...
    {
      dk_set_push (warnings_log_ptr, list (2, inst_rtti, box_sprintf (1000,
		  "%s-rvrDatatype is set to <%.500s> whereas %s-rvrRestrictions has no SPART_VARR_TYPED bit set", structname,
		  rvr->rvrDatatype, structname)));
      rvr->rvrDatatype = NULL;
    }
...
void
jso__quad_map_jsocd_validation_cbk (jso_rtti_t * inst_rtti, dk_set_t * warnings_log_ptr)
{
...
void
jso__qm_value_jsocd_validation_cbk (jso_rtti_t * inst_rtti, dk_set_t * warnings_log_ptr)
{
...
void
jso__qm_format_jsocd_validation_cbk (jso_rtti_t * inst_rtti, dk_set_t * warnings_log_ptr)
{
...
rdf_core_init (void)
{
  hash_table_init (&rdf_iri_always_cached, 223);
  jso_init ();
  ...
  bif_define ("rdf_iri_always_cached", bif_rdf_iri_always_cached);
  bif_define ("rdf_iri_always_cached_init", bif_rdf_iri_always_cached_init);
  bif_define_ex ("iri_to_id", bif_iri_to_id, BMD_ALIAS, "__i2id", BMD_VECTOR_IMPL, bif_iri_to_id_vec, BMD_RET_TYPE, &bt_iri_id,
      BMD_USES_INDEX, BMD_NO_CLUSTER, BMD_OUT_OF_PARTITION, BMD_NEED_ENLIST, BMD_DONE);
  ...
  bif_define_ex ("iri_to_id_nosignal", bif_iri_to_id_nosignal, BMD_ALIAS, "__i2idn", BMD_RET_TYPE, &bt_iri_id, BMD_USES_INDEX,
      BMD_OUT_OF_PARTITION, BMD_DONE);
  bif_define_ex ("iri_to_id_if_cached", bif_iri_to_id_if_cached, BMD_ALIAS, "__i2idc", BMD_RET_TYPE, &bt_iri_id, BMD_DONE);
  bif_define_ex ("id_to_iri", bif_id_to_iri, BMD_ALIAS, "__id2i", BMD_VECTOR_IMPL, bif_id2i_vec, BMD_RET_TYPE,
      &bt_any /* was &bt_varchar */ , BMD_USES_INDEX, BMD_OUT_OF_PARTITION, BMD_DONE);
  bif_define_ex ("id_to_canonicalized_iri", bif_id_to_canonicalized_iri, BMD_RET_TYPE, &bt_any /* was &bt_varchar */ ,
      BMD_USES_INDEX, BMD_OUT_OF_PARTITION, BMD_DONE);
  bif_define_ex ("id_to_iri_nosignal", bif_id_to_iri_nosignal, BMD_ALIAS, "__id2in", BMD_VECTOR_IMPL, bif_id2i_vec_ns, BMD_RET_TYPE,
      &bt_any /* was &bt_varchar */ , BMD_USES_INDEX, BMD_OUT_OF_PARTITION, BMD_DONE);
  ...
  ddl_ensure_table ("DB.DBA.RDF_N_IRI", rdf_n_iri_text);
  ...
  rdf_obj_ft_rules_by_iids = id_hash_allocate (1000,
      sizeof (rdf_obj_ft_rule_iid_hkey_t), sizeof (dk_set_t), rdf_obj_ft_rule_iid_hkey_hash, rdf_obj_ft_rule_iid_hkey_cmp);
  rdf_obj_ft_rules_by_iris = id_hash_allocate (1000,
      sizeof (rdf_obj_ft_rule_iri_hkey_t), sizeof (dk_set_t), rdf_obj_ft_rule_iri_hkey_hash, rdf_obj_ft_rule_iri_hkey_cmp);
  rdf_obj_ft_graph_rule_counts = id_hash_allocate (1000, sizeof (boxint), sizeof (ptrlong), boxint_hash, boxint_hashcmp);
