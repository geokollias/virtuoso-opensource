...
extern sparp_equiv_t *sparp_equiv_get_ro (sparp_equiv_t ** equivs, ptrlong equiv_count, SPART * haystack_gp, SPART * needle_var,
    int flags);
/*! Finds an equiv class that supplies a subvalue to the \c receiver from the specified \c haystack_gp */
extern sparp_equiv_t *sparp_equiv_get_subvalue_ro (sparp_equiv_t ** equivs, ptrlong equiv_count, SPART * haystack_gp,
    sparp_equiv_t * receiver);
 ...
 */
extern int sparp_values_equal (sparp_t * sparp, ccaddr_t first, ccaddr_t first_dt, ccaddr_t first_lang, ccaddr_t second,
    ccaddr_t second_dt, ccaddr_t second_lang);
...
extern SPART *sparp_find_origin_of_external_varname_in_eq (sparp_t * sparp, sparp_equiv_t * eq, caddr_t varname,
    int find_exact_specimen, int null_result_allowed);
...
If no suitable item found then -1 is returned */
extern int sparp_find_sinv_rset_or_param_pos_of_varname (sparp_t * sparp, SPART * service_gp, caddr_t e_varname,
    int do_search_for_param);
...
extern void sparp_gp_attach_member (sparp_t * sparp, SPART * parent_gp, SPART * new_child, int insert_before_idx,
...
extern void sparp_gp_attach_many_members (sparp_t * sparp, SPART * parent_gp, SPART ** new_members, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr);
...
extern SPART **sparp_gp_detach_all_filters (sparp_t * sparp, SPART * parent_gp, int extract_filters_replaced_by_equivs,
    sparp_equiv_t *** touched_equivs_ptr);
...
extern void sparp_gp_attach_filter (sparp_t * sparp, SPART * parent_gp, SPART * new_filter, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr);
...
Filters should be attached after triple patterns are attached, to make proper assignments of triple_with_var_obj->_.triple.ft_type . */
extern void sparp_gp_attach_many_filters (sparp_t * sparp, SPART * parent_gp, SPART ** new_filters, int insert_before_idx,
    sparp_equiv_t *** touched_equivs_ptr);
...
extern void sparp_find_best_join_eq_for_optional (sparp_t * sparp, SPART * parent, int pos_of_curr_memb, sparp_equiv_t * eq,
...
#define SSG_VALMODE_SQLVAL		((ssg_valmode_t)((ptrlong)(0x330)))	/*!< SQL value to be returned to the SQL caller */
...
extern void ssg_print_tmpl (struct spar_sqlgen_s *ssg, qm_format_t * qm_fmt, ccaddr_t tmpl, ccaddr_t alias, qm_value_t * qm_val,
    SPART * tree, const char *asname);
...
extern void ssg_print_fld_var_restrictions_ex (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid,
    SPART * fld_tree, SPART * triple, SPART * fld_if_outer, rdf_val_range_t * rvr, ptrlong restr_bits_to_ignore);
extern void ssg_print_fld_restrictions (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid, SPART * gp,
    SPART * triple, int fld_idx, int print_outer_filter);
extern void ssg_print_all_table_fld_restrictions (spar_sqlgen_t * ssg, quad_map_t * qm, caddr_t alias, SPART * gp, SPART * triple,
...
extern void ssg_print_union (spar_sqlgen_t * ssg, SPART * gp, SPART ** retlist, int head_flags, int retval_flags,
