...
sqlc_insert (sql_comp_t * sc, ST * tree)
{
  ...
  if ((vd = (ST *) sch_view_def (wi_inst.wi_schema, tb->tb_name)) && !tb_is_trig_at (tb, TRIG_INSERT, TRIG_INSTEAD, NULL))
    {
...
sqlc_sch_list (sql_comp_t * sc, ST * tree)
{
  ...
  if (n_elts == 1 && list[0]->type == VIEW_DEF)
      ...
      if (!sc->sc_store_procs)
	{
	  ...
	  sch_set_view_def (wi_inst.wi_schema, elt->_.view_def.name, (caddr_t) elt->_.view_def.exp);
	  return;
	}
