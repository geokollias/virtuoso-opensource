...
box_add_keys (ST ** selection, dbe_table_t * tb, dbe_key_t * key_only, caddr_t * upd_cols)
{
  ...
  DO_SET (dbe_key_t *, key, &tb->tb_keys)
  {
    ...
    DO_SET (dbe_column_t *, col, &key->key_parts)
    {
      ...
      if (((key->key_is_primary && !upd_cols) || (upd_cols && !key->key_is_primary)) && ++n == key->key_n_significant)
	break;
    }
    END_DO_SET ();
  }
  ...
  n_sel = (ST **) t_alloc_box (sizeof (caddr_t) * (len + n_new), DV_ARRAY_OF_POINTER);
  if (selection)
    {
      DO_BOX (ST *, ref, inx, selection)
      {
...
tc_init (trig_cols_t * tc, int event, dbe_table_t * tb, caddr_t * cols, ST ** vals, dbe_key_t * add_pk)
{
  ...
  if (tb_is_trig (tb, event, cols))
    ...
    {
      tc->tc_cols = cols, tc->tc_vals = vals;
...
ins_tb_all_cols (dbe_table_t * tb)
{
  ...
  cols = (dbe_column_t **) t_list_to_array (key_ensure_visible_parts (tb->tb_primary_key));
...
t_box_append_1 (caddr_t box, caddr_t elt)
{
  ...
  b2 = t_alloc_box (box_length (box) + sizeof (caddr_t), box_tag (box));
  memcpy (b2, box, box_length (box));
  *((caddr_t *) (b2 + box_length (box))) = elt;
  return b2;
}
...
void
sqlc_insert_autoincrements (sql_comp_t * sc, insert_node_t * ins, dk_set_t * code)
{
  ...
  DO_SET (dbe_column_t *, col, &tb->tb_primary_key->key_parts)
  {
	...
	if (old_sl)
	  ...
	  {
	    ins->ins_col_ids = (oid_t *) box_append_1_free ((caddr_t) ins->ins_col_ids, (caddr_t) (ptrlong) col->col_id);
	...
	if (col->col_sqt.sqt_col_dtp != DV_TIMESTAMP)
	...
	else
	  {
	    ...
	    args = (state_slot_t **) dk_alloc_box (0, DV_ARRAY_OF_POINTER);
	  }
...
state_slot_t **
sqlc_ins_triggers_1 (sql_comp_t * sc, dbe_table_t * tb, oid_t * col_ids, dk_set_t values, dk_set_t * code)
{
  if (tb_is_trig (tb, TRIG_INSERT, NULL))
    {
      int fill = 0, cinx;
      int n_cols = dk_set_length (tb->tb_primary_key->key_parts);
      state_slot_t **args = (state_slot_t **) dk_alloc_box (n_cols * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
      DO_SET (dbe_column_t *, col, &tb->tb_primary_key->key_parts)
      {
	  ...
	  if (col->col_id == cid)
	    {
	      ...
	      state_slot_t *sl2 = sl;
	      if (!sl)
		sqlc_new_error (sc->sc_cc, "21S01", "SQ093", "Mismatched columns and values in insert.");
	      if (!SSL_IS_REFERENCEABLE (sl))
		{
	...
	if (col->col_sqt.sqt_col_dtp == DV_TIMESTAMP)
	...
	else
	  {
	    ...
	    args[fill++] = tmp;
	    cv_artm (code, (ao_func_t) box_identity, tmp, ssl_new_constant (sc->sc_cc, deflt), NULL);
	  }
      next_col:;
      }
      END_DO_SET ();
...
{
  ins->ins_trigger_args = sqlc_ins_triggers_1 (sc, ins->ins_table, ins->ins_col_ids, ins->ins_values, code);
}
...
sqlc_stmt_nth_col_type (sql_comp_t * sc, dbe_table_t * tb, ST * tree, int nth)
{
  ...
  switch (tree->type)
      ...
      else
	{
	  ...
	  if (!col)
	    sqlc_new_error (sc->sc_cc, "21S01", "SQ094", "Too many (%d) values in insert into %s", nth + 1, tb->tb_name);
...
sqlc_rls_get_condition_string (dbe_table_t * tb, int op, caddr_t * err)
{
  ...
  *err = qr_quick_exec (call_qr, cli, NULL, &lc, 3,
      ":0", tb->tb_rls_procs[op], QRP_STR, ":1", tb->tb_name, QRP_STR, ":2", action_names[op], QRP_STR);
...
sqlc_make_policy_trig (comp_context_t * cc, dbe_table_t * tb, int op)
{
  ...
  if (tb->tb_rls_procs[op] && cli->cli_user && !sec_user_has_group (0, cli->cli_user->usr_g_id))
    {
      caddr_t ret_val;
      caddr_t sql = NULL;
      ...
      if (!err)
	{
	  sql = dk_alloc_box (strlen (create_trigger_mask[op]) + 3 * strlen (tb->tb_name) + strlen (ret_val) + 1, DV_SHORT_STRING);
	  snprintf (sql, box_length (sql) - 1, create_trigger_mask[op],
	      tb->tb_name, tb->tb_qualifier, tb->tb_owner, tb->tb_name_only, ret_val, tb->tb_name);
	  dk_free_tree (ret_val);
...
sqlc_insert (sql_comp_t * sc, ST * tree)
{
  ...
  if (!tb)
    sqlc_new_error (sc->sc_cc, "42S02", "SQ096", "No table %.300s.", tree->_.insert.table->_.table.name);
  ...
  if ((vd = (ST *) sch_view_def (sc->sc_cc->cc_schema, tb->tb_name)) && !tb_is_trig_at (tb, TRIG_INSERT, TRIG_INSTEAD, NULL))
    {
...

  {
	...
	DO_BOX (ST *, exp, inx, vals)
	{
	  ...
	  slots = NCONC (slots, CONS (scalar_exp_generate (sc, exp, &code), NULL));
	}
	END_DO_BOX;
	SC_OLD_EXCEPT (sc);
    ...
    if (dk_set_length (ins->ins_values) != BOX_ELEMENTS (ins->ins_col_ids))
      sqlc_new_error (sc->sc_cc, "21S01", "SQ099", "different number of cols and values in insert.");
    if (TB_MAX_COLS <= dk_set_length (ins->ins_table->tb_primary_key->key_parts))
      sqlc_new_error (sc->sc_cc, "42000", "SQ100", "A local table of over maximum columns may not be inserted");
    sqlc_ins_param_types (sc, ins);
    sqlc_ins_keys (sc->sc_cc, ins);
    sqlg_cl_insert (sc, sc->sc_cc, ins, tree, &code);
    if (sqlo_opt_value (opts, OPT_VECTORED) && !sc->sc_cc->cc_query->qr_proc_vectored)
  ...
  if (ts_cols || ts_cols_pos)
    {
      ...
      ST **new_vals = (ST **) t_alloc_box ((n_cols + n_ts_cols) * sizeof (caddr_t),
	  DV_ARRAY_OF_POINTER);
      ST **new_cols = (ST **) t_alloc_box ((n_cols + n_ts_cols) * sizeof (caddr_t),
	  DV_ARRAY_OF_POINTER);
      memcpy (new_cols, tree->_.update_pos.cols, box_length ((caddr_t) tree->_.update_pos.cols));
      memcpy (new_vals, tree->_.update_pos.vals, box_length ((caddr_t) tree->_.update_pos.vals));
      DO_SET (ptrlong, col_pos, &ts_cols_pos)
      {
	new_vals[col_pos] = (ST *) t_list (3, CALL_STMT, t_sqlp_box_id_upcase ("get_timestamp"), t_list (0));
      }
      END_DO_SET ();
      DO_SET (dbe_column_t *, col, &ts_cols)
      {
	new_vals[n_cols] = (ST *) t_list (3, CALL_STMT, t_sqlp_box_id_upcase ("get_timestamp"), t_list (0));
...
upd_optimize (sql_comp_t * sc, update_node_t * upd)
{
  ...
  DO_BOX (ptrlong, cid, inx, upd->upd_col_ids)
  {
	...
	|| col->col_is_key_part || (upd->upd_table->tb_any_blobs && dtp_is_var (col->col_sqt.sqt_dtp)))
      {
...
void
sqlc_pl_selection (sql_comp_t * sc, dbe_table_t * tb, state_slot_t * place, ST ** exps, state_slot_t *** slots_ret)
{
  ...
  DO_SET (dbe_column_t *, col, &tb->tb_primary_key->key_parts)
  {
    ...
    t_NEW_VARZ (col_ref_rec_t, crr);
    crr->crr_ssl = sl;
    crr->crr_col_ref = (ST *) t_list (3, COL_DOTTED, NULL, t_box_string (col->col_name));
    sqlc_temp_tree (sc, (caddr_t) crr->crr_col_ref);
    t_set_push (&sc->sc_col_ref_recs, (void *) crr);
  }
  END_DO_SET ();

  slots = (state_slot_t **) dk_alloc_box (sizeof (caddr_t) * n_out, DV_ARRAY_OF_POINTER);
...
/*    while (local_crr != org_scope)
    ...
    sql_node_append (&sc->sc_cc->cc_query->qr_head_node, (data_source_t *) pls);
  }
}


void
sqlc_update_pos_selection (sql_comp_t * sc, trig_cols_t * tc, state_slot_t *** slots_ret, state_slot_t * place, dk_set_t * code)
{
  int inx;
  if (tc->tc_is_trigger)
    {
...
upd_value_slots (trig_cols_t * tc, state_slot_t ** slots)
{
  ...
  {
    if (nth < pk->key_n_significant || IS_BLOB_DTP (col->col_sqt.sqt_dtp) || tb_is_key (tc->tc_table, col))
...
sqlc_update_cl_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * sqc)
{
  ...
  stree = (ST *) t_list (5, UPDATE_SRC,
      ...
      t_box_copy_tree ((caddr_t) tree->_.update_pos.cols), t_box_copy_tree ((caddr_t) tree->_.update_pos.vals), texp);
...
sqlc_update_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * cursor_sqc, ST ** src_ret)
{
  dbe_table_t *tb = sch_name_to_table (sc->sc_cc->cc_schema,
      tree->_.update_pos.table->_.table.name);
  sqlc_table_used (sc, tb);
  if (tb && (tb->tb_primary_key->key_is_col || find_remote_table (tb->tb_name, 0) || (tb->tb_primary_key->key_partition
	      && !sqlo_opt_value (tree->_.update_pos.opts, OPT_NO_CLUSTER))))
    {
      if (!src_ret)
	...
	if (!sec_checked && !sec_col_check (col, SC_G_ID (sc), SC_U_ID (sc), GR_UPDATE))
	...
	col_ids[inx] = col->col_id;
      ...
      upd->upd_place = cursor_sqc ? cursor_sqc->sqc_ssl : sqlc_make_co_node (sc, tree->_.update_pos.cursor, tb);
	...
	sqlc_new_error (sc->sc_cc, "09000", "SQ104",
      ...
      tc_init (&tc, TRIG_UPDATE, tb, (caddr_t *) tree->_.update_pos.cols, tree->_.update_pos.vals, NULL);
...
sqlc_update_searched (sql_comp_t * sc, ST * tree)
{
  ...
  if (tb && find_remote_table (tb->tb_name, 0))
  ...
  else
    {
      ...
      upd->upd_hi_id = upd_hi_id_ctr++;
      if (!tb)
	sqlc_new_error (sc->sc_cc, "42S02", "SQ105", "No table %.300s.", tree->_.update_src.table->_.table.name);

      upd->upd_table = tb;

      sec_checked = sec_tb_check (tb, (oid_t) unbox (tb_ref->_.table.g_id), (oid_t) unbox (tb_ref->_.table.u_id), GR_UPDATE);
      upd->upd_policy_qr = sqlc_make_policy_trig (sc->sc_cc, tb, TB_RLS_U);
      if (!sqlo_opt_value (opts, OPT_NO_IDENTITY))
	upd_add_auto_updates (tb, tree);
	...
	if (!sec_checked &&
	    !sec_col_check (col, (oid_t) unbox (tb_ref->_.table.g_id), (oid_t) unbox (tb_ref->_.table.u_id), GR_UPDATE))
	  {
	    dk_free_box ((caddr_t) col_ids);
	    sqlc_new_error (sc->sc_cc,
	...
	col_ids[inx] = col->col_id;
       ...
       */
      tc_init (&tc, trig_event, tb, (caddr_t *) tree->_.update_src.cols, tree->_.update_src.vals, NULL);
	  ...
	  (caddr_t *) tc.tc_selection, tree->_.update_src.table_exp, &sc->sc_cc->cc_query->qr_head_node, &slots);
      sc->sc_is_update = 0;
      sc->sc_need_pk = 0;
...
sqlc_delete_cl_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * sqc)
{
  ...
  texp = sqlp_infoschema_redirect (t_listst (9, TABLE_EXP,
	  ...
	  sqlc_pos_to_searched_where (sc, sqc, tree->_.delete_pos.cursor, tb), NULL, NULL, NULL, NULL, NULL, NULL));
...
sqlc_delete_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * cursor_sqc, ST ** src_ret)
{
  ...
  sqlc_table_used (sc, tb);
  if (tb && (tb->tb_primary_key->key_is_col || find_remote_table (tb->tb_name, 0) || (tb->tb_primary_key->key_partition
	      && !sqlo_opt_value (tree->_.delete_pos.opts, OPT_NO_CLUSTER))))
    {
      if (!src_ret)
	...
	sqlc_new_error (sc->sc_cc, "43000", "SQ108:SECURITY", "Permission denied for delete from %.300s (user ID = %lu)",
	    tb->tb_name, SC_U_ID (sc));

      del->del_table = tb;
      del->del_policy_qr = sqlc_make_policy_trig (sc->sc_cc, tb, TB_RLS_D);
      del->del_place = cursor_sqc ? cursor_sqc->sqc_ssl : sqlc_make_co_node (sc, tree->_.delete_pos.cursor, tb);
      if (!del->del_place)
...
sqlc_delete_searched (sql_comp_t * sc, ST * tree)
{
  ...
  if (tb && find_remote_table (tb->tb_name, 0))
	...
	sqlc_new_error (sc->sc_cc, "42S02", "SQ111", "No table %s.", from->_.table.name);
      ...
      tc_init (&tc, del->del_key_only ? key_trig_event : TRIG_DELETE, tb, NULL, NULL,
	  sqlg_is_vector ? (del->del_key_only ? del->del_key_only : TC_ALL_KEYS) : 0);
      sc->sc_in_cursor_def = 1;
      sc->sc_cc->cc_query->qr_lock_mode = PL_EXCLUSIVE;
      sc->sc_parallel_dml = enable_mt_txn;
      sc->sc_is_update = SC_UPD_PLACE;
      sqlo_query_spec (sc, 0, (caddr_t *) tc.tc_selection, tree->_.delete_src.table_exp,
	  &sc->sc_cc->cc_query->qr_head_node, &slot_array);
      sc->sc_is_update = 0;
      sc->sc_in_cursor_def = 0;
...
ddl_need_schema_reload (ptrlong type)
{
  if (TABLE_DEF == type
      || INDEX_DEF == type
      || TABLE_DROP == type || ADD_COLUMN == type || TABLE_RENAME == type || INDEX_DROP == type || VIEW_DEF == type)
    return 1;
  else
    return 0;
}
...
sqlc_table_from_select_view (query_t * view_qr, ST * view_def)
{
  ...
  DO_BOX (state_slot_t *, ssl, inx, sel_out)
  {
    ...
    col_is_indexable = !((DV_BLOB == sl_dtp) || (DV_BLOB_WIDE == sl_dtp) || (DV_BLOB_BIN == sl_dtp) || (DV_BLOB_XPER == sl_dtp));
    t_dk_set_append_1 (&cols, (void *) t_box_string (ssl->ssl_name));
    t_dk_set_append_1 (&cols, t_list (2, t_list (3, t_box_num (sl_dtp), t_box_num (sl_prec), t_box_num (sl_scale)), NULL));
    if (col_is_indexable)
      t_dk_set_append_1 (&key_parts, (void *) t_box_string (ssl->ssl_name));
  }
  END_DO_BOX;
  t_dk_set_append_1 (&cols, NULL);
  t_dk_set_append_1 (&cols, t_list (5, INDEX_DEF, NULL, NULL, t_list_to_array (key_parts), NULL));

  return ((ST *) t_list (3, TABLE_DEF, t_box_string (view_def->_.view_def.name), t_list_to_array (cols)));
}
...
/*GK: enable that if proc view params to participate in 'select *' */
      ...
      col_defs = (caddr_t *) t_alloc_box (n_cols * sizeof (caddr_t) +
	  +2 * box_length (view_def->_.view_def.exp->_.proc_table.params), DV_ARRAY_OF_POINTER);
	...
	/* GK : = ANY */
	col_defs[n_cols + inx * 2 + 1] = (caddr_t) t_list (2, t_list (2, (long) DV_ANY, (long) 0), NULL);
      }
      END_DO_BOX;
      return ((ST *) t_list (3, TABLE_DEF, t_box_copy_tree (view_def->_.view_def.name), (caddr_t) col_defs));
#else
...
sqlc_sch_list (sql_comp_t * sc, ST * tree)
{
  ...
  if (n_elts == 1 && list[0]->type == VIEW_DEF)
      ...
      if (!sc->sc_store_procs)
	{
	  dbe_table_t *tb = sch_name_to_table (sc->sc_cc->cc_schema,
	      elt->_.view_def.name);
	  if (!tb)
	    sqlc_new_error (sc->sc_cc, "42S02", "SQ112", "View without table %s", elt->_.view_def.name);
	  sqlo_calculate_subq_view_scope (sc, &elt->_.view_def.exp);
	  sch_set_view_def (sc->sc_cc->cc_schema, elt->_.view_def.name, (caddr_t) elt->_.view_def.exp);
	  return;
	}
  ...
  DO_BOX (ST *, elt, inx, list)
  {
	...
	sql_node_append (&sc->sc_cc->cc_query->qr_head_node, (data_source_t *) tcreate);
