...
bif_string_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_STRING)
	...
	"Function %s needs a string as argument %d, not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return arg;
}
...
bif_strict_2type_array_arg (dtp_t element_dtp1, dtp_t element_dtp2, caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_ARRAY_OF_POINTER)
	...
	func, dv_type_title (element_dtp1), dv_type_title (element_dtp2), nth + 1, dv_type_title (dtp), dtp);
  DO_BOX (caddr_t, el, inx, arg)
  {
  ...
  if (DV_LONG_INT == dtp)
    {
      ...
      if (!http_ses)
	sqlr_new_error ("22023", "HT081", "Function %.200s() outside of HTTP context and no stream specified", func);
      return http_ses;
    }
...
bif_tree_ent_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (!XE_IS_TREE (arg))
    sqlr_new_error ("22023", "SR345",
	"Persistent XML not allowed as an argument %d to function %s; this function accepts only XML tree entities", nth + 1, func);
...
bif_bin_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_BIN && dtp != DV_LONG_BIN)
	...
	"Function %s needs a binary as argument %d, not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return arg;
}
...
bif_string_or_wide_or_null_or_strses_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_SHORT_STRING && dtp != DV_LONG_STRING
      && dtp != DV_C_STRING && !IS_WIDE_STRING_DTP (dtp) && dtp != DV_STRING_SESSION)
    {
...
bif_string_or_wide_or_null_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_SHORT_STRING && dtp != DV_LONG_STRING && dtp != DV_C_STRING && !IS_WIDE_STRING_DTP (dtp))
    {
...
bif_string_or_uname_or_wide_or_null_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_STRING && dtp != DV_UNAME && dtp != DV_C_STRING && !IS_WIDE_STRING_DTP (dtp))
    {
      sqlr_new_error ("22023", "SR007",
  ...
  if (dtp != DV_SHORT_INT && dtp != DV_LONG_INT)
    {
	  ...
	  "Function %s needs an integer as argument %d, " "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
    }
  return (unbox (arg));
}
...
bif_iri_id_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_IRI_ID)
    sqlr_new_error ("22023", "SR008",
	"Function %s needs an IRI_ID as argument %d, " "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return (unbox_iri_id (arg));
}
...
bif_long_range_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, ptrlong low, ptrlong hi)
{
  ...
  if (arg < low)
    sqlr_new_error ("22023", "SR339", "Function %s needs an integer not less than %ld as argument %d", func, low, nth + 1);
  if (arg > hi)
    sqlr_new_error ("22023", "SR340", "Function %s needs an integer not greater than than %ld as argument %d", func, hi, nth + 1);
  return arg;
}
...
bif_long_low_range_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, ptrlong low)
{
  ptrlong arg = bif_long_arg (qst, args, nth, func);
  if (arg < low)
    sqlr_new_error ("22023", "SR378", "Function %s needs an integer not less than %ld as argument %d", func, low, nth + 1);
  return arg;
}
...
bif_float_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  sqlr_new_error ("22023", "SR009",
      "Function %s needs a float as argument %d, " "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return 0;			/*dummy */
}
...
bif_double_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  sqlr_new_error ("22023", "SR010",
      "Function %s needs a double as argument %d, " "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return 0;
}
...
bif_varchar_or_bin_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  caddr_t arg = bif_arg_unrdf (qst, args, nth, func);
  dtp_t dtp = DV_TYPE_OF (arg);
  if ((dtp != DV_BIN && dtp != DV_LONG_BIN && !IS_STRING_DTP (dtp)) || (box_length (args) < 1))
...
bif_array_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp == DV_SHORT_STRING || dtp == DV_LONG_STRING
      || IS_NONLEAF_DTP (dtp)
      || dtp == DV_ARRAY_OF_LONG || dtp == DV_ARRAY_OF_FLOAT || dtp == DV_ARRAY_OF_DOUBLE || IS_WIDE_STRING_DTP (dtp))
    return (arg);
...
bif_array_of_pointer_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp != DV_ARRAY_OF_POINTER)
    sqlr_new_error ("22023", "SR014",
	"Function %s needs a generic array as argument %d, not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  return (caddr_t *) arg;
}
...
bif_array_or_strses_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (dtp == DV_SHORT_STRING || dtp == DV_LONG_STRING || dtp == DV_UNAME
      ...
      || dtp == DV_ARRAY_OF_DOUBLE || IS_WIDE_STRING_DTP (dtp) || dtp == DV_STRING_SESSION)
    return (arg);
  sqlr_new_error ("22023", "SR012",
      "Function %s needs a string or an array as argument %d, "
      "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
...
bif_strict_array_or_null_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  ...
  if (IS_NONLEAF_DTP (dtp)
    ...
    return (arg);
  sqlr_new_error ("22023", "SR014",
      "Function %s needs an array as argument %d, " "not an arg of type %s (%d)", func, nth + 1, dv_type_title (dtp), dtp);
  NO_CADDR_T;
}
...
dbg_user_dump_user_ref (oid_t uid)
{
  ...
  sprintf (buf, "[%s%s,#%ld->\"%.100s\"]",
      (user->usr_disabled ? "DISABLED," : ""), (user->usr_is_role ? "group" : "user"), (long) (user->usr_id), user->usr_name);
  return buf;
}
...
bif_dbg_user_dump (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL != user->usr_member_ids)
    {
      ...
      {
	printf ("  %s", dbg_user_dump_user_ref (memb_id));
      } END_DO_BOX_FAST;
      printf ("\n");
    }
  if (NULL != user->usr_g_ids)
    {
      ...
      DO_BOX_FAST (ptrlong, role_id, ctr, user->usr_member_ids)
      {
	printf ("  %s", dbg_user_dump_user_ref (role_id));
      } END_DO_BOX_FAST;
      printf ("\n");
    }
  ...
  if ((1 == user->usr_flatten_g_ids_len) && (user->usr_id == user->usr_flatten_g_ids[0]))
    ...
    {
      int ctr;
      for (ctr = 0; ctr < user->usr_flatten_g_ids_len; ctr++)
	printf ("  %s", dbg_user_dump_user_ref (user->usr_flatten_g_ids[ctr]));
      printf ("\n");
    }
  NO_CADDR_T;
}
  ...
  for (inx = 0; inx < MIN (n_args, n_slots); inx++)
	  ...
	  && !(ha->ha_key_cols[inx + 1].cl_sqt.sqt_is_xml && XE_IS_VALID_VALUE_FOR_XML_COL (v)))
	{
	  *err_ret = srv_make_new_error ("22023", "SR540",
		  ...
		  ha->ha_slots[inx + 1]->ssl_name : ""), inx + 1);
	  return NULL;
	}
...
#else
	{
	  log_info ("bif_define_int: %s should get \t BMD_ALIAS, \"%s\", BMD_DONE", old_meta_for_bif->bmd_name, name);
	  if (old_meta_for_bif->bmd_is_aggregate != bmd->bmd_is_aggregate)
	    GPF_T1 ("bmd_is_aggregate differs in two defines of same bif_t");
	  if (old_meta_for_bif->bmd_no_cluster != bmd->bmd_no_cluster)
	    GPF_T1 ("bmd_no_cluster differs in two defines of same bif_t");
	  if (old_meta_for_bif->bmd_ret_type != bmd->bmd_ret_type)
	    GPF_T1 ("bmd_ret_type differs in two defines of same bif_t");
	}
#endif
    }
end:
...
bif_define (const char *raw_name, bif_t bif)
{
  ...
  bmd->bmd_name = box_dv_uname_string (name);
...
bif_metadata_t *
bif_define_typed (const char *raw_name, bif_t bif, bif_type_t * bt)
{
  caddr_t name;
  bif_metadata_t *bmd;
  name = sqlp_box_id_upcase (raw_name);
...
bif_define_ex (const char *raw_name, bif_t bif, ...)
{
  ...
  for (;;)
	...
	case BMD_ALIAS:
	  {
	    const char *raw_alias_name = va_arg (tail, const char *);
	    caddr_t alias_noupcase_name = box_dv_uname_string (raw_alias_name);
	    caddr_t alias_name = box_dv_uname_string (sqlp_box_id_upcase (raw_alias_name));
	    if (NULL != gethash (alias_noupcase_name, name_to_bif_sparql_only_metadata_hash))
	...
	case BMD_RET_TYPE:
	  ...
	  break;
	case BMD_MIN_ARGCOUNT:
	  bmd->bmd_min_argcount = va_arg (tail, int);
	  break;
	case BMD_ARGCOUNT_INC:
	  bmd->bmd_argcount_inc = va_arg (tail, int);
	  break;
	case BMD_MAX_ARGCOUNT:
	  bmd->bmd_max_argcount = va_arg (tail, int);
	  break;
	case BMD_IS_AGGREGATE:
	  bmd->bmd_is_aggregate = 1;
	  break;
	case BMD_IS_PURE:
	  bmd->bmd_is_pure = 1;
	  break;
	case BMD_IS_DBA_ONLY:
	  bmd->bmd_is_dba_only = 1;
	  break;
	case BMD_USES_INDEX:
	  bmd->bmd_uses_index = 1;
	  break;
	case BMD_SPARQL_ONLY:
	  bif_is_sparql_only = 1;
	  break;
	case BMD_NO_CLUSTER:
	  bmd->bmd_no_cluster |= BIF_NO_CLUSTER;
	  break;
	case BMD_OUT_OF_PARTITION:
	  bmd->bmd_no_cluster |= BIF_OUT_OF_PARTITION;
	  break;
	case BMD_NEED_ENLIST:
  ...
  if (bif_is_sparql_only)
	...
	GPF_T1 ("sparql-only pseudo bif name cannot be redefined");
      ...
      while (NULL != raw_alias_names)
	{
	  const char *raw_alias_name = (const char *) dk_set_pop (&raw_alias_names);
	  caddr_t alias_noupcase_name = box_dv_uname_string (raw_alias_name);
	  dk_set_push (&(bmd->bmd_aliases), alias_noupcase_name);
	  sethash (alias_noupcase_name, name_to_bif_sparql_only_metadata_hash, bmd);
	}
    }
  else
    {
      ...
      while (NULL != raw_alias_names)
	{
	  const char *raw_alias_name = (const char *) dk_set_pop (&raw_alias_names);
	  caddr_t alias_name = box_dv_uname_string (sqlp_box_id_upcase (raw_alias_name));
	  dk_set_push (&(bmd->bmd_aliases), alias_name);
	  id_hash_set (name_to_bif_metadata_idhash, (caddr_t) (&alias_name), (caddr_t) (&bmd));
	}
      bif_define_int (name, bif, bmd);
    }
  return bmd;
  ...
  if (NULL != bmd->bmd_ret_type)
    {
      ...
      dk_set_push_two (&props, box_dv_uname_string ("return_sql_dml_type"),
	  box_dv_short_string ((NULL != n) ? n : "any /* special */"));
    }
  ...
  if ((DV_LONG_INT == state_dtp || DV_SHORT_INT == state_dtp) && SQL_NO_DATA_FOUND == unbox (state))
    *err_ret = box_copy (state);
  else if (!DV_STRINGP (state))
    {
	  ...
	  "signal state should be an integer 100 (NO DATA FOUND) or a string value, not an %s", dv_type_title (state_dtp));
    }
...
bif_length (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (dtp)			/* Was: switch(box_tag (arg)) */
	...
	sqlr_new_error ("22023", "SR016",
	    ...
	    "not an argument of type %d (= %s)", dtp, dv_type_title (dtp));
      }
    }
  NO_CADDR_T;
}
...
strses_aref (caddr_t ses1, int idx)
{
  ...
  if (strses_get_part (ses, &buf, idx, 1))
	...
	" and length %lu.", idx, (unsigned long) strses_length (ses));
  ...
  if ((inx >= n_elems && DV_STRING_SESSION != box_tag (arr)) || (inx < 0))	/* Catch negative indexes also! */
    {
	  ...
	  "(%d) and length %d.", inx, dv_type_title (dtp), dtp, n_elems);
    }
  if (idxctr == idxcount)
...
/* failed_indexing */
  sqlr_new_error ("22003", "SR020", "aref() is called with %d index arguments but only %d first indexes can be accessed.", idxcount,
      idxctr);
  NO_CADDR_T;
}
...
again:
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (arr))
    sqlr_new_error ("22003", "SR020", "aref_set_0() is called with %d index arguments but only %d first indexes can be accessed.",
	argcount - 1, idxctr - 1);
...
again:
  ...
  if (idxctr < idxcount)
    sqlr_new_error ("22003", "SR020", "aset() is called with %d index arguments but only %d first indexes can be accessed.",
	idxcount, idxctr);
  return 0;
...
bs:
  if (1 == idxcount)
    ...
    sqlr_new_error ("22003", "SR020", "Argument %d of aset is a bad array subscript (value %ld exceeds array size).", idxcount + 1,
	(long) inx);
  NO_CADDR_T;
}
...
again:
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (arr))
    sqlr_new_error ("22003", "SR020", "aset_zap_arg() is called with %d index arguments but only %d first indexes can be accessed.",
	argcount - 2, idxctr - 1);
  n_elems = BOX_ELEMENTS (arr);
  if ((inx >= n_elems) || (inx < 0))	/* Catch negative indexes also! */
    sqlr_new_error ("22003", "SR017",
...
bif_aset_1_2_zap (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (tgt_inx >= BOX_ELEMENTS (tgt) || src_inx_1 >= BOX_ELEMENTS (src)
      || DV_ARRAY_OF_POINTER != DV_TYPE_OF (src[src_inx_1]) || src_inx_2 >= BOX_ELEMENTS (src[src_inx_1]))
...
bif_make_array (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (0 == strcmp (tp, "float"))
    ...
    sqlr_new_error ("22023", "SR022", "Type for make_array must be float, double, long or any");
...
strses_subseq (dk_session_t * ses, long from, long to)
{
  ...
  if (ses->dks_session->ses_file->ses_max_blocks_init)
    strses_enable_paging (out, ses->dks_session->ses_file->ses_max_blocks_init * DKSES_IN_BUFFER_LENGTH);
...
retry_unrdf:
  ...
  switch (dtp1)
    ...
    default:
	  ...
	  "not an arg of type %s (%d)", dv_type_title (dtp1), dtp1);
    }
  ...
  else if (!is_some_sort_of_an_integer (dtp2) || ((fail_in_dtp3 = 1), !is_some_sort_of_an_integer (dtp3)))
    {
      sqlr_new_error ("22023", "SR024",
	  "Function subseq needs integers or NULLs as its arguments 2 and 3, "
	  "not an argument of type %s (%d)", dv_type_title (fail_in_dtp3 ? dtp3 : dtp2), (fail_in_dtp3 ? dtp3 : dtp2));
    }
  else
    /* Both are integers. */
    {
  ...
  if (from > to)
	...
	"subseq: Bad string subrange: from=%ld, to=%ld, len=%ld.", (long) from, (long) to, (long) len);
  ...
  if (DV_UNAME == dtp1)
	...
	sqlr_new_error ("22011", "SR541",
	    "subseq: subrange from=%ld, to=%ld crosses UTF-8 character in the middle", (long) from, (long) to);
...
bif_substr (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (from > to)
	...
	"substr: Bad string subrange: from=%ld, to=%ld, len=%ld.", (long) from, (long) to, (long) len);
...
retry_unrdf:
  ...
  if (dtp1 == DV_DB_NULL)
    str = NULL;
      ...
      !IS_WIDE_STRING_DTP (dtp1) && dtp1 != DV_UNAME && dtp1 != DV_BIN && dtp1 != DV_LONG_BIN)
    {
      if (DV_RDF == dtp1)
	{
	  rdf_box_t *rb = (rdf_box_t *) str;
  ...
  if (dtp1 != DV_BIN && dtp1 != DV_LONG_BIN)
	...
	{
	  if (0x80 == (str[to] & 0xC0))
	    sqlr_new_error ("22011", "SR591", "Function LEFT: ending offset %ld crosses UTF-8 character in the middle", (long) to);
...
skip_any_of_these_func (char) skip_any_of_these_func (wchar_t)
...
bif_ltrim (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t skip_str = ((n_args > 1) ? bif_string_arg (qst, args, 1, "ltrim") : (caddr_t) " ");	/* If second arg is not present it's a space by default. */
  long len;
  long from;
  caddr_t res;
  dtp_t dtp1 = DV_TYPE_OF (str);
...
bif_rtrim (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t skip_str = ((n_args > 1) ? bif_string_arg (qst, args, 1, "rtrim") : (caddr_t) " ");	/* If second arg is not present it's a space by default. */
  long len;
  long to;
  caddr_t res;
  dtp_t dtp1 = DV_TYPE_OF (str);
...
bif_trim (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (IS_WIDE_STRING_DTP (dtp1))
    {
      from = (long) ((wchar_t *) (from_ptr =
	      (char *) wchar_t_skip_any_of_these ((wchar_t *) str, (wchar_t *) skip_str, len)) - ((wchar_t *) str));
      if ((len - from) > 1)
	{
	...
	{
	  /* There are at least two characters left at the right side. */
	  to = (long) (char_skip_from_right_any_of_these (from_ptr, skip_str, (len - from)) - str);
	}
      else
	{
	  to = len;
...
bif_replace (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  int sizeof_char =
	  ...
	  IS_WIDE_STRING_DTP (dtp_from) || IS_WIDE_STRING_DTP (dtp_to)) ? sizeof (wchar_t) : sizeof (char));
  ...
  searcher = ((sizeof (wchar_t) == sizeof_char) ? widememmem : ((1 == from_bytes) ? memmem1 : memmem));
 ...
 */
  ...
  res = dk_alloc_box (res_bytes + sizeof_char, (dtp_t) (sizeof_char == sizeof (wchar_t) ? DV_WIDE : DV_LONG_STRING));
  res_ptr = res;
  src_tail = src_str;
  n_changes = 0;
...
caddr_t
sprintf_inverse_ex (caddr_t * qst, caddr_t * err_ret, ccaddr_t str, ccaddr_t fmt, long hide_errors,
    unsigned char *expected_dtp_strg)
{
...
bif_strstr_imp (caddr_t * qst, state_slot_t ** args, int opcode, const char *func_name)
{
  ...
  if (sizeof_char == sizeof (wchar_t))
	...
	case BIF_STRSTR_BOOL_ANY:
	  inx =
	      (char *) virt_wmemmem ((const wchar_t *) str1, len1 / sizeof (wchar_t) - 1, (const wchar_t *) str2,
	      len2 / sizeof (wchar_t) - 1);
	  break;
...
bif_rdf_rng_min (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!DV_STRINGP (str_in) || box_length (str_in) - 1 < 10)
...
dtp_t
dtp_preciser (dtp_t broader, dtp_t dtp)
{
  switch (broader)
    {
    case DV_ARRAY_OF_POINTER:
...
void
bif_all_eq_type (state_slot_t ** args, long *dtp, long *prec, long *scale, collation_t ** collation)
{
  int inx;
  DO_BOX (state_slot_t *, arg, inx, args)
  {
	...
	*scale = arg->ssl_sqt.sqt_scale;
      ...
      {
	dtp_t ndtp = dtp_preciser (*dtp, arg->ssl_sqt.sqt_dtp);
	if (ndtp != *dtp)
	  {
	    *dtp = ndtp;
	    *prec = 0;
...
bif_type_t bt_all_eq = { (bif_type_func_t) bif_all_eq_type, 0, 0, 0 };
...
GENERAL_COMPARISON_FUNC (bif_equ, "equ", ==, DVC_MATCH) GENERAL_COMPARISON_FUNC (bif_neq, "neq", !=, DVC_MATCH)
...
bif_mod (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (0 == long2)
    {
      sqlr_new_error ("22012", "SR046", "Division by zero in mod(%ld,%ld)", long1, long2);
    }
...
bif_sign (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (dtp)
	...
	sqlr_new_error ("22023", "SR047",
	    "Function sign needs a numeric type as its argument, " "not an argument of type %s (%d)", dv_type_title (dtp), dtp);
      }
    }				/* switch */
  NO_CADDR_T;
}
...
bif_abs (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (dtp)
	...
	sqlr_new_error ("22023", "SR048",
	    "Function abs needs a numeric type as its argument, " "not an argument of type %s (%d)", dv_type_title (dtp), dtp);
      }
    }				/* switch */
  NO_CADDR_T;
}
...
GENERAL_DOUBLE_FUNC (bif_tan, "tan", tan (x)) GENERAL_DOUBLE_FUNC (bif_cot, "cot", (((double) 1.0) / tan (x)))
...
{\
  ...
  return ((isnull1 || isnull2) ? NEW_DB_NULL : box_double(OPERATION));\
    ...
    GENERAL_DOUBLE2_FUNC (bif_atan2, "atan2", atan2 (x, y)) GENERAL_DOUBLE2_FUNC (bif_power, "power", pow (x, y))
...
{\
  ...
  return(isnull ? NEW_DB_NULL : box_num(((long int)(OPERATION))));\
    ...
    GENERAL_DOUBLE_TO_INT_FUNC (bif_ceiling, "ceiling", ceil (x)) GENERAL_DOUBLE_TO_INT_FUNC (bif_floor, "floor", floor (x))
...
sqlbif_rnd (int32 * seed)
{
  ...
  if (!seed[0] || seed[0] == RNG_M)
	...
	log_debug ("rnd %s=%ld", seed == &rnd_seed ? "rnd_seed" : "rnd_seed_b", (long) seed[0]);
#endif
    }
...
bif_disconnect (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  DO_SET (dk_session_t *, ses, &users)
  {
	...
	if (cli &&
	    (!name || (cli->cli_user
		    && (DVC_MATCH == cmp_like (cli->cli_user->usr_name, name, NULL, 0, LIKE_ARG_CHAR, LIKE_ARG_CHAR)))))
	  {
	    ASSERT_IN_TXN;
...
bif_connection_set (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!box_outlives_qi (val))
    sqlr_new_error ("22023", "SR049", "Data type is not suitable for storage into a global variable (connection_set)");
...
bif_connection_vars_set (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (dtp != DV_DB_NULL && dtp != DV_ARRAY_OF_POINTER)
	...
	"connenction_vars_set expects a vector or null as argument " "not of type %s (%d)", dv_type_title (dtp), dtp);
  ...
  if (len % 2 != 0)
    {
	  ...
	  "not of length %d (of type %s (%d))", len, dv_type_title (dtp), dtp);
    }
  ...
  for (ix = 0; ix < len; ix += 2)
	...
	sqlr_new_error ("22023", "SR052",
	    "connenction_vars_set expects a string as name of connection variable not of type %s (%d)", dv_type_title (dtp), dtp);
      if (!box_outlives_qi (((caddr_t *) val)[ix + 1]))
	sqlr_new_error ("22023", "SR053", "Data type is not suitable for storage into a global variable (connection_set)");
    }
...
bif_split_and_decode (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t alt_seps = ((n_args > 2) ? bif_string_arg (qst, args, 2, me) : (caddr_t) DEFAULT_SEPARATORS);
  int alt_seps_len = ((n_args > 2) ? (box_length (alt_seps) - 1) : (sizeof (DEFAULT_SEPARATORS) - 1));
 ...
 */
	...
	switch_to_val = 0;
	if (is_at_top_split (ptr) || ((0 == reading_value) && (sep2 == *ptr) && (switch_to_val = 1)))
	  {
	    if (counting_only)	/* We have just counted the name and val len. */
	      {
		item = ((char *) dk_alloc_box (item_len + 1, DV_LONG_STRING));
		if (split_it)
		  {
	  ...
	  /* Any other character. */
	  {
		...
		if (esc_char == *(ptr + 1))
		...
		else
		  {
		    ...
		    int code, code2;
		    if (((code = hexno (*(ptr + 1))) >= 0) && ((code2 = hexno (*(ptr + 2))) >= 0))
		      {
			ptr += 3;
			code = (code * 16) + code2;
 ...
 */
int
find_index_to_vector (caddr_t item, caddr_t vec, int veclen, dtp_t vectype, int start, int skip_value, const char *calling_fun)
{
...
bif_get_keyword (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  boxlen = (is_string_type (vectype) ? box_length (arr) - 1 : box_length (arr));
  ...
  if (len % 2 != 0)
    {
	  ...
	  "not of length %d (of type %s (%d))", len, dv_type_title (vectype), vectype);
    }
...
bif_set_by_keywords_imp (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int is_tweak, const char *fname)
{
  ...
  QR_RESET_CTX
  {
	...
	if (!strcmp (opcode, "new"))
	  {
	    if (-1 != old_kwd_pos)
	      sqlr_new_error ("22024", "SR654",
		  "The function %s() gets opcode '%s' as %d-th argument but the specified keyword %.500s is already in the array at index %d",
		  fname, opcode, argctr + 1, kwd, old_kwd_pos);
	    goto op_do_extend;
	  }
	...
	if (!strcmp (opcode, "replace"))
	  {
	    if (-1 != old_kwd_pos)
	      goto op_do_replace;
	    sqlr_new_error ("22024", "SR654",
		"The function %s() gets opcode '%s' as %d-th argument but the specified keyword %.500s is not found in the array",
		fname, opcode, argctr + 1, kwd);
	  }
	...
	sqlr_new_error ("22024", "SR655", "The function %s() gets invalid opcode '%.500s' as %d-th argument", fname, opcode,
	    argctr + 1);
      op_do_replace:
  ...
  END_QR_RESET return changed_arr;
}
...
bif_position (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (0 == every_nth)
    {
      sqlr_new_error ("22003", "SR060", "%s: cannot check every 0th element of vector of type %s", me, dv_type_title (vectype));
    }
  else if ((every_nth > 1) && ((len % every_nth) != 0))
    {
      sqlr_new_error ("22023", "SR061",
	  "%s: expects a vector whose length is divisible by %ld, "
	  "not of length %d (of type %s (%d))", me, every_nth, len, dv_type_title (vectype), vectype);
    }
...
row_str_check (db_buf_t str)
{
  ...
  if (str_type != DV_LONG_CONT_STRING &&
      str_type != DV_SHORT_CONT_STRING && str_type != DV_LONG_STRING && str_type != DV_SHORT_STRING && str_type != DV_REFERENCE)
    {
...
caddr_t
row_str_column (caddr_t * qst, db_buf_t str, char *tb_name, char *col_name, int *exists)
{
#ifndef KEYCOMP
	...
	if (row_ret)
	  {
	    ...
	    if (cr_key->key_is_primary)
	      {
		*row_ret = itc_box_row ((ITC) ref_itc, ref_buf->bd_buffer);
#ifndef NDEBUG
		...
		dbg_row_deref_pos = ref_itc->itc_position;
	      ...
	      {
		it_cursor_t *main_itc = itc_create (NULL, ref_itc->itc_ltrx);
...
#ifdef USE_TLSF
  tlsf_all_in_use (fd ? fd : stderr, 0, nth);
#else
...
bif_mem_new_in_use (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  int nth = BOX_ELEMENTS (args) > 1 ? bif_long_arg (qst, args, 1, "mem_all_in_use") : 0;
#ifdef USE_TLSF
  tlsf_all_in_use (fd ? fd : stderr, 1, nth);
#else
...
{
#ifdef USE_TLSF
  return box_num (tlsf_get_total (NULL));
#else
#ifdef MALLOC_DEBUG
  ...
  for (n = 0; ((uint32) n) < BOX_ELEMENTS (args); n++)
    {
	  ...
	  sl->ssl_sqt.sqt_dtp == DV_BLOB_WIDE_HANDLE || sl->ssl_sqt.sqt_dtp == DV_BLOB_WIDE)
	{
	  nWides = 1;
	  break;
	}
...
num_check_prec (boxint val, int prec, char *title, caddr_t * err_ret)
{
  ...
  if (val >= prec_lower && val <= prec_upper)
    ...
    {
      caddr_t err = srv_make_new_error ("22023", "SR346",
...
box_cast (caddr_t * qst, caddr_t data, ST * dtp, dtp_t arg_dtp)
{
...
#ifdef DEBUG
	sqlr_new_error ("22023", (IS_BOX_POINTER (qst)
		&& (((query_instance_t *) qst)->qi_no_cast_error)) ? "sR066" : "SR066",
	    "Unsupported case in CONVERT (incomplete RDF box -> %s)", dv_type_title ((int) (dtp->type)));
#else
	sqlr_new_error ("22023", "SR066", "Unsupported case in CONVERT (incomplete RDF box -> %s)",
	    dv_type_title ((int) (dtp->type)));
#endif
      data = rb->rb_box;
  ...
  switch (dtp->type)
    ...
    case DV_SHORT_INT:
      goto do_long_int;
    case DV_SINGLE_FLOAT:
      goto do_single_float;
    case DV_DOUBLE_FLOAT:
      goto do_double_float;
    case DV_NUMERIC:
      goto do_numeric;
    case DV_DATETIME:
    case DV_DATE:
...
do_long_string:
  {
      ...
      case DV_BLOB_WIDE_HANDLE:
	{
	  ...
	  if (!qst)
	    sqlr_new_error ("22023", "SRUUU", "Can't convert data to varchar.");
	  if (bh->bh_ask_from_client)
	    sqlr_new_error ("22023", "SR065",
		"Can't convert SQL_DATA_AT_EXEC blob to varchar. " "Parameter may only be used in insert or update");
	  ret = blob_to_string (qi->qi_trx, data);
	  if (!DV_STRINGP (ret))
	    {
	      caddr_t ret1;
	      err = NULL;
	      ret1 = box_cast_to (qst, ret, DV_TYPE_OF (ret), DV_SHORT_STRING, NUMERIC_MAX_PRECISION, NUMERIC_MAX_SCALE, &err);
...
do_long_int:
  {
	...
	break;
      case DV_SINGLE_FLOAT:
	val = (boxint) unbox_float (data);
	break;
      case DV_DOUBLE_FLOAT:
	val = (boxint) unbox_double (data);
	break;
      case DV_STRING:
...
do_datetime:
  {
	...
	break;
      case DV_BIN:
	if (dt_validate (data))
	  sqlr_new_error ("22003", "SR351", "Invalid data supplied in VARBINARY -> DATETIME conversion");
	res = box_copy (data);
	...
	if (IS_WIDE_STRING_DTP (arg_dtp))
	  {
	    ...
	    if (utf8_len < 0)
	      sqlr_new_error ("22005", "IN014", "Invalid data supplied in NVARCHAR -> VARBINARY conversion");
	...
	if (DV_BLOB_HANDLE == arg_dtp || DV_BLOB_WIDE_HANDLE == arg_dtp)
	  {
	    ...
	    if (bh->bh_ask_from_client)
	      sqlr_new_error ("22023", "SR065",
		  "Can't convert SQL_DATA_AT_EXEC blob to varbinary. " "Parameter may only be used in insert or update");
	    ret = blob_to_string (qi->qi_trx, data);
...
do_wide:
  {
      ...
      case DV_UNAME:
	{
	  ...
	  if (((long) wide_len) < 0)
	    sqlr_new_error ("22005", "IN015", "Invalid data supplied in UNAME -> NVARCHAR conversion");
	  ...
	  if (wide_len != virt_mbsnrtowcs ((wchar_t *) ret, &utf8work, utf8_len, wide_len, &state))
	    {
	      dk_free_box (ret);
	      sqlr_new_error ("22005", "IN015", "Inconsistent UTF-8 data supplied in UNAME -> NVARCHAR conversion");
	    }
      ...
      case DV_BIN:
	{
	  ...
	  if (res)
	    ...
	    sqlr_new_error ("22005", "IN015", "Invalid data supplied in VARBINARY -> NVARCHAR conversion");
	  break;
	}
      case DV_LONG_INT:
      case DV_SHORT_INT:
...
#ifdef DEBUG
  sqlr_new_error ("22023", (IS_BOX_POINTER (qst)
	  && (((query_instance_t *) qst)->qi_no_cast_error)) ? "sR066" : "SR066", "Unsupported case in CONVERT (%s -> %s)",
      dv_type_title (arg_dtp), dv_type_title ((int) (dtp->type)));
#else
  sqlr_new_error ("22023", "SR066", "Unsupported case in CONVERT (%s -> %s)", dv_type_title (arg_dtp),
      dv_type_title ((int) (dtp->type)));
#endif
  NO_CADDR_T;
}
...
caddr_t
box_cast_to (caddr_t * qst, caddr_t data, dtp_t data_dtp, dtp_t to_dtp, ptrlong prec, unsigned char scale, caddr_t * err_ret)
{
  caddr_t tmp[6];
  caddr_t *ptmp;
...
bif_convert (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (BOX_ELEMENTS (args) > 2 && !(ST_P (dtp, DV_SHORT_STRING) || ST_P (dtp, DV_LONG_STRING)))
    sqlr_new_error ("22023", "SR067",
	"Collation specified in cast for non-string datatype %s", ARRAYP (dtp) ? dv_type_title ((int) (dtp->type)) : "unknown");
...
BIF_STUB (bif_stub_xquery_contains, "xquery_contains") BIF_STUB (bif_stub_xcontains, "xcontains") caddr_t
bif_blob_to_string (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (dtp == DV_SHORT_STRING || dtp == DV_LONG_STRING)
    ...
    {
      sqlr_new_error ("22023", "SR070", "blob_to_string requires a blob or string argument");
    }
  if (((blob_handle_t *) bh)->bh_ask_from_client)
    sqlr_new_error ("22023", "SR071", "Blob argument to blob_to_string must be a non-interactive blob");
...
bif_blob_to_string_output (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!IS_BLOB_HANDLE_DTP (dtp))
    {
      sqlr_new_error ("22023", "SR070", "blob_to_string_output requires a blob or string argument");
    }
  if (((blob_handle_t *) bh)->bh_ask_from_client)
    sqlr_new_error ("22023", "SR071", "Blob argument to blob_to_string_output must be a non-interactive blob");
...
static int
registry_name_is_protected (const caddr_t name)
{
...
bif_sequence_set_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int sec_check)
{
  ...
  if (mode == SET_IF_GREATER)
  ...
  else if (mode == SET_ALWAYS)
    {
      ...
      log_array = list (4, box_string ("__sequence_set (?, ?, ?)"), box_string (name), box_num (count), box_num (mode));
...
bif_get_user_id (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (what)
    {
    case 1:
      return box_num (qi->qi_u_id);
    case 2:
      return box_num (qi->qi_g_id);
    case 3:
      return box_num (qi->qi_client->cli_user->usr_id);
    case 4:
      return box_num (qi->qi_client->cli_user->usr_g_id);
    }
  return NULL;
}
...
bif_complete_proc_name (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (mode == DEFAULT_EXISTING)
    {
      char *name = NULL;
      name = sch_full_proc_name (isp_schema (qi->qi_space), proc_name, qi->qi_query->qr_qualifier, CLI_OWNER (qi->qi_client));
      if (name)
	qr = sch_proc_def (isp_schema (qi->qi_space), name);
      else
	{
	  name = sch_full_module_name (isp_schema (qi->qi_space), proc_name, qi->qi_query->qr_qualifier, CLI_OWNER (qi->qi_client));
	  if (name)
...
bif_name_part (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (deflt)
    {
      if (DV_TYPE_OF (deflt) != DV_DB_NULL && !DV_STRINGP (deflt))
...
print_object_to_new_string (caddr_t xx, const char *fun_name, caddr_t * err_ret, int flags)
{
  ...
  FAILED
  {
    ...
    *err_ret = srv_make_new_error ("22023", "SR479",
	"Cannot serialize the data of type %s (%u) in BIF %s", dv_type_title (tag), (unsigned) tag, fun_name);
  }
  END_WRITE_FAIL (out);
...
bif_deserialize (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  END_QR_RESET dk_free_box (tmp_xx);
  return res;
}
...
bif_composite_ref (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  FAILED
  {
    *err_ret = srv_make_new_error ("22003", "FT003", "zero based composite index out of range %d for length %d", inx, ctr);
    dk_free_tree (res);
    res = NULL;
  }
  END_READ_FAIL (pses);
...
icc_lock_t *
icc_lock_alloc (caddr_t name, client_connection_t * cli, query_instance_t * qi)
{
...
icc_lock_t *
icc_lock_from_hashtable (caddr_t name)
{
...
int
icc_lock_release (caddr_t name, client_connection_t * cli)
{
  int sem_leave = 0;
  icc_lock_t *hash_lock;
...
caddr_t
bif_icc_name_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, int name_to_set)
{
  query_instance_t *qi = (query_instance_t *) qst;
  ...
  if (name_to_set)
      ...
      if (NULL != at_sign)
	{
	  ...
	  if (strcmp (at_sign + 1, qi->qi_query->qr_proc_name))
		...
		arg, at_sign, (('\0' == proc_name[0]) ? " outside the function body" : "in function "), proc_name);
	}
    }
  return arg;
}
...
caddr_t
bif_icc_try_lock (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  query_instance_t *qi = (query_instance_t *) qst;
  client_connection_t *cli = qi->qi_client;
...
caddr_t
bif_icc_lock_at_commit (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  query_instance_t *qi = (query_instance_t *) qst;
  client_connection_t *cli = qi->qi_client;
...
caddr_t
bif_icc_unlock_now (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  query_instance_t *qi = (query_instance_t *) qst;
  client_connection_t *cli = qi->qi_client;
...
bif_proc_changed (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!is_trig)
    rdproc = sql_compile ("select coalesce (P_TEXT, blob_to_string (P_MORE)), P_OWNER, P_QUAL "
	"from DB.DBA.SYS_PROCEDURES where P_NAME = ?", cli, &err, SQLC_DEFAULT);
  else
    rdproc =
	sql_compile_static
	("select coalesce (T_TEXT, blob_to_string (T_MORE)), name_part (T_NAME, 1), T_SCH from DB.DBA.SYS_TRIGGERS where T_NAME = ? AND T_TABLE = ?",
	cli, &err, SQLC_DEFAULT);
  if (!err)
    {
      if (!is_trig)
	err = qr_rec_exec (rdproc, cli, &lc, qi, NULL, 1, ":0", name, QRP_STR);
      else
	err = qr_rec_exec (rdproc, cli, &lc, qi, NULL, 2, ":0", name, QRP_STR, ":1", tb_name, QRP_STR);
    }
  CLI_QUAL_ZERO (cli);
  if (!err && lc_next (lc))
    {
...
#if 1
      if (err && 0 != retry && cl_run_local_only == CL_RUN_CLUSTER && 0 == server_lock.sl_count
	  && strstr (((caddr_t *) err)[2], "RDFNI"))
	{
	  dk_free_tree (err);
	  err = NULL;
	  retry = 0;
...
bif_drop_proc (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (is_proc)
    {
      full_name = sch_full_proc_name (isp_schema (qi->qi_space), proc_name, cli_qual (qi->qi_client), CLI_OWNER (qi->qi_client));
      if (full_name)
	proc = sch_proc_def (isp_schema (qi->qi_space), full_name);
    }
  else
    {
      full_name = sch_full_module_name (isp_schema (qi->qi_space), proc_name, cli_qual (qi->qi_client), CLI_OWNER (qi->qi_client));
      if (full_name)
...
bif_proc_exists (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (proc && is_proc && QR_IS_MODULE_PROC (proc))
    sqlr_new_error ("42000", "SR313",
	"The procedure %s is part of module %s. Drop the module instead.", proc->qr_proc_name, proc->qr_module->qr_proc_name);
  return (box_dv_short_string (proc->qr_proc_name));
}
...
iso_string_to_code (char *i)
{
  switch (i[0])
    ...
    case 'U':
    case 'u':
      return (ISO_UNCOMMITTED);
    case 'C':
    case 'c':
      return (ISO_COMMITTED);
    case 'R':
    case 'r':
      return (ISO_REPEATABLE);
    case 'S':
    case 's':
      return (ISO_SERIALIZABLE);
    default:
      sqlr_new_error ("22023", "SR075", "Bad isolation. Must be uncommitted / committed / repeatable / serializable");
    }
  return -1;
}
...
bif_exec (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (n_args > 4)
      ...
      if (DV_ARRAY_OF_POINTER != options_dtp)
	{
	  if (DV_LONG_INT == options_dtp)
	    ...
	    sqlr_new_error ("22023", "SR599",
		"Argument #5 of exec() should be either integer (max no of rows) or array of options or NULL");
	}
      else
	{
  ...
  if (NULL != options)
	...
	{
	  shc =
	      shcompo_get_or_compile (&shcompo_vtable__qr, list (3, box_copy_tree (text), box_num (qi->qi_u_id),
		  box_num (qi->qi_g_id)), 0, qi, NULL, &err);
	  if (NULL == err)
	    {
...
qr_set:
  ...
  err = qr_exec (qi->qi_client, qr, qi, NULL, NULL, &lc, new_params, NULL, 1);
  bif_exec_done (k);
	  ...
	  while (lc_next (lc) && (max == 0 || (max > 0 && curr_row < max)))
	    {
	      ...
	      if (rs_needed)
		{
		  caddr_t *row = (caddr_t *) dk_alloc_box (n_cols * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
	      ...
	      if (curr_row >= MAX_BOX_ELEMENTS)
		{
		  ...
		  lc_free (lc);
		  res = bif_exec_error (qst, args,
		      srv_make_new_error ("22023", "SR078", "The result set is too long, must limit result for at most %lu rows",
			  (unsigned long) MAX_BOX_ELEMENTS), warnings, shc, qr);
		  goto done;
		}
	    }
	  if (rs_needed)
	    qst_set (qst, args[6], list_to_array (dk_set_nreverse (rlist)));
	  else
	    dk_free_tree (list_to_array (rlist));
	}
      err = lc->lc_error;
...
bif_exec_vec (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL != options)
	...
	{
	  shc =
	      shcompo_get_or_compile (&shcompo_vtable__qr, list (3, box_copy_tree (text), qi->qi_u_id, qi->qi_g_id), 0, qi, NULL,
	      &err);
	  if (NULL == err)
	    {
	      shcompo_recompile_if_needed (&shc);
...
bif_trx_disk_log_length (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (mode)
    {
    case 0:
      return box_num (lt->lt_log->dks_bytes_sent);
    case 1:
      if (txn_after_image_limit > 0)
	return box_num ((txn_after_image_limit - 10000L) - lt->lt_log->dks_bytes_sent);
      return box_num (2000000000);
    default:
      sqlr_new_error ("22023", "SR562", "Supported values of argument 1 of __trx_disk_log_length() are 0 and 1 but not %ld", mode);
    }
  return NULL;			/* never reached */
}
...
test_xid_encode_decode (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  fprintf (stderr, "decoded xid = %ld %ld %ld %s\n",
      (long) test_xid_2->formatID, (long) test_xid_2->gtrid_length, (long) test_xid_2->bqual_length, test_xid_2->data);
  ...
  test_xid_2 =
      (virtXID *)
      xid_bin_decode
      ("");
  fprintf (stderr, "decoded xid = %ld %ld %ld %s\n", (long) test_xid_2->formatID, (long) test_xid_2->gtrid_length,
      (long) test_xid_2->bqual_length, test_xid_2->data);
...
static caddr_t
v_bit_print (int ret, unsigned char *b)
{
  char tmp[1024];
  char *ptr = tmp;
  int inx;
...
static caddr_t
bif_bit_print (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_bit_or (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_bit_and (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_bit_not (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_bit_all_pos (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_all_bits_set (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_true (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_v_equal (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_bit_set (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_is_set (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  unsigned char *bitarr = (unsigned char *) bif_varchar_or_bin_arg (qst, args, 0, "bit_set");
  long bitn = bif_long_arg (qst, args, 1, "bit_set");
  int len = BITARR_LEN (bitarr);
...
static caddr_t
bif_bit_clear (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
static caddr_t
bif_bit_v_count (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
bif_rdf_floor_ceil_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int is_floor)
{
  ...
  switch (DV_TYPE_OF (arg))
    ...
    case DV_DOUBLE_FLOAT:
      {
	double x = unbox_double (arg);
	return box_double (is_floor ? floor (x) : ceil (x));
      }
    case DV_SINGLE_FLOAT:
      {
	float x = unbox_float (arg);
	return box_float (is_floor ? floor (x) : ceil (x));
      }
    case DV_NUMERIC:
      {
	numeric_t res;
    ...
    case DV_DB_NULL:
      return NEW_DB_NULL;
    default:
      sqlr_new_error ("22023", "SL002", "The SPARQL 1.1 function %.10s() needs a numeric value as an argument",
	  is_floor ? "floor" : "ceil");
      return NULL;
    }
}
...
bif_rdf_round_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (DV_TYPE_OF (arg))
    ...
    case DV_DOUBLE_FLOAT:
      {
	double x = unbox_double (arg);
	return box_double (((x - floor (x)) >= 0.5 ? ceil (x) : floor (x)));
      }
    case DV_SINGLE_FLOAT:
      {
	float x = unbox_float (arg);
	return box_float (((x - floor (x)) >= 0.5 ? ceil (x) : floor (x)));
      }
    case DV_NUMERIC:
      {
	numeric_t res;
	...
	if (x->n_neg ? ((tenths > 5) || ((5 == tenths) && (1 < x->n_scale))) : (tenths >= 5))
	  {
	    numeric_t temp;
	    numeric_t shifted;
	    /* construct +-0.5 */
...
bif_rdf_substr_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (DV_TYPE_OF (src))
	...
	if (startl < 1 || !str_n_chars || (BOX_ELEMENTS (args) >= 3 && (lenl < 1 || lenl > str_n_chars)))
	  goto bad_subrange;
	...
	if (startl < 1 || startl > str_n_chars || (BOX_ELEMENTS (args) >= 3 && (lenl < 1 || lenl > str_n_chars - start + 1)))
	  goto bad_subrange;
...
bad_subrange:
  ...
  sqlr_new_error ("22011", "SR026", "SPARQL substr: Bad string subrange: from=%ld%s.", (long) startl, b);
  return NEW_DB_NULL;
}
...
caddr_t
bif_rdf_strcontains_x_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, const char *fnname, const char *sparql_fnname,
    int op_flags)
{
...
{
  return bif_rdf_strcontains_x_impl (qst, err_ret, args, "rdf_strstarts_impl", "STRSTARTS",
      STRCONTAINS_AT_START | STRCONTAINS_RET_BOOL);
}
...
{
  return bif_rdf_strcontains_x_impl (qst, err_ret, args, "rdf_contains_impl", "CONTAINS",
      STRCONTAINS_INSIDE | STRCONTAINS_RET_BOOL);
}

caddr_t
bif_rdf_strafter_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  return bif_rdf_strcontains_x_impl (qst, err_ret, args, "rdf_strafter_impl", "STRAFTER",
      STRCONTAINS_INSIDE | STRCONTAINS_RET_AFTER);
}

caddr_t
bif_rdf_strbefore_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  return bif_rdf_strcontains_x_impl (qst, err_ret, args, "rdf_strbefore_impl", "STRBEFORE",
      STRCONTAINS_INSIDE | STRCONTAINS_RET_BEFORE);
}
...
{
  caddr_t res_strg = bif_concat (qst, err_ret, args);
  int n_args = BOX_ELEMENTS (args), inx;
...
#endif
    default:
      sqlr_new_error ("42001", "SR646", "The function %.100s() is not supported in the OpenSSL library used in this Virtuoso build",
	  fname);
    }
  res[res_len * 2] = '\0';
...
bif_rdf_valid_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (2 <= BOX_ELEMENTS (args))
	...
	return box_bool (0);
	  ...
	  || (uname_xmlschema_ns_uri_hash_date == dt_uname) || (uname_xmlschema_ns_uri_hash_dateTime == dt_uname)
	  ...
	  || (uname_xmlschema_ns_uri_hash_gYear == dt_uname) || (uname_xmlschema_ns_uri_hash_gYearMonth == dt_uname)
	  /*|| (uname_xmlschema_ns_uri_hash_hexBinary               == dt_uname) */
	  || (uname_xmlschema_ns_uri_hash_int == dt_uname) || (uname_xmlschema_ns_uri_hash_integer == dt_uname)
	  ...
	  || (uname_xmlschema_ns_uri_hash_positiveInteger == dt_uname) || (uname_xmlschema_ns_uri_hash_short == dt_uname)
	  ...
	  || (uname_xmlschema_ns_uri_hash_unsignedShort == dt_uname) || (uname_xmlschema_ns_uri_hash_yearMonthDuration == dt_uname))
	return box_bool (0);
    }
  return box_bool (1);
}
...
bif_sparql_init (void)
{
  ...
  bif_define_ex ("rdf_strbefore_impl", bif_rdf_strbefore_impl, BMD_RET_TYPE, &bt_any, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("rdf_strstarts_impl", bif_rdf_strstarts_impl, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("rdf_strends_impl", bif_rdf_strends_impl, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);
  ...
  bif_define_ex ("rdf_seconds_impl", bif_rdf_seconds_impl, BMD_RET_TYPE, &bt_double, BMD_DONE);
  bif_define_ex ("rdf_md5_impl", bif_rdf_MD5_impl, BMD_RET_TYPE, &bt_string, BMD_DONE);
  bif_define_ex ("rdf_sha1_impl", bif_rdf_SHA1_impl, BMD_RET_TYPE, &bt_string, BMD_DONE);
  bif_define_ex ("rdf_sha224_impl", bif_rdf_SHA224_impl, BMD_RET_TYPE, &bt_string, BMD_DONE);
  bif_define_ex ("rdf_sha256_impl", bif_rdf_SHA256_impl, BMD_RET_TYPE, &bt_string, BMD_DONE);
...
/* For debugging */
  ...
  bif_define_ex ("__qi_is_branch", bif_qi_is_branch, BMD_RET_TYPE, &bt_integer, BMD_DONE);
...
/* String manipulation. */
  ...
  bif_define_ex ("aref_or_default", bif_aref_or_default, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("aref_set_0", bif_aref_set_0, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("aset", bif_aset, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("aset_zap_arg", bif_aset_zap_arg, BMD_RET_TYPE, &bt_integer, BMD_DONE);
...
/* Finding occurrences of characters and substrings in strings: */
  ...
  bif_define_ex ("strrchr", bif_strrchr, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("strstr", bif_strstr, BMD_ALIAS, "strindex", BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("strcontains", bif_strcontains, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("starts_with", bif_starts_with, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("ends_with", bif_ends_with, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  ...
  bif_define_ex ("matches_like", bif_matches_like, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 3, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("__like_min", bif_like_min, BMD_RET_TYPE, &bt_string, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("__like_max", bif_like_max, BMD_RET_TYPE, &bt_string, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("__rdf_rng_min", bif_rdf_rng_min, BMD_RET_TYPE, &bt_string, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("fix_identifier_case", bif_fix_identifier_case, BMD_RET_TYPE, &bt_varchar, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT,
      1, BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("casemode_strcmp", bif_casemode_strcmp, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);

/* Conversion between cases: */
  bif_define_ex ("lcase", bif_lcase, BMD_ALIAS, "lower", BMD_RET_TYPE, &bt_string, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1,
      BMD_IS_PURE, BMD_DONE);
...
/* Type testing functions. */
  ...
  bif_define_ex ("__box_flags", bif_box_flags, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("__box_flags_set", bif_box_flags_set, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_DONE);
  bif_define_ex ("__box_flags_tweak", bif_box_flags_tweak, BMD_ALIAS, "__bft", BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("dv_to_sql_type", bif_dv_to_sql_type, BMD_ALIAS, "internal_to_sql_type", BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE, BMD_DONE);	/* for sqlext.c */
  ...
  bif_define_ex ("dv_type_title", bif_dv_type_title, BMD_ALIAS, "internal_type_name", BMD_RET_TYPE, &bt_varchar, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE, BMD_DONE);	/* needed by sqlext.c */
  ...
  bif_define_ex ("internal_type", bif_internal_type, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("isinteger", bif_isinteger, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isnumeric", bif_isnumeric, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isfinitenumeric", bif_isfinitenumeric, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("isfloat", bif_isfloat, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isdouble", bif_isdouble, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isnull", bif_isnull, BMD_RET_TYPE, &bt_integer_nn, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isnotnull", bif_isnotnull, BMD_VECTOR_IMPL, bif_isnotnull_vec, BMD_RET_TYPE, &bt_integer_nn, BMD_MIN_ARGCOUNT, 1,
      BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("isblob", bif_isblob_handle, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isentity", bif_isentity, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isstring", bif_isstring, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isstring_session", bif_isstring_session, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1,
      BMD_IS_PURE, BMD_DONE);
  bif_define_ex ("isbinary", bif_isbinary, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isarray", bif_isarray, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("isvector", bif_isvector, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  ...
  bif_define_ex ("__all_eq", bif_all_eq, BMD_RET_TYPE, &bt_all_eq, BMD_MIN_ARGCOUNT, 0, BMD_IS_PURE, BMD_DONE);
...
/* Bitwise: */
  ...
  bif_define_ex ("bit_xor", bif_bit_xor, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("bit_not", bif_bit_not, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 1, BMD_MAX_ARGCOUNT, 1, BMD_IS_PURE,
      BMD_DONE);
  bif_define_ex ("bit_shift", bif_bit_shift, BMD_RET_TYPE, &bt_integer, BMD_MIN_ARGCOUNT, 2, BMD_MAX_ARGCOUNT, 2, BMD_IS_PURE,
      BMD_DONE);

/* Miscellaneous: */
  bif_define_ex ("dbname", bif_dbname, BMD_RET_TYPE, &bt_varchar, BMD_DONE);	/* Standard system function ? */
  bif_define_ex ("get_user", bif_user, BMD_ALIAS, "username", BMD_RET_TYPE, &bt_varchar, BMD_NO_CLUSTER, BMD_DONE);
  bif_define_ex ("pwd_magic_calc", bif_pwd_magic_calc, BMD_RET_TYPE, &bt_varchar, BMD_DONE);
  bif_define_ex ("disconnect_user", bif_disconnect, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  ...
  bif_define_ex ("backup", bif_backup, BMD_NO_CLUSTER, BMD_DONE);
  bif_define_ex ("db_check", bif_check, BMD_NO_CLUSTER, BMD_DONE);
  ...
  bif_define_ex ("vector_zap_args", bif_vector_zap_args, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("get_keyword", bif_get_keyword, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("get_keyword_ucase", bif_get_keyword_ucase, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("set_by_keywords", bif_set_by_keywords, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("tweak_by_keywords", bif_tweak_by_keywords, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("position", bif_position, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("one_of_these", bif_one_of_these, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  uname_one_of_these = box_dv_uname_string (sqlp_box_id_upcase ("one_of_these"));
#if 0
  bif_define_ex ("row_table", bif_row_table, BMD_RET_TYPE, &bt_varchar, BMD_DONE);
  bif_define_ex ("row_column", bif_row_column, BMD_RET_TYPE, &bt_any, BMD_DONE);
  bif_define ("row_identity", bif_row_identity);
  bif_define ("row_deref", bif_row_deref);
#endif

#ifndef NDEBUG
  bif_define_ex ("dbg_row_deref_page", bif_dbg_row_deref_page, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("dbg_row_deref_pos", bif_dbg_row_deref_pos, BMD_RET_TYPE, &bt_integer, BMD_DONE);
#endif
  bif_define ("page_dump", bif_page_dump);
  bif_define_ex ("lisp_read", bif_lisp_read, BMD_RET_TYPE, &bt_any_box, BMD_DONE);

  bif_define_ex ("make_array", bif_make_array, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("lvector", bif_lvector, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("fvector", bif_fvector, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("dvector", bif_dvector, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  ...
  bif_define_ex ("sequence_next_bounded", bif_sequence_next_bounded, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("sequence_remove", bif_sequence_remove, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("__sequence_set", bif_sequence_set, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("sequence_get_all", bif_sequence_get_all, BMD_ALIAS, "get_all_sequences", BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  bif_define_ex ("\x01__sequence_set_no_check", bif_sequence_set_no_check, BMD_MIN_ARGCOUNT, 3, BMD_MAX_ARGCOUNT, 3,
  ...
  bif_define_ex ("set_qualifier", bif_set_qualifier, BMD_RET_TYPE, &bt_integer, BMD_NO_CLUSTER, BMD_DONE);
  ...
  bif_define_ex ("__any_grants_to_user", bif_any_grants_to_user, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define ("txn_error", bif_txn_error);
  bif_define ("__trx_no", bif_trx_no);
  bif_define_ex ("__commit", bif_commit, BMD_NO_CLUSTER, BMD_DONE);
  bif_define_ex ("__rollback", bif_rollback, BMD_NO_CLUSTER, BMD_DONE);
  ...
  bif_define_ex ("__row_count_exceed", bif_row_count_exceed, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  ...
  bif_define_ex ("__copy", bif_copy, BMD_RET_TYPE, &bt_copy, BMD_DONE);
  ...
  bif_define_ex ("vector_concat", bif_vector_concatenate, BMD_RET_TYPE, &bt_any_box, BMD_DONE);
  ...
  bif_define_ex ("rdtsc", bif_rdtsc, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("row_count", bif_row_count, BMD_RET_TYPE, &bt_integer, BMD_DONE);
  bif_define_ex ("set_row_count", bif_set_row_count, BMD_RET_TYPE, &bt_integer, BMD_DONE);
...
static const char *set_var_proc = "select BPEL.BPEL.set_var_to_dump (?, ?, ?, ?, ?, ?)";

caddr_t
bpel_set_var_by_dump (const char *my_name, const char *my_part,
    const char *my_query, const char *my_val, const char *my_vars, const char *my_xmlnss)
{
  ...
  err = qr_quick_exec (qr, cli, "", &lc, 6,
      ":0", my_name, QRP_STR,
      ":1", my_part, QRP_STR, ":2", my_query, QRP_STR, ":3", my_val, QRP_STR, ":4", my_vars, QRP_STR, ":5", my_xmlnss, QRP_STR);
  CHECKP ("3");
  if (err)
    goto fin;
  CHECKP ("4");
...
static const char *get_var_proc = "select BPEL.BPEL.get_var_from_dump (?, ?, ?, ?, ?)";
caddr_t
bpel_get_var_by_dump (const char *my_name, const char *my_part, const char *my_query, const char *my_vars, const char *my_xmlnss)
{
  client_connection_t *cli = client_connection_create ();
  query_t *qr = 0;
  caddr_t err = 0;
  ...
  err = qr_quick_exec (qr, cli, "", &lc, 5,
      ":0", my_name, QRP_STR, ":1", my_part, QRP_STR, ":2", my_query, QRP_STR, ":3", my_vars, QRP_STR, ":4", my_xmlnss, QRP_STR);
  CHECKP ("3");
  if (err)
    goto fin;
  CHECKP ("4");
