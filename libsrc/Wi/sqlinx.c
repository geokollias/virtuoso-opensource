...
{
  DO_SET (df_elt_t *, dfe, &list) dfe->dfe_is_placed = f;
  END_DO_SET ();
}
...
sqlo_after_test_placeable (df_elt_t * tb, dk_set_t path, ST * tree)
{
  ...
  if (ST_COLUMN (tree, COL_DOTTED))
    ...
    {
      int inx;
      DO_BOX (ST *, exp, inx, tree) if (!sqlo_after_test_placeable (tb, path, exp))
	return 0;
      END_DO_BOX;
      return 1;
    }
...
sqlo_inx_path_complete (df_elt_t * tb_dfe, dk_set_t path)
{
  ...
  DO_SET (dbe_column_t *, refd, &tb_dfe->_.table.ot->ot_table_refd_cols)
  {
	...
	goto found;
      if (ic->ic_inx_op)
	{
	  DO_SET (df_inx_op_t *, dio, &ic->ic_inx_op->dio_terms) if (dk_set_member (dio->dio_key->key_parts, (void *) refd))
	    goto found;
	  END_DO_SET ();
	}
    }
  ...
  if (first->ic_inx_op)
    {
      DO_SET (df_inx_op_t *, dio, &first->ic_inx_op->dio_terms) if (dio->dio_key && !dio->dio_key->key_no_pk_ref)
	return 0;
      END_DO_SET ();
    }
  return 1;
...
{
  DO_SET (index_choice_t *, ic, &path) if (dk_set_member (ic->ic_after_preds, (void *) pred))
    return 1;
  END_DO_SET ();
  return 0;
}
...
{
  DO_SET (index_choice_t *, ic, &path) if (dk_set_member (ic->ic_eliminated_after_preds, (void *) pred))
    return 1;
  END_DO_SET ();
  return 0;
}
...
sqlo_ip_leading_text (df_elt_t * tb_dfe, dbe_key_t * key, index_choice_t * ic, df_elt_t ** text_id_pred)
{
  ...
  DO_SET (dbe_column_t *, prev_col, &key->key_parts)
  {
    ...
    if (!dfe_is_eq_pred (eq_pred) || !pred_const_rhs (eq_pred))
      return 0;
  }
  END_DO_SET ();
  ...
  tree =
      t_listst (5, BOP_EQ, t_listst (3, COL_DOTTED, prefix, id_col->col_name), t_listst (3, COL_DOTTED, prefix, id_col->col_name),
      NULL, NULL);
...
{
  op_table_t *ot;
  DO_SET (op_table_t *, ot, &so->so_tables) if (!strcmp (ot->ot_new_prefix, pref))
    return ot;
  END_DO_SET ();
...
{
  dk_set_t res = NULL;
  DO_SET (df_elt_t *, pred, &preds) if (dk_set_member (key->key_parts, (void *) pred->_.bin.left->_.col.col))
...
sqlo_rdf_dt_lang_ck (char *r_prefix, char *dt_col_name, df_elt_t * lower, df_elt_t * upper)
{
  ...
  return t_listst (5, BOP_EQ, t_listst (3, COL_DOTTED, r_prefix, t_box_string (dt_col_name)), t_listst (3, CALL_STMT,
	  t_sqlp_box_id_upcase ("rdf_box_dt_and_lang"), t_listst (1, ro)), NULL, NULL);
}
...
sqlo_rdf_string_range (df_elt_t * tb_dfe, index_choice_t * ic)
{
  ...
  if (lower)
    t_set_push (&r_tb_dfe->_.table.col_preds, (void *) sqlo_df (so, t_listst (5, lower->_.bin.op, r_range_col_dfe->dfe_tree,
		sqlo_rdf_string (lower->_.bin.right->dfe_tree, 1), NULL, NULL)));
  if (upper)
    t_set_push (&r_tb_dfe->_.table.col_preds, (void *) sqlo_df (so, t_listst (5, upper->_.bin.op, r_range_col_dfe->dfe_tree,
		sqlo_rdf_string (upper->_.bin.right->dfe_tree, 0), NULL, NULL)));
  ...
  DO_SET (df_elt_t *, pred, &r_tb_dfe->_.table.col_preds) sqlo_place_exp (so, tb_dfe, pred->_.bin.right);
  END_DO_SET ();
...
sqlo_ip_has_s_or_o_range (dk_set_t path)
{
  DO_SET (index_choice_t *, ic, &path)
  {
    if (ic->ic_inx_op || ic->ic_o_range || ic->ic_text_pred)
      return 1;
    DO_SET (dbe_column_t *, part, &ic->ic_key->key_parts) if (!stricmp (part->col_name, "S"))
      return 1;
    END_DO_SET ();
  }
  END_DO_SET ();
...
sqlo_rdf_o_range (df_elt_t * tb_dfe, dbe_key_t * key, dk_set_t path, index_choice_t * ic)
{
  ...
  sqlo_t *so = tb_dfe->dfe_sqlo;
  if (sqlo_ip_has_s_or_o_range (path) || sqlo_key_has_s_before_o (key))
    return;
  DO_SET (df_elt_t *, pred, &so->so_after_preds)
  {
    df_elt_t *o_col_dfe, *exp_dfe;
    int op;
    if (dfe_is_o_ro2sq_range (pred, tb_dfe, &o_col_dfe, &exp_dfe, &op) && sqlo_eqs_leading (tb_dfe, key, o_col_dfe->_.col.col))
      {
	ST *tree =
	    t_listst (5, (ptrlong) op, t_listst (3, COL_DOTTED, tb_dfe->_.table.ot->ot_new_prefix, o_col_dfe->_.col.col->col_name),
	    exp_dfe->dfe_tree, NULL, NULL);
...
sqlo_key_add_pk_eqs (df_elt_t * tb_dfe, dbe_key_t * key, index_choice_t * ic, dk_set_t * col_preds_save)
{
  ...
  DO_SET (dbe_column_t *, part, &key->key_parts)
  {
	...
	ST *col_tree;
	ST *tree = t_listst (5, BOP_EQ, col_tree =
	    t_listst (3, COL_DOTTED, pref, part->col_name), t_listst (3, COL_DOTTED, pref, part->col_name), NULL, NULL);
	...
	DO_SET (df_elt_t *, range_pred, &old_col_preds) if (range_pred->_.bin.left == col_dfe)
...
sqlo_choose_index_path (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * col_preds_ret, dk_set_t * after_preds_ret)
{
  ...
  if (!so->so_best_index_path)
    sqlc_new_error (so->so_sc->sc_cc, "42000", "NOINP",
	"Table options specify an non-existent index or index path cannot be constructed");
...
table_source_input_rdf_range (table_source_t * ts, caddr_t * inst, caddr_t * state)
{
  ...
  if (DV_STRING == dtp || (DV_RDF == dtp && DV_STRINGP (((rdf_box_t *) rng_val)->rb_box)))
      ...
      if (!alt->ts_alternate_inited)
	{
	  ...
	  if (!next)
	    GPF_T1 ("alt ts must always have a next");
