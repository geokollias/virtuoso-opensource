...
int
http_cli_error (http_cli_ctx * ctx, caddr_t errcode, caddr_t errmsg)
{
...
http_cli_resp_evt_dispatch (http_cli_ctx * ctx, int http_resp)
{
  ...
  DO_SET (http_resp_evt_q_t *, evt_q, &ctx->hcctx_resp_evts)
  {
	...
	{
	  if ((ctx->hcctx_resp_evt_ret = (fm->fn) (ctx, fm->pm, fm->rt, fm->er)) == HC_RET_ERR_ABORT)
	    break;
	}
	END_DO_SET ();
	break;
...
#else
  if (ctx->hcctx_http_out && (!ctx->hcctx_keep_alive || ctx->hcctx_err || !SESSTAT_ISSET (ctx->hcctx_http_out->dks_session, SST_OK)
#ifdef _SSL
	  || ctx->hcctx_ssl_ctx
#endif
...
HC_RET
http_cli_std_auth_destructor (http_cli_ctx * ctx, caddr_t parm, caddr_t ret_val, caddr_t err)
{
  if (!ctx)
    return (HC_RET_ERR_ABORT);
...
void
http_cli_push_hook (http_cli_ctx * ctx, int hook, http_cli_handler_frame_t * handler)
{
...
http_cli_make_handler_frame (http_cli_handler_fn fn, caddr_t pm, caddr_t ret, caddr_t err_ret)
{
...
void
http_cli_push_resp_evt (http_cli_ctx * ctx, int http_resp, http_cli_handler_frame_t * evt)
{
...
http_cli_handle_socks_conn_post (http_cli_ctx * ctx, caddr_t parm, caddr_t ret_val, caddr_t err)
{
  ...
  if (ctx->hcctx_proxy.hcp_socks_ver == 4)
    ...
    {
      if (HC_RET_OK != http_cli_negotiate_socks5 (ctx->hcctx_http_out, ctx->hcctx_host, ctx->hcctx_proxy.hcp_user,
	      ctx->hcctx_proxy.hcp_pass, ctx->hcctx_proxy.hcp_resolve, &err_ret))
	return http_cli_error (ctx, "HTCLI", err_ret);
    }
...
http_cli_send_req (http_cli_ctx * ctx)
{
  ...
  CATCH_WRITE_FAIL (ctx->hcctx_http_out)
  {
    ...
    snprintf (req_tmp, sizeof (req_tmp), " HTTP/%d.%d\r\n", ctx->hcctx_http_maj, ctx->hcctx_http_min);
...
{
  if (F_ISSET (ctx, HC_F_REPLY_READ))
    return (HC_RET_OK);
...
{
  if (!strnicmp ("Transfer-Encoding:", hdr, 18) && nc_strstr ((unsigned char *) hdr, (unsigned char *) "chunked"))
    {
...
http_cli_read_resp_hdrs (http_cli_ctx * ctx)
{
  ...
  *resp_hdr_tmp = 0;
  ...
  CATCH_READ_FAIL (ctx->hcctx_http_out)
  {
    do
      {
	num_chars = dks_read_line (ctx->hcctx_http_out, read_buf, sizeof (read_buf));

	if (resp_hdr_tmp_fill != 0 && (*read_buf != ' ' && *read_buf != '\t'))	/* read buffer not empty and not continuation */
	  {
	...
	strncat_size_ck (resp_hdr_tmp + resp_hdr_tmp_fill, read_buf, num_chars, (int) (sizeof (resp_hdr_tmp) - resp_hdr_tmp_fill));	/* concat buffer and go on */
...
http_cli_read_resp_body (http_cli_ctx * ctx)
{
  ...
  CATCH_READ_FAIL (ctx->hcctx_http_out)
  {
	...
	dk_free_tree (ctx->hcctx_resp_body);
	ctx->hcctx_resp_body =
	    http_read_chunked_content (ctx->hcctx_http_out, &ctx->hcctx_err, ctx->hcctx_url,
	    1 /* allow string session to be returned */ );
	...
	if (ctx->hcctx_resp_content_is_strses)
	  ...
	  {
	    ctx->hcctx_resp_body = dk_alloc_box (ctx->hcctx_resp_content_length + 1, DV_SHORT_STRING);

	    ctx->hcctx_resp_body[ctx->hcctx_resp_content_length] = '\0';
	    session_buffered_read (ctx->hcctx_http_out, ctx->hcctx_resp_body, ctx->hcctx_resp_content_length);
	  }
      }
  }
  FAILED
...
STANDARD_HANDLER (http_cli_handle_timeout,
    "HTCLI", "Timeout in HTTP Client") STANDARD_HANDLER (http_cli_handle_conn_err, "HTCLI", "Connection Error in HTTP Client")
     static const char
	 __tohex[] = "0123456789abcdef";

     void
     http_cli_calc_md5 (caddr_t str, caddr_t digest_buf, int len)
{
  MD5_CTX ctx;
...
http_cli_calc_auth_digest (http_cli_ctx * ctx, caddr_t _p, caddr_t _r, caddr_t _e)
{
  ...
  if (ctx->hcctx_algorithm == HA_ALGORITHM_MD5_SESS)
    {
      len = box_length (ctx->hcctx_user) + box_length (ctx->hcctx_realm) + box_length (ctx->hcctx_pass) + 3;
      ...
      if (len >= sizeof (tmp_buf))
	{
      ...
      len = sizeof (digest_buf) + box_length (ctx->hcctx_nonce) + box_length (ctx->hcctx_cnonce);
  ...
  if (!ctx->hcctx_qop || (ctx->hcctx_qop && stricmp (ctx->hcctx_qop, "auth-int")))
    {
      if ((strlen (http_cli_get_method_string (ctx)) + box_length (ctx->hcctx_uri) + 2) > sizeof (tmp_buf))
	{
	  ctx->hcctx_err = srv_make_new_error ("42000", "HC007", "Buffer overflow generating digest (A2)");
	  return (HC_RET_ERR_ABORT);
	}
      snprintf (tmp_buf, sizeof (tmp_buf), "%s:%s", http_cli_get_method_string (ctx), ctx->hcctx_uri);
    }
  else
    {
      if ((strlen (http_cli_get_method_string (ctx)) + box_length (ctx->hcctx_uri) + sizeof (digest_buf) + 3) > sizeof (tmp_buf))
	{
	  ctx->hcctx_err = srv_make_new_error ("42000", "HC008", "Buffer overflow generating digest (A2)");
	  return (HC_RET_ERR_ABORT);
	}
      http_cli_calc_md5 ((char *) ctx->hcctx_req_body, digest_buf, box_length (ctx->hcctx_req_body));
      snprintf (tmp_buf, sizeof (tmp_buf), "%s:%s:%s", http_cli_get_method_string (ctx), ctx->hcctx_uri, digest_buf);
    }

  http_cli_calc_md5 (tmp_buf, A2, (int) strlen (tmp_buf));
  if (!ctx->hcctx_qop)
    {
      if ((sizeof (A1) + box_length (ctx->hcctx_nonce) + sizeof (A2) + 3) > sizeof (tmp_buf))
	{
	  ctx->hcctx_err = srv_make_new_error ("42000", "HC009", "Buffer overflow generating digest");
	  return (HC_RET_ERR_ABORT);
	}
  ...
  else
    {
      ...
      if ((box_length (ctx->hcctx_nonce) +
	      box_length (ctx->hcctx_cnonce) + box_length (ctx->hcctx_qop) + 64 + 8 + 6) > sizeof (tmp_buf))
	{
	  ctx->hcctx_err = srv_make_new_error ("42000", "HC010", "Buffer overflow generating digest");
	  return (HC_RET_ERR_ABORT);
	}
      snprintf (tmp_buf, sizeof (tmp_buf),
	  "%s:%s:%08x:%s:%s:%s", A1, ctx->hcctx_nonce, ctx->hcctx_nc, ctx->hcctx_cnonce, ctx->hcctx_qop, A2);
    }
  ...
  if (ctx->hcctx_algorithm)
    {
      ...
      SES_PRINT (ctx->hcctx_prv_req_hdrs, http_cli_algorithm_string (ctx->hcctx_algorithm));
      SES_PRINT (ctx->hcctx_prv_req_hdrs, "\"");
    }
  SES_PRINT (ctx->hcctx_prv_req_hdrs, "\r\n");
...
http_cli_calc_auth_basic (http_cli_ctx * ctx, caddr_t _p, caddr_t _r, caddr_t _e)
{
  ...
  if ((len + 1) > sizeof (tmp_buf))
    {
      ctx->hcctx_err = srv_make_new_error ("42000", "HC011", "Userid and Password combination too long");
      return (HC_RET_ERR_ABORT);
    }
...
http_cli_std_handle_auth (http_cli_ctx * ctx, caddr_t parm, caddr_t ret_val, caddr_t err)
{
  ...
  if (!ctx->hcctx_user)
    {
      ctx->hcctx_err = srv_make_new_error ("42000", "HC014", "Authorization required and no credentials available");
      return (HC_RET_ERR_ABORT);
    }
  ...
  switch (ctx->hcctx_auth_type)
    ...
    case 1:
      {
	handler = http_cli_make_handler_frame (http_cli_calc_auth_digest, NULL, NULL, NULL);
	break;
      }
    case 2:
      {
	handler = http_cli_make_handler_frame (http_cli_calc_auth_basic, NULL, NULL, NULL);
	break;
      }
    default:
      {
	ctx->hcctx_err = srv_make_new_error ("42000", "HC015", "Cannot understand authorization challenge");
	return (HC_RET_ERR_ABORT);
      }
    }
...
http_cli_send_request_1 (http_cli_ctx * ctx, int connect)
{
  ...
  while (F_ISSET (ctx, HC_F_RETRY) && !ctx->hcctx_err && ctx->hcctx_retry_count++ <= ctx->hcctx_retry_max);
...
http_cli_init_std_auth (http_cli_ctx * ctx, caddr_t user, caddr_t pass)
{
  ctx->hcctx_user = user;
  ctx->hcctx_pass = pass;
  http_cli_push_resp_evt (ctx, 401, http_cli_make_handler_frame (http_cli_std_handle_auth, NULL, NULL, NULL));
  return (HC_RET_OK);
}
    ...
    int ret_arg_index, int follow_redirects)
{
  ...
  if (body && dtp != DV_DB_NULL)
      ...
      else
	{
      ...
      if (meth == HC_METHOD_POST && (!http_hdr || !nc_strstr ((unsigned char *) http_hdr, (unsigned char *) "Content-Type:")))
...
bif_http_client (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  return bif_http_client_impl (qst, err_ret, args, me, url, uid, pwd, method, http_hdr, body, cert, pk_pass, time_out,
      time_out_is_null, proxy, ca_certs, insecure, 8, follow_redirects);
}
...
bif_http_get (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  return bif_http_client_impl (qst, err_ret, args, me, uri, NULL, NULL, method, header, body, NULL, NULL, to, to_is_null, proxy,
      NULL, 0, 1, follow_redirects);
}
