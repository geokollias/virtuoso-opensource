...
void eql_stmt_comp (comp_context_t * cc, caddr_t stmt, data_source_t ** head, data_source_t ** tail);

query_t *eql_compile_2 (const char *string, client_connection_t * cli, caddr_t * err, int mode);
...
state_slot_t *ssl_new_column (comp_context_t * cc, const char *cr_name, dbe_column_t * col);

state_slot_t *ssl_new_inst_variable (comp_context_t * cc, const char *name, dtp_t dtp);
...
int cc_new_sets_slot (comp_context_t * cc);
...
void ks_add_key_cols (comp_context_t * cc, key_source_t * ks, dbe_key_t * key, char *cr_name);
...
search_spec_t **ks_col_specs (key_source_t * ks, dbe_column_t * col, int rm_from_row_specs);
