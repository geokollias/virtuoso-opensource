...
void
ssg_sdprin_literal (spar_sqlgen_t * ssg, SPART * tree)
{
...
void
ssg_sdprin_qname (spar_sqlgen_t * ssg, SPART * tree)
{
  caddr_t str;
...
void
ssg_sdprint_tree_list (spar_sqlgen_t * ssg, SPART ** trees, char delim)
{
  int ctr;
  DO_BOX_FAST (SPART *, tree, ctr, trees)
  {
...
ssg_sd_opname (sparp_t * sparp, ptrlong opname, int is_op)
{
  ...
  if (is_op)
	...
	spar_internal_error (sparp, "special assignment can not be rendered in SPARQL text");
      case SPAR_BOP_EQNAMES:
	spar_internal_error (sparp, "equivalence of names can not be rendered in SPARQL text");
      case BOP_EQ:
	return "=";
      case BOP_NEQ:
	return "!=";
...
ssg_assert_sd_flags (spar_sqlgen_t * ssg, int sd_needed_flags, const char *feature_name)
{
  ...
  spar_error (ssg->ssg_sparp, "%.100s does not support %.300s so SPARQL query can not be composed%.300s",
      ssg->ssg_sd_service_naming, feature_name, msg_tail);
}
...
int
ssg_filter_is_default_graph_condition (SPART * filt)
{
      ...
      else
	{
	  switch (v->_.var.tr_idx)
	    ...
	    case SPART_TRIPLE_PREDICATE_IDX:
	      ...
	      break;
	    case SPART_TRIPLE_SUBJECT_IDX:
	    case SPART_TRIPLE_GRAPH_IDX:
  ...
  if ((SPART_VARR_FIXED & eq->e_rvr.rvrRestrictions) && !(SPART_VARR_FIXED & mixed_field_restr))
    {
      ...
      ssg_puts (" = ");
      if ((NULL != eq->e_rvr.rvrFixedOrigText) && ((DV_DOUBLE_FLOAT == val_dtp) || (DV_SINGLE_FLOAT == val_dtp)
	      || (DV_NUMERIC == val_dtp)))
...
ssg_sdprint_option_list (spar_sqlgen_t * ssg, SPART ** options)
{
  ...
  DO_BOX_FAST_STEP2 (ptrlong, opt_id, SPART *, opt_val, option_ctr, options)
  {
    switch (opt_id)
      {
      case LIMIT_L:
	continue;		/* It's only a hint, not a requirement, and it's put sometimes via other parts of codegen, but not always */
      case SPAR_SERVICE_INV:
	continue;		/* It's printed via dedicated code */
      }
	...
	break;
      case T_STEP_L:
	ssg_puts (" T_STEP (");
	ssg_sdprint_tree (ssg, opt_val->_.alias.arg);
	ssg_puts (") AS ");
	ssg_sdprin_local_varname (ssg, opt_val->_.alias.aname);
	break;
      case T_IN_L:
      case T_OUT_L:
	if (NULL != old_param_t_inouts)
	  spar_error (ssg->ssg_sparp,
	      "Unsupported combination of transitive subqueries (transitive subquery in options of other transitive subquery?)");
  ...
  switch (tree_type)
	...
	if (!strncmp (fname, "SPECIAL::bif:", 13))
	  {
	    ...
	    if (ECM_MEM_NOT_FOUND != ecm_find_name (fname + 13, sparql11aggregates, sizeof (sparql11aggregates) / sizeof (char *),
		    sizeof (char *)))
	      {
	...
	switch (tree->_.gp.subtype)
	  ...
	  case VALUES_L:
		...
		ssg_sdprint_tree (ssg, binv->_.binv.vars[colctr]);
	      ssg_puts (") {");
	      for (rowctr = 0; rowctr < rowcount; rowctr++)
		{
		  ...
		  ssg->ssg_indent++;
		  for (colctr = 0; colctr < colcount; colctr++)
		    ssg_sdprint_tree (ssg, binv->_.binv.data_rows[rowctr][colctr]);
		  ssg_puts (" )");
		  ssg->ssg_indent--;
		}
	      ssg_puts (" }");
	...
	if ((ASK_L != tree->_.req_top.subtype) && (0 == BOX_ELEMENTS_0 (tree->_.req_top.retvals)))
	  {
	    ...
	    if (SSG_SD_BI_OR_SPARQL11_DRAFT & ssg->ssg_sd_flags)
	      ssg_sdprint_tree (ssg, spartlist (ssg->ssg_sparp, 6, SPAR_ALIAS, (ptrlong) 1, stub_varname, SSG_VALMODE_AUTO,
		      (ptrlong) 0, (ptrlong) 0));
	    else
	      {
		ssg_putchar (' ');
		ssg_sdprin_varname (ssg, stub_varname);
	      }
	  }
	...
	if (ASK_L != tree->_.req_top.subtype)
	  {
		...
		&& ((SELECT_L == tree->_.req_top.subtype) || (DISTINCT_L == tree->_.req_top.subtype) || (NULL != lim)
		    || (NULL != ofs)))
	      {
		ssg_newline (0);
		ssg_puts ("ORDER BY ");
	...
	if (NULL == tree->_.graph.iri)
	  spar_error (ssg->ssg_sparp,
	      "%.100s can be invoked only with constant graphs in FROM... clauses so SPARQL query can not be composed",
	...
	switch (tree->_.graph.subtype)
	  ...
	  case SPART_GRAPH_NOT_NAMED:
	    ssg_puts (" NOT FROM NAMED ");
	    break;
	  default:
	    spar_internal_error (ssg->ssg_sparp, "Bad tree->_.graph.subtype");
	    break;
	  }
	...
	switch (tree->_.ppath.subtype)
	  ...
	  case 0:
		...
		if (0 < ctr)
		  ssg_puts (" |");
		if ((count - tree->_.ppath.num_of_invs) <= ctr)
		  ssg_puts (" ^");
	  ...
	  case '*':
	    {
	      ...
	      if ((0 == minrepeat) && (-1 == maxrepeat))
		ssg_putchar ('*');
	      else if ((1 == minrepeat) && (-1 == maxrepeat))
		ssg_putchar ('+');
	      else if ((0 == minrepeat) && (1 == maxrepeat))
		ssg_putchar ('?');
	      else
		spar_error (ssg->ssg_sparp, "%.100s can not handle property path with {m,n} modifier in the loop",
		    ssg->ssg_sd_service_naming);
	      return;
	    }
	  }
