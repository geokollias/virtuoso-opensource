...
{
  dk_set_t sub = (mode == RI_SUBCLASS || mode == RI_SUBPROPERTY) ? rs->rs_sub : rs->rs_super;
  if (sub)
    {
      is_eq_ret = 0;
      return sub;
...
rit_down (ri_iterator_t * rit)
{
  ...
  if (!sub)
    return 0;
  if (ris && ris->ris_position && !ris->ris_position->next && (ris->ris_is_equiv || !rit->rit_value->rs_equiv))
    {
...
ri_list (rdf_inf_pre_node_t * ri, caddr_t iri, rdf_sub_t ** sub_ret)
{
  ...
  *sub_ret = sub;
  if (sub)
	...
	{
	case RI_SUBCLASS:
	  return sub->rs_sub ? sub->rs_sub : sub->rs_equiv;
	case RI_SUPERCLASS:
	  return sub->rs_super;
	case RI_SUBPROPERTY:
	  return sub->rs_sub ? sub->rs_sub : sub->rs_equiv;
	case RI_SUPERPROPERTY:
	  return sub->rs_super;
	}
    }
  return NULL;
...
char *sas_tn_text =
    "select O from DB.DBA.RDF_QUAD where S = :0 and P = rdf_sas_iri () and G in (:1) and isiri_id (O) union all select S from DB.DBA.RDF_QUAD where O = :0 and P = rdf_sas_iri () and G in (:1) option (quietcast, array)";
char *sas_tn_no_graph_text =
    "select O from DB.DBA.RDF_QUAD where S = :0 and P = rdf_sas_iri () union all select S from DB.DBA.RDF_QUAD where O = :0 and P = rdf_sas_iri () option (quietcast, array)";
char *tn_ifp_text =
...
char *tn_ifp_no_graph_text =
    ...
    " union all select syn.s from DB.DBA.RDF_QUAD org table option (index rdf_quad), DB.DBA.RDF_QUAD syn "
...
char *tn_ifp_dist_text =
    " select syn.s from DB.DBA.RDF_QUAD org table option (index rdf_quad), DB.DBA.RDF_QUAD syn "
    " where org.P in (rdf_inf_ifp_list (:1)) and syn.P = org.P and org.s = :0 and isiri_id (:0) and syn.O = org.O and org.G in (:2) and syn.G in (:2) and not rdf_inf_ifp_is_excluded (:1, org.P, org.O) "
    " union all select rsyn.S from DB.DBA.RDF_QUAD rorg, DB.DBA.RDF_QUAD rsyn "
    " where rorg.P in (rdf_inf_ifp_rel_list (:1)) and rsyn.P in (rdf_inf_ifp_rel_list (:1, rorg.P)) and rorg.S = :0 and isiri_id (:0) and rsyn.O = rorg.O and rorg.G in (:2) and rsyn.G in (:2) and not rdf_inf_ifp_is_excluded (:1, rorg.P, rorg.O) "
    " option (any order)";
char *tn_ifp_dist_no_graph_text =
    " select syn.s from DB.DBA.RDF_QUAD org table option (index rdf_quad), DB.DBA.RDF_QUAD syn where org.p in (rdf_inf_ifp_list (:1)) and syn.p = org.p and org.s = :0 and isiri_id (:0) and syn.o = org.o and not rdf_inf_ifp_is_excluded (:1, org.P, org.O) "
    " union all select rsyn.s from DB.DBA.RDF_QUAD rorg, DB.DBA.RDF_QUAD rsyn "
    " where rorg.P in (rdf_inf_ifp_rel_list (:1)) and rsyn.P in (rdf_inf_ifp_rel_list (:1, rorg.P)) and rorg.S = :0 and isiri_id (:0) and rsyn.o = rorg.O and not rdf_inf_ifp_is_excluded (:1, rorg.P, rorg.O) "
    " option (any order)";
...
{
  if (sas_tn_qr == tn->tn_prepared_step)
    return sas_tn_ht;
  if (sas_tn_no_graph_qr == tn->tn_prepared_step)
    return sas_tn_no_graph_ht;
  if (tn_ifp_qr == tn->tn_prepared_step)
    return tn_ifp_ht;
  if (tn_ifp_no_graph_qr == tn->tn_prepared_step)
    return tn_ifp_no_graph_ht;
  return NULL;
}
...
ri_same_as_iri (rdf_inf_pre_node_t * ri, query_instance_t * qi, caddr_t iri, query_t * qr)
{
  ...
  DO_BOX (state_slot_t *, g_ssl, ginx, ri->ri_sas_g)
  {
	...
	":0", box_copy (g), QRP_RAW, ":1", box_copy (iri), QRP_RAW, ":2", box_copy (same_as_iri), QRP_RAW);
    if (err)
      sqlr_resignal (err);
    while (lc_next (lc))
      {
...
void
ri_same_as_input (rdf_inf_pre_node_t * ri, caddr_t * inst, caddr_t * volatile state)
{
  caddr_t start, next;
  ...
  for (; nth_set < n_sets; nth_set++)
      ...
      if (!nth_val)
	{
	  ...
	  if (ri->ri_given)
	    ...
	    {
	      if ((RI_SUPERCLASS == ri->ri_mode || RI_SUBCLASS == ri->ri_mode) && (!ri->ri_p
		      || box_equal (rdfs_type, qst_get (inst, ri->ri_p))))
		{
...
void
rdf_inf_pre_input (rdf_inf_pre_node_t * ri, caddr_t * inst, caddr_t * volatile state)
{
  ri_iterator_t *rit;
  rdf_sub_t *sub;
  dk_set_t list;
...
bif_ctx_arg (caddr_t * qst, state_slot_t ** args, int nth, char *name, int create)
{
  ...
  if (!place && !create)
    sqlr_new_error ("42000", "RDFI.", "No RDF inference rule set '%.200s' specified as argument #%d of %.200s()", ctx_name, nth,
	name);
  if (!place)
    {
...
bif_rdf_inf_dump (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  while (hit_next (&hiter, (char **) (&key_ptr), (char **) (&rsub_ptr)))
    {
      ...
      {
	dk_set_push (&res_triples, list (3, box_copy_tree (sub_rs->rs_iri),
		box_dv_uname_string ("http://www.w3.org/2000/01/rdf-schema#subClassOf"), box_copy_tree (key_ptr[0])));
      }
      END_DO_SET ()DO_SET (rdf_sub_t *, equiv_rs, &(rsub->rs_equiv))
      {
	dk_set_push (&res_triples, list (3, box_copy_tree (equiv_rs->rs_iri),
		box_dv_uname_string ("http://www.w3.org/2002/07/owl#equivalentClass"), box_copy_tree (key_ptr[0])));
      }
    END_DO_SET ()}
  ...
  while (hit_next (&hiter, (char **) (&key_ptr), (char **) (&rsub_ptr)))
    {
      ...
      {
	dk_set_push (&res_triples, list (3, box_copy_tree (sub_rs->rs_iri),
		box_dv_uname_string ("http://www.w3.org/2000/01/rdf-schema#subPropertyOf"), box_copy_tree (key_ptr[0])));
      }
      END_DO_SET ()DO_SET (rdf_sub_t *, equiv_rs, &(rsub->rs_equiv))
      {
	dk_set_push (&res_triples, list (3, box_copy_tree (equiv_rs->rs_iri),
		box_dv_uname_string ("http://www.w3.org/2002/07/owl#equivalentProperty"), box_copy_tree (key_ptr[0])));
      }
    END_DO_SET ()}
  DO_BOX_FAST_REV (caddr_t, iid, ctr, ctx->ric_ifp_list)
  {
    dk_set_push (&res_triples, list (3, box_copy_tree (iid), uname_rdf_ns_uri_type,
	    box_dv_uname_string ("http://www.w3.org/2002/07/owl#InverseFunctionalProperty"),
	    box_dv_uname_string ("http://www.w3.org/2002/07/owl#InverseFunctionalProperty")));
  }
  END_DO_BOX_FAST;
  ...
  for (ctr = BOX_ELEMENTS_0 (data_ptr) - 2; ctr >= 0; ctr -= 2)
    {
      dk_set_push (&res_triples, list (3, box_copy_tree (data_ptr[ctr]),
	      box_dv_uname_string ("http://www.w3.org/2002/07/owl#inverseOf"), box_copy_tree (data_ptr[ctr + 1])));
    }
  data_ptr = ctx->ric_prop_props;
  for (ctr = BOX_ELEMENTS_0 (data_ptr) - 2; ctr >= 0; ctr -= 2)
    {
      if (0x1 & (ptrlong) (data_ptr[ctr + 1]))
	dk_set_push (&res_triples, list (3, box_copy_tree (data_ptr[ctr]), uname_rdf_ns_uri_type,
		box_dv_uname_string ("http://www.w3.org/2002/07/owl#InverseFunctionalProperty"),
		box_dv_uname_string ("http://www.w3.org/2002/07/owl#TransitiveProperty")));
    }
  ...
  while (hit_next (&hiter, (char **) (&key_ptr), (char **) (&data_ptr)))
    {
      ...
      {
	dk_set_push (&res_triples, list (3, box_copy_tree (key_ptr[0]),
		box_dv_uname_string ("http://www.openlinksw.com/schemas/virtrdf#nullIFPValue"), box_copy_tree (iid)));
      }
      END_DO_BOX_FAST;
    }
#ifndef NDEBUG
  ...
  while (hit_next (&hiter, (char **) (&key_ptr), (char **) (&rels_ptr)))
	...
	dk_set_push (&res_triples, list (3, box_copy_tree (key_ptr[0]),
		box_dv_uname_string ("http://www.openlinksw.com/schemas/virtrdf#relatedIFP"), box_iri_id (rels[rel_ctr])));
    }
#endif
...
bif_rdf_inf_ifp_rel_list (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (1 == BOX_ELEMENTS (args))
	...
	sqlr_new_error ("22023", "SR634", "Corrupted inference rule set '%.200s', please report the ontology and the query",
	    ctx->ric_name);
  ...
  do
      ...
      for (ifp_ctr = ifp_count; ifp_ctr--; /*no step */ )
	{
	  ...
	  DO_SET (rdf_sub_t *, sub_rsub, &subs)
	  {
	    ...
	    if (0 > sub_ifp_pos)
	      sqlr_new_error ("22023", "SR633",
		  "Inconsistent inference rule set '%.200s' in rdf_inf_set_ifp_list(), loading is aborted, please report the ontology",
		  ctx->ric_name);
...
cl_rdf_inf_init (client_connection_t * cli, caddr_t * err_ret)
{
  ...
  if (2 == cl_rdf_inf_inited)
	...
	vdb_leave_lt (cli->cli_trx, err_ret);
      if (1 != cl_rdf_inf_inited)
	*err_ret =
	    srv_make_new_error ("42000", "CLRI1", "Rdf inf init failed while waiting for other thread to do the rdf inf init");
      return;
    }
  ...
  if (!lt_threads)
	...
	{
	  *err_ret =
	      srv_make_new_error ("42000", "CLRI2",
	      "Cluster rdf inf init failed because bad transaction state.  Rollback and retry.");
	  goto init_error;
	}
    }
...
cl_rdf_bif_check_init (bif_t bif)
{
  if (1 == cl_rdf_inf_inited)
    return;
  if (bif_rdf_inf_ifp_list == bif || bif_rdf_inf_ifp_is_excluded == bif || bif_rdf_is_sub == bif || bif_rdf_super_sub_list == bif)
    {
      caddr_t err = NULL;
      client_connection_t *cli = sqlc_client ();
      if (!cli || !cli->cli_trx || 1 != cli->cli_trx->lt_threads)
	GPF_T1 ("reading a qf with a bif needing rdf inf init.  The cli does not have the right lt settings");
...
sqlg_ri_post_filter (table_source_t * ts, df_elt_t * tb_dfe, rdf_inf_pre_node_t * ri, int p_check)
{
  ...
  if (ri->ri_outer_any_passed && (ajt = IS_TS (ts) ? ts->ts_after_join_test : ((hash_source_t *) ts)->hs_after_join_test))
    {
...
void
sqlg_trailing_subclass_inf (sqlo_t * so, data_source_t ** q_head, data_source_t * ts, df_elt_t * p_dfe, caddr_t p_const,
    df_elt_t * o_dfe, caddr_t o_iri, rdf_inf_ctx_t * ctx, df_elt_t * tb_dfe, int inxop_inx)
{
  state_slot_t *o_slot;
  rdf_inf_pre_node_t *ri;
  if (sas_dummy_ctx == ctx || tb_dfe->_.table.is_inf_col_given)
    return;
  if (p_const && !box_equal (rdfs_type, p_const))
    return;
...
void
sqlg_trailing_subproperty_inf (sqlo_t * so, data_source_t ** q_head, data_source_t * ts, df_elt_t * p_dfe, caddr_t p_const,
    df_elt_t * o_dfe, caddr_t o_iri, rdf_inf_ctx_t * ctx, df_elt_t * tb_dfe, int inxop_inx)
{
  state_slot_t *p_slot;
  rdf_inf_pre_node_t *ri;
    ...
    rdf_inf_ctx_t * ctx, df_elt_t * tb_dfe, int inxop_inx, rdf_inf_pre_node_t ** ri_ret)
{
  ...
  *ri_ret = ri;
  ...
  ri->ri_mode = mode;
  ri->ri_output =
      ssl_new_variable (tb_dfe->dfe_sqlo->so_sc->sc_cc,
...
sqlg_rdf_inf_1 (df_elt_t * tb_dfe, data_source_t * ts, data_source_t ** q_head, int inxop_inx)
{
  ...
  if (!IS_TS (((table_source_t *) ts)) && (qn_input_fn) hash_source_input != ts->src_input)
    return;
  if (-1 != inxop_inx)
    {
      df_inx_op_t *dio = (df_inx_op_t *) dk_set_nth (tb_dfe->_.table.inx_op->dio_terms, inxop_inx);
  ...
  DO_SET (df_elt_t *, cp, &col_preds)
  {
	...
	break;
      case 'S':
	s_dfe = cp->_.bin.right;
	break;
      case 'P':
	p_dfe = cp->_.bin.right;
	break;
      case 'O':
	o_dfe = cp->_.bin.right;
	break;
      }
  }
  END_DO_SET ();
...
sqlg_cl_outer_with_iters (df_elt_t * tb_dfe, data_source_t * ts, data_source_t ** head)
{
  ...
  if (!ose)
    SQL_GPF_T1 (tb_dfe->dfe_sqlo->so_sc->sc_cc, "supposed to have ose in bracket outer");
  if (ojt)
