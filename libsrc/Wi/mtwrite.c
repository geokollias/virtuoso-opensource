...
iq_schedule (buffer_desc_t ** bufs, int n)
{
  ...
  DO_SET (io_queue_t *, iq, &mti_io_queues)
  {
	...
	idbg_printf (("IQ %s %d %s added, %s.\n", IQ_NAME (iq),
		n_added, is_reads ? "reads" : "writes", was_empty ? "starting" : "running"));
      }
...
iq_loop (io_queue_t * iq)
{
  ...
  for (;;)
      ...
      if (!iq->iq_current)
	{
      ...
      if (iq->iq_waiting_shut && iq_on && iq->iq_action_ctr > main_bufs / (n_iqs * n_iqs) && !wi_inst.wi_checkpoint_atomic)
	{
	...
	{
	  if (!buf->bd_is_write)
	    GPF_T1 ("read ahead buf must have  bd_is_write");
	  mti_reads_queued--;
	  if (!buf->bd_page)
      ...
      else
	{
	  ...
	  if (bp_buf_enter (buf, &buf_itm))
		  ...
		  && buf->bd_tree && buf->bd_page && !buf->bd_readers && !buf->bd_is_write && !buf->bd_write_waiting)
		{
		  ...
		  if (_thread_sched_preempt == 0 && write_cum_time - start_write_cum_time > 200)
		    {
		...
		{
#ifdef O12DEBUG
		  dbg_printf (("[Cancelled W %ld now %ld %ld]", mtwrite_pages[n], buf->bd_page, buf->bd_physical_page));
		  rdbg_printf (("[Cancelled W ??? now %ld %ld]", buf->bd_page, buf->bd_physical_page));
#endif
		  mutex_leave (&buf_itm->itm_mtx);
		}
	    }
...
mt_write_dirty (buffer_pool_t * bp, int age_limit, int phys_eq_log)
{
  ...
  for (inx = 0; inx < bp->bp_n_bufs; inx++)
      ...
      if (age_limit && ((bp->bp_ts - buf->bd_timestamp)) < age_limit)
	{
      ...
      if (bp_buf_enter (buf, &buf_itm))
	{
	  if (!buf->bd_is_write && !buf->bd_readers && !buf->bd_write_waiting && !buf->bd_iq && buf->bd_is_dirty)
	    {
	      if (buf->bd_being_read)
...
{
  int inx;
  if (mti_writes_queued > 10 || !iq_is_on ())
    return;
