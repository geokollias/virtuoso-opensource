...
static caddr_t soap_box_xml_entity_validating_1 (caddr_t * entity, caddr_t * err_ret, caddr_t type_ref, int elem, soap_ctx_t * ctx,
    sql_type_t * sqt);
...
static void soap_print_schema_fragment (caddr_t type_name, sql_type_t * sqt, dk_session_t * out, dk_set_t * types_set, int sp,
    soap_ctx_t * ctx);
...
ws_uddi_error (dk_session_t * ses, char *state, char *message, int soap_version)
{
  ...
  snprintf (tmp, sizeof (tmp),
      ...
      "</Body>"
      "</Envelope>",
      SOAP_URI (soap_version), uddi_operator ? uddi_operator : UDDI_DEFAULT_OPERATOR, uddi_code, uddi_error, message);
...
void
mime_c_compose (soap_call_ctx_t ctx, caddr_t * input)
{
  ...
  err = qr_quick_exec (mime_call, ctx.sc_client, NULL, &lc, 1, ":0", arg, QRP_RAW);
...
void
mime_compose (ws_connection_t * ws, caddr_t * input)
{
  static query_t *mime_call = NULL;
  local_cursor_t *lc = NULL;
  caddr_t replay = NULL, arg = NULL, hdr = NULL, err = NULL;
  ...
  err = qr_quick_exec (mime_call, ws->ws_cli, NULL, &lc, 1, ":0", arg, QRP_RAW);
...
ws_soap12_error (dk_session_t * ses, char *code, char *state, char *message, int *http_resp_code, soap_ctx_t * ctx)
{
  ...
  if (mcode == 2 && ctx && ctx->not_understood)
	...
	name = ++colon;
	  ...
	  "<env:Header>" "<env:NotUnderstood qname='nu:%s' xmlns:nu='%*.*s' />" "</env:Header>", name, off_c, off_c, ns);
    }
  ...
  snprintf (tmp, sizeof (tmp),
      ...
      code2 ? code2 : "", code2 ? "</env:Value></env:Subcode>" : "", server_default_language_name);
  SES_PRINT (ses, tmp);

  dks_esc_write (ses, message, strlen (message), CHARSET_UTF8, CHARSET_UTF8, DKS_ESC_PTEXT | DKS_ESC_COMPAT_SOAP);

  snprintf (tmp, sizeof (tmp), "</env:Text></env:Reason>" "</env:Fault>" "</env:Body>" "</env:Envelope>");
  SES_PRINT (ses, tmp);
    ...
    int soap_version, int uddi, int *http_resp_code, soap_ctx_t * ctx)
{
  ...
  snprintf (tmp, sizeof (tmp),
      ...
      "<SOAP:Body>"
      "<SOAP:Fault>" "<faultcode>%s</faultcode>" "<faultstring>[Virtuoso SOAP server] ", SOAP_URI (soap_version), code1);
  session_buffered_write (ses, tmp, strlen (tmp));
  dks_esc_write (ses, message, strlen (message), CHARSET_UTF8, CHARSET_UTF8, DKS_ESC_PTEXT | DKS_ESC_COMPAT_SOAP);
  snprintf (tmp, sizeof (tmp), "</faultstring>%s%s%s"
      ...
      "</SOAP:Body>"
      "</SOAP:Envelope>",
      (soap_version == 1 ? "<runcode>" : "<detail />"), (soap_version == 1 ? state : ""), (soap_version == 1 ? "</runcode>" : ""));
  session_buffered_write (ses, tmp, strlen (tmp));
...
xml_find_child (caddr_t * entity, const char *szSearchName, const char *szURI, int nth, int *start_inx)
{
  ...
  DO_BOX (caddr_t *, child, inx, entity)
  {
	...
	if (szColon && !szURI)
	  ...
	  continue;
	else if (szColon && szURI && ((URILen != (szColon - szName)) || strnicmp (szName, szURI, MIN (szColon - szName, URILen))))
	  continue;
...
soap_type_to_dtp (const char *szType1, int mode)
{
  ...
  if (!szType)
    return 0;
      ...
      !strcmp (szType, "i1") ||
      !strcmp (szType, "i2") ||
      !strcmp (szType, "i4") || !strcmp (szType, "ui1") || !strcmp (szType, "ui2") || !strcmp (szType, "ui4"))
    return DV_LONG_INT;
  else if (!strcmp (szType, "double") || !strcmp (szType, "r4") || !strcmp (szType, "r8") || !strcmp (szType, "fixed.14,4"))
    return DV_DOUBLE_FLOAT;
  else if (!strcmp (szType, "float"))
    return DV_SINGLE_FLOAT;
  else if (!strcmp (szType, "number") || !strcmp (szType, "numeric") || !strcmp (szType, "decimal"))
    return DV_NUMERIC;
  else if (!strcmp (szType, "timeInstant") ||
      !strcmp (szType, "date") || !strcmp (szType, "time") || !strcmp (szType, "dateTime") || !strcmp (szType, "dateTime.tz"))
    return DV_DATETIME;
    ...
    return DV_SHORT_INT;
      ...
      !strcmp (szType, "ID") ||
      !strcmp (szType, "token") || !strcmp (szType, "QName") || !strcmp (szType, "ENTITY") || !strcmp (szType, "anyURI"))
...
ws_soap_check_async (client_connection_t * cli, query_t * qr, dk_session_t * ses, int soap_version)
{
  ...
  strses_flush (ses);
  snprintf (tmp, sizeof (tmp), "<SOAP:Envelope xmlns:SOAP=\"%s\">" "<SOAP:Body />" "</SOAP:Envelope>", SOAP_URI (soap_version));
  session_buffered_write (ses, tmp, strlen (tmp));
...
soap_box_xml_entity (caddr_t * entity, caddr_t * err_ret, dtp_t proposed_type, int soap_version)
{
  ...
  if (type != DV_ARRAY_OF_POINTER)
      ...
      else
	{
	  if (proposed_type == DV_SHORT_INT)
	    {
	      if (!strcmp ((caddr_t) entity, "yes") || !strcmp ((caddr_t) entity, "1") || !strcmp ((caddr_t) entity, "true"))
		ret = box_num (1);
	      else
	    ...
	    {
	      ret = box_cast_to (NULL, wide, DV_WIDE, proposed_type, NUMERIC_MAX_PRECISION, NUMERIC_MAX_SCALE, err_ret);
  ...
  if (BOX_ELEMENTS (entity) == 2 && DV_TYPE_OF (XML_ELEMENT_CHILD (entity, 0)) != DV_ARRAY_OF_POINTER)
    ...
    const char *h_namespace, dtp_t obj_type, soap_ctx_t * ctx)
{
  ...
  if (!IS_BOX_POINTER (object))
	...
	SES_PRINT (out, " xsi:type=\"xsd:int\" dt:dt=\"int\"");
      ...
      DV_TYPE_OF (((caddr_t *) object)[0]) == DV_COMPOSITE && DV_TYPE_OF (((caddr_t *) object)[1]) == DV_LONG_INT)
    {
      if (SOAP_USES_TYPES)
		...
		if (is_string)
		  {
			...
			if (soap_version == 11)
			  ...
			  snprintf (temp, sizeof (temp),
			      " SOAP-ENC:arraySize='%ld' SOAP-ENC:nodeType='array' SOAP-ENC:itemType='xsd:string'>", (long) length);
			else
			  snprintf (temp, sizeof (temp),
			      " xsi:type=\"xsd:string[%ld]\" SOAP:arrayType=\"string[%ld]\">", (long) length, (long) length);
			SES_PRINT (out, temp);
		      }
		    else
		      session_buffered_write_char ('>', out);
		...
		else
		  {
			...
			if (soap_version == 11)
			  ...
			  snprintf (temp, sizeof (temp),
			      " xsi:type=\"xsd:variant[%ld]\" SOAP:arrayType=\"item[%ld]\">", (long) length, (long) length);
			SES_PRINT (out, temp);
		      }
		    else
		      session_buffered_write_char ('>', out);
	...
	case DV_ARRAY_OF_LONG:
	  {
		...
		if (soap_version == 11)
		  snprintf (temp, sizeof (temp),
		      " xsi:type=\"xsd:int[%ld]\" SOAP-ENC:arrayType=\"xsd:int[%ld]\">", (long) length, (long) length);
		else if (soap_version == 12)
		  snprintf (temp, sizeof (temp),
		      " SOAP-ENC:arraySize='%ld' SOAP-ENC:nodeType='array' SOAP-ENC:itemType='xsd:int'>", (long) length);
		else
		  snprintf (temp, sizeof (temp),
		      " xsi:type=\"xsd:int[%ld]\" SOAP:arrayType=\"xsd:int[%ld]\">", (long) length, (long) length);
		SES_PRINT (out, temp);
	      }
	    else
	      session_buffered_write_char ('>', out);
	...
	case DV_ARRAY_OF_DOUBLE:
	  {
		...
		if (soap_version == 11)
		  ...
		  snprintf (temp, sizeof (temp),
		      " SOAP-ENC:arraySize='%ld' SOAP-ENC:nodeType='array' SOAP-ENC:itemType='xsd:double'>", (long) length);
		else
		  snprintf (temp, sizeof (temp),
	...
	case DV_ARRAY_OF_FLOAT:
	  {
	    ...
	    if (SOAP_USES_TYPES)
	      {
		if (soap_version == 11)
		  snprintf (temp, sizeof (temp), " xsi:type=\"xsd:float[%ld]\" SOAP-ENC:arrayType=\"xsd:float[%ld]\">",
		      (long) length, (long) length);
		else if (soap_version == 12)
		  snprintf (temp, sizeof (temp),
		      " SOAP-ENC:arraySize='%ld' SOAP-ENC:nodeType='array' SOAP-ENC:itemType='xsd:float'>", (long) length);
		else
		  snprintf (temp, sizeof (temp), " xsi:type=\"xsd:float[%ld]\" SOAP:arrayType=\"xsd:float[%ld]\">", (long) length,
		      (long) length);
		SES_PRINT (out, temp);
	      }
	    else
	...
	case DV_SINGLE_FLOAT:
	  if (SOAP_USES_TYPES)
	    snprintf (temp, sizeof (temp), " xsi:type=\"xsd:float\" dt:dt=\"float\">" SINGLE_E_STAR_FMT, SINGLE_E_PREC,
		*(float *) object);
	  else
	...
	case DV_DOUBLE_FLOAT:
	  if (SOAP_USES_TYPES)
	    snprintf (temp, sizeof (temp), " xsi:type=\"xsd:double\" dt:dt=\"double\">" DOUBLE_E_STAR_FMT, DOUBLE_E_PREC,
		*(double *) object);
	  else
	...
	case DV_XML_ENTITY:
	  {
	    ...
	    if (!XE_IS_TREE (ent))
	      return srv_make_new_error ("42000", "SP038",
		  "SOAP can not print persistent XML entity in the result, only XML trees are supported.");
  ...
  if (DV_ARRAY_OF_POINTER == dtp)
      ...
      if (!strcmp (tag, SOAP_TAG_DT_ELEMENT) || !strcmp (tag, SOAP_TAG_DT_ATTR))
	{
	  ...
	  if (colon &&
	      ...
	      ((colon - type) != strlen (current_ns->ns_uri) || strncmp (current_ns->ns_uri, type, (size_t) (colon - type))))
	    {
	      int found = 0;
    ...
    sql_type_t * sqt, soap_ctx_t * ctx)
{
  ...
  if (!name || !colon || 0 == strncmp (name, W3C_2001_TYPE_SCHEMA_XSD, strlen (W3C_2001_TYPE_SCHEMA_XSD)))	/* not prefixed types are valid, and treated as of XSD */
    goto end_find;
      ...
      if (udt)
	{
	  ...
	  DO_BOX (sql_field_t *, fld, inx, udt->scl_member_map)
	  {
	    if (IS_COMPLEX_SQT (fld->sfl_sqt))
	      {
		caddr_t scl_soap_type = soap_sqt_to_soap_type (&(fld->sfl_sqt), fld->sfl_soap_type, ctx->opts,
		    udt->scl_name_only, fld->sfl_name);

		soap_wsdl_schema_push (ns_set, types_set, scl_soap_type, element, import, &(fld->sfl_sqt), ctx);
	  ...
	  if (IS_COMPLEX_SQT (sqt1))
	    {
	      ...
	      caddr_t soap_type_name = soap_sqt_to_soap_type (&(sqt1), NULL, ctx->opts, nc_type_name, "item");
...
soap_wsdl_print_ns_decl (dk_session_t * out, dk_set_t * ns_set, char *import)
{
  ...
  {
#if 0
    if ((!import && !elm->ns_imported) || (import && !strcmp (elm->ns_uri, import)))
#endif
      {
	if (import)
	  SES_PRINT (out, "\t");
...
ws_get_opt (caddr_t * opts, char *opt_name, char *def)
{
  ...
  for (inx = 0; inx < (ARRAYP (opts) ? BOX_ELEMENTS (opts) : 0); inx += 2)
    {
      if (DV_STRINGP (opts[inx]) && DV_STRINGP (opts[inx + 1]) && !strcmp (opts[inx], opt_name))
	{
	  return opts[inx + 1];
	}
    }
...
static caddr_t
soap_sqt_to_soap_type (sql_type_t * sqt, caddr_t soap_type, caddr_t * opts, const char *op_name, const char *fld_name)
{
  caddr_t ret;
    ...
    caddr_t * err_ret, caddr_t * call_text, caddr_t headers, caddr_t lines, soap_ctx_t * ctx, caddr_t * xml_tree)
{
  ...
  *call_text = dk_alloc_box_zero (strlen (q) + strlen (o) + strlen (n) + strlen (m) + 12 + 12 + npars * 2, DV_SHORT_STRING);
  ...
  DO_SET (state_slot_t *, proc_param, &proc_qry->qr_parms)
  {
    ...
    else if (param_name && (!strnicmp ("uddi_req", param_name, 8) || !strnicmp ("all_params_xml", param_name, 14)))
      {
	  ...
	  if (inx1 > 0 && DV_TYPE_OF (xml_param) == DV_ARRAY_OF_POINTER)
	      ...
	      if (role)
		{
		  ...
		  if (strcmp (role, SOAP_ROLE_NEXT) &&
		      strcmp (role, SOAP_ROLE_ULTIMATE) && (!ctx->role_url || strcmp (role, ctx->role_url)))
		    continue;
		}

	      if (colon)
		szName = colon + 1;
	      if (!stricmp (szName, param_name) || (parm_enc && !strcmp (szFullName, proc_qry->qr_parm_alt_types[n_parm])))
		{
		  if (proc_qry->qr_parm_alt_types[n_parm])
		    params[inx] = soap_box_xml_entity_validating_1 (xml_param, err_ret,
			proc_qry->qr_parm_alt_types[n_parm], parm_enc, ctx, &(proc_param->ssl_sqt));
		  else if (IS_COMPLEX_SQT (proc_param->ssl_sqt))
		    params[inx] = soap_box_xml_entity_validating_1 (xml_param, err_ret, "", parm_enc, ctx, &(proc_param->ssl_sqt));
		  else
...
#if 0
		  if (proc_qry->qr_parm_alt_types[n_parm] &&
		      strncmp (proc_qry->qr_parm_alt_types[n_parm], szFullName, colon - szFullName))
		    break;
#endif
		}
	      if (!stricmp (szName, param_name) ||
		  ((SOAP_MSG_LITERAL & proc_qry->qr_proc_place) &&
		      proc_qry->qr_parm_alt_types[n_parm] && !strcmp (proc_qry->qr_parm_alt_types[n_parm], szFullName)))
		{
		  if (IS_SOAP_MSG_SET (proc_qry->qr_parm_place[n_parm], SOAP_MSG_XML))
		    ...
		    params[inx] = soap_box_xml_entity_validating_1 (xml_param, err_ret,
			proc_qry->qr_parm_alt_types[n_parm], literal, ctx, &(proc_param->ssl_sqt));
		  else if (IS_COMPLEX_SQT (proc_param->ssl_sqt))
		    params[inx] = soap_box_xml_entity_validating_1 (xml_param, err_ret, "", literal, ctx, &(proc_param->ssl_sqt));
		  else
		    params[inx] = soap_box_xml_entity (xml_param, err_ret, param_type, ctx->soap_version);
		  if (!*err_ret)
		    {
    ...
    if (proc_qry->qr_parm_alt_types[n_parm])
      soap_wsdl_schema_push (&(ctx->ns_set), &(ctx->types_set), proc_qry->qr_parm_alt_types[n_parm], parm_enc, 0, NULL, ctx);
    else if (IS_COMPLEX_SQT (proc_param->ssl_sqt))
      {
	caddr_t scl_soap_type = soap_sqt_to_soap_type (&(proc_param->ssl_sqt), NULL,
	    ctx->opts, operation_name, proc_param->ssl_name);
	soap_wsdl_schema_push (&(ctx->ns_set), &(ctx->types_set), scl_soap_type, parm_enc, 0, &(proc_param->ssl_sqt), ctx);
	dk_free_box (scl_soap_type);
      }
    inx++;
    n_parm += 1;
  ...
  if (proc_qry->qr_proc_alt_ret_type)
    soap_wsdl_schema_push (&(ctx->ns_set), &(ctx->types_set), proc_qry->qr_proc_alt_ret_type, literal, 0, NULL, ctx);
    ...
    caddr_t lines, soap_ctx_t * ctx, caddr_t * err_ret, dk_session_t * ses)
{
  ...
  DO_BOX (caddr_t *, xml_param, inx1, headers)
  {
	...
	if (must_understand < 0)
	  {
	    *err_ret = ws_soap_error (ses, "300", "22023", "env:mustUnderstand value is not boolean", soap_version, 0, NULL, ctx);
	    return 0;
	  }
	...
	DO_SET (state_slot_t *, proc_param, &proc_qry->qr_parms)
	{
	  ...
	  if (IS_SOAP_MSG_HEADER (proc_qry->qr_parm_place[n_parm]) &&
	      (!stricmp (szName, param_name) || (parm_enc && !strcmp (szFullName, proc_qry->qr_parm_alt_types[n_parm]))))
	    {
	      /* check NS here if present */
	      if (colon && ns && colon > szFullName && strncmp (ns, szFullName, (size_t) (colon - szFullName)))
		{
		  *err_ret = ws_soap_error (ses, "300", "22023", "Invalid header request namespace.", soap_version, 0, NULL, ctx);
		  goto invalid_ns;
		}
	...
	if (!found && must_understand)
	  {
		...
		(!role ||
		    ...
		    !strcmp (role, SOAP_ROLE_NEXT) || (ctx->role_url && !strcmp (role, ctx->role_url))))
	      {
...
proc_is_granted (query_t * proc, oid_t group, oid_t user)
{
  ...
  if (ht)
    {
      if (sec_user_is_in_hash (ht, group, -1) || sec_user_is_in_hash (ht, user, -1))
	return 1;
    }
  ...
  while (id_casemode_hit_next (&it, (caddr_t *) & ptp))
    {
      ...
      if (!proc || (module && proc->qr_module != module) || !sec_proc_check (proc, user->usr_g_id, user->usr_id))
	continue;
...
proc_find_in_grants (char *name, dk_set_t * qrs, char *soap_action)
{
  ...
  END_DO_SET ()if (found <= 1 || exact == 1)	/* exact one match; OK */
    return res;
  else
...
soap_get_run_time_schema (soap_ctx_t * ctx, const char *tgt_name, caddr_t * schema_tree)
{
  ...
  DO_BOX (caddr_t *, elm, inx, sch)
  {
    ...
    if (type_name && !strcmp (type_name, colon) && (!tns || !offs || !strncmp (tns, tgt_name, offs)))
      {
	if (schema_tree)
	  {
...
static caddr_t
soap_find_doc_literal (caddr_t * body, caddr_t * header, dk_set_t * qrs, const char *qual, const char *owner, caddr_t * opts,
    int *use_literal)
{
  ...
  DO_SET (query_t *, proc, qrs)
  {
    ...
    DO_SET (state_slot_t *, ssl, &proc->qr_parms)
    {
	  ...
	  !IS_SOAP_SERVICE_PARAM (ssl->ssl_name) && !IS_SOAP_MSG_FAULT (place))
	{
	  caddr_t *xml_param;
...
soap12_custom_error (dk_session_t * ses, client_connection_t * cli, soap_ctx_t * ctx,
    query_t * proc_qry, local_cursor_t * lc, caddr_t * fault_err, int *http_resp_code, caddr_t * err_ret)
{
  char tmp[4095];
  caddr_t *proc_ret = (caddr_t *) lc->lc_proc_ret;
  int nProcRet = BOX_ELEMENTS (lc->lc_proc_ret);
  ...
  snprintf (tmp, sizeof (tmp),
      ...
      code1,
      code2 ? "\n<SOAP:Subcode>\n<SOAP:Value>" : "",
      code2 ? code2 : "", code2 ? "</SOAP:Value>\n</SOAP:Subcode>" : "", server_default_language_name);
  ...
  DO_SET (state_slot_t *, parm, &proc_qry->qr_parms)
  {
	...
	IS_SOAP_MSG_FAULT (proc_qry->qr_parm_place[ix]) && DV_DB_NULL != DV_TYPE_OF (proc_ret[inx]))
      {
	if (soap_print_xml_entity (proc_ret[inx], ses, cli))
	...
	else if (proc_qry->qr_parm_alt_types[ix])
	  {
		...
		ses, err_ret, proc_qry->qr_parm_alt_types[ix], ctx, parm_enc, 1, &(parm->ssl_sqt));
    ...
    local_cursor_t * lc, soap_ctx_t * ctx, caddr_t * err, int uddi, int *http_resp_code)
{
  ...
  {
    if (parm && IS_SSL_REF_PARAMETER (parm->ssl_type) && inx < nProcRet &&
	IS_SOAP_MSG_HEADER (proc_qry->qr_parm_place[ix]) && DV_DB_NULL != DV_TYPE_OF (proc_ret[inx]))
      {
	...
	if (soap_print_xml_entity (proc_ret[inx], ses, cli))
	...
	else if (proc_qry->qr_parm_alt_types[ix])
	  {
	    ...
	    ctx->add_type = (parm_enc ? 0 : 1);
	    ctx->literal = parm_enc;
	    ctx->req_resp_namespace = SOAP_OPT (RESP_NS, proc_qry, ix, SOAP_HEADER_NAMESPACE (opts));
	    soap_print_box_validating (proc_ret[inx],
		SOAP_OPT (PART_NAME, proc_qry, ix, parm->ssl_name),
		ses, &err_ret, proc_qry->qr_parm_alt_types[ix], ctx, parm_enc, 1, &(parm->ssl_sqt));
	    ctx->literal = save_literal;
    ...
    soap_ctx_t * ctx, caddr_t * err, int uddi, int *http_resp_code)
{
  ...
  if (err_ret)
    {
      dk_free_tree (*err);
      *err = ws_soap_error (ses, "400", ERR_STATE (err_ret), ERR_MESSAGE (err_ret), ctx->soap_version, uddi, http_resp_code, ctx);
    }
  return fault;
}
    ...
    soap_ctx_t * ctx, caddr_t * err, int uddi, int *http_resp_code, const char *szMethod, const char *szMethodURI)
{
      ...
      if (!uddi || is_xml)
	{
	  ...
	  if (soap_print_xml_entity (ret_val, ses, cli))
	    ;
	      ...
	      DV_TYPE_OF (ret_val) != DV_OBJECT && DV_TYPE_OF (ret_val) != DV_REFERENCE && !IS_COMPLEX_SQT (sqt))
	    {
	      caddr_t err_ret = NULL;
	      ptrlong *rtype = (ptrlong *) proc_qry->qr_proc_ret_type;
	      dtp_t ret_dtp = rtype ? (dtp_t) rtype[0] : 0;

	      if (!use_literal && ctx->soap_version == 12 && (ret_val != NULL || DV_LONG_INT == ret_dtp))
		...
		{		/* when run-time schema is used the encoding like doc/literal */
	      ...
	      if (!use_literal && !ctx->def_enc && ctx->soap_version == 12 && 0 != stricmp (type_name, SOAP_VOID_TYPE))
		SES_PRINT (ses, "<SOAP-RPC:result>CallReturn</SOAP-RPC:result>");

	      soap_print_box_validating (ret_val, tag_name, ses, &err_ret, type_name, ctx, use_literal, 1, &sqt);
  ...
  DO_SET (state_slot_t *, parm, &proc_qry->qr_parms)
  {
    if (parm && IS_SSL_REF_PARAMETER (parm->ssl_type) && inx < nProcRet && !IS_SOAP_MSG_SPECIAL (proc_qry->qr_parm_place[ix]))
      {
	caddr_t err_ret = NULL;
	if (soap_print_xml_entity (proc_ret[inx], ses, cli))
	  ;
	...
	else if (parm->ssl_name && IS_SOAP_SERVICE_PARAM (parm->ssl_name))
	  {
	    ...
	    DV_TYPE_OF (proc_ret[inx]) == DV_OBJECT || DV_TYPE_OF (proc_ret[inx]) == DV_REFERENCE || IS_COMPLEX_SQT (parm->ssl_sqt))
	  {
	    ctx->add_schema = 0;
	    ...
	    soap_print_box_validating (proc_ret[inx],
		SOAP_OPT (PART_NAME, proc_qry, ix, parm->ssl_name),
		ses, &err_ret, proc_qry->qr_parm_alt_types[ix], ctx, use_literal, 1, &(parm->ssl_sqt));
	  }
	else
...
soap_serialize_envelope (dk_session_t * ses, client_connection_t * cli, query_t * qr, local_cursor_t * lc,
    soap_ctx_t * ctx, char *schema_ns, int uddi, int *http_resp_code, caddr_t * err, char *szMethod, const char *szMethodURI)
{
  if (!uddi)
    {
	...
	SES_PRINT (ses,
	    ...
	    "xmlns:xsd=\"" W3C_TYPE_SCHEMA_XSD "\" " "xmlns:dt=\"" MS_TYPE_SCHEMA "\" " ">");
      else
	SES_PRINT (ses, "<?xml version='1.0'?>\n" "<SOAP:Envelope ");
...
#if 0
      SES_PRINT (ses, "xmlns:ds=\"" WSS_DSIG_URI "\" \n "	/* WSS NS decl */
    ...
    {				/* UDDI v1.0 header */
	  ...
	  "xmlns:xsi=\"" W3C_TYPE_SCHEMA_XSI "\" "
	  "xmlns:xsd=\"" W3C_TYPE_SCHEMA_XSD "\" " "xmlns:dt=\"" MS_TYPE_SCHEMA "\" " "xmlns=\"" SOAP_TYPE_SCHEMA11 "\" " ">");
    }
    ...
    caddr_t * qst, int *http_resp_code)
{
  ...
  *uddi_action = 0;
  ...
  ctx.dks_esc_compat = ((soap_escapes && (soap_escapes[0] == 'y'
	      || soap_escapes[0] == 'Y')) ? DKS_ESC_COMPAT_SOAP /*USE_CR_ESCAPES */ : 0 /*1 */ );
  ...
  if (soap_version < 1 || soap_version > 12)
    {
      err = ws_soap_error (ses, "300", "37000", "Unsupported SOAP version", 11, *uddi_action, http_resp_code, &ctx);
    }
  ...
  if (!err)
      ...
      if (err)
	{
	  if (soap_version < 12 || (is_dime && *is_dime))
	  ...
	  else
	    {
	      caddr_t err1;
	      err1 =
		  ws_soap_error (ses, "330", ERR_STATE (err), ERR_MESSAGE (err), soap_version, *uddi_action, http_resp_code, &ctx);
	      dk_free_tree (err);
	      err = err1;
	      goto end;
	    }
      ...
      if (!(envelope = xml_find_child (xml_tree, "Envelope", SOAP_URI (soap_version), 0, NULL)))
	{
	  ...
	  if (envelope_tag && !strcmp (extract_last_xml_name_part (envelope_tag), "Envelope"))
	    {
	      err = ws_soap_error (ses, "100", "37000", "Wrong version", soap_version, *uddi_action, http_resp_code, &ctx);
	    }
	  else
	    {
	      err = ws_soap_error (ses, "300", "37000", "Invalid SOAP request", soap_version, *uddi_action, http_resp_code, &ctx);
	    }
	  goto end;
	}
      ...
      if (ctx.soap_version == 12 && encodingStyle && _soap_enc && 0 != strcmp (encodingStyle, _soap_enc))
	{
      ...
      if (ctx.soap_version > 1 && (!encodingStyle || (encodingStyle && _soap_enc && 0 != strcmp (encodingStyle, _soap_enc))))
	  ...
	  (DV_TYPE_OF (procedure_mappings) == DV_ARRAY_OF_POINTER || DV_STRINGP (procedure_mappings)))
	{
	  ...
	  if (DV_STRINGP (procedure_mappings))
		...
		item = box_dv_short_string (szMethod);
		  ...
		  BOX_ELEMENTS (procedure_mappings), DV_ARRAY_OF_POINTER, 0, 2, "soap_server");
	      if (inx != 0)
	...
	{
	  err =
	      ws_soap_error (ses, "300", ERR_STATE (err_sql), ERR_MESSAGE (err_sql), soap_version, *uddi_action, http_resp_code,
	      &ctx);
	  dk_free_tree (err_sql);
	  goto end;
	}
	...
	{
	  /* FIXME: */
	  err = ws_soap_error (ses, "300", "37000", "Attached procedures not available from SOAP", soap_version, *uddi_action);
	  goto end;
	}
#endif
	...
	{
	  if (!err)
	    err = ws_soap_error (ses, "200", "37000", "Header not understood", soap_version, *uddi_action, http_resp_code, &ctx);
	  else
	    *http_resp_code = 400;
	  goto end;
	}
      ...
      if (!err)
	{
	  ...
	  err = qr_exec (cli, call_qry, CALLER_LOCAL, NULL, NULL, &lc, call_params, NULL, 1);
	...
	{
	  err = soap_serialize (ses, cli, proc_qry, lc, &ctx, schema_ns, *uddi_action, http_resp_code, szMethod, szMethodURI);
	}
      if (lc)
	lc_free (lc);
      qr_free (call_qry);
...
*/
  ...
  if (type_fld)
	...
	{
	  dk_set_t parts = NULL;
	  con_content_enc == WS_ENC_DIME ? soap_dime_tree (req_xml, &parts, &err) : soap_mime_tree (ws, &parts, &err, soap_version);
	  if (!err)
	    {
	      const char *_soap_uri = SOAP_URI (soap_version);
	      dime_msgs = (caddr_t **) list_to_array (dk_set_nreverse (parts));
	      if (BOX_ELEMENTS (dime_msgs) > 0 && dime_msgs[0][1] && _soap_uri && 0 == strcmp (dime_msgs[0][1], _soap_uri))
		{
		  dk_free_box (req_xml);
  ...
  if (!err)
      ...
      if (DO_LOG (LOG_SOAP))
	{
	  ...
	  LOG_GET;
	  log_info ("SOAP_0 %s %s %s %i : %*.*s", user, from, peer, req_len, LOG_PRINT_SOAP_STR_L, LOG_PRINT_SOAP_STR_L, req_xml);
	}
#if 0
  ...
  err = soap_server (soap_version, method_fld, ws->ws_strses, xml_tree,
      ...
      NULL, schema_ns, opts, (caddr_t *) & dime_msgs, &con_content_enc, (caddr_t *) CALLER_LOCAL, &http_resp_code);
...
bif_soap_box_xml_entity (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (dtp == DV_ARRAY_OF_POINTER)
  ...
  else
    {
	  ...
	  "not an arg of type %s (%d)", szMe, 1, dv_type_title (dtp), dtp);
    }
...
bif_soap_box_structure (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t *ret;
  if ((elems % 2) > 0)
    sqlr_new_error ("22023", "SP030", "Invalid number of args supplied to soap_box_structure. The argument count should be even");
  if (!elems)
    return dk_alloc_box (0, DV_DB_NULL);
  else
    {
...
/*	  caddr_t par_name = ((caddr_t *) szParamName)[0];*/
  ...
  if (soap_version == 1)
	...
	" xmlns:SOAP='" SOAP_TYPE_SCHEMA10 "'\n" " xmlns:dt='" MS_TYPE_SCHEMA "'>" "<SOAP:Body>");
  else if (soap_version == 11)
    {
      SES_PRINT (out, "<?xml version='1.0' ?>\n" "<SOAP:Envelope\n");
      if (!ctx.literal)
	SES_PRINT (out, " SOAP:encodingType='" SOAP_ENC_SCHEMA11 "'\n");
      ...
      SES_PRINT (out, " xmlns:dt='" MS_TYPE_SCHEMA "'>" "<SOAP:Body>");
    }
  else
  ...
  for (i = 0; params && BOX_ELEMENTS (params) > 0 && i < (int) (BOX_ELEMENTS (params) - 1); i += 2)
      ...
      if (!szParamName)
	{
	  err = srv_make_new_error ("42000", "SP009",
	      "SOAP Parameters array invalid in call to %s#%s ", szMethodURI ? szMethodURI : "", szMethodName);
	  goto end;
	}
	  ...
	  || (DV_ARRAY_OF_POINTER == name_dtp && BOX_ELEMENTS (szParamName) != 2))
	{
	  err = srv_make_new_error ("42000", "SP010",
	      "SOAP Parameter name %d should be string in call to %s#%s", i / 2, szMethodURI ? szMethodURI : "", szMethodName);
	  goto end;
	}
      if (DV_ARRAY_OF_POINTER == name_dtp)
	{
	  ...
	  if (!IS_STRING_DTP (DV_TYPE_OF (par_name)) || !IS_STRING_DTP (DV_TYPE_OF (par_type)))
	    {
		  ...
		  "name and SOAP type as strings in call to %s#%s", i / 2, szMethodURI ? szMethodURI : "", szMethodName);
	      goto end;
	    }
      ...
      else if (DV_OBJECT == DV_TYPE_OF (param_value) || DV_REFERENCE == DV_TYPE_OF (param_value))
	{
      ...
      else if (NULL != (err = soap_print_box (param_value, out, szParamName, soap_version, NULL, 0, &ctx)) && ctx.literal)
	break;
    }
  if (!err)
    {
  ...
  if (soap_version == 1)
	...
	szSOAPAction || ns_contains ? "" : "#", szSOAPAction ? szSOAPAction : szMethodName, strses_length (out));
  else
    goto end;
  ...
  FAILED
  {
    err = srv_make_new_error ("42000", "SP012", "Can\'t send a request to the SOAP server");
    soap_wsdl_schema_free (&(ctx.ns_set), &(ctx.types_set));
  }
...
/* dbg_print_box (dl_mode, stderr); */
  ...
  CATCH_READ_FAIL (http_out)
  {
	...
	err = srv_make_new_error ("42000", "SP013",
	    "Not well formed HTTP SOAP reply for call of %s#%s", szMethodURI ? szMethodURI : "", szMethodName);
	goto end;
      }
    szTmp[12] = 0;
	...
	err = srv_make_new_error ("42000", "SP014",
	    "HTTP error in SOAP reply for call of %s#%s : %s %s", szMethodURI ? szMethodURI : "", szMethodName, szErr, szState);
	goto end;
      }
    for (;;)
      {
	...
	if (!strnicmp ("Content-Type: ", szTmp, 14))
	  ...
	  is_chunked = 1;
	else if (!strnicmp ("Connection:", szTmp, 11) && nc_strstr ((unsigned char *) szTmp, (unsigned char *) "close"))
	  keep_alive = 0;
      }
    if (is_chunked)
	...
	if (!large_content)
	...
	else
	  {
		...
		"Content length in HTTP SOAP reply is too large for call of %s#%s", szMethodURI ? szMethodURI : "", szMethodName);
	  }
	goto end;
      }
    else if (!is_chunked)
  ...
  if (!(envelope = xml_find_child (xml_tree1, "Envelope", SOAP_URI (soap_version), 0, NULL)))
    {
	  ...
	  "Not well formed SOAP request in call of %s#%s", szMethodURI ? szMethodURI : "", szMethodName);
      dk_free_tree ((box_t) xml_tree);
      goto end;
    }
  ...
  if (!(body = xml_find_child (envelope, "Body", SOAP_URI (soap_version), 0, NULL)))
    {
	  ...
	  "No Body section in a SOAP reply to %s#%s", szMethodURI ? szMethodURI : "", szMethodName);
      dk_free_tree ((box_t) xml_tree);
      goto end;
    }
  ...
  if (NULL != (fault = xml_find_child (body, "Fault", SOAP_URI (soap_version), 0, NULL)))
    {
	  ...
	  szMethodName, szRunCode ? szRunCode : "<no runcode>", szFaultString ? szFaultString : "unreported error");
      dk_free_tree ((box_t) xml_tree);
    }
  else
    {
...
soap_call_schema_prep (soap_ctx_t * ctx, caddr_t * params, int headers)
{
  ...
  for (i = 0; params && BOX_ELEMENTS (params) > 0 && i < (int) (BOX_ELEMENTS (params) - 1); i += 2)
      ...
      if (DV_ARRAY_OF_POINTER == param_name_dtp && BOX_ELEMENTS (param_name) > 1)
	{
	  ...
	  soap_wsdl_schema_push (&(ctx->ns_set), &(ctx->types_set), param_type, elem, 0, NULL, ctx);
	}
    }
  return (HC_RET_OK);
}
...
int
soap_call_write_params (soap_call_ctx_t * ctx, caddr_t * err_ret, int header)
{
  ...
  for (i = 0; params && BOX_ELEMENTS (params) > 0 && i < (int) (BOX_ELEMENTS (params) - 1); i += 2)
      ...
      if (!param_name)
	{
	  *err_ret = srv_make_new_error ("42000", "SP021",
	      "SOAP Parameters array invalid in call to %s#%s ", ctx->sc_method_uri ? ctx->sc_method_uri : "", ctx->sc_method_name);
      ...
      if (DV_ARRAY_OF_POINTER == param_name_dtp)
	{
	  ...
	  soap_print_box_validating (param_value,
	      ...
	      par_type,
	      ctx->sc_ser_ctx,
	      (ctx->sc_ser_ctx->literal && (!ctx->sc_ser_ctx->wrapped || header)) ? 1 : 0, ctx->sc_ser_ctx->literal, NULL);
	  ...
	  if (*err_ret)
	    return (HC_RET_ERR_ABORT);
		  ...
		  ctx->sc_soap_out,
		  param_name, ctx->sc_ser_ctx->soap_version, NULL, param_value_dtp, ctx->sc_ser_ctx)) && ctx->sc_ser_ctx->literal)
	return (HC_RET_ERR_ABORT);
    }
  return (HC_RET_OK);
}
...
int
soap_call_make_body (soap_call_ctx_t * ctx, caddr_t * _err_ret)
{
  ...
  if (!literal || wrapped)
	...
	SES_PRINT (ctx->sc_soap_out, "\n<");
      session_buffered_write (ctx->sc_soap_out, ctx->sc_method_name, strlen (ctx->sc_method_name));
...
dk_session_t *
soap_call_make_envelope (soap_call_ctx_t * ctx, caddr_t * _err_ret)
{
  if (ctx->sc_ser_ctx->soap_version == 1)
    ...
    {
      SES_PRINT (ctx->sc_soap_out, "<?xml version='1.0' ?>\n" "<SOAP:Envelope\n");
	...
	SES_PRINT (ctx->sc_soap_out, " SOAP:encodingStyle='http://schemas.xmlsoap.org/soap/encoding/'\n");
...
soap_call_parse_reply (soap_call_ctx_t * ctx, caddr_t * qst, caddr_t * err, caddr_t * skeys)
{
  ...
  if (ctx->sc_http_client->hcctx_respcode > 399 && (ctx->sc_use_xmlrpc || !(ctx->sc_ser_ctx->soap_version > 1 && ctx->sc_http_client->hcctx_respcode == 500)	/* SOAP 1.1 fault in 500 */
      ))
    {
      if (ctx->sc_http_client->hcctx_response && strlen (ctx->sc_http_client->hcctx_response) > 9)
	{
	  ...
	  *err = srv_make_new_error ("42000", "HT073", "HTTP error (%.100s) received", err_string);
	}
      else
	*err = srv_make_new_error ("42000", "HT074", "HTTP error (%d) received", ctx->sc_http_client->hcctx_respcode);
  ...
  xml_tree = (caddr_t ***) xml_make_tree ((query_instance_t *) qst,
      resp_body, err, encoding[0] ? encoding : 0, server_default_lh, NULL /* no DTD */ );
  if (!*err)
    {
      xml_expand_refs ((caddr_t *) xml_tree, err);
  ...
  if (!(envelope = xml_find_child (xml_tree1, "Envelope", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL)))
    {
      *err = srv_make_new_error ("42000", "SP025",
	  "Not well formed SOAP request in call of %s#%s", ctx->sc_method_uri ? ctx->sc_method_uri : "", ctx->sc_method_name);
      dk_free_tree ((box_t) xml_tree);
      return NULL;
    }
  ...
  if (!(body = xml_find_child (envelope, "Body", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL)))
    {
      *err = srv_make_new_error ("42000", "SP026",
	  "No Body section in a SOAP reply to %s#%s", ctx->sc_method_uri ? ctx->sc_method_uri : "", ctx->sc_method_name);
      dk_free_tree ((box_t) xml_tree);
      return NULL;
    }
  ...
  if (!ctx->sc_return_fault && !ctx->sc_dl_mode &&
	...
	{
	  caddr_t *ptr1, *ptr2;
	  ptr1 = xml_find_child (fault, "Code", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL);
	  ptr2 = xml_find_child (fault, "Reason", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL);
	  fault_code = (char *) xml_find_child (ptr1, "Value", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL);
	  fault_str = (char *) xml_find_child (ptr2, "Text", SOAP_URI (ctx->sc_ser_ctx->soap_version), 0, NULL);
	}
      else
	{
      ...
      *err = srv_make_new_error ("42000", "SP027",
	  ...
	  ctx->sc_method_name,
	  run_code ? " [" : "", run_code ? run_code : "", run_code ? "]" : "", fault_str ? fault_str : "unreported error");
...
soap_call_prep_wss (soap_call_ctx_t * ctx, caddr_t * err_ret)
{
  ...
  pars[0] = box_string ("BODY");
  pars[2] = box_string ("KEYI");
  pars[4] = box_string ("TEMPLATE");
  pars[6] = box_string ("SIGN");
  pars[8] = box_string ("NS");

  body = ctx->sc_soap_out;
  keyi = ctx->sc_wss_key;
  _template = ctx->sc_wss_template;
  sign = box_num (ctx->sc_wss_security);
  ns = ctx->sc_wss_ns;

  pars[1] = (caddr_t) & body;
  pars[3] = (caddr_t) & keyi;
  pars[5] = (caddr_t) & _template;
  pars[7] = (caddr_t) & sign;
  pars[9] = (caddr_t) & ns;
...
/* Set http client to authorize */
  ...
  if (ctx.sc_method_uri && !strncmp (ctx.sc_method_uri, ctx.sc_method_name, strlen (ctx.sc_method_uri)) && ssc.soap_version > 1)
    {
  ...
  if (ssc.soap_version == 11)
    {
	  ...
	  ctx.sc_soap_action || ns_contains ? "" : "#", ctx.sc_soap_action ? ctx.sc_soap_action : ctx.sc_method_name);
    }
  ...
  if (DO_LOG (LOG_SOAP_CLI))
    {
	  ...
	  ses->dks_out_fill > LOG_PRINT_SOAP_STR_L ? LOG_PRINT_SOAP_STR_L : ses->dks_out_fill, ses->dks_out_buffer);
    }
  ...
  if (sep && types_set)
    {
      DO_SET (soap_wsdl_type_t *, elm, types_set)
      {
	if ((br && br > name && !strncmp (elm->type_name, name, (size_t) (br - name))) || (!strcmp (elm->type_name, name)))
	  {
	    if (try_elem >= 0)
	      {
    ...
    const char *qpref, size_t pref_len, char *qual)
{
  ...
  DO_SET (query_t *, proc, procs_set)
  {
    ...
    DO_SET (state_slot_t *, ssl, &proc->qr_parms)
    {
      ...
      if (custom_type && 0 != strncmp (custom_type, W3C_2001_TYPE_SCHEMA_XSD, strlen (W3C_2001_TYPE_SCHEMA_XSD)))
	...
	if (IS_COMPLEX_SQT (sqt))
	  {
	    ...
	    soap_wsdl_schema_push (ns_set, types_set, scl_soap_type, (SOAP_MSG_LITERAL & proc->qr_proc_place), 0, &(sqt), ctx);
	    dk_free_box (scl_soap_type);
	  }
      }
  }
...
soap_print_dl_schema (dk_session_t * out, dk_set_t * proc_set,
    ...
    soap_ctx_t * ctx, dk_set_t * types_set, dk_set_t * ns_set, char *tns, int dl_flag, int type_mask)
{
  char *operation_name, q[MAX_NAME_LEN], o[MAX_NAME_LEN], n[MAX_NAME_LEN];
  int out_pars = 0;
  ...
  DO_SET (query_t *, proc, proc_set)
  {
    ...
    DO_SET (state_slot_t *, ssl, &proc->qr_parms)
    {
	  ...
	  !IS_SOAP_SERVICE_PARAM (ssl->ssl_name) && !IS_SOAP_MSG_SPECIAL (proc->qr_parm_place[ix]))
	{
	  custom_type = proc->qr_parm_alt_types[ix];
	  SES_PRINT (out, "\t\t <element minOccurs='1' maxOccurs='1' name='");
	  SOAP_PRINT (PART_NAME, out, proc, ix, ssl->ssl_name) SES_PRINT (out, "' type='");
	  if (!custom_type)
	    {
	...
	if (IS_UDT_XMLTYPE_SQT (&sqt))
	  {
	    caddr_t type_name = soap_sqt_to_soap_type (&(sqt), NULL, ctx->opts, operation_name, "Response");
    ...
    if (custom_type && DV_STRINGP (custom_type) && !stricmp (custom_type, SOAP_VOID_TYPE) && proc->qr_proc_ret_type)
      {
	sql_type_t sqt;
	ddl_type_to_sqt (&sqt, (caddr_t *) proc->qr_proc_ret_type);
	if (IS_UDT_XMLTYPE_SQT (&sqt))
    ...
    const char *service_name, client_connection_t * cli, caddr_t url)
{
  ...
  /* the schema section */
  SES_PRINT (schema_ses, "\t<ss:schema\n" "\t id='Virtuoso");
  SES_PRINT (out, service_name);
  ...
  while (id_casemode_hit_next (&it, (caddr_t *) & ptp))
    {
      ...
      if (!proc
	  || !cli->cli_user
	  || (module && proc->qr_module != module) || !sec_proc_check (proc, cli->cli_user->usr_g_id, cli->cli_user->usr_id))
	continue;
      if (!strnicmp (proc->qr_proc_name, qpref, pref_len))
	{
	  /* the soap section */
...
bif_soap_sdl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  name = sch_full_module_name (sc, module, cli_qual (qi->qi_client), CLI_OWNER (qi->qi_client));
  if (name)
    ...
    dk_session_t * out, long literal_1, dk_set_t * types_set, int ext)
{
  ...
  DO_SET (state_slot_t *, ssl, &proc->qr_parms)
  {
	...
	IS_SOAP_MSG_SET (proc->qr_parm_place[ix], ext) && (IS_SSL_REF_PARAMETER (ssl->ssl_type) || ssl->ssl_type == SSL_PARAMETER))
      {
	...
	SES_PRINT (out, ssl->ssl_name);
	SES_PRINT (out, "\">\n\t\t<part name=\"");
	SOAP_PRINT (PART_NAME, out, proc, ix, ssl->ssl_name) if (!literal)
	  SES_PRINT (out, "\" type=\"");
	else
	  SES_PRINT (out, "\" element=\"");
	if (!custom_type)
    ...
    char *header_ns, int binding_1, int ext, char what)
{
  ...
  {
    if (!IS_SOAP_SERVICE_PARAM (ssl->ssl_name) &&
	IS_SOAP_MSG_SET (proc->qr_parm_place[ix], ext) && (ssl->ssl_type == SSL_REF_PARAMETER || ssl->ssl_type == what))
      {
	const char *use = SOAP_OPT (USE, proc, ix, NULL);
	int binding;
	...
	SES_PRINT (out, ssl->ssl_name);
	SES_PRINT (out, "\" part=\"");
	SOAP_PRINT (PART_NAME, out, proc, ix, ssl->ssl_name) if (header_ns && !binding)
	  {
  ...
  static void soap_wsdl_print_faults (query_t * proc, char *operation_name, dk_session_t * out)
  {
    int ix = 0;
    ...
    {
      if (!IS_SOAP_SERVICE_PARAM (ssl->ssl_name) &&
	  IS_SOAP_MSG_FAULT (proc->qr_parm_place[ix]) && IS_SSL_REF_PARAMETER (ssl->ssl_type))
	{
	  SES_PRINT (out, "\t\t\t<fault name=\"");
	  SES_PRINT (out, operation_name);
	  SES_PRINT (out, ssl->ssl_name);
      ...
      query_instance_t * qi)
  {
	...
	END_DO_SET ()SES_PRINT (out, "\t</portType>\n");
      }
	...
	SES_PRINT (out, "Binding\" type=\"tns:");
	SES_PRINT (out, service_name);
	...
	DO_SET (query_t *, proc, &proc_set)
	{
	  ...
	  if (proc->qr_text && NULL != (desc = regexp_match_01 ("--##.*", proc->qr_text, 0)))
	    {
	      ...
	      dks_esc_write (out, msg, strlen (msg), CHARSET_UTF8, CHARSET_UTF8, DKS_ESC_PTEXT | DKS_ESC_COMPAT_SOAP);
	      SES_PRINT (out, "</documentation>\n");
	      dk_free_tree (desc);
	    }
	  ...
	  if (dime_enabled || (proc->qr_proc_place & SOAP_MSG_IN))
	    SES_PRINT (out, "\t\t\t\t<dime:message layout='" SOAP_DIME_SCHEMA "closed-layout' wsdl:required='true'/>\n");
	  ...
	  soap_wsdl_print_extensions (proc, operation_name, out, header_ns, bt, SOAP_MSG_HEADER, SSL_PARAMETER);
	  ...
	  if (!async)
		...
		SES_PRINT (out, "\" encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/");
	      ...
	      soap_wsdl_print_extensions (proc, operation_name, out, header_ns, bt, SOAP_MSG_HEADER, SSL_REF_PARAMETER_OUT);
	...
	END_DO_SET ()SES_PRINT (out, "\t</binding>\n");
      }
	...
	SES_PRINT (out, "Port\" binding=\"tns:");
	SES_PRINT (out, service_name);
    ...
    if (make_plink)
      {
	SES_PRINT (out, "\t<plt:partnerLinkType name='");
	SES_PRINT (out, service_name);
	SES_PRINT (out, "'>\n");
	SES_PRINT (out, "\t\t<plt:role name='");
	SES_PRINT (out, service_name);
	SES_PRINT (out, "Provider'>\n");
	if (rpcs && docs)
  ...
  static void soap_wsdl20_rpc_sig (dk_session_t * out, query_t * proc)
  {
    int ix = 0;
    caddr_t custom_type;
    sql_type_t sqt;
...
#define HTTP_INTERFACE 1
  ...
  static caddr_t bif_soap_current_url (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
  {
    query_instance_t *qi = (query_instance_t *) qst;
  ...
  static caddr_t bif_dv_to_soap_type (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
  {
  ...
  static caddr_t bif_soap_wsdl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
  {
    caddr_t module;
    char service_name[MAX_QUAL_NAME_LEN];
...
/* SOAP restricted schema validator code */
  static caddr_t xml_find_schema_instance_attribute (caddr_t * entity, const char *name)
  {
	...
	if (ent_name && (!strncmp (ent_name, W3C_2001_TYPE_SCHEMA_XSI, strlen (W3C_2001_TYPE_SCHEMA_XSI)) ||
  ...
  static caddr_t xml_find_schema_attribute (caddr_t * entity, const char *name)
  {
    ...
    if (!val)
      {
	caddr_t ent_name = XML_ELEMENT_NAME (entity);
	if (ent_name && (!strncmp (ent_name, W3C_2001_TYPE_SCHEMA_XSD, strlen (W3C_2001_TYPE_SCHEMA_XSD)) ||
  ...
  static caddr_t xml_find_wsdl_attribute (caddr_t * entity, char *name)
  {
...
/*XXX: make alias of the above */
  static const char *xml_find_soapenc12_attribute (caddr_t * entity, const char *name)
  {
  ...
  int xml_is_sch_qname (char *tag, char *attr)
  {
  ...
  static caddr_t soap_find_attachment (caddr_t * entity, soap_ctx_t * ctx, int *conv)
  {
    int inx;
    caddr_t id = NULL;
    *conv = 0;
...
#define IS_MULTIDIM_ARR(s,v) ((v) <= 11 ? \
  ...
  static int soap_multi_arr_cmp_offset (const char *offs, dk_set_t * lev)
  {
    int ix = 0;
    long i;
    if (!offs || offs[0] != '[')
	...
	for (ix = 0; ix < n_elm; ix++)
	  {
	    ...
	    if (delim)
	      {
		ret[ix] = soap_box_xml_entity_multi_dim_arr (entity, a_name, a_type, err_ret, n_max, inx, delim, lev, ctx);
	      }
	    else
	      {
  ...
  static caddr_t *soap_box_attrs_validating (caddr_t * entity, caddr_t * extension, soap_ctx_t * ctx)
  {
    caddr_t *e_attr_ptr;
    int attr_inx = 0;
    dk_set_t attrs = NULL;
	...
	if (attr_value)
	  {
	    caddr_t err_ret = NULL;
	    attr_value = soap_box_xml_entity_validating ((caddr_t *) attr_value, &err_ret, attr_type, 0, ctx);
	    if (err_ret)
	      {
		dk_free_tree (err_ret);
		goto attr_error;
  ...
  static int xev_is_extension_to (char *type_ref, char *target_type, int elem)
  {
    caddr_t *schema_tree;
    dtp_t proposed_type;
    caddr_t *e_ptr;
  ...
  static int soap_box_enum_validate (caddr_t * entity, caddr_t box, caddr_t * extension, soap_ctx_t * ctx)
  {
    ...
    if (box)
      {
	caddr_t err = NULL;
	value = box_cast_to (NULL, box, DV_TYPE_OF (box), DV_STRING, NUMERIC_MAX_PRECISION, NUMERIC_MAX_SCALE, &err);
	if (err)
	  {
...
#define SOAP_VALIDATE_ERROR_2(msg,box_to_free) { \
  ...
  static caddr_t *soap_box_next_ext_type (caddr_t * type_ref, caddr_t * err_ret, soap_ctx_t * ctx)
  {
    caddr_t *schema_tree;
    dtp_t proposed_type;
    caddr_t *e_ptr;
	...
	if (NULL == (extension = xml_find_schema_child (e_ptr, "extension", 0)))
	  {
	    if (NULL == (extension = xml_find_schema_child (e_ptr, "restriction", 0)))
	      SOAP_VALIDATE_ERROR (("22023", "SV078", "The type '%s' is derived by extension or restriction, but no such base type",
		      *type_ref));
	  }
	e_ptr = extension;
  ...
  error:
    if (!*err_ret)
      {
	if (!ctx->error_message)
	  *err_ret = srv_make_new_error ("22023", "SV030", "Can't validate according to the parameter schema %s", *type_ref);
	else
	  {
      ...
      soap_ctx_t * ctx, sql_type_t * sqt)
  {
    ...
    if (type_name && ARRAYP (schema_tree) && BOX_ELEMENTS (schema_tree) == 1 && ARRAYP (entity) && BOX_ELEMENTS (entity) == 1)	/* an empty element */
      return dk_alloc_box (0, DV_DB_NULL);
...
/*XXX:	   if (ent_child_inx != inx)
	     goto error;
*/
	    if (!udt_soap_struct_to_udi ((caddr_t *) id_hash_get (ht_soap_udt, (caddr_t) & type_ref), &ret_set, &ret, err_ret))
	      goto error;
	  }
	  ...
	  {			/* array */
		...
		if (BOX_ELEMENTS (entity) == 1 || (BOX_ELEMENTS (entity) == 2 && !ARRAYP (entity[1])))
		...
		else
		  {
			...
			if (offset && 1 != sscanf (offset, "[%ld]", &off))
			  off = -1;
			if (ix >= n_max)
			  SOAP_VALIDATE_ERROR (("22023", "SV007",
				  "Maximum number (%ld) of elements of array '%s' is reached, but there is more data", n_max,
				  type_ref));
		...
		if (ctx->soap_version <= 11)
		  ...
		  brace = wsdl_type;
		ret = soap_box_xml_entity_multi_dim_arr (entity, a_name, a_type, err_ret, n_max, &inx, brace, &lev, ctx);
	      }
	    if (inx - 1 < n_min)
	      SOAP_VALIDATE_ERROR (("22023", "SV008", "Minimum number (%ld) of elements of array '%s' is not reached", n_min,
		      type_ref));
	    if (has_attrs)
	      {
		caddr_t *attrs = soap_box_attrs_validating (entity, extension, ctx);
	...
	if (NULL != (e_ptr = xml_find_schema_child (schema_tree, "simpleContent", 0)))
	  {			/* support for simpleType/extension */
	    ...
	    if (!extension && XML_ELEMENT_ATTR_COUNT (entity))
	      SOAP_VALIDATE_ERROR (("22023", "SV011", "The type '%s' is not derived by extension, but an attribute is supplied",
		      type_ref));
		...
		DO_BOX (sql_field_t *, fld, inx, udt->scl_member_map)
		{
		  ...
		  if (!stricmp (udt->scl_name, "DB.DBA.XMLType") && !strcmp (fld->sfl_name, "xt_ent"))
			...
			SOAP_VALIDATE_ERROR (("22023", "SV074", "The type '%s' requires definition of '%s'", type_ref,
				soap_fld_name));
		      if (elem_entity)
			{
			  caddr_t soap_fld_type;
			  soap_fld_type = soap_sqt_to_soap_type (&(fld->sfl_sqt), fld->sfl_soap_type, ctx->opts, NULL, NULL);
			  value = soap_box_xml_entity_validating_1 (elem_entity, err_ret, soap_fld_type, 0, ctx, &(fld->sfl_sqt));
		...
		while (elem_entity || (max_items > 0 && max_items > ix))
		  {
		    ...
		    if (offset && 1 != sscanf (offset, "[%ld]", &off))
		      off = -1;
		    if (ix >= n_max)
		      SOAP_VALIDATE_ERROR (("22023", "SV070",
			      "Maximum number (%ld) of elements of array is reached, but there is more data", n_max));
		    if ((!elem_entity && max_items > ix) || (off >= 0 && ix != off))
		      ret_elem = dk_alloc_box (0, DV_DB_NULL);
		    else if (elem_entity)
		      {
		...
		if (proposed_type == DV_SHORT_INT)
		  {		/*FIXME: the value if all are digits must be converted to a number */
		    if (!strcmp ((caddr_t) value, "yes") || !strcmp ((caddr_t) value, "1") || !strcmp ((caddr_t) value, "true"))
		      ret = box_num (1);
			...
			if (NULL != err_msg)
			  {
			    dk_free_box (ret);
			    SOAP_VALIDATE_ERROR_2 (("22023", "SV020", "Value for type '%s' can't be converted: %.500s", type_ref,
				    err_msg), err_msg);
			  }
		      }
		    else if (proposed_type == DV_BIN)
		...
		static caddr_t bif_soap_box_xml_entity_validating (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
		{
		  ...
		  if (DV_XML_ENTITY == DV_TYPE_OF (entity_1))
			...
			{
			  *err_ret =
			      srv_make_new_error ("22023", "SV080",
			      "Function '%s' can process only XML tree entities, not e.g. XPER entities", szMe);
			  return NULL;
			}
			...
			{	/* Search for data inside the root */
			  ...
			  for (idx = 1; idx < BOX_ELEMENTS (xe->xte_current); idx++)
				...
				{
				  *err_ret =
				      srv_make_new_error ("22023", "SV067",
				      "The XML fragment is not a valid XML document: it contains more than one top level element");
				  return NULL;
				}
			    }
			  if (NULL == entity)
			    {
			      *err_ret =
				  srv_make_new_error ("22023", "SV068",
				  "The XML fragment is not a valid XML document: it does not contain top level element");
			      return NULL;
			    }
			}
		    }
...
#define SES_PRINT_CHAR(ses, c) session_buffered_write_char (c, ses)
		    ...
		    soap_box_mda_size (caddr_t box, char *delim, caddr_t * err_ret, char *dim, int dim_len, soap_ctx_t * ctx)
		{
		  ...
		  char tmp[10];
		  if (!ARRAYP (box)
		      || (ARRAYP (box) && size > 0 && size != BOX_ELEMENTS (box)) || (!size && ARRAYP (box) && !BOX_ELEMENTS (box)))
		    return -1;
		  if (!size)
		...
		static void
		    soap_box_array_to_mda (caddr_t box, caddr_t * new_box, char *delim, char *dim, int dim_len, caddr_t * err_ret,
		    long *ix, soap_ctx_t * ctx)
		{
		...
		static int
		    soap_print_scalar_value (dtp_t proposed_type, caddr_t value, dk_session_t * ses, soap_ctx_t * ctx,
		    caddr_t * err_ret)
		{
/*  int use_escapes = ctx->use_escapes;*/
		  if (proposed_type == DV_SHORT_INT)
		  ...
		  else if (proposed_type == DV_SINGLE_FLOAT || proposed_type == DV_DOUBLE_FLOAT)
		    {
		      char tmp[500];
		  ...
		  else if (proposed_type == DV_BIN)
		    {
		      ...
		      if (DV_TYPE_OF (value) != DV_BIN && !DV_STRINGP (value) && !DV_WIDESTRINGP (value)
			  && !IS_BLOB_HANDLE_DTP (DV_TYPE_OF (value)))
		      ...
		      if (IS_BLOB_HANDLE_DTP (DV_TYPE_OF (value)))
			{
			  ...
			  if (bh->bh_ask_from_client)
			    SOAP_VALIDATE_ERROR (("22023", "SV091",
		      ...
		      if (*err_ret)
			{
		      ...
		      if (wide)
			{
			  dks_wide_esc_write (ses, (wchar_t *) wide, box_length (wide) / sizeof (wchar_t) - 1, CHARSET_UTF8,
			      DKS_ESC_PTEXT | (ctx->dks_esc_compat));
			  dk_free_box (wide);
			}
		      dk_free_box (bh_result);
		    }
		...
		static int
		    soap_print_attrs_validating (caddr_t * box, caddr_t * extension, dk_session_t * ses, soap_ctx_t * ctx,
		    caddr_t * err_ret)
		{
		  caddr_t *e_attr_ptr;
		  int attr_inx = 0;
		  ...
		  while (e_attr_ptr)
			...
			SOAP_VALIDATE_ERROR (("22023", "SV060", "A required attribute '%s' can't be found", attr_name))
		      ...
		      if (!strcmp (attr_use, "prohibited") && attr_val_inx)
			SOAP_VALIDATE_ERROR (("22023", "SV061", "A prohibited attribute '%s' was found", attr_name))
			    if (attr_val_inx)
			  {
			    ...
			    if (!proposed_type && !schema_tree)
			      SOAP_VALIDATE_ERROR (("22023", "SV062", "Can't find type mapping of an attribute '%s' of type '%s'",
				      attr_name, attr_type)) if (schema_tree)
				{
				  ...
				  if (NULL == e_ptr)
				    SOAP_VALIDATE_ERROR (("22023", "SV081",
					    "Can't find type mapping of an attribute '%s' of type '%s'", attr_name,
					    attr_type)) e_ptr = xml_find_schema_child (e_ptr, "restriction", 0);

				  if (NULL == e_ptr)
				    SOAP_VALIDATE_ERROR (("22023", "SV082",
					    "An attribute '%s' of type '%s' must be based on restriction", attr_name,
					    attr_type)) type_base = xml_find_schema_attribute (e_ptr, "base");

				  if (NULL == type_base)
				    SOAP_VALIDATE_ERROR (("22023", "SV083",
					    "Cannot resolve base type of attribute '%s' of type '%s', based on restriction",
					    attr_name, attr_type)) soap_box_xev_get_type (type_base, &proposed_type,
					(caddr_t *) & new_schema_tree, 0, 0, 0);

				  if (!proposed_type)
				    SOAP_VALIDATE_ERROR (("22023", "SV084",
					    "Can't find type mapping of the base type for an attribute '%s' of type '%s'",
					    attr_name, attr_type))}
					...
					if (DV_TYPE_OF (attr_value) == DV_DB_NULL)
					  SOAP_VALIDATE_ERROR (("22023", "SV063",
						  "Value for attribute '%s' of type '%s' can't be DB_NULL", attr_name,
						  attr_type)) SES_PRINT (ses, " ");
					if (attr_ns)
					  {
					    if (tns)
					      {
			...
			dk_free_box (tns);
		      ...
		      static int
			  soap_print_qname_val (dk_session_t * out, dtp_t proposed_type, caddr_t val, soap_ctx_t * ctx,
			  caddr_t * err_ret)
		      {
			char *sep;
			...
			return 0;
		      ...
		      static int soap_check_xsd_restriction (caddr_t * restriction, caddr_t box, soap_ctx_t * ctx)
		      {
			return 1;
		      }

		      static int soap_same_ns (const char *str1, const char *str2)
		      {
			char *p1, *p2;
			...
			if (!ctx->literal)	/* RPC encoded */
			...
			else
			  {
				...
				char *ns_ref =
				    ref_type ? soap_wsdl_ns_prefix (ref_type, &(ctx->types_set), NULL, &is_ref_elem) : NULL;
			...
			if (udt)
			  {
				...
				if (!udt_instance_of (udt, check_udt))
				  SOAP_VALIDATE_ERROR (("22023", "SV024",
					  "The PL value (instance of '%s') is not an instance of '%s'", udt->scl_name,
					  check_udt->scl_name));
			      }
			  }
			else
			  {
				...
				if (DV_TYPE_OF (box) != DV_DB_NULL)
				  SOAP_VALIDATE_ERROR (("22023", "SV025",
					  "The PL value (%s) is not an instance of the user defined type '%s'",
					  dv_type_title (DV_TYPE_OF (box)), check_udt->scl_name))
				  else
				  {
				    sql_type_t sqt;
				    sqt.sqt_class = check_udt;
				    type_name = type_ref =
					soap_sqt_to_soap_type (&sqt, check_udt->scl_soap_type, ctx->opts, NULL, NULL);
				    soap_print_tag (tag, ses, type_ref, ctx, 0, elem, qualified, NULL);
#if 1
				    if (ctx->add_type)
				      {
			...
			if (NULL != (e_ptr = xml_find_schema_child (schema_tree, "complexContent", 0)))
			  {
				...
				if (!tag)
				  SOAP_VALIDATE_ERROR (("22023", "SV027", "No element name given for an object of type '%s'",
					  type_ref));
				if (!type_name)
				...
				if (DV_TYPE_OF (box) != DV_DB_NULL)
				  {
				    ...
				    if (!udt && ((DV_TYPE_OF (box) != DV_ARRAY_OF_POINTER ||
						...
						BOX_ELEMENTS (box) < 2 ||
						(BOX_ELEMENTS (box) > 2 && DV_TYPE_OF (((caddr_t *) box)[0]) != DV_COMPOSITE))))
					...
					while (e_ptr)
					  {
						...
						if (!ns && !soap_same_ns (type_ref, elem_ns))
						  SOAP_VALIDATE_ERROR (("22023", "SV086",
							  "Can't resolve namespace of element '%s' of derived type '%s'", elem_name,
							  elem_ns));
						dk_set_push (&ctx->ns, box_dv_short_string (ns));
					      }
					    elem_name = xml_find_schema_attribute (e_ptr, "name");
						...
						if (elem_inx == 0	/* elements can be in any order in the structure!
									   ...
									   DV_ARRAY_OF_POINTER, elem_inx, 2, "soap_print_box_validating") */ )
						  {
						    ...
						    SOAP_VALIDATE_ERROR (("22023", "SV032",
							    "Can't resolve value of element '%s' of type '%s'", elem_name,
							    type_ref));
						  }
					      }
					    else
				...
				if (NULL != (ref = xml_find_schema_attribute (e_ptr1, "ref")))
				  {
					...
					if (!new_box || (ARRAYP (new_box) && BOX_ELEMENTS (new_box) != ix))
					  SOAP_VALIDATE_ERROR (("22023", "SV036",
						  "Error mapping multi-dimensional array of type '%s'", type_ref));
					box = new_box;
				      }
				  }

				if (!a_name || !a_type || !a_min || !a_max)
				  SOAP_VALIDATE_ERROR (("22023", "SV037",
					  "No ranges, item name, or item type is defined for an array of type '%s'", type_ref));
				n_min = atol (a_min);
				if (!strcmp (a_max, "unbounded"))
				  n_max = LONG_MAX;
				else
				...
				if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (box) && DV_TYPE_OF (box) != DV_DB_NULL)
				  SOAP_VALIDATE_ERROR (("22023", "SV038",
					  "An array or null expected as PL value of type '%s', not '%s'", type_ref,
					  dv_type_title (DV_TYPE_OF (box))));

				if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (box)
				    && (BOX_ELEMENTS (box) < (unsigned) n_min || BOX_ELEMENTS (box) > (unsigned) n_max))
				  SOAP_VALIDATE_ERROR (("22023", "SV039", "An array of type '%s' with min=%ld and max=%ld expected",
					  type_ref, n_min, n_max));
				...
				if (ctx->add_type)
				  {
					...
					snprintf (temp, sizeof (temp),
					    ...
					    dim,
					    (is_in_schema ? "xsd" : (soap_wsdl_ns_prefix (a_type, &(ctx->types_set), "wsdl",
							NULL))), extract_last_xml_name_part (a_type));
				      }
				    SES_PRINT (ses, temp);
				    if (ADD_CUSTOM_SCH == ctx->add_schema && type_ref)
				      {
				...
				if (DV_TYPE_OF (box) != DV_DB_NULL)
				  {
					...
					soap_ctx_t ctxn;
					if ((inx - 1) > n_max)
					  SOAP_VALIDATE_ERROR (("22023", "SV077", "Upper limit (%ld) of %s exceeded", n_max,
						  type_ref));
					if (DV_TYPE_OF (elem_entity) == DV_COMPOSITE)
					  SOAP_VALIDATE_ERROR (("22023", "SV079", "Can't map soap_struct to array of type %s",
						  type_ref));
					...
					soap_print_box_validating (elem_entity, a_name, ses, err_ret, a_type, &ctxn, 0, elem_qual,
					    NULL);
				...
				if (DV_TYPE_OF (box) == DV_DB_NULL)
				...
				else
				  {
					...
					BOX_ELEMENTS (box) != 3 || (BOX_ELEMENTS (box) == 3 &&
					  ...
					  if (has_attrs)
					{
					  if (!soap_print_attrs_validating (((caddr_t **) box)[1], extension, ses, ctx, err_ret))
					    SOAP_VALIDATE_ERROR (("22023", "SV047", "Cannot map PL value to attributes of type %s",
						    type_ref));
					}
				      else if (!DV_STRINGP (((caddr_t **) box)[1]))
					...
					!soap_print_scalar_value (value_type, (caddr_t) ((caddr_t **) box)[2], ses, ctx, err_ret))
				...
				new_box = type_name = type_ref =
				    soap_sqt_to_soap_type (&sqt, udt->scl_soap_type, ctx->opts, NULL, NULL);
				if (!IS_UDT_XMLTYPE_SQT (&sqt))
				  {
				    soap_print_tag (tag, ses, type_ref, ctx, 0, elem, qualified, NULL);
				    if (ctx->add_type)
				...
				if (!udt->scl_member_map)
				  SOAP_VALIDATE_ERROR (("22023", "SV049", "Cannot find any member for user defined type %s",
					  udt->scl_name));
				...
				QR_RESET_CTX
				{
				  ...
				  DO_BOX (sql_field_t *, fld, inx, udt->scl_member_map)
				  {
				    ...
				    soap_fld_name = fld->sfl_soap_name ? fld->sfl_soap_name : fld->sfl_name;
				    soap_fld_type =
					soap_sqt_to_soap_type (&(fld->sfl_sqt), fld->sfl_soap_type, ctx->opts, NULL, NULL);
				    value = udt_member_observer (NULL, udi, udt->scl_member_map[inx], inx);
				    if (!IS_UDT_XMLTYPE_SQT (&sqt))
				      soap_print_box_validating (value, soap_fld_name, ses, err_ret, soap_fld_type, ctx, 0,
					  elem_qual, &(fld->sfl_sqt));
				    else if (!strcmp (fld->sfl_name, "xt_ent"))
				      {
					caddr_t szSOAP_XML_TYPE = box_dv_short_string (SOAP_XML_TYPE);
					soap_print_box_validating (value, soap_fld_name, ses, err_ret, szSOAP_XML_TYPE, ctx, 0,
					    elem_qual, &(fld->sfl_sqt));
				...
				if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (box) && DV_TYPE_OF (box) != DV_DB_NULL)
				  SOAP_VALIDATE_ERROR (("22023", "SV071",
					  "An array or null expected as PL value of type '%s', not '%s'", a_type,
					  dv_type_title (DV_TYPE_OF (box))));

				if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (box)
				    && (BOX_ELEMENTS (box) < (unsigned) n_min || BOX_ELEMENTS (box) > (unsigned) n_max))
				  SOAP_VALIDATE_ERROR (("22023", "SV072", "An array of type '%s' with min=%ld and max=%ld expected",
					  a_type, n_min, n_max));
				...
				while (elem_entity)
				  {
				    ...
				    if (DV_TYPE_OF (elem_entity) == DV_COMPOSITE)
				      SOAP_VALIDATE_ERROR (("22023", "SV041", "Can't map soap_struct to array of type %s",
					      type_ref));
			...
			static caddr_t bif_soap_dt_define (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
			{
			  ...
			  int is_null;
			  int is_elm =
			      BOX_ELEMENTS (args) > 3 ? (int) bif_long_or_null_arg (qst, args, 3, "__soap_dt_define", &is_null) : 0;
			  caddr_t udt_name =
			      BOX_ELEMENTS (args) > 4 ? bif_string_or_null_arg (qst, args, 4, "__soap_dt_define") : NULL;
...
#if 0
			static caddr_t bif_soap_operation (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
			{
			  query_instance_t *qi = (query_instance_t *) qst;
			...
			static caddr_t *soap_http_params (query_t * qr, caddr_t * in_params, caddr_t * text, caddr_t * err,
			    soap_ctx_t * ctx)
			{
...
#define SOAP_HTTP
			...
			static void
			    soap_udt_print_schema_fragment (sql_class_t * udt, dk_session_t * out, dk_set_t * types_set, int sp,
			    soap_ctx_t * ctx)
			{
			  sql_type_t sqt;
			  char *udt_soap_name;
			  ...
			  {
			    char *soap_fld_type =
				soap_sqt_to_soap_type (&(fld->sfl_sqt), fld->sfl_soap_type, ctx->opts, udt->scl_name_only,
				fld->sfl_name);
			...
			void bif_soap_init (void)
			{
			  ...
			  bif_define_ex ("soap_find_xml_attribute", bif_soap_find_xml_attribute, BMD_RET_TYPE, &bt_varchar,
			      BMD_DONE);
			  bif_define_ex ("soap_print_box", bif_soap_print_box, BMD_RET_TYPE, &bt_varchar, BMD_DONE);
			  bif_define_ex ("soap_print_box_validating", bif_soap_print_box_validating, BMD_RET_TYPE, &bt_varchar,
			      BMD_DONE);
