...
it_free_remap (index_tree_t * it, dp_addr_t logical, dp_addr_t remap, int dp_flags, oid_t col_id)
{
  ...
  if (remap != logical)
	...
	{
	  em_free_dp (IT_COL_EM (it, col_id), logical, (dp_flags == DPF_INDEX || dp_flags == DPF_HASH
		  || dp_flags == DPF_COLUMN) ? EXT_INDEX : EXT_BLOB);
	}
    }
}
...
next:
  ...
  DO_SET (lock_trx_t *, lt, &all_trxs)
  {
	...
	break;
      default:
	GPF_T1 ("Unexpected lt_status in cpt_rollback");
      }
  }
  END_DO_SET ();
  if (CPT_ATOMIC_PENDING == wi_inst.wi_is_checkpoint_pending)
...
dbs_read_checkpoint_remap (dbe_storage_t * dbs, dp_addr_t from)
{
  ...
  while (from)
	...
	{
	  log_error ("cpt remap page L=%d has flag %d, not DPF_CP_MREMAP", cp_buf->bd_page,
	      SHORT_REF (cp_buf->bd_buffer + DP_FLAGS));
      ...
      for (inx = DP_DATA; inx <= PAGE_SZ - 8; inx += 8)
	{
	  ...
	  if (logical)
		...
		GPF_T1 ("Read remap to zero");
	      sethash (DP_ADDR2VOID (logical), dbs->dbs_cpt_remap, DP_ADDR2VOID (physical));
	    }
	}
...
cpt_write_remap (dbe_storage_t * dbs)
{
  ...
  while (cp_remap_pages)
	...
	{
	  dp_addr_t next_remap = cp_remap_pages->next ? (dp_addr_t) (uptrlong) cp_remap_pages->next->data : 0;
...
cpt_ins_image (buffer_desc_t * buf, int map_pos)
{
  ...
  if (rbe->rbe_next)
    GPF_T1 ("uncommitted insert rbe entry can't have a next");
  if (0 != IE_ROW_VERSION (rbe->rbe_string + rbe->rbe_row))
    GPF_T1 ("insert rb entry bad from the start");
...
void
em_unremap (index_tree_t * it, it_cursor_t * itc, extent_map_t * em, int *bufs_done, int buf_quota, int target,
    dk_set_t * bufs_list, int *bufs_done_total)
{
  for (;;)
    {
...
again:
  ...
  if (end_unremap - start_unremap > 5000)
    log_info
	("Checkpoint removed %d MB of remapped pages, leaving %d MB. Duration %9.4g s.  To save this time, increase MaxCheckpointRemap and/or set Unremap quota to 0 in ini file.",
	(initial_remaps - dbs->dbs_cpt_remap->ht_count) / PAGES_PER_MB, dbs->dbs_cpt_remap->ht_count / PAGES_PER_MB,
	(float) (end_unremap - start_unremap) / 1000);
  ...
  if (0 == cpt_write_remap (dbs))
    {
      log_error ("Checkpoint remap write failed, %ld remap pages", dbs->dbs_cpt_remap->ht_count);
      target = 0;
      goto again;
    }
...
cpt_neodisk_page (const void *key, void *value)
{
  ...
  after_image = (buffer_desc_t *) gethash (DP_ADDR2VOID (logical), &IT_DP_MAP (it_from_g, logical)->itm_dp_to_buf);
  if (logical == physical)
    {
      dp_addr_t cp_remap = (dp_addr_t) (uptrlong) gethash (DP_ADDR2VOID (logical), cpt_dbs->dbs_cpt_remap);
      if (cp_remap)
  ...
  if (after_image)
    {
      if (after_image->bd_is_dirty && logical != physical
...
err_end:
  ...
  if (cpt_recov_file_complete)
	...
	{
	  log_error
	      ("The new registry cannot be written during recov because of no space.  Recovery exits. Make more space available and restart recov");
	  call_exit (-1);
	}
      LEAVE_TXN;
...
dbs_cpt_backup (void)
{
  ...
  DO_SET (dbe_storage_t *, dbs, &wi_inst.wi_master_wd->wd_storage)
  {
    ...
    CATCH_WRITE_FAIL (ses)
    {
	  ...
	  box_num (dbs->dbs_incbackup_set->bd_page), box_num (dbs->dbs_registry), dbs_registry_to_array (dbs));
      print_int (0, ses);
...
dbs_checkpoint (char *log_name, int shutdown)
{
  ...
  if (enable_flush_all)
      ...
      for (ctr = 0; ctr < 2; ctr++)
	{
	  ...
	  if (0 == ctr && (float) dirty_after / (n_dirty + 1) > 0.7)
	    {
	      log_info
		  ("Write load very high relative to disk write throughput.  Flushing at %9.2g MB/s while application is making dirty pages at %9.2g MB/s. To checkpoint the database, will now pause the workload with %d MB unflushed.",
		  (n_dirty / PAGES_PER_MB) / ((float) start / 1000), (dirty_after / PAGES_PER_MB) / ((float) start / 1000),
		  dirty_after / PAGES_PER_MB);
	      break;
	    }
	  if (0 == ctr && (float) dirty_after / (n_dirty + 1) > 0.2)
	    {
	      log_info
		  ("Write load high relative to disk write throughput.  Flushing at %9.2g MB/s while application is making dirty pages at %9.2g MB/s. Doing a second flushing pass before checkpoint",
		  (n_dirty / PAGES_PER_MB) / ((float) start / 1000), (dirty_after / PAGES_PER_MB) / ((float) start / 1000));
	    }
	}
    }
  ...
  WITHOUT_SIGNALS
  {
	...
	fprintf (checkpoint_flag_fd,
	    "If this file exists then a checkpoint started at %d " "and has not finished yet", get_msec_real_time ());
	...
	if (it->it_extent_map && it->it_extent_map->em_remap_on_hold)
	  log_info ("In principle should not have em with remap on hold in cpt. key %s hold %d", it->it_key
	      && it->it_key->key_name ? it->it_key->key_name : "unnamed", it->it_extent_map->em_remap_on_hold);
	for (inx = 0; inx < IT_N_MAPS; inx++)
	  {
	    rdbg_printf (("%s %ld\n", it->it_key ? it->it_key->key_name : "no key", (long) it->it_maps[inx].itm_remap.ht_count));
...
cpt_is_global_lock (lock_trx_t * lt)
{
  ...
  if (owner_lt && lt && (lt == owner_lt || lt->lt_rc_w_id == owner_lt->lt_w_id || lt->lt_main_trx_no == owner_lt->lt_trx_no))
    return 0;
  if (server_lock.sl_owner != THREAD_CURRENT_THREAD)
    return 1;
