...
top_is_better (caddr_t val1, caddr_t val2, int cmp)
{
  ...
  if (DVC_NOORDER == rc || DVC_UNKNOWN == rc)
    {
      dtp_t dtp1 = DV_TYPE_OF (val1);
      dtp_t dtp2 = DV_TYPE_OF (val2);
	...
	return DVC_LESS;
      if (DV_DB_NULL == dtp2)
	return DVC_GREATER;
      dtp1 = dtp_canonical[dtp1];
      dtp2 = dtp_canonical[dtp2];
      rc = NUM_COMPARE (dtp1, dtp2);
    }
...
setp_mem_sort (setp_node_t * setp, caddr_t * qst, int n_sets, int merge_set)
{
  ...
  if ((setp_top_row_limit < top) || (setp_top_row_limit < skip) || (setp_top_row_limit < top + skip))
	...
	"Only %ld are allowed. "
	"Either decrease the offset and/or row count or use a scrollable cursor", (long) (top + skip), setp_top_row_limit);
  ...
  for (set = 0; set < n_sets; set++)
    {
      int set_no =
	  merge_set != -1 ? merge_set : setp->setp_ssa.ssa_set_no ? qst_vec_get_int64 (qst, setp->setp_ssa.ssa_set_no, set) : 0;
      if (prev_set != set_no)
	{
	  qi->qi_set = set_no;
...
setp_filled (setp_node_t * setp, caddr_t * qst)
{
  ...
  if (HA_GROUP == ha->ha_op || HA_ORDER == ha->ha_op || HA_DISTINCT == ha->ha_op || HA_FILL == ha->ha_op)
    {
...
union_node_input (union_node_t * un, caddr_t * inst, caddr_t * state)
{
  ...
  for (;;)
      ...
      if (!out_list)
	{
      ...
      if (!un->src_gen.src_query->qr_cl_run_started || CL_RUN_LOCAL == cl_run_local_only || un->uni_sequential)
...
subq_node_input (subq_source_t * sqs, caddr_t * inst, caddr_t * state)
{
  ...
  for (;;)
      ...
      else
	{
      ...
      if (err == (caddr_t) SQL_NO_DATA_FOUND && !any_passed && sqs->sqs_is_outer)
	{
	  ...
	  END_DO_BOX;
	  qn_ts_send_output ((data_source_t *) sqs, inst, sqs->sqs_after_join_test);
	  return;
	}

      if (err == SQL_SUCCESS)
	{
	  if (!sqs->src_gen.src_after_test || code_vec_run (sqs->src_gen.src_after_test, inst))
	    {
	      any_passed = 1;
	      qn_ts_send_output ((data_source_t *) sqs, inst, sqs->sqs_after_join_test);
	    }
...
breakup_node_input (breakup_node_t * brk, caddr_t * inst, caddr_t * state)
{
  ...
  for (;;)
	...
	SRC_IN_STATE ((data_source_t *) brk, inst) = NULL;
      if (current > n_total)
	return;
      DO_BOX (state_slot_t *, out, oinx, brk->brk_output) dc_reset (QST_BOX (data_col_t *, inst, out->ssl_index));
      END_DO_BOX;
      for (set = 0; set < n_sets; set++)
	{
	  qi->qi_set = set;
