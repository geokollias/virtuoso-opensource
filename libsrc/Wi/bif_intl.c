...
collation_define_memonly (caddr_t name, caddr_t table, int is_utf8_if_narrow)
{
  ...
  if (is_narrow && is_utf8_if_narrow)
    {
      ...
      if (raw_tbl_len <= 0)
	{
	  log_error ("Error in collation %s definition: invalid encoding"
      ...
      for (ctr = 0; ctr < raw_tbl_len; ctr++)
	{
	  ...
	  if (read < 0)
	    {
	      log_error ("Error in collation %s definition: invalid encoding at or near offset %ld. "
  ...
  if ((tbl_len & 0xff) && (tbl_len != 0xffff))
    log_warning
	("Collation %s is defined as a table of length %d, that is formally valid but unusual. The length is usually a whole multiple of 256",
	name);
...
collation_order_string (collation_t * coll, caddr_t string, int auto_utf8)
{
  ...
  if (auto_utf8 &&
      ((DV_UNAME == string_dtp) || ((DV_STRING == string_dtp) && ((auto_utf8 & 2) || ((BF_IRI | BF_UTF8) & box_flags (string))))))
    {
...
caddr_t
charset_recode_from_named_to_named (caddr_t narrow, const char *cs1_uppercase, const char *cs2_uppercase, int *res_is_new_ret,
    caddr_t * err_ret)
{
  wcharset_t *cs1, *cs2;
  ...
  if (!cs1 && cs1_uppercase && box_length (cs1_uppercase) > 1)
	...
	{
	  err_ret[0] = srv_make_new_error ("2C000", "IN000", "UTF-16 specified, but no byte-order-mask is given");
	  return NULL;
	}
      else
	eh_cs1 = eh_get_handler (cs1_uppercase);
      if (!eh_cs1)
	{
	  err_ret[0] = srv_make_new_error ("2C000", "IN007", "Charset %s not defined", cs1_uppercase);
	  return NULL;
	}
    }
...
caddr_t
charset_recode_from_cs_or_eh_to_cs (caddr_t narrow, int bom_skip_offset, encoding_handler_t * eh_cs1, wcharset_t * cs1,
    wcharset_t * cs2, int *res_is_new_ret, caddr_t * err_ret)
{
  int inx, to_free = 0;
  caddr_t ret = NULL;
  ...
  if ((DV_UNAME == dtp) && (cs1 != CHARSET_UTF8))
    {
      err_ret[0] =
	  srv_make_new_error ("2C000", "IN016",
	  "Function got a UNAME argument and the source encoding is not UTF-8; this is illegal because UNAMEs are always UTF-8");
      return NULL;
    }
  if (IS_WIDE_STRING_DTP (dtp) && cs1 != CHARSET_WIDE)
    {
      err_ret[0] = srv_make_new_error ("2C000", "IN012", "Narrow source charset specified, but the supplied string is wide");
      return NULL;
    }
  if (IS_STRING_DTP (dtp) && cs1 == CHARSET_WIDE)
    {
      err_ret[0] = srv_make_new_error ("2C000", "IN013", "Wide source charset specified, but the supplied string not wide");
      return NULL;
    }
...
bif_bf_text_to_UTF8 (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!narrow)
    return NEW_DB_NULL;
  expected_bf_if_zero =
      ((NULL == cs_name) ? ((CHARSET_UTF8 == default_charset) ? BF_UTF8 : 0) : (!strcasecmp (cs_name, "UTF-8") ? BF_UTF8 : 0));
...
bif_uname (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (BOX_ELEMENTS (args))
    ...
    default:
    case 3:
      allow_long = bif_long_arg (qst, args, 2, "uname");
    case 2:
      cs1_name = bif_string_or_null_arg (qst, args, 1, "uname");
    case 1:
      if (DV_UNAME == dtp)
	return box_copy (narrow);
      break;
  ...
  if (DV_UNAME == dtp)
    {
      if (cs1 != CHARSET_UTF8)
	sqlr_new_error ("2C000", "IN016",
	    "Function uname() got a UNAME argument and the source encoding is not UTF-8; this is illegal because UNAMEs are always UTF-8");
      return box_copy (narrow);
    }
  if (IS_WIDE_STRING_DTP (dtp) && cs1 != CHARSET_WIDE)
...
bif_unicode_char_properties (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (ub_idx = raw_uniblocks_fill; ub_idx--; /* no step */ )
	...
	continue;
      dk_set_push (&res,
	  list (4, box_dv_short_string (ub->ub_descr), box_num (ub->ub_props), box_num (ub->ub_min), box_num (ub->ub_max)));
    }
...
static int
eh_decode_buffer__wcharset_narrow (unichar * tgt_buf, int tgt_buf_len, __constcharptr * src_begin_ptr, const char *src_buf_end, ...)
{
  int res = 0;
  va_list tail;
  encoding_handler_t *my_eh;
...
static int
eh_decode_buffer__wcharset_wide (unichar * tgt_buf, int tgt_buf_len, __constcharptr * src_begin_ptr, const char *src_buf_end, ...)
{
  int res = 0;
  va_list tail;
  encoding_handler_t *my_eh;
...
char *
eh_encode_char__wcharset_narrow (unichar char_to_put, char *tgt_buf, char *tgt_buf_end, ...)
{
...
char *
eh_encode_char__wcharset_wide (unichar char_to_put, char *tgt_buf, char *tgt_buf_end, ...)
{
...
static char *
eh_encode_buffer__wcharset_narrow (const unichar * src_buf, const unichar * src_buf_end, char *tgt_buf, char *tgt_buf_end, ...)
{
  va_list tail;
  encoding_handler_t *my_eh;
  wcharset_t *my_charset;
...
static char *
eh_encode_buffer__wcharset_wide (const unichar * src_buf, const unichar * src_buf_end, char *tgt_buf, char *tgt_buf_end, ...)
{
  va_list tail;
  encoding_handler_t *my_eh;
  wcharset_t *my_charset;
  ...
  while ('\0' != tail[0])
    {
      ...
      double q = 1.0;
      key = tail;
      while (isalnum (tail[0]) || ('-' == tail[0]) || ('/' == tail[0]) || ('*' == tail[0]))
	tail++;
      key_end = tail;
      if (key_end == key)
	goto garbage_after_q;
      TAIL_SKIP_WS;
      if (';' != tail[0])
	goto garbage_after_q;
      tail++;
