...
sqlo_eq_hash_print (id_hash_t * eq_ht)
{
  ...
  {
    dbg_print_box ((caddr_t) col, stdout);
    printf (" = ");
    DO_SET (df_elt_t *, eq, &eqs)
    {
      dbg_print_box ((caddr_t) eq->dfe_tree, stdout);
      printf (" ");
    }
    END_DO_SET ();
    printf ("\n");
...
sqlo_clear_eqs (id_hash_t * eqs)
{
  ...
  while (hit_next (&hit, (caddr_t *) & ign, (caddr_t *) & place))
    *place = NULL;
}
...
dfe_trans_rewrite (sqlo_t * so, df_elt_t * tb_dfe, df_elt_t * pred)
{
  ...
  if (DFE_BOP == pred->dfe_type && pred->_.bin.right)
    {
      df_elt_t *left = pred->_.bin.left;
...
dfe_reqd_placed_tb (df_elt_t * dfe, df_elt_t * tb)
{
  ...
  DO_SET (op_table_t *, req, &dfe->dfe_tables)
  {
    if (req == tb->_.table.ot)
      tb_found = 1;
    else if (req->ot_dfe || !req->ot_dfe->dfe_is_placed)
      return 0;
  }
  END_DO_SET ();
  return tb_found;
...
sqlo_tb_inx_intersectable (sqlo_t * so, df_elt_t * tb_dfe, df_elt_t * joined, int n_eqs)
{
  ...
  DO_SET (dbe_key_t *, key, &joined->_.table.ot->ot_table->tb_keys)
  {
	...
	if (n_eqs2 && key->key_n_significant - n_eqs2 == tb_dfe->_.table.key->key_n_significant - n_eqs
	    && key->key_n_significant > n_eqs2 && (!key->key_is_bitmap || n_eqs2 == key->key_n_significant - 1))
	  {
		...
		df_elt_t *col_dfe =
		    sqlo_df (so, (ST *) t_list (3, COL_DOTTED, tb_dfe->_.table.ot->ot_new_prefix, tb_col->col_name));
		df_elt_t *joined_col_dfe =
		    sqlo_df (so, (ST *) t_list (3, COL_DOTTED, joined->_.table.ot->ot_new_prefix, joined_col->col_name));
...
sqlo_try_inx_int_joins (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * group_ret, float *best_group)
{
  ...
  if (!inx_int_join || (so->so_sc->sc_cc->cc_query && so->so_sc->sc_cc->cc_query->qr_proc_vectored))
    return;
  if (tb_dfe->_.table.is_unique || tb_dfe->_.table.ot->ot_rds || tb_dfe->_.table.index_path)
    return;
  ...
  DO_SET (df_elt_t *, joined, &ot->ot_from_dfes)
  {
	...
	&& !joined->_.table.ot->ot_is_outer && !joined->_.table.ot->ot_rds && sqlo_opts_inx_int (joined->_.table.ot->ot_opts))
      {
...
void
sqlo_place_inx_int_join (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t group, dk_set_t * after_preds)
{
