...
key_vec_insert (insert_node_t * ins, caddr_t * qst, it_cursor_t * itc, ins_key_t * ik)
{
  ...
  if (ins->src_gen.src_prev)
    ...
    n_rows = qi->qi_n_sets;
  if (n_rows > 81860 / 400)
    ins_mp->mp_block_size = chash_block_size;
	  ...
	  if (col->col_sqt.sqt_non_null && DV_DB_NULL == val_dtp)
	    {
	      ...
	      sqlr_new_error ("22026", "INULL", "Insert of null for non null column %s , key %s", col->col_name, key->key_name);
	    }
	  if (key->key_not_null && !col->col_sqt.sqt_non_null && icol < key->key_n_significant && DV_DB_NULL == val_dtp)
	    {
	      rds[inx] = (row_delta_t *) - 1;
	      null_skipped = 1;
	      goto next_rd;
