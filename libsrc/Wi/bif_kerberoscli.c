...
srv_make_gss_error (OM_uint32 maj_stat, OM_uint32 min_stat, char *virt_code)
{
  ...
  while (!msg_ctx)
    {
      gmaj_stat = gss_display_status (&gmin_stat, maj_stat, GSS_C_GSS_CODE, GSS_C_NULL_OID, &msg_ctx, &msg);
      if ((gmaj_stat == GSS_S_COMPLETE) || (gmaj_stat == GSS_S_CONTINUE_NEEDED))
	{
  ...
  while (!msg_ctx)
    {
      gmaj_stat = gss_display_status (&gmin_stat, min_stat, GSS_C_MECH_CODE, GSS_C_NULL_OID, &msg_ctx, &msg);
      if ((gmaj_stat == GSS_S_COMPLETE) || (gmaj_stat == GSS_S_CONTINUE_NEEDED))
	{
	  strncat_ck (all_error_text, (char *) msg.value, 100);
	  strcat_ck (all_error_text, " ");
...
kerberos_client_auth (long my_gcontext, caddr_t my_tok, caddr_t server_tok, caddr_t service)
{
  ...
  if (my_tok)
    {
      ...
      maj_stat = gss_import_name (&min_stat, &temp_null_tok, gss_nt_service_name, &target_name);
  ...
  maj_stat = gss_init_sec_context (&min_stat, GSS_C_NO_CREDENTIAL, &gcontext, target_name, mech_type,
/*	                          GSS_C_MUTUAL_FLAG | GSS_C_REPLAY_FLAG | GSS_C_DELEG_FLAG,*/
      GSS_C_MUTUAL_FLAG | GSS_C_REPLAY_FLAG, 0, &chan,	/* channel bindings */
      token_ptr, NULL,		/* ignore mech type */
      &send_tok, NULL,		/* ignore ret_flags */
      NULL);
...
kerberos_server_auth (caddr_t in_tok, caddr_t service_name)
{
  ...
  acquire_maj = gss_acquire_cred (&acquire_min, server_name, 0, GSS_C_NULL_OID_SET, GSS_C_ACCEPT, &server_creds, NULL, NULL);
  ...
  accept_maj = gss_accept_sec_context (&accept_min,
      &gcontext, server_creds, &tok, &chan, &client, &mechid, &out_tok, &ret_flags, NULL, &deleg_creds);
...
bif_kerberos_get_tgt (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  krb5_get_init_creds_password (k5.ctx, &my_creds, k5.me, krb_password, NULL, 0, 0, NULL, &options);
...
kerberos_encrypt_tst (long mode, char *message, char *hostname, char *service)
{
  ...
  if ((retval = krb5_mk_req (context, &auth_context, 0, service, full_hname, &inbuf, ccdef, &packet)))
    {
...
kerberos_decrypt_tst (int mode, caddr_t auth_tok, caddr_t mess_tok, caddr_t service)
{
  ...
  if ((retval = krb5_sname_to_principal (context, NULL, service, KRB5_NT_SRV_HST, &sprinc)))
    {
  ...
  if ((retval = krb5_rd_req (context, &auth_context, &packet, sprinc, keytab, NULL, &ticket)))
    {
...
kerberos_get_tiket_info (caddr_t auth_tok, caddr_t service, dk_set_t * ret)
{
  ...
  /* Check authentication info */
  if (krb5_rd_req (context, &auth_context, &packet, sprinc, keytab, NULL, &ticket))
    {
      sqlr_new_error ("42000", "KRBXX", "while check authentication info");
    }
...
krb_unseal (gss_ctx_id_t context, caddr_t in_buf_cont)
{
  ...
  GSS_TRY (maj_stat = gss_unwrap (&min_stat, context, &in_buf, &out_buf, &conf_state, NULL), "KRBXX");
...
krb_seal (gss_ctx_id_t context, caddr_t in_buf_cont)
{
  ...
  GSS_TRY (maj_stat = gss_wrap (&min_stat, context, 0, GSS_C_QOP_DEFAULT, &in_buf, &conf_state, &out_buf), "KRBXX");
...
krb_init_ctx (char *service_name, gss_ctx_id_t * context, caddr_t * tkt)
{
  ...
  GSS_TRY (maj_stat = gss_import_name (&min_stat, &send_tok, (gss_OID) gss_nt_service_name, &target_name), "42000");
  ...
  *context = GSS_C_NO_CONTEXT;
  ...
  GSS_TRY (maj_stat = gss_acquire_cred (&min_stat, GSS_C_NO_NAME, 0,
	  GSS_C_NULL_OID_SET, GSS_C_INITIATE, &creds, NULL, NULL), "42000");

  GSS_TRY1 (maj_stat = gss_init_sec_context (&min_stat, creds /*GSS_C_NO_CREDENTIAL */ ,
	  context, target_name, oid, GSS_C_SEQUENCE_FLAG | GSS_C_REPLAY_FLAG | GSS_C_TRANS_FLAG /*| GSS_C_MUTUAL_FLAG */ ,
	  0, NULL,		/* no channel bindings */
	  token_ptr, NULL,	/* ignore mech type */
	  &send_tok, &ret_flags, NULL), "42000");	/* ignore time_rec */
...
krb_init_srv_ctx (caddr_t service_name, caddr_t tkt, gss_ctx_id_t * context)
{
  ...
  GSS_TRY (maj_stat = gss_import_name (&min_stat, &name_buf, (gss_OID) gss_nt_service_name, &server_name), "42000");

  GSS_TRY (maj_stat = gss_acquire_cred (&min_stat, server_name, 0,
	  GSS_C_NULL_OID_SET, GSS_C_ACCEPT, &server_creds, NULL, NULL), "42000");

  *context = GSS_C_NO_CONTEXT;

  GSS_TRY (maj_stat = gss_accept_sec_context (&min_stat, context, server_creds, &rec_tkt, GSS_C_NO_CHANNEL_BINDINGS, &client, &doid, &send_tok, &ret_flags, NULL,	/* ignore time_rec */
...
_krb_init_srv_ctx (caddr_t service_name, caddr_t tkt, gss_ctx_id_t * context)
{
  ...
  _GSS_TRY (maj_stat = gss_import_name (&min_stat, &name_buf, (gss_OID) gss_nt_service_name, &server_name), "42000");
  ...
  *context = GSS_C_NO_CONTEXT;
  ...
  _GSS_TRY (maj_stat = gss_accept_sec_context (&min_stat, context, server_creds, &rec_tkt, GSS_C_NO_CHANNEL_BINDINGS, &client, &doid, &send_tok, &ret_flags, NULL,	/* ignore time_rec */
	  NULL), "42000");	/* ignore del_cred_handle */
...
bif_krb_inquire_ctx (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  GSS_TRY (maj_stat = gss_inquire_context (&min_stat, context,
	  &src_name, &targ_name, &lifetime, &mechanism, &context_flags, &is_local, &is_open), "42000");

  GSS_TRY (maj_stat = gss_display_name (&min_stat, src_name, &sname, &name_type), "42000");

  GSS_TRY (maj_stat = gss_display_name (&min_stat, targ_name, &tname, (gss_OID *) NULL), "42000");
