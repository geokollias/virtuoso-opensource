...
itc_box_base_uri_column_impl (it_cursor_t * itc, dbe_key_t * key, dtp_t * row, oid_t xml_col_id)
{
  ...
  if (DV_STRING == DV_TYPE_OF (local_uri))
    {
      caddr_t res =
	  dk_alloc_box (11 + strlen (tb->tb_name) + strlen (xml_col->col_name) + strlen (local_uri) +
	  strlen (base_uri_col->col_name), DV_STRING);
      /*            |0              1   */
      /*            |1234567..8..9..0..1 */
      snprintf (res, box_length (res), "virt://%s.%s.%s:%s", tb->tb_name, base_uri_col->col_name, xml_col->col_name, local_uri);
...
page_box_col (it_cursor_t * itc, buffer_desc_t * buf, db_buf_t row, dbe_col_loc_t * cl)
{
  ...
  if (itc && cl == row_key->key_bit_cl && !itc->itc_no_bitmap)
    {
      ...
      if (DV_LONG_INT == itc->itc_row_key->key_bit_cl->cl_sqt.sqt_dtp || DV_INT64 == itc->itc_row_key->key_bit_cl->cl_sqt.sqt_dtp)
  ...
  switch (col_dtp)
	...
	if (DV_BLOB_XPER_HANDLE == DV_TYPE_OF (bh))
	  {
	    ...
	    caddr_t val =
		(caddr_t) xper_entity (qi, bh, NULL, 0, itc_box_base_uri_column (itc, buf->bd_buffer, cl->cl_col_id),
		NULL /* no enc */ , &lh__xany, NULL /* DTD config */ , 1);
	    dk_free_box (bh);
	    return val;
	  }
#endif
	...
	if (cl->cl_sqt.sqt_is_xml && itc->itc_out_state)
	  {
	    caddr_t res =
		xml_deserialize_from_blob (bh, itc->itc_ltrx, itc->itc_out_state, itc_box_base_uri_column (itc, buf->bd_buffer,
		    cl->cl_col_id));
	    dk_free_box (bh);
	    return res;
	  }
	...
	query_instance_t *qi = (query_instance_t *) (itc->itc_out_state);
	caddr_t val =
	    (caddr_t) xper_entity (NULL, bh, NULL, 0, itc_box_base_uri_column (itc, buf->bd_buffer, cl->cl_col_id),
	    NULL /* no enc */ , &lh__xany, NULL /* DTD config */ , 1);
	dk_free_box (bh);
	return val;
      }
...
qst_set_wide_string (caddr_t * state, state_slot_t * sl, db_buf_t data, int len, int isUTF8)
{
  ...
  if (sl->ssl_index < QI_FIRST_FREE)
      ...
      if (IS_BOX_POINTER (old))
	{
	  ...
	  if (!isUTF8 && IS_WIDE_STRING_DTP (old_dtp) && box_length (old) == len + sizeof (wchar_t))
	    {
	  ...
	  else
	    {
	      ...
	      *place = isUTF8 ? box_utf8_as_wide_char ((caddr_t) data, NULL, len, 0) : box_wide_char_string ((caddr_t) data, len);
	    }
	}
      else
	*place = isUTF8 ? box_utf8_as_wide_char ((caddr_t) data, NULL, len, 0) : box_wide_char_string ((caddr_t) data, len);
...
qst_set_bin_string (caddr_t * state, state_slot_t * sl, db_buf_t data, size_t len, dtp_t dtp)
{
  ...
  if (sl->ssl_index < QI_FIRST_FREE)
	...
	{
	  dtp_t old_dtp = box_tag (old);
	  if (!IS_STRING_DTP (old_dtp) && ALIGN_8 (box_length (old)) == ALIGN_8 ((uint32) len))
	    {
...
void
itc_qst_set_column (it_cursor_t * it, buffer_desc_t * buf, dbe_col_loc_t * cl, caddr_t * qst, state_slot_t * target)
{
  float fl;
  double df;
  int32 len;
  ...
  switch (col_dtp)
	...
	if (DV_BLOB_XPER_HANDLE == DV_TYPE_OF (bh))
	  {
	    caddr_t val = (caddr_t) xper_entity ((query_instance_t *) QST_INSTANCE (qst),
		bh, NULL, 0, itc_box_base_uri_column_impl (it, it->itc_row_key, row, cl->cl_col_id), NULL /* no enc */ ,
		server_default_lh, NULL /* DTD config */ , 1);
	    dk_free_box (bh);
	...
	if (cl->cl_sqt.sqt_is_xml && qst)
	  {
	    caddr_t res =
		xml_deserialize_from_blob (bh, it->itc_ltrx, qst, itc_box_base_uri_column_impl (it, it->itc_row_key, row,
		    cl->cl_col_id));
	    dk_free_box (bh);
	    qst_set (qst, target, res);
	    return;
...
safe_atof (const char *data, caddr_t * err_ret)
{
  ...
  if (eno_save == ERANGE || !end_ptr || *end_ptr)
    {
    error:
      err = srv_make_new_error ("22023", "SR334", "Invalid floating point value converting '%.100s'", data);
      if (!err_ret)
	sqlr_resignal (err);
...
box_to_boxint (caddr_t data, dtp_t dtp, oid_t col_id, caddr_t * err_ret, dbe_key_t * key, dtp_t col_dtp)
{
  ...
  if (DV_INT64 == col_dtp)
    return res;
  if (DV_LONG_INT == col_dtp && INT32_MAX >= res && INT32_MIN <= res)
    return res;
 ...
 */
caddr_t
row_set_col_cast (caddr_t data, sql_type_t * tsqt, caddr_t * err_ret, oid_t col_id, dbe_key_t * key, caddr_t * qst)
{
  ...
  if (dtp != tsqt->sqt_dtp && dtp != DV_DB_NULL)
	...
	case DV_NUMERIC:
	  {
	    NUMERIC_VAR (n);
	    *err_ret = numeric_from_x ((numeric_t) n, data, tsqt->sqt_precision, tsqt->sqt_scale, 0, col_id, key);
	...
	case DV_TIMESTAMP:
	  if (DV_DATETIME != dtp)
	    {
	      res = box_cast_to (qst, data, dtp, tsqt->sqt_dtp, 0, 0, err_ret);
	    }
	  break;
	}
    }
...
void
row_insert_cast (row_delta_t * rd, dbe_col_loc_t * cl, caddr_t data, caddr_t * err_ret, db_buf_t old_blob)
{
  ...
  if (DV_DB_NULL == dtp)
      ...
      if (!cl->cl_null_mask[0])
	{
	  ...
	  if (key->key_table)
	    ...
	    {
	      *err_ret = srv_make_new_error ("23000", "SR133",
		  "Can not set NULL to not nullable column '%.*s'", MAX_NAME_LEN, cl_name);
	    }
	  return;
	}
  ...
  switch (cl->cl_sqt.sqt_col_dtp)
    ...
    case DV_NUMERIC:
      {
	NUMERIC_VAR (n);
	*err_ret = numeric_from_x ((numeric_t) n, data, cl->cl_sqt.sqt_precision, cl->cl_sqt.sqt_scale, 0, cl->cl_col_id, key);
	if (!*err_ret)
	  {
	    dtp_t tmp[258];
    ...
    assign_str:
      ...
      if (cl->cl_sqt.sqt_precision && len > (long) cl->cl_sqt.sqt_precision)
	{
	  if (key->key_table)
	    {
	      char *cl_name = __get_column_name (cl->cl_col_id, key);
	      *err_ret = srv_make_new_error ("22026", "SR319",
		  ...
		  (unsigned long) cl->cl_sqt.sqt_precision, 3 * MAX_NAME_LEN, key->key_table->tb_name, MAX_NAME_LEN, cl_name);
	    }
	  else
	    *err_ret = srv_make_new_error ("22026", "SR319",
		"Max column length (%lu) of temp column  exceeded", (unsigned long) cl->cl_sqt.sqt_precision);
	  return;
	}
      if (len + *v_fill > max)
	{
	  if (key->key_table)
	    ...
	    *err_ret = srv_make_new_error ("22026", "SR319",
		"Max row length is exceeded when trying to store a string of %d chars into a temp col", len);
	  return;
	}
      *v_fill += len;
      if (ins_itc)
	...
	*v_fill += len;
	...
	{
	convert_dt:
	  data = box_cast_to (ins_itc ? ins_itc->itc_out_state : NULL, data, dtp, cl->cl_sqt.sqt_col_dtp, 0, 0, err_ret);
	  if (ins_itc)
	    ITC_OWNS_PARAM (ins_itc, data);
	  if (*err_ret)
	    return;
	...
	if (cl->cl_sqt.sqt_class)
	  ...
	  utd_in_blob_error:
	    ...
	    *err_ret = srv_make_new_error ("23000",
		"SR353",
		"Type mismatch inserting user defined type instance " "as a blob for column [%.*s]", MAX_NAME_LEN, cl_name);
	    return;
	...
	if (!data)
	  {
		...
		*err_ret = srv_make_new_error ("23000", "SR246",
		    ...
		    MAX_NAME_LEN, dv_type_title (dtp), 3 * MAX_NAME_LEN, key->key_table->tb_name, MAX_NAME_LEN, cl_name);
	      }
	    else
	      *err_ret = srv_make_new_error ("23000", "SR246",
		  "Type mismatch inserting %.*s as a blob for temp column", MAX_NAME_LEN, dv_type_title (dtp));
	    return;
	  }
	...
	if (cl->cl_sqt.sqt_is_xml && XE_IS_VALID_VALUE_FOR_XML_COL (data))
	  {
		...
		tree1 =
		    xml_make_mod_tree ((query_instance_t *) rd->rd_qst, data, err_ret, GE_XML, NULL, CHARSET_NAME (charset, NULL),
		    server_default_lh, dtd_config, &dtd, NULL, NULL);
		if (!tree1)
		  return;
		data_are_temp = 1;
	      }
	...
	*err_ret = srv_make_new_error ("23000", "SR326",
	    "Column data type %.*s not supported [%.*s]", MAX_NAME_LEN, dtp_name, MAX_NAME_LEN, cl_name);
      }
      break;
    }
  return;
IN009:
  {
    char *cl_name = __get_column_name (cl->cl_col_id, key);
    *err_ret = srv_make_new_error ("22005", "IN009", "Max row length of column [%.*s] exceeded", MAX_NAME_LEN, cl_name);
    return;
  }
...
void
row_insert_cast_temp (row_delta_t * rd, dbe_col_loc_t * cl, caddr_t data, caddr_t * err_ret, db_buf_t old_blob)
{
	...
	case DV_BLOB_XPER_HANDLE:
	  {
	    ...
	    if ((int) (DV_BLOB_LEN + *v_fill) > max)
	      {
		*err_ret = srv_make_new_error ("22026", "SR319", "Max row length of column [temp] exceeded");
		return;
	      }
	...
	default:
	  {
	    ...
	    if (IS_INLINEABLE_DTP (cdtp) && (IS_STRING_OR_BIN_DTP (dtp) && box_length (data) <= space))
	      {
...
itc_delete_blob_search_pars (it_cursor_t * itc, row_delta_t * rd)
{
  ...
  {
    if (IS_BLOB_DTP (cl->cl_sqt.sqt_dtp)
	&& DV_STRING == DV_TYPE_OF (rd->rd_values[cl->cl_nth]) && IS_BLOB_DTP (((db_buf_t) rd->rd_values[cl->cl_nth])[0]))
      {
...
key_insert (insert_node_t * ins, caddr_t * qst, it_cursor_t * it, ins_key_t * ik)
{
  ...
  if (DVC_MATCH == rc)
	...
	case INS_SOFT:
	  ...
	  else if (key->key_table->tb_any_blobs)
...
void
rd_col_change (it_cursor_t * itc, buffer_desc_t * buf, row_delta_t * rd, dbe_column_t * col, int is_drop,
    buffer_desc_t ** col_buf_ret, caddr_t deflt)
{
  caddr_t *values;
  if (is_drop)
    {
...
key_find_col (dbe_key_t * key, caddr_t find_name, int is_dropped)
{
  DO_SET (dbe_column_t *, col, &key->key_parts)
  {
	...
	if (num_len > len)
	  continue;
	if (0 == strncmp (col->col_name + len - num_len, num, num_len))
	  {
...
numeric_from_x (numeric_t res, caddr_t x, int prec, int scale, char *col_name, oid_t cl_id, dbe_key_t * key)
{
  ...
  switch (dtp)
	...
	return srv_make_new_error ("22018", "SR126", "Can't convert %.*s to numeric", MAX_NAME_LEN, col_name);
      }
    }
  if (rc != NUMERIC_STS_SUCCESS)
    {
      if (!col_name)
	col_name = __get_column_name (cl_id, key);
      return srv_make_new_error ("22003", "SR127", "Can't convert %.*s to numeric", MAX_NAME_LEN, col_name);
    }
  if (NUMERIC_STS_SUCCESS != numeric_rescale ((numeric_t) res, (numeric_t) tnum, prec, scale))
    {
      if (!col_name)
	col_name = __get_column_name (cl_id, key);
      return srv_make_new_error
	  ("22003", "SR128", "Numeric value out of range for %.*s(%d, %d)", MAX_NAME_LEN, col_name, prec, scale);
    }
  return NULL;
}
...
{
  if (key && (key->key_id == KI_TEMP || !key->key_table || (key->key_storage && key->key_storage->dbs_type == DBS_TEMP)))
    return "<temporary space column>";
  else
    {
      dbe_column_t *col = (dbe_column_t *) gethash ((void *) (ptrlong) col_id, wi_inst.wi_schema->sc_id_to_col);
      if (!col)
	return "<unknown>";
      return col->col_name;
    }
