...
collect_listeners (co_req_2_t * req)
{
  ...
  {
    if (SESSTAT_ISSET (peer->dks_session, SST_LISTENING) && !PrpcIsListen (peer))
      {
...
bif_sys_lockdown (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (lockdown && !lockdown_mode)
	...
	this_client_ses = NULL;
	  ...
	  disconnect_mode ? "OFF" : "UNSERVED", qi->qi_client->cli_user ? qi->qi_client->cli_user->usr_name : "<internal>", buffer);
      lockdown_mode = 1;
#ifndef NDEBUG
      if (listeners)
...
bif_os_chown (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
#elif defined (WIN32)
  {
	...
	LookupAccountName (NULL, group, (PSID) group_sid, &group_sid_sz, dom2, &d2, &use_group))
      {
	if (ERROR_SUCCESS == SetNamedSecurityInfo (fname_cvt, SE_FILE_OBJECT,
		OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION, (PSID) user_sid, (PSID) group_sid, NULL, NULL))
	  res = NEW_DB_NULL;
      }
    if (!res)
      {
	LPVOID lpMsgBuf;
	if (FormatMessage (FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, GetLastError (), MAKELANGID (LANG_NEUTRAL, SUBLANG_DEFAULT),	/* Default language */
...
bif_client_attr (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!stricmp ("client_protocol", mode))
	...
	return box_num (1);
#else
      sqlr_new_error ("22005", "SR403",
	  "'client_ssl' value of client_attr option is not supported by this build of the Virtuoso server");
      return NULL;
#endif
    }
...
#else
      sqlr_new_error ("22005", "SR403",
	  "'client_certificate' value of client_attr option is not supported by this build of the Virtuoso server");
      return NULL;
#endif
    }
...
/* Here we know SS Nn pp pp qQ FF  t */
  if (('/' == iri[split_ret->netloc_begin]) && ('/' == iri[split_ret->netloc_begin + 1]))
	...
	{
	  split_ret->netloc_end = split_ret->path_begin = split_ret->path_end = split_ret->params_begin = split_ret->params_end =
	      split_ret->query_begin = split_ret->query_end;
	  return;
	}
    }
  else
...
bif_rfc1808_parse_uri (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (SMALLEST_POSSIBLE_POINTER <= uri_len)
	...
	sqlr_new_error ("22023", "SR570",
	    "Function rfc1808_parse_uri() got abnormally long URI as argument (%ld chars, '%.50s ... %50s')", (long) (uri_len - 1),
	    uri, uri + uri_len - 51);
    }
  if (DV_WIDE == uri_dtp)
  ...
  if (DV_WIDE == uri_dtp)
    ...
    {
      box_t (*box_x_nchars) (const char *buf, size_t len) = ((DV_UNAME == uri_dtp) ? box_dv_uname_nchars : box_dv_short_nchars);
      return list (6,
...
rfc1808_expand_uri ( /*query_instance_t *qi, */ ccaddr_t base_uri, ccaddr_t rel_uri,
    ccaddr_t output_cs_name, int do_resolve_like_http_get, ccaddr_t base_string_cs_name,	/* Encoding used for base_uri IFF it is a narrow string, neither DV_UNAME nor WIDE */
    ...
    caddr_t * err_ret)
{
  ...
  switch (base_uri_dtp)
    ...
    case DV_UNAME:
      ...
      break;
    case DV_STRING:
      base_cs = base_string_cs_name;
      break;
    default:
      base_cs = buffer_cs_upcase;
      break;
    }
  switch (rel_uri_dtp)
    {
    ...
    case DV_UNAME:
      ...
      break;
    case DV_STRING:
      rel_cs = rel_string_cs_name;
      break;
    default:
      rel_cs = buffer_cs_upcase;
      break;
    }
  ...
  if ((NULL == rel_uri) || (('\0' == rel_uri[0]) && (NULL == strchr (base_uri, '#')) && (NULL == strchr (base_uri, '?'))))
    {
  ...
  if ((base_split.fragment_begin == base_split.fragment_end) && (base_split.query_end == base_split.fragment_begin - 1))
      ...
      if ((0 == rel_split.path_begin) && (rel_split.path_end == rel_split.fragment_end) && (NULL == strchr (rel_uri, '/')))
	{
	  rel_split.path_end =
	      ...
	      rel_split.query_begin = rel_split.query_end = rel_split.fragment_begin = 0;
	}
    }
...
#define IF_NONEMPTY_THEN_TAIL_APPEND_CUT(pref,fld,prefix,prefix_len,suffix,suffix_len) \
  ...
  IF_NONEMPTY_THEN_TAIL_APPEND_CUT (base, netloc, "//", 2, "", 0) if (0 == rel_split.path_end)
    {
      ...
      while (NULL != hit)
	{
	  ...
	  if (hit == buf_prev_tail)
	    ...
	    {
	      crop_begin = hit - 1;
	      while ('/' != crop_begin[0] && crop_begin > buf_prev_tail)
		crop_begin--;
	    }
	  hit = crop_begin;
	  while ('\0' != (crop_begin[0] = crop_end[0]))
	    {
	      crop_begin++;
	      crop_end++;
	    }
...
if ((rel_split.schema_begin == rel_split.schema_end) &&
    (rel_split.netloc_begin == rel_split.netloc_end) && (rel_split.path_begin == rel_split.path_end))
  {
  ...
  IF_NONEMPTY_THEN_TAIL_APPEND_CUT (rel, query, "?", 1, "", 0)
    ...
    (rel_split.path_begin == rel_split.path_end) && (rel_split.params_begin == rel_split.params_end))
  {
...
bif_rfc1808_expand_uri (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t err = NULL;
  caddr_t res =
      rfc1808_expand_uri ( /*(query_instance_t *)qst, */ base_uri, rel_uri, output_cs_name, resolve_like_http_get, base_cs_name,
      rel_cs_name, &err);
...
#undef o
  ...
  if (!SSL_IS_REFERENCEABLE (args[0]))
    sqlr_new_error ("22023", "SR642",
	"The first argument of patch_restricted_xml_chars() should be a variable, not a constant or an expression");
  if (DV_WIDE == src_dtp)
    {
...
#define raw_toupper(C) ((C) & (255-32))
  ...
  if (box_length (name) > 1)
    {
      while (name[inx] && !isalpha (name[inx]))
	inx++;
      if (name[inx])
	tmp[i++] = raw_toupper (name[inx++]);
