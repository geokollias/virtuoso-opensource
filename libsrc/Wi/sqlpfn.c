...
sqlp_hex_literal (char *yytext, int unprocess_chars_at_end)
{
  ...
  for (src = (unsigned char *) (yytext), ptr = (unsigned char *) ret; (src - ((unsigned char *) yytext)) < len; src += 2, ptr++)
    {
      _lo = toupper (src[1]);
      _hi = toupper (src[0]);
...
sqlc_ensure_primary_key (dk_set_t elements)
{
  ...
  END_DO_SET ();
  t_set_push (&elements,
      t_list (5, INDEX_DEF, NULL, NULL, t_list (1, t_box_string ("_IDN")), t_list_to_array (sqlp_index_default_opts (NULL))));
  t_set_push (&elements, NULL);
  t_set_push (&elements, t_list (2, t_list (2, (ptrlong) DV_LONG_INT, (ptrlong) 0), t_list (1, CO_IDENTITY)));
...
sqlp_infoschema_redirect_tbs (ST ** tree, ST ** where_cond)
{
  ...
  if (ST_COLUMN ((*tree), COL_DOTTED) && (*tree)->_.col_ref.prefix)
    {
      (*tree)->_.col_ref.prefix = sqlp_infoschema_redirect_tb ((*tree)->_.col_ref.prefix, NULL, 0, NULL);
    }
  else if (ST_P ((*tree), TABLE_DOTTED))
    {
      ...
      new_name = sqlp_infoschema_redirect_tb ((*tree)->_.table.name, NULL, 0, &q_ptr);
      if (new_name != old_name)
	{
	  ...
	  BIN_OP (cmp, BOP_EQ,
		  ...
		  (*tree)->_.table.prefix ? (*tree)->_.table.prefix : new_name, t_sym_string (qual_col)), (ST *) q_ptr);
...
sqlp_view_def_1 (ST ** names, ST * exp)
{
  if (ST_P (exp, UNION_ST)
	...
	yyerror ("Different number of names and columns in VIEW");
      ...
      {
	if (!ST_P (col, BOP_AS))
	  selection[inx] = t_listst (5, BOP_AS, col, NULL, t_box_copy ((caddr_t) names[inx]), NULL);
      }
      END_DO_BOX;
    }
}
...
sqlp_dt_header (ST * exp)
{
  if (ST_P (exp, UNION_ST)
    ...
    {
      ST **selection = (ST **) exp->_.select_stmt.selection;
      exp->_.select_stmt.selection =
	  (caddr_t *) sqlp_stars (sqlp_wrapper_sqlxml ((ST **) selection), exp->_.select_stmt.table_exp->_.table_exp.from);
      sqlp_breakup (exp);
    }
}
...
{
  char temp[500];
  snprintf (temp, sizeof (temp), "No table %s%s%s in * reference", pref ? pref : "", pref ? "." : "", name ? name : "");
...
sqlp_expand_1_star_table_ref (ST * col, ST * tb_ref, dk_set_t * exp)
{
  ...
  switch (tb_ref->type)
    ...
    case TABLE_DOTTED:
      ...
      if (!col_pref || (tb_pref && 0 == strcmp (tb_pref, col_pref)) || 0 == strcmp (tb_name, col_pref))
	{
	  ...
	  DO_SET (dbe_column_t *, t_col, &parts)
	  {
		...
		t_list (3, COL_DOTTED, t_box_copy (tb_pref ? tb_pref : tb->tb_name), t_box_copy (t_col->col_name)));
	  }
	  END_DO_SET ();
	}
      break;
    ...
    case DERIVED_TABLE:
      ...
      if (!col_pref || 0 == strcmp (col_pref, tb_pref))
	{
	  int n_col;
	  ST **selection;
	  ...
	  if (ST_P (table, SELECT_STMT))
		...
		if (sel_n_breakup (table) && n_col >= sel_n_breakup (table) - 1)
		  break;
		t_dk_set_append_1 (exp, t_list (3, COL_DOTTED, t_box_copy (tb_pref), t_box_copy (as_exp->_.as_exp.name)));
	      }
	      END_DO_BOX;
	    }
	      ...
	      for (inx = 0; ((uint32) inx) < BOX_ELEMENTS (table->_.proc_table.cols); inx += 2)
		{
		  ...
		  if (0 == strcmp ((caddr_t) table->_.proc_table.cols[inx], "_IDN"))
		    continue;
		  t_dk_set_append_1 (exp,
		      t_list (3, COL_DOTTED, t_box_copy ((caddr_t) tb_pref), t_box_copy ((caddr_t) table->_.proc_table.cols[inx])));
		}
		...
		if (!param)
		  continue;
		t_dk_set_append_1 (exp, t_list (3, COL_DOTTED, t_box_copy ((caddr_t) tb_pref), t_box_copy (param)));
	      }
	      END_DO_BOX;
#endif
	    }
...
sqlp_stars (ST ** selection, ST ** from)
{
  ...
  DO_BOX (ST *, col, inx, selection)
  {
	...
	if (prev_last == dk_set_last (exp_list))
	  {
	    char temp[2000];
	    snprintf (temp, sizeof (temp), "Reference %s.* cannot be resolved", col->_.col_ref.prefix ? col->_.col_ref.prefix : "");
	    yyerror (temp);
	  }
      }
    else
		...
		if (IS_BOX_POINTER (co))
		  {
		    switch (co->type)
		      ...
		      case FOREIGN_KEY:
			co->_.fkey.fk_cols = (caddr_t *) t_list (1, t_box_copy (col_name));
			break;

		      case INDEX_DEF:
			co->_.index.cols = (caddr_t *) t_list (1, t_box_copy (col_name));
			break;
		      ...
		      case CHECK_XMLSCHEMA_CONSTR:
			{
		      ...
		      default:
			goto dont_add_table_opts;
		      }
		    table_opts = t_CONS (NULL, t_CONS (t_box_copy_tree ((caddr_t) co), table_opts));
		  dont_add_table_opts:;
		  }
	      }
	      END_DO_BOX;
...
/*  dk_free_box (ss);
  ...
  if (0 == prec)
    return ((ST *) t_list (3, (ptrlong) DV_NUMERIC, t_box_num (NUMERIC_MAX_PRECISION), t_box_num (NUMERIC_MAX_SCALE)));
...
caddr_t
sqlp_known_function_name (caddr_t name)
{
  if (!bif_find (name))
    {
...
ST *
sqlp_make_user_aggregate_fun_ref (caddr_t function_name, ST ** arglist, int allow_yyerror)
{
  ST *acc;
  caddr_t full_name;
  query_t *aggr;
  ...
  full_name = sch_full_proc_name (wi_inst.wi_schema, function_name, cli_qual (sqlc_client ()), CLI_OWNER (sqlc_client ()));
  aggr = ((NULL == full_name) ? NULL : sch_proc_def (wi_inst.wi_schema, full_name));
  if (NULL == aggr)
    {
      if (allow_yyerror)
	yyerror ("Unknown aggregate name");
      else
	return NULL;
    }
  if (NULL == aggr->qr_aggregate)
    {
      if (allow_yyerror)
	yyerror ("The specified name is not an aggregate name");
      else
	return NULL;
    }
  acc = (ST *) t_alloc_box (sizeof (ST), DV_ARRAY_OF_POINTER);
...
sqlp_complete_fun_ref (ST * tree)
{
  if (tree->_.fn_ref.fn_code == AMMSC_COUNT
      && !tree->_.fn_ref.all_distinct && tree->_.fn_ref.fn_arg && DV_LONG_INT != DV_TYPE_OF (tree->_.fn_ref.fn_arg))
    {
      /* count of non-* */
  ...
  if (!v_user)
    {
      ...
      if (cli->cli_user && !sec_user_has_group (G_ID_DBA, cli->cli_user->usr_id) && 0 != CASEMODESTRCMP ("INFORMATION_SCHEMA", o))
...
sqlp_cr_vars (ST * sel)
{
  ...
  DO_BOX (ST *, exp, inx, res)
  {
    if (ST_COLUMN (exp, COL_DOTTED))
	  ...
	  t_list (3, COL_DOTTED, NULL, t_box_copy (exp->_.col_ref.name)), t_list (3, (ptrlong) DV_ANY, (ptrlong) 0, (ptrlong) 0));
    else if (ST_P (exp, BOP_AS))
      res[inx] = (ST *) t_list (4, LOCAL_VAR, IN_MODE,
	  t_list (3, COL_DOTTED, NULL, t_box_copy (exp->_.as_exp.name)), t_list (3, (ptrlong) DV_ANY, (ptrlong) 0, (ptrlong) 0));
    else
...
sqlp_xml_col_name (ST * tree)
{
  ...
  snprintf (xx, sizeof (xx), "%s!%d!%s", tree->_.xml_col.element, (int) unbox (tree->_.xml_col.tag), tree->_.xml_col.attr_name);
  return (t_box_string (xx));
}


int
sqlp_xml_col_directive (char *id)
{
  if (!stricmp (id, "ID"))
    return XML_COL_ID;
  if (!stricmp (id, "IDREF"))
...
sqlp_inline_order_by (ST * tree, ST ** oby)
{
  ...
  if (!ST_P (right, SELECT_STMT) || !right->_.select_stmt.table_exp || right->_.select_stmt.table_exp->_.table_exp.order_by)
...
sqlp_raw_name_string_to_col_dotted (const char *fullname)
{
  ...
  part3 = strchr (part2, '.');
  if (NULL == part3)
    return t_listst (3, COL_DOTTED, c_pref (NULL, 0, t_sqlp_box_id_upcase (buf), part1 - buf, t_sqlp_box_id_upcase (part1)),
	t_sqlp_box_id_upcase (part2));
  (part3++)[0] = '\0';
  part4 = strchr (part3, '.');
  if (NULL != part4)
    yy_new_error ("Too many part characters in column name", "37000", "SQ197");
  if (part2 == (part1 + 1))
    return t_listst (3, COL_DOTTED, c_pref (t_sqlp_box_id_upcase (buf), part1 - buf, NULL, 0, t_sqlp_box_id_upcase (part2)),
	t_sqlp_box_id_upcase (part3));
  return t_listst (3, COL_DOTTED, c_pref (t_sqlp_box_id_upcase (buf), part1 - buf, t_sqlp_box_id_upcase (part1), part2 - part1,
	  t_sqlp_box_id_upcase (part2)), t_sqlp_box_id_upcase (part3));
}
...
{
  int inx;
  if (0 == stricmp (tree->_.call.name, "contains") || 0 == stricmp (tree->_.call.name, "xcontains"))
    {
	...
	if (ST_COLUMN (arg, COL_DOTTED))
	  {
		...
		|| 0 == stricmp (name, "score_limit") || 0 == stricmp (name, "end_id") || 0 == stricmp (name, "ext_fti"))
	      {
...
sqlp_check_arg (ST * tree)
{
  ...
  DO_BOX (ST *, arg, inx, tree->_.call.params)
  {
	...
	if (			/* This is Bug 7585 workaround */
	    ...
	    (NULL == arg->_.as_exp.left->_.col_ref.prefix) && !stricmp (arg->_.as_exp.left->_.col_ref.name, "NAME"))
	  {
...
sqlp_sqlxml (ST * tree)
{
  ...
  if (0 == stricmp (tree->_.call.name, "XMLATTRIBUTES") || 0 == stricmp (tree->_.call.name, "XMLFOREST"))
    {
...
sqlp_xpath_or_xquery_eval (ST * funcall_tree)
{
  ...
  if (DV_STRING == DV_TYPE_OF (old_params[0]))
      ...
      if (NULL != sql_columns)
	{
	  ...
	  if (2 == old_argcount)
	    {
	      funcall_tree->_.call.params =
		  (ST **) t_list_concat ((caddr_t) old_params, (caddr_t) t_list (1, t_box_num_and_zero (1)));
...
sqlp_patch_call_if_special_or_optimizable (ST * funcall_tree)
{
  ...
  if ((DV_SYMBOL != DV_TYPE_OF (call_name)) && (DV_STRING != DV_TYPE_OF (call_name)))
    call_name = "";
  if (0 == stricmp (call_name, "contains") || 0 == stricmp (call_name, "xcontains"))
    {
...
/*sqlxml*/
  if (0 == stricmp (call_name, "XMLELEMENT")
      ...
      || 0 == stricmp (call_name, "XMLCONCAT") || 0 == stricmp (call_name, "XMLAGG") || 0 == stricmp (call_name, "DB.DBA.XMLAGG"))
    {
  ...
  if (0 == stricmp (call_name, "xpath_eval") || 0 == stricmp (call_name, "xquery_eval"))
    {
      sqlp_xpath_or_xquery_eval (funcall_tree);
      goto generic_check;
    }
  if (0 == stricmp (call_name, "xpath_funcall") || 0 == stricmp (call_name, "xquery_apply"))
    {
      sqlp_xpath_funcall_or_apply (funcall_tree);
      goto generic_check;
    }
  if ((0 == strnicmp (call_name, "__I2ID", 6) || strstr (call_name, "IID_OF_QNAME")) && argcount >= 1)
    {
...
generic_check:
  ...
  if (NULL != bmd)
	...
	yyerror (t_box_sprintf (1000,
		"Insufficient number of arguments in %.200s() function call (%d arguments, the minimum is %d):", call_name,
		argcount, bmd->bmd_min_argcount));
	...
	yyerror (t_box_sprintf (1000,
		"Wrong number of arguments in %.200s() function call (%d arguments, valid counts are %d, %d, %d etc.):", call_name,
		argcount, bmd->bmd_min_argcount, bmd->bmd_min_argcount + bmd->bmd_argcount_inc,
		bmd->bmd_min_argcount + 2 * bmd->bmd_argcount_inc));
#endif
      if (bmd->bmd_is_pure)
	{
	  ...
	  if (NULL != err)
	    {
	      ...
	      res = t_listst (3, CALL_STMT, t_box_dv_short_string ("signal"), t_list (2,
		      t_box_dv_short_string (ERR_STATE (err)), t_box_dv_short_string (ERR_MESSAGE (err))));
	      dk_free_tree (err);
	      return res;
#else
...
sqlp_in_exp (ST * left, dk_set_t right, int is_not)
{
  ...
  if (ST_P (left, COMMA_EXP))
	...
	{
	  ands = sql_tree_and (ands, t_listst (4, BOP_EQ, t_box_copy_tree ((caddr_t) left_exp), exp->_.comma_exp.exps[inx], NULL));
	}
	END_DO_BOX;
	if (ors)
...
caddr_t
_br_cstm (caddr_t stmt)
{
  ST *res = (parse_pldbg && !ST_P ((ST *) (stmt), LABELED_STMT) ? (ST *) t_list (6, COMPOUND_STMT,
	  t_list (2,
...
ST *
sqlp_c_for_statement (ST ** init, ST * cond, ST ** inc, ST * body)
{
  ST *res;
...
ST *
sqlp_foreach_statement (ST * data_type, caddr_t var, ST * arr, ST * body)
{
  static int ctr = 0;
  char cn[30];
...
sqlp_add_top_1 (ST * select_stmt)
{
  if (0 && !SEL_TOP (select_stmt))
    {
      select_stmt->_.select_stmt.top = t_listst (7, SELECT_TOP, t_box_num (SEL_IS_DISTINCT (select_stmt) ? 1 : 0),	/* preserve distinct */
	  box_num (1),		/* TOP 1 */
	  t_box_num (0), 0, box_num (0), NULL);
    }
  return select_stmt;
}
...
sqlp_breakup (ST * sel)
{
  ...
  if (BOX_ELEMENTS (terms) < 2 || !ST_P (((ST *) terms[0]), SELECT_BREAKUP))
    return;
...
sqlp_minus (caddr_t x)
{
  switch (DV_TYPE_OF (x))
    ...
    case DV_SINGLE_FLOAT:
      return t_box_float (-unbox_float (x));
    case DV_DOUBLE_FLOAT:
      return t_box_double (-unbox_double (x));
    default:
      yyerror ("unary minus of non-number");
    }
  return NULL;
}
  ...
  if (enable_col_by_default)
    {
      ...
      {
	if (0 == stricmp (opt, "not_column") || 0 == stricmp (opt, "column") || 0 == stricmp (opt, "bitmap"))
	  return opts;
      }
      END_DO_SET ();
