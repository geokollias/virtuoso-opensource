...
void
sp_lk_add (search_spec_t * sp, int op, char *start, char *end)
{
  state_slot_t *ssl = sp->sp_max_ssl;
  int n_len = end - start;
  char *nxt;
  int prev_len;
  caddr_t prev = NULL;
  if (!n_len)
    return;
  if (ssl)
    prev = ssl->ssl_constant;
  prev_len = prev ? box_length (prev) - 1 : 0;
  nxt = dk_alloc_box (prev_len + 1 + 3 + n_len, DV_STRING);
  if (prev)
    memcpy (nxt, prev, prev_len);
  nxt[prev_len] = op;
  nxt[prev_len + 1] = n_len;
  nxt[prev_len + 2] = n_len >> 8;
  memcpy (nxt + prev_len + 3, start, n_len);
  nxt[prev_len + 3 + n_len] = 0;
  dk_free_box (prev);
  if (ssl)
    ssl->ssl_constant = nxt;
  else
    {
      ssl = sp->sp_max_ssl = ssl_new_constant (top_sc->sc_cc, nxt);
      dk_free_box (nxt);
    }
  sp->sp_max_op = CMP_LIKE;
}

void
sp_set_like_flags (search_spec_t * sp)
{
  int first = 1;
  char *pct, *dbl;
  if (SSL_CONSTANT == sp->sp_min_ssl->ssl_type && DV_STRINGP (sp->sp_min_ssl->ssl_constant) && !sp->sp_like_escape
      && sp->sp_col && DV_STRING == sp->sp_col->col_sqt.sqt_col_dtp)
    {
      ...
      if (!pct)
	{
      ...
      while ((dbl = strstr (pattern, "%%")))
	{
	  memmove (dbl, dbl + 1, (pattern + pattern_len + 1) - dbl);
	  pattern_len--;
	}
      if (1 == pattern_len && '%' == pattern[0])
	{
	  return;
	}
      pct = strchr (pattern, '%');
      for (;;)
	{
	  if (pct == pattern + pattern_len - 1)
	    {
	      if (!first)
		sp_lk_add (sp, LK_SUBSTR, pattern, pct);
	      else
		sp_lk_add (sp, LK_LEADING, pattern, pct);
	      return;
	    }
	  if (pct == pattern)
	    {
	      char *next_pct = strchr (pattern + 1, '%');
	      if (!next_pct)
		{
		  sp_lk_add (sp, LK_TRAILING, pattern + 1, pattern + pattern_len);
		  return;
		}
	      sp_lk_add (sp, LK_SUBSTR, pattern + 1, next_pct);
	      pattern = next_pct;
...
int
dfe_is_rdf_type_p (df_elt_t * dfe)
{
  if (DFE_TABLE != dfe->dfe_type || !tb_is_rdf_quad (dfe->_.table.ot->ot_table))
    return 0;
...
int
col_is_rdf (dbe_column_t * col, char name)
{
  return tb_is_rdf_quad (col->col_defined_in) && toupper (name) == toupper (col->col_name[0]);
}
...
dfe_to_spec (df_elt_t * lower, df_elt_t * upper, dbe_key_t * key)
{
  ...
  if (lower->_.bin.left->_.col.col == (dbe_column_t *) CI_ROW)
    SQL_GPF_T (NULL);
  if (key->key_is_cset_temp)
    {
      sp->sp_cl.cl_col_id = lower->_.bin.left->_.col.col->col_id;
      sp->sp_cl.cl_sqt = lower->_.bin.left->_.col.col->col_sqt;
    }
  else
  ...
  if (col_is_rdf (sp->sp_col, 'P'))
    so->so_sc->sc_no_lit_param = 1;
  if (col_is_rdf (sp->sp_col, 'O') && lower->dfe_nth_param && so->so_sc->sc_is_rdf_type_p)
    so->so_sc->sc_no_lit_param = 1;
  if (!upper)
    {
      int op = bop_to_dvc (lower->_.bin.op);
      if (DFE_BOP_PRED == lower->dfe_type && lower->_.bin.is_not)
	sp->sp_is_reverse = 1;
      ...
      else
	{
	  ...
	  sp->sp_min_op = op;
	  if (CMP_NULL != op && CMP_NON_NULL != op)
	    {
  ...
  sp->sp_selectivity = (lower ? lower->dfe_arity : 1) * (upper ? upper->dfe_arity : 1);
  so->so_sc->sc_no_lit_param = 0;
  return sp;
}
...
dbe_column_t *
dfe_in_list_left (df_elt_t * call, ST ** subr)
{
  if (!subr)
    return call->_.call.args[0]->_.col.col;
...
{
  /* make a search spec for chash based in predicate */
  dtp_t dtp;
  dbe_column_t *col = dfe_in_list_left (call, subr);
  hash_range_spec_t *hrng = (hash_range_spec_t *) dk_alloc (sizeof (hash_range_spec_t));
  state_slot_t **init_params = (state_slot_t **) sqlg_qn_dfe ((data_source_t *) call->_.call.extra);
  NEW_VARZ (search_spec_t, sp);
  ...
  sp->sp_is_reverse = is_not;
  hrng->hrng_ht = init_params[1];
  hrng->hrng_ht_id = init_params[2];
  sp->sp_col = col;
  if (ks->ks_key->key_is_col)
    sp->sp_cl = *cl_list_find (ks->ks_key->key_row_var, col->col_id);
  else
    sp->sp_cl = *key_find_cl (ks->ks_key, col->col_id);
  dtp = col->col_sqt.sqt_col_dtp;
  hrng->hrng_flags = HRNG_IN;
  hrng->hrng_dc = ssl_new_vec (sc->sc_cc, "hrng_tmp", dtp);
  if (subr)
    {
      hrng->hrng_substr_start = unbox ((caddr_t) subr[1]);
      hrng->hrng_substr_len = unbox ((caddr_t) subr[2]);
    }
  return sp;
}
...
void
sqlg_non_index_ins (sql_comp_t * sc, df_elt_t * tb_dfe, key_source_t * ks)
{
...
int
col_is_o_or_cset (dbe_column_t * col)
{
  return 0 != col->col_cset_iri || 0 == stricmp (col->col_name, "O");
}
...
sqlg_rdf_text_check (df_elt_t * tb_dfe, text_node_t * txs, df_elt_t * col_dfe, state_slot_t * id_ssl, dk_set_t * code)
{
  ...
  if (tb_is_rdf_quad_or_cset (tb_dfe->_.table.ot->ot_table))
    {
      if (id_ssl->ssl_column && col_is_o_or_cset (id_ssl->ssl_column))
	{
...
sqlg_geo_index_table (dbe_key_t * text_key, ST ** geo_args)
{
  ...
  if (!text_key || text_key->key_table->tb_cset)
    {
      dbe_table_t *r_geo = sch_name_to_table (wi_inst.wi_schema, "DB.DBA.RDF_GEO");
      if (!r_geo)
	sqlc_new_error (top_sc->sc_cc, "NOGEO", "NOGEO", "RDF_GEO  table does not exist and no other geo inx table is given");
      return r_geo;
    }
...
void
sqlg_cset_leading_text (sqlo_t * so, df_elt_t * tb_dfe, text_node_t * txs)
{
  /* text node, posg, then cset table on s. */
  dbe_column_t *s_col, *cset_col;
  df_elt_t *cset_col_dfe;
  state_slot_t *s_ssl;
  sql_comp_t *sc = so->so_sc;
  ...
  state_slot_t *text_id = sqlg_dfe_ssl (so, col_dfe);
  {
    dbe_table_t *rq = tb_dfe->_.table.ot->ot_table->tb_closest_cset->cset_rq_table;
    key_source_t *ks = (key_source_t *) dk_alloc (sizeof (key_source_t));
    SQL_NODE_INIT (table_source_t, ts, table_source_input, ts_free);
    search_spec_t *sp1 = (search_spec_t *) dk_alloc (sizeof (search_spec_t));
    search_spec_t *sp2 = (search_spec_t *) dk_alloc (sizeof (search_spec_t));
    memzero (ks, sizeof (key_source_t));
    memzero (sp1, sizeof (search_spec_t));
    memzero (sp2, sizeof (search_spec_t));
    clb_init (so->so_sc->sc_cc, &ts->clb, 1);
    ts->ts_order_cursor = ssl_new_itc (sc->sc_cc);
    cset_col_dfe = sqlo_df (so, tb_dfe->_.table.text_pred->_.text.args[0]);
    if (DFE_COLUMN != cset_col_dfe->dfe_type)
      sqlc_new_error (sc->sc_cc, "NGCOL", "NGCOL", "cset geo pred does not ref a col in the cset");
    cset_col = cset_col_dfe->_.col.col;
    ts->ts_order_ks = ks;
    ks->ks_key = tb_px_key (rq, tb_name_to_column (rq, "O"));
    ks->ks_spec.ksp_spec_array = sp1;
    sp1->sp_min_op = CMP_EQ;
    sp1->sp_min_ssl = ssl_new_constant (sc->sc_cc, t_box_iri_id (cset_col->col_cset_iri));
    sp1->sp_col = tb_name_to_column (rq, "P");
    sp1->sp_cl = *cl_list_find (ks->ks_key->key_key_fixed, sp1->sp_col->col_id);
    sp1->sp_next = sp2;
    sp2->sp_min_op = CMP_EQ;
    sp2->sp_min_ssl = txs->txs_d_id;
    sp2->sp_col = tb_name_to_column (rq, "O");
    sp2->sp_cl = *cl_list_find (ks->ks_key->key_key_var, sp2->sp_col->col_id);
    s_col = tb_name_to_column (rq, "S");
    dk_set_push (&ks->ks_out_cols, (void *) s_col);
    ts->ts_cardinality = 1;
    ts->ts_inx_cardinality = 1;
    ts->ts_cost = 2;
    s_ssl = ssl_new_column (sc->sc_cc, "txs_s", s_col);
    dk_set_push (&ks->ks_out_slots, (void *) s_ssl);
    txs->src_gen.src_continuations = dk_set_cons ((void *) ts, NULL);
    sqlg_ks_col_alter (ks);
...
void
sqlg_cset_leading_text_final (sqlo_t * so, df_elt_t * tb_dfe, table_source_t * ts)
{
  /* join between posg and the cset on s */
  text_node_t *txs = (text_node_t *) tb_dfe->_.table.text_node;
  table_source_t *posg_ts = (table_source_t *) qn_next ((data_source_t *) txs);
  key_source_t *posg_ks = posg_ts->ts_order_ks;
  key_source_t *ks = ts->ts_order_ks;
  NEW_VARZ (search_spec_t, sp);
  txs->txs_d_id = ssl_new_variable (so->so_sc->sc_cc, "d_id", DV_ANY);
  ks->ks_spec.ksp_spec_array = sp;
  sp->sp_min_op = CMP_EQ;
  sp->sp_min_ssl = (state_slot_t *) posg_ks->ks_out_slots->data;
  sp->sp_col = (dbe_column_t *) ks->ks_key->key_parts->data;
  sp->sp_cl = *cl_list_find (ks->ks_key->key_row_var, sp->sp_col->col_id);
  posg_ks->ks_spec.ksp_spec_array->sp_next->sp_min_ssl = txs->txs_d_id;
  ks_set_search_params (so->so_sc->sc_cc, NULL, posg_ks);
}
...
sqlg_text_node (sqlo_t * so, df_elt_t * tb_dfe, index_choice_t * ic)
{
  ...
  if (tb_dfe->_.table.is_text_order && tb_dfe->_.table.ot->ot_table->tb_closest_cset)
    {
      sqlg_cset_leading_text (so, tb_dfe, txs);
    }
  else if (tb_dfe->_.table.is_text_order && !ic)
    {
      df_elt_t *text_pred = tb_dfe->_.table.text_pred;
  ...
  else
    {
      ...
      df_elt_t *col_dfe = sqlo_df (so, t_listst (3, COL_DOTTED, ot->ot_new_prefix, col->col_name));
      if (geo_args && !tb_is_rdf_quad_or_cset (tb_dfe->_.table.ot->ot_table))
  ...
  if (ot->ot_table && tb_is_rdf_quad_or_cset (ot->ot_table))
    txs->txs_is_rdf = 1;
...
int
sqlg_is_top_first (df_elt_t * col_dfe)
{
  if (DFE_CALL == col_dfe->dfe_type)
    {
      if (st_is_call (col_dfe->dfe_tree, "__ro2sq", 1) || st_is_call (col_dfe->dfe_tree, "__i2idn", 1))
	return 1;
      return 0;
    }
  return DFE_COLUMN == col_dfe->dfe_type;
  return 0;
}
...
sqlg_top_oby_pre (sqlo_t * so, df_elt_t * dfe, table_source_t * ts, dk_set_t * pre_code)
{
  ...
  return;
found:
  if (dfe->dfe_super && DFE_DT == dfe->dfe_super->dfe_type)
    dfe->dfe_super->_.sub.ot->ot_has_top_test = 1;
  if (DFE_COLUMN == col_dfe->dfe_type && dk_set_member (dfe->_.table.out_cols, col_dfe) && HR_NONE == dfe->_.table.hash_role)
    {
      key_source_t *ks = ts->ts_order_ks;
	  ...
	  if (sqlg_is_top_first (col_dfe))
	    {
	      ...
	      top_test_cv = ts->src_gen.src_after_test;
	    }
	  else
	    {
	      top_test_cv = en->src_gen.src_after_test = sqlg_pred_body (so, dfe->_.table.top_pred);
	    }
	}
      else
	{
      ...
      DO_INSTR (ins, 0, top_test_cv)
      {
	if (INS_CALL_BIF == ins->ins_type && (bif_topk == ins->_.bif.bif || (bif_t) bif_topk_vec == ins->_.bif.bif))
	  {
...
{
  /* if the ks does not have out ssl for the col, add */
  dbe_column_t *col;
  state_slot_t *ssl;
  key_source_t *ks = ts->ts_order_ks;
  ...
  ssl = ssl_new_column (sc->sc_cc, tb_dfe->_.table.ot->ot_new_prefix, col);
  dk_set_push (&ks->ks_out_cols, (void *) col);
...
void
sqlg_file_source (sqlo_t * so, df_elt_t * tb_dfe, table_source_t * ts, dk_set_t * pre_code)
{
  key_source_t *ks = ts->ts_order_ks;
  search_spec_t *sp;
  dk_set_t col_pos = NULL;
  dk_set_t out_cols = NULL;
  dk_set_t out_slots = NULL;
  dk_set_t cols;
  sql_comp_t *sc = so->so_sc;
  op_table_t *ot = tb_dfe->_.table.ot;
  file_table_t *ft = ot->ot_table->tb_ft;
  ST *f_name = (ST *) sqlo_opt_value (ot->ot_opts, OPT_FROM_FILE);
  ST *f_start = (ST *) sqlo_opt_value (ot->ot_opts, OPT_FILE_START);
  ST *f_end = (ST *) sqlo_opt_value (ot->ot_opts, OPT_FILE_END);
  int nth_col = 0;
  NEW_VARZ (file_source_t, fs);
  ts->clb.clb_nth_set = cc_new_instance_slot (sc->sc_cc);
  ts->ts_fs = fs;
  fs->fs_table = ot->ot_table;
  fs->fs_ses = ssl_new_variable (sc->sc_cc, "fs_ses", DV_ANY);
  if (!f_name)
    fs->fs_file_name = ssl_new_constant (sc->sc_cc, ft->ft_file);
  else
    {
      fs->fs_file_name = scalar_exp_generate (sc, f_name, pre_code);
      fs->fs_file_name->ssl_qr_global = 1;
    }
  if (!f_start)
    fs->fs_start_offset = ssl_new_constant (sc->sc_cc, box_num (0));
  else
    {
      fs->fs_start_offset = scalar_exp_generate (sc, f_start, pre_code);
      fs->fs_start_offset->ssl_qr_global = 1;
    }
  if (!f_end)
    fs->fs_limit = ssl_new_constant (sc->sc_cc, box_num (-1));
  else
    {
      fs->fs_limit = scalar_exp_generate (sc, f_end, pre_code);
      fs->fs_limit->ssl_qr_global = 1;
    }
  fs->fs_delim = ft->ft_delim;
  fs->fs_escape = ft->ft_escape;
  ...
  for (sp = ts->ts_order_ks->ks_spec.ksp_spec_array; sp; sp = sp->sp_next)
    {
      fs_ensure_out_col (sc, tb_dfe, ts, sp->sp_cl.cl_col_id);
    }
  for (sp = ts->ts_order_ks->ks_row_spec; sp; sp = sp->sp_next)
    {
      fs_ensure_out_col (sc, tb_dfe, ts, sp->sp_cl.cl_col_id);
    }
  cols = key_ensure_visible_parts (ot->ot_table->tb_primary_key);
  DO_SET (dbe_column_t *, col, &cols)
  {
    int nth = dk_set_position (ks->ks_out_cols, col);
    if (-1 == nth)
      {
	nth_col++;
	continue;
      }
    t_set_push (&col_pos, (void *) (ptrlong) nth_col);
    t_set_push (&out_slots, dk_set_nth (ks->ks_out_slots, nth));
    t_set_push (&out_cols, (void *) col);
    nth_col++;
  }
  END_DO_SET ();
  fs->fs_out_slots = (state_slot_t **) dk_set_to_array (dk_set_nreverse (out_slots));
  fs->fs_out_cols = (dbe_column_t **) dk_set_to_array (dk_set_nreverse (out_cols));
  fs->fs_out_pos = (ptrlong *) dk_set_to_array (dk_set_nreverse (col_pos));
}
...
iri_id_t
dfe_lit_p (sql_comp_t * sc, df_elt_t * dfe)
{
  dbe_column_t *p_col = (dbe_column_t *) dfe->_.table.ot->ot_table->tb_primary_key->key_parts->data;
  df_elt_t *pred = NULL;
  DO_SET (df_elt_t *, cp, &dfe->_.table.col_preds)
  {
    if (PRED_IS_EQ (cp) && DFE_COLUMN == cp->_.bin.left->dfe_type && p_col == cp->_.bin.left->_.col.col)
      {
	pred = cp;
	break;
      }
  ...
  END_DO_SET ();
  if (pred && PRED_IS_EQ (pred) && DV_IRI_ID == DV_TYPE_OF (pred->_.bin.right->dfe_tree))
    return unbox_iri_id (pred->_.bin.right->dfe_tree);
  return 0;
}


#define COL_IS_CSET(col) ((col) && (col)->col_id > 2)


void
sqlg_csts_inx_o_out (sql_comp_t * sc, df_elt_t * inx_dfe, df_elt_t * tb_dfe, table_source_t * ts)
{
  int pos = 0;
  cset_ts_t *csts = ts->ts_csts;
  DO_SET (dbe_column_t *, out_col, &ts->ts_order_ks->ks_out_cols)
  {
    if (out_col->col_cset_iri == csts->csts_o_index_p)
      {
	csts->csts_o_from_posg = sqlg_dfe_ssl (sc->sc_so, inx_dfe->_.table.ot->ot_cset_inx_o_out);
	csts->csts_posg_o_out = (state_slot_t *) dk_set_nth (ts->ts_order_ks->ks_out_slots, pos);
      }
    pos++;
  }
  END_DO_SET ();
...
ts_add_cset (sql_comp_t * sc, df_elt_t * tb_dfe, table_source_t * ts)
{
  ...
  csts->csts_nth_cset = cc_new_instance_slot (sc->sc_cc);
  csts->csts_qr = cc_new_instance_slot (sc->sc_cc);
  csts->csts_qr_key = cc_new_instance_slot (sc->sc_cc);
  csts->csts_cset_mode = cc_new_instance_slot (sc->sc_cc);
  if (!csts->csts_is_scan)
    {
      /* if by index on some o, mark the p for this in csts and if the o is to be returned, set the ssls for this return */
      df_elt_t *inx_dfe = tb_dfe->_.table.ot->ot_cset_inx_dfe;
      op_table_t *inx_ot = inx_dfe ? inx_dfe->_.table.ot : NULL;
      csts->csts_s_sets = cc_new_instance_slot (sc->sc_cc);
      csts->csts_s = ssl_new_vec (sc->sc_cc, "cset_s", DV_IRI_ID);
      if (DFE_TABLE == tb_dfe->dfe_prev->dfe_type && inx_ot == tb_dfe->dfe_prev->_.table.ot)
	{
	  inx_dfe = tb_dfe->dfe_prev;
	  if (inx_dfe->_.table.ot != inx_ot)
	    return;
	  csts->csts_o_index_p = dfe_lit_p (sc, inx_dfe);
	  sqlg_csts_inx_o_out (sc, inx_dfe, tb_dfe, ts);
	}
    }
  else
    {
      csts->csts_prev_last_cset_s = ssl_new_variable (sc->sc_cc, "prev_last_s", DV_IRI_ID);
  ...
  END_DO_SET ();
  for (sp = ks->ks_row_spec; sp; sp = sp->sp_next)
    if (COL_IS_CSET (sp->sp_col))
      sethash (sp->sp_col, ht, (void *) 1);
  ...
  hash_table_free (ht);
  csts->csts_o_scan_mode = sc->sc_cset_o_scan_mode;
  sc->sc_cset_o_scan_mode = NULL;
}

void
v_out_replace (v_out_map_t * om, state_slot_t * ssl, state_slot_t * new_ssl)
{
  int inx, len = box_length (om) / sizeof (v_out_map_t);
  for (inx = 0; inx < len; inx++)
    if (ssl == om[inx].om_ssl)
      {
	om[inx].om_ssl = new_ssl;
	return;
      }
}
...
void
psog_add_o (key_source_t * ks, search_spec_t * sp)
{
  search_spec_t *cp = sp_copy (sp);
  cp->sp_cl = ks->ks_key->key_row_var[2];
  if (CMP_HASH_RANGE == cp->sp_min_op)
    {
      cp->sp_next = NULL;
    ...
    {
      search_spec_t *prev;
      for (prev = ks->ks_row_spec; prev; prev = prev->sp_next)
	{
	  if (prev->sp_min_op == sp->sp_min_op && prev->sp_min_ssl == sp->sp_min_ssl && prev->sp_max_op == sp->sp_max_op
	      && prev->sp_max_ssl == sp->sp_max_ssl)
	    {
	      dk_free ((caddr_t) cp, sizeof (search_spec_t));
	      return;
	    }
	}
...
void
ts_add_cset_posg_psog (sql_comp_t * sc, df_elt_t * tb_dfe, table_source_t * posg_ts)
{
  key_source_t *posg_ks = posg_ts->ts_order_ks;
  search_spec_t *p_sp, *s_sp, *o_sp;
  dk_set_t ssl_iter;
  cset_quad_t *csq = (cset_quad_t *) dk_alloc (sizeof (cset_quad_t));
  key_source_t *ks = (key_source_t *) dk_alloc (sizeof (key_source_t));
  dbe_table_t *rq = sch_name_to_table (wi_inst.wi_schema, "DB.DBA.RDF_QUAD");
  dbe_key_t *key = posg_ks->ks_key->key_table->tb_primary_key;
  SQL_NODE_INIT (table_source_t, ts, table_source_input, ts_free);
  memzero (ks, sizeof (key_source_t));
  memzero (csq, sizeof (cset_quad_t));
  ts->ts_csq = csq;
  ts->ts_order_ks = ks;
  ts->ts_order_cursor = ssl_new_itc (sc->sc_cc);
  ts->ts_cardinality = 1;
  csq->csq_mode = CSQ_CSET_SCAN;
  csq->csq_cset = cc_new_instance_slot (sc->sc_cc);
  csq->csq_nth_step = cc_new_instance_slot (sc->sc_cc);
  ...
  if (p_sp)
    {
      p_sp = ks->ks_spec.ksp_spec_array = sp_copy (p_sp);
      p_sp->sp_next = NULL;
      p_sp->sp_cl = *key_find_cl (key, p_sp->sp_col->col_id);
    }
  s_sp = (search_spec_t *) dk_alloc (sizeof (search_spec_t));
  memzero (s_sp, sizeof (search_spec_t));
  p_sp->sp_next = s_sp;
  s_sp->sp_min_op = CMP_GTE;
  s_sp->sp_max_op = CMP_LT;
  s_sp->sp_col = (dbe_column_t *) key->key_parts->next->data;
  s_sp->sp_cl = *key_find_cl (key, s_sp->sp_col->col_id);
  csq->csq_s_min = s_sp->sp_min_ssl = ssl_new_variable (sc->sc_cc, "cset_min_s", DV_IRI_ID);
  csq->csq_s_max = s_sp->sp_max_ssl = ssl_new_variable (sc->sc_cc, "cset_max_s", DV_IRI_ID);
  for (o_sp = posg_ks->ks_spec.ksp_spec_array; o_sp; o_sp = o_sp->sp_next)
    if (o_sp->sp_col && 'O' == o_sp->sp_col->col_name[0])
      psog_add_o (ks, o_sp);
  for (o_sp = posg_ks->ks_row_spec; o_sp; o_sp = o_sp->sp_next)
    if (o_sp->sp_col && 'O' == o_sp->sp_col->col_name[0])
      psog_add_o (ks, o_sp);
  DO_SET (search_spec_t *, o_sp, &ks->ks_hash_spec) if (o_sp->sp_col && 'O' == o_sp->sp_col->col_name[0])
    psog_add_o (ks, o_sp);
  END_DO_SET ();
  ssl_iter = posg_ts->ts_order_ks->ks_out_slots;
  DO_SET (dbe_column_t *, col, &posg_ts->ts_order_ks->ks_out_cols)
  {
    QNCAST (state_slot_t, ssl, ssl_iter->data);
    state_slot_t *intm_ssl;
    switch (toupper (col->col_name[0]))
      {
      case 'O':
	intm_ssl = ssl_new_column (sc->sc_cc, "intm", col);
	csq->csq_org_o = intm_ssl;
	csq->csq_o_out = ssl;
	break;
      case 'S':
	intm_ssl = ssl_new_column (sc->sc_cc, "intm", col);
	csq->csq_org_s = intm_ssl;
	csq->csq_s_out = ssl;
	break;
      case 'G':
	intm_ssl = ssl_new_column (sc->sc_cc, "intm", col);
	csq->csq_org_g = intm_ssl;
	csq->csq_g_out = ssl;
	break;
      default:
	goto next;
      }
    dk_set_push (&ks->ks_out_cols, col);
    dk_set_push (&ks->ks_out_slots, (void *) ssl);
    ssl_iter->data = intm_ssl;
    /* v_out_replace (posg_ts->ts_order_ks->ks_v_out_map, ssl, intm_ssl); */
  next:
    ssl_iter = ssl_iter->next;
  }
  END_DO_SET ();
  csq->csq_nth_step = cc_new_instance_slot (sc->sc_cc);
  csq->csq_at_end = cc_new_instance_slot (sc->sc_cc);
  csq->csq_ts = cc_new_instance_slot (sc->sc_cc);
  if (posg_ks->ks_top_oby_spec)
    {
      ks->ks_top_oby_spec = sp_copy (posg_ks->ks_top_oby_spec);
      ks->ks_top_oby_spec->sp_cl = *cl_list_find (ks->ks_key->key_row_var, ks->ks_top_oby_spec->sp_cl.cl_col_id);
    }
}


void
ts_add_cset_posg (sql_comp_t * sc, df_elt_t * tb_dfe, table_source_t * ts)
{
  NEW_VARZ (cset_mode_t, csm);
  ts->ts_csm = csm;
  csm->csm_role = TS_CSET_POSG;
...
ts_set_csq (sql_comp_t * sc, df_elt_t * tb_dfe, table_source_t * ts)
{
  ...
  if (!sp || CMP_EQ != sp->sp_min_op)
    return;
  p_ssl = sp->sp_min_ssl;
  if (SSL_CONSTANT == p_ssl->ssl_type)
    ric = dfe_ric (tb_dfe);
  if (SSL_CONSTANT != p_ssl->ssl_type || gethash ((void *) unbox_iri_id (p_ssl->ssl_constant), &p_to_csetp_list)
      || (ric != empty_ric && ric_sub_p_in_cset (ric, p_ssl->ssl_constant)))
    {
      NEW_VARZ (cset_quad_t, csq);
      csq->csq_nth_step = cc_new_instance_slot (sc->sc_cc);
      csq->csq_last_p = cc_new_instance_slot (sc->sc_cc);
      ...
      csq->csq_ts = cc_new_instance_slot (sc->sc_cc);
      csq->csq_at_end = cc_new_instance_slot (sc->sc_cc);
      csq->csq_param_order = cc_new_instance_slot (sc->sc_cc);
      csq->csq_itc = ssl_new_variable (sc->sc_cc, "csq_itc", DV_ITC);
      csq->csq_mode = CSQ_QUAD;
      csq->csq_rq_itc = ts->ts_order_cursor;
      ts->ts_csq = csq;
    }
}
...
void
sqlg_ts_cset_posg_prepare (sql_comp_t * sc, df_elt_t * tb_dfe)
{
  ...
  {
    dbe_column_t *col = out_col->_.col.col;
    if (p_iri == col->col_cset_iri)
      {
	df_elt_t *o_inx_out = sqlo_df (sc->sc_so, t_listst (3, COL_DOTTED, tb_dfe->_.table.ot->ot_new_prefix, t_box_string ("O")));
	t_set_push (&tb_dfe->_.table.out_cols, (void *) o_inx_out);
	tb_dfe->_.table.ot->ot_cset_inx_o_out = o_inx_out;
      }
  }
  END_DO_SET ();
}
...
sqlg_make_np_ts (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * pre_code)
{
  ...
  dbe_key_t *main_key;
  SQL_NODE_INIT (table_source_t, ts, table_source_input, ts_free);
  if (TS_CSET_POSG == tb_dfe->_.table.cset_role)
    sqlg_ts_cset_posg_prepare (sc, tb_dfe);
...
#endif
  sqlg_non_index_ins (sc, tb_dfe, ts->ts_main_ks);
  ...
  if (tb_dfe->_.table.ot->ot_table->tb_closest_cset)
    {
      if (tb_dfe->_.table.is_text_order)
	sqlg_cset_leading_text_final (so, tb_dfe, ts);
      ts_add_cset (sc, tb_dfe, ts);
    }
  if (!no_cset && TS_CSET_POSG == tb_dfe->_.table.cset_role)
    ts_add_cset_posg (sc, tb_dfe, ts);
...
data_source_t *sqlg_hash_source (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * pre_code);


data_source_t *
sqlg_make_temp_ts (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * pre_code)
{
  sql_comp_t *sc = so->so_sc;
  dk_set_t refs = NULL;
  dbe_key_t *key = tb_dfe->_.table.key;
  ...
  int nth = 0;
  dk_set_t out_slots = NULL, out_cols = NULL;
  DO_SET (dbe_column_t *, col, &key->key_parts)
  {
    df_elt_t *cp = sqlo_key_part_best (col, tb_dfe->_.table.col_preds, 0);
    if (!cp || !PRED_IS_EQ (cp))
      break;
    NCONCF1 (refs, cp->_.bin.right);
    if (++nth == key->key_n_significant)
      {
	tb_dfe->_.table.hash_refs = refs;
	return sqlg_hash_source (so, tb_dfe, pre_code);
      }
  }
  END_DO_SET ();
  /* not an eq by pk, scan like a gby temp */
  {
    SQL_NODE_INIT (table_source_t, ts, table_source_input, ts_free);
    {
      NEW_VARZ (key_source_t, ks);
      ts->ts_order_ks = ks;
      ks->ks_key = tb_dfe->_.table.key;
      ks->ks_row_check = itc_row_check;
	...
	NCONCF1 (out_cols, out->_.col.col);
      }
      END_DO_SET ();

      if (tb->tb_set_no_in_key)
	{
	...
	ssl->ssl_always_vec = 1;
      ...
      ks->ks_from_temp_tree = tb->tb_temp_ssl;
...
{
  so->so_sc->sc_is_rdf_type_p = dfe_is_rdf_type_p (tb_dfe);
  if (tb_dfe->_.table.ot->ot_table->tb_is_temp)
    return sqlg_make_temp_ts (so, tb_dfe, pre_code);
  else if (tb_dfe->_.table.index_path)
...
data_source_t *
sqlg_ts_exec_opt (sqlo_t * so, df_elt_t * tb_dfe, data_source_t * last_qn)
{
  /* table option execute as part of trans statement.  queries in the exec block have tb_dfe as previous dfg stage */
  QNCAST (table_source_t, ts, last_qn);
  end_node_t *en2;
  sql_comp_t *sc = so->so_sc;
  ST *stmt = (ST *) sqlo_opt_value (tb_dfe->_.table.ot->ot_opts, OPT_EXECUTE);
  if (stmt && !IS_QN (ts, chash_read_input))
    sqlc_new_error (sc->sc_cc, "SCEXE", "SCEXE", "Scan execute option is only allowed after a trans temp  scan");
  if (stmt)
    {
      int n_cols;
      gb_op_t *save_go = sc->sc_in_group_by_exec;
      NEW_VAR (trans_read_t, trr);
      NEW_VARZ (gb_op_t, go);
      SQL_PROC_SAVE;
      memzero (trr, sizeof (trans_read_t));
      go->go_op = AMMSC_TRANS_SCAN;
      go->go_exec_rows = ssl_new_vec (sc->sc_cc, "trans_scan_rows", DV_LONG_INT);
      go->go_exec_base_set = ssl_new_constant (sc->sc_cc, NULL);
      go->go_exec_ssls = (state_slot_t **) dk_alloc_box_zero (sizeof (caddr_t) * n_cols, DV_BIN);
      trr->trr_rows_dc = go->go_exec_rows;
      ts->ts_trans_read = trr;
      sqlg_sdfg_empty_qf (sc, last_qn);
      go->go_temp_table = tb_dfe->_.table.ot->ot_table;
      n_cols = dk_set_length (go->go_temp_table->tb_primary_key->key_parts);
      {
	SQL_NODE_INIT (end_node_t, en, end_node_input, NULL);
	sc->sc_in_trans_clause = 1;
	sc->sc_in_group_by_exec = go;
	sc->sc_cc->cc_query->qr_proc_vectored = QR_VEC_STMT;
	sqlc_compound_stmt (sc, stmt, NULL);
	en->src_gen.src_pre_code = code_to_cv (sc, sc->sc_routine_code);
	sc->sc_in_trans_clause = 0;
	sc->sc_in_group_by_exec = save_go;
	last_qn->src_continuations = CONS (en, NULL);
	en2 = en;
	SQL_PROC_RESTORE;
	return (data_source_t *) en2;
      }
    }
  return last_qn;
}
...
sqlg_is_hj_result_col (sql_comp_t * sc, df_elt_t * fill_dfe, df_elt_t * out_dfe)
{
  ...
  if (fill_dfe->_.table.hash_filler_reused)
    return 1;
...
sqlg_set_no_bloom (fun_ref_node_t * fref)
{
  ...
  if (ts->src_gen.src_after_test || ts->src_gen.src_after_code)
    return;
  if (next != (data_source_t *) fref->fnr_setp
      && !(IS_QN (next, stage_node_input) && qn_next (next) == (data_source_t *) fref->fnr_setp))
    return;
  ks = ts->ts_order_ks;
...
sqlg_hash_filler (sqlo_t * so, df_elt_t * tb_dfe, data_source_t * ts_src)
{
  ...
  ha = setp->setp_ha;
  if (2 == tb_dfe->_.table.hash_filler_of->_.table.is_right_oj)
  ...
  if (dt_dfe->_.sub.gby_hash_filler)
    {
      ...
      DO_SET (setp_node_t *, qn, &sqs->sqs_query->qr_nodes)
      {
	if (IS_QN (qn, setp_node_input) && qn->setp_distinct && !setp)
	  setp = qn;
	else if (IS_QN (qn, setp_node_input) && qn->setp_ha && HA_GROUP == qn->setp_ha->ha_op)
	  {
	    setp = qn;
	  }
	else if (IS_QN (qn, fun_ref_node_input))
	  fref = (fun_ref_node_t *) qn;
      }
      END_DO_SET ();
      if (!setp || !fref)
	...
	setp->setp_cl_partition = HS_CL_PART;
      fref->src_gen.src_input = (qn_input_fn) hash_fill_node_input;
      fref->fnr_parallel_hash_fill = 1;
      fref->fnr_n_part = cc_new_instance_slot (sc->sc_cc);
      fref->fnr_nth_part = cc_new_instance_slot (sc->sc_cc);
      fref->fnr_hash_part_min = cc_new_instance_slot (sc->sc_cc);
      fref->fnr_hash_part_max = cc_new_instance_slot (sc->sc_cc);
      fref->fnr_no_hash_partition = 1;
      dk_set_free (fref->src_gen.src_continuations);
      fref->src_gen.src_continuations = NULL;
      sqs->sqs_is_hash_filler = 1;
      for (qn = fref->fnr_select; qn; qn = qn_next (qn))
	{
	  dk_set_delete (&sqs->sqs_query->qr_nodes, (void *) qn);
	  dk_set_push (&sc->sc_cc->cc_query->qr_nodes, (void *) qn);
	  qn->src_query = sc->sc_cc->cc_query;
	}
      dk_set_delete (&sqs->sqs_query->qr_nodes, (void *) fref);
      dk_set_push (&sc->sc_cc->cc_query->qr_nodes, (void *) fref);
      fref->src_gen.src_query = sc->sc_cc->cc_query;

      return (data_source_t *) fref;
    }
  {
    SQL_NODE_INIT (setp_node_t, setp2, setp_node_input, setp_node_free);
    setp = setp2;
    dt_dfe->_.sub.hash_filler_of->_.table.ot->ot_hash_filler = setp;
  }
  if (2 == dt_dfe->_.sub.hash_filler_of->_.table.is_right_oj)
    setp->setp_cl_partition = HS_CL_PART;
...
static void
setp_ha_find_col (setp_node_t * setp, df_elt_t * tb_dfe, dbe_column_t * col, dbe_col_loc_t * ret_loc, ptrlong * ret_idx)
{
  ...
  if (!setp)
    {
      /* named temp hash */
      dbe_key_t *key = tb_dfe->_.table.ot->ot_table->tb_primary_key;
      int pos = dk_set_position (key->key_parts, col);
      if (-1 == pos)
	SQL_GPF_T1 (top_sc->sc_cc, "no col in temp table");
      *ret_idx = pos;
      *ret_loc = *key_find_cl (key, col->col_id);
      return;
    }
  n_keys = dk_set_length (setp->setp_keys);
  DO_SET (state_slot_t *, ssl, &setp->setp_keys)
  {
  ...
  END_DO_SET ();
  SQL_GPF_T1 (top_sc->sc_cc,
      "hash join col ref not in the hash out cols.  Can try disabling hash build side reuse, __dbf_set key enable_hash_fill_reuse = 0");
}
...
setp_node_t *
sqlg_tb_hash_filler (df_elt_t * tb_dfe)
{
  df_elt_t *filler;
  if (!tb_dfe->_.table.hash_filler)
    return NULL;
  if (!tb_dfe->_.table.hash_filler_reuse)
    return tb_dfe->_.table.ot->ot_hash_filler;
  filler = tb_dfe->_.table.hash_filler;
  if (DFE_TABLE == filler->dfe_type)
    return tb_dfe->_.table.hash_filler->_.table.ot->ot_hash_filler;
  sqlc_new_error (top_sc->sc_cc, "DTHRU", "DTHRU", "No hash build reuse supported for derived table builds");
  return NULL;
}


int
key_non_null (dbe_key_t * key)
{
  DO_SET (dbe_column_t *, col, &key->key_parts) if (!col->col_sqt.sqt_non_null)
    return 0;
  END_DO_SET ();
  return 1;
}
...
void
sqlg_temp_hs_ha (sql_comp_t * sc, dbe_table_t * tb, hash_area_t * ha)
{
  int n_slots = dk_set_length (tb->tb_primary_key->key_parts), inx;
  memzero (ha, sizeof (hash_area_t));
  ha->ha_key = tb->tb_primary_key;
  ha->ha_n_keys = tb->tb_primary_key->key_n_significant;
  ha->ha_ch_len = sizeof (int64) * n_slots;
  ha->ha_tree = tb->tb_temp_ssl;
  ha->ha_key_cols = (dbe_col_loc_t *) dk_alloc_box_zero ((n_slots + 1) * sizeof (dbe_col_loc_t), DV_BIN);
  for (inx = 0; inx < n_slots; inx++)
    {
      dbe_col_loc_t *cl = key_find_cl (ha->ha_key, inx + 1);
      ha->ha_key_cols[inx] = cl[0];
    }
  if (key_non_null (tb->tb_primary_key))
    {
      ha->ha_ch_nn_flags = 0;
    }
  else
...
sqlg_hash_source (sqlo_t * so, df_elt_t * tb_dfe, dk_set_t * pre_code)
{
  ...
  dk_set_t asg_code = NULL;
  dk_set_t roj_key_out = NULL;
  ...
  op_table_t *ot = tb_dfe->_.table.ot;
  int is_fill_dt = tb_dfe->_.table.hash_filler && DFE_DT == tb_dfe->_.table.hash_filler->dfe_type;
  setp_node_t *setp = sqlg_tb_hash_filler (tb_dfe);
  ...
  if (setp)
    {
      ...
      if (tb_dfe->_.table.hash_need_colocate)
	hs->hs_cl_part_opt = HS_CL_REPLICATED;
    }
  ...
  if (setp)
    ...
    {
      sqlg_temp_hs_ha (sc, tb_dfe->_.table.ot->ot_table, ha_copy);
      hs->hs_cl_id = tb_dfe->_.table.ot->ot_table->tb_temp_id_ssl;
    }
  ...
  if (enable_chash_join)
	...
	if (is_fill_dt)
	  ...
	  setp_ha_find_col (setp, tb_dfe, out->_.col.col, &cl, &nth);
	if (nth < ha_copy->ha_n_keys)
	  {
	    if (2 == tb_dfe->_.table.is_right_oj)
	      t_set_push (&roj_key_out, (void *) ssl);
	    if (!tb_dfe->_.table.ot->ot_is_outer /* ssl->ssl_sqt.sqt_dtp != hs->hs_ref_slots[nth]->ssl_sqt.sqt_dtp */ )
	      {
		cv_artm (&asg_code, (ao_func_t) box_identity, ssl, hs->hs_ref_slots[nth], NULL);
	...
	if (is_fill_dt)
	  ...
	  setp_ha_find_col (setp, tb_dfe, out->_.col.col, hs->hs_out_cols + inx, hs->hs_out_cols_indexes + inx);
	inx++;
      }
      END_DO_SET ();
  ...
  if (asg_code)
    hs->src_gen.src_after_code = code_to_cv (sc, asg_code);
  if (2 == tb_dfe->_.table.is_right_oj)
    {
      /* two consec instance words for the roj record */
      ...
      if (roj_key_out)
	hs->hs_roj_key_out = (state_slot_t **) dk_set_to_array (dk_set_nreverse (roj_key_out));
    }
  hs->src_gen.src_after_test = sqlg_pred_body (so, tb_dfe->_.table.join_test);
  if (hs->src_gen.src_after_code && hs->src_gen.src_after_test)
    {
      SQL_NODE_INIT (end_node_t, en, end_node_input, NULL);
      hs->src_gen.src_continuations = dk_set_cons ((void *) en, NULL);
      en->src_gen.src_pre_code = hs->src_gen.src_after_code;
      hs->src_gen.src_after_code = NULL;
      en->src_gen.src_after_test = hs->src_gen.src_after_test;
      hs->src_gen.src_after_test = NULL;
    }
  hs->hs_is_outer = tb_dfe->_.table.ot->ot_is_outer;
  hs->hs_after_join_test = sqlg_pred_body (so, tb_dfe->_.table.after_join_test);
  sqlg_top_oby_pre (so, tb_dfe, (table_source_t *) hs, pre_code);
...
void
sqlg_append_end_node (sql_comp_t * sc, data_source_t * after, code_vec_t pre_code, code_vec_t after_test, code_vec_t after_code)
{
  while (qn_next (after))
    after = qn_next (after);
  {
    SQL_NODE_INIT (end_node_t, en, end_node_input, NULL);
    after->src_continuations = dk_set_cons ((void *) en, NULL);
    en->src_gen.src_pre_code = pre_code;
    en->src_gen.src_after_test = after_test;
...
data_source_t *
sqlg_make_dt_hash_ref (sqlo_t * so, df_elt_t * tb_dfe, ST ** target_names, dk_set_t * pre_code)
{
  code_vec_t asg_cv = NULL, jtest = NULL;
  hash_area_t *ha_copy = (hash_area_t *) dk_alloc (sizeof (hash_area_t));
  dk_set_t asg_code = NULL;
  dk_set_t roj_key_out = NULL;
  sql_comp_t *sc = so->so_sc;
  int inx = 0;
  dk_set_t ref_slots = NULL;
  op_table_t *ot = tb_dfe->_.sub.ot;
  setp_node_t *setp = ot->ot_hash_filler;
  hash_area_t *ha = setp->setp_ha;
  df_elt_t *fill_dt = tb_dfe->_.sub.hash_filler;
  SQL_NODE_INIT (hash_source_t, hs, hash_source_input, hash_source_free);
  hs->hs_cardinality = tb_dfe->dfe_arity;
  hs->hs_filler = setp->setp_fref;
  hs->hs_part_min = hs->hs_filler->fnr_hash_part_min;
  ...
  hs->hs_cl_part_opt = sqlo_opt_value (ot->ot_opts, OPT_HASH_REPLICATION) ? HS_CL_REPLICATED
      : sqlo_opt_value (ot->ot_opts, OPT_HASH_PARTITION) ? HS_CL_PART : 0;
  if (tb_dfe->_.sub.hash_need_colocate)
    hs->hs_cl_part_opt = HS_CL_REPLICATED;
  hs->clb.clb_fill = cc_new_instance_slot (so->so_sc->sc_cc);
  hs->hs_current_inx = cc_new_instance_slot (so->so_sc->sc_cc);
  hs->hs_saved_hmk = cc_new_instance_slot (so->so_sc->sc_cc);
  hs->hs_is_unique = sqlg_dt_hash_is_unq (tb_dfe);
  DO_SET (df_elt_t *, ref, &tb_dfe->_.sub.hash_refs)
  {
    state_slot_t *ssl = scalar_exp_generate (so->so_sc, ref->dfe_tree, pre_code);
    if (ssl->ssl_type == SSL_CONSTANT)
      {
  ...
  hs->hs_ref_slots = (state_slot_t **) list_to_array (ref_slots);
  hs->hs_ha = ha_copy;
  DO_BOX (df_elt_t *, out, inx, tb_dfe->_.sub.dt_hash_out)
  {
    state_slot_t *ssl = sqlg_dfe_ssl (so, out);
    ptrlong nth = inx;
    if (!out)
    ...
    if (nth < ha->ha_n_keys)
      {
	if (2 == tb_dfe->_.sub.is_right_oj)
	  t_set_push (&roj_key_out, (void *) ssl);
	if (!tb_dfe->_.sub.ot->ot_is_outer)
	  {
	    cv_artm (&asg_code, (ao_func_t) box_identity, ssl, hs->hs_ref_slots[nth], NULL);
	    if (DV_ANY == ssl->ssl_sqt.sqt_dtp)
	      ssl->ssl_sqt.sqt_col_dtp = DV_ANY;
  ...
  if (tb_dfe->_.sub.hash_ref_after_code)
    {
      dk_set_t iter;
      DO_BOX (df_elt_t *, dfe, inx, tb_dfe->_.sub.hash_ref_after_code);
      {
	if (IS_BOX_POINTER (dfe))
	  sqlg_dfe_code (so, dfe, &asg_code, 0, 0, 0);
      }
      END_DO_BOX;
      for (iter = tb_dfe->_.sub.hash_ref_defs; iter; iter = iter->next->next)
	{
	  df_elt_t *defd = sqlo_df (so, (ST *) iter->data);
	  df_elt_t *defining = sqlo_df (so, (ST *) iter->next->data);
	  if (defd && defining && defining->dfe_ssl)
	    defd->dfe_ssl = defining->dfe_ssl;
	}
    }
  if (asg_code)
    asg_cv = code_to_cv (sc, asg_code);

  sc->sc_is_first_cond = 0;
  if (tb_dfe->_.sub.join_test)
    jtest = sqlg_pred_body (so, tb_dfe->_.sub.join_test);
  if (asg_cv || jtest)
    sqlg_append_end_node (sc, (data_source_t *) hs, asg_cv, jtest, NULL);
  hs->hs_is_outer = tb_dfe->_.sub.ot->ot_is_outer;
  hs->hs_after_join_test = sqlg_pred_body (so, tb_dfe->_.sub.after_join_test);
  return ((data_source_t *) hs);
}
...
void
sqlg_proc_table_opts (sqlo_t * so, tvf_node_t * tvf, op_table_t * ot, dk_set_t * pre_code)
{
  int inx1, inx2;
  ST ***values = (ST ***) sqlo_opt_value (ot->ot_opts, OPT_VALUES);
  if (!values)
    return;
  int n = BOX_ELEMENTS (values);
  int nr = BOX_ELEMENTS (values[0]);
  state_slot_t **ssls = (state_slot_t **) dk_alloc_box_zero (n * nr * sizeof (caddr_t), DV_BIN);
  int fill = 0;
  DO_BOX (ST **, vals, inx1, values) DO_BOX (ST *, val, inx2, vals) ssls[fill++] = scalar_exp_generate (so->so_sc, val, pre_code);
  END_DO_BOX;
  END_DO_BOX;
  tvf->tvf_values = ssls;
}
...
void
tvf_free (tvf_node_t * tvf)
{
  dk_free_box ((caddr_t) tvf->tvf_out_slots);
  dk_free_box ((caddr_t) tvf->tvf_params);
  dk_free_box ((caddr_t) tvf->tvf_values);
}

data_source_t *
sqlg_tvf (sqlo_t * so, df_elt_t * dt_dfe, dk_set_t * precompute)
{
  ptrlong out_ctr = 3;
  ...
  sqlg_proc_table_opts (so, tvf, ot, precompute);
  ...
  END_DO_BOX;
  tvf->tvf_params = params;
  tvf->tvf_out_slots = list_to_array (out_slots);
  tvf->src_gen.src_after_test = sqlg_pred_body_1 (so, dt_dfe->_.sub.after_join_test, blob_to_string_code);
  tvf->tvf_ctr = cc_new_instance_slot (sc->sc_cc);
  tvf->tvf_nth_set = cc_new_instance_slot (sc->sc_cc);
  return (data_source_t *) tvf;
}


data_source_t *
sqlg_generate_proc_ts_1 (sqlo_t * so, df_elt_t * dt_dfe, dk_set_t * precompute)
{
  ptrlong out_ctr = 3;
  state_slot_t **params = NULL;
  int inx;
...
{
  ST ***vals = (ST ***) sqlo_opt_value (dt_dfe->_.sub.ot->ot_opts, OPT_VALUES);
  if (vals)
    return sqlg_tvf (so, dt_dfe, precompute);
  return sqlg_generate_proc_ts_1 (so, dt_dfe, precompute);
}
...
data_source_t *
sqlg_inline_sqs_1 (sql_comp_t * sc, df_elt_t * dfe, subq_source_t * sqs, data_source_t ** head, dk_set_t * pre_code)
{
  query_t *sqr = sqs->sqs_query;
  data_source_t *qn;
...
{
  data_source_t *qn = sqlg_inline_sqs_1 (sc, dfe, sqs, head, pre_code);
  int delay = sc->sc_delay_colocate;
  sc->sc_delay_colocate = 0;
  if ((data_source_t *) sqs != qn)
    return qn;
  return qn;
}
...
sqlg_make_dt (sqlo_t * so, df_elt_t * dt_dfe, ST ** target_names, dk_set_t * pre_code)
{
  ...
  if (dt_dfe->_.sub.hash_filler)
    return sqlg_make_dt_hash_ref (so, dt_dfe, target_names, pre_code);
...

  {
    ...
    if (!sc->sc_delay_colocate)
      sc->sc_delay_colocate = SC_DELAY_FOR_DT;
  ...
  if (!ST_P (tree, UNION_ALL_ST))
    {
      ...
      setp_left->setp_ssa.ssa_set_no = sc->sc_is_single_state ? NULL : sc->sc_set_no_ssl;
...
void
sqlg_chash_in_init (sqlo_t * so, df_elt_t * init_dfe, dk_set_t * code)
{
  /* a function for initing a chash for an in predicate has 3 out params, filled here:  The tree for the chash, the id of the free for cluster, the qi holding the tree for cluster, so to free the tree, the qi must be freed */
  instruction_t *ins = (instruction_t *) (*code)->data;
  if (CL_RUN_LOCAL == cl_run_local_only)
    {
      ins->_.call.params[1] = ssl_new_variable (so->so_sc->sc_cc, "chash_in_tree", DV_ANY);
      ins->_.call.params[1]->ssl_qr_global = 1;
    }
  else
    {
      ins->_.call.params[1] = ssl_new_variable (so->so_sc->sc_cc, "chash_in_tree", DV_ANY);
      ins->_.call.params[1]->ssl_qr_global = 1;
      ins->_.call.params[2] = ssl_new_variable (so->so_sc->sc_cc, "chash_in_tree_id", DV_ANY);
      ins->_.call.params[3] = ssl_new_variable (so->so_sc->sc_cc, "chash_in_tree_qi", DV_ANY);
      ins->_.call.params[2]->ssl_qr_global = 1;
      ins->_.call.params[3]->ssl_qr_global = 1;
    }
  /* record the args list in the contexxt so that the use of the hash finds them.  The extra field in the call for the init and the test matches the two */
  sqlg_qn_has_dfe ((data_source_t *) init_dfe->_.call.extra, (df_elt_t *) ins->_.call.params);
}
...
sqlg_dfe_code (sqlo_t * so, df_elt_t * dfe, dk_set_t * code, int succ, int fail, int unk)
{
  ...
  switch (dfe->dfe_type)
	...
	if (DFE_CALL == dfe->dfe_type && dfe->_.call.func_name && 0 == stricmp (dfe->_.call.func_name, "chash_in_init"))
	  sqlg_chash_in_init (so, dfe, code);
	break;
      }
    case DFE_EXISTS:
      {
    ...
    case DFE_DT:
      {
	/* select for effect &in a trans clause */
	char old_colo = so->so_sc->sc_delay_colocate;
	int old_ord = so->so_sc->sc_order;
	query_t *qr;
	df_elt_t *org_dfe;
	so->so_sc->sc_order = TS_ORDER_NONE;
	qr = sqlg_dt_query (so, dfe, NULL, (ST **) t_list (0));	/* this is to prevent assignment of NULL to constant ssl */
	cv_subq_qr (sc, code, qr);
	so->so_sc->sc_order = old_ord;
	so->so_sc->sc_delay_colocate = old_colo;
	sqlg_parallel_ts_seq (sc, dfe, (table_source_t *) qr->qr_head_node, NULL, qr->qr_select_node);
	dk_set_push (&sc->sc_cc->cc_query->qr_subq_queries, qr);
...
sqlg_qr_hf_nodes (query_t * qr)
{
  ...
  DO_SET (fun_ref_node_t *, fref, &qr->qr_nodes)
  {
    if (IS_QN (fref, hash_fill_node_input))
      {
	data_source_t *qn2;
	for (qn2 = fref->fnr_select; qn2; qn2 = qn_next (qn2))
	  {
	    if (IS_QN (qn2, stage_node_input))
	      t_set_push (&hf, ((stage_node_t *) qn2)->stn_loc_ts);
	    t_set_push (&hf, (void *) qn2);
	  }
      }
  }
  END_DO_SET ();
...
sqlg_is_multistate_gb (sqlo_t * so)
{
  ...
  for (qr = sc->sc_cc->cc_query->qr_super; qr; qr = qr->qr_super)
    {
      dk_set_t hf_nodes = sqlg_qr_hf_nodes (qr);
  ...
  for (sc = sc->sc_super; sc; sc = sc->sc_super)
    {
...
sqlg_distinct_fun_ref_col (sql_comp_t * sc, state_slot_t * data, dk_set_t prev_keys, long n_rows, state_slot_t * set_no)
{
  ...
  setp.setp_ha->ha_tree->ssl_qr_global = 1;
  setp.setp_ha->ha_set_no = set_no;
  dk_set_free (setp.setp_keys);
  setp.setp_ha->ha_is_gby_distinct = 1;
  return (setp.setp_ha);
}
  ...
  if (SSL_CONSTANT == ssl->ssl_type)
    {
      ...
      ssl2->ssl_sqt.sqt_non_null = ssl->ssl_sqt.sqt_non_null;
...
{
  int txf = enable_mt_txn ? 1 : CV_IS_LOCAL_NO_ENLIST;
  table_source_t *ts;
  ...
  for (ts = ts; ts; ts = (table_source_t *) qn_next ((data_source_t *) ts))
    {
      if (IS_TS (ts))
	{
	  cost += ts->ts_cost * card;
	  if (ts->ts_cardinality)
	    card *= ts->ts_cardinality;
	}
      if (IS_HS (ts))
	{
	  QNCAST (hash_source_t, hs, ts);
	  cost += card * 0.03;
	  if (hs->hs_cardinality)
	    card *= hs->hs_cardinality;
	}
    }
  return cost;
}
...
/* reasons for making sdfg */
#define SDFG_HIGH_CARD_GBY 1	/* partition because high card group by or distinct or user aggr */
...
void
sqlg_sdfg_empty_qf (sql_comp_t * sc, data_source_t * first_qn)
{
  SQL_NODE_INIT (query_frag_t, qf, query_frag_input, query_frag_free);
  sc->sc_any_clb = 1;
  top_sc->sc_any_clb = 1;
  qf->qf_is_sdfg_dummy = 1;
  qf->qf_n_stages = 1;
  qf->qf_dfg_state = cc_new_instance_slot (sc->sc_cc);
  qf->qf_dfg_agg_map = ssl_new_variable (sc->sc_cc, "dfg_map", DV_BIN);
  sc->sc_qf = qf;
}


void
sqlg_sdfg_part (sql_comp_t * sc, setp_node_t * setp, table_source_t * prev_ts, data_source_t * prev_qn, int is_sdfg)
{
  /* add a dfg stage to partition input of a partitioned setp */
  ...
  if (!is_hs)
    {
      ...
      if (SDFG_POST_SDFG == is_sdfg)
	{
	  /* can be two group bys one after the other.  So second may be low card, so no partition unless 2nd is high card also */
	  if (!setp_is_high_card (setp) && !setp->setp_loc_ts)
	    return;
	}
      ...
      if (reader)
	{
	  ...
	  reader->ts_nth_slice = cc_new_instance_slot (sc->sc_cc);
	}
    }
  if (sqlg_sdfg_colocated (sc, setp))
    return;
  SQL_NODE_INIT (stage_node_t, stn, stage_node_input, stage_node_free);
  if (!sc->sc_qf)
    sqlg_sdfg_empty_qf (sc, (data_source_t *) prev_ts);
  stn->src_gen.src_pre_code = setp->src_gen.src_pre_code;
  setp->src_gen.src_pre_code = NULL;
  stn_init (sc, stn);
  stn->stn_qf = sc->sc_qf;
  t_set_push (&sc->sc_dfg_stages, (void *) stn);
  dk_set_delete (&sc->sc_cc->cc_query->qr_nodes, (void *) stn);
  dk_set_ins_before (&qr->qr_nodes, setp, stn);
  if (prev_qn)
    prev_qn->src_continuations->data = (void *) stn;
  stn->src_gen.src_continuations = dk_set_cons ((void *) setp, NULL);
  if (is_hs)
    {
      sdfg_hs_loc_ts (sc, hs);
      stn->stn_loc_ts = hs->hs_loc_ts;
    }
  else
    {
      sdfg_setp_loc_ts (sc, setp);
      stn->stn_loc_ts = setp->setp_loc_ts;
    }
}

data_source_t *
qf_first_qn (query_frag_t * qf, qn_input_fn in)
{
  data_source_t *found = NULL;
...
table_source_t *
qf_first_ts (query_frag_t * qf)
{
  stage_node_t *first_stn = NULL;
  table_source_t *found = NULL;
  DO_SET (table_source_t *, qn, &qf->qf_nodes)
  {
    if (IS_QN (qn, stage_node_input))
      first_stn = (stage_node_t *) qn;
    if (IS_TS (qn))
      found = qn;
  }
  END_DO_SET ();
  if (!found && first_stn)
    {
      /* in a bsp block can be a stn with no ts.  In this case return loc ts of stn with the needed fields set after the stn */
      table_source_t *loc_ts = first_stn->stn_loc_ts;
      loc_ts->ts_aq_qis = first_stn->stn_slice_qis;
      loc_ts->ts_aq = first_stn->stn_aq;
      return loc_ts;
    }
  return found;
}
...
{
  int ts_ctr = 0, is_sdfg = 0, first_ts_only = 0;
  table_source_t *prev_ts = NULL;
  data_source_t *prev_qn = NULL;
  data_source_t *first_qn = (data_source_t *) ts, *qn;
  stage_node_t *first_stn = NULL;
  state_slot_t *sdfg_itcl = NULL;
  ssl_index_t sdfg_state = 0;
  state_slot_t *aq_ssl = NULL;
  state_slot_t *qi_ssl = NULL;
  if (CL_RUN_LOCAL != cl_run_local_only)
    return;
  if (!sqlg_may_parallelize (sc, (data_source_t *) ts))
    return;
  sc->sc_dfg_stages = NULL;
  if (sc->sc_qf)
    {
      table_source_t *prev_qf_ts = qf_first_ts (sc->sc_qf);
      first_stn = (stage_node_t *) qf_first_qn (sc->sc_qf, (qn_input_fn) stage_node_input);
      is_sdfg = SDFG_POST_SDFG;
      aq_ssl = prev_qf_ts->ts_aq;
      qi_ssl = prev_qf_ts->ts_aq_qis;
      sdfg_state = prev_qf_ts->ts_in_sdfg;
      sdfg_itcl = sc->sc_qf->clb.clb_itcl;
    }
  else
    {
	...
	sdfg_itcl = ssl_new_variable (sc->sc_cc, "sdfg_clrg", DV_ANY);
    }
  for (ts = ts; ts; ts = (table_source_t *) qn_next ((data_source_t *) ts))
    {
      if (is_sdfg)
	{
	  sqlg_process_all_eq (sc, ts->src_gen.src_pre_code);
	  sqlg_process_all_eq (sc, ts->src_gen.src_after_test);
	  sqlg_process_all_eq (sc, ts->src_gen.src_after_code);
	}
      if (IS_QN (ts, set_ctr_input))
	{
	  QNCAST (set_ctr_node_t, sctr, ts);
	  if (sctr->sctr_ose && SCTR_RIGHT_OJ != sctr->sctr_role)
	    {
	      for (ts = ts; ts; ts = (table_source_t *) qn_next ((data_source_t *) ts))
		{
      ...
      if (IS_TS (ts))
	{
	  ...
	  if (first_ts_only && ts_ctr)
	    ;			/* if ordered gby, do not make extra threads after the first ts & */
	  else if (is_sdfg)
	    {
	      ts->ts_aq = aq_ssl ? aq_ssl : (aq_ssl = ssl_new_variable (sc->sc_cc, "aq", DV_ANY));
	      ts->ts_aq_qis = qi_ssl ? qi_ssl : (qi_ssl = ssl_new_variable (sc->sc_cc, "branch_qis", DV_ANY));
	      ts->ts_in_sdfg = sdfg_state ? sdfg_state : (sdfg_state = cc_new_instance_slot (sc->sc_cc));
	    }
	  else
	    {
      ...
      if (is_sdfg && IS_QN (ts, chash_read_input) && ts->ts_nth_slice)
	{
	  ...
	  ts->ts_aq_qis = qi_ssl;
	  ts->clb.clb_itcl = sdfg_itcl;
	  ts->ts_in_sdfg = sdfg_state;
	  ts->ts_qf = sc->sc_qf;
	  prev_ts = ts;
	}
      if (is_sdfg && IS_QN (ts, chash_read_input) && ts->ts_is_bsp_reader)
	{
	  ts->ts_part_gby_reader = 1;
	  ts->ts_aq_state = cc_new_instance_slot (sc->sc_cc);
	  ts->ts_aq_qis = qi_ssl;
      ...
      if (is_sdfg && IS_QN (ts, setp_node_input))
	{
	  if (fref)
	    fref->fnr_has_sdfg = 1;
	  sqlg_sdfg_part (sc, (setp_node_t *) ts, prev_ts, prev_qn, is_sdfg);
	}
      else if (IS_QN (ts, hash_source_input)
	  && (((hash_source_t *) ts)->hs_roj || HS_CL_PART == ((hash_source_t *) ts)->hs_cl_partition))
	sqlg_sdfg_part (sc, (setp_node_t *) ts, prev_ts, prev_qn, is_sdfg);
      prev_qn = (data_source_t *) ts;
    }
  if (sc->sc_dfg_stages || SDFG_POST_SDFG == is_sdfg)
    {
      int out_bytes;
      int nth = 2;
      if (first_stn)
	{
	  out_bytes = first_stn->stn_out_bytes;
	  nth = BOX_ELEMENTS (sc->sc_qf->qf_stages) + 1;
	}
      else
	{
	  out_bytes = cc_new_instance_slot (sc->sc_cc);
	}
      sc->sc_dfg_stages = dk_set_nreverse (sc->sc_dfg_stages);
      sc->sc_qf->clb.clb_itcl = sdfg_itcl;
      DO_SET (stage_node_t *, stn, &sc->sc_dfg_stages)
      {
	if (!first_stn)
	  {
	    sc->sc_qf->qf_head_node = (data_source_t *) stn;
	    stn->stn_coordinator_id = cc_new_instance_slot (sc->sc_cc);
	    stn->stn_coordinator_req_no = ssl_new_variable (sc->sc_cc, "coord_req_no", DV_LONG_INT);
	    stn->stn_bulk_input = ssl_new_variable (sc->sc_cc, "bulk_input", DV_ARRAY_OF_POINTER);
	    stn->stn_out_bytes = out_bytes;
	    stn->stn_need_enlist = sc->sc_qf->qf_need_enlist;
	    sc->sc_qf->qf_dfg_req_no = stn->stn_coordinator_req_no;
	    first_stn = stn;
	  }
	else
	  {
	...
	if (!prev_ts)
	  {
	    stn->stn_slice_qis = stn->stn_qf->qf_slice_qis = ssl_new_variable (sc->sc_cc, "qis", DV_ARRAY_OF_POINTER);
	    stn->stn_aq = stn->stn_qf->qf_aq = ssl_new_variable (sc->sc_cc, "aq", DV_ANY);
	  }
	else
	  {
	...
	{
	  caddr_t new_stages = (caddr_t) dk_set_to_array (sc->sc_dfg_stages);
	  caddr_t old_stages = (caddr_t) first_stn->stn_qf->qf_stages;
	  first_stn->stn_qf->qf_stages = (stage_node_t **) box_concat (old_stages, new_stages);
	  dk_free_box (old_stages);
	  dk_free_box (new_stages);
	}
      else
	first_stn->stn_qf->qf_stages = (stage_node_t **) dk_set_to_array (t_cons (NULL, sc->sc_dfg_stages));
      sc->sc_dfg_stages = NULL;
      for (qn = first_qn; qn; qn = qn_next (qn))
	dk_set_push (&sc->sc_qf->qf_nodes, (void *) qn);
    }
  sc->sc_qf_ret = sc->sc_qf;
  sc->sc_qf = NULL;
}
...
cv_copy (code_vec_t cv)
{
  ...
  DO_INSTR (ins, 0, copy)
  {
      ...
      case INS_QNODE:
	{
	  ...
	  DO_BOX (caddr_t, ia, inx, dp->dp_input_args) BOXC (dp->dp_input_args[inx]);
	  END_DO_BOX;
	  break;
	}
      }
...
fnr_qf (data_source_t * qn)
{
  for (qn = qn; qn; qn = qn_next (qn))
	...
	return ((table_source_t *) qn)->ts_qf;
    }
  return NULL;
}
...
sqlg_fref_qp (sql_comp_t * sc, fun_ref_node_t * fref, df_elt_t * dt_dfe)
{
  ...
  for (ts = ts; ts; ts = (table_source_t *) qn_next ((data_source_t *) ts))
      ...
      if (IS_TS (ts) || (IS_QN (ts, setp_node_input) && ((setp_node_t *) ts)->setp_partitioned))
	{
      ...
      if (IS_QN (ts, fun_ref_node_input))
	prev_fref = (fun_ref_node_t *) ts;
...
void
sqlg_gby_exec (sqlo_t * so, df_elt_t * dt_dfe, setp_node_t * setp)
{
  sql_comp_t *sc = so->so_sc;
  ST *exec = (ST *) sqlo_opt_value (dt_dfe->_.sub.ot->ot_opts, GROUP_BY_EXEC);
  if (exec)
    {
      SQL_PROC_SAVE;
      int n_cols;
      dbe_table_t *tmp_tb = so_name_to_temp_table (so, exec->_.gb_ext.table);
      NEW_VARZ (gb_op_t, go);
      sc->sc_in_group_by_exec = go;
      if (tmp_tb->tb_primary_key->key_partition)
	{
	  setp->setp_partitioned = 1;
	  setp->setp_ht_id = tmp_tb->tb_temp_id_ssl;
	}
      n_cols = dk_set_length (tmp_tb->tb_primary_key->key_parts);
      dk_set_push (&setp->setp_gb_ops, (void *) go);
      go->go_op = AMMSC_EXEC;
      go->go_temp_table = tmp_tb;
      go->go_exec_base_set = ssl_new_variable (sc->sc_cc, "base_set", DV_LONG_INT);
      go->go_exec_base_set->ssl_qr_global = 1;
      go->go_exec_rows = ssl_new_variable (sc->sc_cc, "gby_rows", DV_LONG_INT);
      go->go_exec_ssls = (state_slot_t **) dk_alloc_box_zero (sizeof (caddr_t) * n_cols, DV_BIN);
      sqlc_compound_stmt (so->so_sc, exec->_.gb_ext.stmt, NULL);
      go->go_ua_acc_setp_call = code_to_cv (sc, sc->sc_routine_code);
      SQL_PROC_RESTORE;

      sc->sc_in_group_by_exec = NULL;
    }
}
    ...
    state_slot_t ** ssl_out, df_elt_t * tb_dfe, int is_gb, dk_set_t o_code, df_elt_t * oby, ST ** selection)
{
  ...
  for (;;)
	...
	setp->setp_card = oby->_.setp.gb_card;
	setp->setp_ssa.ssa_set_no = sc->sc_is_single_state ? NULL : sqlg_set_no_if_needed (sc, head);
	nth_part = 0;
	if (is_grouping_sets)
	  {
	    if (so->so_sc->sc_grouping)
	...
	DO_BOX (ST *, spec, inx, order_by)
	{
	  ...
	  sqlg_qn_has_dfe ((data_source_t *) ssl, (df_elt_t *) spec->_.o_spec.col);
	  NCONCF1 (setp->setp_keys, ssl);
	...
	if (is_gb)
	  {
	    if (!is_grouping_sets && oby->_.setp.is_linear && !setp->setp_any_user_aggregate_gos && !setp->setp_any_distinct_gos)
	      {
		setp->setp_is_streaming = 1;
		setp->setp_streaming_ssl = scalar_exp_generate (sc, oby->_.setp.order_col->dfe_tree, &code);
	      }
	    sqlg_gby_exec (so, tb_dfe, setp);
	...
	END_DO_SET ();
	if (DFE_DT == tb_dfe->dfe_type && tb_dfe->_.sub.ot->ot_first_oby && tb_dfe->_.sub.ot->ot_has_top_test && is_gb
	    && SEL_TOP (dt))
	  {
	    if (setp->setp_is_streaming)
	...
	if (!is_gb && (gs_top || (SEL_TOP (dt) && !dfe_ot (tb_dfe)->ot_oby_dfe->dfe_next)))
	  {
	    ...
	    if (!tb_dfe->_.sub.ot->ot_is_top_gby && tb_dfe->_.sub.ot->ot_has_top_test)
	      {
	...
	if (is_grouping_sets)
	...
	else
	  {
	    ...
	    if (setp->setp_is_streaming)
	      sql_node_append ((data_source_t **) & setp, read_node);
	    else
...
sqlg_simple_fun_ref (sqlo_t * so, data_source_t ** head, df_elt_t * tb_dfe, dk_set_t cum_code)
{
...

  {
    ...
    else if (last->src_after_code)
      {
	SQL_NODE_INIT (end_node_t, en, end_node_input, NULL);
	last->src_continuations = dk_set_cons ((void *) en, NULL);
	last = (data_source_t *) en;
...
sqlg_oby_node (sqlo_t * so, data_source_t ** head, df_elt_t * oby, df_elt_t * dt_dfe, dk_set_t pre_code)
{
  ...
  if (oby->_.setp.is_late_proj)
    ...
    {
      ssl_out = (state_slot_t **) t_box_copy ((caddr_t) tree->_.select_stmt.selection);
  ...
  sc->sc_fref->fnr_setp->setp_multistate_oby = sqlg_is_multistate_gb (so);
...
/*  caddr_t *new_sel = NULL;*/
  ...
  if (sc->sc_is_single_state)
    {
      sel->sel_top_level_single_set = 1;
      sel->sel_set_no = NULL;
    }
...
void
csa_free (cset_align_node_t * csa)
{
  dk_free_box ((caddr_t) csa->csa_ssls);
}


void
sqlg_cset_dt_end (sqlo_t * so, data_source_t * head, dk_set_t pre_code)
{
  sql_comp_t *sc = so->so_sc;
  SQL_NODE_INIT (cset_align_node_t, csa, cset_align_input, csa_free);
  sql_node_append (&head, (data_source_t *) csa);
  csa->src_gen.src_pre_code = code_to_cv (sc, pre_code);
}
...
sqlg_dt_query_1 (sqlo_t * so, df_elt_t * dt_dfe, query_t * ext_query, ST ** target_names, state_slot_t *** sel_out_ret)
{
  ...
  switch (dt_dfe->dfe_type)
	  ...
	  switch (dfe->dfe_type)
	      ...
	      if (DFE_TABLE == dfe->dfe_type)
		{
		  ...
		  if (dfe->_.table.text_only)
		    ...
		    last_qn = dfe->_.table.text_node;
		  last_qn = sqlg_ts_exec_opt (so, dfe, last_qn);
		}
		...
		sqlg_rdf_inf (dfe, qn, &head);
	      if ((DFE_TABLE == dfe->dfe_type && dfe->_.table.ot->ot_is_outer && HR_FILL != dfe->_.table.hash_role
		      && !dfe->_.table.is_right_oj) || (DFE_TABLE == dfe->dfe_type && 2 == dfe->_.table.is_right_oj))
		{
		  sqlg_set_no_if_needed (sc, &head);
		  qn_ensure_prev (sc, &head, qn);
		  sqlg_outer_with_iters (dfe, qn, &head);
		}
	      else if (DFE_DT == dfe->dfe_type && dfe->_.sub.ot->ot_is_outer
		  && (IS_QN (qn, hash_source_input) || dfe->_.sub.ot->ot_is_proc_view))
		{
	      ...
	      else if (DFE_DT == dfe->dfe_type && dfe->_.sub.ot->ot_is_outer && IS_QN (qn, subq_node_input))
		{
      ...
      if (sel_out_ret)
	{
	  *sel_out_ret = sqlg_handle_select_list (so, dt_dfe, &head, pre_code, last_qn, target_names);
      ...
      else if (dt_dfe->_.sub.ot->ot_is_cset_dt)
	sqlg_cset_dt_end (so, head, pre_code);
      else if (!was_setp)
  ...
  sc->sc_delay_colocate = delay_colo;
  return qr;
}
...
{
  dk_set_t parts = NULL;
  if (!dfe || (DFE_QEXP == dfe->dfe_type && UNION_ALL_ST == dfe->_.qexp.op && dfe_qexp_list (dfe, UNION_ALL_ST, &parts)))
    return 1;
  return 0;
}
...
dfe_union_dfe (df_elt_t * dt_dfe)
{
  ...
  char not_alone = 0;
  if (!dt_dfe || DFE_DT != dt_dfe->dfe_type || dt_dfe->_.sub.trans)
    return NULL;
...
int
t_set_pushfirst (dk_set_t * l, void *elt)
{
  int is_in = t_set_delete (l, elt);
  t_set_push (l, elt);
  return is_in;
}
...
dfe_unit_col_loci (df_elt_t * dfe)
{
  ...
  switch (dfe->dfe_type)
	...
	{
	  if (t_set_pushfirst (&dfe->dfe_sqlo->so_hash_fillers, (void *) dfe))
	...
	if (dfe->_.sub.generated_dfe)
	...
	else
	  {
	    ...
	    if (dfe->_.sub.hash_filler)
	      dfe_unit_col_loci (dfe->_.sub.hash_filler);
	    dfe_list_col_loci ((df_elt_t *) dfe->_.sub.hash_ref_after_code);
...
void
sqlg_lit_params (sql_comp_t * sc)
{
  st_lit_state_t *stl = sc->sc_stl;
  query_t *qr = sc->sc_cc->cc_query;
  int inx;
  if (!stl || !stl->stl_lit_dfes)
    return;
  for (inx = BOX_ELEMENTS (stl->stl_lit_dfes) - 1; inx >= 0; inx--)
    {
      df_elt_t *dfe = stl->stl_lit_dfes[inx];
      char str[20];
      state_slot_t *ssl;
      if (!dfe)
	continue;
...
{
  DO_SET (state_slot_t *, ssl, &qr->qr_state_map) if (i == ssl->ssl_index)
    printf ("%p\n", ssl);
  END_DO_SET ();
}
