...
sprintff_is_proven_bijection (const char *f)
{
  ...
  while ('\0' != tail[0])
	...
	{
	  tail += 2;
	  while (isalnum ((unsigned char) (tail[0])) || ('_' == tail[0]))
	    tail++;
	  if ('}' != tail[0])
...
again:
  ...
  for (f1_var = f1; ((f1_var < f1_end) && ('%' != f1_var[0])); f1_var++)
    { /* no-op */ ;
    }
  for (f2_var = f2; ((f2_var < f2_end) && ('%' != f2_var[0])); f2_var++)
    { /* no-op */ ;
    }
  ...
  if ((f1_var == f1) && (f1_var < f1_end - 1))
      ...
      for (f1_var_end = f1_var + 2; (f1_var_end < f1_end); f1_var_end++)
	{
      ...
      for (f1_rest = f1_var_end; ((f1_rest < f1_end) && ('%' != f1_rest[0])); f1_rest++)
	{ /* no-op */ ;
	}
    }
  else
  ...
  if ((f2_var == f2) && (f2_var < f2_end - 1))
      ...
      for (f2_var_end = f2_var + 2; (f2_var_end < f2_end); f2_var_end++)
	{
      ...
      for (f2_rest = f2_var_end; ((f2_rest < f2_end) && ('%' != f2_rest[0])); f2_rest++)
	{ /* no-op */ ;
	}
    }
  else
...
/* Now it's time to do optimized left-hand searches */
  ...
  if (('d' == f1_v) && (f1_var_end < f1_rest) && !isdigit (f1_var_end[0]) && ('-' != f1_var_end[0]))
	...
	f2_aux += 2;
      while ((f2_aux < f2_var) && isdigit (f2_aux[0]))
	f2_aux++;
...
again:
  ...
  for (dbg_ctr = f1 - f1_initial; dbg_ctr--; /* no step */ )
    putchar ('~');
  printf ("\n");
  printf ("sff_isect: f2 = %s\n''''''''''''''''", f2_initial);
  for (dbg_ctr = f2 - f2_initial; dbg_ctr--; /* no step */ )
    putchar ('~');
  printf ("\n");
  printf ("sff_isect: res= ");
  for (dbg_ctr = 0; (dbg_ctr < (res_tail - res_buf)); dbg_ctr++)
    putchar (res_buf[dbg_ctr]);
  printf ("\n");
#endif
  ...
  if ((('%' != f1[0]) && ('%' != f2[0]) && (0 < strcmp_res)) ||	/* If both v1 and f2 starts from fixed chars and 2nd is greater than 1st */
	  ...
	  (('%' != f1[0]) ? 1 : ((('%' != f2[1]) && ('%' == f1[1])) || (('{' != f2[1]) && ('{' == f1[1]))))))
    {
      ...
      f2_tail = f1_tail;
      f1_tail = swap;
      swap = f2_last_replaced;
      f2_last_replaced = f1_last_replaced;
      f1_last_replaced = swap;
#ifdef SFF_DEBUG
      swap = f2_initial;
      f2_initial = f1_initial;
      f1_initial = swap;
#endif
    }
...
/* Now we know that f1 starts with variable part, f2 is either fixed or variable part */
  if (f1_tail <= f1)
	...
	{
	  f1_tail++;
	  while (isalnum ((unsigned char) (f1_tail[0])) || ('_' == f1_tail[0]))
	    f1_tail++;
	  if ('}' != f1_tail[0])
	    goto generic_tails;	/* Syntax error -- no '}' after "%{connvar" */
	  f1_tail++;
	  while (!isalpha (f1_tail[0]) && ('\0' != f1_tail[0]))
	    f1_tail++;
	  f1_v = (f1_tail++)[0];
	  if ('U' != f1_v)
  ...
  if (f2_tail <= f2)
      ...
      if ('%' == f2[0])
	{
	  if ('%' == f2[1])
	    ...
	    {
	      f2_tail = f2 + 2;
	      while (isalnum ((unsigned char) (f2_tail[0])) || ('_' == f2_tail[0]))
		f2_tail++;
	      if ('}' != f2_tail[0])
		goto generic_tails;	/* Syntax error -- no '}' after "%{connvar" */
	      f2_tail++;
	      while (!isalpha (f2_tail[0]) && ('\0' != f2_tail[0]))
		f2_tail++;
	      f2_v = (f2_tail++)[0];
	      f2_fix = '\0';
	      if ('U' != f2_v)
		goto generic_tails;	/* Syntax error -- no 'U' after "%{connvar}" */
		  ...
		  (f1_last_replaced != f1) && (f2_last_replaced != f2) && !memcmp (f1, f2, (f1_tail - f1)))
...
/* Now we slightly normalize the rest of processing to write code only for a half of f1_v and f2_v combinations */
  ...
  switch (f1_v)
    {
    case 'D':
      goto f1_v_is_D;		/* see below */
    case 'U':
      goto f1_v_is_U;		/* see below */
    case 'd':
      goto f1_v_is_d;		/* see below */
    case 'g':
      goto f1_v_is_g;		/* see below */
    case 's':
      goto f1_v_is_s;		/* see below */
    case 'u':
      goto f1_v_is_u;		/* see below */
    default:
      GPF_T;
...
static int
sprintff_pair_cmp (char *x, char *y)
{
  ...
  if (NULL == sprintff_intersect_mtx)
    {
      ...
      sprintff_known_intersects =
	  id_hash_allocate (1021, 2 * sizeof (caddr_t), sizeof (caddr_t), sprintff_pair_hash, sprintff_pair_cmp);
    }
...
static int
sff_dislike (const char *s1, const char *f2)
{
...
/* Now we know that f2 starts with variable part */
  ...
  switch (f2_v)
    {
    case 'D':
      goto f2_v_is_D;		/* see below */
    case 'U':
      goto f2_v_is_U;		/* see below */
    case 'd':
      goto f2_v_is_d;		/* see below */
    case 'g':
      goto f2_v_is_g;		/* see below */
    case 's':
      goto f2_v_is_s;		/* see below */
    case 'u':
      goto f2_v_is_u;		/* see below */
    default:
      goto generic_tails;	/* see below */
  ...
  for (;;)
      ...
      if ('%' == c2)
	{
	  if (('%' == c1) && ('%' == f2[f2_chk_pos + 1]))
	    {
	      s1_chk_pos++;
	      f2_chk_pos += 2;	/* Escaped percent char in f2 */
	      continue;
	    }
	  break;		/* Variable part vs fixed char */
	}
