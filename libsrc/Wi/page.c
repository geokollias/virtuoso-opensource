...
row_length (db_buf_t row, dbe_key_t * key)
{
  ...
  if (!kv)
  ...
  else
    {
      ...
      if (kv >= KV_LONG_GAP || !(row_key = key->key_versions[kv]))
	{
...
void
kc_var_col (dbe_key_t * key, buffer_desc_t * buf, db_buf_t row, dbe_col_loc_t * cl, db_buf_t * p1, row_size_t * l1, db_buf_t * p2,
    row_size_t * l2, unsigned short *offset)
{
  row_ver_t rv = IE_ROW_VERSION (row);
  int off, len;
...
int
str_delta (db_buf_t str1, db_buf_t str2, int len1, int len2, row_size_t * prefix_ref, unsigned short *delta, unsigned char *extra,
    int mode)
{
...
pfh_set_int (pf_hash_t * pfh, int32 v, short nth_cl, short irow, short place)
{
  ...
  if (place & 1)
    GPF_T1 ("should not have int at odd address");
...
short
pfh_var (pf_hash_t * pfh, dbe_col_loc_t * cl, db_buf_t str, int len, unsigned short *prefix_bytes, unsigned short *prefix_ref,
...
int
col_compressed_value (buffer_desc_t * buf, dbe_col_loc_t * cl, int left_of, caddr_t val, row_size_t * prefix_bytes,
    unsigned short *prefix_ref, unsigned char *extra, int mode, int first_only, key_ver_t kv_wanted)
{
  int delta;
    ...
    unsigned short *prefix_bytes, unsigned short *prefix_ref, dtp_t * extra, char *was_null, char first_only, row_fill_t * rf)
{
  ...
  if (cl->cl_row_version_mask)
      ...
      if (DV_DB_NULL == DV_TYPE_OF (val))
	{
      ...
      *was_null = 0;
      if (rf && rf->rf_pf_hash && cl->cl_nth < rf->rf_pf_hash->pfh_n_cols && rf->rf_pf_hash->pfh_kv == IE_KEY_VERSION (rf->rf_row))
	{
	  switch (cl->cl_sqt.sqt_dtp)
	    {
	    case DV_ANY:
      ...
      if (-1 == rc)
	rc = col_compressed_value (buf, cl, irow, val, prefix_bytes, prefix_ref, extra, CC_OFFSET, first_only,
	    IE_KEY_VERSION (row));
      /*ck1618 (rf, val, cl); */
      if (CC_OFFSET == rc)
	{
...
void
page_set_values (buffer_desc_t * buf, row_fill_t * rf, dbe_key_t * key, int irow, caddr_t * values, dp_addr_t leaf)
{
  dbe_col_loc_t *cl;
  ...
  if (rf->rf_no_compress)
	...
	res[inx] =
	    page_try_offset (buf, rf->rf_row, irow, cl, values[cl->cl_nth], &prefix_bytes[inx], &prefix_ref[inx], &extra[inx],
	    &comp_null[inx], first_only, rf);
	inx++;
      }
      END_DO_SET ();
    }
  ...
  for (inx = 0; key->key_key_var[inx].cl_col_id; inx++)
      ...
      if (cl->cl_row_version_mask)
	{
	  ...
	  if (CC_PREFIX == res[compressible_inx - 1])
	    {
	      row_set_prefix (rf, cl, values[val_inx], prefix_bytes[compressible_inx - 1], prefix_ref[compressible_inx - 1],
		  extra[compressible_inx - 1]);
	    }
	  else
	    row_set_col (rf, cl, values[val_inx]);
	}
	...
	{
	  if (CC_PREFIX == page_try_offset (buf, row, irow, cl, values[cl->cl_nth], &prefix_bytes[0], &prefix_ref[0], &extra[0],
		  &comp_null[0], 0, NULL))
  ...
  DO_CL (cl, key->key_row_var)
  {
	...
	if (CC_PREFIX == res[compressible_inx - 1])
      ...
      {
	if (CC_NONE != cl->cl_compression
	    && CC_PREFIX == page_try_offset (buf, row, irow, cl, values[cl->cl_nth], &prefix_bytes[0], &prefix_ref[0], &extra[0],
		&comp_null[0], 0, NULL))
	  row_set_prefix (rf, cl, values[val_inx], prefix_bytes[0], prefix_ref[0], extra[0]);
	else
	  row_set_col (rf, cl, values[val_inx]);
	  ...
	  if (rv)
	    {
	      ...
	      DO_SET (dbe_col_loc_t *, cl, &compressible)
	      {
		if ((rv & cl->cl_row_version_mask) && (ROW_NO_MASK & (ref = SHORT_REF (row + cl->cl_pos[rv]))) >= rd->rd_map_pos)
		  {
#if 0
...
int
row_refit_col (buffer_desc_t * buf, int map_pos, dbe_key_t * key, db_buf_t row, dbe_col_loc_t * cl, db_buf_t bytes, int new_len,
    int *space_ret)
{
  ...
  DO_CL (upd_cl, key->key_row_var)
  {
    if (upd_cl->cl_col_id == cl->cl_col_id)
      shift = 1;
    if (shift && 0 == (rv & upd_cl->cl_row_version_mask))
      {
  ...
  *space_ret -= new_len - len;
  ...
  if (new_gap < 0)
    GPF_T1 ("in place upd gone wrong, negative space left");
  if (new_gap)
...
page_row_spacing (buffer_desc_t * buf, short row_gap, short ins_inx, int ins_gap)
{
  ...
  DO_ROWS (buf, map_pos, row, NULL)
  {
    ...
    if (map_pos == ins_inx)
      ...
      gap = row_gap;
    if (fill + len + gap > PAGE_SZ)
      GPF_T1 ("respacing goes over end");
...
pf_rd_refit_1 (page_fill_t * pf, row_delta_t * rd, int recursive)
{
  ...
  if (!buf->bd_is_write || !buf->bd_is_dirty)
    GPF_T1 ("refit1 for non excl or non dirty buffer");
  for (inx = 0; inx < rd->rd_n_values; inx++)
    {
      if (rd->rd_upd_change[inx])
	{
	  rc = row_refit_col (buf, rd->rd_map_pos, key, row, rd->rd_upd_change[inx], (db_buf_t) rd->rd_values[inx],
	      box_length_on_row (rd->rd_values[inx]), &space);
	  if (!rc)
	    break;
	}
    }
...
pf_rd_insert_1 (page_fill_t * pf, row_delta_t * rd)
{
  ...
  page_map_t *pm = buf->bd_content_map;
  if (!buf->bd_is_write || !buf->bd_is_dirty)
    GPF_T1 ("not in on write in insert_1 ");
  n_ins_1++;
  ...
  if (len > rf.rf_space)
    GPF_T1 ("pf_rd_insert_1 overflowed target without splitting the page.");
...
pf_rd_append (page_fill_t * pf, row_delta_t * rd, row_size_t * split_after)
{
  ...
  if (KV_LEFT_DUMMY == rd->rd_key_version)
    {
      if (rf.rf_space != PAGE_DATA_SZ)
	GPF_T1 ("left dummy inserted not at start");
  ...
  if (pf->pf_current->bd_content_map->pm_filled_to > *split_after || rf.rf_row == rf.rf_large_row)
    {
	...
	{
	  extend = it_new_page (pf->pf_itc->itc_tree, pf->pf_org->bd_page, DPF_INDEX, 0, pf->pf_itc);
	  if (!extend)
	  ...
	  rdbg_printf_2 ((" page L=%d split under L=%d exte L=%d  org ct=%d post ct=%d \n", pf->pf_org->bd_page,
		  LONG_REF (pf->pf_org->bd_buffer + DP_PARENT), extend->bd_page, pf->pf_org->bd_content_map->pm_count,
		  pf->pf_current->bd_content_map->pm_count));
	}
  ...
  else
    {
      ...
      if (len > rf.rf_space)
	GPF_T1 ("pf_rd_append overflowed target without splitting the page.");
      if (rd->rd_copy_of_deleted)
	IE_SET_FLAGS (rf.rf_row, IEF_DELETE);
      if (len != ROW_ALIGN (len))
	rf.rf_row[len] = 0;	/* 0 for compression */
	...
	lt_rb_insert (pf->pf_itc->itc_lock_lt, pf->pf_current,
	    pf->pf_current->bd_buffer + pf->pf_current->bd_content_map->pm_filled_to);
      pm->pm_filled_to += ROW_ALIGN (len);
      page_write_gap (pf->pf_current->bd_buffer + pm->pm_filled_to, PAGE_SZ - pm->pm_filled_to);
      pm->pm_bytes_free -= ROW_ALIGN (len);
    }
...
pf_rd_list (page_fill_t * pf, char first_affected, dp_addr_t * first_dp_ret, char *del_current)
{
  ...
  int is_first = 1;
  *del_current = 0;
  if (!pf->pf_left && 1 == pf->pf_current->bd_content_map->pm_count && pf_is_single_leaf (pf))
    {
...
void
pg_delete_move_cursors (it_cursor_t * itc, buffer_desc_t * buf_from, int pos_from, buffer_desc_t * buf_to, int pos_to)
{
  it_cursor_t *it_list;
  ...
  while (it_list)
      ...
      if (buf_to)
	{
	  rdbg_printf (("  itc delete move by T=%d moved itc=%x  from L=%d pos=%d to L=%d pos=%d was_on_row=%d \n",
		  TRX_NO (itc->itc_ltrx), it_list, it_list->itc_page, it_list->itc_map_pos, buf_to->bd_page, pos_to,
		  it_list->itc_is_on_row));
	...
	{
	  if (it_list->itc_page == buf_from->bd_page && it_list->itc_map_pos == pos_from)
	    {
	      rdbg_printf (("  itc delete move inside page by T=%d moved itc=%x  from L=%d pos=%d to L=%d pos=%d was_on_row=%d \n",
		      TRX_NO (itc->itc_ltrx), it_list, it_list->itc_page, it_list->itc_map_pos, buf_from->bd_page, pos_to,
		      it_list->itc_is_on_row));
	      it_list->itc_map_pos = pos_to;
	      if (it_list->itc_desc_order)
...
rd_list_bytes (buffer_desc_t * buf, int n, row_delta_t ** rds, row_size_t * split_after, char *all_ins)
{
  ...
  for (inx = 0; inx < n; inx++)
	...
	case RD_INSERT:
	  ...
	  break;
	case RD_DELETE:
	  bytes -= row_length (buf->bd_buffer + buf->bd_content_map->pm_entries[rds[inx]->rd_map_pos], buf->bd_tree->it_key);
	  break;
	case RD_UPDATE:
...
pf_change_org (page_fill_t * pf)
{
  ...
  if (!LONG_REF (org->bd_buffer + DP_KEY_ID))
    GPF_T1 ("no key id in jbuffer");
  buf_set_dirty (org);
...
void
page_apply_1 (it_cursor_t * itc, buffer_desc_t * buf, int n_delta, row_delta_t ** delta, int op, page_apply_frame_t * paf)
{
    ...
    }
  {
	...
	if (delta_inx < n_delta && irow == delta[delta_inx]->rd_map_pos - irow_shift)
	  {
	    if (delta_inx < n_delta - 1 && delta[delta_inx + 1]->rd_map_pos <= delta[delta_inx]->rd_map_pos)
	      GPF_T1 ("deltas in page apply are in non increasing row number order");
		...
		pg_move_lock (pf.pf_itc, pf.pf_rls, pf.pf_rl_fill, irow, ITC_AT_END, pf.pf_current->bd_pl, pf.pf_left != NULL);
		if (0 == irow)
		  first_affected = 1;
		delta_inx++;
		break;
	      ...
	      case RD_UPDATE_LOCAL:
		if (KV_LEFT_DUMMY == IE_KEY_VERSION (buf->bd_buffer + org_pm->pm_entries[irow])
		    && KV_LEFT_DUMMY != delta[delta_inx]->rd_key_version)
	...
	if (irow < -1 || irow > org_pm->pm_count + n_delta + 1)
	  GPF_T1 ("irow out of range");
      }
  ...
  if (pf.pf_left)
	...
	GPF_T1 ("geo is not supposed to split like other inxes");
      page_reloc_right_leaves (pf.pf_itc, pf.pf_current);
    }
  if (pf.pf_rewrite_overflow)
    {
  ...
  if (PA_REWRITE_ONLY == op)
    return;
  if (PA_AUTOCOMPACT == op || itc->itc_insert_key->key_is_geo)
    first_affected = 0;
  ...
  if (1 == buf->bd_content_map->pm_count)
    {
      ...
      key_ver_t kv = IE_KEY_VERSION (row);
      dp_addr_t leaf = LONG_REF (row + LD_LEAF);
      if (kv != KV_LEFT_DUMMY && !pf->pf_itc->itc_insert_key->key_is_geo)
	...
	return 0;
      ...
      pg_delete_move_cursors (pf->pf_itc, buf, 0, leaf_buf, 0);
...
void
page_apply_parent (buffer_desc_t * buf, page_fill_t * pf, char first_affected, char op, char change, page_apply_frame_t * paf)
{
  ...
  if (!leaves)
	...
	pf_release_pl (pf);
      pg_check_map (buf);
      if (0 == LONG_REF (buf->bd_buffer + DP_PARENT) && pf_pop_root (pf))
	return;
  ...
  if (!dp_parent)
    {
      ...
      if (1 == first_affected)
	GPF_T1 ("first affected on root not right, should be left dummy");
      if (del_current)
	GPF_T1 ("not supposed to del root page");
  ...
  else
    {
      ...
      if (-1 == ileaf)
	GPF_T1 ("can't find leaf on parent in split/del;");
      ...
      if (del_current)
	{
	  ...
	  pg_delete_move_cursors (pf->pf_itc, buf, 0, parent, leaves[0]->rd_map_pos);
