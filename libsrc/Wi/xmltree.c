  ...
  if (is_string_type (dtp))
    {
      /* No need: this is checked above, because now there's only one string dtp:
         ...
         return val; */
      if (DV_ARRAY_OF_POINTER == val_dtp && DV_ARRAY_OF_POINTER == DV_TYPE_OF (XTE_HEAD (val)))
...
xqi_truth_value_of_box (caddr_t val)
{
  switch (DV_TYPE_OF (val))
    ...
    case DV_STRING:
    case DV_UNAME:
      return (1 < box_length (val));
    case DV_WIDE:
      return (sizeof (wchar_t) < box_length (val));
    case DV_LONG_INT:
      return unbox (val);
    case DV_DOUBLE_FLOAT:
      return (0 != unbox_double (val));
    case DV_SINGLE_FLOAT:
      return (0 != unbox_float (val));
    case DV_NUMERIC:
      return !num_is_zero ((numeric_t) val);
    case DV_XML_ENTITY:
      {
...
xqi_is_value (xp_instance_t * xqi, XT * tree)
{
  switch (tree->type)
	...
	if (NULL == body)
	  sqlr_new_error ("42000", "XI???",
	      "The XQuery text passed to XQuery processor is a library module, not an XQuery expression");
...
int
xt_predict_returned_type (XT * tree)
{
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (tree))
...
xqi_is_next_value (xp_instance_t * xqi, XT * tree)
{
  switch (tree->type)
    ...
    case CALL_STMT:
      return ((NULL != tree->_.xp_func.var) ? xqi_is_next_value (xqi, tree->_.xp_func.var) : 0);
    case XQ_QUERY_MODULE:
...
xp_compare_like (xp_instance_t * xqi, caddr_t x, caddr_t y)
{
  ...
  if (DV_STRINGP (x) && DV_STRINGP (y) && DVC_MATCH == cmp_like (x, y, NULL, 0, st, pt))
    return DVC_MATCH;
  return DVC_LESS;
}
...
{
  if ((DV_XML_ENTITY == DV_TYPE_OF (x)) && (DV_XML_ENTITY == DV_TYPE_OF (y)))
    {
...
xp_string (query_instance_t * qi, caddr_t val)
{
  ...
  if (DV_XML_ENTITY == dtp)
    {
      xml_entity_t *xe = (xml_entity_t *) val;
      xe->_->xe_string_value (xe, &res, DV_SHORT_STRING);
      return res;
    }
  else
    {
...
caddr_t
xslt_format_number (numeric_t value, caddr_t format, xslt_number_format_t * nf)
{
  int currency_format = 0;

  wchar_t *pattern = (wchar_t *) box_utf8_as_wide_char (format, NULL, strlen (format), 0);
  ...
  wchar_t digit, zero_digit, grouping_sep, decimal_sep, percent, per_mille, separator, minus_sign;
...
#define LOAD_FROM_XSNF(varname,field_name) \
  ...
  for (j = 1; j >= 0 && start < pattern_length; --j)
    {
      ...
      wchar_t *prefix = (wchar_t *) dk_alloc (pref_suf_size);
      wchar_t *suffix = (wchar_t *) dk_alloc (pref_suf_size);
      int decimal_pos = -1;
      ...
      for (pos = start; pos < pattern_length; ++pos)
	{
	  ...
	  switch (phase)
	      ...
	      if (in_quote)
		{
		  ...
		  if (ch == FN_QUOTE)
		    {
		      if ((pos + 1) < pattern_length && pattern[pos + 1] == FN_QUOTE)
			{
			  ++pos;
			  *affix_ptr++ = ch;	/* 'don''t' */
			}
	      ...
	      else
		{
		  /* Process unquoted characters seen in prefix or suffix
		     phase. */
		  if (ch == digit || ch == zero_digit || ch == grouping_sep || ch == decimal_sep)
		    {
		  ...
		  else if (ch == CURRENCY_SIGN)
		    {
		      /* Use lookahead to determine if the currency sign is
		         doubled or not. */
		      int doubled = (pos + 1) < pattern_length && pattern[pos + 1] == CURRENCY_SIGN;
		      *affix_ptr++ = doubled ? INTL_CURRENCY_SYMBOL : CURRENCY_SYMBOL;
		      if (doubled)
		  ...
		  /* Next handle characters which are appended directly. */
		      ...
		      if (multiplier != 1)
			{
			  ...
			  dk_free (suffix, pref_suf_size);
			  dk_free_box ((box_t) pattern);
			  sqlr_new_error ("22023", "XS039", "Too many percent/permille characters in format-number() pattern");
			}
		      multiplier = 100;
		    }
		      ...
		      if (multiplier != 1)
			{
			  ...
			  dk_free (suffix, pref_suf_size);
			  dk_free_box ((box_t) pattern);
			  sqlr_new_error ("22023", "XS040", "Too many percent/permille characters in format-number() pattern");
			}
		      multiplier = 1000;
		    }
		}
		  ...
		  if (digit_right_count > 0)
		    {
		      ...
		      dk_free (suffix, pref_suf_size);
		      dk_free_box ((box_t) pattern);
		      sqlr_new_error ("22023", "XS041", "Unexpected '0' in format-number() pattern");
		    }
		  ++zero_digit_count;
		  if (grouping_count >= 0 && decimal_pos < 0)
		    ++grouping_count;
		  ...
		  if (decimal_pos >= 0)
		    {
		      ...
		      dk_free (suffix, pref_suf_size);
		      dk_free_box ((box_t) pattern);
		      sqlr_new_error ("22023", "XS042", "Multiple decimal separators in format-number() pattern");
		    }
      ...
      if (zero_digit_count == 0 && digit_left_count > 0)
	{
	  ...
	  (decimal_pos >= 0 &&
		  ...
		  decimal_pos > (digit_left_count + zero_digit_count))) || grouping_count == 0 || in_quote)
	{
	  dk_free (prefix, pref_suf_size);
	  dk_free (suffix, pref_suf_size);
	  dk_free_box ((box_t) pattern);
      ...
      if (j == 1)
	{
	  ...
	  positive_prefix = box_wide_as_utf8_char ((ccaddr_t) prefix, wcslen (prefix), DV_SHORT_STRING);
	  positive_suffix = box_wide_as_utf8_char ((ccaddr_t) suffix, wcslen (suffix), DV_SHORT_STRING);
	  ...
	  max_int_digits = 127;
	  max_frac_digits = decimal_pos >= 0 ? (digit_total_count - decimal_pos) : 0;
	  min_frac_digits = decimal_pos >= 0 ? (digit_left_count + zero_digit_count - decimal_pos) : 0;
	  grouping_used = grouping_count > 0;
	  grouping_size = (grouping_count > 0) ? grouping_count : 0;
	  decimal_sep_always_shown = decimal_pos == 0 || decimal_pos == digit_total_count;
	}
      else
	{
	  negative_prefix = box_wide_as_utf8_char ((ccaddr_t) prefix, wcslen (prefix), DV_SHORT_STRING);
	  negative_suffix = box_wide_as_utf8_char ((ccaddr_t) suffix, wcslen (suffix), DV_SHORT_STRING);
	  got_negative = 1;
	}
      dk_free (prefix, pref_suf_size);
      dk_free (suffix, pref_suf_size);
  ...
  if (!got_negative || (!strcmp (negative_prefix, positive_prefix) && !strcmp (negative_suffix, positive_suffix)))
    {
      caddr_t new_prefix;
      ...
      new_prefix = dk_alloc_box (box_length (positive_prefix) + box_length (nf->xsnf_minus_sign) - 1, DV_SHORT_STRING);
  ...
  res = NULL;
  switch (numeric_rescale (_value, value, max_int_digits + max_frac_digits, max_frac_digits))
    {
    case NUMERIC_STS_UNDERFLOW:
    do_underflow:
      res = dk_alloc_box (box_length (nf->xsnf_minus_sign) + box_length (nf->xsnf_infinity) - 1, DV_SHORT_STRING);
	  ...
	  default:
	    {
	      ...
	      int padding_zero_count = min_int_digits - numeric_raw_precision (_value) + numeric_scale (_value);
	      int right_padding_zero_count = min_frac_digits - numeric_scale (_value);
	      int n_digits = 0;
	      int total_digits;
		...
		right_padding_zero_count = 0;
		  ...
		  max_frac_digits + padding_zero_count + right_padding_zero_count + box_length (res_suffix) + 1, DV_SHORT_STRING);
	      in_ptr = buffer;
		...
		{
		  if (grouping_used && n_digits && total_digits && (total_digits % grouping_size) == 0)
		    {
		      memcpy (out_ptr, nf->xsnf_grouping_sep, box_length (nf->xsnf_grouping_sep));
	      ...
	      if (max_frac_digits)
		{
		  ...
		  if (*in_ptr == '.' || right_padding_zero_count)
		    {
		      memcpy (out_ptr, nf->xsnf_decimal_sep, box_length (nf->xsnf_decimal_sep));
...
bif_xslt_format_number (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL != (err = numeric_from_x (value, val, NUMERIC_MAX_PRECISION, NUMERIC_MAX_SCALE, "xslt_format_number", -1, NULL)))
    sqlr_resignal (err);
  ...
  switch (tree->type)
	...
	QR_RESET_CTX
	{
	  switch (tree->type)
	    {
	    case BOP_PLUS:
	      box_add (left, right, (caddr_t *) xqi, &res_slot);
	      break;
	    case BOP_MINUS:
	      box_sub (left, right, (caddr_t *) xqi, &res_slot);
	      break;
	    case BOP_TIMES:
	      box_mpy (left, right, (caddr_t *) xqi, &res_slot);
	      break;
	    case BOP_DIV:
/* IvAn/Bug3413/021004 Right argument should be made double if it is integer */
...
#endif
	    case BOP_MOD:
	      box_mod (left, right, (caddr_t *) xqi, &res_slot);
	      break;
	    }
	...
	QR_RESET_CODE
	{
	  ...
	  state = ERR_STATE (err);
	  POP_QR_RESET;
	  if (!strcmp (state, "22012") && DV_TYPE_OF (left) != DV_NUMERIC && DV_TYPE_OF (right) != DV_NUMERIC)
	    {
	...
	if (!fp)
	  {
	    sqlr_new_error_xqi_xdl ("XP420", "XI002", xqi, "Undefined XPATH function '%.300s'", tree->_.xp_func.qname);
	  }
	fp (xqi, tree, ctx_xe);
	return;
      }
	...
	if (param_names)
	  {			/*create vector of pairs - (index, value) from tree->_.xq_for_sql.flwr_params */
		...
		if (!xb)
		  sqlr_new_error_xqi_xdl ("XP220", "XI...", xqi, "Unbound XPATH parameter '%.300s' used in SQL subquery",
		      param_names[inx]);
		sprintf (tmp, ":%d", inx + 1);
		...
		if (DV_ARRAY_OF_XQVAL != DV_TYPE_OF (val))
		...
		else
		  {
			...
			break;
		      default:
			sqlr_new_error_xqi_xdl ("XP220", "XI...", xqi,
			    "Sequence is passed as a value of XPATH parameter '%.300s' used in SQL subquery", param_names[inx]);
		      }
		  }
		parms[inx * 2 + 1] = val;
	      }
	...
	DO_BOX_FAST (XT *, def, inx, tree->_.module.defglobals)
	{
	  ...
	  if (NULL == init_expn)
		...
		{
		  if (!xb->xb_name)
		    sqlr_new_error_xqi_xdl ("XP220", "XI035", xqi,
			"Variable $%.300s is declared as extern but has no value passed to the query interpreter", name);
		  if (xb->xb_name == name)
		    break;
		}
	    }
	      ...
	      for (xb = xqi->xqi_xp_globals; NULL != xb; xb = xb->xb_next)
		{
		  if (!xb->xb_name)
		    break;
		  if (xb->xb_name == name)
		    sqlr_new_error_xqi_xdl ("XP220", "XI036", xqi,
			"non-external variable $%.300s is initialized by external parameter passed to the query interpreter", name);
		}
	      do
		{
...
xi_init (xp_instance_t * xqi, XT * tree, xml_entity_t * init)
{
  ...
  if (DV_XML_ENTITY != DV_TYPE_OF (init) && step_axis != XP_SELF)
    {
...
/* IvAn/SmartXContains/001025 Cases added */
	...
	{
	  rc = init->_->xe_attribute (init, -1, tree->_.step.node, XQI_ADDRESS (xqi, tree->_.step.iterator), NULL);
	  if (XI_NO_ATTRIBUTE == rc)
	    return XI_AT_END;
	}
	...
	rc = current->_->xe_up (current, tree->_.step.node,
	    (XE_UP_MAY_TRANSIT | XE_UP_SIDEWAY | ((XP_FOLLOWING == step_axis) ? XE_UP_SIDEWAY_FWD : 0)));
      if (XI_AT_END == rc)
	{
	  dk_free_box ((caddr_t) current);
	  return rc;
...
xqi_step_init_with_node_set (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  for (;;)
	...
	{
	  xml_entity_t **new_node_set =
	      (xml_entity_t **) dk_alloc_box_zero (2 * alloc_size * sizeof (xml_entity_t *), DV_ARRAY_OF_POINTER);
...
*/
int
xqi_pred_failed (xp_instance_t * xqi, XT * pred, xml_entity_t * current)
{
  int predicted;
	...
	if (!rc)
	  {
	    ...
	    rc = ((NULL == lattr) ? ((NULL == rattr) ? 0 : -1) : ((NULL == rattr) ? 1 : strcmp (lattr, rattr)));
	  }
	if (!rc)
	  {
...
int
xe_are_equal (xml_entity_t * this_xe, xml_entity_t * that_xe)
{
...
ptrlong
xe_equal_fingerprint (xml_entity_t * xe)
{
...
int
xe_have_equal_heads (xml_entity_t * this_xe, xml_entity_t * that_xe)
{
  int res;
...
ptrlong
xe_equal_heads_fingerprint (xml_entity_t * xe)
{
  ptrlong res;
...
xqr_instance (xp_query_t * xqr, query_instance_t * qi)
{
  ...
  xp_instance_t *xqi = (xp_instance_t *) dk_alloc_box (xqr->xqr_instance_length + sizeof (ptrlong) * n_slots, DV_XQI);
...
snprint_xdl (char *buffer, size_t buflength, xp_debug_location_t * xdl)
{
  ...
  if (NULL != xdl->xdl_attribute)
    {
      item_len = snprintf (buffer, buflength, " in attribute %.300s", xdl->xdl_attribute);
      res += item_len;
      buffer += item_len;
      buflength -= item_len;
    }
  ...
  if (NULL != xdl->xdl_element)
    {
      item_len = snprintf (buffer, buflength, " %.10s tag %.300s", ((NULL != xdl->xdl_attribute) ? "of" : "in"), xdl->xdl_element);
      res += item_len;
      buffer += item_len;
      buflength -= item_len;
    }
  ...
  if (NULL != xdl->xdl_file)
    {
      item_len = snprintf (buffer, buflength, " %.10s file %.300s", ((0 != xdl->xdl_line) ? "of" : "in"), xdl->xdl_file);
      res += item_len;
      buffer += item_len;
      buflength -= item_len;
    }
...
void
bif_xquery_arg (caddr_t * qst, state_slot_t ** args, int main_arg_inx, int cache_ssl_inx, int key2, const char *func,
    const char *keyname, caddr_t * str_ret, int *str_is_temp_ret, xml_entity_t ** ent_ret, xp_query_t ** xqr_ret)
{
  ...
  switch (dtp)
      ...
      if (NULL != xe->xe_attr_name)
	{
	  ...
	  sqlr_new_error ("22023", "SR014",
	      "Function %.200s needs %.200s as argument %d, not a special attribute XML entity '%s'", func, keyname,
	      main_arg_inx + 1, xe->xe_attr_name);
	}
...
caddr_t
xpath_or_xquery_eval (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, const char *funname, ptrlong predicate_type,
    int with_cache)
{
	...
	if (cvt)
	  {
		...
		if (NULL == val)
		  sqlr_new_error ("22003", "SR476",
		      "Out of memory allocation limits: %s() tries to return an abnormally long NVARCHAR", funname);
	      }
	    else
	      val = box_copy_tree (val);
	  }
...
const char *
xt_dump_opname (ptrlong opname, int is_op)
{
  ...
  if (is_op)
	...
	return "arithmetic operation '*'";
      case BOP_DIV:
	return "arithmetic operation 'div'";
      case BOP_MOD:
  ...
  switch (opname)
    ...
    case XP_ABS_DESC:
      return "descendant:: axis, from root, plain";
    case XP_ABS_DESC_WR:
      return "descendant:: axis, from root, free-text optimization";
    case XP_ABS_DESC_OR_SELF:
      return "descendant-or-self:: axis, from root, plain";
    case XP_ABS_DESC_OR_SELF_WR:
      return "descendant-or-self:: axis, from root, free-text optimization";
    case XP_ABS_CHILD:
      return "child:: axis, from root, plain";
    case XP_ABS_CHILD_WR:
      return "child:: axis, from root, free-text optimization";
    case XP_STEP:
      return "XPath step";
    case XP_FILTER:
      return "XPath filter";
    case XP_ANCESTOR:
      return "ancestor:: axis, from current node, plain";
    case XP_ANCESTOR_OR_SELF:
      return "ancestor-or-self:: axis, from current node, plain";
    case XP_ATTRIBUTE:
      return "attribute:: axis, from current node, plain";
    case XP_ATTRIBUTE_WR:
      return "ancestor:: axis, from current node, free-text optimization";
    case XP_CHILD:
      return "child:: axis, from current node, plain";
    case XP_CHILD_WR:
      return "child:: axis, from current node, free-text optimization";
    case XP_DESCENDANT:
      return "descendant:: axis, from current node, plain";
    case XP_DESCENDANT_WR:
      return "descendant:: axis, from current node, free-text optimization";
    case XP_DESCENDANT_OR_SELF:
      return "descendant-or-self:: axis, from current node, plain";
    case XP_DESCENDANT_OR_SELF_WR:
      return "descendant-or-self:: axis, from current node, free-text optimization";
    case XP_FOLLOWING:
      return "following:: axis, from current node, plain";
    case XP_FOLLOWING_SIBLING:
      return "following-sibling:: axis, from current node, plain";
    case XP_NAMESPACE:
      return "namespace:: axis, from current node, plain";
    case XP_PARENT:
      return "parent:: axis, from current node, plain";
    case XP_PRECEDING:
      return "preceding:: axis, from current node, plain";
    case XP_PRECEDING_SIBLING:
      return "preceding-sibling:: axis, from current node, plain";
    case XP_SELF:
      return "self:: axis, from current node, plain";
    case XP_NODE:
      return "node() test";
    case XP_NAME_EXACT:
      return "exact name check, free-text optimization";
    case XP_NAME_LOCAL:
      return "local-name check, partial free-text optimization";
    case XP_NAME_NSURI:
      return "namespace check";
    case XP_ROOT:
      return "root:: axis, plain";
    case XP_DEREF:
      return "pointer operator as pseudo-axis, from current node, plain";
    case XP_BY_MAIN_STEP:
      return "child:: axis, from current node, plain";
    case XP_TEXT:
      return "text() test";
    case XP_PI:
      return "processing-instruction() test";
    case XP_COMMENT:
      return "comment() test";
    case XP_VARIABLE:
      return "input from named variable on stack";
    case XP_LITERAL:
      return "input from constant value";
    case XP_ELT:
      return "element() test";
    case XP_ELT_OR_ROOT:
      return "element() or document-node() test";
    case XP_FAKE_VAR:
      return "input from a special or fake variable";
/*
...
*/
    case XP_NEAR:
      return "'near' free-text condition";
    case XP_WORD_CHAIN:
      return "'phrase' free-text condition";
    case XP_AND_NOT:
      return "'and not' free-text condition";
/*
...
*/
    ...
    case XQ_NS_DECL:
      return "XQuery namespace declaration";
      /*case XQ_SCHEMA_DECL: return "XQuery schema declaration"; */
    case XQ_DEFPARAM:
      return "Definition of parameter of user-defined XQuery function";
    case XQ_DEFGLOBAL:
      return "Definition of global variable";
    case XQ_DEFUN:
      return "User-defined XQuery function";
    case XQ_SEQTYPE:
      return "XQuery sequence type";
    case XQ_SEQTYPE_OPT_ONE:
      return "Sequence length: zero or one item";
    case XQ_SEQTYPE_REQ_ONE:
      return "Sequence length: exactly one item";
    case XQ_SEQTYPE_OPT_MANY:
      return "Sequence length: zero or more items";
    case XQ_SEQTYPE_REQ_MANY:
      return "Sequence length: one or more items";
    case XQ_SEQTYPE_DOCELEMENT:
      return "Sequence should consist of document nodes";
    case XQ_SEQTYPE_ELEMENT:
      return "Sequence should consist of elements";
    case XQ_SEQTYPE_ATTRIBUTE:
      return "Sequence should consist of attributes";
    case XQ_SEQTYPE_NODE:
      return "Sequence should consist of XML nodes";
    case XQ_NCNAME:
      return "Name without colon (NcName)";
    case XQ_QNAME:
      return "Name with colon (Qname)";
    case XQ_INSERT:
      return "XQuery DML command 'insert'";
    case XQ_MOVE:
      return "XQuery DML command 'move'";
    case XQ_UPSERT:
      return "XQuery DML command 'upsert'";
    case XQ_DELETE:
      return "XQuery DML command 'delete'";
    case XQ_RENAME:
      return "XQuery DML command 'rename'";
    case XQ_REPLACE:
      return "XQuery DML command 'replace'";
    case XQ_NOT:
      return "XQuery boolean operation 'not'";
    case XQ_INSTANCEOF:
      return "XQuery boolean operation 'instanceof'";
    case XQ_IN:
      return "XQuery operation 'in'";
    case XQ_ASSIGN:
      return "XQuery operation 'assign'";
    case XQ_BEFORE:
      return "XQuery boolean operation 'before'";
    case XQ_AFTER:
      return "XQuery boolean operation 'after'";
    case XQ_ASCENDING:
      return "XQuery sort specifier 'ascending'";
    case XQ_DESCENDING:
      return "XQuery sort specifier 'descending'";
    case XQ_CAST:
      return "XQuery operator 'cast'";
    case XQ_TREAT:
      return "XQuery operator 'treat'";
    case XQ_FOR:
      return "XQuery operator 'for'";
    case XQ_LET:
      return "XQuery operator 'let'";
    case XQ_IMPORT_MODULE:
      return "XQuery 'import module'";
    case XQ_IMPORT_SCHEMA:
      return "XQuery 'import schema'";
    }
  return NULL;
}
...
void
xt_dump_long (void *addr, dk_session_t * ses, int is_op)
{
  ...
  switch (hint)
	...
	switch (tree->type)
	  ...
	  case XP_NAME_NSURI:
		...
		{
		case XP_NAME_EXACT:
		  sprintf (buf, "EXACT NAME TEST: ");
		  break;
		case XP_NAME_LOCAL:
		  sprintf (buf, "LOCAL-NAME TEST: ");
		  break;
		case XP_NAME_NSURI:
		  sprintf (buf, "NAMESPACE TEST: ");
		  break;
		}
	      SES_PRINT (ses, buf);
	  ...
	  case XQ_DEFPARAM:
	    {
	      ...
	      sprintf (buf, "', %.20s, %.20s",
		  (tree->_.paramdef.is_bool ? "boolean" : "generic"), (tree->_.paramdef.is_iter ? "iterator" : "sequence"));
	      SES_PRINT (ses, buf);
...
caddr_t
xpath_funcall_or_apply (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, const char *funname, ptrlong predicate_type,
    int with_cache)
{
  int first;
  dk_set_t set = NULL;
...
*/
  ...
  if (NULL == xqr)
    {
      ...
      if (NULL == metas_ptr)
	sqlr_new_error ("22023", "XI039", "Unknown XPath function name %.200s is used as first argument of %.200s function", str,
	    funname);
      metas = metas_ptr[0];
      if (metas->xpfm_min_arg_no > param_count)
	sqlr_new_error ("22023", "XI040",
	    "The XPATH function %.200s() mentioned in %.200s() requires %d arguments but the call contains only %d", str, funname,
	    (int) (metas->xpfm_min_arg_no), param_count);
      if (metas->xpfm_main_arg_no < param_count)
	{
	  if (0 == metas->xpfm_tail_arg_no)
	    sqlr_new_error ("22023", "XI041",
		"The XPATH function %.200s() mentioned in %.200s() can handle only %d arguments but the call provides %d", str,
		funname, (int) (metas->xpfm_main_arg_no), param_count);
	  else
	    {
	      int tail_mod = (param_count - metas->xpfm_main_arg_no) % metas->xpfm_tail_arg_no;
	      if (tail_mod)
		sqlr_new_error ("22023", "XI042",
		    "The XPATH function %.200s() mentioned in %.200s() can handle %d, %d, %d etc. arguments but the call provides %d",
		    str, funname, (int) (metas->xpfm_main_arg_no), (int) (metas->xpfm_main_arg_no + metas->xpfm_tail_arg_no),
		    (int) (metas->xpfm_main_arg_no + 2 * metas->xpfm_tail_arg_no), param_count);
	    }
	}
  ...
  QR_RESET_CTX
  {
	...
	if (xqi_set_odometer >= xqi_set_debug_start)
	  dk_check_fake_var (param_val);
#endif
	if ((DV_DB_NULL == DV_TYPE_OF (param_val)) || (DV_ARRAY_OF_XQVAL == DV_TYPE_OF (param_val)
		&& 0 == BOX_ELEMENTS (param_val)))
	  {
	...
	else
	  {
	    ...
	    switch (DV_TYPE_OF (param_val))
	      {
	      case DV_LONG_INT:
		converted_copy = box_num_nonull (unbox (param_val));
		break;
	      case DV_ARRAY_OF_POINTER:
		{
		  int ctr;
		  ...
		  {
		    if (NULL == v)
		      converted_copy = box_num_nonull (0);
		  }
		  END_DO_BOX_FAST;
		  break;
		}
...
static void
xpathp_lex_props (int val, const char *yname, char fmttype, const char *fmt, ...)
{
  va_list tail;
  const char *cmd;
  dk_set_t tests = NULL;
      ...
      for (cmd_idx = 0; cmd_idx < BOX_ELEMENTS (ld->ld_tests); cmd_idx++)
	{
	  ...
	  switch (cmd)
	      ...
	      if (1 == lex_count)
		{
	      ...
	      if ('E' == cmd)
		{
		  dk_set_push (&report, box_sprintf (0x1000, "FAILED: last lexem is %d, must be error", last_lval));
		  break;
		}
	      if ('K' == cmd)
		{
	      ...
	      if ('M' == cmd)
		{
		  ...
		  for (lctr = 0; lctr < lex_count; lctr++)
			...
			{
			  dk_set_push (&report,
			      box_sprintf (0x1000, "FAILED: lexem %d is found but it should not occur", tested_lex_val));
			  goto end_of_test_m;
			}
		    }
		  dk_set_push (&report,
		      box_sprintf (0x1000, "PASSED: lexem %d is not found and it should not occur", tested_lex_val));
		end_of_test_m:
		  break;
		}
	      GPF_T;
...
void
xe_insert_external_dtd (xml_entity_t * xe)
{
...
xml_entity_t *
xte_reference (query_instance_t * qi, caddr_t base, caddr_t ref, xml_doc_t * from_doc, caddr_t * err_ret)
{
  xml_tree_ent_t *xte;
  dtd_t **xte_dtd_ptr;
  caddr_t str;
...
/*  dbg_printf (("Resolving %s (base %s)\n", ref, base); */
  ...
  {
    if (NULL != ref->xe_doc.xd->xd_uri && 0 == strcmp (ref->xe_doc.xtd->xd_uri, path_utf8))
      {
  ...
  if (NULL != err)
    goto error_cleanup;
  tree =
      xml_make_mod_tree (qi, str, &err, GE_XML, path_utf8, NULL /* no enc */ , server_default_lh, NULL /* no config */ ,
      NULL /* no DTD */ , NULL /* no ID cache */ , NULL /* no namespace 2dict */ );
  if (NULL != err)
    goto error_cleanup;
...
const char *
xe_get_sysid (xml_entity_t * xe, const char *ref_name)
{
...
const char *
xe_get_sysid_base_uri (xml_entity_t * xe)
{
  while (NULL != xe->xe_referer)
    xe = xe->xe_referer;
...
int
xte_is_same_as (const xml_entity_t * this_xe, const xml_entity_t * that_xe)
{
...
/* Top of the (sub)document */
  if (!XTE_HAS_PARENT (xte) || (!XTE_HAS_2PARENTS (xte) && (NULL != xte->xe_referer)))
    {
...
int DBG_NAME (xte_attribute) (DBG_PARAMS xml_entity_t * xe, int nth, XT * node, caddr_t * res, caddr_t * name_ret)
{
  int inx, len;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  caddr_t *current = xte->xte_current;
...
caddr_t
xte_attrvalue (xml_entity_t * xe, caddr_t qname)
{
  int inx, len;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  caddr_t *current = xte->xte_current;
...
caddr_t
xte_currattrvalue (xml_entity_t * xe)
{
  int inx, len;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  caddr_t *current = xte->xte_current;
...
xml_entity_t *DBG_NAME (xte_copy) (DBG_PARAMS xml_entity_t * xe)
{
...
xp_box_number (caddr_t n)
{
  ...
  if (dtp == DV_SINGLE_FLOAT || dtp == DV_DOUBLE_FLOAT || dtp == DV_NUMERIC)
    return (box_copy (n));
  if (DV_STRINGP (n))
    {
      if (strlen (n) < 10 && !strchr (n, '.') && !strchr (n, 'e'))
  ...
  if (DV_ARRAY_OF_POINTER == dtp && DV_ARRAY_OF_POINTER == DV_TYPE_OF (XTE_HEAD (n)))
    {
      caddr_t res = NULL;
...
void DBG_NAME (xte_string_value) (DBG_PARAMS xml_tree_ent_t * xte, caddr_t * ret, dtp_t dtp)
{
  ...
  if (XTE_IS_XSLT_OUTPUT (xte))
      ...
      if (OUT_METHOD_XML == xsst.xsst_out_method)
	{
	  ...
	  ((OUT_METHOD_XML == xsst.xsst_out_method) ||
	      (OUT_METHOD_HTML == xsst.xsst_out_method) || (OUT_METHOD_XHTML == xsst.xsst_out_method)))
	{
	  caddr_t **first_child = NULL;
	  caddr_t **current = (caddr_t **) xte->xte_current;
	  int inx;
	  int curr_len;
	  if ((DV_ARRAY_OF_POINTER == DV_TYPE_OF (current)) && (' ' != current[0][0][0]))
	    {
	      first_child = current;
	      goto doctype_may_be_printed;
	    }
	  curr_len = ((DV_ARRAY_OF_POINTER == DV_TYPE_OF (current)) ? (int) BOX_ELEMENTS (current) : 0);
...
void
ses_print_uint32_packed__aux (uint32 len, dk_session_t * ses)
{
  unsigned char buf[5];
#ifdef DEBUG
  if (!(len & ~0x7F))
...
uint32
ses_read_uint32_packed (dk_session_t * ses)
{
...
xte_word_count_1 (caddr_t * tree, xml_tree_doc_t * xtd, wpos_t * poss, lang_handler_t * lh, char hider)
{
  ...
  if (is_string_type (dtp))
    {
      word_count = lh_count_words (&eh__UTF8, lh, (const char *) tree, box_length ((caddr_t) tree), lh->lh_is_vtb_word);
      if (0 != word_count)
	hider = XML_MKUP_TEXT;
  ...
  if (DV_ARRAY_OF_POINTER == dtp)
      ...
      for (attr_idx = 1; attr_idx < attr_idx_max; attr_idx += 2)
	{
	  ...
	  word_count = lh_count_words (&eh__UTF8, lh,
	      (const char *) attr_value, box_length ((caddr_t) attr_value), lh->lh_is_vtb_word);
...
int
xte_get_logical_path (xml_entity_t * xe, dk_set_t * path)
{
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  int res = 1;
  xte_bmk_t *xbmk;
...
void
xte_rescan_id_dict_subtree (caddr_t ** tree, id_hash_t * id_dict, ptrlong * scan, dtd_t * dtd)
{
  int len, ctr;
...
xte_rescan_id_dict (xml_tree_ent_t * xe)
{
  ...
  xe->xe_doc.xd->xd_id_scan = (caddr_t) (scan =
      (ptrlong *) dk_alloc_box (sizeof (ptrlong) * (ECM_MAX_DEPTH + 1), DV_ARRAY_OF_LONG));
  scan[0] = 0;
  if (NULL == id_dict)
...
xml_entity_t *
xte_deref_id (xml_entity_t * xe, const char *idbegin, size_t idlength)
{
  id_hash_t *id_dict;
...
xml_entity_t *
xte_follow_path (xml_entity_t * xe, ptrlong * path, size_t path_depth)
{
  int len, res;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  caddr_t *current = xte->xte_current;
...
dtd_t *
xte_get_addon_dtd (xml_entity_t * xe)
{
  return NULL;
}
...
const char *
xte_get_sysid (xml_entity_t * xe, const char *ent_name)
{
  dtd_t *xe_dtd;
  xe_insert_external_dtd (xe);
...
xml_entity_t *DBG_NAME (xte_cut) (DBG_PARAMS xml_entity_t * xe, query_instance_t * qi)
{
  size_t stack_elems = 0x10;
  size_t stack_sz = stack_elems * sizeof (xte_bmk_t);
  xte_bmk_t *newstack = (xte_bmk_t *) dk_alloc (stack_sz);
  ...
  add_new_root = ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (tree_copy)) || (uname__root != ((caddr_t *) (tree_copy[0]))[0]));
  if (add_new_root)
...
xml_entity_t *DBG_NAME (xte_clone) (DBG_PARAMS xml_entity_t * xe, query_instance_t * qi)
{
...
caddr_t *
xte_copy_to_xte_head (xml_entity_t * xe)
{
  caddr_t *res;
  int ctr;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
...
caddr_t *
xte_copy_to_xte_subtree (xml_entity_t * xe)
{
  caddr_t *res;
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
...
caddr_t **
xte_copy_to_xte_forest (xml_entity_t * xe)
{
#if 1
  GPF_T;
  return NULL;
...
void
xte_emulate_input (xml_entity_t * xe, struct vxml_parser_s *parser)
{
  caddr_t *current;
  caddr_t *head;
  size_t head_len;
  ...
  if (' ' == name[0])
      ...
      if (uname__pi == name)
	{
	  if (NULL == parser->masters.pi_handler)
	    return;
	  parser->masters.pi_handler (parser->masters.user_data,
	      ((head_len > 2) ? head[2] : NULL), ((BOX_ELEMENTS (current) > 1) ? current[1] : NULL));
	  return;
	}
      if (uname__comment == name)
	{
	  if (NULL == parser->masters.comment_handler)
	    return;
	  parser->masters.comment_handler (parser->masters.user_data, ((BOX_ELEMENTS (current) > 1) ? current[1] : ""));
	  return;
	}
      if (uname__ref == name)
	{
	  ...
	  if (NULL != rel_uri)
	      ...
	      if (NULL == refd)
		{
		  ...
		  if (NULL != parser->masters.entity_ref_handler)
		    parser->masters.entity_ref_handler (parser->masters.user_data, head[2], strlen (head[2]), 0,	/* = not parameter entity ref */
  ...
  if (parser->masters.start_element_handler)
      ...
      while (ctr--)
	{
	  ...
	  if (parser->attrdata.local_attrs_count > XML_PARSER_MAX_ATTRS)
	    sqlr_new_error ("37000", "XI030", "Unable to validate an XML tree entity: too many attributes in element '%.300s'",
		name);
	}
...
caddr_t
xte_find_expanded_name_by_qname (xml_entity_t * xe, const char *qname, int use_default)
{
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  xte_bmk_t *iter;
  ...
  for (iter = xte->xte_stack_top; iter >= xte->xte_stack_buf; iter--)
      ...
      if (ns_array)
	{
	  ...
	  for (inx = 0; inx < BOX_ELEMENTS (ns_array); inx += 2)
		...
		{
		  if ((box_length (ns_array[inx]) == prefix_sz + 1) && !memcmp (qname, ns_array[inx], prefix_sz))
		    {
		      caddr_t uri = ns_array[inx + 1];
		      size_t uri_sz = box_length (uri);
...
dk_set_t
xte_namespace_scope (xml_entity_t * xe, int use_default)
{
  xml_tree_ent_t *xte = (xml_tree_ent_t *) xe;
  dk_set_t res = NULL;
  xte_bmk_t *iter;
...
bif_xml_tree_doc (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!IS_STRING_DTP (dtp) && DV_BLOB_HANDLE != dtp && DV_ARRAY_OF_POINTER != dtp)
    sqlr_new_error ("37000", "XI020", "Argument of xml_tree_doc must be an array not arg of type %.300s (%d)", dv_type_title (dtp),
	dtp);
  ...
  if (dtp == DV_ARRAY_OF_POINTER && (BOX_ELEMENTS (tree) < 1 ||
	  DV_ARRAY_OF_POINTER != DV_TYPE_OF (((caddr_t *) tree)[0]) || BOX_ELEMENTS (((caddr_t *) tree)[0]) < 1))
...
#define XD_SET_BOOL_VALUE(Elem, Name) \
  ...
  if (!strcmp (option_name, "cdata-section-elements"))
    sqlr_new_error ("XS370", "XS066",
	"Output option '%.200s' can be set only by xsl:output element of a stylesheet, xml_doc_output_option() can not set it",
	option_name);
  sqlr_new_error ("XS370", "XS067", "Unknown output option name '%.200s' is specified as argument #2 of xml_doc_output_option()",
      option_name);
...
bif_xml_tree_doc_set_output (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (strcmp (output, "html") && strcmp (output, "xml") && strcmp (output, "text") && strcmp (output, "xhtml"))
    sqlr_new_error ("22023", "XS044", "Function xml_tree_doc_set_output accepts html, xhtml, xml or text as second argument");
...
bif_xml_tree_doc_encoding (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (BOX_ELEMENTS (args) > 1)
    {				/* set case */
      ...
      if (stricmp (enc, "UTF-8") && !sch_name_to_charset (enc))
  ...
  if (xd->xout_encoding)
    {
      if (!stricmp (xd->xout_encoding, "UTF-8") || sch_name_to_charset (xd->xout_encoding))
	ret = box_dv_short_string (xd->xout_encoding);
    }
  return ret;
}
...
xn_xe_from_text (xpath_node_t * xn, query_instance_t * qi)
{
  ...
  if ((NULL == res || NULL == res->xe_doc.xd->xd_uri) && (xn->xn_base_uri))
	...
	{
	  if (strlen (path) > 2000)
	    sqlr_new_error ("HT001", "XI023", "URI path for document from %.300s.%.300s is not a string or is over 2K long",
		tb->tb_name, text_col->col_name);
	  snprintf_ck (abs_uri, sizeof (abs_uri), "virt://%s.%s.%s:%s", tb->tb_name, text_col->col_xml_base_uri, text_col->col_name,
	      path);
	}
    }
  if (NULL != res)
...
*/
      ...
      if (xqr->xqr_xml_parser_cfg)
	{
	  ...
	  tree =
	      (caddr_t *) xml_make_mod_tree (qi, str, &err, GE_XML, boxed_abs_uri, charset, server_default_lh,
	      xqr->xqr_xml_parser_cfg, &dtd, NULL, NULL);
    ...
    case XE_XPER_SERIALIZATION:
      {
	caddr_t boxed_abs_uri = (('\0' != abs_uri[0]) ? box_dv_short_string (abs_uri) : NULL);
	xe = (xml_entity_t *) xper_entity (qi, val, NULL, 0, boxed_abs_uri, NULL /* no encoding */ , server_default_lh,
	    NULL /* DTD config */ , 1);
      }
      break;
...
xn_init (xpath_node_t * xn, query_instance_t * qi)
{
  ...
  if (xn->xn_exp_for_xqr_text)
      ...
      if (!prev_str || !box_equal (prev_str, str))
	{
	  caddr_t _str = DV_WIDESTRINGP (str) ?
	      box_wide_as_utf8_char (str, box_length (str) / sizeof (wchar_t) - 1, DV_SHORT_STRING) : NULL;
  ...
  try_next_val:
	...
	XN_QST_SET (xn, qst, xn->xn_output_val, DV_STRINGP (val) ?
	    box_utf8_as_wide_char (val, NULL, box_length (val), 0) : box_copy_tree (val));
	save_xqi = 1;
      }
    else
      {
...
xn_input (xpath_node_t * xn, caddr_t * inst, caddr_t * state)
{
  ...
  for (;;)
      ...
      else
	{
      ...
      if (err == SQL_SUCCESS)
	{
	  if (!xn->src_gen.src_after_test || code_vec_run (xn->src_gen.src_after_test, inst))
	    {
...
xn_text_query (xpath_node_t * xn, query_instance_t * qi, caddr_t xp_str)
{
  ...
  if (prev_text && 0 == strcmp (prev_text, xp_str))
  ...
  else
    {
	  ...
	  box_wide_as_utf8_char (xp_str, box_length (xp_str) / sizeof (wchar_t) - 1, DV_SHORT_STRING) : NULL;
  ...
  switch (bh_sort)
	...
	QR_RESET_CTX
	{
	  ...
	  tree1 = (caddr_t *) xml_make_mod_tree ((query_instance_t *) qst, bh, (caddr_t *) & err,
	      GE_XML, uri, CHARSET_NAME (charset, NULL), server_default_lh, dtd_config, &dtd, &id_cache, &ns_2dict);
	  if (NULL == tree1)
	    {
	      dk_free_box (uri);
	      sqlr_resignal (err);
...
int
xu_are_items_neighbour (ptrlong * pos1, ptrlong * pos2)
{
...
int
xu_remove_redundant (xmlupdate_item_t * xu_list, int old_size, ptrlong doc, dk_set_t * garbage)
{
  int res_size = 0;
  int xu_ctr;
  ...
  for (xu_ctr = 0; xu_ctr < old_size; xu_ctr++)
      ...
      if (NULL == curr->xu_src_attr_name)
	{
	  ...
	  cmp =
	      xe_compare_logical_paths (prev_nonattr->xu_src_pos, BOX_ELEMENTS (prev_nonattr->xu_src_pos), path,
	      BOX_ELEMENTS (path));
	  switch (cmp)
	    {
...
void
xu_list_subtrees_of_repl (caddr_t repl, caddr_t * repl_current, caddr_t ** subtrees_ret, ptrlong * subtrees_ctr_ret)
{
  switch (DV_TYPE_OF (repl))
    {
    case DV_XML_ENTITY:
      repl_current[0] = (caddr_t) (((xml_tree_ent_t *) (repl))->xte_current);
      if ((DV_ARRAY_OF_POINTER == DV_TYPE_OF (repl_current[0])) && (uname__root == XTE_HEAD_NAME (XTE_HEAD (repl_current[0]))))
	{
...
caddr_t
xte_replace (xml_tree_ent_t * xte, caddr_t repl, int flags, xml_tree_doc_t * gc)
{
  if (xte->xe_attr_name)
    {
...
/* Normalizations: */
    normalize_again:
      might_concat_before = (
	  (XE_REPLACE_CONCAT_BEFORE & flags) && (1 < old_begin_len) && (DV_STRING == DV_TYPE_OF (old_children[old_begin_len - 1])));
      might_concat_after = (
	  (XE_REPLACE_CONCAT_AFTER & flags) &&
	  (old_tail_ofs < old_children_no) && (DV_STRING == DV_TYPE_OF (old_children[old_tail_ofs])));
...
/* Composing new array of children and return */
      ...
      else			/* if the length changes, make a patched copy */
	{
	  ...
	  memcpy (new_children + old_begin_len + subtrees_ctr, old_children + old_tail_ofs,
	      sizeof (caddr_t) * (old_children_no - old_tail_ofs));
	  if (XTE_HAS_2PARENTS (xte))
	    {
...
caddr_t
xu_replace (xmlupdate_item_t * xu_list, int xu_count, xml_tree_ent_t * tgt, xml_tree_doc_t * gc)
{
  int xu_ctr;
...
void
xu_free_items (xmlupdate_item_t * xu_list, int xu_count)
{
  int xu_ctr;
  for (xu_ctr = xu_count; xu_ctr--; /* no step */ )
    {
...
caddr_t
xmlupdate_impl (caddr_t * qst, xml_tree_ent_t * src, xml_tree_ent_t * tgt, xmlupdate_item_t * xu_list)
{
  int xu_ctr, xu_ctr2;
  int xu_count;
  caddr_t res;
...
bif_updateXML_ent (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (arg_pair_ctr = arg_pair_count; arg_pair_ctr--; /* no step */ )
	...
	if (NULL != pos_ent->xe_referer)
	  sqlr_new_error ("22023", "SR358",
	      "Function updateXML_ent can not search for non-standalone XML entity passed as argument %d", arg_pair_ctr * 2 + 1);
      }
  for (arg_pair_ctr = arg_pair_count; arg_pair_ctr--; /* no step */ )
    {
      ...
      if (0 == pos_ent->_->xe_get_logical_path ((xml_entity_t *) (pos_ent), &path))
	{
	  sqlr_new_error ("22023", "SR474",
	      "Function updateXML_ent can not search for outdated XML entity passed as argument %d", arg_pair_ctr * 2 + 1);
	}
  ...
  tgt = (xml_tree_ent_t *) xmlupdate_impl (qst, src, tgt, (xmlupdate_item_t *) (list_to_array (item_parts)));
	...
	QR_RESET_CTX
	{
	  ...
	  while (hit)
	      ...
	      if (DV_XML_ENTITY == DV_TYPE_OF (pos_ent))
		{
		  ...
		  if (NULL != pos_ent->xe_referer)
		    sqlr_new_error ("22023", "SR358",
			"Function updateXML can not search for non-standalone XML entity found by %s (argument %d).", pos_expn_arg,
			arg_pair_ctr * 2 + 1);
		  if (0 == pos_ent->_->xe_get_logical_path ((xml_entity_t *) (pos_ent), &path))
		    sqlr_new_error ("22023", "SR473",
			"Function updateXML can not search for outdated XML entity found by %s (argument %d).", pos_expn_arg,
			arg_pair_ctr * 2 + 1);
		  dk_set_push (&item_parts, (caddr_t) (XU_OP_REPLACE));
		  dk_set_push (&item_parts, (caddr_t) (1));	/* is constant */
		  dk_set_push (&item_parts, (caddr_t) (1));	/* is owned by caller */
...
caddr_t
bif_XMLReplace (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (arg_pair_ctr = arg_pair_count; arg_pair_ctr--; /* no step */ )
	...
	if (NULL != pos_ent->xe_referer)
	  sqlr_new_error ("22023", "SR380",
	      "Function XMLReplace can not search for non-standalone XML entity passed as argument %d", arg_pair_ctr * 2 + 1);
      }
  for (arg_pair_ctr = arg_pair_count; arg_pair_ctr--; /* no step */ )
    {
  ...
  tgt = (xml_tree_ent_t *) xmlupdate_impl (qst, src, src, (xmlupdate_item_t *) (list_to_array (item_parts)));
...
caddr_t
xu_insert_impl (caddr_t * qst, state_slot_t ** args, const char *funname, int bif_code)
{
  ...
  switch (bif_code)
	...
	const char *parentname = (
	    (DV_ARRAY_OF_POINTER != DV_TYPE_OF (src->xte_current)) ? " text" : XTE_HEAD_NAME (XTE_HEAD (src->xte_current)));
	if ((' ' == parentname[0]) && (uname__root != parentname))
	  {
	    err =
		srv_make_new_error ("22023", "SR385", "Function %s can modify only element entities, not one of type '%s'", funname,
		parentname + 1);
	    goto err_exit;	/* see below */
	  }
...
/* At this point the tree is about to be modified */
  {
	...
	memcpy (new_children + ins_before_pos + ins_len, old_children + ins_before_pos,
	    (old_children_len - ins_before_pos) * sizeof (caddr_t));
...
/* At this point the tree is stable again */
  ...
  while (NULL != local_items)
    dk_free_tree ((caddr_t) dk_set_pop (&local_items));
  xte_tree_check (src->xe_doc.xtd->xtd_tree);
  xte_tree_check (src->xte_current);
  return NEW_DB_NULL;
...
caddr_t
bif_XMLInsertBefore (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
caddr_t
bif_XMLInsertAfter (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
caddr_t
bif_XMLAppendChildren (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
caddr_t
bif_XMLAddAttribute (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (' ' == attr_xe_elemname[0])
    sqlr_new_error ("22023", "SR393", "Function XMLAddAttribute can modify only element entities, not entity of type '%s'",
	attr_xe_elemname + 1);
  ...
  switch (DV_TYPE_OF (raw_attr_name))
    ...
    case DV_OBJECT:
      ...
      if (NULL == attr_xe)
	sqlr_new_error ("22023", "SR390", "Function XMLAddAttribute can not accept object of type '%s' as argument 3",
	    UDT_I_CLASS (raw_attr_name)->scl_name);
	...
	sqlr_new_error ("22023", "SR395",
	    "The XML entity passed as argument 3 to function XMLAddAttribute() is not an attribute entity");
  ...
  if (old_attr_idx)
	...
	case XU_ADD_ATTR_INTO:
	  dk_free_box (attr_value);
	  sqlr_new_error ("22023", "SR397", "Attribute '%s' already exists in entity passed to function XMLAddAttribute()",
	      old_head[old_attr_idx]);
  ...
  while ((ins_before > 2) && ('\0' == old_head[ins_before - 2][0]))
    ins_before -= 2;
...
caddr_t
bif_xml_serialize_packed (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
xml_deserialize_packed (caddr_t * qst, caddr_t strg)
{
  ...
  if (DV_STRING == strg_dtp)
	...
	sqlr_new_error ("22023", "SR560", "__xml_deserialize_packed() requires a blob or string argument");
      bh = (blob_handle_t *) strg;
      if (bh->bh_ask_from_client)
	sqlr_new_error ("22023", "SR561", "Blob argument to __xml_deserialize_packed() must be a non-interactive blob");
...
caddr_t
bif_xml_get_logical_path (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
caddr_t
bif_xml_follow_logical_path (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  xml_entity_t *src = bif_entity_arg (qst, args, 0, "xml_get_logical_path");
  xml_entity_t *tgt;
...
caddr_t
bif_xtree_tridgell32 (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (tree))
    {
      ...
      data = (unsigned char *) (XTE_HEAD_NAME (head));
      head_len = BOX_ELEMENTS_INT (head);
      SUM64 (data, lo, med, hi) loxor = medxor = hixor = 0;
      for (attrctr = 2; attrctr < head_len; attrctr += 2)
	{
	  lon = medn = hin = 0;
	  data = (unsigned char *) (head[attrctr - 1]);
	  SUM64 (data, lon, medn, hin) data = (unsigned char *) (head[attrctr]);
...
caddr_t
bif_xtree_sum64 (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
...
again:
  switch (dtp)
	...
	{
	case DT_TYPE_DATE:
	  return uname_xmlschema_ns_uri_hash_date;
	case DT_TYPE_TIME:
	  return uname_xmlschema_ns_uri_hash_time;
	default:
	  return uname_xmlschema_ns_uri_hash_dateTime;
	}
    case DV_STRING:
    case DV_BLOB_HANDLE:
    case DV_WIDE:
...
xte_subtrees_are_equal (caddr_t * node1, caddr_t * node2)
{
  ...
  switch (dtp)
    {
    case DV_LONG_INT:
      return (unbox ((caddr_t) (node1)) == unbox ((caddr_t) (node2)));
    case DV_SHORT_STRING:
      len = box_length (node1);
      return ((len == box_length (node2)) && !memcmp (node1, node2, len));
    case DV_UNAME:
      return node1 == node2;
    case DV_ARRAY_OF_POINTER:
      {
...
/* No need in "if (XE_IS_LAZY (xe))", because there's no position in not-yet-loaded doc */
  return 17 * (ptrlong) (xe->_) +
      13 * (ptrlong) (xe->xe_doc.xd) + 11 * (ptrlong) (xe->xe_nth_attr) + 9 * (ptrlong) (xe->xe_referer) + chld_hash;
}
...
xml_global_ns_2dict_get (caddr_t * qst, const char *fname)
{
  if ((NULL != fname) && !sec_bif_caller_is_dba ((query_instance_t *) qst))
    sqlr_new_error ("42000", "SR585:SECURITY",
	"Function %.300s() is restricted to DBA group when it tries to access to the global namespace dictionary.", fname);
...
void
xml_doc_cache_free (xml_doc_cache_t * xdc)
{
  if (NULL != xdc)
    {
...
caddr_t
xml_doc_cache_get_copy (xml_doc_cache_t * xdc, ccaddr_t key)
{
  id_hash_t *hash;
  caddr_t *doc_ptr;
...
void
xml_doc_cache_add_copy (xml_doc_cache_t ** xdc_ptr, ccaddr_t key, caddr_t doc)
{
  id_hash_t *hash;
  ptrlong type = ((ptrlong *) key)[0];
...
void
xml_doc_cache_shrink (xml_doc_cache_t * xdc, size_t weight_limit)
{
/* TBD */
}
