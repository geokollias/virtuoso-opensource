...
function get_awk_macro_defines (awk_command)
{
  ...
  else if (np > 1 && p[2] == "LONG")
    {
      text = " "
      define_proc_macro = "DEFINE_LONG_PROC"
      long_proc = 1
    }
...
BEGIN   {
	  ...
	  print "#include \"sqlfn.h\""
	  print "#include \"sqltype.h\""
	  print "#include \"sqlbif.h\""
          if (extra_include != "")
            print "#include \"" extra_include "\"\n"
	  filename = FILENAME
	  nfiles = 1
	  end_name = ""
	  qualifier = "DB"
		...
		if (npieces > 2 && pieces[1] == "CREATE")
		  {
			...
			print "static const char *proc" nproc arrspec " = " arrstart "\n\"#line " line_begin_no+1 " \\\"[executable]/" end_name "\\\"\\n\"\n" fun
...
END 	{
	   ...
	   if (n_long > 0) 
	   {    
	   print "#define DEFINE_LONG_PROC(name, text1) \\"
	   print "  { \\"
	   print "    caddr_t text = NULL;\\"
	   print "    static const char *elm;\\"
	   print "    int inx, len = 0;\\"
