...
rdb2rdf_optree_t *
rdb2rdf_create_optree (rdb2rdf_ctx_t * rrc, rdb2rdf_optree_t * parent, rdb2rdf_optree_t * prev_sibling, quad_map_t * qm,
    ccaddr_t table_name)
{
  ...
  if (NULL == qm->qmAliasesKeyrefdByQuad)
	...
	"Some quad maps for table %.500s needs upgrade before generating RDB2RDF code, please call DB.DBA.RDF_UPGRADE_METADATA()",
  ...
  if (table_aliases_count)
	...
	sqlr_new_error ("22023", "SR637", "Overcomplicated quad map contains more than 8 aliases for table \"%.200s\"",
	    table_name ? table_name : "UNKNOWN");
      for (fld_ctr = SPART_TRIPLE_FIELDS_COUNT; fld_ctr--; /*no step */ )
	{
	  qm_value_t *fld_qmv = SPARP_FIELD_QMV_OF_QM (qm, fld_ctr);
...
rdb2rdf_list_tables (rdb2rdf_optree_t * rro, dk_set_t * tables_ret)
{
  ...
  DO_BOX_FAST (rdb2rdf_optree_t *, sub_qm_rro, sub_qm_rro_ctr, rro->rro_subtrees) rdb2rdf_list_tables (sub_qm_rro, tables_ret);
  END_DO_BOX_FAST;
}
...
rdb2rdf_set_rvr_by_const_or_qmv (rdb2rdf_ctx_t * rrc, rdf_val_range_t * rvr, ccaddr_t fld_const, qm_value_t * fld_qmv)
{
  if (NULL != fld_const)
	...
	qmv_or_fmt_rvr = &(fld_qmv->qmvRange);
      else if ((NULL != fld_qmv->qmvFormat)
	  && ((SPART_VARR_SPRINTFF | SPART_VARR_FIXED) & fld_qmv->qmvFormat->qmfValRange.rvrRestrictions))
...
rdb2rdf_ctx_dump (rdb2rdf_ctx_t * rrc, dk_session_t * ses)
{
  ...
  for (main_qm_ctr = 0; main_qm_ctr < rrc->rrc_all_qm_count; main_qm_ctr++)
      ...
      for (other_qm_ctr = rrc->rrc_all_qm_count; other_qm_ctr--; /* no step */ )
	{
      ...
      SES_PRINT (ses, buf);
      SES_PRINT (ses, rrc->rrc_conflicts_of_qms[main_qm_ctr]);
      SES_PRINT (ses, "\n");
    }
  ses_print_ruler_10 (ses, rrc->rrc_all_qm_count, "      ", "", "      ", "");
  SES_PRINT (ses, "\n");
...
rdb2rdf_optree_dump (rdb2rdf_ctx_t * rrc, rdb2rdf_optree_t * rro, dk_session_t * ses)
{
  ...
  sprintf (buf, "-----\n%s optree %d from qm %d %s, subtree of %d, breakup starts at %d\n",
      ...
      ((NULL == qm_rtti) ? "UNKNOWN QM" : ((NULL == qm_rtti->jrtti_inst_iri) ? "NULL IRI" : qm_rtti->jrtti_inst_iri)),
  ...
  if (NULL != rro->rro_qm)
      ...
      for (ctr = 0; ctr < SPART_TRIPLE_FIELDS_COUNT; ctr++)
	{
	  ...
	  if (fld_const_rvr->rvrRestrictions & SPART_VARR_FIXED)
		...
		{
		case DV_UNAME:
		  sprintf (buf, " <%s>", fld_const_rvr->rvrFixedValue);
		  SES_PRINT (ses, buf);
		  break;
		case DV_STRING:
		  sprintf (buf, " '''%s'''", fld_const_rvr->rvrFixedValue);
		  ...
		  break;
		case DV_LONG_INT:
		  sprintf (buf, " " BOXINT_FMT, (boxint) (fld_const_rvr->rvrFixedValue));
		  SES_PRINT (ses, buf);
		  break;
		default:
		  SES_PRINT (ses, " const");
  ...
  if (rro->rro_is_leaf)
      ...
      for (other_qm_ctr = rrc->rrc_all_qm_count; other_qm_ctr--; /* no step */ )
	{
      ...
      ses_print_ruler_10 (ses, rrc->rrc_all_qm_count, "      ", "", "      ", "");
      SES_PRINT (ses, "QMQM: ");
      SES_PRINT (ses, rrc->rrc_conflicts_of_qms[rro->rro_qm_index]);
      SES_PRINT (ses, "\n");
    }
  /*  quad_map_t *rro_qm; */
  for (blockers_tail = rro->rro_blockers_before_next; (blockers_tail != NULL) && (blockers_tail != rro->rro_blockers_before);
      blockers_tail = blockers_tail->rrb_next)
    {
      int ctr;
      ...
      for (ctr = 0; ctr < SPART_TRIPLE_FIELDS_COUNT; ctr++)
	{
	  ...
	  switch (DV_TYPE_OF (cv))
	    ...
	    case DV_UNAME:
	      ...
	      break;
	    case DV_LONG_INT:
	      sprintf (buf, "%ld", (long) (unbox (cv)));
	      break;
	    default:
	      sprintf (buf, "const with tag %d", DV_TYPE_OF (cv));
	      break;
	    }
	  SES_PRINT (ses, buf);
	}
  ...
  for (ctr = 0; ctr < 32; ctr++)
    {
	  ...
	  ((rro->rro_bits_of_fields_here_and_above & (1 << ctr)) ?
	      "gspogspogspogspogspogspogspogspo"[ctr] : "{--}{--}{--}{--}{--}{--}{--}{--}"[ctr]));
    }
  buf2[4 * (dk_set_length (rro->rro_aliases_of_main_table) + 1)] = '\0';
  sprintf (buf, "Fields here: %s\n", buf2);
  SES_PRINT (ses, buf);
  /*  uint32 rro_bits_of_fields_here_and_above; */
  DO_BOX_FAST (rdb2rdf_optree_t *, sub_qm_rro, sub_qm_rro_ctr, rro->rro_subtrees) rdb2rdf_optree_dump (rrc, sub_qm_rro, ses);
  END_DO_BOX_FAST;
}

void
rdb2rdf_print_fld_expn (rdb2rdf_optree_t * rro, int cast_to_obj, int alias_no, quad_map_t * qm, qm_value_t * fld_qmv,
    const char *prefix, spar_sqlgen_t * ssg)
{
...
rdb2rdf_qm_after_del_codegen (rdb2rdf_ctx_t * rrc, rdb2rdf_optree_t * rro, ccaddr_t table_name, int opcode, spar_sqlgen_t * ssg)
{
  ...
  ssg_newline (0);
  sprintf (buf,
      "for (select * from DB.DBA.RDF_QUAD_DELETE_QUEUE q where q.EVENT_ID=query_instance_id(1) and RULE_ID=" BOXINT_FMT
      " and not exists (", rro->rro_rule_id);
  ssg_puts (buf);
  ssg->ssg_indent += 2;
  qm_conflicts = rrc->rrc_conflicts_of_qms[rro->rro_qm_index];
  ...
  for (other_qm_ctr = rrc->rrc_all_qm_count; other_qm_ctr--; /* no step */ )
    {
      ...
      if ((RDB2RDF_QMQM_INTERSECT != qm_conflicts[other_qm_ctr]) && !(rro->rro_self_multi && other_qm_ctr == rro->rro_qm_index))
	continue;
      other_qm_rtti = gethash (other_qm, jso_rttis_of_structs);
      if ((NULL == other_qm_rtti) || (other_qm != other_qm_rtti->jrtti_self))
	sqlr_new_error ("22023", "SR638",
	    "Some quad maps of quad store virtrdf:SyncToQuads are not loaded or corrupted or being edited during SQL code generation");
      ssg_newline (0);
      if (need_union++)
	ssg_puts ("UNION ");
      ssg_puts (" { QUAD MAP <");
  ...
  ssg_puts ("      delete from DB.DBA.RDF_QUAD where G=QG and S=QS and P=QP and O=QO option (QUIETCAST);\n"),
      ssg_puts
      ("    else if (exists (select top 1 1 from DB.DBA.RDF_QUAD where G=QG and S=QS and P=QP and O=QO option (QUIETCAST)))\n");
  ssg_puts ("      {\n");
...
void
rdb2rdf_qm_codegen (rdb2rdf_ctx_t * rrc, rdb2rdf_optree_t * rro, caddr_t table_name, int opcode, int subopcode, const char *prefix,
    ccaddr_t alias, int alias_no, spar_sqlgen_t * ssg)
{
  ...
  bits_of_fields_above =
      (NULL == rro->rro_parent) ? 0 : ((rro->rro_parent->rro_bits_of_fields_here_and_above >> (alias_no * 4)) & 0xF);
...
/* Now a similar loop deals with consts. Blockers are not checked here because they've inspected when optree is composed. */
  ...
  if ((1 == dk_set_length (rro->rro_aliases_of_main_table)) && (1 == rro->rro_qm->qmAllATableUseCount) && (0xF == bits_of_fields_here))	/* i.e. all fields are set, no inner loop */
    {
      int cond_ctr = 0;
      ssg_newline (0);
      ...
      if (two_phase_delete)
	{
	  ...
	  if (RDB2RDF_CODEGEN_SUB_BEFORE_DEL != subopcode)
	    GPF_T;
	  sprintf (buf,
	      "insert soft DB.DBA.RDF_QUAD_DELETE_QUEUE (EVENT_ID,RULE_ID,QG,QS,QP,QO) values (query_instance_id(1), " BOXINT_FMT
	      ", g_val, s_val, p_val, o_val);", rro->rro_rule_id);
	  ssg_puts (buf);
	}
      else if (RDB2RDF_CODEGEN_SUB_BEFORE_DEL == subopcode)
	{
	  ...
	  ssg_puts ("      delete from DB.DBA.RDF_QUAD where G=g_val and S=s_val and P=p_val and O=o_val option (QUIETCAST);\n"),
	      ssg_puts
	      ("    else if (exists (select top 1 1 from DB.DBA.RDF_QUAD where G=g_val and S=s_val and P=p_val and O=o_val option (QUIETCAST)))\n");
	  ssg_puts ("      {\n");
	  ssg_puts ("        declare triples any;\n");
...
#define NTH_FLD(n) ((bits_of_fields_here & (1 << (n))) ? x_vals[n] : x_locals[n])
  if (two_phase_delete)
	...
	GPF_T;
      sprintf (buf,
	  "insert soft DB.DBA.RDF_QUAD_DELETE_QUEUE (EVENT_ID,RULE_ID,QG,QS,QP,QO) values (query_instance_id(1), " BOXINT_FMT
	  ", %s, %s, %s, %s);", rro->rro_rule_id, NTH_FLD (0), NTH_FLD (1), NTH_FLD (2), NTH_FLD (3));
      ssg_puts (buf);
    }
  else if (RDB2RDF_CODEGEN_SUB_BEFORE_DEL == subopcode)
    {
      ...
      for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
	{
      ...
      ssg_puts (buf);
      sprintf (buf,
	  "    else if (exists (select top 1 1 from DB.DBA.RDF_QUAD where G=%s and S=%s and P=%s and O=%s option (QUIETCAST)))\n",
	  NTH_FLD (0), NTH_FLD (1), NTH_FLD (2), NTH_FLD (3));
      ssg_puts (buf);
      ssg_puts ("      {\n");
      ssg_puts ("        declare triples any;\n");
      sprintf (buf, "        triples := vector (vector (%s, %s, %s));\n", NTH_FLD (1), NTH_FLD (2), NTH_FLD (3));
      ssg_puts (buf);
...
void
rdb2rdf_optree_codegen (rdb2rdf_ctx_t * rrc, rdb2rdf_optree_t * rro, caddr_t table_name, int opcode, int subopcode,
    const char *prefix, spar_sqlgen_t * ssg)
{
  ...
  switch (subopcode)
	...
	rdb2rdf_qm_codegen (rrc, rro, table_name, opcode, subopcode, prefix, (ccaddr_t) (rro->rro_aliases_of_main_table->data), 0,
	    ssg);
      break;
...
rdb2rdf_print_body_begin (spar_sqlgen_t * ssg, int inserts_are_possible)
{
  ...
  ssg_puts ("      resignal;\n };\n");
  ssg_puts
      ("  declare exit handler for sqlstate '*' { dbg_obj_princ ('RDB2RDF trigger fail: ', __SQL_STATE, ': ', __SQL_MESSAGE); resignal; };\n");
  if (inserts_are_possible)
    {
...
rdb2rdf_codegen (rdb2rdf_ctx_t * rrc, caddr_t table_name, int opcode, dk_session_t * ses)
{
  ...
  switch (opcode)
    ...
    case RDB2RDF_CODEGEN_INITIAL_LOOPS:
      ssg_puts ("create procedure DB.DBA.\"RDB2RDF_FILL_BY_LOOPS__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" ()\n");
      rdb2rdf_print_body_begin (ssg, 1);
      ssg_puts ("  for (select * from ");
      ssg_puts (table_name);
      ssg_puts (") do\n");
      ssg_puts ("    {\n");
      ssg->ssg_indent = 3;
    ...
    case RDB2RDF_CODEGEN_AFTER_INSERT:
      ssg_puts ("create trigger \"RDB2RDF_AI__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" after insert on ");
      ssg_puts (table_name_in_quotes);
      ssg_puts (" referencing new as \"RDF2RDB_N\"\n");
      rdb2rdf_print_body_begin (ssg, 1);
    ...
    case RDB2RDF_CODEGEN_BEFORE_UPDATE:
      ssg_puts ("create trigger \"RDB2RDF_BU__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" before update on ");
      ssg_puts (table_name_in_quotes);
      ssg_puts (" referencing new as \"RDF2RDB_N\", old as \"RDF2RDB_O\"\n");
    ...
    case RDB2RDF_CODEGEN_AFTER_UPDATE:
      ssg_puts ("create trigger \"RDB2RDF_AU__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" after update on ");
      ssg_puts (table_name_in_quotes);
      ssg_puts (" referencing new as \"RDF2RDB_N\", old as \"RDF2RDB_O\"\n");
      rdb2rdf_print_body_begin (ssg, 1);
    ...
    case RDB2RDF_CODEGEN_BEFORE_DELETE:
      ssg_puts ("create trigger \"RDB2RDF_BD__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" before delete on ");
      ssg_puts (table_name_in_quotes);
      ssg_puts (" referencing old as \"RDF2RDB_O\"\n");
      rdb2rdf_print_body_begin (ssg, 0);
      rdb2rdf_optree_codegen (rrc, &(rrc->rrc_root_rro), table_name, opcode, RDB2RDF_CODEGEN_SUB_BEFORE_DEL, "RDF2RDB_O", ssg);
      rdb2rdf_print_body_end (ssg, 0);
    ...
    case RDB2RDF_CODEGEN_AFTER_DELETE:
      ssg_puts ("create trigger \"RDB2RDF_AD__");
      ssg_puts (table_name_for_proc);
      ssg_puts ("\" after delete on ");
      ssg_puts (table_name_in_quotes);
      ssg_puts (" referencing old as \"RDF2RDB_O\"\n");
      rdb2rdf_print_body_begin (ssg, 0);
      rdb2rdf_optree_codegen (rrc, &(rrc->rrc_root_rro), table_name, opcode, RDB2RDF_CODEGEN_SUB_AFTER_DEL, "RDF2RDB_O", ssg);
...
caddr_t
bif_sparql_rdb2rdf_impl (caddr_t * qst, caddr_t table_name, int opcode, caddr_t * graph_xlat, int rule_id_seed,
    int only_list_tables)
{
  ...
  QR_RESET_CTX
  {
	...
	if (rrc.rrc_graph_xlat_count % 2)
	  sqlr_new_error ("22023", "SR639", "Vector of graph IRIs to translate should be of even length, not of length %d",
	      rrc.rrc_graph_xlat_count);
	if (rrc.rrc_graph_xlat_count)
	  {
	    int ctr, ctrL;
		...
		for (ctrL = ctr - 2; 0 <= ctrL; ctrL -= 2)
		  {
		    ...
		    if (!cmp)
		      sqlr_new_error ("22023", "SR639", "Graph IRI '%.100s' is used twice as a key in array of graph translations",
			  rrc.rrc_graph_xlat[ctr]);
		    if (0 < cmp)
		      {
			caddr_t swap;
			swap = rrc.rrc_graph_xlat[ctrL];
			rrc.rrc_graph_xlat[ctrL] = rrc.rrc_graph_xlat[ctrL + 2];
			rrc.rrc_graph_xlat[ctrL + 2] = swap;
			swap = rrc.rrc_graph_xlat[ctrL + 1];
			rrc.rrc_graph_xlat[ctrL + 1] = rrc.rrc_graph_xlat[ctrL + 3];
			rrc.rrc_graph_xlat[ctrL + 3] = swap;
		      }
		  }
	      }
	  }
...
bif_sparql_rdb2rdf_codegen (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, const char *fname)
{
  ...
  caddr_t *graph_xlat = ((3 <= argcount) ? bif_array_of_pointer_arg (qst, args, 2, "sparql_rdb2rdf_codegen") : NULL);
