...
xml_lazy_ent_t *DBG_NAME (xlazye_from_cache_key) (DBG_PARAMS xml_doc_cache_stdkey_t * cache_key, query_instance_t * qi)
{
...
xml_entity_t *DBG_NAME (xlazye_cut) (DBG_PARAMS xml_entity_t * xe, query_instance_t * qi)
{
  xlazye_make_actual_load (xe);
  return xe->_->DBG_NAME (xe_cut) (DBG_ARGS xe, qi);
}

xml_entity_t *DBG_NAME (xlazye_clone) (DBG_PARAMS xml_entity_t * xe, query_instance_t * qi)
{
  xlazye_make_actual_load (xe);
  return xe->_->DBG_NAME (xe_clone) (DBG_ARGS xe, qi);
}

int DBG_NAME (xlazye_attribute) (DBG_PARAMS xml_entity_t * xe, int start, XT * node, caddr_t * ret, caddr_t * name_ret)
{
  xlazye_make_actual_load (xe);
  return xe->_->DBG_NAME (xe_attribute) (DBG_ARGS xe, start, node, ret, name_ret);
}

void DBG_NAME (xlazye_string_value) (DBG_PARAMS xml_entity_t * xe, caddr_t * ret, dtp_t dtp)
{
  xlazye_make_actual_load (xe);
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_up (xe, node, up_flags);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_down_rev (xe, node);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_first_child (xe, node_test);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_last_child (xe, node_test);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_get_child_count_any (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_next_sibling (xe, node_test);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_next_sibling_wr (xe, node_test);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_prev_sibling (xe, node_test);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_attrvalue (xe, qname);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_currattrvalue (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_data_attribute_count (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_element_name (xe);
}
...
{
  xlazye_make_actual_load (xe);
  xe->_->xe_attr_word_range (xe, start, this_end, last_end);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_get_logical_path (xe, path);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_get_addon_dtd (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_element_name_test (xe, wname_node);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_ent_name_test (xe, wname_node);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_ent_node_test (xe, node);
}

int
xlazye_is_same_as (const xml_entity_t * this_xe, const xml_entity_t * that_xe)
{
  xlazye_make_actual_load ((xml_entity_t *) this_xe);
  return this_xe->_->xe_is_same_as (this_xe, that_xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_deref_id (xe, idbegin, idlength);
}

xml_entity_t *
xlazye_follow_path (xml_entity_t * xe, ptrlong * path, size_t path_depth)
{
  GPF_T1 ("xlazye_follow_path() is called");
  return NULL;
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_copy_to_xte_head (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_copy_to_xte_subtree (xe);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_copy_to_xte_forest (xe);
}
...
{
  GPF_T1 ("xlazye_reference() is called");
  return NULL;
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_find_expanded_name_by_qname (xe, qname, use_default);
}
...
{
  xlazye_make_actual_load (xe);
  return xe->_->xe_namespace_scope (xe, use_default);
}
