...
{
  const char *p = str;
  if (simple_rdf_numbers)
    {
      if (__chk_float_string (str) < 0)
	sqlr_new_error ("42001", "XPQ??", "Incorrect argument in decimal-as-double constructor:\"%s\"", str);
...
__integer_from_string (caddr_t * n, const char *str, int do_what)
{
  ...
  while ('0' == *p)
    p++;
  if ((s && ((XQ_NNINT == do_what) || (XQ_PINT == do_what) || ((XQ_UINT8 <= do_what) && (XQ_UINT64 >= do_what)))) ||
      (!s && ((XQ_NPINT == do_what) || (XQ_NINT == do_what))) || (('\0' == p[0]) && ((XQ_NINT == do_what) || (XQ_PINT == do_what))))
...
__duration_from_string (caddr_t * n, const char *str, int do_what)
{
  ...
  if (timeflag && 0 == hour && 0 == minute && 0 == second)
    sqlr_new_error ("42001", "XPQ??",
	"Incorrect argument in duration constructor ('T' char is specified with no time data after it):\"%s\"", str);
  if (timeflag && 0 != year && 0 != month)
    sqlr_new_error ("42001", "XPQ??", "Incorrect argument in duration constructor ('T' char is specified with no date data):\"%s\"",
	str);
  if (year || month)
...
xq_dt_mode_t xq_dt_modes[] = {
  {"dateTime", DT_TYPE_DATETIME,
	DTFLAG_YY | DTFLAG_MM | DTFLAG_DD | DTFLAG_HH | DTFLAG_MIN | DTFLAG_SS | DTFLAG_SF | DTFLAG_ZH | DTFLAG_ZM |
	DTFLAG_T_FORMAT_SETS_TZL | DTFLAG_DATES_AND_TIMES_ARE_ISO},
  {"date", DT_TYPE_DATE,
	DTFLAG_YY | DTFLAG_MM | DTFLAG_DD | DTFLAG_ZH | DTFLAG_ZM | DTFLAG_T_FORMAT_SETS_TZL | DTFLAG_DATES_AND_TIMES_ARE_ISO},
  {"time", DT_TYPE_TIME,
	DTFLAG_HH | DTFLAG_MIN | DTFLAG_SS | DTFLAG_SF | DTFLAG_ZH | DTFLAG_ZM | DTFLAG_T_FORMAT_SETS_TZL |
	DTFLAG_DATES_AND_TIMES_ARE_ISO},
  {"gYearMonth", DT_TYPE_DATE,
	DTFLAG_YY | DTFLAG_MM | DTFLAG_ZH | DTFLAG_ZM | DTFLAG_T_FORMAT_SETS_TZL | DTFLAG_DATES_AND_TIMES_ARE_ISO},
  {"gYear", DT_TYPE_DATE, DTFLAG_YY | DTFLAG_ZH | DTFLAG_ZM | DTFLAG_T_FORMAT_SETS_TZL | DTFLAG_DATES_AND_TIMES_ARE_ISO},
  {"gMonthDay", DT_TYPE_DATE,
	DTFLAG_MM | DTFLAG_DD | DTFLAG_ZH | DTFLAG_ZM | DTFLAG_T_FORMAT_SETS_TZL | DTFLAG_DATES_AND_TIMES_ARE_ISO},
...
__datetime_rcheck (caddr_t * n, int do_what)
{
  ...
  switch (do_what)
    {
    case XQ_DATETIME:
      return (DT_TYPE_DATETIME == dttype);
    case XQ_DATE:
      if (DT_TYPE_TIME == dttype)
	return 0;
      break;
    case XQ_TIME:
      if (DT_TYPE_DATE == dttype)
	return 0;
      break;
    case XQ_YEARMONTH:
    case XQ_YEAR:
...
static collation_t *
xpf_arg_collation (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe, int argnum)
{
...
xqf_substring (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  length = (tree->_.xp_func.argcount > 2 ? xpf_arg (xqi, tree, ctx_xe, DV_NUMERIC, 2) : 0);

  n1 = (int) virt_rint (unbox_double (box_cast ((caddr_t *) xqi->xqi_qi, start_loc, (sql_tree_tmp *) st_double,
	      DV_TYPE_OF (start_loc))));
  if (length)
    n2 = n1 + (int) virt_rint (unbox_double (box_cast ((caddr_t *) xqi->xqi_qi, length, (sql_tree_tmp *) st_double,
		DV_TYPE_OF (length))));
...
__xqf_ends_with (caddr_t str, caddr_t mstr, collation_t * coll)
{
  ...
  if (coll)
      ...
      for (idx = 0; idx < str_len; idx++)
	{
	  ...
	  utf8_idx++;
	  while (!UTF8_IS_HEADCHAR (str[utf8_idx]))
	    utf8_idx++;
	}
    }
  /* must not be here */
...
xqf_substring_before (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  if (tree->_.xp_func.argcount > 2)
    coll = xpf_arg_collation (xqi, tree, ctx_xe, 2);
  first_pointer =
      strstr_utf8_with_collation (arg1, utf8_strlen ((utf8char *) arg1), arg2, utf8_strlen ((utf8char *) arg2), NULL, coll);
...
xqf_substring_after (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  if (tree->_.xp_func.argcount > 2)
    coll = xpf_arg_collation (xqi, tree, ctx_xe, 2);
  first_pointer =
      strstr_utf8_with_collation (arg1, utf8_strlen ((utf8char *) arg1), arg2, utf8_strlen ((utf8char *) arg2), &end, coll);
...
xqf_empty (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  if (DV_ARRAY_OF_XQVAL == DV_TYPE_OF (val) && BOX_ELEMENTS (val) == 0)
    XQI_SET (xqi, tree->_.xp_func.res, (caddr_t) 1L);
  else
    XQI_SET (xqi, tree->_.xp_func.res, (caddr_t) 0L);
}
  ...
  if (tree->_.xp_func.argcount > 2)
    {
      ...
      n2 = n1 + (int) virt_rint (unbox_double (box_cast ((caddr_t *) xqi->xqi_qi, sub_len, (sql_tree_tmp *) st_double,
		  DV_TYPE_OF (sub_len))));
    }
  if (n2 > len)
    n2 = len;
...
xqf_is_same_node (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  ...
  if ((DV_XML_ENTITY != DV_TYPE_OF (i1)) || (DV_XML_ENTITY != DV_TYPE_OF (i2)))
...
int
xqf_dT_op_eq (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
  return ((t1->year == t2->year) &&
      (t1->month == t2->month) &&
      (t1->day == t2->day) &&
      (t1->hour == t2->hour) && (t1->minute == t2->minute) && (t1->second == t2->second) && (t1->fraction == t2->fraction));
}

int
xqf_dT_op_cmp (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
...
int
xqf_date_op_eq (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
  return ((t1->year == t2->year) && (t1->month == t2->month) && (t1->day == t2->day));
}

int
xqf_date_op_cmp (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
  uint32 days1 = date2num (t1->year, t1->month, t1->day);
  uint32 days2 = date2num (t2->year, t2->month, t2->day);
...
int
xqf_time_op_eq (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
  return ((t1->hour == t2->hour) && (t1->minute == t2->minute) && (t1->second == t2->second) && (t1->fraction == t2->fraction));
}

int
xqf_time_op_cmp (TIMESTAMP_STRUCT * t1, TIMESTAMP_STRUCT * t2)
{
  long sec1, sec2;
  sec1 = t1->hour * 3600 + t1->minute * 60 + t1->second;
  sec2 = t2->hour * 3600 + t2->minute * 60 + t2->second;
...
static void
xqf_dt_check (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe, xqf_dt_operation_f op, int expected)
{
...
static void
xqf_date_eq (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_date_op_eq, 1);
}

static void
xqf_date_lt (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_date_op_cmp, -1);
}

static void
xqf_date_gt (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_date_op_cmp, 1);
}

static void
xqf_time_eq (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_time_op_eq, 1);
}

static void
xqf_time_lt (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_time_op_cmp, -1);
}

static void
xqf_time_gt (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  xqf_dt_check (xqi, tree, ctx_xe, xqf_time_op_cmp, 1);
}

static void
xqf_yMd_add (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
...
static void
xqf_dT_add (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
...
static numeric_t
xqf_num_from_ts (TIMESTAMP_STRUCT * ts1)
{
...
static void
xqf_dT_sub_dTD (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  caddr_t dt1 = xpf_arg (xqi, tree, ctx_xe, DV_DATETIME, 0);
  caddr_t dt2 = xpf_arg (xqi, tree, ctx_xe, DV_DATETIME, 1);
...
static void
xqf_add_yMD_to_dT (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
...
static void
xqf_add_dTD_to_dT (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
...
static void
xqf_ts_add_year_month (TIMESTAMP_STRUCT * ts, int years, int months)
{
...
static void
xqf_sub_yMD_to_dT (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  long ymd = xqf_YM_arg (xqi, tree, ctx_xe, 1);
  caddr_t dt1 = xpf_arg (xqi, tree, ctx_xe, DV_DATETIME, 0);
  caddr_t res = box_copy (dt1);
...
static void
xqf_sub_dTD_to_dT (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  double dtd = xqf_DT_arg (xqi, tree, ctx_xe, 1);
  caddr_t dt1 = xpf_arg (xqi, tree, ctx_xe, DV_DATETIME, 0);
  caddr_t res = box_copy (dt1);
...
/* QNames */
static void
xqf_qname_eq (xp_instance_t * xqi, XT * tree, xml_entity_t * ctx_xe)
{
  caddr_t qn1 = xpf_raw_arg (xqi, tree, ctx_xe, 0), qn2 = xpf_raw_arg (xqi, tree, ctx_xe, 1);
  if ((DV_XML_ENTITY != DV_TYPE_OF (qn1)) || (DV_XML_ENTITY != DV_TYPE_OF (qn2)))
...
void
xqf_define_builtin (const char *xpfm_name,
...
/* The order of these declarations is important because the first one is used for reverse searches */
  xpfm_create_and_store_builtin (xpfm_name, xpfm_executable, xpfm_res_dtp, xpfm_min_arg_no, xpfm_main_args, xpfm_tail_args,
      XFN_NS_URI "/#");
...
void
xsd_define_builtin (const char *xpfm_name,
    xp_func_t xpfm_executable,
    ptrlong xpfm_res_dtp, ptrlong xpfm_min_arg_no, xpfm_arg_descr_t ** xpfm_main_args, xpfm_arg_descr_t ** xpfm_tail_args)
{
/* The order of these declarations is important because the first one is used for reverse searches */
...
void
xop_define_builtin (const char *xpfm_name,
    xp_func_t xpfm_executable,
    ptrlong xpfm_res_dtp, ptrlong xpfm_min_arg_no, xpfm_arg_descr_t ** xpfm_main_args, xpfm_arg_descr_t ** xpfm_tail_args)
{
/* The order of these declarations is important because the first one is used for reverse searches */
...
void
xdt_define_builtin (const char *xpfm_name,
    xp_func_t xpfm_executable,
    ptrlong xpfm_res_dtp, ptrlong xpfm_min_arg_no, xpfm_arg_descr_t ** xpfm_main_args, xpfm_arg_descr_t ** xpfm_tail_args)
{
/* The order of these declarations is important because the first one is used for reverse searches */
...
/*	p_name			| p_proc			| p_rcheck		| p_opcode		| null	| box	| p_dest_dtp	| p_typed_bif_name		| p_sql_cast_type */
  ...
  {"dayTimeDuration", __duration_from_string, NULL /*??? */ , 0, 0, 3, 0, "__xqf_str_parse_datetime", NULL},
  ...
  {"duration", __duration_from_string, NULL /*??? */ , 0, 0, 3, 0, "__xqf_str_parse_datetime", NULL},
  {"float", __float_from_string, NULL, XQ_FLOAT, 0, 0, DV_SINGLE_FLOAT, "__xqf_str_parse_float", "REAL"},
  {"gDay", __datetime_from_string, __datetime_rcheck, XQ_DAY, 0, 3, DV_DATETIME, "__xqf_str_parse_datetime", NULL},
  {"gMonth", __datetime_from_string, __datetime_rcheck, XQ_MONTH, 0, 3, DV_DATETIME, "__xqf_str_parse_datetime", NULL},
  {"gMonthDay", __datetime_from_string, __datetime_rcheck, XQ_MONTHDAY, 0, 3, DV_DATETIME, "__xqf_str_parse_datetime", NULL},
  {"gYear", __datetime_from_string, __datetime_rcheck, XQ_YEAR, 0, 3, DV_DATETIME, "__xqf_str_parse_datetime", NULL},
  {"gYearMonth", __datetime_from_string, __datetime_rcheck, XQ_YEARMONTH, 0, 3, DV_DATETIME, "__xqf_str_parse_datetime", NULL},
  ...
  {"normalizedString", __gen_string_from_string, __gen_string_rcheck, XQ_NORM_STRING, 0, 3, 0, "__xqf_str_parse_nvarchar", NULL},
  ...
  {"token", __gen_string_from_string, __gen_string_rcheck, XQ_TOKEN, 0, 3, 0, "__xqf_str_parse_nvarchar", NULL},
  ...
  {"yearMonthDuration", __duration_from_string, NULL /*??? */ , 0, 0, 3, 0, "__xqf_str_parse_datetime", NULL}
};
...
bif_xqf_str_parse_to_rdf_box (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  xqf_str_parser_desc_t *desc;
  {
	...
	sqlr_new_error ("22023", "SR007",
	    ...
	    "not an arg of type %s (%d)", "__xqf_str_parse_to_rdf_box", dv_type_title (type_iri_dtp), type_iri_dtp);
      }
  }
  if ((strlen (type_iri) <= XMLSCHEMA_NS_URI_LEN) || ('#' != type_iri[XMLSCHEMA_NS_URI_LEN]))
      ...
      if (!strcmp (type_iri, uname_rdf_ns_uri_XMLLiteral))
	{
	  ...
	  tree =
	      xml_make_mod_tree ((query_instance_t *) qst, arg, &err, FINE_XML | GE_XML, NULL /* no uri! */ , "UTF-8", NULL, NULL,
...
res_ready:
  if ((!desc->p_rdf_boxed) || (simple_rdf_numbers && !(desc->p_rdf_boxed & 0x2)))
    return res;
  else
    {
      rdf_box_t *rb = rb_allocate ();
...
XQF_STR_FN (boolean) XQF_STR_FN (date) XQF_STR_FN (datetime) XQF_STR_FN (double) XQF_STR_FN (float)
XQF_STR_FN (integer)
XQF_STR_FN (numeric)
XQF_STR_FN (nvarchar)
XQF_STR_FN (time)
     ...
     void xqf_init (void)
{
  ...
  bif_define_ex ("__xqf_str_parse", bif_xqf_str_parse, BMD_ALIAS, "__xqf_str_parse_boolean", BMD_ALIAS, "__xqf_str_parse_date",
      BMD_ALIAS, "__xqf_str_parse_datetime", BMD_ALIAS, "__xqf_str_parse_double", BMD_ALIAS, "__xqf_str_parse_float", BMD_ALIAS,
      "__xqf_str_parse_integer", BMD_ALIAS, "__xqf_str_parse_numeric", BMD_ALIAS, "__xqf_str_parse_nvarchar", BMD_ALIAS,
      "__xqf_str_parse_time", BMD_RET_TYPE, &bt_any, BMD_DONE);
  ...
  /* Functions */
  x2f_define_builtin ("ENTITY", NULL /*xqf_entity *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  x2f_define_builtin ("ID", NULL /*xqf_id *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("IDREF", NULL /*xqf_idref *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  x2f_define_builtin ("NCName", NULL /*xqf_ncname *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  x2f_define_builtin ("NMTOKEN", NULL /*xqf_nmtoken *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING,
	      1)), NULL);
  x2f_define_builtin ("add-days", xqf_add_days /* ??? */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL, DV_DATE, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("add-gMonth", xqf_add_gmonth /* ??? */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL, DV_DATE, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("add-gYear", xqf_add_gyear /* ??? */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL, DV_DATE, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("add-months", xqf_add_months /* ??? */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL, DV_DATE, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("add-years", xqf_add_years /* ??? */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL, DV_DATE, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xsd_define_builtin ("boolean", xqf_boolean /* XQuery 1.0 */ , DV_UNKNOWN, 1, xpfmalist (1, xpfma (NULL, XPDV_BOOL, 1)), NULL);
  x2f_define_builtin ("boolean-from-string", NULL /*xqf_boolean_from_string *//* ??? */ , XPDV_BOOL, 1, xpfmalist (1, xpfma (NULL,
	      DV_SHORT_STRING, 1)), NULL);
  ...
  x2f_define_builtin ("codepoint-compare", xqf_codepoint_compare /* ??? */ , DV_LONG_INT, 2, xpfmalist (2, xpfma (NULL,
	      DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("codepoint-contains", NULL /*xqf_codepoint_contains *//* ??? */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL,
	      DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("codepoint-substring-after", NULL /*xqf_codepoint_substring_after *//* ??? */ , DV_SHORT_STRING, 2,
      xpfmalist (2, xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("codepoint-substring-before", NULL /*xqf_codepoint_substring_before *//* ??? */ , DV_SHORT_STRING, 2,
      xpfmalist (2, xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("codepoints-to-string", xqf_codepoints_to_string /* XQuery 1.0 */ , DV_STRING, 0, xpfmalist (1, xpfma (NULL,
	      DV_LONG_INT, 1)), xpfmalist (1, xpfma (NULL, DV_LONG_INT, 1)));
  x2f_define_builtin ("compare", xqf_compare /* XQuery 1.0 */ , DV_LONG_INT, 2, xpfmalist (3, xpfma (NULL, DV_STRING, 1),
	  xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  /*xpf_define_alias ("concat", XFN_NS_URI, "concat", NULL); */
  xqf_define_builtin ("contains", xqf_contains /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (3, xpfma (NULL, DV_STRING, 1),
	  xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  ...
  xsd_define_builtin ("dateTime", xqf_datetime /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  ...
  xqf_define_builtin ("ends-with", xqf_ends_with /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (3, xpfma (NULL, DV_STRING, 1),
	  xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  x2f_define_builtin ("escape-uri", xqf_escape_uri /* XQuery 1.0 */ , DV_STRING, 2, xpfmalist (2, xpfma (NULL, DV_STRING, 1),
	  xpfma (NULL, DV_LONG_INT, 1)), NULL);
  ...
  xsd_define_builtin ("gMonth", xqf_gMonth /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  xsd_define_builtin ("gMonthDay", xqf_gMonthDay /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)),
      NULL);
  xsd_define_builtin ("gYear", xqf_gYear /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  xsd_define_builtin ("gYearMonth", xqf_gYearMonth /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING,
	      1)), NULL);
  x2f_define_builtin ("get-Century-from-date", xqf_get_Century_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      DV_DATE, 1)), NULL);
  ...
  x2f_define_builtin ("get-days", xqf_get_Day_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL, XPDV_DURATION,
	      1)), NULL);
  x2f_define_builtin ("get-duration", xqf_get_duration /* ??? */ , XPDV_DURATION, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  ...
  x2f_define_builtin ("get-gMonth-from-date", xqf_get_Month_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      DV_DATE, 1)), NULL);
  ...
  x2f_define_builtin ("get-gYear-from-date", xqf_get_Year_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      DV_DATE, 1)), NULL);
  ...
  x2f_define_builtin ("get-hour-from-dateTime", xqf_get_hour_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      DV_DATETIME, 1)), NULL);
  ...
  x2f_define_builtin ("get-minutes-from-dateTime", xqf_get_minutes_from_dateTime /* ??? */ , DV_LONG_INT, 1, xpfmalist (1,
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  ...
  x2f_define_builtin ("get-seconds-from-dateTime", xqf_get_seconds_from_dateTime /* ??? */ , DV_NUMERIC, 1, xpfmalist (1,
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  ...
  x2f_define_builtin ("get-timezone-from-date", xqf_get_TZ_from_dateTime /* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL,
	      DV_DATE, 1)), NULL);
  ...
  x2f_define_builtin ("language", NULL /*xqf_language *//* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING,
	      1)), NULL);
  ...
  x2f_define_builtin ("lower-case", xqf_lower_case /* XQuery 1.0 */ , DV_STRING, 1, xpfmalist (1, xpfma (NULL, DV_STRING, 1)),
      NULL);
  x2f_define_builtin ("match", NULL /*xqf_match *//* ??? */ , DV_ARRAY_OF_XQVAL, 2, xpfmalist (3, xpfma (NULL, DV_SHORT_STRING, 1),
	  xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("matches", xqf_matches /* ??? */ , XPDV_BOOL, 2, xpfmalist (3, xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL,
	      DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  ...
  x2f_define_builtin ("normalizedString", xqf_normalized_string /* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL,
	      DV_SHORT_STRING, 1)), NULL);
  /*xpf_define_alias   ("normalize-space", XFN_NS_URI, "normalize-space", NULL); */
  x2f_define_builtin ("normalize-unicode", NULL /*xqf_normalize_unicode */ , DV_SHORT_STRING, 2, xpfmalist (2, xpfma (NULL,
	      DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  ...
  xqf_define_builtin ("replace", xqf_replace /* XQuery 1.0 */ , DV_STRING, 3, xpfmalist (4, xpfma (NULL, DV_STRING, 1), xpfma (NULL,
	      DV_STRING, 1), xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  ...
  xqf_define_builtin ("starts-with", xqf_starts_with /* ??? */ , XPDV_BOOL, 2, xpfmalist (3, xpfma (NULL, DV_SHORT_STRING, 1),
	  xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  xqf_define_builtin ("string-length", xqf_string_length /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL, DV_STRING,
	      1)), NULL);
  xsd_define_builtin ("string", xqf_string /* ??? */ , DV_SHORT_STRING, 1, xpfmalist (1, xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("string-join", xqf_string_join /* XQuery 1.0 */ , DV_STRING, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN, 1),
	  xpfma (NULL, DV_STRING, 1)), NULL);
  x2f_define_builtin ("string-pad-beginning", NULL /*xqf_string_pad_beginning *//* ??? */ , DV_SHORT_STRING, 3, xpfmalist (3,
	  xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("string-pad-end", NULL /*xqf_string_pad_end *//* ??? */ , DV_SHORT_STRING, 3, xpfmalist (3, xpfma (NULL,
	      DV_SHORT_STRING, 1), xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  x2f_define_builtin ("string-to-codepoints", xqf_string_to_codepoints /* XQuery 1.0 */ , DV_UNKNOWN, 1, xpfmalist (1, xpfma (NULL,
	      DV_STRING, 1)), NULL);
  xqf_define_builtin ("substring", xqf_substring /* XQuery 1.0 */ , DV_STRING, 2, xpfmalist (3, xpfma (NULL, DV_STRING, 1),
	  xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xqf_define_builtin ("substring-after", xqf_substring_after /* XQUery 1.0 */ , DV_STRING, 2, xpfmalist (3, xpfma (NULL, DV_STRING,
	      1), xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  xqf_define_builtin ("substring-before", xqf_substring_before /* XQuery 1.0 */ , DV_STRING, 2, xpfmalist (3, xpfma (NULL,
	      DV_STRING, 1), xpfma (NULL, DV_STRING, 1), xpfma (NULL, DV_STRING, 1)), NULL);
  x2f_define_builtin ("temporal-dateTimeDuration-contains", xqf_dateTimeDuration_contains /* ??? */ , DV_UNKNOWN, 0, NULL,
      xpfmalist (1, xpfma (NULL, DV_UNKNOWN, 0)));
  x2f_define_builtin ("temporal-dateTimes-contains", xqf_dateTimes_contains /* ??? */ , DV_UNKNOWN, 0, NULL, xpfmalist (1,
	  xpfma (NULL, DV_UNKNOWN, 0)));
  x2f_define_builtin ("temporal-durationDateTime-contains", xqf_durationDateTime_contains /* ??? */ , DV_UNKNOWN, 0, NULL,
      xpfmalist (1, xpfma (NULL, DV_UNKNOWN, 0)));
  ...
  x2f_define_builtin ("tokenize", xqf_tokenize /* ??? */ , DV_UNKNOWN, 2, xpfmalist (3, xpfma (NULL, DV_SHORT_STRING, 1),
	  xpfma (NULL, DV_SHORT_STRING, 1), xpfma (NULL, DV_SHORT_STRING, 1)), NULL);
  xpf_define_alias ("translate", XFN_NS_URI, "translate", NULL);
  xpf_define_alias ("true", XFN_NS_URI, "true", NULL);
  x2f_define_builtin ("upper-case", xqf_upper_case /* XQuery 1.0 */ , DV_STRING, 1, xpfmalist (1, xpfma (NULL, DV_STRING, 1)),
      NULL);
  ...
  xop_define_builtin ("numeric-equal", xqf_numeric_equal, XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-greater-than", xqf_numeric_gt, XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-integer-divide", xqf_numeric_idivide, DV_NUMERIC, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1),
	  xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-less-than", xqf_numeric_lt, XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-mod", xqf_numeric_mod, DV_NUMERIC, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-multiply", xqf_numeric_multiply, DV_NUMERIC, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1),
	  xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-subtract", xqf_numeric_subtract /* ??? */ , DV_NUMERIC, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1),
	  xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-divide", xqf_numeric_divide, DV_NUMERIC, 2, xpfmalist (2, xpfma (NULL, DV_NUMERIC, 1), xpfma (NULL,
	      DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("numeric-unary-minus", xqf_numeric_uminus, DV_NUMERIC, 1, xpfmalist (1, xpfma (NULL, DV_NUMERIC, 1)), NULL);



  x2f_define_builtin ("zero-or-one", xqf_zero_or_one /* XQuery 1.0 */ , DV_UNKNOWN, 0, NULL, xpfmalist (1, xpfma (NULL, DV_UNKNOWN,
	      0)));
  x2f_define_builtin ("one-or-more", xqf_one_or_more /* XQuery 1.0 */ , XPDV_NODESET, 0, NULL, xpfmalist (1, xpfma (NULL,
	      DV_UNKNOWN, 0)));
  x2f_define_builtin ("exactly-one", xqf_exactly_one /* XQuery 1.0 */ , DV_UNKNOWN, 0, NULL, xpfmalist (1, xpfma (NULL, DV_UNKNOWN,
	      0)));
  ...
  xqf_define_builtin ("distinct-values", xqf_distinct_values /* XQuery 1.0 */ , DV_UNKNOWN, 0, NULL, xpfmalist (1, xpfma (NULL,
	      DV_UNKNOWN, 0)));
  x2f_define_builtin ("insert-before", xqf_insert_before /* XQuery 1.0 */ , XPDV_NODESET, 0, NULL, xpfmalist (1, xpfma (NULL,
	      DV_UNKNOWN, 0)));
  ...
  x2f_define_builtin ("subsequence", xqf_subsequence /* XQuery 1.0 */ , XPDV_NODESET, 0, NULL, xpfmalist (1, xpfma (NULL,
	      DV_UNKNOWN, 0)));
  ...
  xop_define_builtin ("boolean-equal", xqf_boolean_equal /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, XPDV_BOOL, 0),
	  xpfma (NULL, XPDV_BOOL, 0)), NULL);
  xop_define_builtin ("boolean-less-than", xqf_boolean_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, XPDV_BOOL, 0),
	  xpfma (NULL, XPDV_BOOL, 0)), NULL);
  xop_define_builtin ("boolean-greater-than", xqf_boolean_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, XPDV_BOOL,
	      0), xpfma (NULL, XPDV_BOOL, 0)), NULL);
...
#else
  xdt_define_builtin ("yearMonthDuration", xqf_duration /* XQuery 1.0 */ , XPDV_DURATION, 1, xpfmalist (1, xpfma (NULL,
	      DV_SHORT_STRING, 1)), NULL);
  xdt_define_builtin ("dayTimeDuration", xqf_duration /* XQuery 1.0 */ , XPDV_DURATION, 1, xpfmalist (1, xpfma (NULL,
	      DV_SHORT_STRING, 1)), NULL);
#endif
  x2f_define_builtin ("year-from-duration", xqf_get_Year_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      XPDV_DURATION, 1)), NULL);
  x2f_define_builtin ("day-from-duration", xqf_get_Day_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1, xpfma (NULL,
	      XPDV_DURATION, 1)), NULL);
  x2f_define_builtin ("month-from-duration", xqf_get_Month_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1,
	  xpfma (NULL, XPDV_DURATION, 1)), NULL);
  x2f_define_builtin ("minutes-from-duration", xqf_get_minutes_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1,
	  xpfma (NULL, XPDV_DURATION, 1)), NULL);
  x2f_define_builtin ("hours-from-duration", xqf_get_hour_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1,
	  xpfma (NULL, XPDV_DURATION, 1)), NULL);
  x2f_define_builtin ("seconds-from-duration", xqf_get_seconds_from_duration /* XQuery 1.0 */ , DV_LONG_INT, 1, xpfmalist (1,
	  xpfma (NULL, XPDV_DURATION, 1)), NULL);

  /* duration operations */
  xop_define_builtin ("yearMonthDuration-equal", xqf_YM_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN,
	      1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("yearMonthDuration-less-than", xqf_YM_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("yearMonthDuration-greater-than", xqf_YM_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("dayTimeDuration-equal", xqf_DT_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN, 1),
	  xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("dayTimeDuration-less-than", xqf_DT_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN,
	      1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("dayTimeDuration-greater-than", xqf_DT_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);

  /* arithmetic operations over durations */
  xop_define_builtin ("add-yearMonthDurations", xqf_yMd_add /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN,
	      1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("subtract-yearMonthDurations", xqf_yMd_sub /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("multiply-yearMonthDuration", xqf_yMd_mult /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("divide-yearMonthDuration", xqf_yMd_div /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("divide-yearMonthDuration-by-yearMonthDuration", xqf_yMd_div_yMd /* XQuery 1.0 */ , DV_UNKNOWN, 2,
      xpfmalist (2, xpfma (NULL, DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("add-dayTimeDurations", xqf_dT_add /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN,
	      1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("subtract-dayTimeDurations", xqf_dT_sub /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);
  xop_define_builtin ("multiply-dayTimeDuration", xqf_dT_mult /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL,
	      DV_UNKNOWN, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("divide-dayTimeDuration", xqf_dT_div /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN,
	      1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("divide-dayTimeDuration-by-dayTimeDuration", xqf_dT_div_dT /* XQuery 1.0 */ , DV_UNKNOWN, 2, xpfmalist (2,
	  xpfma (NULL, DV_UNKNOWN, 1), xpfma (NULL, DV_UNKNOWN, 1)), NULL);


  /* dateTime operations */
  xop_define_builtin ("dateTime-equal", xqf_dT_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("dateTime-less-than", xqf_dT_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("dateTime-greater-than", xqf_dT_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME,
	      1), xpfma (NULL, DV_DATETIME, 1)), NULL);
  /* Date operations */
  xop_define_builtin ("date-equal", xqf_date_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("date-less-than", xqf_date_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("date-greater-than", xqf_date_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  /* time operations */
  xop_define_builtin ("time-equal", xqf_time_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("time-less-than", xqf_time_lt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);
  xop_define_builtin ("time-greater-than", xqf_time_gt /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_DATETIME, 1)), NULL);

  x2f_define_builtin ("adjust-dateTime-to-timezone", xqf_adj_tz /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (2, xpfma (NULL,
	      DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
#if 1
...
#else
  x2f_define_builtin ("adjust-date-to-timezone", xqf_adj_tz /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (2, xpfma (NULL,
	      DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("adjust-time-to-timezone", xqf_adj_tz /* XQuery 1.0 */ , DV_DATETIME, 1, xpfmalist (2, xpfma (NULL,
	      DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
#endif

  /* Arithmetic functions on Durations, Dates and Times */
  x2f_define_builtin ("subtract-dateTimes-yielding-yearMonthDuration", xqf_dT_sub_yMD /* XQuery 1.0 */ , DV_NUMERIC, 1,
      xpfmalist (2, xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("subtract-dateTimes-yielding-dayTimeDuration", xqf_dT_sub_dTD /* XQuery 1.0 */ , DV_NUMERIC, 1, xpfmalist (2,
	  xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
#if 1
  xpf_define_alias ("subtract-dates-yielding-yearMonthDuration", XFN_NS_URI, "subtract-dateTimes-yielding-yearMonthDuration",
      XFN_NS_URI);
  xpf_define_alias ("subtract-dates-yielding-yearMonthDuration", XXF_NS_URI, "subtract-dateTimes-yielding-yearMonthDuration",
      XFN_NS_URI);
  xpf_define_alias ("subtract-dates-yielding-dayTimeDuration", XFN_NS_URI, "subtract-dateTimes-yielding-dayTimeDuration",
      XFN_NS_URI);
  xpf_define_alias ("subtract-dates-yielding-dayTimeDuration", XXF_NS_URI, "subtract-dateTimes-yielding-dayTimeDuration",
      XFN_NS_URI);
...
#else
  x2f_define_builtin ("subtract-dates-yielding-yearMonthDuration", xqf_dT_sub_yMD /* XQuery 1.0 */ , DV_NUMERIC, 1, xpfmalist (2,
	  xpfma (NULL, DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("subtract-dates-yielding-dayTimeDuration", xqf_dT_sub_dTD /* XQuery 1.0 */ , DV_NUMERIC, 1, xpfmalist (2,
	  xpfma (NULL, DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  x2f_define_builtin ("subtract-times", xqf_dT_sub_dTD /* XQuery 1.0 */ , DV_NUMERIC, 1, xpfmalist (2, xpfma (NULL, DV_DATETIME, 1),
	  xpfma (NULL, DV_NUMERIC, 1)), NULL);
#endif

  xop_define_builtin ("add-yearMonthDuration-to-dateTime", xqf_add_yMD_to_dT /* XQuery 1.0 */ , DV_DATETIME, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("add-dayTimeDuration-to-dateTime", xqf_add_dTD_to_dT /* XQuery 1.0 */ , DV_DATETIME, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("subtract-yearMonthDuration-from-dateTime", xqf_sub_yMD_to_dT /* XQuery 1.0 */ , DV_DATETIME, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("subtract-dayTimeDuration-from-dateTime", xqf_sub_dTD_to_dT /* XQuery 1.0 */ , DV_DATETIME, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATETIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
#if 1
...
#else
  xop_define_builtin ("add-yearMonthDuration-to-date", xqf_add_yMD_to_dT /* XQuery 1.0 */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL,
	      DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("add-dayTimeDuration-to-date", xqf_add_dTD_to_dT /* XQuery 1.0 */ , DV_DATE, 2, xpfmalist (2, xpfma (NULL,
	      DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("subtract-yearMonthDuration-from-date", xqf_sub_yMD_to_dT /* XQuery 1.0 */ , DV_DATE, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("subtract-dayTimeDuration-from-date", xqf_sub_dTD_to_dT /* XQuery 1.0 */ , DV_DATE, 2, xpfmalist (2,
	  xpfma (NULL, DV_DATE, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("add-dayTimeDuration-to-time", xqf_add_dTD_to_dT /* XQuery 1.0 */ , DV_TIME, 2, xpfmalist (2, xpfma (NULL,
	      DV_TIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
  xop_define_builtin ("subtract-dayTimeDuration-from-time", xqf_sub_dTD_to_dT /* XQuery 1.0 */ , DV_TIME, 2, xpfmalist (2,
	  xpfma (NULL, DV_TIME, 1), xpfma (NULL, DV_NUMERIC, 1)), NULL);
#endif

  /* QNames */
  xop_define_builtin ("QName-equal", xqf_qname_eq /* XQuery 1.0 */ , XPDV_BOOL, 2, xpfmalist (2, xpfma (NULL, DV_UNKNOWN, 1),
	  xpfma (NULL, DV_UNKNOWN, 1)), NULL);
