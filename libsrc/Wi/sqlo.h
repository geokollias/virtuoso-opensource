...
void sqlg_print_pred_and_list (sqlo_t * so, dk_set_t list, int force_local, int *is_first, char *text, size_t len, int *fill);
void sqlg_print_tb_pred_body (sqlo_t * so, df_elt_t ** body, int *first, char *text, size_t tlen, int *fill);
...
#ifdef BIF_XML
extern void xr_auto_meta_data (sql_comp_t * sc, ST * tree);
...
df_inx_op_t *inx_op_copy (sqlo_t * so, df_inx_op_t * dio, df_elt_t * org_tb_dfe, df_elt_t * tb_dfe);
