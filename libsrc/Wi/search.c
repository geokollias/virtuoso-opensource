...
itc_dive_cache_check (it_cursor_t * itc)
{
  ...
  if (!buf
      ...
      || buf->bd_to_bust || DPF_INDEX != SHORT_REF (buf->bd_buffer + DP_FLAGS) || it_is_free_page (db_main_tree, dp))
    return NULL;
  ...
  rc1 = pg_insert_key_compare (buf, buf->bd_content_map->pm_entries[0], itc);
  if (rc1 == DVC_GREATER)
    return NULL;
  rc2 = pg_insert_key_compare (buf, buf->bd_content_map->pm_entries[ct - 1], itc);
  if (rc2 == DVC_LESS)
    return NULL;
  buf->bd_readers++;
...
{
  if (it->itc_type != ITC_PLACEHOLDER && it->itc_is_geo_registered && it->itc_tree && it->itc_tree->it_key
      && it->itc_tree->it_key->key_is_geo)
...
plh_free (placeholder_t * pl)
{
  ...
  if (pl->itc_type != ITC_PLACEHOLDER || DV_ITC != DV_TYPE_OF (pl))
    GPF_T1 ("plh_free applied to non-placeholder");
...
int
itc_like_any_check (it_cursor_t * itc, db_buf_t dv1, db_buf_t dv3, row_size_t len1, row_size_t len3, unsigned short offset,
    db_buf_t pattern)
{
...
itc_row_check (it_cursor_t * itc, buffer_desc_t * buf)
{
  ...
  search_spec_t *sp;
  if (itc->itc_batch_size && itc->itc_n_results >= itc->itc_batch_size)
    GPF_T1 ("batch over end");
...
start:
  ...
  if (!it->itc_landed)
	...
	res = it->itc_simple_ps
	    ? itc_page_rcf_search (it, buf_ret, &leaf) : itc_page_search (it, buf_ret, &leaf, just_landed_match);
      just_landed_match = 0;
    }
...
search_switch:
  switch (res)
	...
	if (it->itc_owns_page != it->itc_page && lock_after_match && !it->itc_is_col)
	  {
	...
	if (it->itc_search_mode == SM_READ)
	  {
	    ...
	    if (((it->itc_ks && it->itc_ks->ks_is_last)
		    || ITC_VEC_MORE (it)) && !it->itc_cl_batch_done && !it->itc_insert_key->key_is_bitmap)
	      {
		if (it->itc_desc_order)
		  itc_prev_entry (it, *buf_ret);
		else
...
itc_hash_page_search (it_cursor_t * itc, buffer_desc_t ** buf_ret)
{
  ...
  for (;;)
      ...
      if (kv != 1)
	{
      ...
      rc = itc->itc_ks ? itc->itc_ks->ks_row_check (itc, *buf_ret) : itc_row_check (itc, *buf_ret);
...
itc_page_search (it_cursor_t * it, buffer_desc_t ** buf_ret, dp_addr_t * leaf_ret, int skip_first_key_cmp)
{
  ...
  if (it->itc_is_col)
    GPF_T1 ("itc_page_search does not apply for column-wise indices");
  if (it->itc_wst)
  ...
  while (1)
      ...
      if (PS_LOCKS == txn_clear)
	{
	  if (it->itc_owns_page != it->itc_page)
		      ...
		      && ITC_MAYBE_LOCK (it, it->itc_map_pos)))
		{
		  for (;;)
		    {
		      int wrc = ITC_IS_LTRX (it) ? itc_landed_lock_check (it, buf_ret) : NO_WAIT;
      ...
      if (skip_first_key_cmp)
	{
      ...
      else if (it->itc_key_spec.ksp_key_cmp != pg_key_compare && it->itc_key_spec.ksp_key_cmp != pg_insert_key_compare)
	{
      ...
      else
	{
	  ...
	  && PL_EXCLUSIVE != it->itc_lock_mode && ISO_REPEATABLE == min_iso_that_waits)
	{
	...
	return DVC_MATCH;
      if (DVC_MATCH == res)
	{
	  row_check = it->itc_ks ? it->itc_ks->ks_row_check (it, *buf_ret) : itc_row_check (it, *buf_ret);
	  ...
	  if (DVC_MATCH == row_check)
		      ...
		      || ISO_SERIALIZABLE == it->itc_isolation))
		{
	      ...
	      if (it->itc_cl_results && !itc_lock_after_match (it))
		{
		  /* A RR or *exckl RC cursor that does not own the page must return to itc_search for the locks.  */
		  goto next_row;
		}
	      if (ITC_VEC_MORE (it) && !itc_lock_after_match (it))
		{
		  /* A RR or *exckl RC cursor that does not own the page must return to itc_search for the locks.  */
		  goto next_row;
		}
  ...
  if (SM_READ_EXACT == itc->itc_search_mode)
    {
      ...
      if (KV_LEFT_DUMMY == IE_KEY_VERSION (itc->itc_row_data))
	GPF_T1 ("read exact hits left edge");
      if (PS_LOCKS == txn_clear)
	{
  ...
  for (;;)
      ...
      if (KV_LEFT_DUMMY == kv)
	{
	  ...
	  if (itc->itc_insert_key->key_is_geo)
		...
		GPF_T1 ("in geo index, not supposed to have left dummy in a non-leaf position");
	      goto next_row;
	    }
	  /* In random access one does not hit amatch with a left dummy.  In seq access can happen if other itc deletes the row and page this is on then this gets relocated to the leaf ptr to the left of the delete, in case whole pages are deleted.
	   * If so, then take the first vranch down from whatever is the next leftmost leaf ptr.  If on leaf already,, so left dummy does not point down, go to next row */
	  if (!itc->itc_landed)
      ...
      for (;;)
	{
	  ...
	  switch (res)
	    ...
	    case DVC_GREATER:
	      ...
	      break;
	    default:
	      GPF_T1 ("can't have this res for key_cmp_t");
	    }
	}
    }
}
  ...
  if (!kfs || BOX_ELEMENTS (kfs) <= slice || !key->key_fragments[slice])
    {
      ...
      if (err_ret)
	{
	  snprintf (msg, sizeof (msg), "The key %s does not have slice %d on host %d", key->key_name, slice,
...
itc_read_ahead1 (it_cursor_t * itc, buffer_desc_t ** buf_ret)
{
  ...
  if (itc->itc_n_reads < ra_threshold || !was_data || itc_is_ra_root (itc, itc->itc_page) || !iq_is_on ())
    return NULL;
  ...
  for (;;)
      ...
      while (ITC_AT_END != pos)
	{
      ...
      if (itc->itc_n_reads > 200 && ra->ra_nsiblings < RA_MAX_ROOTS / 2 && ra->ra_fill < RA_MAX_BATCH - 60)
	{
	  if (!pl)
	    {
...
itc_read_ahead_blob (it_cursor_t * itc, ra_req_t * ra, int flags)
{
  ...
  for (inx = 0; inx < ra->ra_fill; inx++)
	...
	{
	  log_error ("*** read-ahead of a free or out of range page dp L=%ld, database not necessarily corrupted.", ra->ra_dp[inx]);
	  ITC_LEAVE_MAP_NC (itc);
	  continue;
	}
...
int
em_ext_ra_pages (extent_map_t * em, it_cursor_t * itc, dp_addr_t ext_dp, dp_addr_t * leaves, int max, dp_addr_t except_dp,
    dk_hash_t * except)
{
  extent_t *ext;
  int fill = 0, inx;
  ...
  DO_SET (dbe_column_t *, col, &itc->itc_row_key->key_parts)
  {
	...
	if (itc->itc_insert_key->key_is_col)
	...
	else
	  {
	    ...
	    if (col == (dbe_column_t *) key->key_parts->data && 1 == itc->itc_search_par_fill
		&& !box_equal (data, itc->itc_search_params[0]))
	      {
		dk_free_tree (data);
		return;
	      }
      ...
      else
	{
	  ...
	  for (r = itc->itc_map_pos; r < buf->bd_content_map->pm_count; r++)
		...
		continue;
		  ...
		  || itc->itc_st.n_sample_rows < prev_sample + 1 || itc->itc_st.n_sample_rows - init_sample > 40000)
		break;
	    }
	}
    }
...
int
itc_page_split_search_1 (it_cursor_t * it, buffer_desc_t * buf, dp_addr_t * leaf_ret)
{
  db_buf_t page = buf->bd_buffer, row;
  int res;
  page_map_t *map = buf->bd_content_map;
...
int
itc_matches_on_page (it_cursor_t * itc, buffer_desc_t * buf, int *leaf_ctr_ret, int *rows_per_bm, dp_addr_t * alt_leaf_ret,
    int angle, int *ends_with_match)
{
...
start:
  ...
  if (!(*buf_ret)->bd_content_map)
    {
      log_error ("Suspect index page dp=%d key=%s, probably blob ref'd as index node.", (*buf_ret)->bd_page,
	  it->itc_insert_key->key_name);
      return 0;
    }
  rnd_leaf = 0;
  if (RANDOM_SEARCH_ON == it->itc_random_search)
...
make_est:
  ...
  if (!leaf_estimate && -1 != level_of_single_leaf_match && level > level_of_single_leaf_match - 1)
