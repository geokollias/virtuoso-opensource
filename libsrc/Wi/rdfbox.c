...
RDF_FETCH_OR_CREATE_1 (virtrdf_ns_uri)
      RDF_FETCH_OR_CREATE_1 (virtrdf_ns_uri_rdf_repl_all) RDF_FETCH_OR_CREATE_1 (virtrdf_ns_uri_rdf_repl_graph_group)}
...
rb_uname_to_flags_of_parseable_datatype (ccaddr_t dt_uname)
{
  if ((uname_xmlschema_ns_uri_hash_byte == dt_uname)
      ...
      || (uname_xmlschema_ns_uri_hash_unsignedLong == dt_uname) || (uname_xmlschema_ns_uri_hash_unsignedShort == dt_uname))
    return RDF_TYPE_PARSEABLE | RDF_TYPE_PARSEABLE_TO_NUMERIC;
  if ((uname_xmlschema_ns_uri_hash_dayTimeDuration == dt_uname))
...
{
  if ((rb_type__xsd_byte == dt_twobyte)
      ...
      || (rb_type__xsd_unsignedLong == dt_twobyte) || (rb_type__xsd_unsignedShort == dt_twobyte))
    return RDF_TYPE_PARSEABLE | RDF_TYPE_PARSEABLE_TO_NUMERIC;
  if ((rb_type__xsd_dayTimeDuration == dt_twobyte))
    return RDF_TYPE_PARSEABLE | RDF_TYPE_PARSEABLE_TO_DTDURATION;
  if ((rb_type__xsd_boolean == dt_twobyte) || (rb_type__xsd_date == dt_twobyte) || (rb_type__xsd_dateTime == dt_twobyte)
      ...
      || (rb_type__xsd_gMonthDay == dt_twobyte) || (rb_type__xsd_gYear == dt_twobyte) || (rb_type__xsd_gYearMonth == dt_twobyte)
...
rb_complete_1 (rdf_box_t * rb, lock_trx_t * lt, void * /*actually query_instance_t * */ caller_qi_v, int is_local)
{
 ...
 16843009, \
 RO_VAL \
 from DB.DBA.RDF_OBJ table option (no cluster) where RO_ID = ?", bootstrap_cli, NULL, SQLC_DEFAULT);
 ...
 16843009, \
 RO_VAL \
 from DB.DBA.RDF_OBJ where RO_ID = ?", bootstrap_cli, NULL, SQLC_DEFAULT);
 ...
 from DB.DBA.RDF_OBJ where RO_ID = ?", bootstrap_cli, NULL, SQLC_DEFAULT);
  ...
  err = qr_rec_exec (is_local ? (DV_XML_ENTITY == value_dtp ? rdf_box_qry_complete_xml_l : rdf_box_qry_complete_text_l)
      : (DV_XML_ENTITY == value_dtp ? rdf_box_qry_complete_xml : rdf_box_qry_complete_text),
      lt->lt_client, &lc, caller_qi, NULL, 1, ":0", box_num (rb->rb_ro_id), QRP_RAW);
  if (NULL != err)
    {
  ...
  if (lc_next (lc))
	  ...
	  if (rb->rb_chksum_tail)
	    {
		  ...
		  (box_length (cached_chksum) != box_length (chksum)) || memcmp (cached_chksum, chksum, box_length (chksum)))
		sqlr_new_error ("22023", "SR579",
		    "RDF integrity issue: the checksum of value retrieved from DB.DBA.RDF_OBJ with RO_ID = " BOXINT_FMT
		    " is not equal to checksum stored in RDF box", (boxint) (rb->rb_ro_id));
	    }
	  else if (NULL != chksum)
	    {
	...
	sqlr_new_error ("22023", "SR579",
	    "RDF integrity issue: the type %ld of value retrieved from DB.DBA.RDF_OBJ with RO_ID = " BOXINT_FMT
	    " is not equal to preset type %ld of RDF box", (long) DV_TYPE_OF (val), (boxint) (rb->rb_ro_id),
	    ((long) (((rdf_bigbox_t *) rb)->rbb_box_dtp)));
  ...
  if ((!rb->rb_is_complete) && (CALLER_LOCAL != caller_qi))
    sqlr_new_error ("22023", "SR580",
	"RDF box refers to row with RO_ID = " BOXINT_FMT " of table DB.DBA.RDF_OBJ, but no such row in the table",
	(boxint) (rb->rb_ro_id));
}
...
bif_rdf_box (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (type == RDF_BOX_GEO && box_dtp != DV_GEO)
    sqlr_new_error ("22023", "SR559", "The RDF box of type geometry needs a spatial object as a value, not a value of type %s (%d)",
	dv_type_title (box_dtp), box_dtp);
...
bif_ro_digest_from_parts (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if ((RDF_BOX_DEFAULT_TYPE != type) && (RDF_BOX_DEFAULT_LANG != lang))
    sqlr_new_error ("22023", "SR549", "Both datatype id %ld and language id %ld are not default in call of ro_digest_from_parts()",
	type, lang);
  if ((0 == ro_id) && !is_complete)
  ...
  switch (box_dtp)
    {
    case DV_STRING:
      if (is_complete && (box_length (box) <= (RB_MAX_INLINED_CHARS + 1)) && (RDF_BOX_DEFAULT_TYPE == type)
	  && (RDF_BOX_DEFAULT_LANG == lang))
	return box_copy (box);
      break;
    case DV_DB_NULL:
      return NEW_DB_NULL;
    ...
    case DV_BLOB_XPER:
    case DV_BLOB_XPER_HANDLE:
      sqlr_new_error ("22023", "SR559", "Large object (tag %d) is not a valid argument #1 in call of ro_digest_from_parts()",
	  box_dtp);
    }
  if (flags & 2)
    {
...
bif_rdf_box_data (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (1 < BOX_ELEMENTS (args))
	...
	sqlr_new_error ("22023", "SR545", "An incomplete RDF box '%.100s' is passed to rdf_box_data (..., %ld)", rb->rb_box,
	    should_be_complete);
    }
...
bif_rdf_box_strcmp (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if ((DV_RDF != DV_TYPE_OF (rb1)) || (DV_RDF != DV_TYPE_OF (rb2)) ||
      (!rb1->rb_is_complete) || (!rb2->rb_is_complete) || (!DV_STRINGP (rb1->rb_box)) || (!DV_STRINGP (rb2->rb_box)))
    return NEW_DB_NULL;
  rdf_box_audit (rb1);
  rdf_box_audit (rb2);
...
rb_serialize (caddr_t x, dk_session_t * ses)
{
  ...
  if ((rdf_no_string_inline || rb->rb_serialize_id_only) && rb->rb_ro_id && !with_content)
    {
...
dv_rdf_ro_id (db_buf_t dv2)
{
  ...
  int len;
  flags = dv2[1];
  if (!(RBS_OUTLINED & flags) || (RBS_EXT_TYPE & flags))
    return -1;
  if (RBS_SKIP_DTP & flags)
    len = dv2[2] + 1;
  else
...
rdf_box_hash (caddr_t box)
{
  ...
  return rb->rb_lang * 17 + rb->rb_type * 13 + rb->rb_is_complete * 9 +
      (rb->rb_chksum_tail ? (box_hash (((rdf_bigbox_t *) rb)->rbb_chksum) + 113) : box_hash (rb->rb_box));
}
...
rdf_box_hash_strong_cmp (ccaddr_t b1, ccaddr_t b2)
{
  ...
  if (DV_XML_ENTITY == data1_dtp)
	...
	return 0;
      if (((rdf_bigbox_t *) rb1)->rbb_chksum == ((rdf_bigbox_t *) rb2)->rbb_chksum)
	return 1;
      return 0;
    }
...
rb_ebv_of_plain_box (caddr_t val)
{
  switch (DV_TYPE_OF (val))
    {
    case DV_LONG_INT:
      return rb_new_bool (unbox (val));
    case DV_SINGLE_FLOAT:
      return rb_new_bool (unbox_float (val) != 0);
    case DV_DOUBLE_FLOAT:
      return rb_new_bool (unbox_double (val) != 0);
    case DV_NUMERIC:
      return rb_new_bool (!num_is_zero ((numeric_t) (val)));
    case DV_STRING:
      return (box_flags (val) & BF_IRI) ? NEW_DB_NULL : rb_new_bool (1 < box_length (val));
    case DV_BIN:
      return rb_new_bool (1 < box_length (val));
    case DV_WIDE:
      return rb_new_bool (sizeof (wchar_t) < box_length (val));
    default:			/* case DV_DATETIME: case DV_XML_ENTITY: case DV_GEO: case DV_IRI_ID: case DV_UNAME: case DV_DB_NULL: */
      return NEW_DB_NULL;
    }
}
...
rdf_ebv_int_of_plain_box (caddr_t val)
{
  switch (DV_TYPE_OF (val))
    {
    case DV_LONG_INT:
      return (caddr_t) ((ptrlong) (unbox (val) ? 1 : 0));
    case DV_SINGLE_FLOAT:
      return (caddr_t) ((ptrlong) (unbox_float (val) ? 1 : 0));
    case DV_DOUBLE_FLOAT:
      return (caddr_t) ((ptrlong) (unbox_double (val) ? 1 : 0));
    case DV_NUMERIC:
      return (caddr_t) ((ptrlong) (!num_is_zero ((numeric_t) (val)) ? 1 : 0));
    case DV_STRING:
      return (box_flags (val) & BF_IRI) ? NEW_DB_NULL : ((caddr_t) ((ptrlong) ((1 < box_length (val)) ? 1 : 0)));
    case DV_BIN:
      return (caddr_t) ((ptrlong) ((1 < box_length (val)) ? 1 : 0));
    case DV_WIDE:
      return (caddr_t) ((ptrlong) ((sizeof (wchar_t) < box_length (val)) ? 1 : 0));
    default:			/* case DV_DATETIME: case DV_XML_ENTITY: case DV_GEO: case DV_IRI_ID: case DV_UNAME: case DV_DB_NULL: */
      return NEW_DB_NULL;
    }
}
...
bif_rdf_long_from_batch_params (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL == rdf_long_from_batch_params_qr3)
	...
	rdf_long_from_batch_params_text5, &rdf_long_from_batch_params_qr5, NULL);
  switch (mode)
    {
    case 1:
      {
...
rdf_dist_or_redu_ser_long (caddr_t val, caddr_t * err_ret, int is_reduiced, const char *fun_name)
{
  ...
  if (DV_STRING == val_dtp)
  ...
  else if (DV_RDF == val_dtp)
    {
	  ...
	  (DV_STRING == DV_TYPE_OF (rbb->rbb_base.rb_box)) && (1024 > box_length (rbb->rbb_base.rb_box)))
	{
...
bif_rdf_dist_deser_long (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (deser))
	...
	case 6:
	  ...
	  break;
	default:
	  sqlr_new_error ("RDFXX", ".....", "Argument of __rdf_distinct_deser_long() is not made by __rdf_distinct_ser_long()");
	}
...
int
iri_cast_and_split_ttl_qname_impl (query_instance_t * qi, caddr_t iri, caddr_t * ns_prefix_ret, caddr_t * local_ret,
    ptrlong * is_bnode_ret, int flag)
{
  is_bnode_ret[0] = 0;
...
int
iri_cast_and_split_ttl_qname (query_instance_t * qi, caddr_t iri, caddr_t * ns_prefix_ret, caddr_t * local_ret,
    ptrlong * is_bnode_ret)
{
  return iri_cast_and_split_ttl_qname_impl (qi, iri, ns_prefix_ret, local_ret, is_bnode_ret, SPLIT_MODE_TTL);
}

int
iri_cast_and_split_xml_qname (query_instance_t * qi, caddr_t iri, caddr_t * ns_prefix_ret, caddr_t * local_ret,
    ptrlong * is_bnode_ret)
{
...
/*                                             0123456789 */
	...
	is_bnode_ret[0] =
	    0
	    /* Maybe this would be more accurate, but there are questions with named bnodes: ((('_' == iri[0]) && (':' == iri[1])) ? 1 : 0) */
	    ;
	return 1;
      }
    case DV_IRI_ID:
...
http_ttl_or_nt_prepare_obj (query_instance_t * qi, caddr_t obj, dtp_t obj_dtp, ttl_iriref_t * dt_ret)
{
  switch (obj_dtp)
	...
	{
	case DT_TYPE_DATE:
	  dt_ret->uri = uname_xmlschema_ns_uri_hash_date;
	  return;
	case DT_TYPE_TIME:
	  dt_ret->uri = uname_xmlschema_ns_uri_hash_time;
	  return;
	default:
	  dt_ret->uri = uname_xmlschema_ns_uri_hash_dateTime;
	  return;
	}
    case DV_SINGLE_FLOAT:
      dt_ret->uri = uname_xmlschema_ns_uri_hash_float;
      return;
    case DV_DOUBLE_FLOAT:
      dt_ret->uri = uname_xmlschema_ns_uri_hash_double;
      return;
    case DV_GEO:
      dt_ret->uri = uname_virtrdf_ns_uri_Geometry;
      return;
...
http_ttl_or_nt_write_xe (dk_session_t * ses, query_instance_t * qi, xml_entity_t * xe, int print_type_suffix, int html_ttl)
{
  ...
  if (!STRSES_CAN_BE_STRING (tmp_ses))
    {
      strses_free (tmp_ses);
      sqlr_new_error ("22023", "HT057",
	  "The serialization of XML literal as TURTLE or NT is longer than 10Mb, this is not supported");
    }
...
http_ttl_write_obj (dk_session_t * ses, ttl_env_t * env, query_instance_t * qi, caddr_t obj, dtp_t obj_dtp, ttl_iriref_t * dt_ptr)
{
  ...
  switch (obj_box_value_dtp)
	...
	if ((NULL == strchr (tmpbuf + 1, '.')) && (NULL == strchr (tmpbuf + 1, 'E')) && (NULL == strchr (tmpbuf + 1, 'e')))
	  {
		...
		if (myPOSINF_d == boxdbl)
		  buffill = sprintf (tmpbuf, "\"INF\"");
		else if (myNEGINF_d == boxdbl)
		  buffill = sprintf (tmpbuf, "\"-INF\"");
		else
		  buffill = sprintf (tmpbuf, "\"NAN\"");
	      }
	    else
	      {
	...
	if ((NULL == strchr (tmpbuf, '.')) && (NULL == strchr (tmpbuf, 'E')) && (NULL == strchr (tmpbuf, 'e')))
	  {
		...
		double myNEGINF_d = -1.0 / myZERO;
		if (myPOSINF_d == boxdbl)
		  buffill = sprintf (tmpbuf, "\"INF\"^^xsd:double");
		else if (myNEGINF_d == boxdbl)
		  buffill = sprintf (tmpbuf, "\"-INF\"^^xsd:double");
		else
		  buffill = sprintf (tmpbuf, "\"NAN\"^^xsd:double");
	      }
	    else
	      {
		strcpy (tmpbuf + buffill, ".0");
    ...
    default:
      {
	caddr_t tmp_utf8_box;
	int need_quotes, rb_type = ((DV_RDF == obj_dtp) ? ((rdf_box_t *) obj)->rb_type : RDF_BOX_MIN_TYPE);
...
fail:
  ...
  dk_free_box (tii.s.ns);
  dk_free_box (tii.s.loc);
  dk_free_box (tii.s.prefix);
  dk_free_box (tii.p.uri);
  dk_free_box (tii.p.ns);
  dk_free_box (tii.p.loc);
  dk_free_box (tii.p.prefix);
  dk_free_box (tii.o.uri);
  dk_free_box (tii.o.ns);
  dk_free_box (tii.o.loc);
  dk_free_box (tii.o.prefix);
  dk_free_box (tii.dt.uri);
  dk_free_box (tii.dt.ns);
  dk_free_box (tii.dt.loc);
  dk_free_box (tii.dt.prefix);
...
bif_http_ttl_triple (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if ((NULL != tii.o.ns) && ('\0' != tii.o.ns[0]))
    status += ttl_http_write_prefix_if_needed (qst, ses, env, &(env->te_ns_count_s_o), &(tii.o));
  if ((NULL == env->te_prev_subj_ns) ||
      strcmp (env->te_prev_subj_ns, tii.s.ns) || strcmp (env->te_prev_subj_loc, ((NULL != tii.s.uri) ? tii.s.uri : tii.s.loc)))
    {
      if (NULL != env->te_prev_subj_ns)
	{
	  session_buffered_write (ses, " .\n", 3);
	...
	{
	  env->te_prev_subj_loc = tii.s.loc;
	  tii.s.loc = NULL;
	}
    }
  if ((NULL == env->te_prev_pred_ns) ||
      strcmp (env->te_prev_pred_ns, tii.p.ns) || strcmp (env->te_prev_pred_loc, ((NULL != tii.p.uri) ? tii.p.uri : tii.p.loc)))
    {
      if (NULL != env->te_prev_pred_ns)
	{
	  session_buffered_write (ses, " ;\n\t", 4);
      ...
      session_buffered_write_char ('\t', ses);
      env->te_prev_pred_ns = tii.p.ns;
      tii.p.ns = NULL;
      if (NULL != tii.p.uri)
	{
	  env->te_prev_pred_loc = tii.p.uri;
	  tii.p.uri = NULL;
	}
      else
	{
	  env->te_prev_pred_loc = tii.p.loc;
	  tii.p.loc = NULL;
	}
    }
  else
...
bif_http_rdfxml_p_ns (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) env) || (sizeof (ttl_env_t) != box_length ((caddr_t) env)) || (DV_DICT_ITERATOR != DV_TYPE_OF (env->te_used_prefixes))	/* ||
																						   (((DV_STRING == DV_TYPE_OF (env->te_prev_subj_ns)) || (DV_UNAME == DV_TYPE_OF (env->te_prev_subj_ns))) &&
																						   ((DV_STRING != DV_TYPE_OF (env->te_prev_subj_loc)) ||
																						   ((DV_STRING != DV_TYPE_OF (env->te_prev_pred_ns)) && (DV_UNAME != DV_TYPE_OF (env->te_prev_pred_ns))) ||
																						   (DV_STRING != DV_TYPE_OF (env->te_prev_pred_loc)) ) ) */  ||
...
static void
http_rdfxml_write_obj (dk_session_t * ses, ttl_env_t * env, query_instance_t * qi, caddr_t obj, dtp_t obj_dtp,
    ttl_iriref_t * dt_ptr)
{
  caddr_t obj_box_value;
  dtp_t obj_box_value_dtp;
...
bif_http_rdfxml_triple (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  switch (obj_dtp)
    ...
    case DV_XML_ENTITY:
      ...
      break;
    case DV_RDF:
      if (DV_XML_ENTITY == DV_TYPE_OF (((rdf_box_t *) obj)->rb_box))
	obj_is_xml = 1;
      break;
    }
  if (obj_is_iri)
  ...
  if ((NULL == env->te_prev_subj_loc) || strcmp (env->te_prev_subj_loc, tii.s.uri))
    {
...
bif_http_nt_triple (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  ttl_iriref_items_t tii;
  memset (&tii, 0, sizeof (ttl_iriref_items_t));
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) env) || (sizeof (nt_env_t) != box_length ((caddr_t) env)))
  ...
  if (obj_is_iri)
      ...
      if (!(tii.o.is_bnode))
	{
	  ...
	  if (cache_ok)
	    {
	      err =
		  srv_make_new_error ("22023", "SR601",
		  "Argument 1 of http_ttl_value() needs a namespace declaration, use http_ttl_prefixes() in advance");
	      goto fail;
	    }
	}
    }
	  ...
	  if (cache_ok)
	    {
	      err =
		  srv_make_new_error ("22023", "SR601",
		  "Argument 1 of http_ttl_value() needs a namespace declaration for the type of the literal, use http_ttl_prefixes() in advance");
	      goto fail;
	    }
	}
    }
  ...
  switch (obj_box_value_dtp)
	...
	if (NULL == type_uri)
	  switch (DT_DT_TYPE (obj_box_value))
	    {
	    case DT_TYPE_DATE:
	      type_uri = uname_xmlschema_ns_uri_hash_date;
	      break;
	    case DT_TYPE_TIME:
	      type_uri = uname_xmlschema_ns_uri_hash_time;
	      break;
	    default:
	      type_uri = uname_xmlschema_ns_uri_hash_dateTime;
	      break;
	    }
	break;
      }
...
bif_http_talis_json_triple (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if ((DV_STRING != DV_TYPE_OF (env->tje_prev_subj)) && (DV_UNAME != DV_TYPE_OF (env->tje_prev_subj)))
    {
      dk_free_tree (env->tje_prev_subj);
      env->tje_prev_subj = NULL;
      dk_free_tree (env->tje_prev_pred);
  ...
  if ((NULL == env->tje_prev_subj) || strcmp (env->tje_prev_subj, subj_iri))
	...
	{			/* 012345.6789 */
      ...
      /* .0123456 */
      session_buffered_write (ses, "\" : { ", 6);
      env->tje_prev_subj = subj_iri_is_new ? subj_iri : box_copy (subj_iri);
      subj_iri_is_new = 0;
    }
...
bif_sparql_rset_ttl_write_row (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_DB_NULL == DV_TYPE_OF (ses))
  ...
  else if (DV_LONG_INT == DV_TYPE_OF (ses))
    {
      ...
      if (!ses)
	sqlr_new_error ("37000", "HT081", "Function sparql_rset_ttl_write_row() outside of HTTP context and no stream specified");
    }
  ...
  if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (row)) || (BOX_ELEMENTS (row) != colcount))
    sqlr_new_error ("22023", "SR606",
	"Argument 3 of sparql_rset_ttl_write_row() should be an array of values and length should match to the argument 2");
  if (NULL != env->te_prev_subj_ns)
    {
  ...
  for (colctr = 0; colctr < colcount; colctr++)
	...
	ttl_http_write_prefix_if_needed (qst, ses, env, (obj_is_iri ? &(env->te_ns_count_s_o) : &(env->te_ns_count_p_dt)), col_ti);
    }
  ...
  for (colctr = 0; colctr < colcount; colctr++)
      ...
      if (DV_DB_NULL != obj_dtp)
	{
      ...
      dk_free_box (col_ti->uri);
      col_ti->uri = NULL;
      dk_free_box (col_ti->ns);
      col_ti->ns = NULL;
      dk_free_box (col_ti->loc);
      col_ti->loc = NULL;
      dk_free_box (col_ti->prefix);
      col_ti->prefix = NULL;
    }
  SES_PRINT (ses, " ] .\n");
  return NULL;
...
bif_sparql_rset_nt_write_row (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) env) ||
      ...
      ((DV_LONG_INT != DV_TYPE_OF (env->ne_out_ses)) && (DV_STRING_SESSION != DV_TYPE_OF (env->ne_out_ses))
	  && (DV_DB_NULL != DV_TYPE_OF (env->ne_out_ses))) || (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) (env->ne_cols)))
  ...
  if (DV_DB_NULL == DV_TYPE_OF (ses))
  ...
  else if (DV_LONG_INT == DV_TYPE_OF (ses))
    {
      ses = qi->qi_client->cli_http_ses;	/*(dk_session_t *)((ptrlong)(unbox ((caddr_t)ses))); */
      if (!ses)
	sqlr_new_error ("37000", "HT081", "Function sparql_rset_nt_write_row() outside of HTTP context and no stream specified");
    }
  ...
  if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (row)) || (BOX_ELEMENTS (row) != colcount))
    sqlr_new_error ("22023", "SR606",
	"Argument 3 of sparql_rset_nt_write_row() should be an array of values and length should match to the argument 2");
  for (colctr = 0; colctr < colcount; colctr++)
    {
  ...
  SES_PRINT (ses, "_:ResultSet2053 <http://www.w3.org/2005/sparql-results#solution> ");
  SES_PRINT (ses, rowid_label);
  SES_PRINT (ses, " .\n");
  for (colctr = 0; colctr < colcount; colctr++)
    {
      ttl_iriref_t *col_ti = env->ne_cols + colctr;
	...
	continue;
      ...
      SES_PRINT (ses, rowid_label);
      SES_PRINT (ses, " <http://www.w3.org/2005/sparql-results#binding> ");
      SES_PRINT (ses, colid_label);
      SES_PRINT (ses, " .\n");
      SES_PRINT (ses, colid_label);
      SES_PRINT (ses, " <http://www.w3.org/2005/sparql-results#variable> \"");
      dks_esc_write (ses, col_ti->colname, strlen (col_ti->colname), CHARSET_UTF8, CHARSET_UTF8, DKS_ESC_TTL_DQ);
      SES_PRINT (ses, "\" .\n");
      SES_PRINT (ses, colid_label);
      SES_PRINT (ses, " <http://www.w3.org/2005/sparql-results#value> ");
      if (col_ti->is_iri)
...
bif_sparql_rset_json_write_row (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) env) ||
      (sizeof (talis_json_env_t) != box_length ((caddr_t) env)) ||
      ((DV_LONG_INT != DV_TYPE_OF (env->tje_out_ses)) && (DV_STRING_SESSION != DV_TYPE_OF (env->tje_out_ses))
	  && (DV_DB_NULL != DV_TYPE_OF (env->tje_out_ses))) || (DV_ARRAY_OF_POINTER != DV_TYPE_OF ((caddr_t) (env->tje_colnames))))
  ...
  if (DV_DB_NULL == DV_TYPE_OF (ses))
  ...
  else if (DV_LONG_INT == DV_TYPE_OF (ses))
    {
      ses = qi->qi_client->cli_http_ses;	/*(dk_session_t *)((ptrlong)(unbox ((caddr_t)ses))); */
      if (!ses)
	sqlr_new_error ("37000", "HT081", "Function sparql_rset_json_write_row() outside of HTTP context and no stream specified");
    }
  colcount = BOX_ELEMENTS ((caddr_t) (env->tje_colnames));
  if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (row)) || (BOX_ELEMENTS (row) != colcount))
    sqlr_new_error ("22023", "SR606",
	"Argument 3 of sparql_rset_json_write_row() should be an array of values and length should match to the argument 2");
  for (colctr = 0; colctr < colcount; colctr++)
    {
	...
	SES_PRINT (ses, "\n    { \"");
      need_comma = 1;
      dks_esc_write (ses, colname, strlen (colname), CHARSET_UTF8, CHARSET_UTF8, DKS_ESC_TTL_DQ);
      SES_PRINT (ses, "\": ");
      if (obj_is_iri)
	http_talis_json_write_ref_obj (ses, obj_iri, obj_is_bnode);
      else
...
sparql_rset_xml_write_row_impl (query_instance_t * qi, dk_session_t * ses, caddr_t * colnames, caddr_t * row)
{
  ...
  for (colctr = 0; colctr < colcount; colctr++)
	...
	sqlr_new_error ("22023", "SR604",
	    "Argument 2 of sparql_rset_xml_write_row() should be an array of strings and only strings");
      SES_PRINT (ses, "\n   <binding name=\"");
      SES_PRINT (ses, name);
      SES_PRINT (ses, "\">");
      switch (val_dtp)
...
bif_sparql_rset_xml_write_row (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  colcount = BOX_ELEMENTS (colnames);
  if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (row)) || (BOX_ELEMENTS (row) != colcount))
    sqlr_new_error ("22023", "SR603",
	"Argument 3 of sparql_rset_xml_write_row() should be an array of values and length should match to the argument 2");
...
bif_sparql_iri_split_rdfa_qname (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  do
	...
	{
	  res =
	      (flags & 0x2) ? list (3, box_copy (prefix_ptr[0]), box_copy (ns_iri), box_dv_short_nchars (tail,
...
bif_rdf_graph_default_perms_enable_raw_access_check (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (new_control > old_control)
    {
      int ctr;
      for (ctr = 0; ctr < spar_unsafe_table_name_count; ctr++)
	{
	  char buf[100];
	  snprintf (buf, sizeof (buf) - 1, "DB.DBA.%s", spar_unsafe_table_names[ctr]);
...
int
rdf_graph_configured_perms (query_instance_t * qst, caddr_t graph_boxed_iid, user_t * u, int check_usr_rdf_graph_perms,
    int req_perms)
{
  int perms = 0;
  caddr_t boxed_u_id;
  ...
  boxed_u_id = box_num (u->usr_id);
  do
    {
      if (U_ID_DBA == u->usr_id)
	{
	  perms = 0x8000 | 0x3FF;
	  break;
	}
      ...
      if (NULL != hit)
	{
      ...
      dict =
	  graph_is_private ? rdf_graph_default_private_perms_of_user_dict_htable :
	  rdf_graph_default_world_perms_of_user_dict_htable;
      hit = (caddr_t *) id_hash_get (dict, (caddr_t) (&boxed_u_id));
      if (NULL != hit)
	{
	  perms |= 0x8000 | unbox (hit[0]);
	  if (!(req_perms & ~perms))
  ...
  while (0);
  dk_free_box (boxed_u_id);
  return perms;
}

int
rdf_graph_app_cbk_perms (query_instance_t * qst, caddr_t graph_boxed_iid, user_t * u, caddr_t app_cbk, caddr_t app_uid,
    caddr_t * err_ret)
{
  ...
  err_ret[0] = qr_quick_exec (app_cbk_qr, cli, NULL, &lc, 3,
      ":0", app_cbk, QRP_STR, ":1", box_copy_tree (graph_boxed_iid), QRP_RAW, ":2", app_uid, QRP_STR);
...
bif_rgs_impl_graph_boxed_iid (caddr_t * qst, caddr_t graph, int bif_can_use_index)
{
  if (DV_IRI_ID == DV_TYPE_OF (graph))
    return graph;
  else if (bif_can_use_index && (((DV_STRING == DV_TYPE_OF (graph)) /*&& (BF_IRI & box_flags(graph)) */ )
	  || (DV_UNAME == DV_TYPE_OF (graph))))
    {
      caddr_t err = NULL;
...
void
bif_rgs_impl_graph_set_userdetails (caddr_t * qst, state_slot_t ** args, const char *fname, int bif_can_use_index,
    rgs_userdetails_t * ud_ret)
{
  ...
  if (bif_can_use_index)
	...
	ud_ret->ud_u =
	    bif_user_t_arg (qst, args, 1, fname,
      ...
      else
	{
	  ...
	  if ((DV_ARRAY_OF_POINTER != DV_TYPE_OF (user_and_cbk)) ||
		...
		"The second argument of %s() should be a string (user name), an integer (user ID) or an array of special format",
		fname);
	  uid_or_uname = user_and_cbk[0];
	  ud_ret->ud_app_cbk = user_and_cbk[1];
	  ud_ret->ud_app_uid = user_and_cbk[2];
	  ud_ret->ud_u =
	      bif_user_t_arg_int (uid_or_uname, 1, fname,
	      (USER_SHOULD_EXIST | USER_SHOULD_BE_SQL_ENABLED | USER_NOBODY_IS_PERMITTED | USER_SPARQL_IS_PERMITTED), 1);
	}
    }
  else
...
caddr_t
bif_rgs_impl_make_error_for_assert (caddr_t * qst, const char *fname, int bif_can_use_index, caddr_t graph, caddr_t graph_boxed_iid,
    int failed_perms, const char *opname, const char *user_type, rgs_userdetails_t * ud_ptr)
{
  query_instance_t *qi = (query_instance_t *) qst;
  caddr_t graph_iri;
  caddr_t err;
  ...
  err = srv_make_new_error ("RDF02", "SR619", "%.50s access denied: %.20s user %d (%.200s) has no %.50s permission on graph %.500s",
      ...
      rdf_graph_user_perm_title (failed_perms), (graph_iri ? graph_iri : "???"));
  ...
  if ((NULL != ud.ud_app_cbk) && (NULL != ud.ud_u))
    {
      ...
      failed_perms = req_perms & ~perms;
      if (failed_perms && (RGU_ASSERT == mode))
	sqlr_resignal (bif_rgs_impl_make_error_for_assert (qst, fname, bif_can_use_index, graph, graph_boxed_iid, failed_perms,
		opname, "application", &ud));
    }
  if (graph_boxed_iid != graph)
    dk_free_tree (graph_boxed_iid);
  switch (mode)
...
bif_rgs_prepare_del_or_ins (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  caddr_t dflt_graph_boxed_iid =
      ((DV_DB_NULL == DV_TYPE_OF (dflt_graph)) ? NULL : bif_rgs_impl_graph_boxed_iid (qst, dflt_graph, 1));
  caddr_t *all_flds[4];
  ...
  data_to_drop =
      (caddr_t **) ((void *) (list (5, NULL /* place for good_repl_idxs */ , all_flds[0], all_flds[1], all_flds[2], all_flds[3])));
  for (quad_ctr = 0; quad_ctr < quad_count; quad_ctr++)
    {
      caddr_t graph = quads[quad_ctr][3];
      caddr_t graph_boxed_iid =
	  ((DV_DB_NULL == DV_TYPE_OF (graph)) ? dflt_graph_boxed_iid : bif_rgs_impl_graph_boxed_iid (qst, graph, 1));
      caddr_t err;
      iri_id_t graph_iid;
      int g_props;
      int g_repl;
	...
	hash_table_free (g_props_hash);
      err =
	  bif_rgs_impl_make_error_for_assert (qst, fname, 1, graph, graph_boxed_iid, RDF_GRAPH_PERM_WRITE, opname, user_type, &ud);
...
static int
rdf_single_check (query_instance_t * qi, caddr_t start_box_arg, boxint ro_id, rdf_box_t ** complete_rb_ptr, long opcode,
    caddr_t opval)
{
...
bif_rdf_range_check (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  for (;;)
	...
	{
	  if (!(res =
		  rdf_single_check ((query_instance_t *) qst, start_box_arg, ro_id, &complete_box, lo_op, bif_arg (qst, args, 2,
			  "__rdf_range_check"))))
	    break;
	}
      ...
      if (hi_op)
	res =
	    rdf_single_check ((query_instance_t *) qst, start_box_arg, ro_id, &complete_box, hi_op, bif_arg (qst, args, 4,
		"__rdf_range_check"));
      break;
    }
...
/* TBD: call rdf_type_id (dt_iri) */
    ...
    void rdf_box_init ()
    {
      ...
      bif_define_ex ("sparql_ebv_int", bif_sparql_ebv_int, BMD_ALIAS, "sparql_ebv_int_of_sqlval", BMD_ALIAS,
	  "sparql_ebv_int_of_obj", BMD_ALIAS, "__ro2ebv_int", /*BMD_VECTOR_IMPL, bif_ro2ebv_int_vec, */ BMD_RET_TYPE, &bt_integer,
