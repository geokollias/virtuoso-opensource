...
sparp_mp_sparql_cap_cbk (mem_pool_t * mp, void *cbk_env)
{
  ...
  if (NULL != sparqre->sparqre_dbg_sparp)
    spar_error (sparqre->sparqre_dbg_sparp, "SPARQL compiler run out of temporary memory, report saved in %s, report ID %s",
	sparp_dump_weird_query_path, digest_hex);
}
...
spart_count_specific_elems_by_type (ptrlong type)
{
  ...
  switch (type)
    {
    case SPAR_ALIAS:
      return sizeof (sample._.alias);
    case SPAR_BLANK_NODE_LABEL:
      return sizeof (sample._.bnode);
    case SPAR_BUILT_IN_CALL:
      return sizeof (sample._.builtin);
    case SPAR_CONV:
      return sizeof (sample._.conv);
    case SPAR_FUNCALL:
      return sizeof (sample._.funcall);
    case SPAR_GP:
      return sizeof (sample._.gp);
    case SPAR_REQ_TOP:
      return sizeof (sample._.req_top);
    case SPAR_RETVAL:
      return sizeof (sample._.retval);
    case SPAR_LIT:
      return sizeof (sample._.lit);
    case SPAR_QNAME:
      return sizeof (sample._.qname);
    case SPAR_SQLCOL:
      return sizeof (sample._.qm_sqlcol);
    case SPAR_VARIABLE:
      return sizeof (sample._.var);
    case SPAR_TRIPLE:
      return sizeof (sample._.triple);
    case SPAR_QM_SQL_FUNCALL:
      return sizeof (sample._.qm_sql_funcall);
    case SPAR_CODEGEN:
      return sizeof (sample._.codegen);
    case SPAR_LIST:
      return sizeof (sample._.list);
    case SPAR_GRAPH:
      return sizeof (sample._.graph);
    case SPAR_WHERE_MODIFS:
      return sizeof (sample._.wm);
    case SPAR_SERVICE_INV:
      return sizeof (sample._.sinv);
    case SPAR_BINDINGS_INV:
      return sizeof (sample._.binv);
    case SPAR_DEFMACRO:
      return sizeof (sample._.defmacro);
    case SPAR_MACROCALL:
      return sizeof (sample._.macrocall);
    case SPAR_MACROPU:
      return sizeof (sample._.macropu);
    case ORDER_L:
      return sizeof (sample._.oby);
    case SPAR_PPATH:
      return sizeof (sample._.ppath);
    case BOP_NOT:
    case BOP_OR:
    case BOP_AND:
    case BOP_PLUS:
    ...
    case BOP_GTE:
    case BOP_LIKE:
      return sizeof (sample._.bin_exp);
    }
  GPF_T;
  return 0;
}
...
#ifdef MALLOC_DEBUG
  tree =
      (SPART *) dbg_mp_alloc_box (spartlist_impl_file, spartlist_impl_line, THR_TMP_POOL, sizeof (caddr_t) * length,
      DV_ARRAY_OF_POINTER);
#else
...
#ifdef MALLOC_DEBUG
  tree =
      (SPART *) dbg_dk_alloc_box (spartlist_impl_file, spartlist_impl_line, sizeof (caddr_t) * (1 + length + tail_len),
      DV_ARRAY_OF_POINTER);
#else
...
spar_source_place (sparp_t * sparp, char *raw_text)
{
  ...
  sprintf (buf, "%.400s, line %d%.6s%.1000s%.5s%.15s%.1000s%.5s",
      ...
      ((NULL == raw_text) ? "" : "'"),
      ((NULL == next_text) ? "" : ", before '"), ((NULL == next_text) ? "" : next_text), ((NULL == next_text) ? "" : "'"));
...
spar_dbg_string_of_triple_field (sparp_t * sparp, SPART * fld)
{
  switch (SPART_TYPE (fld))
    {
    case SPAR_BLANK_NODE_LABEL:
      return t_box_sprintf (210, "_:%.200s", fld->_.var.vname);
    case SPAR_VARIABLE:
      return t_box_sprintf (210, "?%.200s", fld->_.var.vname);
    case SPAR_QNAME:
	...
	return t_box_sprintf (210, "\"%.200s\"", fld->_.lit.val);
      if ((uname_xmlschema_ns_uri_hash_integer == fld->_.lit.datatype) ||
	  ...
	  (uname_xmlschema_ns_uri_hash_double == fld->_.lit.datatype))
	{
	  if (DV_STRINGP (fld->_.lit.val))
	  ...
	  else
	    {
	      ...
	      str =
		  box_cast_to (NULL, fld->_.lit.val, DV_TYPE_OF (fld->_.lit.val), DV_SHORT_STRING, NUMERIC_MAX_PRECISION,
		  NUMERIC_MAX_SCALE, &err);
	      if (!err && str)
...
sparyyerror_impl_1 (sparp_t * sparp, char *raw_text, int yystate, short *yyssa, short *yyssp, const char *strg)
{
  ...
  sqlr_new_error ("37000", "SP030",
      ...
      sparp->sparp_err_hdr,
      lineno, strg, sm2, sm1, yystate, ((sp1 & ~0x7FF) ? -1 : sp1) /* stub to avoid printing random garbage in logs */ ,
      ((NULL == raw_text) ? "" : " at '"),
      ((NULL == raw_text) ? "" : raw_text),
      ((NULL == raw_text) ? "" : "'"),
      ((NULL == next_text) ? "" : " before '"), ((NULL == next_text) ? "" : next_text), ((NULL == next_text) ? "" : "'"));
...
spar_error_if_unsupported_syntax_imp (sparp_t * sparp, int feature_in_use, const char *feature_name)
{
  if (NULL != sparp->sparp_env->spare_context_sinvs)
      ...
      if ((SSG_SD_NEED_LOAD_SERVICE_DATA & sparp->sparp_permitted_syntax) && (SPAR_QNAME == SPART_TYPE (sinv->_.sinv.endpoint)))
	{
	  ...
	  if (NULL != cli)
	    connection_set (cli, box_dv_uname_string ("SPARQL_endpoint_to_load_service_metadata"),
		box_dv_short_string (sinv->_.sinv.endpoint->_.qname.val));
  ...
  spar_error (sparp,
      "The support of %.200s syntax is disabled for debugging or security purpose by disabling bit 0x%x of define lang:dialect",
...
#if 0
void
sparqr_free (spar_query_t * sparqr)
{
  ...
  if (NULL != sparp->sparp_env->spare_base_uri)
    {
      ...
      path_utf8_tmp = xml_uri_resolve (qi, &err, sparp->sparp_env->spare_base_uri, ref, "UTF-8");
...
sparp_exec_Narg (sparp_t * sparp, const char *pl_call_text, query_t ** cached_qr_ptr, caddr_t * err_ret, int argctr, ccaddr_t arg1)
{
  ...
  err = qr_rec_exec (cached_qr_ptr[0], cli, &lc, CALLER_LOCAL, NULL, 1, ":0", box_copy_tree (arg1), QRP_RAW);
  if (SQL_SUCCESS != err)
    {
      LC_FREE (lc);
      if (err_ret)
...
sparp_iri_to_id_nosignal (sparp_t * sparp, ccaddr_t qname)
{
  ...
  dk_free_tree (err);
  spar_error (sparp, "Unable to get IRI_ID for IRI <%.200s>: %.10s %.600s", qname, ERR_STATE (t_err), ERR_MESSAGE (t_err));
...
sparp_id_to_iri (sparp_t * sparp, iri_id_t iid)
{
  ...
  t_err = t_full_box_copy_tree (err);
  dk_free_tree (err);
  spar_error (sparp, "unknown IRI_ID %Lu: %.10s %.600s", iid, ERR_STATE (t_err), ERR_MESSAGE (t_err));
  return NULL;			/* to keep compiler happy */
}

caddr_t
spar_strliteral (sparp_t * sparp, const char *strg, int strg_is_long, int is_json)
{
  caddr_t tmp_buf;
  caddr_t res;
  const char *err_msg;
...
spar_mkid (sparp_t * sparp, const char *prefix)
{
  return t_box_sprintf (0x100, "%s_%ld_%ld",
      prefix,
      (long) ((NULL != sparp->sparp_curr_lexem) ? sparp->sparp_curr_lexem->sparl_lineno : 0), (long) (sparp->sparp_unictr++));
}

void
spar_change_sign (caddr_t * lit_ptr)
{
  switch (DV_TYPE_OF (lit_ptr[0]))
    ...
    case DV_DOUBLE_FLOAT:
      ...
      break;
    default:
      GPF_T1 ("spar_change_sign(): bad box type");
    }
}
...
sparp_define (sparp_t * sparp, caddr_t param, ptrlong value_lexem_type, caddr_t value)
{
  ...
  if ((7 < strlen (param)) && !memcmp (param, "output:", 7))
	...
	{
	  sparp->sparp_sg->sg_output_valmode_name = t_box_dv_uname_string (value);
	  return;
	}
      if (!strcmp (param, "output:format"))
	{
	  sparp->sparp_sg->sg_output_format_name = t_box_dv_uname_string (value);
	  return;
	}
      if (!strcmp (param, "output:scalar-format"))
	{
	  sparp->sparp_sg->sg_output_scalar_format_name = t_box_dv_uname_string (value);
	  return;
	}
      if (!strcmp (param, "output:dict-format"))
	{
	  sparp->sparp_sg->sg_output_dict_format_name = t_box_dv_uname_string (value);
	  return;
	}
      if (!strcmp (param, "output:route"))
	{
	  sparp->sparp_sg->sg_output_route_name = t_box_dv_uname_string (value);
	  return;
	}
      if (!strcmp (param, "output:maxrows"))
	{
	  sparp->sparp_sg->sg_output_maxrows = t_box_num_nonull (unbox (value));
	  return;
	}
    }
  if ((6 < strlen (param)) && !memcmp (param, "input:", 6))
      ...
      if (!strcmp (param, "input:default-graph-uri"))
	{
	  sparp_make_and_push_new_graph_source (sparp, SPART_GRAPH_FROM,
	      spartlist (sparp, 2, SPAR_QNAME, t_box_dv_uname_string (value)), NULL, SPARP_SSRC_FROZEN_BY_PROTOCOL);
      ...
      if (!strcmp (param, "input:default-graph-exclude"))
	{
	  sparp_make_and_push_new_graph_source (sparp, SPART_GRAPH_NOT_FROM,
	      spartlist (sparp, 2, SPAR_QNAME, t_box_dv_uname_string (value)), NULL, 0);
      ...
      if (!strcmp (param, "input:storage"))
	{
	  ...
	  if ((sparp->sparp_storage != old_storage)
	    ...
	    spar_error (sparp,
		"Can't change quad storage via 'define %.30s' in subqueries inside QUAD MAP group patterns other than 'QUAD MAP * {...}'",
		param);
	  return;
	}
      ...
      if ((11 < strlen (param)) && !memcmp (param, "input:grab-", 11))
	{
	  ...
	  if (sparp->sparp_env->spare_src.
	      ssrc_freeze_status & (SPARP_SSRC_FROZEN_BY_PROTOCOL | SPARP_SSRC_FROZEN_BY_USING | SPARP_SSRC_FROZEN_EXPLICITLY))
	    {
	  ...
	  if (!strcmp (param, "input:grab-group-destination"))
	    {
	      rgc->rgc_group_destination = t_box_dv_uname_string (value);
	      return;
	    }
	...
	{
	  sparp->sparp_sg->sg_input_param_valmode_name = t_box_dv_uname_string (value);
	  return;
	}
    }
  ...
  if ((4 < strlen (param)) && !memcmp (param, "sql:", 4))
      ...
      if (!strcmp (param, "sql:assert-user"))
	{
	  ...
	  if (strcmp (value, u->usr_name))
	    spar_error (sparp, "Assertion \"define sql:assert-user '%.200s'\" failed: the user is set to '%.200s'", value,
		u->usr_name);
	  return;
	}
...
sparp_configure_storage_and_macro_libs (sparp_t * sparp)
{
	  ...
	  if (0 == smlib->smlCompilationState)
	    {
	      ...
	      if (NULL == sml_rtti)
		spar_error (sparp, "Quad storage <%.100s> refers to inexisting or corrupted SPARQL macro library",
		...
		spar_error (sparp, "Compilation error in SPARQL macro library <%.100s> refered by quad storage <%.100s>",
		    sml_rtti->jrtti_inst_iri, sparp->sparp_env->spare_storage_name);
	    }
	  smllist = (SPART **) (smlib->smlList);
  ...
  if ((NULL != sparp->sparp_macro_libs) && (NULL == sparp->sparp_macrolib_to_create))
	...
	jso_get_cd_and_rtti (uname_virtrdf_ns_uri_QuadStorage, lib_name, &sml_cd, &sml_rtti, 1);
	...
	DO_BOX_FAST (SPART *, new_defm, new_defm_ctr, smllist)
	{
	  ...
	  for (old_defm_ctr = old_defm_count; old_defm_ctr--; /* no step */ )
		...
		spar_error (sparp, "Macro <%.100s> is defined in both <%100s> and <%100s>", old_defm->_.defmacro.mname,
		    old_defm->_.defmacro.sml_iri, new_defm->_.defmacro.sml_iri);
...
void
sparp_compile_smllist (sparp_t * outer_sparp, caddr_t sml_iri_uname,
    void /* actually struct sparql_macro_library_t */ *smlib_as_voidptr)
{
  ...
  sparp =
      sparp_query_parse (t_box_sprintf (50 + box_length (smlib->smlSourceText), "define input:disable-storage-macro-lib \"yes\" %s",
	  smlib->smlSourceText), &sparqre, 0);
...
int
spar_triple_matches_macro_quad_pattern (sparp_t * sparp, SPART * defm, SPART * graph, SPART * subject, SPART * predicate,
    SPART * object)
{
  SPART *tg, *ts, *tp, *to;
...
sparp_make_defmacro_paramnames_from_template (sparp_t * sparp, SPART * defm)
{
  ...
  DO_BOX_FAST (SPART *, fld, ctr, defm->_.defmacro.quad_pattern)
  {
	...
	if (0 <= dk_set_position_of_string (sparp->sparp_env->spare_protocol_params, vname))
	  spar_error (sparp,
	      "The name of protocol parameter ?%.200s is (mis)used in the template of the declaration of macro <%.200s>", vname,
	      defm->_.defmacro.mname);
	if (0 <= dk_set_position_of_string (paramnames, vname))
	  spar_error (sparp, "The name of valiable ?%.200s is used twice in the template of the declaration of macro <%.200s>",
	      vname, defm->_.defmacro.mname);
...
sparp_check_dm_arg_for_redecl (sparp_t * sparp, dk_set_t recent, caddr_t dm_arg_vname)
{
  ...
  if (0 <= dk_set_position_of_string (sparp->sparp_env->spare_protocol_params, dm_arg_vname))
    spar_error (sparp, "The name of protocol parameter ?%.200s is (mis)used in the declaration of macro <%.200s>", dm_arg_vname,
	mdef->_.defmacro.mname);
...
spar_gp_replace_selid (sparp_t * sparp, dk_set_t membs, caddr_t old_selid, caddr_t new_selid)
{
  DO_SET (SPART *, memb, &membs)
  {
	...
	if ((SPAR_VARIABLE != SPART_TYPE (fld)) && (SPAR_BLANK_NODE_LABEL != SPART_TYPE (fld)))
	  continue;
...
spar_gp_finalize (sparp_t * sparp, SPART ** options)
{
  ...
  sparp->sparp_sg->sg_invalidated_bnode_labels = dk_set_conc (
      (dk_set_t) t_set_pop (&(sparp->sparp_sg->sg_bnode_label_sets)), sparp->sparp_sg->sg_invalidated_bnode_labels);
...
/* Create triple patterns for distinct '+>' propvars and OPTIONAL triple patterns for distinct '*>' propvars */
	...
	spar_gp_add_triplelike (sparp, NULL,
	    ...
	    pv->sparpv_verb_qname, spar_make_variable (sparp, pv->sparpv_obj_var_name), NULL, NULL, 0x0);
	if (_STAR_GT == pv->sparpv_op)
	  {
	    SPART *pv_gp;
...
/* Plain composing of SPAR_GP tree node */
      ...
      t_revlist_to_array (dk_set_conc (local_filts, movable_filts)), NULL, orig_selid, NULL, (ptrlong) (0), (ptrlong) (0), options);
  return res;
}
...
void
spar_gp_add_member (sparp_t * sparp, SPART * memb)
{
  dk_set_t *set_ptr;
...
spar_filter_is_freetext_or_rtree (sparp_t * sparp, SPART * filt, SPART * base_triple)
{
  ...
  if (SPAR_FT_TYPE_IS_GEO (fname))
	...
	spar_error (sparp, "Not enough arguments for spatial function %s", fname);
	  ...
	  && ((uname_bif_c_spatial_intersects == (fname))
	      || (uname_bif_c_st_intersects == (fname)) || (uname_bif_c_st_may_intersect == (fname))))
	{
	  SPART *swap = args[0];
	  args[0] = args[1];
	  args[1] = swap;
	  ...
	  || !sparp_tree_is_global_expn (sparp, args[1]) || ((2 < argcount) && !sparp_tree_is_global_expn (sparp, args[1])))
	return NULL;
    }
  else if ((fname == uname_bif_c_contains)
      || (fname == uname_bif_c_xcontains) || (fname == uname_bif_c_xpath_contains) || (fname == uname_bif_c_xquery_contains))
    {
      if (2 > argcount)
  ...
  if (NULL != base_triple)
	...
	return NULL;
      ...
      if ((SPAR_VARIABLE != SPART_TYPE (args[0])) || strcmp (args[0]->_.var.vname, ft_var_name))
	return NULL;
    }
  return fname;
}
...
spar_tree_is_var_with_forbidden_ft_name (sparp_t * sparp, SPART * tree, int report_error)
{
  ...
  if (strcasecmp (vname, "SCORE") && strcasecmp (vname, "XCONTAINS_MAIN_RANGES") && strcasecmp (vname, "XCONTAINS_ATTR_RANGES"))
    return 0;
  if (report_error)
    spar_error (sparp,
	"Free-text triple pattern uses variable name '%s'; this name has special meaning in free-text SQL; please rename it to avoid unexpected effects",
	vname);
  return 1;
}
...
spar_gp_add_filter (sparp_t * sparp, SPART * filt, int filt_is_movable)
{
  ...
  if (ft_type)
	...
	if (memb->_.triple.ft_type)
	  spar_error (sparp,
	      "%s() predicate for '$%s' variable found with another %s() predicate found before for same variable in same basic group pattern",
	      ft_pred_name, var_name, memb->_.triple.ft_type);
	if (NULL == triple_with_var_obj)
	  triple_with_var_obj = memb;
      }
      END_DO_SET ()if (NULL == triple_with_var_obj)
	spar_error (sparp, "The group does not contain triple pattern with '$%s' object before %s() predicate", var_name,
	    ft_pred_name);
      ...
      if (!ft_is_geo)
	{
	  ...
	  if (2 < argcount)
		...
		triple_with_var_obj->_.triple.options = t_spartlist_concat (triple_with_var_obj->_.triple.options,
...
spar_gp_add_filters_for_graph (sparp_t * sparp, SPART * graph_expn, int graph_is_named, int suppress_filters_for_good_names)
{
  ...
  if ((NULL != sources) && (NULL == sources->next))
      ...
      if (!(src->_.graph.subtype & SPART_GRAPH_GROUP_BIT))
	{
	  SPART *graph_expn_copy = (
		  ...
		  spar_make_blank_node (sparp, varname, 0)) : (SPART *) t_box_copy ((caddr_t) graph_expn));
	  filter = spartlist (sparp, 3,
	      ((SPART_GRAPH_MIN_NEGATION < src->_.graph.subtype) ? BOP_NEQ : BOP_EQ), graph_expn_copy, src->_.graph.expn);
  ...
  if (graph_is_named)
    ...
    {
      good_list_expn =
	  spar_make_list_of_sources_expn (sparp, SPART_GRAPH_FROM, SPART_GRAPH_GROUP, 0, RDF_GRAPH_PERM_READ, graph_expn);
...
SPART *
spar_make_list_of_sources_expn (sparp_t * sparp, ptrlong from_subtype, ptrlong from_group_subtype, ptrlong from2_subtype,
    ptrlong req_perms, SPART * needle_expn)
{
  ...
  END_DO_SET ()DO_SET (SPART *, g, &(sparp->sparp_env->spare_src.ssrc_named_graphs))
  {
    ptrlong st = g->_.graph.subtype;
    if ((st == from_subtype) || (st == from2_subtype))
      t_set_push (&single_graphs, g->_.graph.expn);
  ...
  END_DO_SET ()if ((NULL == graph_groups) && (NULL == single_graphs))
    {
      if (NULL == needle_expn)
  ...
  if (NULL != graph_groups)
    {
	  ...
	  (SPART *) (graph_groups->data) : spar_make_funcall (sparp, 0, "LONG::bif:vector",
	      (SPART **) t_list_to_array (graph_groups)));
      SPART *singles_expn = ((NULL == single_graphs) ? (SPART *) t_NEW_DB_NULL : spar_make_funcall (sparp, 0, "LONG::bif:vector",
	...
	return lst_expn;
#if 0				/* For version 5 */
      return spar_make_funcall (sparp, 0, "LONG::bif:position", (SPART **) t_list (2, needle_expn, lst_expn));
#else
      return sparp_make_builtin_call (sparp, SPAR_BIF__ITEM_IN_VECTOR, (SPART **) t_list (2, needle_expn, lst_expn));
#endif
    }
  if (NULL == needle_expn)
    return spar_make_funcall (sparp, 0, "LONG::bif:vector", (SPART **) t_list_to_array (single_graphs));
  t_set_push (&single_graphs, needle_expn);
  return sparp_make_builtin_call (sparp, IN_L, (SPART **) t_list_to_array (single_graphs));
}
...
spar_add_propvariable (sparp_t * sparp, SPART * lvar, int opcode, SPART * verb_qname, int verb_lexem_type, caddr_t verb_lexem_text)
{
  ...
  caddr_t *spec_pred_names = jso_triple_get_objs ((caddr_t *) (sparp->sparp_sparqre->sparqre_qi),
  ...
  DO_SET (spar_propvariable_t *, prev, &(sparp->sparp_propvars))
  {
    if (strcmp (prev->sparpv_key, new_key))
      continue;
    if ((prev->sparpv_verb_lexem_type != verb_lexem_type) || strcmp (prev->sparpv_verb_lexem_text, verb_lexem_text))
      spar_error (sparp,
	  "Variables ?%.200s %s %s?%.200s%s and ?%.200s %s %s?%.200s%s are written different ways but may mean the same; remove this ambiguity",
	  lvar->_.var.vname, optext, ((Q_IRI_REF == verb_lexem_type) ? "<" : ""), verb_lexem_text,
	  ((Q_IRI_REF == verb_lexem_type) ? ">" : ""), prev->sparpv_subj_var->_.var.vname, optext,
  ...
  if (NULL == curr_pv)
      ...
      if (lvar_blen + strlen (verb_lexem_text) + ((Q_IRI_REF == verb_lexem_type) ? 2 : 0) < 30)
	{
	  curr_pv->sparpv_obj_var_name = t_box_sprintf (40, "%s%s%s%s%s",
	      ...
	      ((Q_IRI_REF == verb_lexem_type) ? "<" : ""), verb_lexem_text, ((Q_IRI_REF == verb_lexem_type) ? ">" : ""));
      ...
      else
	{
	  ...
	  if (strlen (buf) + strlen (verb_lexem_text) + ((Q_IRI_REF == verb_lexem_type) ? 2 : 0) < 30)
	    ...
	    curr_pv->sparpv_obj_var_name = t_box_sprintf (40, "%s%s!%d", buf, optext, (sparp->sparp_unictr)++);
...
void
spar_list_triple_varnames_and_macropunames_in_tree (sparp_t * sparp, SPART * tree, dk_set_t * all_varnames_ret,
    dk_set_t * undiscovered_ret)
{
  int ctr;
  switch (SPART_TYPE (tree))
    {
    case SPAR_ALIAS:
      t_set_push_new_string (all_varnames_ret, tree->_.alias.aname);
      break;
    case SPAR_VARIABLE:
      t_set_push_new_string (all_varnames_ret, tree->_.var.vname);
      break;
    case SPAR_TRIPLE:
      for (ctr = SPART_TRIPLE_FIELDS_COUNT; ctr--; /* no step */ )
	{
	  ...
	  switch (SPART_TYPE (fld))
	    ...
	    case SPAR_VARIABLE:
	      ...
	      break;
	    case SPAR_MACROPU:
	      t_set_push_new_string (all_varnames_ret, fld->_.macropu.pname);
	      break;
	    }
	}
      break;
	  ...
	  if (_STAR == (ptrlong) optvalue)
	    {
		  ...
		  NULL != gp_subtypes_iter; gp_subtypes_iter = gp_subtypes_iter->next, acc_triples_iter = acc_triples_iter->next)
		{
		  ...
		  DO_SET (SPART *, member, &members_before)
		  {
		    ...
		    if (NULL != undiscovered)
		      spar_error (sparp,
			  "SERVICE invocation is used in combination with macro so it should have explicit list of IN variables in options");
		  }
		END_DO_SET ()}
	    }
	  else
...
spar_sinv_naming (sparp_t * sparp, SPART * sinv)
{
  switch (SPART_TYPE (sinv->_.sinv.endpoint))
    {
    case SPAR_VARIABLE:
      return t_box_sprintf (300, "SERVICE ?%.200s at line %ld", sinv->_.sinv.endpoint->_.var.vname,
...
spar_describe_restricted_by_physical (sparp_t * sparp, SPART * req_top, SPART ** retvals)
{
  ...
  triple = spar_make_plain_triple (sparp,
      ...
      NULL,
      spar_make_fake_blank_node (sparp), spar_make_fake_blank_node (sparp), (SPART **) t_list (2, NULL, (ptrlong) (_STAR)), NULL);
...
spar_retvals_of_describe (sparp_t * sparp, SPART * req_top, SPART ** retvals, SPART * limit_expn, SPART * offset_expn)
{
  ...
  int need_limofs_trick = (
      (NULL != limit_expn) || (DV_LONG_INT != DV_TYPE_OF (offset_expn)) || (0 != unbox ((caddr_t) (offset_expn))));
/* Making lists of variables, blank nodes, fixed triples, triples with variables and blank nodes. */
  ...
  var_vector_expn = spar_make_funcall (sparp, 0, "LONG::bif:vector", (SPART **) t_list_to_array (vars));
  ...
  agg_call = spar_make_funcall (sparp, 1, "sql:SPARQL_DESC_AGG", (SPART **) t_list (1, var_vector_arg));
  if (NULL != sparp->sparp_env->spare_describe_mode)
      ...
      else
	{
	  phys_only = ((NULL == sparp->sparp_env->spare_inference_name)
	      && (NULL == sparp->sparp_env->spare_use_same_as) && spar_describe_restricted_by_physical (sparp, req_top, retvals));
	  if (phys_only)
  ...
  if (sparp->sparp_env->spare_src.ssrc_default_graphs_listed || sparp->sparp_env->spare_src.ssrc_named_graphs_listed)
    good_graphs =
	spar_make_list_of_sources_expn (sparp, SPART_GRAPH_FROM, SPART_GRAPH_GROUP, SPART_GRAPH_NAMED, RDF_GRAPH_PERM_READ, NULL);
  else
    good_graphs = (SPART *) t_box_num_nonull (0);
  bad_graphs =
      spar_make_list_of_sources_expn (sparp, SPART_GRAPH_NOT_FROM, SPART_GRAPH_NOT_GROUP, SPART_GRAPH_NOT_NAMED, 0x0, NULL);
  ...
  descr_call = spar_make_funcall (sparp, 0, descr_name,
	  ...
	  spar_make_funcall (sparp, 0, "LONG::bif:vector",
	      (SPART **) t_list_to_array (consts)), good_graphs, bad_graphs, storage_name_or_null, opts_arg));
  if (NULL != postproc_name)
    descr_call = spar_make_funcall (sparp, 0, postproc_name,
	(SPART **) t_list (5, descr_call, good_graphs, bad_graphs, storage_name_or_null, opts_arg));
  if (need_limofs_trick)
    return (SPART **) t_list (2, descr_call,
	spartlist (sparp, 6, SPAR_ALIAS, var_vector_expn, t_box_dv_short_string ("describe-1"), SSG_VALMODE_AUTO, (ptrlong) 0,
	    (ptrlong) 0));
  else
...
spar_add_rgc_vars_and_consts_from_retvals (sparp_t * sparp, SPART ** retvals)
{
  ...
  DO_BOX_FAST (SPART *, retval, retctr, retvals)
  {
	...
	NULL, NULL,
	sparp_gp_trav_add_rgc_vars_and_consts_from_retvals, NULL, NULL, sparp_gp_trav_add_rgc_vars_and_consts_from_retvals);
  }
  END_DO_BOX_FAST;
}
...
sparp_gp_trav_rename_for_having (sparp_t * sparp, SPART * curr, sparp_trav_state_t * sts_this, void *common_env)
{
  ...
  switch (curr->type)
    ...
    case SPAR_VARIABLE:
    case SPAR_BLANK_NODE_LABEL:
      selid_to_patch_ptr = &(curr->_.var.selid);
      break;
    case SPAR_GP:
      if ((WHERE_L != curr->_.gp.subtype) && (SELECT_L != curr->_.gp.subtype))
	return SPAR_GPT_NODOWN;
      selid_to_patch_ptr = &(curr->_.gp.selid);
      break;
    case SPAR_TRIPLE:
      selid_to_patch_ptr = &(curr->_.triple.selid);
      break;
    default:
      return 0;
    }
...
spar_make_top_or_special_case_from_wm (sparp_t * sparp, ptrlong subtype, SPART ** retvals, SPART * wm)
{
  ...
  if ((NULL == sparp->sparp_sg->sg_output_format_name) && (NULL == sparp->sparp_env->spare_parent_env) && ssg_is_odbc_cli ())
    {
  ...
  do
    {				/* Special case for selection of all distinct graphs */
      SPART *grp, *triple, *retvar, *gvar;
      caddr_t retname, gname;
      int ctr1, ctr2;
      if (NULL != sparp->sparp_env->spare_parent_env)
	break;
      if (DISTINCT_L != subtype)
	break;
      if (NULL != sparp->sparp_env->spare_src.ssrc_named_graphs)
	break;
      if (!IS_BOX_POINTER (retvals))
	break;
      if (1 != BOX_ELEMENTS (retvals))
	break;
      if (0 != BOX_ELEMENTS_0 (groupings))
	break;
      if (0 != BOX_ELEMENTS_0 (order))
	break;
      if (NULL != binv)
	break;
      if (0 != BOX_ELEMENTS_0 (pattern->_.gp.filters))
	break;
      if (1 != BOX_ELEMENTS (pattern->_.gp.members))
	break;
      if (0 != BOX_ELEMENTS_0 (pattern->_.gp.options))
	break;
      grp = pattern->_.gp.members[0];
      if (0 != grp->_.gp.subtype)
	break;
      if (0 != BOX_ELEMENTS_0 (grp->_.gp.filters))
	break;
      if (1 != BOX_ELEMENTS (grp->_.gp.members))
	break;
      if (0 != BOX_ELEMENTS_0 (grp->_.gp.options))
	break;
      triple = grp->_.gp.members[0];
      if (SPAR_TRIPLE != triple->type)
	break;
      if (0 != BOX_ELEMENTS_0 (triple->_.triple.options))
	break;
      ...
      for (ctr1 = 0; ctr1 < SPART_TRIPLE_FIELDS_COUNT; ctr1++)
	{
      ...
      if (ctr1 < SPART_TRIPLE_FIELDS_COUNT)
	break;
      if (SPAR_VARIABLE != SPART_TYPE (triple->_.triple.tr_predicate))
	break;
      if (SPAR_VARIABLE != SPART_TYPE (triple->_.triple.tr_subject))
	break;
      ...
      else
	{
      ...
      if (SPAR_VARIABLE != SPART_TYPE (retvar))
	break;
      gname = gvar->_.var.vname;
      if (strcmp (gname, retvar->_.var.vname))
	break;
      return spartlist (sparp, 8, SPAR_CODEGEN, t_box_num ((ptrlong) (ssg_select_known_graphs_codegen)), sparp->sparp_sg->sg_output_valmode_name,	/* #2 */
...
spar_make_bindings_inv_with_fake_equivs (sparp_t * sparp, SPART ** vars, SPART *** data_rows, SPART * wrapper_gp)
{
  ...
  SPART *binv =
      spartlist (sparp, 8, SPAR_BINDINGS_INV, 0, vars, data_rows, data_rows_mask, counters_of_unbound, (ptrlong) rowcount,
      (ptrlong) 0);
...
spar_make_sources_like_top (sparp_t * sparp, ptrlong top_subtype)
{
  ...
  END_DO_SET ()DO_SET (SPART *, g, &(env->spare_src.ssrc_named_graphs))
  {
    t_set_push (&src, sparp_tree_full_copy (sparp, g, NULL));
  }
  END_DO_SET ()sources = (SPART **) t_revlist_to_array (src);
#if 0
  if ((0 == BOX_ELEMENTS (sources)) && (NULL != (env->spare_src.ssrc_common_sponge_options)) && (LOAD_L != top_subtype))
    spar_error (sparp,
	"Retrieval options for source graphs (e.g., '%s') may be useless if the query does not contain 'FROM' or 'FROM NAMED'",
	env->spare_src.ssrc_common_sponge_options->data);
#endif
  return sources;
}
    ...
    SPART * pattern, SPART ** groupings, SPART * having, SPART ** order, SPART * limit, SPART * offset, SPART * binv)
{
  ...
  if ((NULL != env->spare_parent_env) || (sparp->sparp_env->spare_disable_output_formatting))
	...
	final_output_format_name =
	    ((NULL != sg->sg_output_dict_format_name) ? sg->sg_output_dict_format_name : sg->sg_output_format_name);
	break;
      case ASK_L:
	final_output_format_name =
	    ((NULL != sg->sg_output_scalar_format_name) ? sg->sg_output_scalar_format_name : sg->sg_output_format_name);
	break;
      default:
  ...
  return spartlist (sparp, 17, SPAR_REQ_TOP, subtype,
      ...
      final_output_format_name,
      t_box_copy (env->spare_storage_name), retvals, /* NULL, orig_retvals */ NULL /* expanded_orig_retvals */ , orig_selid,
      sources, pattern, groupings, having, order, limit, offset, binv, t_box_num ((ptrlong) (env)));
}

SPART *
spar_gp_add_union_of_triple_and_inverses (sparp_t * sparp, SPART * graph, SPART * subject, SPART * predicate, SPART * object,
    SPART ** qm_iri_or_pair, SPART ** options, int banned_tricks, dk_set_t inv_props)
{
  ...
  if (NULL != options)
    spar_error (sparp,
	"Options are not supported for a triple pattern with property name \"%.200s\" that is declared in inference rules \"%.200s\" as a property with inverse",
	predicate->_.qname.val, sparp->sparp_env->spare_inference_name);
  sparp->sparp_env->spare_inference_ctx = NULL;
  triple =
      spar_gp_add_triplelike (sparp, graph, subject, predicate, object, qm_iri_or_pair, options,
      banned_tricks | SPAR_TRIPLE_TRICK_INV_UNION);
  if (SPAR_TRIPLE != SPART_TYPE (triple))
    spar_error (sparp,
	"Property name \"%.200s\" has special meaning that conflicts with its declaration in inference rules \"%.200s\" as a property with inverse",
	predicate->_.qname.val, sparp->sparp_env->spare_inference_name);
  if (NULL != options)
  ...
  while (NULL != inv_props)
    {
      ...
      spar_gp_init (sparp, 0);
      triple = spar_gp_add_triplelike (sparp, (SPART *) t_full_box_copy_tree ((caddr_t) graph), (SPART *) t_full_box_copy_tree ((caddr_t) object),	/* object is swapped with subject */
	  ...
	  (SPART **) t_full_box_copy_tree ((caddr_t) options), banned_tricks | SPAR_TRIPLE_TRICK_INV_UNION);
      if (SPAR_TRIPLE != SPART_TYPE (triple))
	spar_error (sparp,
	    "Property name \"%.200s\" has special meaning but it is declared in inference rules \"%.200s\" as an inverse property of \"%.200s\"",
...
#else
    filt =
	spartlist (sparp, 3, SPAR_BOP_EQNAMES, spar_make_variable (sparp, fld_vname), spar_make_variable (sparp,
	    orig_fld->_.var.vname));
#endif
  spar_gp_add_filter (sparp, filt, 0);
}

SPART *
spar_gp_add_transitive_triple (sparp_t * sparp, SPART * graph, SPART * subject, SPART * predicate, SPART * object,
    SPART ** qm_iri_or_pair, SPART ** options, int banned_tricks)
{
#ifdef DEBUG
  ...
  for (fld_ctr = 0; fld_ctr < 4; fld_ctr++)
	...
	{
	case SPAR_BLANK_NODE_LABEL:
	  rval = spar_make_blank_node (sparp, fields[fld_ctr]->_.var.vname, 0);
	  break;
	case SPAR_VARIABLE:
	  rval = spar_make_variable (sparp, fields[fld_ctr]->_.var.vname);
	  break;
	}
      if (NULL != rval)
  ...
  spar_gp_add_triplelike (sparp, graph, subj_var, predicate, obj_var, qm_iri_or_pair, NULL,
      banned_tricks | SPAR_TRIPLE_TRICK_TRANSITIVE);
  if (NULL != options)
    {
      int idx;
      for (idx = BOX_ELEMENTS_0 (options) - 2; idx >= 0; idx -= 2)
	{
	  ...
	  switch (key)
		...
		if (subj_is_plain_var && !strcmp (subject->_.var.vname, v->_.var.vname))
		  ...
		  spar_error (sparp,
		      "T_IN and T_OUT options of a triple pattern should contain only subject/object variables of the triple pattern, not something else");
		continue;
	      }
	    case T_STEP_L:
	      {
  ...
  if ('\0' != t_in_preset_fld)
	...
	spar_error (sparp,
	    "T_IN and T_OUT options of a transitive triple points to same field, but one should point to subject and another one to the object");
    }
  else
    {
      sparp_set_option (sparp, &options, T_IN_L,
	  spartlist (sparp, 2, SPAR_LIST, t_list (1, spar_make_variable (sparp, subj_vname))), SPARP_SET_OPTION_REPLACING);
      sparp_set_option (sparp, &options, T_OUT_L,
	  spartlist (sparp, 2, SPAR_LIST, t_list (1, spar_make_variable (sparp, obj_vname))), SPARP_SET_OPTION_REPLACING);
    }
...
SPART *
spar_gp_add_ppath_leaf (sparp_t * sparp, SPART ** parts, int part_count, int pp_makes_union, int pp_subtype, SPART * graph,
    SPART * subject, SPART * object, SPART ** qm_iri_or_pair, int banned_tricks)
{
  SPART *res;
  ...
  if ((0 == pp_subtype) && (1 == part_count))
  ...
  else
    {
      ...
      SPART *filt;
      res =
	  spar_gp_add_triplelike (sparp, graph, subject, spar_make_variable (sparp, pred_vname), object, qm_iri_or_pair, NULL,
	  banned_tricks);
      if (1 == part_count)
    ...
    int banned_tricks)
{
  ...
  switch (pp->_.ppath.subtype)
	...
	if (fwd_count)
	  spar_gp_add_ppath_leaf (sparp, parts, fwd_count, pp_makes_union, pp->_.ppath.subtype, graph, subject, object,
	      qm_iri_or_pair, banned_tricks);
	if (rev_count)
	  spar_gp_add_ppath_leaf (sparp, parts + fwd_count, rev_count, pp_makes_union, pp->_.ppath.subtype, graph,
	      object /* yes, before subject, this makes the reverse */ , subject, qm_iri_or_pair, banned_tricks);
	if (wrapper_gp_needed)
	  {
	    res = spar_gp_finalize (sparp, NULL);
	    spar_gp_add_member (sparp, res);
	...
	for (part_ctr = 0; part_ctr < part_count; part_ctr++)
	  {
	    ...
	    res =
		spar_gp_add_ppath_triples (sparp, graph, subj_connection, parts[part_ctr], obj_connection, qm_iri_or_pair,
		banned_tricks);
	...
	t_set_push (&opts, (caddr_t) ((ptrlong) T_DISTINCT_L));
	t_set_push (&opts, (caddr_t) ((ptrlong) 1));
	t_set_push (&opts, (caddr_t) ((ptrlong) TRANSITIVE_L));
	if (wrapper_gp_needed)
	  spar_gp_init (sparp, 0);
	res =
	    spar_gp_add_transitive_triple (sparp, graph, subject, part0, object, qm_iri_or_pair, (SPART **) t_list_to_array (opts),
	    banned_tricks);
	if (wrapper_gp_needed)
	  {
	    res = spar_gp_finalize (sparp, NULL);
	  ...
	  if (!SPAR_IS_LIT_OR_QNAME (single_dflt->_.graph.expn))	/* FROM iriref OPTION (...) case */
	    {
	      ...
	      graph = spar_make_blank_node (sparp, spar_mkid (sparp, "_::default"), 2);
	      ret_graph_eq_from_option_expn[0] =
		  spartlist (sparp, 3, BOP_EQ, sparp_tree_full_copy (sparp, graph, NULL), sparp_tree_full_copy (sparp,
		      single_dflt->_.graph.expn, NULL));
	      if (NULL != iri_arg)
		{
...
SPART *
spar_gp_add_triplelike (sparp_t * sparp, SPART * graph, SPART * subject, SPART * predicate, SPART * object, SPART ** qm_iri_or_pair,
    SPART ** options, int banned_tricks)
{
  sparp_env_t *env = sparp->sparp_env;
  ...
  if (NULL != options)
	...
	return spar_gp_add_transitive_triple (sparp, graph, subject, predicate, object, qm_iri_or_pair, options,
	    SPAR_TRIPLE_TRICK_TRANSITIVE | banned_tricks);
    }
  if (SPAR_PPATH == SPART_TYPE (predicate))
    {
	...
	return spar_gp_add_ppath_triples (sparp, graph, subject, predicate, object, qm_iri_or_pair,
	    banned_tricks | SPAR_TRIPLE_TRICK_TRANSITIVE | SPAR_TRIPLE_TRICK_INV_UNION | SPAR_TRIPLE_TRICK_MACRO);
    }
  if ((NULL != inf_ctx) && (SPAR_QNAME == SPART_TYPE (predicate)))
      ...
      if (SPAR_TRIPLE_TRICK_TRANSITIVE & tricks)
	{
	  ...
	  sparp_set_option (sparp, &options, T_DISTINCT_L, (SPART *) ((ptrlong) 1), SPARP_SET_OPTION_REPLACING);
	  return spar_gp_add_transitive_triple (sparp, graph, subject, predicate, object, qm_iri_or_pair, options,
	      SPAR_TRIPLE_TRICK_TRANSITIVE | banned_tricks);
	}
      if (SPAR_TRIPLE_TRICK_INV_UNION & tricks)
	return spar_gp_add_union_of_triple_and_inverses (sparp, graph, subject, predicate, object, qm_iri_or_pair, options,
	    SPAR_TRIPLE_TRICK_INV_UNION | banned_tricks, inv_names);
    }
  if (SPAR_QNAME == SPART_TYPE (predicate))
      ...
      if (0 != BOX_ELEMENTS (spec_pred_names))
	{
	  ...
	  if (is_geo)
		...
		spar_error (sparp,
		    "Spatial options can be used only with bif:contains and bif:spatial_contains special predicates, not with %.200s",
		    pname);
	    }
	  spar_gp_add_filter (sparp,
	      spar_make_funcall (sparp, 0, pname, t_spartlist_concat ((SPART **) t_list (2, subject, object), options)), 0);
...
SPART *
spar_make_plain_triple (sparp_t * sparp, SPART * graph, SPART * subject, SPART * predicate, SPART * object, SPART ** qm_iri_or_pair,
    SPART ** options)
{
  sparp_env_t *env = sparp->sparp_env;
  int fctr;
  ...
  triple = spartlist (sparp, 17, SPAR_TRIPLE,
      (ptrlong) 0, graph, subject, predicate, object, qm_iri_or_pair, NULL /*selid */ , NULL /*tabid */ , NULL,
      NULL, NULL, NULL, NULL, options, (ptrlong) 0, (ptrlong) ((sparp->sparp_unictr)++));
  if (CONSTRUCT_L == SPARP_ENV_CONTEXT_GP_SUBTYPE (sparp))
    return triple;
  for (fctr = 0; fctr < SPART_TRIPLE_FIELDS_COUNT; fctr++)
      ...
      if ((env->spare_src.ssrc_grab.rgc_all) && (SPART_TRIPLE_PREDICATE_IDX != fctr))
	{
	  ...
	  (SPART_TRIPLE_SUBJECT_IDX == fctr) && (SPAR_VARIABLE == ft) && !(env->spare_src.ssrc_grab.rgc_all))
	{
	  SPART *obj = triple->_.triple.tr_object;
	  ptrlong objt = SPART_TYPE (obj);
	  if (((SPAR_VARIABLE == objt) &&
		  (0 <= dk_set_position_of_string (env->spare_src.ssrc_grab.rgc_vars, obj->_.var.vname))) ||
	      ((SPAR_QNAME == objt) && (0 <= dk_set_position_of_string (env->spare_src.ssrc_grab.rgc_consts, obj->_.lit.val))))
	    {
...
spar_make_ppath (sparp_t * sparp, char subtype, SPART * part1, SPART * part2, ptrlong mincount, ptrlong maxcount)
{
  ...
  switch (subtype)
    {
    case '^':
      return spar_inverse_ppath (sparp, part1);
    case '!':
      ...
      if (subtype == part2subt)
	{
      ...
      return spartlist (sparp, 6, SPAR_PPATH, (ptrlong) subtype, (SPART **) t_list (2, part1, part2), (ptrlong) 0, (ptrlong) 0,
	  (ptrlong) 0);
    case 'D':
      if (('|' == part1subt) || ('D' == part1subt))
	{
      ...
      return spartlist (sparp, 6, SPAR_PPATH, (ptrlong) subtype, (SPART **) t_list (1, part1), (ptrlong) 0, (ptrlong) 0,
	  (ptrlong) 0);
    case '*':
      if (('*' == part1subt) && (2 > maxcount) && (2 > unbox (part1->_.ppath.maxrepeat)) && ((2 > mincount)
	      || (2 > unbox (part1->_.ppath.minrepeat))))
	{
	  if (0 == mincount)
	    part1->_.ppath.minrepeat = t_box_num (0);
	  if (-1 == maxcount)
	    part1->_.ppath.maxrepeat = t_box_num (-1);
	  return part1;
	}
      return spartlist (sparp, 6, SPAR_PPATH, (ptrlong) subtype, (SPART **) t_list (1, part1), t_box_num (mincount),
	  t_box_num (maxcount), (ptrlong) 0);
    }
...
spar_make_variable (sparp_t * sparp, caddr_t name)
{
  ...
  res = spartlist (sparp, 7 + (sizeof (rdf_val_range_t) / sizeof (caddr_t)),
      SPAR_VARIABLE, name,
      selid, NULL, (ptrlong) (SPART_VAR_OUTSIDE_TRIPLE), SPART_BAD_EQUIV_IDX, SPART_RVR_LIST_OF_NULLS, (ptrlong) (0x0));
...
SPART *
spar_make_blank_node (sparp_t * sparp, caddr_t name, int bracketed)
{
  SPART *res;
...
SPART *
spar_make_fake_blank_node (sparp_t * sparp)
{
  SPART *res;
  res = spartlist (sparp, 8 + (sizeof (rdf_val_range_t) / sizeof (caddr_t)),
      SPAR_BLANK_NODE_LABEL, uname__ref,
      uname__ref, NULL, (ptrlong) (0), SPART_BAD_EQUIV_IDX, SPART_RVR_LIST_OF_NULLS, (ptrlong) (0x0), (ptrlong) 0x2);
  res->_.var.rvr.rvrRestrictions = /*SPART_VARR_IS_REF | SPART_VARR_IS_BLANK | */ SPART_VARR_NOT_NULL;
  return res;
}

SPART *
spar_make_typed_literal (sparp_t * sparp, caddr_t strg, caddr_t type, caddr_t lang)
{
  dtp_t tgt_dtp;
  ...
  if (!strncmp (type, XMLSCHEMA_NS_URI "#", XMLSCHEMA_NS_URI_LEN + 1 /* +1 is for '#' */ ))
      ...
      if (DV_STRING != strg_dtp)
	{
	  if (desc->p_dest_dtp == strg_dtp)
	    ...
	    {
	      tgt_dtp_tree =
		  (sql_tree_tmp *) t_list (3, (ptrlong) (desc->p_dest_dtp), (ptrlong) NUMERIC_MAX_PRECISION,
		  (ptrlong) NUMERIC_MAX_SCALE);
...
sparp_make_and_push_new_graph_source (sparp_t * sparp, ptrlong subtype, SPART * iri_expn, SPART ** options, int freeze_ignore_mask)
{
  ...
  if ((subtype < SPART_GRAPH_MIN_NEGATION) && (is_frozen & ~freeze_ignore_mask))
    {
      ...
      if (NULL == dupe_found)
	spar_error (sparp,
	    "FROM %s <%.200s> clause (or its equivalent) violates restrictions on allowed graph names set by protocol parameters or define pragmas",
	    fty, ((NULL == iri) ? "..." : iri));
      else if (signal_error_even_if_dupe)
	spar_error (sparp,
	    "FROM %s <%.200s> clause (or its equivalent) conflicts with protocol parameters or define pragmas and thus violates SPARQL 1.1 Protocol spec",
	    fty, ((NULL == iri) ? "..." : iri));
    }
  if ((SPART_GRAPH_MIN_NEGATION < subtype) && (NULL != options))
    {
      const char *fty = ((SPART_GRAPH_NAMED == subtype) ? " NAMED" : "");
      spar_error (sparp, "NOT FROM%s <%.200s> clause can not have options, only FROM and FROM NAMED can", fty,
	  ((NULL == iri) ? "..." : iri));
    }
  ...
  if (NULL != group_members_ptr)
	...
	{
	case SPART_GRAPH_FROM:
	  subtype = SPART_GRAPH_GROUP;
	  break;
	case SPART_GRAPH_NOT_FROM:
	  subtype = SPART_GRAPH_NOT_GROUP;
	  break;
	case SPART_GRAPH_NAMED:
  ...
  if ((NULL != dupe_found) && (dupe_found->_.graph.subtype == subtype) &&
      (SPAR_QNAME == SPART_TYPE (dupe_found->_.graph.expn)) && (SPAR_QNAME == SPART_TYPE (precode->_.graph.expn)))
    return;
...
sparp_make_graph_precode (sparp_t * sparp, ptrlong subtype, SPART * iriref, SPART ** options)
{
  ...
  mixed_tail = mixed_options =
      (SPART **) t_alloc_box ((common_count + 2 + BOX_ELEMENTS_0 (options)) * sizeof (SPART *), DV_ARRAY_OF_POINTER);
  DO_SET (SPART *, val, opts_ptr)
  {
    (mixed_tail++)[0] = (SPART *) t_full_box_copy_tree ((caddr_t) (val));
  }
  END_DO_SET ()for (ctr = BOX_ELEMENTS_0 (options) - 2; 0 <= ctr; ctr -= 2)
    {
  ...
  return spartlist (sparp, 5, SPAR_GRAPH, subtype, iriref->_.qname.val,
	  ...
	  (SPART **) t_list (1,
		  ...
		  (SPART **) t_list (3,
		      ...
		      ((NULL != exec_user) ? exec_user->usr_id : U_ID_NOBODY))))), (ptrlong) use_expn_in_gs_checks);
...
spar_default_sparul_target (sparp_t * sparp, const char *reason_to_use, int preliminary_call)
{
  ...
  if ((NULL != dflt_graphs->next) && (((SPART *) (dflt_graphs->next->data))->_.graph.subtype < SPART_GRAPH_MIN_NEGATION))
    spar_error (sparp, "More than one default graph specified in the preamble; single default graph is needed for %.200s",
	reason_to_use);
  if (SPART_GRAPH_GROUP == ((SPART *) (dflt_graphs->data))->_.graph.subtype)
    spar_error (sparp,
	"The IRI in preamble refers to default graph group, not to a single default graph; the default graph is needed for %.200s",
	reason_to_use);
...
/*#endif*/
  return sparp_make_builtin_call (sparp, LIKE_L,
      (SPART **) t_list (2, strg, spartlist (sparp, 5, SPAR_LIT, like_tmpl, NULL, NULL, NULL)));

bad_regex:
  return sparp_make_builtin_call (sparp, SPAR_BIF_REGEX, (SPART **) t_list (2, strg, regexpn));
}

/* Note dirty hack in sqlpfn.c, sqlp_new_table_dotted(): the function expectes that unsafe names begins with "RO_", "RDF_" or "SYS_".
Add more prefixes there if needed */
const char *spar_unsafe_table_names[] = {
  "RO_START",
  "RDF_DATATYPE",
...
int spar_unsafe_table_name_count = sizeof (spar_unsafe_table_names) / sizeof (spar_unsafe_table_names[0]);
...
int
sparp_table_name_is_unsafe (const char *buf)
{
  return (ECM_MEM_NOT_FOUND != ecm_find_name (buf, spar_unsafe_table_names, spar_unsafe_table_name_count, sizeof (caddr_t)));
}
...
spar_verify_funcall_security (sparp_t * sparp, int *is_agg_ret, const char **fname_ptr, SPART ** args)
{
  ...
  if (need_check_for_infection_chars)
    for (c = tail + 4; '\0' != c[0]; c++)
      if (strchr ("\'\"\\()., +-/*|\t\n\r", c[0]))
	spar_error (sparp,
	    "Function name \"%.200s\" contains invalid characters; this may be an attempt of bypassing security restrictions",
	    fname);
  return;
restricted:
...
SPART *
spar_run_pure_bif_in_sandbox (sparp_t * sparp, const char *funname, SPART ** args, int argcount, bif_metadata_t * bmd,
    int *trouble_ret)
{
  int argctr;
  caddr_t *sqlargs;
  ...
  if (NULL == err)
  ...
  else
    {
      ...
      return spartlist (sparp, 5, SPAR_FUNCALL, t_box_dv_uname_string ("bif:signal"), t_list (2,
	      ...
	      spar_make_literal_from_sql_box (sparp, ERR_MESSAGE (err), SPAR_ML_SAFEST)), (ptrlong) 0, (ptrlong) 0);
    }
  trouble_ret[0] = 2;
...
spar_make_funcall (sparp_t * sparp, int aggregate_mode, const char *funname, SPART ** args)
{
  ...
  if (sparp->sparp_in_precode_expn)
    spar_error (sparp,
	"Aggregate function %.100s() is not allowed in 'precode' expressions that should be calculated before the result-set of the query",
	funname);
      ...
      if (metas->xpfm_main_arg_no < argcount)
	{
	  if (0 == metas->xpfm_tail_arg_no)
		...
		spar_error (sparp, "The XPATH function %.200s() can handle %d, %d, %d etc. arguments but the call provides %d",
		    funname, (int) (metas->xpfm_main_arg_no), (int) (metas->xpfm_main_arg_no + metas->xpfm_tail_arg_no),
		    (int) (metas->xpfm_main_arg_no + 2 * metas->xpfm_tail_arg_no), argcount);
	    }
	}
...
/*  sbd_name		| sbd_subtype			, impl	| sbd_required_syntax	| min-/maxargs	| ret_valmode		| sbd_arg_valmodes					| sbd_result_restr_bits		*/
  ...
  {"abs", SPAR_BIF_ABS, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"assume", ASSUME_L, '-', SSG_SD_VIRTSPECIFIC, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_BOOL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"bnode", SPAR_BIF_BNODE, '-', SSG_SD_SPARQL11_DRAFT, 0, 1, SSG_VALMODE_LONG, {SSG_VALMODE_SQLVAL, NULL, NULL},
  ...
  {"in operator", IN_L, '-', 0, 1, 0xFFF, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"in-vector operator", SPAR_BIF__ITEM_IN_VECTOR, '-', SSG_SD_BI, 2, 2, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, SSG_VALMODE_SQLVAL,
	      NULL}, SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"iri", IRI_L, '-', SSG_SD_BI_OR_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_LONG, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_IRI | SPART_VARR_IS_REF | SPART_VARR_NOT_NULL},
  {"isblank", SPAR_BIF_ISBLANK, '-', 0, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"isiri", SPAR_BIF_ISIRI, '-', 0, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"isliteral", SPAR_BIF_ISLITERAL, '-', 0, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"isnumeric", SPAR_BIF_ISNUMERIC, '-', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"isref", SPAR_BIF_ISREF, '-', SSG_SD_BI, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"isuri", SPAR_BIF_ISURI, '-', 0, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"lang", LANG_L, '-', 0, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_LONG, NULL, NULL}, SPART_VARR_IS_LIT},
  {"langmatches", SPAR_BIF_LANGMATCHES, '-', 0, 2, 2, SSG_VALMODE_BOOL, {SSG_VALMODE_LONG, SSG_VALMODE_SQLVAL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"like operator", LIKE_L, '-', 0, 2, 2, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, SSG_VALMODE_SQLVAL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"lcase", SPAR_BIF_LCASE, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_LONG, {SSG_VALMODE_LONG, NULL, NULL}, SPART_VARR_IS_LIT},
  {"md5", SPAR_BIF_MD5, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"minutes", SPAR_BIF_MINUTES, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"month", SPAR_BIF_MONTH, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"now", SPAR_BIF_NOW, 'B', SSG_SD_SPARQL11_DRAFT, 0, 0, SSG_VALMODE_NUM, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"rand", SPAR_BIF_RAND, 'B', SSG_SD_SPARQL11_DRAFT, 0, 1, SSG_VALMODE_NUM, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"regex", SPAR_BIF_REGEX, 'B', 0, 2, 3, SSG_VALMODE_BOOL, {SSG_VALMODE_SQLVAL, SSG_VALMODE_SQLVAL, SSG_VALMODE_SQLVAL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  ...
  {"seconds", SPAR_BIF_SECONDS, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"sha1", SPAR_BIF_SHA1, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"sha224", SPAR_BIF_SHA224, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"sha256", SPAR_BIF_SHA256, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"sha384", SPAR_BIF_SHA384, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"sha512", SPAR_BIF_SHA512, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"str", SPAR_BIF_STR, '-', 0, 1, 1, SSG_VALMODE_SQLVAL, {SSG_VALMODE_SQLVAL, NULL, NULL}, SPART_VARR_IS_LIT},
  {"strafter", SPAR_BIF_STRAFTER, 'B', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_SQLVAL, {SSG_VALMODE_LONG, SSG_VALMODE_LONG, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"strbefore", SPAR_BIF_STRBEFORE, 'B', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_SQLVAL, {SSG_VALMODE_LONG, SSG_VALMODE_LONG,
	      NULL}, SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  {"strdt", SPAR_BIF_STRDT, 'S', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_LONG, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT},
  {"strends", SPAR_BIF_STRENDS, 'B', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_BOOL, {SSG_VALMODE_LONG, SSG_VALMODE_LONG, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"strlang", SPAR_BIF_STRLANG, 'S', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_LONG, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT},
  {"strlen", SPAR_BIF_STRLEN, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL},
  {"strstarts", SPAR_BIF_STRSTARTS, 'B', SSG_SD_SPARQL11_DRAFT, 2, 2, SSG_VALMODE_BOOL, {SSG_VALMODE_LONG, SSG_VALMODE_LONG, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"struuid", SPAR_BIF_STRUUID, 'B', SSG_SD_SPARQL11_DRAFT, 0, 0, SSG_VALMODE_SQLVAL, {SSG_VALMODE_LONG, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL},
  ...
  {"uri", SPAR_BIF_URI, '-', SSG_SD_BI_OR_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_LONG, {SSG_VALMODE_SQLVAL, NULL, NULL},
      SPART_VARR_IS_IRI | SPART_VARR_IS_REF},
  {"uuid", SPAR_BIF_UUID, 'S', SSG_SD_SPARQL11_DRAFT, 0, 0, SSG_VALMODE_LONG, {SSG_VALMODE_LONG, NULL, NULL},
      SPART_VARR_IS_IRI | SPART_VARR_NOT_NULL},
  {"valid", SPAR_BIF_VALID, 'B', SSG_SD_VOS_6, 1, 1, SSG_VALMODE_BOOL, {SSG_VALMODE_LONG, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_IS_BOOL},
  {"year", SPAR_BIF_YEAR, 'B', SSG_SD_SPARQL11_DRAFT, 1, 1, SSG_VALMODE_NUM, {SSG_VALMODE_NUM, NULL, NULL},
      SPART_VARR_IS_LIT | SPART_VARR_NOT_NULL | SPART_VARR_LONG_EQ_SQL}
};
...
sparp_make_builtin_call (sparp_t * sparp, ptrlong bif_id, SPART ** arguments)
{
  ...
  for (ofs = sizeof (sparp_bif_descs) / sizeof (sparp_bif_descs[0]); ofs--; /* no step */ )
	...
	sparyyerror_impl (sparp, NULL, t_box_sprintf (100, "Insufficient number of arguments of a standard built-in function %s()",
		sbd->sbd_name));
      if (argcount > sbd->sbd_maxargs)
	sparyyerror_impl (sparp, NULL, t_box_sprintf (100, "Too many arguments of a standard built-in function %s()",
		sbd->sbd_name));
...
spar_make_create_macro_lib (sparp_t * sparp)
{
  ...
  top = spar_make_top_or_special_case_from_wm (sparp, SPAR_SML_CREATE, (SPART **) t_list (1, call), fake_sol);
  return top;
}
...
spar_make_drop_macro_lib (sparp_t * sparp, SPART * sml_precode, int silent)
{
  ...
  top = spar_make_top_or_special_case_from_wm (sparp, SPAR_SML_DROP, (SPART **) t_list (1, call), fake_sol);
  return top;
}
...
spar_make_sparul_mdw (sparp_t * sparp, ptrlong subtype, const char *opname, SPART * graph_precode, SPART * aux_op, int silent)
{
  ...
  if (LOAD_L == subtype)
	...
	spar_error (sparp,
	    "DEFINE get:destination ... is not applicable for SPARUL LOAD statement, use LOAD ... INTO ... form instead");
  ...
  if (NULL != sparp->sparp_sg->sg_output_route_name)
	...
	(SPART **) t_list (13, graph_precode,
	    ...
	    ((sparp->sparp_env->spare_disable_output_formatting
		    || (NULL == sparp->sparp_sg->sg_output_format_name)) ? t_NEW_DB_NULL : sparp->sparp_sg->sg_output_format_name),
	    aux_op, t_NEW_DB_NULL, t_NEW_DB_NULL, spar_exec_uid_and_gs_cbk (sparp), log_mode, spar_compose_report_flag (sparp),
	    options_vector_call, (SPART *) t_box_num_nonull (silent)));
  else
  ...
  top = spar_make_top_or_special_case_from_wm (sparp, subtype, (SPART **) t_list (1, call), fake_sol);
  return top;
}

SPART *
spar_make_graph_precode_for_clear (sparp_t * sparp, SPART * graph_precode, const char *opname)
{
  int clear_default = 0;
  int clear_named = 0;
  ...
  switch ((ptrlong) graph_precode)
    ...
    case NAMED_L:
      ...
      break;
    case ALL_L:
      clear_default = clear_named = 1;
      break;
    default:
      return graph_precode;
    }
  if (clear_default)
    {
	...
	if (SPART_GRAPH_FROM != src->_.graph.subtype)
	  spar_error (sparp,
	      "Graph groups, NOT FROM and its equivalents are not supported for SPARQL %.200s DEFAULT and SPARQL %.200s ALL",
	      opname, opname);
  ...
  if (clear_named)
	...
	if (SPART_GRAPH_NAMED != src->_.graph.subtype)
	  spar_error (sparp,
	      "Graph groups, NOT FROM NAMED and its equivalents are not supported for SPARQL %.200s NAMED and SPARQL %.200s ALL",
	      opname, opname);
	t_set_push (&graphs, spar_simplify_graph_to_patch (sparp, src));
      }
      END_DO_SET ();
...
spar_make_sparul_clear (sparp_t * sparp, SPART * graph_precode, int silent)
{
  ...
  return spar_make_sparul_mdw (sparp, CLEAR_L, "CLEAR", graph_arg, (SPART *) t_box_num_nonull (0) /* i.e. not inside sponge */ ,
      silent);
}
...
/* The need for fake action solution means no USING and WHERE clauses, hence the code that would be otherwise called at the very beginning of WHERE clause */
  if (NULL != sparp->sparp_env->spare_need_for_default_sparul_target)
    sparp->sparp_env->spare_found_default_sparul_target =
	spar_default_sparul_target (sparp, sparp->sparp_env->spare_need_for_default_sparul_target, 0);
/* Now the fake gp of subtype WHERE_L */
  spar_gp_init (sparp, WHERE_L);
  fake_gp = spar_gp_finalize (sparp, NULL);
  return spartlist (sparp, 8, SPAR_WHERE_MODIFS, fake_gp, NULL, NULL, NULL, t_box_num (1), t_box_num (0), NULL);
}
...
spar_var_cmp (caddr_t p_data1, caddr_t p_data2)
{
  ...
  int res;
  res = ((v2->_.var.tr_idx > v1->_.var.tr_idx) ? 1 : ((v2->_.var.tr_idx < v1->_.var.tr_idx) ? -1 : 0));
  if (0 != res)
    return res;
...
spar_exec_uid_and_gs_cbk (sparp_t * sparp)
{
  ...
  if (NULL == sparp->sparp_gs_app_callback)
    return (SPART *) uid;
  return spar_make_funcall (sparp, 0, "bif:vector", (SPART **) t_list (3, uid, sparp->sparp_gs_app_callback, appid));
}
...
int
spar_world_and_private_perms_differ (sparp_t * sparp, int req_perms)
{
  caddr_t boxed_uid = spar_boxed_exec_uid (sparp);
  caddr_t *priv_hit, *world_hit;
  int priv_perms, world_perms;
  rwlock_rdlock (rdf_graph_default_private_perms_of_user_dict_htable->ht_rwlock);
  priv_hit = (caddr_t *) id_hash_get (rdf_graph_default_private_perms_of_user_dict_htable, (caddr_t) (&(boxed_uid)));
  priv_perms = (NULL != priv_hit) ? unbox (priv_hit[0]) : RDF_GRAPH_PERM_DEFAULT;
  rwlock_unlock (rdf_graph_default_private_perms_of_user_dict_htable->ht_rwlock);
  rwlock_rdlock (rdf_graph_default_world_perms_of_user_dict_htable->ht_rwlock);
  world_hit = (caddr_t *) id_hash_get (rdf_graph_default_world_perms_of_user_dict_htable, (caddr_t) (&(boxed_uid)));
  world_perms = (NULL != world_hit) ? unbox (world_hit[0]) : RDF_GRAPH_PERM_DEFAULT;
  rwlock_unlock (rdf_graph_default_world_perms_of_user_dict_htable->ht_rwlock);
  if ((world_perms & req_perms) != (priv_perms & req_perms))
    return 1;
  return 0;
}
...
/*!!! maybe TBD: add retrieval of permissions of specific user on specific graph */
  ...
  if (NULL != hit)
    ...
    res = RDF_GRAPH_PERM_DEFAULT;
  rwlock_unlock (dflt_perms_of_user->ht_rwlock);
  if ((NULL != query_with_deps) || (NULL == graph_iri))
    {
      rwlock_rdlock (dflt_other_perms_of_user->ht_rwlock);
      ...
      rwlock_unlock (dflt_other_perms_of_user->ht_rwlock);
      if (NULL == potential_hit)
	{
	  rwlock_rdlock (rdf_graph_public_perms_dict_htable->ht_rwlock);
	  potential_hit =
	      (caddr_t *) id_hash_get (rdf_graph_public_perms_dict_htable,
	      (caddr_t) (graph_is_private ? &boxed_zero_iid : &boxed_8192_iid));
	  potential_res = (NULL != potential_hit) ? unbox (potential_hit[0]) : RDF_GRAPH_PERM_DEFAULT;
  ...
  hit =
      (caddr_t *) id_hash_get (rdf_graph_public_perms_dict_htable,
      (caddr_t) (graph_is_private ? &boxed_8192_iid : &boxed_zero_iid));
  if (NULL != hit)
    res = unbox (hit[0]);
  else
    res = RDF_GRAPH_PERM_DEFAULT;
  ...
  if (!DV_STRINGP (str))
	...
	for (ctr = 0; ctr < buflen; ctr++)
	  {
	    ...
	    dk_set_push (&lexems, list (5,
		    ...
		    curr->sparl_depth, box_copy (curr->sparl_raw_text), curr->sparl_lex_value, curr->sparl_state));
#else
	    dk_set_push (&lexems, list (4,
		    box_num (curr->sparl_lineno), curr->sparl_depth, box_copy (curr->sparl_raw_text), curr->sparl_lex_value));
#endif
	  }
      }
      END_DO_SET ();
      if (NULL != sparp->sparp_sparqre->sparqre_catched_error)
	{
	  dk_set_push (&lexems, list (3,
		  ...
		  sparp->sparp_lexdepth, box_copy (ERR_MESSAGE (sparp->sparp_sparqre->sparqre_catched_error))));
	}
      sparp_free (sparp);
      MP_DONE ();
...
sparp_query_parse (const char *str, spar_query_env_t * sparqre, int rewrite_all)
{
  ...
  sparp->sparp_lang = server_default_lh;
  spare->spare_namespace_prefixes_outer = spare->spare_namespace_prefixes = sparqre->sparqre_external_namespaces;
  ...
  QR_RESET_CTX
  {
    ...
    sparyyparse (sparp);
    if (rewrite_all)
      sparp->sparp_entire_query =
	  sparp_rewrite_all (sparp, sparp->sparp_entire_query, 0 /* top is never cloned, hence zero safely_copy_retvals */ );
  ...
  END_QR_RESET if (NULL != sparp->sparp_macrolib_to_create && !sparp->sparp_macrolib_ignore_create)
...
bif_sparql_detalize (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  END_QR_RESET if (NULL != sparqre.sparqre_catched_error)
    {
      SPARP_RESTORE_MP_SIZE_CAP (THR_TMP_POOL);
      ssg_free_internals (&ssg);
      MP_DONE ();
...
spar_make_literal_from_sql_box (sparp_t * sparp, caddr_t box, int mode)
{
  switch (DV_TYPE_OF (box))
    ...
    case DV_DOUBLE_FLOAT:
      return spartlist (sparp, 5, SPAR_LIT, t_box_copy (box), uname_xmlschema_ns_uri_hash_double, NULL, NULL);
    case DV_UNAME:
	...
	if (!rb->rb_is_complete ||
	    (RDF_BOX_DEFAULT_TYPE != rb->rb_type) || (RDF_BOX_DEFAULT_LANG != rb->rb_lang) || DV_STRING != DV_TYPE_OF (rb->rb_box))
	  spar_internal_error (sparp,
	      "spar_" "make_literal_from_sql_box() does not support rdf boxes other than complete untyped strings, sorry");
...
spar_make_qname_or_literal_from_rvr (sparp_t * sparp, rdf_val_range_t * rvr, int make_naked_box_if_possible)
{
  ...
  if (make_naked_box_if_possible && !(rvr->rvrRestrictions & SPART_VARR_TYPED) && (NULL == rvr->rvrFixedValue))
    return (SPART *) t_box_copy ((caddr_t) (rvr->rvrFixedValue));
  return spartlist (sparp, 5, SPAR_LIT, t_box_copy ((caddr_t) (rvr->rvrFixedValue)), t_box_copy ((caddr_t) (rvr->rvrDatatype)),
      t_box_copy ((caddr_t) (rvr->rvrLanguage)), NULL);
}
...
bif_sparql_quad_maps_for_quad_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int resulting_format, const char *fname)
{
  ...
  switch (BOX_ELEMENTS (args))
    {
    case 8:
      flags = bif_long_arg (qst, args, 7, fname);
    case 7:
      bad_sources_val = bif_strict_2type_array_arg (DV_UNAME, DV_IRI_ID, qst, args, 6, fname);
    case 6:
      good_sources_val = bif_strict_2type_array_arg (DV_UNAME, DV_IRI_ID, qst, args, 5, fname);
    case 5:
      storage_name = bif_string_or_uname_or_wide_or_null_arg (qst, args, 4, fname);
    case 4:
      sqlvals[SPART_TRIPLE_OBJECT_IDX] = bif_arg (qst, args, 3, fname);
    case 3:
      sqlvals[SPART_TRIPLE_PREDICATE_IDX] = bif_arg (qst, args, 2, fname);
    case 2:
      sqlvals[SPART_TRIPLE_SUBJECT_IDX] = bif_arg (qst, args, 1, fname);
    case 1:
      sqlvals[SPART_TRIPLE_GRAPH_IDX] = bif_arg (qst, args, 0, fname);
    case 0:;			/* no break */
    }
  MP_START ();
  memset (&sparqre, 0, sizeof (spar_query_env_t));
  ...
  QR_RESET_CTX
  {
	...
	sources[src_total_count++] = spartlist (&sparp, 5, SPAR_GRAPH, SPART_GRAPH_FROM, expn->_.qname.val, expn, (ptrlong) 0);
    }
    END_DO_BOX_FAST;
    DO_BOX_FAST (caddr_t, itm, src_idx, bad_sources_val)
    {
      SPART *expn = spar_make_literal_from_sql_box (&sparp, itm, SPAR_ML_SAFEST);
      if (SPAR_QNAME == SPART_TYPE (expn))
	sources[src_total_count++] = spartlist (&sparp, 5, SPAR_GRAPH, SPART_GRAPH_NOT_FROM, expn->_.qname.val, expn, (ptrlong) 0);
    }
    END_DO_BOX_FAST;
	      ...
	      for (fld_ctr = SPART_TRIPLE_FIELDS_COUNT; fld_ctr--; /* no step */ )
		{
		  ...
		  for (col_ctr = col_count; col_ctr--; /* no step */ )
		      ...
		      for (alias_ctr = alias_count; alias_ctr--; /* no step */ )
			{
		      ...
		      col_desc =
			  (caddr_t *) list (4, box_copy (table_alias), box_copy (table_name), box_copy (qmc->qmvcColumnName), NULL);
		  ...
		  switch (sqlval_dtp)
			...
			fake_global_long_param = spar_make_variable (&sparp, t_box_dv_uname_string (":LONG::0"));
		      ...
		      tmpl = qmv->qmvFormat->qmfShortOfLongTmpl;
		      break;
		    default:
		      if (NULL == fake_global_sql_param)
			fake_global_sql_param = spar_make_variable (&sparp, t_box_dv_uname_string (":0"));
		  ...
		  map_desc = list (4, box_num (fld_ctr), box_num (qmv->qmvColumnsFormKey), col_descs, expn_text);
  ...
  END_QR_RESET ssg_free_internals (&ssg);
  MP_DONE ();
  return (caddr_t) res;
}
...
static void
spar_lex_props (int val, const char *yname, char fmttype, const char *fmt, ...)
{
  va_list tail;
  const char *cmd;
  dk_set_t tests = NULL;
  ...
  for (tested_lex_val = 0; tested_lex_val < __SPAR_NONPUNCT_END; tested_lex_val++)
      ...
      for (cmd_idx = 0; cmd_idx < BOX_ELEMENTS (ld->ld_tests); cmd_idx++)
	{
	  ...
	  switch (cmd)
	    {
	    case 's':
	      break;		/* Fake, SPARQL has only one mode */
	    case 'K':
	    case 'L':
	    case 'M':
	    case 'E':
		...
		{
		  dk_set_push (&report,
		      box_sprintf (0x1000, "%s: ERROR %s", ('E' == cmd) ? "PASSED" : "FAILED", lexems[lex_count - 1][2]));
		  goto end_of_test;
		}
		...
		{
		  dk_set_push (&report,
		      box_sprintf (0x1000, "FAILED: %d lexems found, last lexem is %d, must be error", lex_count, last_lval));
		  goto end_of_test;
		}
	      if ('K' == cmd)
		{
		  if (4 > lex_count)
		    {
		      dk_set_push (&report,
			  box_sprintf (0x1000, "FAILED: %d lexems found, the number of actual lexems is less than two", lex_count));
		      goto end_of_test;
		    }
