...
sqlc_xj_cond (sql_comp_t * sc, comp_table_t * ct, dk_set_t * join_preds)
{
  ...
  DO_SET (predicate_t *, pred, &sc->sc_preds)
  {
	...
	{
	  if (req_ct->ct_sc == sc && !req_ct->ct_generated && req_ct != ct)
	    goto next_pred;
	}
	END_DO_SET ();
...
sqlc_xj_cols (sql_comp_t * sc, comp_table_t * ct, xv_join_elt_t * xj)
{
  ...
  DO_SET (col_ref_rec_t *, crr, &sc->sc_col_ref_recs)
  {
	...
	t_set_push (&res, t_list (5,
		...
		t_box_copy (crr->crr_dbe_col->col_name), XV_XC_ATTRIBUTE, NULL, NULL));
	continue;
      }
	...
	t_set_push (&res, t_list (5,
		...
		t_box_copy (crr->crr_col_ref->_.col_ref.name), XV_XC_ATTRIBUTE, NULL, NULL));
      }
  }
  END_DO_SET ();
...
{
  int inx;
  xv_join_elt_t *prev = (xv_join_elt_t *) dk_alloc_box (sizeof (xv_join_elt_t), DV_ARRAY_OF_POINTER);
  ...
  DO_BOX (comp_table_t *, ct, inx, sc->sc_tables)
  {
    xv_join_elt_t *xj = (xv_join_elt_t *) dk_alloc_box (sizeof (xv_join_elt_t), DV_ARRAY_OF_POINTER);
...
xv_check_from_expn_of_select_stmt (ST * from_expn, int is_in_left_join, int is_last)
{
  ...
  switch (from_expn->type)
	...
	return srv_make_new_error ("42000", "SX001", "Unable to create XML view: left argument of JOIN may not be JOIN again");
      switch (from_expn->_.join.type)
	{
	case OJ_LEFT:
	  if (!is_last)
	    return srv_make_new_error ("42000", "SX002",
		"Unable to create XML view from SELECT statement: " "LEFT OUTER JOIN should be the last expression of FROM clause");
	  break;
	case J_INNER:
	case J_CROSS:
	  if (2 == is_in_left_join)
	    return srv_make_new_error ("42000", "SX003",
		"Unable to create XML view: right argument of LEFT OUTER JOIN " "may be only a table or a nested LEFT OUTER JOIN");
	  break;
	case OJ_FULL:
	  return srv_make_new_error ("42000", "SX004",
	      "Unable to create XML view: FULL JOIN may not be grouped by left " "table to compose a tree");
	case OJ_RIGHT:
	  return srv_make_new_error ("42000", "SX005",
	      "Unable to create XML view: RIGHT JOIN may not be grouped by left " "table to compose a tree");
	default:
    ...
    case TABLE_DOTTED:
      return NULL;
    default:
      return srv_make_new_error ("42000", "SX007", "Unable to create XML view: unsupported table expression in FROM clause");
    }
}
...
xv_check_select_stmt (ST * sel_stmt)
{
  ...
  if (SEL_IS_DISTINCT (sel_stmt))
    return srv_make_new_error ("42000", "SX008", "Unable to create XML view from SELECT statement: DISTINCT is not supported");
  ...
  if (NULL != sel_stmt->_.select_stmt.table_exp->_.table_exp.order_by)
    return srv_make_new_error ("42000", "SX012", "ORDER BY may not work for XML view from SELECT statement");
...
void
xj_tree_prepare (xv_join_elt_t * xj, xv_join_elt_t * parent, query_instance_t * qi, int all_cols_as_subelements)
{
  int inx, inx2;
  xj->xj_parent = parent;
  ...
  if (xj->xj_table)
      ...
      if (!xj->xj_pk)
	{
	  ...
	  xj->xj_pk = (caddr_t *) dk_alloc_box (pk->key_n_significant * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
...
/*  char * def_name = "xmlns";*/
  ...
  for (nsidx = 0; nsidx < length - 1; nsidx++)
      ...
      for (nsidx1 = nsidx + 1; nsidx1 < length; nsidx1++)
	{
	  ...
	  if (!strcmp (nsdef->xvns_prefix, nsdef1->xvns_prefix))
	    sqlr_new_error ("37000", "SQ167", "The namespace name '%s' is declared more than once in create xml view %s.",
		nsdef->xvns_prefix, xvc->xvc_tree->xv_full_name);
	}
    }
  ...
  DO_BOX (xml_view_nsdef_t *, nsdef, nsidx, namespaces)
  {
    ...
    if (!strcmp (nsdef->xvns_uri, ""))
      sqlr_new_error ("37000", "SQ166", "Empty namespace local part is not valid in create xml view %s.",
	  xvc->xvc_tree->xv_full_name);
	...
	memcpy (ns_name + length, ":\0", 2);
      ...
      sqlr_new_error ("37000", "SQ169", "The first three letters of namespace name '%s' must not be 'xml' in create xml view %s.",
	  nsdef->xvns_prefix, xvc->xvc_tree->xv_full_name);
  }
  END_DO_BOX;
  return ns_name;
...
caddr_t
xv_get_name_with_nsprefix (xv_context_t * xvc, caddr_t name, int is_attr, caddr_t default_namespace)
{
  caddr_t prefix;
  caddr_t postfix;
  caddr_t full_name;
...
void
xmlg_printf (xv_context_t * xvc, const char *string, ...)	/* IvAn/AutoDTD/000919 Added */
{
  int len;
...
in order to relax the check for expression length inside sqlc_exp_print. */
  ...
  CATCH (CATCH_LISP_ERROR)
  {
    ...
    sqlc_exp_print (xvc->xvc_sc, NULL, exp, xvc->xvc_text + xvc->xvc_text_fill, xvc->xvc_text_len - xvc->xvc_text_fill, &fill);
  }
  THROW_CODE
  {
...
void
xmlg_quote_dotted (xv_context_t * xvc, caddr_t name)
{
  int fill = 0;
/* The trick with passing shifted xvc_text and zero fill is made
in order to relax the check for expression length inside sqlc_exp_print. */
  sqlc_quote_dotted (xvc->xvc_text + xvc->xvc_text_fill, xvc->xvc_text_len - xvc->xvc_text_fill, &fill, name);
  xvc->xvc_text_fill += fill;
}
...
xmlg_start_tag_xmlview (xv_context_t * xvc, xv_join_elt_t * xj, int nth, dk_set_t map, int acc_nth, int depth)
{
  ...
  DO_BOX (xj_col_t *, xc, inx, xj->xj_cols)
  {
	...
	nth + 1, nth, inx, nth, inx, nth + 1, nth, inx, col_full_name, nth + 1, nth, inx, col_full_name, nth, inx);
  }
  END_DO_BOX;
...
xmlg_end_tag_xmlview (xv_context_t * xvc, xv_join_elt_t * xj, int nth, int acc_nth, int depth, int simple_subelement)
{
  xmlg_printf (xvc, "\n--(Begin xmlg_end_tag_xmlview %s nth=%u acc_nth=%u depth=%u, simple_subelement=%u)\n", xj->xj_element, nth,
      acc_nth, depth, simple_subelement);
...
xmlg_join_simple_loop (xv_context_t * xvc, xv_join_elt_t * xj, int nth, dk_set_t map,
    int pk_args, int http_out, int depth, int *count_addition, int head_nth, int make_xte, int simple_subelement)
{
  int inx;
...
/*simple subelement value*/
	  xmlg_printf (xvc, "\n xte_nodebld_acc (acc_%u, cast (v%u_%u as varchar)); ", (int) (ptrlong) xj->xj_prefix, nth,
	      (*count_addition)++);
	}
      DO_BOX (xv_join_elt_t *, xc, inx, xj->xj_children)
      {
	if (xc->xj_mp_schema->xj_same_table)
	  xmlg_join_simple_loop (xvc, xc, nth, map, 0, http_out, depth, count_addition, (int) (ptrlong) xj->xj_prefix, make_xte, 0);
	else
	  {
	    inx = 0;
  ...
  if (xj->xj_mp_schema && xj->xj_mp_schema->xj_is_constant)
    {
      ...
      if (make_xte)		/*xquery */
	{
	  xmlg_printf (xvc, "\n  declare acc_%s any;" "  xte_nodebld_init(acc_%s);", xj->xj_element, xj->xj_element);
...
/*end mapping schema*/
  ...
  if (http_out)
    ...
    {
      xmlg_printf (xvc, "    insert into %s (E_ID, E_LEVEL, E_NAME ", target_tb);
      first = 0;
      DO_BOX (xj_col_t *, xc, inx, xj->xj_cols)
      {
	XVC_COMMA (xvc, first);
	xmlg_printf (xvc, " \"%s\" ", xc->xc_xml_name);
      ...
      END_DO_BOX;
      xmlg_printf (xvc, ")\n      values (_id := xml_eid (_id, _bound, 0), _lev + %u, '%s'", nth, xj->xj_element);
...
/*      xmlg_printf (xvc, " http_value (v%u_%u, 0, _out);", nth, 0);*/
	  ...
	  xmlg_printf (xvc, " http_value (v%u_%u, 0, _out);", nth, count_addition++);
	  if (xj->xj_children)
	    sqlr_error ("S0002", "It's not supported subelements of the element %s annotated by sql:limit-field in mapping schema",
		xj->xj_element);
	}
    }
/*end mapping schema*/
...
/*mapping schema. subelement from the same table. Print subelement and its value*/
	if (c->xj_mp_schema && c->xj_mp_schema->xj_same_table)
	  xmlg_join_simple_loop (xvc, c, nth, map, 0, http_out, depth, ref_count_addition, nth + 1, make_xte, simple_subelement);
	else
/* end mapping schema*/
...
xmlg_xj_http_func_attr (xv_context_t * xvc, xj_col_t * xc, int make_xte)
{
  ...
  if (make_xte)			/*xquery */
    {
      xmlg_printf (xvc, "\ncreate procedure \"%s\".\"%s\".\"xte_%s_%s_%s\" (", xvc->xvc_schema, xvc->xvc_user, xvc->xvc_local_name,
	  xj->xj_element, xj->xj_prefix);
    }
  else
    xmlg_printf (xvc, "\ncreate procedure \"%s\".\"%s\".\"http_%s_%s_%s\" (", xvc->xvc_schema, xvc->xvc_user, xvc->xvc_local_name,
	xj->xj_element, xj->xj_prefix);
...
#ifdef XMLVIEW_DEBUG
      xmlg_printf (xvc, "dbg_obj_print(\'%s.%s.%s_%s_%s\');\n", xvc->xvc_schema, xvc->xvc_user, xvc->xvc_local_name, xj->xj_element,
	  xj->xj_prefix);
      for (n = 0; n < (int) BOX_ELEMENTS (xj->xj_pk); n++)
	{
...
xmls_publish (query_instance_t * qi, xml_view_t * xv)
{
  ...
  err = qr_rec_exec (qr, qi->qi_client, NULL, qi, NULL, 7,
      ...
      ":5", (ptrlong) unbox (metas->xmetas_mode), QRP_INT, ":6", metas->xmetas_custom_text, QRP_STR);
  if (err)
    {
...
xmls_proc (query_instance_t * qi, caddr_t name)
{
...

  {
    ...
    if (NULL == xml_view_drop_proc_qr)
      xml_view_drop_proc_qr =
	  sql_compile_static ("DB.DBA.XML_VIEW_DROP_PROCS (concat (?, '.', ?, '.', ?), 1)", bootstrap_cli, &err, SQLC_DEFAULT);
    err =
	qr_rec_exec (xml_view_drop_proc_qr, xvc.xvc_qi->qi_client, NULL, xvc.xvc_qi, NULL, 3, ":0", xvc.xvc_schema, QRP_STR, ":1",
	xvc.xvc_user, QRP_STR, ":2", xvc.xvc_local_name, QRP_STR);
    if (NULL != err)
      sqlr_resignal (err);
  }
  ...
  QR_RESET_CTX
  {
	...
	ERR_MESSAGE (err) = box_sprintf (2000,
	    "Failed to create internal stored procedure '%.200s' of XML view '%.200s': %.1000s", proc_name, name, msg);
	dk_free_box (msg);
...
xv_dtd_debug (xv_join_elt_t * tree, char *res, size_t tlen, int *fill)
{
  ...
  tailprintf (res, tlen, fill,
      ...
      (long) (void *) (tree), tree->xj_table, tree->xj_prefix, tree->xj_element, tree->xj_all_cols_as_subelements);
  DO_BOX (xj_col_t *, column, col_idx, tree->xj_cols)
  {
	...
	(long) (void *) (tree), col_idx, column->xc_xml_name, column->xc_usage);
  }
  END_DO_BOX;
  DO_BOX (xv_join_elt_t *, chld, chld_idx, tree->xj_children)
  {
...
xv_schema_builder_allocate (char *view_name, char *top_el_name)
{
  ...
  res->xs_typenames = id_hash_allocate (61, sizeof (char *), sizeof (int), strhash, strhashcmp);
  return res;
}
...
xv_schema_builder_free (xv_schema_builder_t * bld)
{
  ...
  for (id_hash_iterator (&dict_hit, bld->xs_typenames); hit_next (&dict_hit, (char **) (&dict_key), (char **) (&dict_val));
      /*no step */ )
    {
...
xv_schema_eval_output_len (xv_schema_builder_t * bld, const xv_join_elt_t * curr_join_elt)
{
  ...
  if (NULL != el_name)
    res +=
	(int) (strlen (el_name) * 2 * (1 + BOX_ELEMENTS_0 (curr_join_elt->xj_cols) + BOX_ELEMENTS_0 (curr_join_elt->xj_children)));
...
xv_schema_sprintf_xsdtype (char *col_xml_name, xv_schema_xsdtype_t * xsd, char *res, size_t tlen, int *fill)
{
  ...
  if (xsd_ok)
    ...
    {				/* When in trouble, use xsd:string and add an comment with hard type */
...
static void
xv_schema_emit_el_type (xv_schema_builder_t * bld, const xv_join_elt_t * curr_join_elt, char *type_name, char *res, size_t tlen,
    int *fill)
{
  char *el_name;		/* Name of current join element */
  ...
  if (NULL == curr_join_elt->xj_parent)
    {				/* Top-level element should be reported, if named */
      if (NULL != el_name)
	tailprintf (res, tlen, fill, "\n\n <xsd:element name=\"%s\" type=\"%s\"/>", el_name, type_name);
    }
  ...
  if ((no_of_child_els == 0) && (BOX_ELEMENTS_0 (curr_join_elt->xj_cols) == 0))
    {
      tailprintf (res, tlen, fill, "\n\n <xsd:simpleType name=\"%s\" base=\"%\"/>", type_name, "xsd:string");
      return;
    }
  ...
  DO_BOX (xv_join_elt_t *, chld, chld_idx, curr_join_elt->xj_children)
  {
	...
	tailprintf (res, tlen, fill,
	    "\n  <xsd:element name=\"%s\" type=\"%s\" minOccurs=\"0\" maxOccurs=\"unbounded\"/>", chld->xj_element, "xsd:string");
      }
    else
      {
...
void
xr_open (xr_state_t * state, local_cursor_t * lc, caddr_t * prev, xr_element_t * elt, dk_session_t * stream)
{
...
xr_explicit_row (local_cursor_t * lc, xr_state_t * state, dk_session_t * stream)
{
  ...
  switch (DV_TYPE_OF (tag_box))
    ...
    default:
      sqlr_error ("42000",
	  "Column 1 of the result set of the select statement should be of INTEGER type when FOR XML EXPLICIT clause is used");
  ...
  switch (DV_TYPE_OF (parent_box))
    ...
    default:
      sqlr_error ("42000",
	  "Column 2 of the result set of the select statement should be of INTEGER type when FOR XML EXPLICIT clause is used");
...
xr_result_auto (query_instance_t * qi,
    query_t * qr, caddr_t * params, xr_element_t ** cols, dk_session_t * stream, int mode, int named_params)
{
  xr_state_t state;
  caddr_t err = NULL;
...
xr_auto_meta_data (sql_comp_t * sc, ST * tree)
{
  ...
  DO_BOX (comp_table_t *, ct, inx, elements)
  {
	...
	((NULL == ct->ct_prefix) ? box_dv_short_string (ct->ct_table->tb_name_only) : box_dv_short_string (ct->ct_prefix)));
    col_list = NULL;
    DO_BOX (ST *, exp, expinx, tree->_.select_stmt.selection)
    {
      if (ST_COLUMN (exp, COL_DOTTED))
...
static void
xr_compile (query_instance_t * qi, xr_compilation_res_t * res, caddr_t sql)
{
  caddr_t err = NULL;
...
bif_xml_auto (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  xr_result_auto (qi, xrcr.xrcr_qr, (caddr_t *) box_copy_tree (params), xrcr.xrcr_elements, stream, xrcr.xrcr_mode, 0);
...
xr_xml_row_dtd (xr_compilation_res_t * xrcr, char *toplev, char *res, size_t tlen, int *fill)
{
  ...
  DO_BOX (xr_element_t *, el, el_idx, xrcr->xrcr_elements)
  {
	...
	{
	  tailprintf (res, tlen, fill, "%c%s", (is_subel_before ? ' ' : '('), col->xrc_name);
	  is_subel_before = 1;
	}
    }
    END_DO_BOX;
...
bif_xml_auto_dtd (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  tailprintf (buf, buf_size, &fill, "<!-- dtd for output of the following SQL statement:\n%s\n-->", sql);
  (				/* Apply one of functions... */
      ((xrcr.xrcr_mode & XR_ROW) ? xr_xml_row_dtd : ((xrcr.xrcr_mode & XR_EXPLICIT) ? xr_xml_explicit_dtd : xr_xml_auto_dtd))
...
xr_xml_explicit_schema (xr_compilation_res_t * xrcr, char *toplev, char *res, size_t tlen, int *fill)
{
  ...
  DO_BOX (xr_element_t *, el, el_idx, xrcr->xrcr_elements)
  {
    ...
    tailprintf (res, tlen, fill, "\n <xsd:complexType name=\"%s_Type\" %s>", el->xre_element, len == 1 ? "" : "mixed='true'");
    if (len == 1)
      tailprintf (res, tlen, fill, "\n  <xsd:group ref=\"Mix__Group\" />");
    else
      {
...
/* The idea is every pass through the loop closes previously opened
  ...
  {
    tailprintf (res, tlen, fill, "\n  <xsd:sequence>");
    tailprintf (res, tlen, fill, "\n   <xsd:element name=\"%s\" type=\"%s_Type\" minOccurs=\"0\" maxOccurs=\"unbounded\"/>",
	el->xre_element, el->xre_element);
    tailprintf (res, tlen, fill, "\n  </xsd:sequence>");
...
/*  tailprintf (buf, &fill,	"<!DOCTYPE xsd:schema SYSTEM \"XMLSchema.dtd\" [\n"
  ...
  (				/* Apply one of functions... */
	  ...
	  ((xrcr.xrcr_mode & XR_EXPLICIT) ? xr_xml_explicit_schema : xr_xml_auto_schema))
      /* ... to the following arguments: */
      (&xrcr, toplev, buf, buf_size, &fill));
  tailprintf (buf, buf_size, &fill, "\n</xsd:schema>");
  dk_free_tree ((caddr_t) xrcr.xrcr_elements);
...
xs_find_id (xmlsql_ugram_t * xs, caddr_t table, caddr_t id)
{
  ...
  while (elm)
    {
      if (elm && elm->table_name && elm->sql_id && (0 == strcmp (elm->table_name, table)) && (0 == strcmp (elm->sql_id, id)))
	return elm;
      elm = elm->next;
    }
  return NULL;
...
caddr_t
xs_fill (caddr_t * current, xmlsql_ugram_t ** xs, int where, caddr_t * err_ret, dk_set_t * vars, caddr_t nullvalue)
{
  int inx;
	  ...
	  if (!IN_HDR_TAG (where) && IS_HDR_TAG (scol))
	      ...
	      if (IN_SYNC_TAG (where) && (IN_BEFORE_TAG (where) || IN_AFTER_TAG (where)))
		{
			  ...
			  if (193 == DV_TYPE_OF ((caddr_t *) current[ix]))
			    {
			      ...
			      if (lem > 0 && 193 == DV_TYPE_OF ((((caddr_t *) current[ix])[0]))
		  ...
		  if (place)
			...
			{
			  *err_ret = srv_make_new_error ("42000", "SX013", "Duplicated value of sql:id attribute in before");
			  dk_free_tree ((box_t) cvd);
			  return NULL;
			}
		      else if (!before && (place->after_cvp))
			{
			  *err_ret = srv_make_new_error ("42000", "SX014", "Duplicated value of sql:id attribute in after");
			  dk_free_tree ((box_t) cvd);
			  return NULL;
			}
		      else if (!before && !(place->after_cvp))
  ...
  for (ix = 0; ix < vlen; ix += 2)
      ...
      if (is_string_type (DV_TYPE_OF (var[ix])))
	{
	  for (ix1 = 0; ix1 < plen; ix1 += 2)
	    {
	      if (is_string_type (DV_TYPE_OF (parms[ix1])) && 0 == strcmp (parms[ix1], var[ix]))
		{
    ...
    caddr_t * var, caddr_t * parms, id_hash_t * variables, int debug)
{
  ...
  while (xs)
      ...
      if (table)
	{
	  ...
	  if (!stmt)
	    {
	      *err_ret = srv_make_new_error ("42000", "SX015", "No columns specified in update gram");
	      goto error;
	    }
	  /* Compilation & values */
#ifndef XS_DEBUG
	  ...
	  DO_SET (state_slot_t *, ss, &qr->qr_parms)
	  {
	    ...
	    if (ix >= nparsa + nparsb)
	      {
		*err_ret = srv_make_new_error ("42000", "SX016", "Too many values for query");
		...
		proposed = (sql_tree_tmp *) list (3,
		    box_num (ss->ssl_sqt.sqt_dtp), box_num (ss->ssl_sqt.sqt_precision), box_num (ss->ssl_sqt.sqt_scale));
...
static char *xml_template_ft_str = "SELECT 1 AS TAG, NULL AS PARENT, "
    " RES_FULL_PATH AS [\"resource\"!1!\"name\"], "
    " RES_MOD_TIME AS [\"resource\"!1!\"modified\"], RES_OWNER AS [\"resource\"!1!\"owner\"], "
    " RES_GROUP AS [\"resource\"!1!\"group\"], " " length (RES_CONTENT) as [\"resource\"!1!\"length\"]"
/* 		      " RES_CONTENT AS [\"resource\"!1!\"content\"!\"element\"] " */
    " FROM WS.WS.SYS_DAV_RES WHERE CONTAINS (RES_CONTENT, '%s') FOR XML EXPLICIT";

static char *xml_template_xp_str = "SELECT 1 AS TAG, NULL AS PARENT, "
    " RES_FULL_PATH AS [\"resource\"!1!\"name\"], "
    " RES_MOD_TIME AS [\"resource\"!1!\"modified\"], RES_OWNER AS [\"resource\"!1!\"owner\"], "
    " RES_GROUP AS [\"resource\"!1!\"group\"], " " length (RES_CONTENT) as [\"resource\"!1!\"length\"]"
/*		      " RES_CONTENT AS [\"resource\"!1!\"content\"!\"element\"] " */
    " FROM WS.WS.SYS_DAV_RES WHERE RES_FULL_PATH like '%s%%' AND " " XPATH_CONTAINS (RES_CONTENT, '%s') FOR XML EXPLICIT";
...
xml_template_node_serialize (caddr_t * current, dk_session_t * ses, void *xsst1)
{
  ...
  if (ARRAYP (current) && BOX_ELEMENTS (current) > 1)
	...
	xts->xts_xsl_template = xslt;
      ...
      if (0 == strcmp (name, XMLSQL_NS ":query") || 0 == strcmp (name, XMLSQL_NS ":text") || 0 == strcmp (name, XMLSQL_NS ":xpath"))
	{
	  ...
	  if (sqlx_qr)
	      ...
	      if (flag)
		{
		  ...
		  if (def_graph_uri != NULL)
		    snprintf (sqlx_query, q_len, "SPARQL define input:default-graph-uri \"%s\" %s", def_graph_uri, _text);
		  else
