  ...
  for (;;)
      ...
      if (LC_ROW == rc)
	{
	  ...
	  if (THR_TMP_POOL->mp_bytes > tn->tn_max_memory)
	    {
	      SET_THR_TMP_POOL (NULL);
	      sqlr_new_error ("42000", "TN...",
		  "Exceeded " BOXINT_FMT
		  " bytes in transitive temp memory.  This was detected in expanding a owl:sameAs or IFP implied identity.",
		  (boxint) tn->tn_max_memory);
	    }
	  if (place)
      ...
      if (LC_AT_END == rc)
	{
	  ...
	  for (inx = 0; inx < fill; inx++)
	      ...
	      if (mem_co && mp->mp_bytes > mem_co)
		{
	      ...
	      if (!any[inx] && !id_hash_get (relation, (caddr_t) & arr[inx]))
	  ...
	  if (THR_TMP_POOL->mp_bytes > tn->tn_max_memory)
	    {
	      ...
	      sqlr_new_error ("42000", "TN...",
		  "Exceeded " BOXINT_FMT
		  " bytes in transitive temp memory.  use t_distinct, t_max or more T_MAX_memory options to limit the search or increase the pool",
		  (boxint) tn->tn_max_memory);
	    }
	  if (place)
...
tst_next_states (trans_node_t * tn, caddr_t * inst, trans_set_t * ts, trans_state_t * tst, dk_set_t * new_ret)
{
  ...
  TN_LIMIT_DECL;
  dk_set_t *rel_place;
  if (tn->tn_max_depth && tst->tst_depth >= ts->ts_max_depth)
    return;
  if (mem_co && mp->mp_bytes > mem_co)
    {
      qi->qi_client->cli_activity.da_trans_partial = 1;
  ...
  if (!rel_place)
    GPF_T1 ("should have been fetched");
...
void
tn_result_row (trans_node_t * tn, caddr_t * inst, trans_state_t * tst, trans_state_t * path_end_tst, int res_depth, int path_no,
    int last_of_set)
{
  QNCAST (QI, qi, inst);
  int inx;
...
data_source_t *
sqlg_distinct_same_as_1 (sqlo_t * so, data_source_t ** q_head, ST ** col_sts, df_elt_t * dt_dfe, dk_set_t * pre_code, int is_ifp)
{
  op_table_t *tb_ot;
  dk_set_t col_preds;
  df_elt_t *g_dfe = NULL;
  ...
  DO_BOX (ST *, col, inx, col_sts)
  {
    ...
    if (!is_ifp && !sqlo_opt_value (tb_ot->ot_opts, OPT_SAME_AS) && !sqlg_rdf_inf_same_as_opt (dt_dfe))
      continue;
    if (is_ifp)
      {
	...
	tn->tn_sas_g = gs;
      ...
      tn->tn_sas_g =
	  (state_slot_t **) list (1, g_dfe->_.bin.right->dfe_ssl ? g_dfe->_.bin.right->dfe_ssl : g_dfe->_.bin.left->dfe_ssl);
    tn->tn_lowest_sas = 1;
    tn->tn_distinct = 1;
    tn->tn_commutative = 1;
    tn->tn_is_primary = 1;
...
data_source_t *
sqlg_distinct_same_as (sqlo_t * so, data_source_t ** q_head, ST ** col_sts, df_elt_t * dt_dfe, dk_set_t pre_code)
{
  data_source_t *a1, *a2;
