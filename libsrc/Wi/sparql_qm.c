...
spar_qm_get_local (sparp_t * sparp, int key, int error_if_missing)
{
  ...
  if ((NULL == res) && error_if_missing)
    spar_error (sparp, "%s: The '%s' property is not defined", spar_source_place (sparp, NULL), spart_dump_opname (key, 0));
  return res;
}

void
spar_qm_pop_key (sparp_t * sparp, int key_to_pop)
{
  dk_set_t *locptr = &(sparp->sparp_e4qm->e4qm_locals);
  for (;;)
    {
  ...
  if (NULL != cmd)
    {
      ...
      if (NULL == subst)
	spar_error (sparp,
	    "%s: Unable to use ^{URIQADefaultHost}^ in IRI template if DefaultHost is not specified in [URIQA] section of Virtuoso config",
...
/*                     01234567890123456789012*/
  ...
  if (NULL != cmd)
    {
      spar_error (sparp,
	  "%s: Unable to use ^{DynamicLocalFormat}^ outside format string of IRI class because it is expanded into sprintf format string",
	  spar_source_place (sparp, NULL));
    }
...
sparp_make_qm_sqlcol (sparp_t * sparp, ptrlong type, caddr_t name)
{
  ...
  switch (type)
    ...
    default:
      spar_internal_error (sparp, "sparp_" "make_qm_sqlcol(): Unsupported argument type");
    }
...
spar_make_qm_col_desc (sparp_t * sparp, SPART * col)
{
  return spar_make_vector_qm_sql (sparp,
      (SPART **) t_list (3, col->_.qm_sqlcol.qtable, col->_.qm_sqlcol.alias, col->_.qm_sqlcol.col));
}
...
spar_make_qm_value (sparp_t * sparp, caddr_t format_name, SPART ** cols)
{
  ...
  DO_BOX_FAST (SPART *, col, colctr, cols)
  {
	...
	t_set_push (&map_atables, spar_make_vector_qm_sql (sparp, (SPART **) t_list (2, a, col->_.qm_sqlcol.qtable)));
      }
  }
  END_DO_BOX_FAST;
  ...
  if (NULL != sparp->sparp_e4qm->e4qm_ft_indexes_of_columns)
	...
	END_DO_SET ()}
	  ...
	  (SPART **) t_list (5,
	      ...
	      spar_make_vector_qm_sql (sparp, (SPART **) t_list_to_array (ft_cond_tmpls))), (SPART **) (ft->sparqft_options));
      ft->sparqft_use_ctr++;
    }
end_of_free_text:;
  ...
  return spar_make_vector_qm_sql (sparp,
	  ...
	  spar_make_vector_qm_sql (sparp, col_descs_array),
	  spar_make_vector_qm_sql (sparp, (SPART **) t_list_to_array (cond_tmpls)), ft_generator));
}
...
spar_qm_find_all_conditions (sparp_t * sparp, dk_set_t map_aliases, dk_set_t * cond_tmpls_ptr)
{
  DO_SET (sparp_qm_table_condition_t *, cond, &(sparp->sparp_e4qm->e4qm_where_conditions))
  {
	...
	{			/* If the alias is not found in map directly, let's search for descendants */
	  ...
	  END_DO_SET ()goto cond_is_redundant;	/* see below */
...
spar_qm_add_aliased_table_or_sqlquery (sparp_t * sparp, caddr_t parent_qtable, caddr_t new_alias)
{
  ...
  if (NULL != prev_use)
    spar_error (sparp, "Alias %.100s is in use already for %.500s", new_alias, spar_qm_table_or_sqlquery_report_name (prev_use));
...
void
spar_qm_check_filter_aliases (sparp_t * sparp, dk_set_t used_aliases)
{
	...
	if (pos >= 0)
	  {
	    ...
	    spar_error (sparp,
		"Aliases %.100s and %.100s have common base alias %.100s so they can not be used together in a WHERE condition",
		used_alias, prev_base_use, base);
	  }
      }
  }
END_DO_SET ()}
...
spar_make_topmost_qm_sql (sparp_t * sparp)
{
  ...
  t_set_push (acc_ptr,
	  ...
	  (SPART **) t_list (2,
	      ...
	      spar_make_vector_qm_sql (sparp, (SPART **) t_revlist_to_array (sparp->sparp_e4qm->e4qm_affected_jso_iris))), NULL));
  ...
  DO_BOX_FAST (SPART *, op, ctr, ops)
  {
    ...
    if (NULL == op->_.qm_sql_funcall.named)
      ...
      arlst = (SPART **) t_list (3, op->_.qm_sql_funcall.fname, fixed, spar_make_vector_qm_sql (sparp, op->_.qm_sql_funcall.named));
    ops[ctr] = spar_make_vector_qm_sql (sparp, arlst);
  }
  END_DO_BOX_FAST;
  return spar_make_qm_sql (sparp, "DB.DBA.RDF_QM_CHANGE_OPT", (SPART **) t_list (1, spar_make_vector_qm_sql (sparp, ops)), NULL);

generic_change:
  return spar_make_qm_sql (sparp, "DB.DBA.RDF_QM_CHANGE", (SPART **) t_list (1, spar_make_vector_qm_sql (sparp, ops)), NULL);
}


static void
spar_qm_get_atables_and_aliases (sparp_t * sparp, caddr_t qm_id, caddr_t alias, caddr_t atbl, dk_set_t * map_atables_ret,
    dk_set_t * map_aliases_ret)
{
  caddr_t old_atbl;
  if (NULL == alias)
  ...
  if (strcmp (old_atbl, atbl))
    spar_error (sparp,
	"One alias %.100s is used for %.300s and %.300s in two different quad map values of the quad map pattern <%.300s>", alias,
	spar_qm_table_or_sqlquery_report_name (atbl), spar_qm_table_or_sqlquery_report_name (old_atbl), qm_id);
}
...
spar_qm_make_mapping_impl (sparp_t * sparp, int is_real, caddr_t qm_id, SPART ** options)
{
  ...
  if (NULL != qm_id)
	...
	spar_error (sparp,
	    "The declaration of quad mapping contains two identifiers for the mapping: CREATE <id1> AS ... AS <id2>");
      raw_id = qm_id;
    }
  if (NULL == raw_id)
    {
  ...
  if (is_real)
	...
	spar_qm_get_atables_and_aliases (sparp, qm_id, extra_alias, atbl, &all_atables, &map_aliases);
      ...
      END_DO_SET ()while (all_atables != sub_atables)
	{
	  caddr_t alias = (caddr_t) t_set_pop (&all_atables);
	  caddr_t atbl = (caddr_t) t_set_pop (&all_atables);
	  t_set_push (&final_atables, spar_make_vector_qm_sql (sparp, (SPART **) t_list (2, alias, atbl)));
	}
	...
	END_DO_SET ()if (0 > dk_set_position_of_string (all_cond_tmpls, tmpl))
  ...
  return spar_make_qm_sql (sparp, "DB.DBA.RDF_QM_DEFINE_MAPPING",
	  ...
	  spar_make_vector_qm_sql (sparp, (SPART **) (t_revlist_to_array (final_cond_tmpls)))), options);
}
...
SPART *
spar_qm_make_real_mapping (sparp_t * sparp, caddr_t qm_id, SPART ** options)
{
