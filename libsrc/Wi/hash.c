...
id_hashed_key_t
hi_memcache_hash (caddr_t p_data)
{
...
int
hi_memcache_cmp (caddr_t d1, caddr_t d2)
{
...
hi_alloc_elements (hash_index_t * hi)
{
  ...
  hi->hi_elements = (hash_inx_elt_t **) dk_alloc_box (inx * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
  while (inx--)
    {
  ...
  if (!hi->hi_size)
    GPF_T1 ("0 hi_size is not possible");
  hi->hi_buckets = (dp_addr_t *) dk_alloc_box (he_inx * sizeof (dp_addr_t), DV_CUSTOM);
...
hi_allocate (unsigned int32 sz, int use_memcache, hash_area_t * ha)
{
	  ...
	  if (ha->ha_memcache_only && HI_CHASH != use_memcache)
	    {
	      ...
	      hi->hi_memcache =
		  t_id_hash_allocate (513, sizeof (hi_memcache_key_t), sizeof (caddr_t *), hi_memcache_hash, hi_memcache_cmp);
  ...
  if (!hi->hi_size)
    GPF_T1 ("0 size hi is not possible");
  return hi;
}
...
{
  dk_free_tree ((caddr_t) hi->hi_slice_trees);
  if (hi->hi_thread_cha)
    {
  ...
  if (hi->hi_gb_dist_trees)
    {
      DO_HT (ptrlong, ign, index_tree_t **, dist, hi->hi_gb_dist_trees) dk_free_tree ((caddr_t) dist);
      END_DO_HT;
      hash_table_free (hi->hi_gb_dist_trees);
    }
  if (hi->hi_is_rdf_ctx)
    {
      IN_HIC;
      rdf_ctx_in_use -= hi->hi_chash->cha_pool->mp_bytes;
      LEAVE_HIC;
    }
  if (hi->hi_chash)
    {
...
hi_add (hash_index_t * hi, uint32 code, dp_addr_t dp, int pos)
{
    ...
    }
  {
    ...
    if (!hi->hi_pages || hi->hi_page_fill >= HI_PAGE_SIZE - sizeof (hash_inx_elt_t))
      {
    ...
    set_dbg_fprintf ((stdout, "hi_add:befo bu: page=%lu next=%p pos=%lu, no=%lu before page=%lu next=%p pos=%lu, no=%lu\n",
	    ...
	    (unsigned long) new_he->he_next->he_page, new_he->he_next->he_next, (unsigned long) new_he->he_next->he_pos,
	    (unsigned long) new_he->he_next->he_no));
  }
}
#endif
...
key_hash_box (caddr_t box, dtp_t dtp, uint32 code, int *var_len, collation_t * collation, dtp_t col_dtp, int allow_shorten_any)
{
  ...
  switch (dtp)
	...
	*var_len += len;
    ...
    default:
      sqlr_new_error ("22023", "SR445", "Value type (%s) not suitable for use in a hash index", dv_type_title (dtp));
      len = box_length (box);
      *var_len += len;
    }
  if (collation)
...
hash_cast (query_instance_t * qi, hash_area_t * ha, int inx, state_slot_t * ssl, caddr_t data)
{
  ...
  if (DV_DB_NULL == dtp || IS_BLOB_HANDLE_DTP (dtp) || DV_ANY == target_dtp || sqt->sqt_is_xml)
    return data;
  if ((target_dtp == DV_INT64 && dtp == DV_LONG_INT) || (DV_IRI_ID_8 == target_dtp && DV_IRI_ID == dtp))
    return data;
  ...
  data = box_cast_to (qst, data, dtp, target_dtp, sqt->sqt_precision, sqt->sqt_scale, &err);
  if (err)
    sqlr_resignal (err);
    ...
    )
{
  ...
  if (key_len > 0)
      ...
      for (inx = ha->ha_n_keys; ha->ha_key_cols[inx].cl_col_id; inx++)
	{
	  ...
	  if ((feed_temp_blobs || !IS_INLINEABLE_DTP (cl->cl_sqt.sqt_dtp))
	      && IS_BLOB_DTP (cl->cl_sqt.sqt_dtp) && !cl->cl_sqt.sqt_is_xml && !IS_BLOB_HANDLE_DTP (dtp) && DV_DB_NULL != dtp)
	    dtp = DV_BLOB_HANDLE_DTP_FOR_BLOB_DTP (DV_BLOB_DTP_FOR_INLINE_DTP (dtp));

	  if (dtp != DV_DB_NULL && (cl->cl_sqt.sqt_dtp == DV_ANY || cl->cl_sqt.sqt_dtp == DV_OBJECT))
	    {
	      caddr_t err = NULL;
	  ...
	  if (IS_BLOB_DTP (cl->cl_sqt.sqt_dtp) && !cl->cl_sqt.sqt_is_xml && !IS_BLOB_HANDLE_DTP (dtp) && DV_DB_NULL != dtp)
  ...
  if (!hash_buf || hb_fill + row_len + HASH_HEAD_LEN > PAGE_SZ)
    {
  ...
  set_dbg_fprintf ((stdout, "itc_ha_disk_row: code %lu next of page=%lu/ofs=%lu is at page=%lu/ofs=%lu code=%lu\n",
	  ...
	  (unsigned long) hb_fill, (unsigned long) hibp->hibp_page, (unsigned long) hibp->hibp_pos, hibp->hibp_no));
...
itc_ha_equal (it_cursor_t * itc, hash_area_t * ha, caddr_t * qst, db_buf_t hash_row, int allow_shorten_any)
{
  ...
  for (inx = 0; inx < ha->ha_n_keys; inx++)
	...
	KEY_PRESENT_VAR_COL (ha->ha_key, hash_row, (*h_cl), h_off, h_len);
      h_is_null = h_cl->cl_null_mask[0] && (0 != (h_cl->cl_null_mask[0] & hash_row[h_cl->cl_null_flag[0]]));
      if (ha->ha_slots[inx])
	{
	  state_slot_t *ssl = ha->ha_slots[inx];
	  ...
	  if (h_cl->cl_sqt.sqt_dtp == DV_ANY)
	      ...
	      if (err)
		{
	      ...
	      ret = ((b_len != h_len) || 0 != memcmp (any_val, hash_row + h_off, h_len)) ? 1 : 0;
	  ...
	  if (IS_WIDE_STRING_DTP (v_dtp))
	    {
	      if (DVC_MATCH == compare_wide_to_utf8 ((caddr_t) (hash_row + h_off),
		      h_len, value, box_length (value) - sizeof (wchar_t), ssl->ssl_sqt.sqt_collation))
		continue;
	      return DVC_LESS;
	    }
	  if (DV_NUMERIC == v_dtp)
    ...
    hash_area_t * ha, caddr_t * qst, uint32 code, dp_addr_t he_page, short he_pos)
{
  ...
  do
    {
      if (itc && itc->itc_hash_buf && itc->itc_hash_buf->bd_page == he_page)
	h_buf = itc->itc_hash_buf;
      else
	{
	  if (ref_itc->itc_page != he_page || !ref_itc->itc_buf)
	    {
	      if (ref_itc->itc_buf)
...
itc_ha_flush_memcache (hash_area_t * ha, caddr_t * qst, int is_in_fill)
{
  ...
  index_tree_t *tree =
      SETP_HASH_FILL == is_in_fill ? qst_tree (qst, ha->ha_tree, ha->ha_set_no) : (index_tree_t *) qst_get (qst, ha->ha_tree);
  hash_index_t *hi;
  hi_memcache_key_t *key;
  caddr_t **val;
  id_hash_iterator_t hit;
...
itc_ha_feed (itc_ha_feed_ret_t * ret, hash_area_t * ha, caddr_t * qst, unsigned long feed_temp_blobs, setp_node_t * setp)
{
  ...
  if (!tree)
    {
      ...
      tree->it_hi = hi_allocate ((int) MIN (ha->ha_row_count, (long) HA_MAX_SZ), HA_MEMCACHE (ha), ha);
  ...
  if (hi->hi_memcache)
	  ...
	  if (next_link)
	    {
	      ...
	      caddr_t *new_deps =
		  (caddr_t *) mp_alloc_box (hi->hi_pool, (n_deps + next_link) * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
      ...
      else
	{
      ...
      if ((!ha->ha_memcache_only) && ((long) hi->hi_memcache->ht_count) > hi_end_memcache_size)
	do_flush = 1;
      else
	{
...
/* Now we start disk-based processing */
  ...
  ITC_FAILED END_FAIL (itc);

  if (ha_rehash_pct && hi->hi_count > (hi->hi_size / 100) * ha_rehash_pct && HI_NEXT_SIZE (hi) < (int64) HI_MAX_SIZE)
  ...
  if (ihfr.ihfr_memcached)
	...
	switch (op->go_op)
	  ...
	  case AMMSC_MAX:
		...
		goto next_mem_col;
	      rc = cmp_boxes (new_val, dep_ptr[0], cl->cl_sqt.sqt_collation, cl->cl_sqt.sqt_collation);
	  ...
	  case AMMSC_MAX:
	    {
	      ...
	      if (DV_DB_NULL == DV_TYPE_OF (new_val))
		goto next_disk_col;
	      rc = cmp_boxes (new_val, QST_GET_V (qst, op->go_old_val), cl->cl_sqt.sqt_collation, cl->cl_sqt.sqt_collation);
	      if (DVC_UNKNOWN == rc || (rc == DVC_LESS && op->go_op == AMMSC_MIN) || (rc == DVC_GREATER && op->go_op == AMMSC_MAX))
		{
		  rf.rf_row = ihfr.ihfr_disk_buf->bd_buffer + itc->itc_map_pos;
...
setp_order_row (setp_node_t * setp, caddr_t * qst)
{
  ...
  DO_ALL_CL (cl, ha->ha_key)
  {
    ...
    row_insert_cast_temp (&rd, cl, QST_GET (qst, ssl), &err, NULL);

    if (err)
      sqlr_resignal (err);
    if (nth < key->key_n_significant
	&& rd.rd_non_comp_len - key->key_row_var_start[0] + key->key_key_var_start[0] > MAX_RULING_PART_BYTES)
  ...
  for (inx = 0; inx < len; inx++)
      ...
      if (!dk_set_member (deps, (void *) it))
	{
	  if (deps)
	    ...
	    sethash ((void *) (ptrlong) hsi->hsi_col_ids[inx], hash_index_cache.hic_col_to_it, (void *) dk_set_cons ((void *) it,
		    NULL));
	}
    }
...
it_hi_bpages_set_locks (query_instance_t * qi, index_tree_t * it)
{
  ...
  hi = it->it_hi;
  if (qi->qi_trx->lt_is_excl || (hi->hi_lock_mode == PL_SHARED && hi->hi_isolation < ISO_REPEATABLE))
    return;
...
hash_temp_reclaim_space (hash_area_t * ha, index_tree_t * it)
{
  ...
  while (((long) dbs->dbs_n_free_pages) - ha_n_pages < 0)
    {
      if (((short) (dbs->dbs_n_pages * 100 / wi_inst.wi_master->dbs_n_pages)) < wi_inst.wi_temp_allocation_pct)
	{
...
hash_fill_node_input (fun_ref_node_t * fref, caddr_t * inst, caddr_t * qst)
{
  ...
  if (qi->qi_query && qi->qi_query->qr_select_node && qi->qi_query->qr_lock_mode != PL_EXCLUSIVE)
