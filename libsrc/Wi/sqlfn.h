...
void ts_set_placeholder (table_source_t * ts, caddr_t * state, it_cursor_t * itc, buffer_desc_t ** buf_ret);
...
EXE_EXPORT (caddr_t, qr_exec, (client_connection_t * cli, query_t * qr, query_instance_t * caller, caddr_t cr_name,
	srv_stmt_t * stmt, local_cursor_t ** ret, caddr_t * parms, stmt_options_t * opts, int named_params));
...
caddr_t qr_quick_exec (query_t * qr, client_connection_t * cli, const char *id, local_cursor_t ** cursor, long n_pars, ...);
...
void ddl_create_sub_table (query_instance_t * cli, char *name, caddr_t * supers, caddr_t * cols);

void ddl_create_primary_key (query_instance_t * cli, char *table, char *key,
    caddr_t * parts, int cluster_on_id, int is_object_id, caddr_t * opts, caddr_t * cols);
...
void sinv_builtin_inverse (const char **f1, const char **f2, int *flags, int n);
...
void table_source_input_unique (table_source_t * ts, caddr_t * inst, caddr_t * state);
...
client_connection_reset_hook_type client_connection_set_reset_hook (client_connection_reset_hook_type new_hook);
...
void cli_scrap_cursors (client_connection_t * cli, query_instance_t * exceptions, lock_trx_t * this_trx_only);
...
void upd_insert_2nd_key (dbe_key_t * key, it_cursor_t * ins_itc, row_delta_t * rd);

int itc_from_sort_temp (it_cursor_t * itc, query_instance_t * qi, state_slot_t * tree);
int itc_replace_row (it_cursor_t * main_itc, buffer_desc_t * main_buf, row_delta_t * rd, caddr_t * state, int this_key_only);
...
void log_update (lock_trx_t * lt, row_delta_t * rd, update_node_t * upd, caddr_t * qst);
...
void row_insert_node_input (row_insert_node_t * ins, caddr_t * inst, caddr_t * state);

void key_insert_node_input (key_insert_node_t * ins, caddr_t * inst, caddr_t * state);

void fun_ref_node_input (fun_ref_node_t * fref, caddr_t * inst, caddr_t * state);
...
void qst_set_bin_string (caddr_t * state, state_slot_t * sl, db_buf_t data, size_t len, dtp_t dtp);

void ssl_free_data (state_slot_t * sl, caddr_t data);

void itc_qst_set_column (it_cursor_t * it, buffer_desc_t * buf, dbe_col_loc_t * cl, caddr_t * qst, state_slot_t * target);
...
void itc_delete_this (it_cursor_t * del_itc, buffer_desc_t ** del_buf, int res, int maybe_blobs);
...
void pl_source_input (pl_source_t * pls, caddr_t * inst, caddr_t * state);
void pl_source_free (pl_source_t * pls);

int err_is_state (caddr_t err, const char *state);
...
void ks_get_cols (key_source_t * ks, caddr_t * state, it_cursor_t * it, buffer_desc_t * buf);
long bh_get_data_from_http_user (blob_handle_t * bh, client_connection_t * cli, db_buf_t to, int max_bytes);
long bh_get_data_from_http_user_no_err (blob_handle_t * bh, client_connection_t * cli, db_buf_t to, int max_bytes);

int itc_get_alt_key (it_cursor_t * del_itc, buffer_desc_t ** alt_buf_ret, dbe_key_t * alt_key, row_delta_t * rd);
...
void trig_wrapper (caddr_t * qst, state_slot_t ** args, dbe_table_t * tb, int event, data_source_t * qn, qn_input_fn qn_run);
void trig_call (query_t * qr, caddr_t * qst, state_slot_t ** args, dbe_table_t * calling_tb, data_source_t * qn);
...
void sf_sql_extended_fetch (caddr_t stmt_id, long type, long irow, long n_rows, long is_autocommit, caddr_t bookmark);
	...
	DO_SET (state_slot_t *, parm, &qr->qr_parms) \
	  { \
	    if (!IS_SSL_REF_PARAMETER (parm->ssl_type)) \
	      dk_free_tree ((caddr_t)((params)[pinx])); \
	    pinx ++; \
	  } \
	END_DO_SET (); \
      } \
...
caddr_t row_set_col_cast (caddr_t data, sql_type_t * tsqt, caddr_t * err_ret, oid_t col_id, dbe_key_t * key, caddr_t * qst);
...
caddr_t stmt_set_query (srv_stmt_t * stmt, client_connection_t * cli, caddr_t text, stmt_options_t * opts);
query_t *cli_cached_sql_compile (caddr_t query_text, client_connection_t * cli, caddr_t * err_ret, const char *stmt_id_name);
...
/* hash.c */
uint32 key_hash_box (caddr_t box, dtp_t dtp, uint32 code, int *var_len, collation_t * collation, dtp_t col_dtp,
...
void go_ua_store (caddr_t * inst, gb_op_t * go, index_tree_t * tree, chash_t * cha, caddr_t * dep_ptr);
...
typedef enum
{ MPU_MODULE, MPU_PROC, MPU_UDT } mpu_name_type_t;
...
caddr_t *qi_alloc (query_t * qr, stmt_options_t * opts, caddr_t * auto_qi, int auto_qi_len, int n_sets);
...
caddr_t xml_deserialize_packed (caddr_t * qst, caddr_t strg);
...
void ssl_insert_cast (insert_node_t * ins, caddr_t * inst, int nth_col, caddr_t * err_ret, row_delta_t * rd, int from_row,
    int to_row, int no_blobs);
...
caddr_t cl_vec_exec (query_t * qr, client_connection_t * cli, mem_pool_t * mp, caddr_t * params, slice_id_t * slices,
    slice_id_t slid, db_buf_t * set_mask_ret, data_col_t ** dc_ret, int set_no_in_params);
...
void complete_proc_name (char *proc_name, char *ret_complete, const char *def_qual, const char *def_owner);
