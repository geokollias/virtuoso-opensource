...
typedef int (*xenc_encryptor_f) (dk_session_t * ses_in, long len, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * tb);

typedef int (*xenc_decryptor_f) (dk_session_t * ses_in, long len, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * tb);
...
xenc_key_t *xenc_build_encrypted_key (const char *carried_name, dk_session_t * in, const char *algo_name, xenc_try_block_t * t);
...
caddr_t dsig_sign_signature_ (query_instance_t * qi, dsig_signature_t * dsig, caddr_t dsig_template, xenc_err_code_t * c,
    char **c_err);
...
int xml_c_build_ancessor_ns_link (caddr_t * doc_tree, caddr_t * select_tree, id_hash_t * nss, dk_set_t * parent_link);
...
xenc_key_t *certificate_decode (caddr_t encoded_cert, const char *value_type, const char *encoding_type);
...
int xenc_algorithms_create (const char *ns0, const char *name, xenc_encryptor_f enc, xenc_decryptor_f dect, DSIG_KEY_TYPE key_type);
