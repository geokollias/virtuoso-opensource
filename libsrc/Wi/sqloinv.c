...
sinv_find_func_map (const char *function_name, client_connection_t * cli)
{
  ...
  old_map = (sinv_map_t **) id_hash_get (sinv_func_hash, (caddr_t) & function_name);
  if (old_map)
    return *old_map;
  ...
  if (case_mode == CM_MSSQL)
    {
      ...
      nm[sizeof (nm) - 1] = 0;
      strlwr (nm);
      old_map = (sinv_map_t **) id_hash_get (sinv_func_hash, (caddr_t) & pnm);
    }
  ...
  if (old_map)
  ...
  else
    {
      ...
      if (proc_name)
	old_map = (sinv_map_t **) id_hash_get (sinv_func_hash, (caddr_t) & proc_name);
    }
...
sinv_make_decoy_procs (client_connection_t * cli, sinv_map_t * map)
{
  ...
  if (IS_BOX_POINTER (err))
    {
      log_error ("Internal error making decoys for the SYS_SQL_INVERSE [%.5s] %.1000s", ERR_STATE (err), ERR_MESSAGE (err));
      dk_free_tree (err);
      err = NULL;
    }
  strses_flush (ses);
  ...
  DO_BOX (caddr_t, inverse, inx, map->sinvm_inverse)
  {
    ...
    if (err)
      {
	log_error ("Internal error making rev decoys for the SYS_SQL_INVERSE [%.5s] %.1000s", ERR_STATE (err), ERR_MESSAGE (err));
	dk_free_tree (err);
	err = NULL;
      }
    strses_flush (ses);
  ...
  if (function_name)
    {
	  ...
	  " where SINV_FUNCTION = ? " " order by SINV_FUNCTION, SINV_ARGUMENT", cli, &err, SQLC_DEFAULT);
      REPORT_ERR (err);

      err = qr_quick_exec (rd, cli, "q", &lc, 1, ":0", function_name, QRP_STR);
      REPORT_ERR (err);
    }
  else
    {
	  ...
	  "  SINV_ARGUMENT, "
	  "  SINV_INVERSE, "
	  "  SINV_FLAGS " " from " "  DB.DBA.SYS_SQL_INVERSE " " order by SINV_FUNCTION, SINV_ARGUMENT", cli, &err, SQLC_DEFAULT);
      REPORT_ERR (err);
  ...
  while (lc_next (lc))
      ...
      if (!map || CASEMODESTRCMP (map->sinvm_name, sinvm_name))
	{
	  if (map)
	    ...
	    {
	      map->sinvm_inverse = (caddr_t *) list_to_array (dk_set_nreverse (inverse_set));
	      inverse_set = NULL;
#ifndef NDEBUG
	  ...
	  id_hash_set (sinv_func_hash, (caddr_t) & map->sinvm_name, (caddr_t) & map);
	}

      dk_set_push (&inverse_set, (void *) box_dv_short_string (sinvm_inverse));
...
static int
sinv_normalize_func_name (const char *fname, client_connection_t * cli, char *nm, int sizeof_nm)
{
  if (!bif_find (fname))
    {
...
static caddr_t
bif_sinv_read_invers_sys (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!sinv_normalize_func_name (fname, qi->qi_client, nm, sizeof (nm)))
    sqlr_new_error ("22023", "SR461", "Procedure %.*s not found in sinv_read_invers_sys", 3 * MAX_NAME_LEN, fname);

  sinv_read_sql_inverses (&nm[0], qi->qi_client);

  arr = list (2, box_dv_short_string ("sinv_read_invers_sys(?)"), box_dv_short_string (&nm[0]));
  ...
  if (ST_P (tree, CALL_STMT) && SINV_DV_STRINGP (tree->_.call.name))
    {
      ...
      if (pres && BOX_ELEMENTS (pres->sinvm_inverse) == BOX_ELEMENTS (tree->_.call.params))
	return pres;
    }
  return NULL;
}
...
sinv_check_inverses (ST * tree, client_connection_t * cli)
{
  ...
  if (map)
      ...
      if (BOX_ELEMENTS (tree->_.call.params) == 1)
	{
	  ...
	  SKIP_AS (arg);
	  if (ST_P (arg, CALL_STMT) && SINV_DV_STRINGP (arg->_.call.name) && BOX_ELEMENTS (arg->_.call.params) == 1)
	    {
	      char nm[3 * MAX_NAME_LEN + 3];
	      if (sinv_normalize_func_name (arg->_.call.name, cli, nm, sizeof (nm)) && !CASEMODESTRCMP (nm, map->sinvm_inverse[0]))
		return sinv_check_inverses (arg->_.call.params[0], cli);
	    }
	}
    }
  else if (ST_P (tree, CALL_STMT) && SINV_DV_STRINGP (tree->_.call.name) && BOX_ELEMENTS (tree->_.call.params) == 1)
    {
      char nm[3 * MAX_NAME_LEN + 3];
...
sinv_check_exp (sqlo_t * so, ST * tree)
{
  ...
  if (tree->type != BOP_PLUS &&
      ...
      tree->type != BOP_DIV &&
      tree->type != BOP_EQ &&
      tree->type != BOP_NEQ && tree->type != BOP_LT && tree->type != BOP_LTE && tree->type != BOP_GT && tree->type != BOP_GTE)
    return otree;
  ...
  if (lmap && rmap && lmap == rmap && lmap_col)
	...
	new_left = (ST *) t_box_copy_tree ((caddr_t) left->_.call.params[inx_inv]);
	new_right = (ST *) t_box_copy_tree ((caddr_t) right->_.call.params[inx_inv]);
      ...
      if (do_it)
	{
	  DO_BOX (caddr_t, inverse, inx_inv, map->sinvm_inverse)
	  {
	    ...
	    ST *new_left, *new_right;
	    new_left = (ST *) t_box_copy_tree ((caddr_t) left->_.call.params[inx_inv]);
	    new_right = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase (inverse), t_list (1, right));
