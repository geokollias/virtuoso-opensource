...
typedef int (*sqlc_exp_print_hook_t) (struct sql_comp_s *, comp_table_t * ct, ST * exp, char *text, size_t len, int *fill);
...
void comp_scalar_exp (sql_comp_t * sc, dk_set_t * code_vec, sql_tree_t * tree, state_slot_t * res);
...
void pred_gen_1 (sql_comp_t * sc, ST * tree, dk_set_t * code, int succ, int fail, int unkn);
...
state_slot_t *select_ref_generate (sql_comp_t * sc, ST * tree, dk_set_t * code, dk_set_t * fun_ref_acc_code, int *is_fun_ref);

col_ref_rec_t *sqlc_col_or_param (sql_comp_t * sc, ST * tree, int is_recursive);

col_ref_rec_t *sqlc_col_ref_rec (sql_comp_t * sc, ST * col_ref, int err_if_not);

col_ref_rec_t *sqlc_find_crr (sql_comp_t * sc, ST * ref);


subq_compilation_t *sqlc_subquery_1 (sql_comp_t * super_sc, predicate_t * pred, ST ** ptree, int mode, ST ** params);

subq_compilation_t *sqlc_subquery (sql_comp_t * super_sc, predicate_t * pred, ST ** ptree);

subq_compilation_t *sqlc_subq_compilation (sql_comp_t * sc, ST * tree, char *name);
subq_compilation_t *sqlc_subq_compilation_1 (sql_comp_t * sc, ST * tree, char *name, int scrollables);
...
void sqlc_update_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * cursor_sqc, ST ** src_ret);

void sqlc_update_searched (sql_comp_t * sc, ST * tree);

void sqlc_delete_pos (sql_comp_t * sc, ST * tree, subq_compilation_t * cursor_sqc, ST ** src_ret);
...
void sqlc_trig_const_params (sql_comp_t * sc, state_slot_t ** params, dk_set_t * code);
void sqlc_update_set_keyset (sql_comp_t * sc, table_source_t * ts);


void tc_init (trig_cols_t * tc, int event, dbe_table_t * tb, caddr_t * cols, ST ** vals, dbe_key_t * add_pk);
...
void qr_replace_node (query_t * qr, data_source_t * to_replace, data_source_t * replace_with, int move_after_code);
...
/* sqlcr.c */
query_t *sql_compile_st (ST ** ptree, client_connection_t * cli, caddr_t * err, sql_comp_t * super_sc);
...
query_t *sqlc_make_proc_store_qr (client_connection_t * cli, query_t * proc_or_trig, const char *text);
...
ST *sqlc_pos_to_searched_where (sql_comp_t * sc, subq_compilation_t * sqc, char *cr_name, dbe_table_t * tb);
