...
DBS_REVERSE_LONG (db_buf_t pl)
{
  if (h_index && gethash ((void *) pl, row_hash))
  ...
  else
    {
      ...
      LONG_SET (pl, npl);
      if (h_index)
	sethash ((void *) pl, row_hash, (void *) 1);
    }
}
...
DBS_REVERSE_SHORT (db_buf_t ps)
{
  if (h_index && gethash ((void *) ps, row_hash))
  ...
  else
    {
      ...
      SHORT_SET (ps, nps);
      if (h_index)
	sethash ((void *) ps, row_hash, (void *) 1);
    }
}
...
page_set_checksum (db_buf_t page)
{
  ...
  for (inx = 0; inx < ((PAGE_DATA_SZ / sizeof (uint32)) / 17) * 17; inx += 17)
	...
	^ p[inx + 9] ^ p[inx + 10] ^ p[inx + 11] ^ p[inx + 12] ^ p[inx + 13] ^ p[inx + 14] ^ p[inx + 15] ^ p[inx + 16];
...
it_not_in_any (du_thread_t * self, index_tree_t * except)
{
  ...
  DO_SET (dbe_storage_t *, dbs, &wi_inst.wi_storage)
  {
    ...
    {
      if (it != except && it->it_page_map_mtx->mtx_owner == self)
...
it_temp_free (index_tree_t * it)
{
  ...
  if (!it->it_hi)
    {
      ...
      n = --it->it_ref_count;
      if (n < 0)
	GPF_T1 ("neg ref count of oby temp");
      LEAVE_HIC;
      if (n)
	return;
    }
  ...
  for (inx = 0; inx < IT_N_MAPS; inx++)
      ...
      while (dk_hit_next (&hit, (void **) &dp, (void **) &buf))
	{
      ...
      ITC_LEAVE_MAPS (itc);
      if (DBS_TEMP != it->it_storage->dbs_type)
	GPF_T1 ("temp tree free with a non temp dbs");
...
void buf_qsort (buffer_desc_t ** in, buffer_desc_t ** left, int n_in, int depth, sort_key_func_t key);
...
bp_stats (buffer_pool_t * bp)
{
  ...
  if (fill < 1 || fill * sizeof (bp_ts_t) > sizeof (sample_2))
    GPF_T1 ("buf fill anomaly");
...
bp_stat_action (buffer_pool_t * bp, int stat_only)
{
  ...
  if (stat_only)
    return -bucket;
  if ((n_dirty * 100) / (n_clean + n_dirty) > bp_flush_trig_pct || n_dirty > wi_inst.wi_max_dirty / wi_inst.wi_n_bps
      /*|| action_ctr++ % 1000 == 0 */ )
    {
...
bp_n_being_written (buffer_pool_t * bp)
{
  ...
  for (inx = 0; inx < bp->bp_n_bufs; inx++)
    {
      if (bp->bp_bufs[inx].bd_iq && bp->bp_bufs[inx].bd_is_dirty)
	c++;
    }
  return c;
}
...
bp_get_buffer_1 (buffer_pool_t * bp, buffer_pool_t ** action_bp_ret, int mode)
{
  ...
  for (n_again = 1;; ++n_again)
      ...
      for (buf = &bp->bp_bufs[bp->bp_next_replace]; buf < &bp->bp_bufs[bp->bp_n_bufs]; buf++)
	{
	  ...
	  if (!buf->bd_is_dirty && ((int) (bp->bp_ts - buf->bd_timestamp)) >= age_limit)
	    {
	      if (bp_found (buf, 0))
		return buf;
	    }
  ...
  if (!buf->bd_is_dirty)
    {
      ...
      buf->bd_is_dirty = 1;
      wi_inst.wi_n_dirty++;
      if (0 && wi_inst.wi_n_dirty > wi_inst.wi_max_dirty && !mt_write_pending)
	{
...
buf_set_dirty_inside (buffer_desc_t * buf)
{
...
#ifdef MTX_DEBUG
  {
    ...
    if (!gethash (DP_ADDR2VOID (buf->bd_page), &itm->itm_remap))
      GPF_T1 ("not remapped when being set to dirty");
  }
#endif
...
buf_prot_read (buffer_desc_t * buf)
{
  ...
  if (rc)
    GPF_T1 ("can't set mem [protection");
  /*buf->bd_buffer[0] ++; */
  /*buf->bd_buffer[0] --; */
#endif
}
	  ...
	  if (cf_lock_in_mem)
	      ...
	      while (bs_tail < bs_end)
		{
		  ...
		  if (rc)
		    {
		      log_error
			  ("mlock() system call for disk buffers failed with %d, only first %d buffers were mlock-ed with succsess.",
...
dp_disk_locate (dbe_storage_t * dbs, dp_addr_t target, OFF_T * place, int is_write, ext_ref_t * er)
{
  ...
  DO_SET (disk_segment_t *, seg, &dbs->dbs_disks)
  {
	...
	*place =
	    PAGE_SZ * (OFF_T) (((ext_no / seg->ds_n_stripes) * dbs->dbs_stripe_unit) + (target - (ext_no * dbs->dbs_stripe_unit)));
  ...
  if (dbs->dbs_disks)
      ...
      if (off_dest > dbs->dbs_file_length)
	{
	  ...
	  while (dbs->dbs_file_length <= off_dest)
	    {
	      if (PAGE_SZ != write (dbs->dbs_fd, (char *) out, PAGE_SZ))
		{
...
long
dbs_count_free_pages (dbe_storage_t * dbs)
{
  long res;
  IN_DBS (dbs);
...
{
  if (!flag)
    GPF_T1 ("must have non-0 flag for unfreeable dp");
...
void
buf_qsort (buffer_desc_t ** in, buffer_desc_t ** left, int n_in, int depth, sort_key_func_t key)
{
  if (n_in < 2)
    return;
  if (n_in < 3)
	...
	return;			/* all are equal */
      ...
      buf_qsort (left + n_right + 1, in + n_right + 1, (n_in - n_right) - 1, depth + 1, key);
...
void
gen_qsort (int *in, int *left, int n_in, int depth, sort_cmp_func_t cmp, void *cd)
{
  ...
  if (n_in < 3)
      ...
      if (!is_mid)
	{
      ...
      if (n_retries < 3 && n_in > 20 && (n_left < n_in / 16 || (n_in - n_left) < n_in / 16))
	{
	  n_retries++;
      ...
      gen_qsort (left + n_right + 1, in + n_right + 1, (n_in - n_right) - 1, depth + 1, cmp, cd);
      memcpy (in, left, n_left * sizeof (int));
      in[n_left] = mid;
      memcpy (in + n_right + 1, left + n_right + 1, ((n_in - n_right) - 1) * sizeof (int));
...
void
gen_qmsort (int *in, int *left, int n_in, sort_cmp_func_t cmp, void *cd, int key_bytes)
{
  ...
  for (;;)
      ...
      if (DVC_LESS != cmp (in[bucket_start[bucket_order[0]] - 1], in[bucket_start[bucket_order[1]] - 1], cd))
	{
	  short prev_first;
	  int place =
	      1 + mrg_place (&bucket_order[1], in[bucket_start[bucket_order[0]] - 1], in, bucket_start, n_in_merge - 1, cmp, cd);
...
void
dss_pass (digit_sort_set_t * target, digit_sort_set_t * source, int from_byte, int to_byte, data_col_t * dc, int nth_byte)
{
  ...
  for (inx = 0; inx < n_sets; inx++)
    {
      ...
      mask |= i ^ prev;
      if (in_order && (is_signed ? (prev > i) : ((unsigned int64) prev > (unsigned int64) i)))
	in_order = 0;
      prev = i;
...
int
dss_dss_initial (digit_sort_set_t * target, digit_sort_set_t * source, data_col_t * dc, unsigned int64 * mask_ret, int prev_signed)
{
...
bp_write_dirty (buffer_pool_t * bp, int force, int is_in_bp, int n_oldest)
{
  ...
  for (inx = 0; inx < bp->bp_n_bufs; inx++)
      ...
      else
	{
	  ...
	  if (tree && (ALL_DIRTY == n_oldest || BUF_AGE (buf) >= n_oldest))
		...
		{
		  if (buf->bd_is_dirty
		      && buf->bd_tree && buf->bd_page && !buf->bd_readers && !buf->bd_is_write && !buf->bd_write_waiting)
		    {
  ...
  if (fill)
    {
      dbg_printf (("     Flush %d buffers, pages %ld - %ld.\n", fill, bufs[0]->bd_page, bufs[fill - 1]->bd_page));
    }
  for (n = 0; n < fill; n++)
    {
	  ...
	  for (inx = 0; inx < n_fds_per_file; inx++)
	    {
	      ...
	      if (fd < 0)
		{
		  log_error ("Cannot open stripe on %s (%d)", dst->dst_file, errno);
		  call_exit (1);
		}
#if 0
#ifdef LOCK_EX
...
dbs_rev_h_index (dbe_storage_t * dbs, buffer_desc_t * buf)
{
...
#endif
  {
    ...
    dbe_key_t *pg_key = KI_TEMP == pg_key_id ? buf->bd_tree->it_key : sch_id_to_key (wi_inst.wi_schema, pg_key_id);
...
void
dbs_rev_h_blob (dbe_storage_t * dbs, buffer_desc_t * buf)
{
  DBS_REVERSE_LONG (buf->bd_buffer + DP_PARENT);
...
void
dbs_rev_h_free (dbe_storage_t * dbs, buffer_desc_t * buf)
{
  GPF_T1 ("not implemented");
}

void
dbs_rev_h_head (dbe_storage_t * dbs, buffer_desc_t * buf)
{
  GPF_T1 ("not implemented");
}

void
dbs_rev_h_remap (dbe_storage_t * dbs, buffer_desc_t * buf)
{
  int inx;
...
void
dbs_rev_h_blob_dir (dbe_storage_t * dbs, buffer_desc_t * buf)
{
  ...
  if (real_sz > temp_db_size * 1024L * 1024L)
      ...
      else
	{
	  log_info ("Unlinked the temp db file %.1000s as its size (%ldMB)"
	      " was greater than TempDBSize INI (%ldMB)", dbs->dbs_file, (real_sz / 1024 / 1024), temp_db_size);
	  return 1;
	}
    }
  return 0;
...
wi_init_globals (void)
{
  ...
  lock_rc = resource_allocate (2000, (rc_constr_t) pl_allocate, (rc_destr_t) pl_free, NULL, 0);
  row_lock_rc = resource_allocate (5000, (rc_constr_t) rl_allocate, (rc_destr_t) rl_free, NULL, 0);
  pfh_rc = resource_allocate (10, (rc_constr_t) pfh_allocate, (rc_destr_t) pfh_free, NULL, 0);

  rb_page_rc = resource_allocate (100, (rc_constr_t) rbp_allocate, (rc_destr_t) rbp_free, NULL, 0);
...
dbe_key_open_dbs (dbe_key_t * key, dbe_storage_t * dbs)
{
  ...
  DO_BOX (dbe_key_frag_t *, kf, inx, key->key_fragments)
  {
      ...
      for (inx = 0; inx < IT_N_MAPS; inx++)
	{
	  ...
	  mutex_option (&(kf->kf_it->it_maps[inx].itm_mtx), mtx_name, NULL /*it_page_map_entry_check */ ,
	      (void *) &kf->kf_it->it_maps[inx]);
	}
    }
    kf->kf_it->it_key = key;
    if (start_str)
...
dbe_key_open (dbe_key_t * key)
{
  if (DBS_PRIMARY == key->key_storage->dbs_type)
  ...
  else if (DBS_ELASTIC == key->key_storage->dbs_type)
    {
      ...
      int inx;
      DO_BOX (dbe_storage_t *, sdbs, inx, dbs->dbs_slices) if (sdbs)
