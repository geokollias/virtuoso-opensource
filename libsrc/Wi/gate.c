...
int
itc_async_read_1 (it_cursor_t * itc, dp_addr_t dp, dp_addr_t phys_dp, buffer_desc_t * buf, buffer_desc_t * decoy)
{
#if 0
    ...
    buffer_desc_t ** buf_ret, int mode, int max_change)
{
...
#endif
      if ((DP_DELETED == phys_dp || dbs_may_be_free (itc->itc_tree->it_storage, phys_dp)) && !strchr (wi_inst.wi_open_mode, 'a'))
	{
	  log_error ("Reference to page with free remap dp = %ld, remap = %ld", (long) phys_dp, (long) dp);
  ...
  if (PA_READ == mode)
    {
      if (!buf->bd_is_write && !buf->bd_write_waiting)
	{
	  buf->bd_readers++;
	  if (buf_from)
	    page_leave_inner (buf_from);
    ...
    {
      if (!buf->bd_is_write && 0 == buf->bd_readers)
	{
	  if (buf_from)
	    page_leave_inner (buf_from);
  ...
  while (waiting)
      ...
      else
	{
	  rdbg_printf (("pw reset itc %p chg %d max %d landed %d a for %d itc_page %d \n",
		  ...
		  (int) waiting->itc_max_transit_change, (int) waiting->itc_landed, buf->bd_page, waiting->itc_page));
...
void DBGP_NAME (page_leave_inner) (DBGP_PARAMS buffer_desc_t * buf)
{
  ...
  BUF_BOUNDS_CHECK (buf);
#ifdef PAGE_DEBUG
  if (enable_col_leave_check && buf->bd_buffer && DPF_INDEX == SHORT_REF (buf->bd_buffer + DP_FLAGS) && buf->bd_tree
      && buf->bd_tree->it_key && buf->bd_tree->it_key->key_is_col && BUF_SEEMS_LEAF (buf, buf->bd_content_map))
    {
...
page_write_queue_add (buffer_desc_t * buf, it_cursor_t * itc)
{
  ...
  TC (tc_write_wait);
  if (!buf->bd_being_read && buf->bd_is_write && !buf->bd_iq && buf->bd_buffer
      && DPF_COLUMN == SHORT_REF (buf->bd_buffer + DP_FLAGS))
    log_error
	("Write wait on column page %d.  Waits should be on the index leaf page, except when col page is held for read by background write",
	buf->bd_page);
...
page_read_queue_add (buffer_desc_t * buf, it_cursor_t * itc)
{
  ...
  if (!buf->bd_being_read && buf->bd_buffer && DPF_COLUMN == SHORT_REF (buf->bd_buffer + DP_FLAGS))
    log_error ("Read wait on column page %d.  Waits should be on the index leaf page", buf->bd_page);
  itc->itc_thread = THREAD_CURRENT_THREAD;
...
retry:
  if (ctl_itc->itc_ltrx && ctl_itc->itc_ltrx->lt_status != LT_CLOSING)
    {
      ...
      if (!ctl_itc->itc_fail_context)
	GPF_T1 ("must have fail ctx in pl_enter");
  ...
  if (!buf || buf->bd_page != target_dp_1)
    {
  ...
  if (!buf->bd_is_write && (PA_READ_ONLY == ctl_itc->itc_dive_mode || !buf->bd_readers) && BUF_NONE_WAITING (buf))
    {
    ...
    buffer_desc_t * old_buf, dp_addr_t back_link, int was_after_wait)
{
  ...
  rdbg_printf (("Bad parent link in %ld, coming from %ld, parent link = %ld.\n", (*buf)->bd_page, old_buf->bd_page, back_link));
  if (1 || was_after_wait)
    {
...
void DBGP_NAME (itc_landed_down_transit) (DBGP_PARAMS it_cursor_t * itc, buffer_desc_t ** buf_ret, dp_addr_t to)
{
  buffer_desc_t *old_buf = *buf_ret;
  int waited = 0;
  dp_addr_t dp_from = itc->itc_page, back_link;
...
#ifndef NDEBUG
  if (PA_READ_ONLY != itc->itc_dive_mode && !old_buf->bd_is_write)
    GPF_T1 ("landed transit got no write");
#endif
  itc->itc_is_on_row = 1;
  ...
  if (itc->itc_to_reset > RWG_NO_WAIT)
    {
      ...
      *buf_ret = page_reenter_excl (itc);
      if (itc->itc_desc_order && (itc->itc_is_on_row || ITC_AT_END == itc->itc_map_pos))
	{
...
void DBGP_NAME (itc_down_transit) (DBGP_PARAMS it_cursor_t * itc, buffer_desc_t ** buf_ret, dp_addr_t to)
{
  if (itc->itc_landed)
...
up_again:
  ...
  ITC_IN_TRANSIT (itc, up, (*buf_ret)->bd_page) if (!LONG_REF (up_field))
    {
...
it_new_root_image (index_tree_t * tree, buffer_desc_t * buf)
{
  ...
  if (tree->it_root_image_version || buf->bd_page != tree->it_root)
    {
...
buffer_desc_t *DBGP_NAME (itc_reset) (DBGP_PARAMS it_cursor_t * it)
{
  ...
  if ((PA_READ == it->itc_dive_mode && tree->it_is_single_page) || (tree->it_key->key_is_geo && SM_INSERT == it->itc_search_mode))
  ...
  BUF_TOUCH (buf);
  if (!tree->it_is_single_page && !tree->it_root_image_version && tree->it_key->key_id != KI_TEMP && !tree->it_key->key_is_geo)
    {
  ...
  if (back_link)
    {
      log_error ("Bad parent link in the tree start page %ld, parent link=%ld." " Please do a dump and restore.", dp, back_link);
    }
  it->itc_page = dp;
#ifdef NEW_HASH
  ...
  if (!buf)
	...
	GPF_T1 ("setting parent of deleted page in split");
      ...
      rdbg_printf_2 (("	 After disk read: Parent of L=%d from L=%d to L=%d .\n",
	      buf->bd_page, LONG_REF (buf->bd_buffer + DP_PARENT), new_parent));
...
itc_unregister (it_cursor_t * it_in)
{
  ...
  if (reg_buf && !reg_buf->bd_is_write && !reg_buf->bd_readers && dp == itc->itc_page && reg_buf->bd_page == dp)
    {
...
itc_unregister_while_on_page (it_cursor_t * it_in, it_cursor_t * preserve_itc, buffer_desc_t ** preserve_buf)
{
  ...
  mutex_enter (&itm->itm_mtx);
  reg_buf = itc->itc_buf_registered;
  if (reg_buf && !reg_buf->bd_is_write && !reg_buf->bd_readers && itc->itc_page == dp && reg_buf->bd_page == dp)
    {
      itc_unregister_inner_safe ((it_cursor_t *) itc, reg_buf, 0);
      mutex_leave (&itm->itm_mtx);
      return;
...
enter:
  TC (tc_unregister_enter);
  {
    ...
    it_cursor_t *reg_itc = &itc_auto;
    ITC_INIT (reg_itc, NULL, NULL);
    if (!preserve_itc->itc_fail_context)
      GPF_T1 ("must have fail ctx in itc_unregister_while_on_page ");
    itc_from_it (reg_itc, itc->itc_tree);
