...
geo_wkt (caddr_t x)
{
  ...
  if ((GEO_POINT == GEO_TYPE (g->geo_flags)) && (GEO_SRCODE_DEFAULT == g->geo_srcode))
    {
      char xx[100];
      snprintf (xx, sizeof (xx), "POINT(" GEOC_WKT_STAR_FMT " " GEOC_WKT_STAR_FMT ")", GEOC_WKT_PREC, Xkey (g), GEOC_WKT_PREC,
	  Ykey (g));
...
dbg_geo_to_text (caddr_t x)
{
  ...
  switch (GEO_TYPE (g->geo_flags))
    {
    case GEO_POINT:
      snprintf (tmp, sizeof (tmp), "<point " GEOC_WKT_STAR_FMT " " GEOC_WKT_STAR_FMT ">", GEOC_WKT_PREC, Xkey (g), GEOC_WKT_PREC,
	  Ykey (g));
      break;
    default:
