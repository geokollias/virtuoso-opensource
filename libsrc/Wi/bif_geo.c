...
bif_geoc_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func, int *isnull_ret, geo_srcode_t srcode, int coord_idx)
{
  ...
  switch (dtp)
    ...
    case DV_SINGLE_FLOAT:
      c = (double) unbox_float (arg);
      break;
    case DV_DOUBLE_FLOAT:
      c = unbox_double (arg);
      break;
    case DV_NUMERIC:
      numeric_to_double ((numeric_t) arg, &c);
      break;
    case DV_DB_NULL:
      if (NULL != isnull_ret)
	{
  ...
  if ((c >= geoc_FARAWAY - 16 * geoc_EPSILON) || (c <= -(geoc_FARAWAY - 16 * geoc_EPSILON)))
    sqlr_new_error ("22023", "GEO..",
	"Function %s needs a coordinate as argument %d, the number %g is not a valid coordiate", func, nth + 1, c);
...
geo_cast_box_to_geoc (caddr_t v, geo_srcode_t srcode, int coord_idx, int *isnull_ret, const char **err_text_ret)
{
  ...
  switch (dtp)
    ...
    case DV_LONG_INT:
      ...
      break;
    case DV_SINGLE_FLOAT:
      c = (double) unbox_float (v);
      break;
    case DV_DOUBLE_FLOAT:
      c = unbox_double (v);
      break;
    case DV_NUMERIC:
      numeric_to_double ((numeric_t) v, &c);
      break;
    case DV_DB_NULL:
      if (NULL != isnull_ret)
	{
      ...
      /* no break */
    default:
      err_text_ret[0] = "Coordinate is not a number";
      return 0;
    }
  if (NULL != isnull_ret)
    isnull_ret[0] = 0;
      ...
      if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (arg))
	{
	  ...
	  if (0 == v_len)
	  ...
	  else
	    {
	      ...
	      if (!(DV_SHORT_INT == dtp0 || DV_LONG_INT == dtp0 || DV_SINGLE_FLOAT == dtp0 || DV_DOUBLE_FLOAT == dtp0
		      || DV_NUMERIC == dtp0))
		arg_is_list_and_not_point = 1;
	    }
	}
	...
	{			/* Vector of points or geometries */
	  ...
	  if (DV_GEO == DV_TYPE_OF (v))
		...
		case GEO_ARCSTRING:
		  ...
		  break;
		default:
		  sqlr_new_error ("22023", "SP...",
		      "Invalid argument %d in call of %s(): the vector contains shape that can not be used as a fragment of the result",
		      argctr + 1, fname);
		}
	      ...
	      if (pass2)
		{
		  ...
		  if (flags & itm->geo_flags & GEO_A_M)
		    memcpy (res->_.pline.Ms + pointctr, itm->_.pline.Ms + first_new_point_ofs,
			sizeof (geo_measure_t) * addon_count);
  ...
  if ((0 < prec) && GEO_SR_SPHEROID_DEGREES (srcode))
    {
      ...
      return ((lat_prec_deg >= dist_from_point_to_line_segment (pX * lon_to_lat, pY, p1X * lon_to_lat, p1Y, p2X * lon_to_lat,
		  p2Y)) ? 1 : 0);
    }
  ...
  if ((0 < prec) && GEO_SR_SPHEROID_DEGREES (srcode))
    {
      ...
      bXmin = b->Xmin - lon_prec_deg;
      bXmax = b->Xmax + lon_prec_deg;
      bYmin = b->Ymin - lat_prec_deg;
      bYmax = b->Ymax + lat_prec_deg;
    }
  else
    {
      bXmin = b->Xmin - prec;
...
int
geo_line_intersects_line (geo_srcode_t srcode, geoc p1X, geoc p1Y, geoc p2X, geoc p2Y, geoc q3X, geoc q3Y, geoc q4X, geoc q4Y,
    double prec)
{
  double sc1, sc2;
  if ((0 < prec) && GEO_SR_SPHEROID_DEGREES (srcode))
	  ...
	  for (cctr = 0; cctr < g2gcb->gcb_box_count; cctr++)
	      ...
	      for (ictr = 0; ictr < g2gcb->gcb_step; ictr++)
		{
		  ...
		  if (itemctr >= g2->_.pline.len - 1)
		    return 0;
		  if (geo_point_intersects_line (srcode, pX, pY, g2->_.pline.Xs[itemctr], g2->_.pline.Ys[itemctr],
			  g2->_.pline.Xs[itemctr + 1], g2->_.pline.Ys[itemctr + 1], prec))
		    return 1;
		}
	    }
	}
  ...
  switch (GEO_TYPE_NO_ZM (g2->geo_flags))
      ...
      if (GEO_IS_CHAINBOXED & g2->geo_flags)
	{
	  ...
	  for (cctr = 0; cctr < g2gcb->gcb_box_count; cctr++)
	      ...
	      for (ictr = 0; ictr < g2gcb->gcb_step; ictr++)
		{
		  ...
		  if (itemctr >= g2->_.pline.len - 1)
		    return 0;
		  if (geo_line_intersects_line (srcode, p1X, p1Y, p2X, p2Y, g2->_.pline.Xs[itemctr], g2->_.pline.Ys[itemctr],
			  g2->_.pline.Xs[itemctr + 1], g2->_.pline.Ys[itemctr + 1], prec))
		    return 1;
		}
	    }
      ...
      if (GEO_IS_CHAINBOXED & g2->geo_flags)
	{
	  ...
	  for (cctr = 0; cctr < g2gcb->gcb_box_count; cctr++)
	      ...
	      for (ictr = 0; ictr < g2gcb->gcb_step; ictr++)
		{
		  ...
		  if (itemctr >= g2->_.pline.len)
		    return 0;
		  if (geo_point_intersects_line (srcode, g2->_.pline.Xs[itemctr], g2->_.pline.Ys[itemctr], p1X, p1Y, p2X, p2Y,
			  prec))
		    return 1;
		}
	    }
	...
	if (GEO_IS_CHAINBOXED & g2->geo_flags)
		...
		for (ictr = 0; ictr < g2gcb->gcb_step; ictr++)
		  {
		    ...
		    if (itemctr >= (g2->_.pline.len - 1))
		      return 0;
		    if (geo_line_intersects_line (srcode, g2->_.pline.Xs[itemctr], g2->_.pline.Ys[itemctr],
			    g2->_.pline.Xs[itemctr + 1], g2->_.pline.Ys[itemctr + 1], p1X, p1Y, p2X, p2Y, prec))
		      return 1;
		  }
	      }
	  }
	else
	  {
	    for (itemctr = 0; itemctr < g2->_.pline.len - 1; itemctr++)
	      {
		if (geo_line_intersects_line (srcode, g2->_.pline.Xs[itemctr], g2->_.pline.Ys[itemctr], g2->_.pline.Xs[itemctr + 1],
			g2->_.pline.Ys[itemctr + 1], p1X, p1Y, p2X, p2Y, prec))
		  return 1;
	      }
	  }
	return 0;
...
unsupported:
  sqlr_new_error ("42000", "GEO..", "The check for spatial intersection is not implemented for a line and a shape of type %d",
      g2->geo_srcode);
  return 0;
}
...
geo_may_intersect_XYbox (geo_t * g, geo_XYbox_t * b, double prec)
{
  ...
  if ((g->XYbox.Xmax < b->Xmin - prec)
      || (g->XYbox.Xmin > b->Xmax + prec) || (g->XYbox.Ymax < b->Ymin - prec) || (g->XYbox.Ymin > b->Ymax + prec))
    return 0;
  return 1;
}
		...
		if (0 < prec)
		  {
			...
			|| (g1->XYbox.Ymax < g2->XYbox.Ymin) || (g1->XYbox.Ymin > g2->XYbox.Ymax))
		...
		for (cctr = 0; cctr < g1gcb->gcb_box_count; cctr++)
		  {
			...
			if (itemctr >= (g1->_.pline.len - 1))
			  return 0;
			if (geo_line_intersects (g1->geo_srcode, g1->_.pline.Xs[itemctr], g1->_.pline.Ys[itemctr],
				g1->_.pline.Xs[itemctr + 1], g1->_.pline.Ys[itemctr + 1], g2, prec))
			  return 1;
		      }
		  }
	      }
	...
	if (GEO_POINT == GEO_TYPE_NO_ZM (g1->geo_flags))
	  {
	    ...
	    if (GEO_BOX == GEO_TYPE_NO_ZM (g2->geo_flags))
	      {
		geoc boxproximaX =
		    ((Xkey (g1) > g2->XYbox.Xmax) ? g2->XYbox.Xmax : ((Xkey (g1) < g2->XYbox.Xmin) ? g2->XYbox.Xmin : Xkey (g1)));
		geoc boxproximaY =
		    ((Ykey (g1) > g2->XYbox.Ymax) ? g2->XYbox.Ymax : ((Ykey (g1) < g2->XYbox.Ymin) ? g2->XYbox.Ymin : Ykey (g1)));
		...
		|| (Ykey (g1) < g2->XYbox.Ymin - prec) || (Ykey (g1) > g2->XYbox.Ymax + prec))
	      return 0;
	    return 1;
	  }
	if ((g1->XYbox.Xmax < g2->XYbox.Xmin - prec)
	    || (g1->XYbox.Xmin > g2->XYbox.Xmax + prec)
	    || (g1->XYbox.Ymax < g2->XYbox.Ymin - prec) || (g1->XYbox.Ymin > g2->XYbox.Ymax + prec))
	  return 0;
	return 1;
      }
    case GSOP_CONTAINS:
	...
	if (GEO_POINT == GEO_TYPE_NO_ZM (g2->geo_flags))
	  {
	    ...
	    if (GEO_BOX == GEO_TYPE_NO_ZM (g1->geo_flags))
	      {
		geoc boxproximaX =
		    ((Xkey (g2) > g1->XYbox.Xmax) ? g1->XYbox.Xmax : ((Xkey (g2) < g1->XYbox.Xmin) ? g1->XYbox.Xmin : Xkey (g2)));
		geoc boxproximaY =
		    ((Ykey (g2) > g1->XYbox.Ymax) ? g1->XYbox.Ymax : ((Ykey (g2) < g1->XYbox.Ymin) ? g1->XYbox.Ymin : Ykey (g2)));
		...
		|| (Ykey (g2) < g1->XYbox.Ymin - prec) || (Ykey (g2) > g1->XYbox.Ymax + prec))
	      return 0;
	    return 1;
	  }
	if ((g2->XYbox.Xmin < g1->XYbox.Xmin - prec)
	    || (g2->XYbox.Xmax > g1->XYbox.Xmax + prec)
	    || (g2->XYbox.Ymin < g1->XYbox.Ymin - prec) || (g2->XYbox.Ymax > g1->XYbox.Ymax + prec))
	  return 0;
	return 1;
      }
    }
...
bif_geometry_type (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL == metas)
    sqlr_new_error ("22023", "GEO..",
	"Unsupported shape type %u, geometry instance is corrupted or created by Virtuoso server of later version",
	GEO_TYPE (g->geo_flags));
...
bif_st_geometry_n (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if ((idx < 1) || (idx > g->_.parts.len))
    sqlr_new_error ("22023", "GEO..", "Invalid index value " BOXINT_FMT ", valid values for this geometery are 1 to %ld",
	(boxint) idx, (long) (g->_.parts.len));
...
geo_proj_point_olaeaps (void *geo_proj, geoc longP, geoc latP, double *retX, double *retY)
{
  ...
  double kdiv, k;
  kdiv =
      1 + ((geo_proj_olaeaps_t *) geo_proj)->sin_lat0 * sin_latP +
      ((geo_proj_olaeaps_t *) geo_proj)->cos_lat0 * cos_latP * cos_longdiff;
  ...
  retY[0] =
      k * ((geo_proj_olaeaps_t *) geo_proj)->cos_lat0 * sin_latP -
      ((geo_proj_olaeaps_t *) geo_proj)->sin_lat0 * cos_latP * cos_longdiff;
  return NULL;
}
