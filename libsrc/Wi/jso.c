	  ...
	  if (NULL != sd->jsosd_fields_by_idx[fld_idx_as_in_dv_array_of_pointers])
	    {
		  ...
		  cd->jsocd_c_typedef, fldd->jsofd_property_iri,
		  sd->jsosd_fields_by_idx[fld_idx_as_in_dv_array_of_pointers]->jsofd_property_iri);
	      GPF_T1 (buf);
	    }
...
jso_define_class (jso_class_descr_t * cd)
{
  ...
  if (NULL != old_cd)
    {
	  ...
	  cd->jsocd_class_iri, cd->jsocd_c_typedef, old_cd->jsocd_c_typedef);
      GPF_T1 (buf);
    }
  ...
  switch (cd->jsocd_cat)
    {
    case JSO_CAT_STRUCT:
      jso_define_struct (cd);
      break;
    case JSO_CAT_ARRAY:
      jso_define_array (cd);
      break;
    default:
      GPF_T1 ("jso_define_class (): unknown value of jsocd_cat");
    }
}
...
bif_jso_new (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL == inst_rtti)
	...
	switch (cd->jsocd_cat)
	  {
	  case JSO_CAT_STRUCT:
	    inst = dk_alloc_box_zero (cd->_.sd.jsosd_sizeof, DV_ARRAY_OF_POINTER);
	    break;
	  case JSO_CAT_ARRAY:
	    {
	      ...
	      if (init_size > 16)
		init_size = 16;
	      inst = dk_alloc_box_zero ((init_size + 1) * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
	      break;
	    }
	  default:
	    GPF_T1 ("jso_new (): unknown value of jsocd_cat");
	  }
...
void
jso_validate (jso_rtti_t * inst_rtti, jso_rtti_t * root_rtti, dk_hash_t * known, int change_status, dk_set_t * errors_log_ptr,
...
#define SET_STATUS_FAILED \
  ...
  switch (inst_rtti->jrtti_status)
    ...
    case JSO_STATUS_NEW:
      break;
    default:
      GPF_T1 ("jso_validate(): unknown status");
    }
  switch (cd->jsocd_cat)
    {
    case JSO_CAT_ARRAY:
	...
	if (full_length > cd->_.ad.jsoad_min_length)
	  {
	    while ((0 < used_length) && (NULL == inst[used_length - 1]))
	      used_length--;
	  }
	if (used_length < cd->_.ad.jsoad_min_length)
	  {
	    ...
	    dk_set_push (errors_log_ptr, list (2, inst_rtti,
		    box_sprintf (2000,
			"JSO array instance <%.500s> of type <%.500s> is only %ld items long, should have at least %ld",
			inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri, (long) used_length,
			(long) (cd->_.ad.jsoad_min_length))));
	    return;
	  }
	for (ctr = 0; ctr < used_length; ctr++)
	  {
		...
		if (NULL != fld_type_cd)
		  {
			...
			dk_set_push (errors_log_ptr, list (2, inst_rtti,
				box_sprintf (2000,
				    "Item # %d of JSO array <%.500s> of type <%.500s> is of wrong type <%.500s>, must be <%.500s>",
		...
		if (&jso_cd_array_of_any != cd)
		  {
		    ...
		    dk_set_push (errors_log_ptr, list (2, inst_rtti,
			    box_sprintf (2000,
				"The value of rdf:_%d in JSO instance <%.500s> of type <%.500s> is an integer, not an object of type <%.500s>",
				ctr, inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri,
				cd->_.ad.jsoad_member_type)));
		  }
		break;
	      case DV_STRING:
		if (&jso_cd_array_of_string != cd)
		  {
		    ...
		    dk_set_push (errors_log_ptr, list (2, inst_rtti,
			    box_sprintf (2000,
				"The value of rdf:_%d in JSO instance <%.500s> of type <%.500s> is a string, not an object of type <%.500s>",
				ctr, inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri,
				cd->_.ad.jsoad_member_type)));
		  }
		break;
		...
		dk_set_push (errors_log_ptr, list (2, inst_rtti,
			box_sprintf (2000,
			    "The value of rdf:_%d in JSO instance <%.500s> of type <%.500s> is not an object of type <%.500s>", ctr,
			    inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri, cd->_.ad.jsoad_member_type)));
	      }
	  }
	break;
      }
		...
		if ((NULL != fld_type_cd) && (DV_CUSTOM == DV_TYPE_OF (sub)))
		  {
			...
			dk_set_push (errors_log_ptr, list (2, inst_rtti,
				box_sprintf (2000,
				    "Property %.500s of JSO instance <%.500s> of type <%.500s> is of wrong type <%.500s>, must be <%.500s>",
				    fldd->jsofd_local_name, inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri,
		...
		if (fld_in_radio)
		  {
			...
			dk_set_push (errors_log_ptr, list (2, inst_rtti,
				box_sprintf (2000,
				    "Property %.500s is set in JSO instance <%.500s> of type <%.500s> but that conflicts with some other properties of the instance",
				    fldd->jsofd_local_name, inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri)));
		      }
		    else
	...
	if (!is_midtree)
	  for (radio_ctr = 0; radio_ctr < JSO_RADIO_COUNT; radio_ctr++)
	      ...
	      if ((NULL != radio_is_declared[radio_ctr]) && (0 == radio_counters[radio_ctr]))
		{
		  ...
		  dk_set_push (errors_log_ptr, list (2, inst_rtti,
			  box_sprintf (2000,
			      "Neither property %.500s nor one of its alternatives is initialized in JSO instance <%.500s> of type <%.500s>",
			      radio_is_declared[radio_ctr]->jsofd_local_name, inst_rtti->jrtti_inst_iri,
			      inst_rtti->jrtti_class->jsocd_class_iri)));
		}
	    }
	break;
      }
...
bif_jso_validate (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  END_QR_RESET hash_table_free (known);
  ...
  DO_BOX_FAST (caddr_t *, wrnng, wrnng_ctr, report)
  {
    ...
    report[wrnng_ctr] =
	(caddr_t *) list (3, box_copy (wrnng_rtti->jrtti_class->jsocd_class_iri), box_copy (wrnng_rtti->jrtti_inst_iri), wrnng[1]);
...
bif_jso_validate_and_pin_batch (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
	...
	if (NULL != cli->cli_icc_lock)
	  {
		...
		"The client connection has %s the %s lock '%.500s', but jso_validate_and_pin_batch() should obtain ICC lock '%.500s' for JSO object <%.500s> of class <%.500s>",
  ...
  END_QR_RESET if (NULL == errors_log)
    report = (caddr_t **) revlist_to_array (warnings_log);
  else
    {
  ...
  DO_BOX_FAST (caddr_t *, msg, msg_ctr, report)
  {
    ...
    report[msg_ctr] =
	(caddr_t *) list (4, box_copy (msg_rtti->jrtti_class->jsocd_class_iri), box_copy (msg_rtti->jrtti_inst_iri),
	box_num (report_list_errors), msg[1]);
    dk_free_box ((caddr_t) msg);
  }
  END_DO_BOX_FAST;
  return (caddr_t) report;
...
bif_jso_set_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int do_set, const char *func_name)
{
  ...
  switch (cd->jsocd_cat)
	...
	if (IS_NUM_DTP (jprop_dtp))
	...
	else
	  {
		...
		!memcmp (jprop, RDF_NS_URI, RDF_NS_URI_LEN) && ('_' == jprop[RDF_NS_URI_LEN]))
	      {
		idx = atol (jprop + RDF_NS_URI_LEN + 1);
		if (idx <= 0)
		  {
		    sqlr_new_error ("22023", "SR516",
			"Invalid index 'rdf:%s' specified for instance %.500s of RDF array class <%.500s>", jprop + RDF_NS_URI_LEN,
			jinstance, jclass);
		  }
		idx--;
	      }
	      ...
	      {
		sqlr_new_error ("22023", "SR509",
		    "The instance %.500s of RDF class <%.500s> is an array and can not have property <%.500s>", jinstance, jclass,
		    jprop);
	      }
	  }
	...
	if (idx >= (ptrlong) (BOX_ELEMENTS (inst)))
	  {
		...
		sethash (new_inst, jso_rttis_of_structs, inst_rtti);
	      ...
	      {
		sqlr_new_error ("22023", "SR511",
		    "Index %ld specified for instance %.500s of RDF array class <%.500s> exceed array length %ld", (long) idx,
		    jinstance, jclass, BOX_ELEMENTS (inst));
	      }
	  }
    ...
    default:
      GPF_T1 ("jso_set_impl (): unknown value of jsocd_cat");
    }
  if (do_set)
    {
      ...
      if (NULL == fld_ptr[0])
	{
	  if (JSO_REQUIRED == fldd->jsofd_required)
	    sqlr_new_error ("22023", "SR498",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is mandatory but it has not been set", jprop,
		jinstance, jclass);
	  return NEW_DB_NULL;
	}
    }
  ...
  if (do_set)
      ...
      if (NULL != fld_type_cd)
	{
	  ...
	  value_iri =
	      (defarg_dtp ? box_dv_uname_string (defarg) : box_cast_to_UTF8_uname (qst, (bif_string_or_wide_or_uname_arg (qst, args,
			  3, func_name))));
	  value_rtti = (jso_rtti_t *) gethash (value_iri, jso_draft_rttis_of_names);
	  if (NULL == value_rtti)
	    {
	      sqlr_new_error ("22023", "SR512", "JSO instance IRI <%.500s> does not exist", value_iri);
	    }
      ...
      if (uname_xmlschema_ns_uri_hash_anyURI == dt)
	{
	  if (defarg_dtp && (DV_STRING != defarg_dtp) && (DV_UNAME != defarg_dtp))
	    sqlr_new_error ("22023", "SR519",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:anyURI, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if ((DV_STRING != arg_dtp) && (DV_UNAME != arg_dtp))
	    sqlr_new_error ("22023", "SR520",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:string, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
	  if (defarg_dtp)
      ...
      if (uname_xmlschema_ns_uri_hash_boolean == dt)
	{
	  if (defarg_dtp && (DV_LONG_INT != defarg_dtp))
	    sqlr_new_error ("22023", "SR517",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:boolean, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if (!IS_NUM_DTP (arg_dtp))
	    sqlr_new_error ("22023", "SR506",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:boolean, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
      ...
      if (uname_virtrdf_ns_uri_bitmask == dt)
	{
	  if (defarg_dtp && (DV_LONG_INT != defarg_dtp))
	    sqlr_new_error ("22023", "SR517",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:bitmask, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if (DV_LONG_INT != arg_dtp)
	    sqlr_new_error ("22023", "SR506",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:bitmask, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
      ...
      if (uname_xmlschema_ns_uri_hash_double == dt)
	{
	  if (defarg_dtp && (DV_DOUBLE_FLOAT != defarg_dtp))
	    sqlr_new_error ("22023", "SR517",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:double, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if (!IS_NUM_DTP (arg_dtp))
	    sqlr_new_error ("22023", "SR507",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:double, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
      ...
      if (uname_xmlschema_ns_uri_hash_integer == dt)
	{
	  if (defarg_dtp && (DV_LONG_INT != defarg_dtp))
	    sqlr_new_error ("22023", "SR517",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:integer, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if (!IS_NUM_DTP (arg_dtp))
	    sqlr_new_error ("22023", "SR505",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:integer, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
	...
	{
	  if (defarg_dtp && (DV_STRING != defarg_dtp) && (DV_UNAME != defarg_dtp))
	    sqlr_new_error ("22023", "SR521",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:string, can not set it to constant <%.500s> of type %s",
		jprop, jinstance, jclass, const_name, dv_type_title (defarg_dtp));
	  if (!IS_STRING_DTP (arg_dtp) && (DV_WIDE != arg_dtp))
	    sqlr_new_error ("22023", "SR522",
		"Property <%.500s> of instance <%.500s> of RDF class <%.500s> is an xsd:string, can not set it to %s", jprop,
		jinstance, jclass, dv_type_title (arg_dtp));
	  if (defarg_dtp)
  ...
  if (NULL == val)
    {
	  ...
	  (!strcmp (JSO_INTEGER, fld_type) || (!strcmp (JSO_ANY, fld_type) && (JSO_REQUIRED == fld_req))))
...
jso_dbg_text_fd_and_member_field (jso_field_descr_t * fd, void *inst_member_field)
{
  ...
  if (NULL == val)
    {
      if ((!strcmp (JSO_INTEGER, fld_type) || (!strcmp (JSO_ANY, fld_type) && (JSO_REQUIRED == fd->jsofd_required))))
  ...
  switch (DV_TYPE_OF (val))
    {
    case DV_LONG_INT:
      return box_sprintf (1000, "%.500s " BOXINT_FMT, fd->jsofd_local_name, unbox (val));
    case DV_STRING:
    ...
    default:
      return box_sprintf (1000, "%.500s /* box with tag %d */", fd->jsofd_local_name, DV_TYPE_OF (val));
    }
  return box_copy_tree (val);
}
...
jso_rtti_proplist (caddr_t iri, jso_rtti_t * rtti, void *acc_env)
{
  ...
  dk_set_push (&(acc->acc_set), list (3, box_copy (iri),
	  ...
	  box_sprintf (200, "%s%s%s%s%s%s", status_strg,
	      (((JSO_STATUS_LOADED == rtti->jrtti_status)
		      && (pinned_rtti_of_name == rtti)) ? "" : ", not key of jso_pinned_rttis_of_names"),
	      (((JSO_STATUS_LOADED == rtti->jrtti_status)
		      && (draft_rtti_of_name == rtti)) ? ", weird key of jso_draft_rttis_of_names" : ""),
	      (((JSO_STATUS_LOADED != rtti->jrtti_status)
		      && (draft_rtti_of_name == rtti)) ? "" : ", not key of jso_draft_rttis_of_names"),
	      (((JSO_STATUS_LOADED != rtti->jrtti_status)
		      && (pinned_rtti_of_name == rtti)) ? ", weird key of jso_pinned_rttis_of_names" : ""),
  ...
  dk_set_push (&(acc->acc_set), list (3, box_copy (iri), uname_rdf_ns_uri_type, box_copy_tree (cd->jsocd_class_iri)));
  switch (cd->jsocd_cat)
    {
    case JSO_CAT_STRUCT:
      {
	...
	for (ctr = cd->_.sd.jsosd_field_count; ctr--; /* no step */ )
	  {
	    ...
	    dk_set_push (&(acc->acc_set), list (3, box_copy (iri), box_copy (fldd->jsofd_property_iri), o));
	  }
	break;
      }
    case JSO_CAT_ARRAY:
	...
	DO_BOX_FAST (caddr_t, val, ctr, rtti->jrtti_self)
	{
	  ...
	  if (NULL != o)
	    dk_set_push (&(acc->acc_set), list (3, box_copy (iri), box_dv_uname_string (buf), o));
	}
	END_DO_BOX_FAST;
	break;
      }
    default:
      GPF_T1 ("jso_rtti_proplist(): bad jsocd_cat");
    }
}
  ...
  if (1 == BOX_ELEMENTS (args))
      ...
      else
	{
	  ...
	  if (NULL == inst_rtti)
	    {
	      if (acc.acc_only_loaded)
		sqlr_new_error ("22023", "SR515",
		    "JSO instance IRI <%.500s> does not exist (maybe it is being loaded but not yet finalized)", jinstance);
...
bif_jso_dbg_dump_rtti (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  res = box_sprintf (1000,
      ...
      (((JSO_STATUS_LOADED == rtti->jrtti_status)
	      && (pinned_rtti_of_name == rtti)) ? ", pinned rtti of name matches" : ", not key of jso_pinned_rttis_of_names"),
      (((JSO_STATUS_LOADED == rtti->jrtti_status)
	      && (draft_rtti_of_name == rtti)) ? ", weird key of jso_draft_rttis_of_names" : ""),
      (((JSO_STATUS_LOADED != rtti->jrtti_status)
	      && (draft_rtti_of_name == rtti)) ? ", draft rtti of name matches" : ", not key of jso_draft_rttis_of_names"),
      (((JSO_STATUS_LOADED != rtti->jrtti_status)
	      && (pinned_rtti_of_name == rtti)) ? ", weird key of jso_pinned_rttis_of_names" : ""),
      ((rtti_of_self == rtti) ? ", rtti of self matches" : ", not key of jso_rttis_of_structs"), rtti->jrtti_inst_iri,
      rtti->jrtti_class->jsocd_class_iri);
#else
...
jso_triple_add (caddr_t * qst, caddr_t jsubj, caddr_t jpred, caddr_t jobj)
{
  ...
  if (jso_objs != gethash (jsubj, jso_single_pred))
    GPF_T1
	("jso_triple_add(): gethash (jpred, gethash (jsubj, jso_triple_subjs)) != gethash (jsubj, gethash (jpred, jso_triple_preds))");
#endif
