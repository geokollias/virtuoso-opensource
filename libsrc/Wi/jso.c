		...
		if (NULL != fld_type_cd)
		  {
		    if (sub->jrtti_class->jsocd_class_iri != cd->_.ad.jsoad_member_type)
		      {
			SET_STATUS_FAILED;
			sqlr_new_error ("22023", "SR531",
			    "Item # %d of JSO array <%.500s> of type <%.500s> is of wrong type <%.500s>, must be <%.500s>", ctr,
			    inst_rtti->jrtti_inst_iri, inst_rtti->jrtti_class->jsocd_class_iri, sub->jrtti_class->jsocd_class_iri,
...
bif_jso_validate (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  DO_BOX_FAST (caddr_t *, wrnng, wrnng_ctr, report)
  {
    ...
    report[wrnng_ctr] =
	(caddr_t *) list (3, box_copy (wrnng_rtti->jrtti_inst_iri), box_copy (wrnng_rtti->jrtti_class->jsocd_class_iri), wrnng[1]);
    dk_free_box ((caddr_t) wrnng);
  }
  END_DO_BOX_FAST;
