...
caddr_t
get_view_name (utf8char * fullname, char sign)
{
  utf8char *hit;
  while (
...
#ifdef MS2003
caddr_t
form_prefix (caddr_t table_name)
{
  char str[60];
...
set_xj_pk (xv_join_elt_t * xj)	/*get primary key */
{
  if (xj->xj_table)
      ...
      if (!xj->xj_pk)
	{
	  ...
	  xj->xj_pk = (caddr_t *) dk_alloc_box_zero (pk->key_n_significant * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
...
extend_parent_xj_cols_by_parent_keys (xv_join_elt_t * parent_xj, caddr_t * parent_keys)
{
  ...
  _DO_BOX (inx, parent_keys)
  {
    ...
    new_col->xc_exp =
	(ST *) list (3, COL_DOTTED, NULL /* Not box_copy (parent_xj->xj_prefix) due to internal xmlsql errors */ ,
	box_string (p_name));
...
form_auto_element_join (xv_join_elt_t * element, xv_join_elt_t * child)
{
  ...
  {
    left = (ST *) list (3, COL_DOTTED, box_copy (element->xj_prefix), box_string (element->xj_pk[inx]));
    right = (ST *) list (3, COL_DOTTED, box_copy (child->xj_prefix), box_string (child->xj_pk[inx]));
    bin_exp_tmp = (ST *) list (4, BOP_EQ, left, right, NULL);
    if (inx == 0)
      bin_exp = bin_exp_tmp;
    else
...
/*              return;*/
	  ...
	  if (attr_component->cm_type.spec.attribute.ann.sql_field)
	    col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (attr_component->cm_type.spec.attribute.ann.sql_field));	/*column (sql) name of the attribute */
	  else
	    col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (component->cm_type.spec.cstype.composed_atts[idx].xa_name));	/*column (sql) name == xml name */
...
/*                  xv_child->xj_mp_schema->xj_parent_cols = dk_set_nreverse(xv_child->xj_mp_schema->xj_parent_cols);*/
		...
		col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (sub_component->cm_type.spec.element.ann.sql_field));	/*column (sql) name of the subelement */
	      else
		col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (xml_element_name));	/*column (sql) name == xml name */
...
/*              ST* limit_field_ref = (ST *) list (3, COL_DOTTED, box_copy (xv_child->xj_prefix),
	      ...
	      ST *col_ref;
	      if (sub_component->cm_type.spec.element.ann.sql_field)
		col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (sub_component->cm_type.spec.element.ann.sql_field));	/*column (sql) name of the attribute */
	      else
		col_ref = (ST *) list (3, COL_DOTTED, NULL, box_string (xml_element_name));	/*column (sql) name == xml name */
	      xv_child->xj_mp_schema->xj_column = col_ref;
	      xv_child->xj_mp_schema->xj_limit_field = (ST *) list (3, COL_DOTTED, NULL, box_string (sub_component->cm_type.spec.element.ann.sql_limit_field));	/*name of the limit-field */

	      xv_child->xj_mp_schema->xj_limit_value = IS_BOX_POINTER (sub_component->cm_type.spec.element.ann.sql_limit_value) ?
		  box_copy (sub_component->cm_type.spec.element.ann.sql_limit_value) : NULL;
  ...
  while (set)
    {
      ...
      if (!strcmp (ft->field, child_field_name) && !strcmp (ft->ref_table, parent_table)
	  && !strcmp (ft->ref_field, parent_field_name))
	return set;
      set = set->next;
    }
  return NULL;
      ...
      for (;;)
	{
	  ...
	  if (IS_BOX_POINTER (sub_component->cm_type.spec.element.ann.sql_relationship))	/*if the subelement from another table */
		...
		idx_type = unbox ((box_t) sub_component->cm_typename);
	      ...
	      if (sub_component->cm_type.spec.element.ann.sql_field)
		insert_new_field (&(table->fields), sub_component->cm_type.spec.element.ann.sql_field, &(table->fields_types), xml_sql_type[idx_type]);	/*column (sql) name of the subelement */
	      else
		insert_new_field (&(table->fields), xml_element_name, &(table->fields_types), xml_sql_type[idx_type]);	/*column (sql) name == xml name */
	      if (IS_BOX_POINTER (sub_component->cm_typename))	/*if subelement has attributes */
		attributes_analyze (tables, table, (xs_component_t *) sub_component->cm_typename);
	      if (IS_BOX_POINTER (sub_component->cm_typename))	/*if subelement has children */
		children_analyze (tables, table, (xs_component_t *) sub_component->cm_typename);
	  ...
	  if (IS_BOX_POINTER (sub_component->cm_type.spec.element.ann.sql_limit_field))
		...
		insert_new_field (&(table->fields), xml_element_name, &(table->fields_types), xml_sql_type[idx_type]);	/*column (sql) name == xml name */
	      ...
	      insert_new_field (&(table->fields), sub_component->cm_type.spec.element.ann.sql_limit_field, &(table->fields_types), "VARCHAR");	/*name of the limit-field */
	    }
