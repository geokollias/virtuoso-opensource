...
/* field for graph, subject and predicate */
  ...
  qmf->qmfBoolTmpl = box_dv_short_string (" sparql_ebv_int_of_sqlval (__ro2sq (^{alias-dot}^^{column}^))");
  qmf->qmfSparqlEbvTmpl = box_dv_short_string (" sparql_ebv_of_sqlval (__ro2sq (^{alias-dot}^^{column}^))");
  ...
  qmf->qmfBoolOfShortTmpl = box_dv_short_string (" sparql_ebv_int_of_obj (__ro2sq (^{tree}^))");
  qmf->qmfSparqlEbvOfShortTmpl = box_dv_short_string (" sparql_ebv_of_obj (__ro2sq (^{tree}^))");
  ...
  qmf->qmfShortOfTypedsqlvalTmpl =
      box_dv_short_string
      (" DB.DBA.RDF_MAKE_OBJ_OF_TYPEDSQLVAL (^{sqlval-of-tree}^, __i2idn (^{datatype-of-tree}^), ^{language-of-tree}^)");
  ...
  qm_format_default_ref->qmfSuperFormats =
      (qm_format_t **) list (3, qm_format_default_ref_nullable, qm_format_default, qm_format_default_nullable);
  qm_format_default_iri_ref->qmfSuperFormats =
      (qm_format_t **) list (5, qm_format_default_iri_ref_nullable, qm_format_default_ref, qm_format_default_ref_nullable,
      qm_format_default, qm_format_default_nullable);
  qm_format_default_ref_nullable->qmfSuperFormats = (qm_format_t **) list (1, qm_format_default_nullable);
  qm_format_default_iri_ref_nullable->qmfSuperFormats =
      (qm_format_t **) list (2, qm_format_default_ref_nullable, qm_format_default_nullable);
...
/*                         0123456789 */
	...
	switch (subtype)
	  ...
	  case DISTINCT_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_RSET_XML_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_RSET_XML_HTTP_PRE";
	    return;
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_DICT_XML_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_DICT_XML_HTTP_PRE";
	    return;
	  case ASK_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_XML";
	    return;
	  default:
	    return;
	  }
...
/*                         0123456789012 */
	...
	switch (subtype)
	  ...
	  case COUNT_DISTINCT_L:
	  case DISTINCT_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_RSET_RDFXML_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_RSET_RDFXML_HTTP_PRE";
	    return;
#if 0
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_DICT_RDFXML_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_DICT_RDFXML_HTTP_PRE";
	    return;
	  case ASK_L:
	    goto bad_ask;	/* see below */
#else
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_RDF_XML";
	    return;
	  case ASK_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_RDF_XML";
	    return;
#endif
	  default:
	    return;
...
/*                         0123456789 */
	...
	switch (subtype)
	  ...
	  case COUNT_DISTINCT_L:
	  case DISTINCT_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_RSET_TTL_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_RSET_TTL_HTTP_PRE";
	    return;
#if 0
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_DICT_TTL_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_DICT_TTL_HTTP_PRE";
	    return;
	  case ASK_L:
	    goto bad_ask;	/* see below */
#else
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_TTL";
	    return;
	  case ASK_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_TTL";
	    return;
#endif
	  default:
	    return;
...
/*                         012345678 */
	...
	switch (subtype)
	  ...
	  case COUNT_DISTINCT_L:
	  case DISTINCT_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_RSET_NT_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_RSET_NT_HTTP_PRE";
	    return;
#if 0
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_agg_formatter[0] = "DB.DBA.SPARQL_DICT_N3_HTTP";
	    ret_agg_mdata[0] = "DB.DBA.SPARQL_DICT_N3_HTTP_PRE";
	    return;
	  case ASK_L:
	    goto bad_ask;	/* see below */
#else
	  case CONSTRUCT_L:
	  case DESCRIBE_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_NT";
	    return;
	  case ASK_L:
	    ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_NT";
	    return;
#endif
	  default:
	    return;
  ...
  if (!strcmp (name, "RDF/XML"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_RDF_XML";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_RDF_XML";
  ...
  if (!strcmp (name, "TURTLE") || !strcmp (name, "TTL"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_TTL";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_TTL";
  ...
  if (!strcmp (name, "NICE_TTL"))
	...
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_NICE_TTL";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_TTL";
	return;
  ...
  if (!strcmp (name, "TRIG"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_TRIG";
	return;
      default:
	return;
      }
  if (!strcmp (name, "NT"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_NT";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_NT";
  ...
  if (!strcmp (name, "JSON"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_JSON";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_TALIS_JSON";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_JSON";
	return;
      default:
	return;
      }
  if (!strcmp (name, "RDFA;XHTML"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_RDFA_XHTML";
	return;
      default:
	return;
      }
  if (!strcmp (name, "HTML;MICRODATA"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_HTML_MICRODATA";
	return;
      default:
	return;
      }
  if (!strcmp (name, "HTML;NICE_MICRODATA"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_HTML_NICE_MICRODATA";
	return;
      default:
	return;
      }
  if (!strcmp (name, "HTML;NICE_TTL"))
    switch (subtype)
      {
      case SELECT_L:
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_HTML_NICE_TTL";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_HTML_NICE_TTL";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_HTML_NICE_TTL";
	return;
      default:
	return;
      }
  if (!strcmp (name, "JSON;MICRODATA"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_JSON_MICRODATA";
	return;
      default:
	return;
      }
  ...
  if (!strcmp (name, "ATOM;XML"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_ATOM_XML";
	return;
      default:
	return;
      }
  if (!strcmp (name, "JSON;ODATA"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_ODATA_JSON";
	return;
      default:
	return;
      }
  if (!strcmp (name, "JSON;LD"))
    switch (subtype)
      ...
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_JSON_LD";
	return;
      default:
	return;
      }
  if (!strcmp (name, "CXML"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_CXML";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_CXML";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_CXML";
	return;
      default:
	return;
      }
  if (!strcmp (name, "CXML;QRCODE"))
	...
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_CXML_QRCODE";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_CXML";
	return;
  ...
  if (!strcmp (name, "CSV"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_CSV";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_CSV";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_CSV";
	return;
      default:
	return;
      }
  if (!strcmp (name, "TSV"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_TSV";
	return;
      case CONSTRUCT_L:
      case DESCRIBE_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_TRIPLE_DICT_AS_TSV";
	return;
      case ASK_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_BOOL_RESULT_AS_CSV";
	return;
  ...
  if (!strcmp (name, "BINDINGS"))
    switch (subtype)
      ...
      case COUNT_DISTINCT_L:
      case DISTINCT_L:
	ret_formatter[0] = "DB.DBA.RDF_FORMAT_RESULT_SET_AS_BINDINGS";
	return;
      default:
	return;
      }
...
ssg_free_internals (spar_sqlgen_t * ssg)
{
  ...
  if (((NULL != ssg->ssg_out) && (NULL != ssg->ssg_parent_ssg) && (ssg->ssg_out != ssg->ssg_parent_ssg->ssg_out)) ||
      ((NULL != ssg->ssg_out) && (NULL == ssg->ssg_parent_ssg)))
    {
      strses_free (ssg->ssg_out);
      ssg->ssg_out = NULL;
    }
  if (((NULL != ssg->ssg_sd_used_namespaces) && (NULL != ssg->ssg_parent_ssg)
	  && (ssg->ssg_sd_used_namespaces != ssg->ssg_parent_ssg->ssg_sd_used_namespaces)) || ((NULL != ssg->ssg_sd_used_namespaces)
	  && (NULL == ssg->ssg_parent_ssg)))
    {
...
ccaddr_t
ssg_id_of_gp_or_triple (SPART * tree)
{
  switch (tree->type)
    {
    case SPAR_GP:
      return tree->_.gp.selid;
    case SPAR_TRIPLE:
      return tree->_.triple.tabid;
    default:
      spar_internal_error (NULL, "ssg_id_of_gp_or_triple(): bad arg");
    }
  return NULL;
}

ccaddr_t
ssg_find_jl_by_jr (spar_sqlgen_t * ssg, SPART * gp, ccaddr_t jr_alias)
{
  int ctr;
...
ssg_prin_subalias (struct spar_sqlgen_s *ssg, const char *alias, const char *colalias, int dot_after_alias)
{
  if ((NULL != ssg->ssg_alias_to_search) &&
      !strcmp (((NULL == colalias) ? "" : colalias), ((NULL == ssg->ssg_alias_to_search) ? "" : ssg->ssg_alias_to_search)))
    {
...
int
ssg_print_tmpl_phrase (struct spar_sqlgen_s *ssg, qm_format_t * qm_fmt, const char *tmpl, const char *tmpl_end, ccaddr_t alias,
    qm_value_t * qm_val, SPART * tree, int col_idx, const char *asname)
{
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("alias-dot", 9))
	{
	  ...
	  if (1 != BOX_ELEMENTS (qm_val->qmvColumns))
	    {
	      if ((2 < BOX_ELEMENTS (qm_val->qmvColumns))
		  || strcmp (qm_val->qmvColumns[0]->qmvcAlias, qm_val->qmvColumns[1]->qmvcAlias))
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{alias-dot}^ if qm_val has many columns", asname_printed);
	    }
	  if (col_idx >= 0)
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{alias-dot}^ inside a loop, should be ^{column-N}^",
		asname_printed);
	  colalias = qm_val->qmvColumns[0]->qmvcAlias;
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("alias-N-dot", 11))
	{
	  ...
	  if (col_idx < 0)
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{alias-N-dot}^ outside a loop, should be ^{alias-dot}^",
		asname_printed);
...
/*                        012345678901234567890 */
	...
	{
	  if (col_idx >= 0)
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{as-name}^ inside a loop, should be ^{as-name-N}^",
		asname_printed);
	  if (IS_BOX_POINTER (asname))
	    {
	      ssg_puts_with_comment (" AS", "as-name");
	      ssg_putchar (' ');
	  ...
	  if (IS_BOX_POINTER (asname))
	    {
	      ...
	      ssg_puts_with_comment (" AS", "as-name-0");
	      ssg_putchar (' ');
	      ssg_prin_id (ssg, buf);
	      asname_printed = 1;
	    }
	  ...
	  if (IS_BOX_POINTER (asname))
	    {
	      ...
	      ssg_puts_with_comment (" AS", "as-name-1");
	      ssg_putchar (' ');
	      ssg_prin_id (ssg, buf);
	      asname_printed = 1;
	    }
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("as-name-dt", 10))
	{
	  ...
	  if (IS_BOX_POINTER (asname))
	    {
	      ssg_puts_with_comment (" AS", "as-name-dt");
	      ssg_putchar (' ');
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("as-name-lang", 12))
	{
	  ...
	  if (IS_BOX_POINTER (asname))
	    {
	      ssg_puts_with_comment (" AS", "as-name-lang");
	      ssg_putchar (' ');
...
/*                        012345678901234567890 */
	...
	{
	  if (col_idx < 0)
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{as-name-N}^ outside a loop, should be ^{as-name}^",
		asname_printed);
	  if (IS_BOX_POINTER (asname))
	    {
	      char buf[60];
	      snprintf (buf, sizeof (buf), "%s~%d", asname, col_idx);
	      ssg_puts_with_comment (" AS", "as-name-N");
	      ssg_putchar (' ');
	      ssg_prin_id (ssg, buf);
	      asname_printed = 1;
	    }
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("column", 6))
	{
	  ...
	  if (col_idx >= 0)
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{column}^ inside a loop, should be ^{column-N}^",
		asname_printed);
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("tree", 4))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree}^ in qmfShortTmpl: infinite recursion",
		    asname_printed);
      ...
      else if (CMD_EQUAL ("tree-0", 6))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-0}^ if qm_fmt is NULL", asname_printed);
	      if (tmpl == qm_fmt->qmfShortTmpl)
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-0}^ in qmfShortTmpl: infinite recursion",
		    asname_printed);
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("tree-1", 6))
	{
	  if (!(IS_BOX_POINTER (qm_fmt) && (2 <= qm_fmt->qmfColumnCount)))
	    spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-1}^ if qm_fmt is not short or have less than 2 cols",
		asname_printed);
	  if (NULL == tree)
	    {
	      if (NULL == qm_fmt)
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-1}^ if qm_fmt is NULL", asname_printed);
	      if (tmpl == qm_fmt->qmfShortTmpl)
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-1}^ in qmfShortTmpl: infinite recursion",
		    asname_printed);
      ...
      else if (CMD_EQUAL ("tree-N", 6))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-N}^ if qm_fmt is NULL", asname_printed);
	      if (tmpl == qm_fmt->qmfShortTmpl)
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-N}^ in qmfShortTmpl: infinite recursion",
		    asname_printed);
      ...
      else if (CMD_EQUAL ("tree-as-name", 12))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-as-name}^ if qm_fmt is NULL", asname_printed);
	      if (tmpl == qm_fmt->qmfShortTmpl)
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-as-name}^ in qmfShortTmpl: infinite recursion",
		    asname_printed);
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("tree-dt", 7))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-dt}^ in qmfDatatypeOfShortTmpl: infinite recursion",
		    asname_printed);
...
/*                        012345678901234567890 */
      else if (CMD_EQUAL ("tree-lang", 9))
	{
	  if (NULL == tree)
		...
		spar_sqlprint_error2 ("ssg_" "print_tmpl(): can't use ^{tree-lang}^ in qmfLanguageOfShortTmpl: infinite recursion",
		    asname_printed);
...
/*                         012345678901234567890 */
      else if (CMD_EQUAL ("custom-string-1", 15))
	{
	  if ((NULL == qm_fmt) || (NULL == qm_fmt->qmfCustomString1))
	    spar_sqlprint_error2 ("ssg_"
		"print_tmpl(): can't use ^{custom-string-1}^ if qm_fmt is NULL or qmfCustomString1 is not set", asname_printed);
...
/*                         012345678901234567890 */
      else if (CMD_EQUAL ("custom-verbatim-1", 17))
	{
	  if ((NULL == qm_fmt) || (NULL == qm_fmt->qmfCustomString1))
	    spar_sqlprint_error2 ("ssg_"
		"print_tmpl(): can't use ^{custom-verbatim-1}^ if qm_fmt is NULL or qmfCustomString1 is not set", asname_printed);
...
/*                         012345678901234567890 */
      else if (CMD_EQUAL ("N-aref-of-spfinv", 16))
	{
	  ...
	  if ((NULL == qm_fmt) || (NULL == qm_fmt->qmfCustomString1))
	    spar_sqlprint_error2 ("ssg_"
		"print_tmpl(): can't use ^{N-aref-of-spfinv}^ if qm_fmt is NULL or qmfCustomString1 is not set", asname_printed);
	  if (NULL == tree)
	  ...
	  res =
	      (caddr_t *) sprintf_inverse_ex ((caddr_t *) ssg->ssg_sparp->sparp_sparqre->sparqre_qi, &err, str,
	      qm_fmt->qmfCustomString1, 2, (unsigned char *) qm_fmt->qmfArgDtps);
	  dk_free_tree (err);
...
/*                         012345678901234567890 */
      else if (CMD_EQUAL ("URIQADefaultHost", 16))
	{
	  ...
	  LEAVE_TXN;
	  if (NULL == subst)
	    spar_sqlprint_error2
		("Unable to use ^{URIQADefaultHost}^ in IRI template if DefaultHost is not specified in [URIQA] section of Virtuoso config",
		asname_printed);
	  ssg_puts (subst);
	}
      else
...
void
ssg_print_tmpl (struct spar_sqlgen_s *ssg, qm_format_t * qm_fmt, ccaddr_t tmpl, ccaddr_t alias, qm_value_t * qm_val, SPART * tree,
    const char *asname)
{
  const char *tmpl_end;
  int asname_printed;
  if (NULL == tmpl)
    spar_internal_error (ssg->ssg_sparp,
	"Uninitialized template detected; it's probably due to incomplete declaration of some quad mapping data format");
#ifdef DEBUG
...
/*                                       012345678901234567890 */
	  ...
	  if (in_list)
	    spar_error (sparp,
		"Template string contains macro ^{%.100s}^ inside a list (say, between ^{comma-list-begin}^ and ^{end}^",
		t_box_dv_short_nchars (cmd, cmdlen));
	  if (after_list)
	    spar_error (sparp, "Template string contains more than one list (like ^{%.100s}^ ... ^{end}^)",
		t_box_dv_short_nchars (cmd, cmdlen));
	  in_list = 1;
	  goto inloop_cmd;
	}
...
/*                         012345678901234567890 */
    ...
    inloop_cmd:
      if (!in_list)
	spar_error (sparp, "Macro ^{%.100s}^ can be used only inside a list (say, between ^{comma-list-begin}^ and ^{end}^",
	    t_box_dv_short_nchars (cmd, cmdlen));
    col_cmd:
      if (!qmv_known)
	spar_error (sparp, "Macro ^{%.100s}^ can be used only in format strings, not in table conditions",
	    t_box_dv_short_nchars (cmd, cmdlen));
      continue;
    ...
    noloop_tbl_cmd:
      if (in_list)
	spar_error (sparp, "Macro ^{%.100s}^ can not be used inside a list (say, between ^{comma-list-begin}^ and ^{end}^",
	    t_box_dv_short_nchars (cmd, cmdlen));
      continue;
...
sparp_equiv_native_valmode (sparp_t * sparp, SPART * gp, sparp_equiv_t * eq)
{
	  ...
	  for (ctr = BOX_ELEMENTS (gp->_.gp.options); 1 < ctr; ctr -= 2)
	      ...
	      if (T_STEP_L == ((ptrlong) (gp->_.gp.options[ctr - 2])) && !strcmp (val->_.alias.aname, varname))
		{
		  ...
		  if (SPAR_VARIABLE == SPART_TYPE (expn))
		    {
		      retval =
			  sparp_find_subexpn_in_retlist (sparp, expn->_.var.vname, gp->_.gp.subquery->_.req_top. /*orig_ */ retvals,
			  1);
		      if (NULL == retval)
  ...
  if ((UNION_L == gp->_.gp.subtype) || (SPAR_UNION_WO_ALL == gp->_.gp.subtype))
	...
	if ((SPART_VARR_FIXED & member_eq->e_rvr.rvrRestrictions) &&
	    (SPART_VARR_NOT_NULL & member_eq->e_rvr.rvrRestrictions) && (SPART_VARR_IS_REF & member_eq->e_rvr.rvrRestrictions))
	  {
...
#endif
      if (var_miss_in_some_members && IS_BOX_POINTER (smallest_union))
  ...
  if (!(eq->e_rvr.rvrRestrictions & SPART_VARR_NOT_NULL) && IS_BOX_POINTER (res)
      && (res->qmfValRange.rvrRestrictions & SPART_VARR_NOT_NULL))
  ...
  switch (SPART_TYPE (tree))
...
#endif
	      spar_error (sparp,
		  "SPARQL optimizer can not generate SQL code for variable ?%.200s at line %ld of query, the variable can be misused",
		  tree->_.var.vname, (long) unbox (tree->srcline));
	    }
...
#endif
	  tr_valmode = triple->_.triple.native_formats[tr_idx];
	  if ((SPART_VARR_IS_REF & tree->_.var.restr_of_col) && IS_BOX_POINTER (tr_valmode)
	      && (NULL != tr_valmode->qmfSubFormatForRefs))
	    {
	      sparp_jso_validate_format (sparp, tr_valmode);
      ...
      if (NULL != tree->_.retval.tabid)
	{
	  ...
	  if (SSG_VALMODE_AUTO == tr_valmode)
		...
		return SSG_VALMODE_BOOL;
	      if ((rvr & SPART_VARR_IS_REF)
		  || ((rvr & (SPART_VARR_IS_LIT | SPART_VARR_LONG_EQ_SQL)) == (SPART_VARR_IS_LIT | SPART_VARR_LONG_EQ_SQL)))
		return SSG_VALMODE_SQLVAL;
	      return SSG_VALMODE_LONG;
	    }
	  return tr_valmode;
      ...
      else
	{
	  ...
	  if (SPART_BAD_EQUIV_IDX == tree->_.retval.equiv_idx)
	    {
	      eq = sparp_equiv_get_ro (sparp->sparp_sg->sg_equivs, sparp->sparp_sg->sg_equiv_count, gp, tree,
...
sparp_set_valmodes_of_t_inouts (sparp_t * sparp, sparp_t * sub_sparp, SPART * wrapping_gp)
{
  ...
  for (v_ctr = BOX_ELEMENTS_0 (t_in_vars); v_ctr--; /*no step */ )
	...
	spar_error (sparp, "Variable ?%.100s in T_IN list is made from %d database columns, should be made from exactly one",
	    in_vname, (int) (in_vmode->qmfColumnCount));
	...
	spar_error (sparp, "Variable ?%.100s in T_IN list is made from %d database columns, should be made from exactly one",
	    out_vname, (int) (out_vmode->qmfColumnCount));
	...
	spar_error (sparp,
	    "Variable ?%.100s in T_IN list and corresponding variable ?%.100s in T_OUT get values from columns that are too different",
	    in_vname, out_vname);
...
ptrlong
sparp_restr_bits_of_dtp (dtp_t dtp)
{
  switch (dtp)
    {
    case DV_ANY:
...
sparp_restr_bits_of_expn (sparp_t * sparp, SPART * tree)
{
  switch (SPART_TYPE (tree))
    ...
    case BOP_NOT:
	  ...
	  (sparp_restr_bits_of_expn (sparp, tree->_.bin_exp.left) & SPART_VARR_NOT_NULL));
    case BOP_EQ:
    case SPAR_BOP_EQNAMES:
    case SPAR_BOP_EQ_NONOPT:
    case BOP_NEQ:
    case BOP_LT:
    case BOP_LTE:
    case BOP_GT:
    ...
    case BOP_OR:
	  ...
	  (sparp_restr_bits_of_expn (sparp, tree->_.bin_exp.left) &
	      sparp_restr_bits_of_expn (sparp, tree->_.bin_exp.right) & SPART_VARR_NOT_NULL));
    case BOP_PLUS:
    case BOP_MINUS:
    case BOP_TIMES:
    case BOP_DIV:
    case BOP_MOD:
	  ...
	  (sparp_restr_bits_of_expn (sparp, tree->_.bin_exp.left) &
	      sparp_restr_bits_of_expn (sparp, tree->_.bin_exp.right) & SPART_VARR_NOT_NULL));
    case SPAR_LIT:
      return sparp_restr_bits_of_dtp ((DV_ARRAY_OF_POINTER ==
	      DV_TYPE_OF (tree)) ? DV_TYPE_OF (tree->_.lit.val) : DV_TYPE_OF (tree));
    case SPAR_QNAME:
      /*case SPAR_QNAME_NS: */
	...
	switch (tree->_.builtin.btype)
	  ...
	  case BOUND_L:
		...
		case SPAR_BIF_ISURI:
		case SPAR_BIF_ISBLANK:
		  if ((arg_bits & SPART_VARR_IS_IRI) || (arg_bits & SPART_VARR_IS_BLANK) || (arg_bits & SPART_VARR_ALWAYS_NULL))
		    return (res_bits | SPART_VARR_FIXED);
		  /* no break; */
		case SPAR_BIF_ISREF:
		case SPAR_BIF_ISLITERAL:
		  if ((arg_bits & SPART_VARR_IS_REF) || (arg_bits & SPART_VARR_IS_LIT)
		      || (arg_bits & SPART_VARR_IS_IRI) || (arg_bits & SPART_VARR_IS_BLANK) || (arg_bits & SPART_VARR_ALWAYS_NULL))
		    return (res_bits | SPART_VARR_FIXED);
		  break;
		case SPAR_BIF_ISNUMERIC:
	  ...
	  case SPAR_BIF_EBV_INT:
		...
		return (SPART_VARR_ALWAYS_NULL);
	      if ((SPART_VARR_FIXED | SPART_VARR_NOT_NULL) == (arg_bits & (SPART_VARR_FIXED | SPART_VARR_NOT_NULL)))
		return (SPART_VARR_IS_LIT | SPART_VARR_IS_BOOL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_FIXED | SPART_VARR_NOT_NULL);
	      if (SPART_VARR_NOT_NULL & arg_bits)
		return (SPART_VARR_IS_LIT | SPART_VARR_IS_BOOL | SPART_VARR_LONG_EQ_SQL | SPART_VARR_NOT_NULL);
	      return SPART_VARR_IS_LIT | SPART_VARR_IS_BOOL | SPART_VARR_LONG_EQ_SQL;
	    }
	  case SPAR_BIF_CASEWHEN:
	  case SPAR_BIF_CASEX:
	    {
	  ...
	  case SPAR_BIF__ITEM_IN_VECTOR:
		...
		(sparp_restr_bits_of_expn (sparp, tree->_.builtin.args[0]) &
		    sparp_restr_bits_of_expn (sparp, tree->_.builtin.args[1]) & SPART_VARR_NOT_NULL));
	  default:
	    return res_bits;
	  }
      }
	...
	if (uname_SPECIAL_cc_bif_c_AVG == qname && (1 == BOX_ELEMENTS (tree->_.funcall.argtrees)))
	...
	if (uname_SPECIAL_cc_bif_c_SUM == qname && (1 == BOX_ELEMENTS (tree->_.funcall.argtrees)))
	  return (SPART_VARR_IS_LIT | SPART_VARR_LONG_EQ_SQL |
    ...
    default:
      spar_internal_error (sparp, "sparp_" "restr_bits_of_expn(): unsupported case");
    }
...
ssg_print_box_as_sql_atom (spar_sqlgen_t * ssg, ccaddr_t box, int mode)
{
  ...
  switch (dtp)
	...
	case SQL_ATOM_SPARQL_INTEROP:
	  sqlc_string_virtuoso_literal (tmpbuf, buflen, &buffill, box);
	  break;
	default:
	  spar_internal_error (ssg->ssg_sparp, "ssg_" "print_box_as_sql_atom (): bad mode");
	}
      break;
    case DV_WIDE:
      ssg_puts ("N");
	...
	if (1.0 > ((2 - 1.41484755040568800000e+16) + 1.41484755040568800000e+16))
	  spar_error (ssg->ssg_sparp,
	      "Platform-specific error: this build of Virtuoso does not support literals of type %s due to rounding errors in math functions",
	      dv_type_title (dtp));
		...
		if (isalpha (tmpbuf[6 + 1]))
		  {
		    ...
		    if (myPOSINF_d == boxdbl)
		      buffill = sprintf (tmpbuf, "cast ('Inf'");
		    else if (myNEGINF_d == boxdbl)
		      buffill = sprintf (tmpbuf, "cast ('-Inf'");
		    else
		      buffill = sprintf (tmpbuf, "cast ('nan'");
		  }
		else
		  {
		...
		if (isalpha (tmpbuf[1]))
		  {
		    ...
		    double myNEGINF_d = -1.0 / myZERO;
		    if (myPOSINF_d == boxdbl)
		      buffill = sprintf (tmpbuf, "cast ('Inf' as double precision)");
		    else if (myNEGINF_d == boxdbl)
		      buffill = sprintf (tmpbuf, "cast ('-Inf' as double precision)");
		    else
		      buffill = sprintf (tmpbuf, "cast ('NaN' as double precision)");
		  }
		else
		  {
		    strcpy (tmpbuf + buffill, ".0e0");
  ...
  if ((NULL != type) && (NULL == lang))
    {
      ...
      if (box_is_plain_num)
	{
	  if ((DV_ARRAY_OF_POINTER == lit_dtp) && (NULL != lit->_.lit.original_text) && ((DV_DOUBLE_FLOAT == DV_TYPE_OF (value))
		  || DV_SINGLE_FLOAT == DV_TYPE_OF (value) || DV_NUMERIC == DV_TYPE_OF (value)))
	    {
	      ssg_print_float_literal_as_original (ssg, lit);
	      return;
...
ssg_print_literal_as_long (spar_sqlgen_t * ssg, SPART * lit)
{
  ...
  if ((DV_STRING == value_dtp) || (DV_WIDE == value_dtp) ||
      (DV_BLOB_HANDLE == value_dtp) || (DV_UNAME == value_dtp) || (DV_XML_ENTITY == value_dtp))
    {
...
ssg_print_equiv (spar_sqlgen_t * ssg, caddr_t selectid, sparp_equiv_t * eq, ccaddr_t asname)
{
  ...
  if (IS_BOX_POINTER (asname) && ((NULL == name_as_expn) || strcmp (asname, name_as_expn)))
    {
      ssg_puts_with_comment (" AS", "equiv");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
}
...
ssg_print_tr_var_expn (spar_sqlgen_t * ssg, SPART * var, ssg_valmode_t needed, const char *asname)
{
  ...
  if (IS_BOX_POINTER (native) && (native->qmfValRange.rvrRestrictions & SPART_VARR_NOT_NULL) &&
	...
	spar_error (ssg->ssg_sparp,
	    "Unable to compose SQL text because quad map format %.200s has no version that could handle NULL values",
	    native->qmfName);
      native = nullable_native;
    }
  if (IS_BOX_POINTER (needed) && IS_BOX_POINTER (native) && ssg_valmode_is_subformat_of (needed, native))
	  ...
	  if (IS_BOX_POINTER (asname))
	    {
	      ...
	      ssg_puts_with_comment (" AS", "tr_var_expn");
	      ssg_putchar (' ');
	      ssg_prin_id (ssg, buf);
	    }
	}
...
ssg_smallest_union_valmode (ssg_valmode_t m1, ssg_valmode_t m2, int *cheap_ret)
{
  ...
  if (((SSG_VALMODE_LONG == m1) && !(cheap_ret[0] & 0x2))
      ...
      || (SSG_VALMODE_SHORT_OR_LONG == m1) || (SSG_VALMODE_SHORT_OR_LONG == m2))
    cheap_ret[0] = 0;
  if (m2 == m1)
    return m1;
  if (m2 < m1)
...
ssg_largest_eq_valmode (ssg_valmode_t m1, ssg_valmode_t m2)
{
  ...
  if (m2 == m1)
    {
      if (!IS_BOX_POINTER (m1))
	return m1;
	  /* !!!TBD: SSG_VALMODE_AUTO may be bad for LONG and LONG due to an error when native valmode is LONG but the printed expn is short */
	  ;
  ...
  if (!(m1->qmfIsBijection) || !(m2->qmfIsBijection))
    {
      if ((SPART_VARR_IS_REF & m1->qmfValRange.rvrRestrictions) && (SPART_VARR_IS_REF & m2->qmfValRange.rvrRestrictions))
	return SSG_VALMODE_SQLVAL;
      if ((SPART_VARR_IS_LIT & m1->qmfValRange.rvrRestrictions) && (SPART_VARR_IS_LIT & m2->qmfValRange.rvrRestrictions))
	return SSG_VALMODE_SQLVAL;
      return SSG_VALMODE_LONG;
    }
  DO_BOX_FAST (qm_format_t *, sup1, ctr1, m1->qmfSuperFormats)
...
ssg_const_is_good_for_split_into_short (spar_sqlgen_t * ssg, SPART * tree, int tree_is_qname, ssg_valmode_t fmt)
{
  ...
  switch (tree_type)
	...
	spar_internal_error (ssg->ssg_sparp,
	    "ssg_" "const_is_good_for_split_into_short(): NULL == qmfShortOfUriTmpl, so there are corrupted metadata");
	...
	spar_internal_error (ssg->ssg_sparp,
	    "ssg_" "const_is_good_for_split_into_short(): NULL == qmfShortOfSqlvalTmpl, so there are corrupted metadata");
...
void
ssg_print_bop_bool_expn (spar_sqlgen_t * ssg, SPART * tree, const char *bool_op, const char *sqlval_fn, int top_filter_op,
...
/* Valmode of global expressions does not really matter because they're calculated only once, hence the comparison prefers valmode of the non-global expression argument */
  if (!bop_is_comparison && ((SSG_VALMODE_LONG == right_vmode) || (SSG_VALMODE_SQLVAL == right_vmode)) &&	/* case (SSG_VALMODE_LONG == right_vmode) happens for IRI(?::0) and the like */
      ((SSG_VALMODE_LONG == left_vmode) ||
	  (IS_BOX_POINTER (left_vmode) && left_vmode->qmfIsBijection)) && sparp_tree_is_global_expn (ssg->ssg_sparp, right))
    {
	...
	right_restr_bits = sparp_restr_bits_of_expn (ssg->ssg_sparp, right);
	  ...
	  (SPART_VARR_IS_REF & right_restr_bits) && !sparp_tree_is_global_expn (ssg->ssg_sparp, left))
	{
	  min_mode = left_vmode;
	  goto vmodes_found;	/* see below */
	}
  ...
  if (!bop_is_comparison && (SSG_VALMODE_SQLVAL == left_vmode) &&
	  ...
	  (IS_BOX_POINTER (right_vmode) && right_vmode->qmfIsBijection)) && sparp_tree_is_global_expn (ssg->ssg_sparp, left))
    {
...
void
ssg_print_bop_cmp_expn (spar_sqlgen_t * ssg, SPART * tree, const char *bool_op, const char *sqlval_fn, int top_filter_op,
    ssg_valmode_t needed)
{
  SPART *left = tree->_.bin_exp.left;
  SPART *right = tree->_.bin_exp.right;
  ...
  if ((needed != SSG_VALMODE_SQLVAL) &&	/* Trick! All representations are equal. */
      (needed != SSG_VALMODE_LONG) && (needed != SSG_VALMODE_BOOL))
...
#endif
  if ((SSG_VALMODE_LONG == op_arg_valmode) || (IS_BOX_POINTER (op_arg_valmode) && op_arg_valmode->qmfIsSubformatOfLong))
    {
...
ssg_safe_op_valmode_for_bif_IN (sparp_t * sparp, SPART ** args, ssg_valmode_t arg1_native)
{
  ...
  if (SSG_VALMODE_LONG == op_fmt)
	...
	if (IS_BOX_POINTER (argN_native) &&
	  ...
	  continue;
	if ((IS_BOX_POINTER (argN_native) && argN_native->qmfIsSubformatOfLong) || (SSG_VALMODE_LONG == argN_native))
	  {
...
ssg_print_builtin_expn (spar_sqlgen_t * ssg, SPART * tree, int top_filter_op, ssg_valmode_t needed, const char *asname)
{
  ...
  if (SSG_VALMODE_BOOL == sbd->sbd_ret_valmode)
    {
      int needed_is_like_bool = ((SSG_VALMODE_AUTO == needed) || (SSG_VALMODE_BOOL == needed) ||
	  ((0 == ssg->ssg_sparp->sparp_ebv_mode) &&
	      ((SSG_VALMODE_LONG == needed) || (SSG_VALMODE_SQLVAL == needed) ||
		  (SSG_VALMODE_NUM == needed) || (SSG_VALMODE_SHORT_OR_LONG == needed))));
      if (!needed_is_like_bool)
	{
  ...
  switch (tree->_.builtin.btype)
	...
	if (IS_BOX_POINTER (arg1_native) && arg1_native->qmfMapsOnlyNullToNull)
	  {
	    switch (arg1_native->qmfColumnCount)
	      {
	      case 0:
		ssg_puts_with_comment (" 1", "zero cols");
		break;
	      case 1:
	...
	expanded = spartlist (ssg->ssg_sparp, 3, BOP_AND,
		...
		spartlist (ssg->ssg_sparp, 3, BOP_OR,
			...
			spartlist (ssg->ssg_sparp, 3, BOP_NOT,
			    sparp_make_builtin_call (ssg->ssg_sparp, BOUND_L, (SPART **) t_list (1,
				    sparp_make_builtin_call (ssg->ssg_sparp, LANG_L, (SPART **) t_list (1, arg2)))), NULL)))));
	...
	if ((SPAR_LIT == SPART_TYPE (arg2)) && (DV_STRING == DV_TYPE_OF (SPAR_LIT_VAL (arg2))))
	  ...
	  {
	    ssg_puts (" cast (");
	    ssg_print_scalar_expn (ssg, arg2, SSG_VALMODE_SQLVAL, NULL_ASNAME);
	    ssg_puts (" as varchar))");
	  }
	goto print_asname;
      }
	...
	if (union_valmode != needed)
	...
	else
	  {
		...
		SPART *argwhen = tree->_.builtin.args[argctr];
		arg_cmp_valmode =
		    ssg_smallest_union_valmode (arg_cmp_valmode, sparp_expn_native_valmode (ssg->ssg_sparp, argwhen),
		    &sqlval_is_ok_and_cheap);
	      }
	...
	switch (argctr)
	  {
	  case 0:
	    ssg_puts (top_filter_op ? " (" : " ( one_of_these (");
	    break;
	  case 1:
	    ssg_puts (top_filter_op ? " in (" : ", ");
	    break;
	  default:
	    ssg_puts (", ");
	  }
	  ...
	  && (SSG_VALMODE_LONG != needed))
	{
      ...
      if ((SSG_VALMODE_NUM == arg1_native) || (SSG_VALMODE_BOOL == arg1_native))
	{
	  ssg_puts_with_comment (" 0", "optimized isBLANK of num or bool");
	  goto print_asname;
	}
      arg1_restr_bits = sparp_restr_bits_of_expn (ssg->ssg_sparp, arg1);
	...
	ssg_print_tmpl (ssg, arg1_native,
		...
		" either (isiri_id (^{tree}^), gte (^{tree}^, min_bnode_iri_id ()), 0)"), NULL, NULL, arg1, NULL_ASNAME);
	  ...
	  && (SSG_VALMODE_LONG != needed))
	{
      ...
      if ((SSG_VALMODE_NUM == arg1_native) || (SSG_VALMODE_BOOL == arg1_native))
	{
	  ssg_puts_with_comment (" 0", "optimized isIRI of num or bool");
	  goto print_asname;
	}
      arg1_restr_bits = sparp_restr_bits_of_expn (ssg->ssg_sparp, arg1);
      if (arg1_restr_bits & (SPART_VARR_IS_LIT | SPART_VARR_ALWAYS_NULL | SPART_VARR_CONFLICT))
	...
	ssg_print_tmpl (ssg, arg1_native,
	    (top_filter_op ?
		" (isiri_id (^{tree}^) and (^{tree}^ < min_bnode_iri_id ()))" :
		" either (isiri_id (^{tree}^), lt (^{tree}^, min_bnode_iri_id ()), 0)"), NULL, NULL, arg1, NULL_ASNAME);
      else if (SSG_VALMODE_SQLVAL == arg1_native)
	  ...
	  && (SSG_VALMODE_LONG != needed))
	{
      ...
      if ((SSG_VALMODE_NUM == arg1_native) || (SSG_VALMODE_BOOL == arg1_native))
	{
	  ssg_puts_with_comment (" 0", "optimized isREF of num or bool");
	  goto print_asname;
	}
      arg1_restr_bits = sparp_restr_bits_of_expn (ssg->ssg_sparp, arg1);
      if (arg1_restr_bits & (SPART_VARR_IS_LIT | SPART_VARR_ALWAYS_NULL | SPART_VARR_CONFLICT))
	...
	{
	  if (arg1_restr_bits & SPART_VARR_NOT_NULL)
	    ssg_print_tmpl (ssg, arg1_native, " isnotnull (^{tree}^)", NULL, NULL, arg1, NULL_ASNAME);
	  else
	    ssg_puts_with_comment (" 1", "optimized isLITERAL of num or bool");
	  goto print_asname;
	}
      if (IS_BOX_POINTER (arg1_native))
	{
	...
	ssg_print_tmpl (ssg, arg1_native,
	    (top_filter_op ? " (not (isiri_id (^{tree}^)))" : " iszero (isiri_id (^{tree}^))"), NULL, NULL, arg1, NULL_ASNAME);
      else if (SSG_VALMODE_SQLVAL == arg1_native)
      ...
      if (IS_BOX_POINTER (arg1_native))
	{
      ...
      else if ((SSG_VALMODE_SQLVAL == arg1_native) || (SSG_VALMODE_NUM == arg1_native) || (SSG_VALMODE_BOOL == arg1_native))
	ssg_print_tmpl (ssg, arg1_native, " isnumeric (^{tree}^)", NULL, NULL, arg1, NULL_ASNAME);
      else if (SSG_VALMODE_LONG == arg1_native)
	ssg_print_tmpl (ssg, arg1_native, " isnumeric (rdf_box_data (__ro2sq (^{tree}^)))", NULL, NULL, arg1, NULL_ASNAME);
      else
	...
	if ((SSG_VALMODE_SQLVAL == needed) || (SSG_VALMODE_LONG == needed))
	  {
	    if (IS_BOX_POINTER (arg1_native))
	      tmpl = arg1_native->qmfSparqlEbvOfShortTmpl;
	    else if ((SSG_VALMODE_LONG == arg1_native) || (SSG_VALMODE_SQLVAL == arg1_native) || (SSG_VALMODE_AUTO == arg1_native))
	      tmpl = " sparql_ebv_of_sqlval (__ro2sq (^{tree}^))";	/* SSG_VALMODE_AUTO is here as a fallback for a query optimized down to an empty select */
	    else if (SSG_VALMODE_NUM == arg1_native)
	      tmpl = " case (^{tree}^) when null then null when 0 then sparql_ebv_pure (0) else sparql_ebv_pure (1) end";
	    else if (SSG_VALMODE_BOOL == arg1_native)
	      tmpl = " sparql_ebv_pure (^{tree}^)";
	    else
	      spar_sqlprint_error ("ssg_" "print_builtin_expn(): bad native type for EBV()");
	  }
	else if (SSG_VALMODE_DATATYPE == needed)
	  {
	    arg1_restr_bits = sparp_restr_bits_of_expn (ssg->ssg_sparp, arg1);
	    if (arg1_restr_bits & SPART_VARR_NOT_NULL)
	      {
		ssg_puts (" UNAME'" XMLSCHEMA_NS_URI "#boolean" "'");
		goto print_asname;
	      }
	    tmpl = "case (^{tree}^) when null then null else UNAME'" XMLSCHEMA_NS_URI "#boolean" "' end";
	  }
	else if (SSG_VALMODE_LANGUAGE == needed)
	  {
	    ssg_puts (" NULL");
	...
	else if ((SSG_VALMODE_BOOL == needed) || (SSG_VALMODE_NUM == needed))
	  {
	    if (IS_BOX_POINTER (arg1_native))
	      tmpl = arg1_native->qmfBoolOfShortTmpl;
	    else if ((SSG_VALMODE_LONG == arg1_native) || (SSG_VALMODE_SQLVAL == arg1_native) || (SSG_VALMODE_AUTO == arg1_native))
	      tmpl = " sparql_ebv_int_of_sqlval (__ro2sq (^{tree}^))";	/* SSG_VALMODE_AUTO is here as a fallback for a query optimized down to an empty select */
	    else if (SSG_VALMODE_NUM == arg1_native)
	      tmpl = " case (^{tree}^) when null then null when 0 then 0 else 1 end";
	    else if (SSG_VALMODE_BOOL == arg1_native)
	      {
		ssg_print_scalar_expn (ssg, arg1, SSG_VALMODE_BOOL, asname);
		return;
	      }
	    else
	      spar_sqlprint_error ("ssg_" "print_builtin_expn(): bad native type for EBV()");
	...
	if ((SSG_VALMODE_SQLVAL == needed) || (SSG_VALMODE_LONG == needed) || (SSG_VALMODE_NUM == needed)
	...
	else if (SSG_VALMODE_DATATYPE == needed)
	  {
	    ...
	    if (arg1_restr_bits & SPART_VARR_NOT_NULL)
	      {
		ssg_puts (" UNAME'" XMLSCHEMA_NS_URI "#integer" "'");
		goto print_asname;
	      }
	    tmpl = "case (^{tree}^) when null then null else UNAME'" XMLSCHEMA_NS_URI "#integer" "' end";
	  }
	else if (SSG_VALMODE_LANGUAGE == needed)
	  {
	    ssg_puts (" NULL");
	...
	switch (argctr)
	  ...
	  case 0:
	    ...
	    break;
	  case 1:
	    ssg_puts (top_filter_op ? " in /* =item-in-vector */ (" : ", ");
	    break;
	  default:
	    ssg_puts (", ");
	  }
	...
	switch (sbd->sbd_implementation)
	  ...
	  case 'B':
	    ...
	    break;
	  case 'S':
	    ssg_puts (" DB.DBA.rdf_");
	    break;
	  case '-':
	    switch (sbd->sbd_subtype)
	      {
	      case SPAR_BIF__CUBE:
	      case SPAR_BIF__GROUPING_LIST:
	      ...
	      case TOP_L:
		{
		  ...
		  switch (all_or_distinct)
		    ...
		    case 0:
		      break;
		    default:
		      spar_internal_error (ssg->ssg_sparp, "ssg_print_builtin_expn(): neither ALL nor DISTINCT in TOP");
		    }
		  if (NULL != from_n)
		    {
		      ssg_puts (" TOP (");
		  ...
		  switch (with_ties)
		    ...
		    case 0:
		      break;
		    default:
		      spar_internal_error (ssg->ssg_sparp, "ssg_print_builtin_expn(): something other than WITH TIES in TOP");
		    }
		  goto cant_have_asname;	/* see below */
		}
		/* no break */
	...
	DO_BOX_FAST (SPART *, arg, argctr, tree->_.builtin.args)
	{
	  ...
	  arg_valmode =
	      ((argctr <
		  (sizeof (sbd->sbd_arg_valmodes) / sizeof (sbd->sbd_arg_valmodes[0]))) ? sbd->sbd_arg_valmodes[argctr] : NULL);
	  if (NULL == arg_valmode)
...
print_asname:
  if (IS_BOX_POINTER (asname))
    {
      ssg_puts_with_comment (" AS", "builtin");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
  return;
...
function_is_xqf_str_parser (caddr_t name)
{
  long desc_idx;
  ...
  if (':' != name[0])
    return NULL;
  name++;
  desc_idx = ecm_find_name (name, xqf_str_parser_descs_ptr, xqf_str_parser_desc_count, sizeof (xqf_str_parser_desc_t));
...
/*                           0         1      */
/*                           0123456789012345 */
  if (!strncmp (name_begin, "SQLVAL::", 8))
    return SSG_VALMODE_SQLVAL;
  if (!strncmp (name_begin, "LONG::", 6))
    return SSG_VALMODE_LONG;
  if (!strncmp (name_begin, "BOOL::", 6))
    return SSG_VALMODE_BOOL;
  if (!strncmp (name_begin, "NUM::", 5))
    return SSG_VALMODE_NUM;
  if (!strncmp (name_begin, "SHORT::", 7))
    return qm_format_default;
  if (!strncmp (name_begin, "SHORT_REF::", 11))
    return qm_format_default_iri_ref;
  if (!strncmp (name_begin, "SPECIAL::", 9))
    return SSG_VALMODE_SPECIAL;
  if (!strncmp (name_begin, "SHORT_OR_LONG::", 15))
...
valmode_is_found:
  if (SSG_VALMODE_SPECIAL == res)
	...
	return SSG_VALMODE_NUM;
      if (!strcmp (name, "SPECIAL::bif:__rgs_assert_cbk") || !strcmp (name, "SPECIAL::bif:__rgs_assert"))
	return SSG_VALMODE_SQLVAL;
      if (!strcmp (name, "SPECIAL::bif:__rgs_ack_cbk") ||
  ...
  if (NULL != xsd_name)
    {
      if (!strcmp (xsd_name, "boolean"))
	return SSG_VALMODE_BOOL;
      if (!strcmp (xsd_name, "date"))
	return SSG_VALMODE_NUM;
      if (!strcmp (xsd_name, "dateTime"))
	return SSG_VALMODE_NUM;
      if (!strcmp (xsd_name, "decimal"))
	return SSG_VALMODE_NUM;
      if (!strcmp (xsd_name, "double"))
	return SSG_VALMODE_NUM;
      if (!strcmp (xsd_name, "float"))
	return SSG_VALMODE_NUM;
      if (!strcmp (xsd_name, "string"))
	return SSG_VALMODE_LONG;
      if (!strcmp (xsd_name, "time"))
	return SSG_VALMODE_NUM;
    }
  if (!strncmp (name, "xpath:", 6))
    {
...
ssg_prin_function_name (spar_sqlgen_t * ssg, ccaddr_t name)
{
  ...
  else if (!strncmp (name, "sql:", 4))
    {
      name = name + 4;
      ssg_puts ("DB.DBA.");
      ssg_puts (name);		/*not ssg_prin_id (ssg, name); */
    }
  else if (!strcmp (name, "_STAR"))
    {
      ssg_puts_comment ("expansion of '*' argument:");
    }
  else if ('\0' != name[0])
    {
      ssg_puts ("DB.DBA.");
...
const char *
ssg_tmpl_literal_short_of_X (qm_format_t * qm_fmt, ssg_valmode_t native)
{
  if (SSG_VALMODE_LONG == native)
    return qm_fmt->qmfShortOfLongTmpl;
  if (SSG_VALMODE_SQLVAL == native)
    return qm_fmt->qmfShortOfSqlvalTmpl;
...
const char *
ssg_tmpl_ref_short_of_X (qm_format_t * qm_fmt, ssg_valmode_t native)
{
  if (SSG_VALMODE_LONG == native)
    return qm_fmt->qmfShortOfLongTmpl;
  if (SSG_VALMODE_SQLVAL == native)
    return qm_fmt->qmfShortOfUriTmpl;
...
const char *
ssg_tmpl_X_of_Y (spar_sqlgen_t * ssg, ssg_valmode_t needed, ssg_valmode_t native)
{
  if (SSG_VALMODE_LONG == needed)
    {
      if (SSG_VALMODE_LONG == native)
	return SSG_IDENTITY_VALMODED_TMPL;
      if (SSG_VALMODE_SQLVAL == native)
	return " DB.DBA.RDF_LONG_OF_SQLVAL (^{tree}^)";
      if (SSG_VALMODE_NUM == native)
	return SSG_IDENTITY_VALMODED_TMPL;
      if (SSG_VALMODE_BOOL == native)
	return ssg->ssg_sparp->sparp_ebv_mode ? " sparql_ebv_of_sqlval (^{tree}^)" : SSG_IDENTITY_VALMODED_TMPL;
    }
  else if (SSG_VALMODE_SQLVAL == needed)
    {
      if (SSG_VALMODE_LONG == native)
	return " __rdf_sqlval_of_obj /*l*/ (^{tree}^)";
      if (SSG_VALMODE_SQLVAL == native)
	return SSG_IDENTITY_VALMODED_TMPL;
      if (SSG_VALMODE_NUM == native)
	return SSG_IDENTITY_VALMODED_TMPL;
    ...
    {
      if (SSG_VALMODE_LONG == native)
	return " DB.DBA.RDF_DATATYPE_OF_LONG (^{tree}^)";
      if (SSG_VALMODE_SQLVAL == native)
	return " DB.DBA.RDF_DATATYPE_OF_SQLVAL (^{tree}^)";
      if (SSG_VALMODE_NUM == native)
	return " DB.DBA.RDF_DATATYPE_OF_SQLVAL (^{tree}^)";
      if (SSG_VALMODE_BOOL == native)
	return " UNAME" XMLSCHEMA_NS_URI "#boolean";
    }
  else if (SSG_VALMODE_LANGUAGE == needed)
    {
      if (SSG_VALMODE_LONG == native)
	return " DB.DBA.RDF_LANGUAGE_OF_LONG (^{tree}^)";
      if (SSG_VALMODE_SQLVAL == native)
	return " DB.DBA.RDF_LANGUAGE_OF_SQLVAL (^{tree}^)";
      if (SSG_VALMODE_NUM == native)
	return " NULL";
      if (SSG_VALMODE_BOOL == native)
	return " NULL";
    ...
    {
      if (SSG_VALMODE_LONG == native)
	return " sparql_ebv_int_of_sqlval (__ro2sq (^{tree}^))";
      if (SSG_VALMODE_SQLVAL == native)
	return " sparql_ebv_int_of_sqlval (^{tree}^)";
      if (SSG_VALMODE_NUM == native)
	return SSG_IDENTITY_VALMODED_TMPL;
    }
  else if (SSG_VALMODE_NUM == needed)
	...
	return SSG_IDENTITY_VALMODED_TMPL;
      if (SSG_VALMODE_BOOL == native)
	return " sparql_ebv_int_of_sqlval (^{tree}^)";
      return ((native->qmfIsSubformatOfLong
	      || native->qmfIsSubformatOfLongWhenEqToSql) ? SSG_IDENTITY_VALMODED_TMPL : native->qmfLongOfShortTmpl);
    }
...
ssg_print_valmoded_scalar_expn (spar_sqlgen_t * ssg, SPART * tree, ssg_valmode_t needed, ssg_valmode_t native, const char *asname)
{
  ...
  if (SSG_VALMODE_BOOL == native)
      ...
      if ((SSG_VALMODE_SQLVAL == needed) || (SSG_VALMODE_LONG == needed) || (SSG_VALMODE_SHORT_OR_LONG == needed))
	{
	  if (ssg->ssg_sparp->sparp_ebv_mode)
	    goto use_standard_template;	/* see below */
  ...
  if (((SSG_VALMODE_LONG == native) && (SSG_VALMODE_SQLVAL == needed)) ||
  ...
  if (IS_BOX_POINTER (native))
	  ...
	  /* checked above: IS_BOX_POINTER (native) && */ (native->qmfValRange.rvrRestrictions & SPART_VARR_NOT_NULL))
	{
	  ...
	  if (!IS_BOX_POINTER (nullable_native))
	    spar_error (ssg->ssg_sparp,
		"Unable to compose SQL text for ?%.200s because quad map format %.200s has no version that could handle NULL values",
		tree->_.retval.vname, native->qmfName);
	  native = nullable_native;
	}
      if (IS_BOX_POINTER (needed))
	{
      ...
      if ((SSG_VALMODE_SHORT_OR_LONG == needed) &&
	  (!strcmp (native->qmfLongOfShortTmpl, " __ro2lo (^{tree}^)") || !strcmp (native->qmfLongOfShortTmpl, " ^{tree}^")))
	{
	  ssg_print_scalar_expn (ssg, tree, SSG_VALMODE_AUTO, asname);
	  return;
	}
  ...
  if ((SSG_VALMODE_BOOL == needed) && (SPAR_FUNCALL == SPART_TYPE (tree)))
    {
      ...
      if (!strcmp (name, "LONG::bif:position") || !strcmp (name, "bif:position"))
	{
	  ssg_print_scalar_expn (ssg, tree, SSG_VALMODE_AUTO, asname);
	  return;
	}
	  ...
	  if ((DV_UNAME == DV_TYPE_OF (v)) || (DV_STRING == DV_TYPE_OF (v)))
	    {
	      ...
	      int ctr = 20;
	      while ((0 < ctr--) && (v < t) && (isalnum ((unsigned char) (t[-1]))))
		t--;
	      tail = t;
	    }
...
ssg_print_bool_of_plain_box (spar_sqlgen_t * ssg, caddr_t val)
{
  switch (DV_TYPE_OF (val))
    {
    case DV_LONG_INT:
      ssg_puts (unbox (val) ? " 1" : " 0");
      ssg_puts_comment ("int as bool");
      break;
    case DV_SINGLE_FLOAT:
      ssg_puts (unbox_float (val) ? " 1" : " 0");
      ssg_puts_comment ("real as bool");
      break;
    case DV_DOUBLE_FLOAT:
      ssg_puts (unbox_double (val) ? " 1" : " 0");
      ssg_puts_comment ("double as bool");
      break;
    case DV_NUMERIC:
      ssg_puts (num_is_zero ((numeric_t) (val)) ? " 1" : " 0");
      ssg_puts_comment ("decimal as bool");
      break;
    case DV_DATETIME:
    case DV_XML_ENTITY:
    case DV_GEO:
      ssg_puts (" 1");
      ssg_puts_comment ("atom as bool");
      break;
    case DV_STRING:
    case DV_BIN:
      ssg_puts ((1 < box_length (val)) ? " 1" : " 0");
      ssg_puts_comment ("string as bool");
      break;
    case DV_WIDE:
      ssg_puts ((sizeof (wchar_t) < box_length (val)) ? " 1" : " 0");
      ssg_puts_comment ("wide as bool");
      break;
    default:			/* case DV_IRI_ID: case DV_UNAME: case DV_DB_NULL: */
      ssg_puts (" NULL");
      ssg_puts_comment ("unusual atom as bool");
      break;
    }
}

void
ssg_print_star_retvals_by_selid (spar_sqlgen_t * ssg, caddr_t selid)
{
  int needs_comma, eq_ctr, eq_count;
  ssg_puts (" (");
  ssg->ssg_indent++;
  eq_count = SPARP_EQUIV_COUNT (ssg->ssg_sparp);
  for (eq_ctr = 0; eq_ctr < eq_count; eq_ctr++)
    {
      sparp_equiv_t *eq = SPARP_EQUIV (ssg->ssg_sparp, eq_ctr);
      if ((NULL == eq) || eq->e_deprecated || strcmp (eq->e_gp->_.gp.selid, selid))
	continue;
      if (needs_comma)
	ssg_putchar (',');
      else
	needs_comma = 1;
      ssg_print_equiv (ssg, selid, eq, NULL_ASNAME);
    }
  if (!needs_comma)
    ssg_puts ("1");
  ssg->ssg_indent--;
...
ssg_print_scalar_expn (spar_sqlgen_t * ssg, SPART * tree, ssg_valmode_t needed, const char *asname)
{
  ...
  switch (SPART_TYPE (tree))
    {
    case BOP_AND:
      ssg_print_bop_bool_expn (ssg, tree, " AND ", " __and (", 0, needed);
      goto print_asname;
    case BOP_OR:
      ssg_print_bop_bool_expn (ssg, tree, " OR ", " __or (", 0, needed);
      goto print_asname;
    case SPAR_BOP_EQ_NONOPT:	/* no break */
    case BOP_EQ:
      ssg_print_bop_bool_expn (ssg, tree, " = ", " equ (", 0, needed);
      goto print_asname;
    case BOP_NEQ:
      ssg_print_bop_bool_expn (ssg, tree, " <> ", " neq (", 0, needed);
      goto print_asname;
    case BOP_LT:
      ssg_print_bop_bool_expn (ssg, tree, " < ", " lt (", 0, needed);
      goto print_asname;
    case BOP_LTE:
      ssg_print_bop_bool_expn (ssg, tree, " <= ", " lte (", 0, needed);
      goto print_asname;
    case BOP_GT:
      ssg_print_bop_bool_expn (ssg, tree, " > ", " gt (", 0, needed);
      goto print_asname;
    case BOP_GTE:
      ssg_print_bop_bool_expn (ssg, tree, " >= ", " gte (", 0, needed);
      goto print_asname;
      /*case BOP_LIKE: Like is built-in in SPARQL, not a BOP!
...
*/
    ...
    case BOP_PLUS:
      ssg_print_bop_calc_expn (ssg, tree, " (", " + ", ")", needed);
      goto print_asname;
    case BOP_MINUS:
      ssg_print_bop_calc_expn (ssg, tree, " (", " - ", ")", needed);
      goto print_asname;
    ...
    case BOP_DIV:
      ...
      goto print_asname;
    case BOP_MOD:
      ssg_print_bop_calc_expn (ssg, tree, " mod (", ", ", ")", needed);
      goto print_asname;
    case SPAR_ALIAS:
	...
	if (NULL == ssg->ssg_equivs)	/* This is for case when parts of the SPARQL front-end are used to produce small SQL fragments */
	...
	else
	  {
	    ...
	    if (SPART_BAD_EQUIV_IDX == eq_idx)
	      spar_error (ssg->ssg_sparp, "Unable to use variable ?%.100s in OPTION () clause, try to rephrase the query",
		  tree->_.var.vname);
	    eq = ssg->ssg_equivs[eq_idx];
	    gp = eq->e_gp;
	    ssg_print_equiv_retval_expn (ssg, gp, eq,
		SSG_RETVAL_FROM_JOIN_MEMBER | SSG_RETVAL_MUST_PRINT_SOMETHING | SSG_RETVAL_CAN_PRINT_NULL | SSG_RETVAL_USES_ALIAS,
		needed, asname);
	  }
	return;
      }
	...
	if ((tree->_.conv.needed == needed) ||
		...
		IS_BOX_POINTER (needed) && ssg_valmode_is_subformat_of (tree->_.conv.needed, needed)))
	...
	if (NULL != parser_desc)
	  {
	    ...
	    else if (ssg->ssg_sparp->sparp_ebv_mode && !strcmp (cvtname, "__xqf_str_parse_boolean"))
	      {
		ssg_puts (" sparql_ebv_of_sqlval (");
		ssg->ssg_indent++;
		ssg_print_scalar_expn (ssg, tree->_.funcall.argtrees[0], SSG_VALMODE_SQLVAL, NULL_ASNAME);
		ssg->ssg_indent--;
		ssg_putchar (')');
...
#endif
	ssg_putchar (' ');
	bigtext = (		/*(NULL != strstr (tree->_.funcall.qname, "bif:")) ||
	...
	for (arg_ctr = 0; arg_ctr < arg_count; arg_ctr++)
	  {
	    ...
	    if (bigtext || curr_arg_is_long || prev_arg_is_long)
	      ssg_newline (0);
	    else
	      ssg_putchar (' ');
	...
	if (SSG_VALMODE_NUM == needed)
	  {
		...
		desc_idx =
		    ecm_find_name (p_name, xqf_str_parser_descs_ptr, xqf_str_parser_desc_count, sizeof (xqf_str_parser_desc_t));
		if (ECM_MEM_NOT_FOUND == desc_idx)
		...
		END_QR_RESET ssg_print_box_as_sql_atom (ssg, parsed_value, SQL_ATOM_UNAME_ALLOWED);
	...
	if (SSG_VALMODE_BOOL == needed)
	  {
	    ...
	    if ((DV_STRING == DV_TYPE_OF (litvalue)) && (NULL != littype)
		&& (rb_uname_to_flags_of_parseable_datatype (littype) & RDF_TYPE_PARSEABLE))
	...
	if (IS_BOX_POINTER (needed))
	  {
	    ...
	    if ((DV_STRING == DV_TYPE_OF (litvalue)) && ((NULL != littype) || (NULL != litlang)))
	      ssg_print_tmpl (ssg, needed, needed->qmfShortOfTypedsqlvalTmpl, NULL, NULL, tree, asname);
	    else if ((0 != needed->qmfDtpOfNiceSqlval) &&
		(needed->qmfDtpOfNiceSqlval == DV_TYPE_OF (litvalue)) && (NULL != needed->qmfShortOfNiceSqlvalTmpl))
...
print_asname:
  if (IS_BOX_POINTER (asname))
    {
      ssg_puts_with_comment (" AS", "scalar");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
}
...
void
ssg_print_procview_rset_item (spar_sqlgen_t * ssg, caddr_t selid, int pos, const char *colname, int view_cn_trick,
    caddr_t e_varname)
{
  char buf[50];
  ssg_puts_comment ("procview[");
  ssg_putchar (' ');
  if (-1 == pos)
    ssg_puts ("NULL");
  else
...
ssg_print_retval (spar_sqlgen_t * ssg, SPART * tree, ssg_valmode_t vmode, const char *asname)
{
  ...
  if (NULL == eq->e_front_varname)
    ...
    spar_internal_error (ssg->ssg_sparp,
	"ssg_"
	"print_retval(): interesting: strcmp (eq->e_front_varname, eq->e_varnames[0]) for SSG_RETVAL_FROM_ANY_SELECTED in UNION_L gp");
#endif
  ...
  if (NULL != tree->_.retval.gp)
	...
	{
	  char buf[20];
	  sprintf (buf, ":binv_%ld", (long) (tree->_.retval.equiv_idx));
	  ssg_puts (buf);
	  goto print_asname;	/* see below */
	}
	...
	{
	  int pos =
	      sparp_find_sinv_rset_or_param_pos_of_varname (ssg->ssg_sparp, tree->_.retval.gp, e_varname,
	      0 /* search in result-set */ );
...
print_asname:
  if (IS_BOX_POINTER (asname))
    {
      ssg_puts_with_comment (" AS", "retval");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
}
...
ssg_print_filter_expn (spar_sqlgen_t * ssg, SPART * tree)
{
  ...
  switch (SPART_TYPE (tree))
    {
    case BOP_AND:
      ssg_print_bop_bool_expn (ssg, tree, " AND ", " __and (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_OR:
      ssg_print_bop_bool_expn (ssg, tree, " OR ", " __or (", 1, SSG_VALMODE_BOOL);
      return;
    case SPAR_BOP_EQ_NONOPT:	/* no break */
    case BOP_EQ:
      ssg_print_bop_bool_expn (ssg, tree, " = ", " equ (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_NEQ:
      ssg_print_bop_bool_expn (ssg, tree, " <> ", " neq (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_LT:
      ssg_print_bop_cmp_expn (ssg, tree, " < ", " lt (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_LTE:
      ssg_print_bop_cmp_expn (ssg, tree, " <= ", " lte (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_GT:
      ssg_print_bop_cmp_expn (ssg, tree, " > ", " gt (", 1, SSG_VALMODE_BOOL);
      return;
    case BOP_GTE:
      ssg_print_bop_cmp_expn (ssg, tree, " >= ", " gte (", 1, SSG_VALMODE_BOOL);
      return;
/*case BOP_LIKE: Like is built-in in SPARQL, not a BOP!
...
*/
    case BOP_NOT:
      ssg_puts (" not (");
      ssg_print_filter_expn (ssg, tree->_.bin_exp.left);
      ssg_putchar (')');
      return;
    case SPAR_BUILT_IN_CALL:
      {
...
void
ssg_print_fld_lit_restrictions (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid, SPART * triple,
    int fld_idx, int print_outer_filter)
{
...
void
ssg_print_fld_uri_restrictions (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid, caddr_t uri, SPART * gp,
    SPART * triple, int fld_idx, int print_outer_filter)
{
  SPART_buf rv_buf;
  SPART *rv = NULL;
  ptrlong field_restr = field->qmvFormat->qmfValRange.rvrRestrictions;
  ...
  if (field->qmvFormat->qmfIsBijection)
      ...
      for (col_ctr = 0; col_ctr < col_count; col_ctr++)
	{
	  ...
	  if (NULL == split)
	    ...
	    {
	      int is_wide =
		  (NULL == arg_dtps) ? 0 : ((col_ctr >= box_length (arg_dtps)) ? 0 : ((arg_dtps[col_ctr] & 0x7F) ==
		      (DV_WIDE & 0x7F)));
  ...
  if (!(SPART_VARR_IS_REF & field_restr) ||
      (strlen (field->qmvFormat->qmfStrsqlvalOfShortTmpl) > strlen (field->qmvFormat->qmfLongOfShortTmpl)))
    {
...
void
ssg_print_fld_var_restrictions_ex (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid, SPART * fld_tree,
    SPART * triple, SPART * fld_if_outer, rdf_val_range_t * rvr, ptrlong restr_bits_to_ignore)
{
  sparp_env_t *env = ssg->ssg_sparp->sparp_env;
  ptrlong field_restr = field->qmvFormat->qmfValRange.rvrRestrictions;
      ...
      for (;;)
	{
	  ...
	  if ((SPAR_VARIABLE == graph_tree_type) || (SPAR_BLANK_NODE_LABEL == graph_tree_type))
	      ...
	      if (SPART_VARR_GLOBAL & graph_tree_restr)
		{
	      ...
	      ssg_print_tmpl (ssg, graph_qmv->qmvFormat, graph_qmv->qmvFormat->qmfIidOfShortTmpl, tabid, graph_qmv, NULL,
		  NULL_ASNAME);
	      break;
	    }
	  ssg_puts ("NULL");
...
ssg_print_retval_restrictions_ex (spar_sqlgen_t * ssg, SPART * retval, rdf_val_range_t * rvr)
{
  ...
  if (((SPART_VARR_NOT_NULL & tree_restr) ||
	  ((SPART_VARR_IS_REF & tree_restr) && (!(SPART_VARR_IS_REF & retval_restr)))) && (!(SPART_VARR_NOT_NULL & retval_restr)))
    {
      ssg_print_where_or_and (ssg, "nullable retval is not null");
      ssg_print_scalar_expn (ssg,
	  sparp_make_builtin_call (ssg->ssg_sparp, BOUND_L, (SPART **) t_list (1, retval)), SSG_VALMODE_BOOL, NULL_ASNAME);
    }
  if ((SPART_VARR_IS_BLANK & tree_restr) && (!(SPART_VARR_IS_BLANK & retval_restr)))
    {
      ssg_print_where_or_and (ssg, "retval is blank node");
      ssg_print_scalar_expn (ssg,
	  sparp_make_builtin_call (ssg->ssg_sparp, SPAR_BIF_ISBLANK, (SPART **) t_list (1, retval)), SSG_VALMODE_BOOL, NULL_ASNAME);
    }
  else if ((SPART_VARR_IS_IRI & tree_restr) && (!(SPART_VARR_IS_IRI & retval_restr)))
    {
      ssg_print_where_or_and (ssg, "retval is IRI");
      ssg_print_scalar_expn (ssg,
	  sparp_make_builtin_call (ssg->ssg_sparp, SPAR_BIF_ISIRI, (SPART **) t_list (1, retval)), SSG_VALMODE_BOOL, NULL_ASNAME);
    }
  else if ((SPART_VARR_IS_REF & tree_restr) && (!(SPART_VARR_IS_REF & retval_restr)))
    {
      ssg_print_where_or_and (ssg, "'any' retval is a reference");
      ssg_print_scalar_expn (ssg,
	  sparp_make_builtin_call (ssg->ssg_sparp, SPAR_BIF_ISREF, (SPART **) t_list (1, retval)), SSG_VALMODE_BOOL, NULL_ASNAME);
    }
...
void
ssg_print_fld_restrictions (spar_sqlgen_t * ssg, quad_map_t * qmap, qm_value_t * field, caddr_t tabid, SPART * gp, SPART * triple,
    int fld_idx, int print_outer_filter)
{
  SPART *fld_tree = triple->_.triple.tr_fields[fld_idx];
  ...
  switch (fld_tree_type)
	...
	if (SPART_VARR_CONFLICT & tree_restr)
	  {
	    ssg_print_where_or_and (ssg, t_box_sprintf (100, "conflict on ?%.50s! The query remained not entirely optimized",
		    fld_tree->_.var.vname));
	    ssg_puts (" 0");
	    return;
	  }
	restr_bits_to_ignore = (((NULL != field)
		&& field->qmvFormat->qmfIsBijection) ? ssg_restr_bits_derived_for_var_from_equiv_qm_vals (ssg, field->qmvFormat,
		fld_tree) : 0);
	ssg_print_fld_var_restrictions_ex (ssg, qmap, field, tabid, fld_tree, triple, fld_if_outer, &(fld_tree->_.var.rvr),
	    restr_bits_to_ignore);
	return;
      }
    default:
...
int
ssg_print_equiv_retval_expn (spar_sqlgen_t * ssg, SPART * gp, sparp_equiv_t * eq, int flags, ssg_valmode_t needed,
    const char *asname)
{
  caddr_t name_as_expn = NULL;
  ...
  if ((flags & SSG_RETVAL_OPTIONAL_MAKES_NULLABLE) &&
	...
	spar_error (ssg->ssg_sparp,
	    "Unable to compose SQL text for ?%.200s because quad map format %.200s has no version that could handle NULL values",
	    eq->e_varnames[0], needed->qmfName);
  ...
  if (SPAR_BINDINGS_INV == gp->type)
    {
      char buf[20];
      sprintf (buf, " :binv_%ld", (long) (eq->e_own_idx));
      ssg_puts (buf);
      return 1;
    }
  ...
  if (flags & SSG_RETVAL_FROM_GOOD_SELECTED)
      ...
      for (var_ctr = 0; var_ctr < var_count; var_ctr++)
	{
	  ...
	  if (NULL == eq->e_front_varname)
	    ...
	    spar_internal_error (ssg->ssg_sparp,
		"ssg_"
		"print_equiv_retval_expn(): interesting: strcmp (eq->e_front_varname, rv->_.retval.vname) for plain SSG_RETVAL_FROM_GOOD_SELECTED");
#endif
	...
	{			/* Special case of an equiv used only to pass column of UNION to the next level, can print it always */
	  ...
	  if (NULL == eq->e_front_varname)
	    ...
	    spar_internal_error (ssg->ssg_sparp,
		"ssg_"
		"print_equiv_retval_expn(): interesting: strcmp (eq->e_front_varname, eq->e_varnames[0]) for SSG_RETVAL_FROM_ANY_SELECTED in UNION_L gp");
#endif
	  ...
	  if ((native == needed) || (SSG_VALMODE_AUTO == needed))
		...
		spar_error (ssg->ssg_sparp,
		    "No way to calculate value of variable '%.200s'; the query might contain triple patterns that can not be bound",
		    rv->_.retval.vname);
  ...
  switch (gp->_.gp.subtype)
	...
	spar_internal_error (ssg->ssg_sparp, "ssg_" "print_equiv_retval_expn(): select selid is out of scope");
      /* no break */
    case UNION_L:
    case SPAR_UNION_WO_ALL:	/* This line should be commented out if a separate branch is un-ifdef-ed below */
    case SERVICE_L:
    case VALUES_L:
      {
	SPART_buf rv_buf;
	...
	if (NULL == eq->e_front_varname)
	  ...
	  spar_internal_error (ssg->ssg_sparp,
	      "ssg_"
	      "print_equiv_retval_expn(): interesting: strcmp (eq->e_front_varname, eq->e_varnames[0]) for SERVICE_L/VALUES_L gp");
#endif
	if ((SERVICE_L == gp->_.gp.subtype)
	    && (-1 == sparp_find_sinv_rset_or_param_pos_of_varname (ssg->ssg_sparp, gp, eq->e_front_varname,
		    0 /* search in rset */ )))
	  {
	    if (flags & SSG_RETVAL_MUST_PRINT_SOMETHING)
	      {
		int param_pos =
		    sparp_find_sinv_rset_or_param_pos_of_varname (ssg->ssg_sparp, gp, eq->e_front_varname,
		    1 /* search in params */ );
		if ((0 <= param_pos) && (BOX_ELEMENTS_0 (eq->e_receiver_idxs)))
		  {
		    sparp_equiv_t *recv_eq = SPARP_EQUIV (ssg->ssg_sparp, eq->e_receiver_idxs[0]);
		    return ssg_print_equiv_retval_expn (ssg, recv_eq->e_gp, recv_eq,
			flags | SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_FROM_ANY_SELECTED | SSG_RETVAL_FROM_JOIN_MEMBER, needed,
			asname);
		  }
	      }
	    goto try_write_null;
	...
	if ((VALUES_L == gp->_.gp.subtype)
	    && (-1 == sparp_find_binv_rset_pos_of_varname (ssg->ssg_sparp, gp, gp->_.gp.subquery, eq->e_front_varname)))
	  goto try_write_null;
	rv->_.retval.equiv_idx = eq->e_own_idx;
	sparp_rvr_copy (ssg->ssg_sparp, &(rv->_.retval.rvr), &(eq->e_rvr));
	rv->_.retval.gp = gp;
...
try_write_null:
  if ((flags & SSG_RETVAL_MUST_PRINT_SOMETHING) &&
    ...
    return ssg_print_equiv_retval_expn (ssg, gp, eq, flags | SSG_RETVAL_FROM_ANY_SELECTED, needed, asname);
...
write_assuffix:
  if (IS_BOX_POINTER (asname) && ((NULL == name_as_expn) || strcmp (asname, name_as_expn)))
    {
      ssg_puts_with_comment (" AS", "eqretval");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
  return 1;
...
#if 0
	    ssg_print_equiv_retval_expn (ssg, var_rv->_.retval.gp, SPARP_EQUIV (ssg->ssg_sparp, var_rv->_.retval.equiv_idx),
		/*SSG_RETVAL_FROM_GOOD_SELECTED | */ SSG_RETVAL_MUST_PRINT_SOMETHING, vmode, eq_idx_asname);
#endif
...
#if 0
	ssg_print_equiv_retval_expn (ssg, var_rv->_.retval.gp, SPARP_EQUIV (ssg->ssg_sparp, var_rv->_.retval.equiv_idx),
	    /*SSG_RETVAL_FROM_GOOD_SELECTED | */ SSG_RETVAL_MUST_PRINT_SOMETHING, SSG_VALMODE_SQLVAL, NULL_ASNAME);
#endif
...
void
ssg_print_equalities_of_vars_with_globals_and_constants (spar_sqlgen_t * ssg, SPART * gp, sparp_equiv_t * eq, SPART * sample_var,
    dk_set_t ghost_varnames, int print_nice_eq_to_fixed_val)
{
  ...
  for (var_ctr = 0; var_ctr < eq->e_var_count; var_ctr++)
      ...
      if (((SPART_VARR_GLOBAL | SPART_VARR_EXTERNAL) & mixed_restrictions) && (NULL != tabid))
	{
	  ...
	  ((NULL != tabid) || ((SPART_VARR_TYPED & eq->e_replaces_filter) && (0 == eq->e_gspo_uses) && (0 == var_ctr))))
	{
	  if (SPART_VARR_TYPED & var->_.var.rvr.rvrRestrictions)
	    {
	      if (eq->e_rvr.rvrDatatype != var->_.var.rvr.rvrDatatype)
		spar_internal_error (ssg->ssg_sparp,
		    "Fixed type of equiv class is not equal to fixed type of one of its variables");
	    }
	  else
	    {
  ...
  if (((SPART_VARR_EXTERNAL | SPART_VARR_GLOBAL) & eq->e_rvr.rvrRestrictions)
      && ((NULL != ghost_varnames) || (0 != BOX_ELEMENTS_0 (eq->e_subvalue_idxs))) && (NULL == sample_global_rv))
    {
      if (NULL == sample_global_rv)
  ...
  if (NULL != sample_global_rv)
	...
	ssg_print_bop_bool_expn (ssg, bop, " = ", " equ (", 1, SSG_VALMODE_BOOL);
      ...
      END_DO_SET ()DO_BOX_FAST_REV (ptrlong, subv_idx, subv_idx_ctr, eq->e_subvalue_idxs)
      {
	...
	for (col_ctr = 0; col_ctr < col_count; col_ctr++)
	  {
		...
		SSG_RETVAL_FROM_JOIN_MEMBER | SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_MUST_PRINT_SOMETHING, common_native,
		eq_idx_asname);
	  }
	if ((0 == col_count) && (SSG_VALMODE_AUTO == common_native))
	  {			/* Two zeropart values of same class are always equal, the check is needed only if (SSG_VALMODE_AUTO == common_native) */
	    ...
	    ssg_puts (" =");
	    ssg_print_equiv_retval_expn (ssg, sub_gp, sub_eq,
		SSG_RETVAL_FROM_JOIN_MEMBER | SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_MUST_PRINT_SOMETHING, SSG_VALMODE_SQLVAL,
		NULL_ASNAME);
	  }
      }
      END_DO_BOX_FAST_REV;
    ...
    ccaddr_t jright_alias, int print_cross_join_conds, int print_inner_filter_conds)
{
  ...
  inner_filters_need_explicit_print = (0 == eq->e_gspo_uses) && ((0 != eq->e_nested_bindings)
  ...
  if (restrs_not_filtered_in_subqs && inner_filters_need_explicit_print)
      ...
      if ((UNION_L != eq->e_gp->_.gp.subtype) && (SPAR_UNION_WO_ALL != eq->e_gp->_.gp.subtype))
	{
	  DO_BOX_FAST (ptrlong, sub_idx, sub_ctr, eq->e_subvalue_idxs)
	  {
		...
		(DVC_MATCH == cmp_boxes_safe ((caddr_t) eq->e_rvr.rvrFixedValue, (caddr_t) sub_eq->e_rvr.rvrFixedValue, NULL,
			NULL)))
...
ghost variable can be used as a sample variable only in absence of plain vars */
  ...
  ssg_print_equalities_of_vars_with_globals_and_constants (ssg, gp, eq, sample_var, ghost_varnames,
      1 /* print nice equalities with constants */ );
  ...
  for (var_ctr = 0; var_ctr < eq->e_var_count; var_ctr++)
      ...
      for (var2_ctr = var_ctr + 1; var2_ctr < eq->e_var_count; var2_ctr++)
	{
	  ...
	  if ((SPART_VARR_NOT_NULL & var->_.var.restr_of_col)
	      ...
	      && (SPART_VARR_FIXED & eq->e_rvr.rvrRestrictions) && !(SPART_VARR_CONFLICT & eq->e_rvr.rvrRestrictions))
  ...
  for (var_ctr = 0; var_ctr < eq->e_var_count; var_ctr++)
      ...
      for (sub2_ctr = 0; sub2_ctr < BOX_ELEMENTS_INT_0 (eq->e_subvalue_idxs); sub2_ctr++)
	{
	  ...
	  if (SPARP_FIXED_AND_NOT_NULL (var->_.var.restr_of_col)
	      && SPARP_EQ_IS_FIXED_AND_NOT_NULL (sub2_eq) && !(SPART_VARR_CONFLICT & eq->e_rvr.rvrRestrictions))
	    continue;		/* No need to write an equality of two constants */
	  if (NULL != jright_alias)
	    {
	      if (!strcmp (sub2_gp->_.gp.selid, jright_alias) && (0 <= dk_set_position_of_string (jleft_aliases, tabid)))
		var_is_left = 1;
	      else if (!strcmp (tabid, jright_alias) && (0 <= dk_set_position_of_string (jleft_aliases, sub2_gp->_.gp.selid)))
		var_is_left = 0;
	      else
		continue;
	    }
	  ...
	  if (sub2_is_nullable_inline)
	    {
	      ssg_puts (" (");
	      ssg_print_equiv_retval_expn (ssg, sub2_gp, sub2_eq, SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_MUST_PRINT_SOMETHING,
	  ...
	  if ((0 == col_count) && (SSG_VALMODE_AUTO == common_native))
	    {			/* Two zeropart values of same class are always equal, the check is needed only if (SSG_VALMODE_AUTO == common_native) */
	      ...
	      ssg_puts (" =");
	      ssg_print_equiv_retval_expn (ssg, sub2_gp, sub2_eq, SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_MUST_PRINT_SOMETHING,
		  SSG_VALMODE_SQLVAL, NULL_ASNAME);
	    }
	  if (sub2_is_nullable_inline)
	    {
	      ssg_puts (" ) )");
...
void
ssg_print_retval_bop_calc_expn (spar_sqlgen_t * ssg, SPART * gp, SPART * tree,
    const char *s1, const char *s2, const char *s3, int cmp_op, ssg_valmode_t needed)
{
  SPART *left = tree->_.bin_exp.left;
  SPART *right = tree->_.bin_exp.right;
  ssg_valmode_t op_arg_valmode;
  ...
  if (cmp_op)
    ...
    switch (tree->type)
      ...
      case BOP_AND:
      case BOP_OR:
	op_arg_valmode = SSG_VALMODE_BOOL;
	break;
      case BOP_PLUS:
      case BOP_MINUS:
      case BOP_TIMES:
...
{				/* Very special case. Arguments are texts of queries. */
  ...
  for (arg_ctr = 0; arg_ctr < arg_count; arg_ctr++)
	...
	ssg_putchar (',');
      ...
      ssg_puts (" (");
      ssg->ssg_indent += 2;
      if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (arg) && SPAR_LIT == arg->type && DV_STRING == DV_TYPE_OF (arg->_.lit.val))
  ...
  ssg_puts (")");
  if (compose_report)
    ssg_puts_with_comment (", 1 ", "to compose report");
  ssg_puts (")");
  ssg->ssg_indent -= 2;
...
ssg_print_retval_simple_expn (spar_sqlgen_t * ssg, SPART * gp, SPART * tree, ssg_valmode_t needed, const char *asname)
{
  switch (SPART_TYPE (tree))
    {
    case BOP_AND:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " __and (", ", ", ")", 0, needed);
      goto print_asname;
    case BOP_OR:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " __or (", ", ", ")", 0, needed);
      goto print_asname;
    case SPAR_BOP_EQ_NONOPT:	/* no break */
    case BOP_EQ:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " equ (", ", ", ")", 0, needed);
      goto print_asname;
    case BOP_NEQ:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " neq (", ", ", ")", 0, needed);
      goto print_asname;
    case BOP_LT:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " lt (", ", ", ")", 1, needed);
      goto print_asname;
    case BOP_LTE:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " lte (", ", ", ")", 1, needed);
      goto print_asname;
    case BOP_GT:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " gt (", ", ", ")", 1, needed);
      goto print_asname;
    case BOP_GTE:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " gte (", ", ", ")", 1, needed);
      goto print_asname;
/*case BOP_LIKE: Like is built-in in SPARQL, not a BOP!
...
*/
    ...
    case BOP_PLUS:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " (", " + ", ")", 0, needed);
      goto print_asname;
    case BOP_MINUS:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " (", " - ", ")", 0, needed);
      goto print_asname;
    ...
    case BOP_DIV:
      ...
      goto print_asname;
    case BOP_MOD:
      ssg_print_retval_bop_calc_expn (ssg, gp, tree, " mod (", ", ", ")", 0, needed);
      goto print_asname;
    case SPAR_ALIAS:
	...
	sparp_equiv_t *eq = sparp_equiv_get_ro (ssg->ssg_equivs, ssg->ssg_equiv_count,
	    gp, (SPART *) name, SPARP_EQUIV_GET_NAMESAKES | SPARP_EQUIV_GET_ASSERT);
	int printed =
	    ssg_print_equiv_retval_expn (ssg, gp, eq, SSG_RETVAL_FROM_JOIN_MEMBER | SSG_RETVAL_MUST_PRINT_SOMETHING, needed,
	    asname);
	if (!printed)
	  {
#ifdef DEBUG
	...
	if (((SSG_VALMODE_SHORT_OR_LONG == needed) && ((SSG_VALMODE_LONG == native) || (SSG_VALMODE_NUM == native)
		    || (SSG_VALMODE_BOOL == native))) || ((SSG_VALMODE_NUM == needed) && ((SSG_VALMODE_SQLVAL == native)
		    || (SSG_VALMODE_BOOL == native))) || ((SSG_VALMODE_SQLVAL == needed) && ((SSG_VALMODE_NUM == native)
		    || (SSG_VALMODE_BOOL == native))) || ((SSG_VALMODE_LONG == needed) && ((SSG_VALMODE_NUM == native)
		    || (SSG_VALMODE_BOOL == native))))
	  needed = native;
...
print_asname:
  if (IS_BOX_POINTER (asname))
    {
      ssg_puts_with_comment (" AS", "retsimple");
      ssg_putchar (' ');
      ssg_prin_id (ssg, asname);
    }
}
  ...
  if (IS_BOX_POINTER (vmode))
      ...
      if (NULL != dt_iri)
	{
	  if (!strncmp (dt_iri, uname_xmlschema_ns_uri_hash, strlen (uname_xmlschema_ns_uri_hash)))
	    {
	      int pos =
		  ecm_find_name (dt_iri + strlen (uname_xmlschema_ns_uri_hash), xsd2sql_names,
		  sizeof (xsd2sql_names) / (2 * sizeof (caddr_t)), 2 * sizeof (caddr_t));
	      if (0 <= pos)
...
void
ssg_print_retval_expn (spar_sqlgen_t * ssg, SPART * gp, SPART * ret_column, int col_idx, int flags, SPART * auto_valmode_gp,
    ssg_valmode_t needed)
{
  int printed;
  int eq_flags;
  ...
  if ((SSG_RETVAL_DIST_SER_LONG & flags) && sparp_retval_should_wrap_distinct (ssg->ssg_sparp, ssg->ssg_tree, ret_column))
    {
      ret_column = spar_make_funcall (ssg->ssg_sparp, 0, "SPECIAL::sql:RDF_DIST_SER_LONG", (SPART **) t_list (1, ret_column));
      var_name = NULL;
    }
  ...
  if (SSG_VALMODE_AUTO == needed)
    {
      sparp_equiv_t *auto_eq =
	  sparp_equiv_get_ro (ssg->ssg_equivs, ssg->ssg_equiv_count, auto_valmode_gp, (SPART *) var_name, eq_flags);
      if (NULL != auto_eq)
	needed = sparp_equiv_native_valmode (ssg->ssg_sparp, auto_valmode_gp, auto_eq);
    }
  if (IS_BOX_POINTER (needed) && (0 == needed->qmfColumnCount))
    {
      ssg_puts_with_comment (" 1", "fake");
...
void
ssg_print_retval_cols (spar_sqlgen_t * ssg, SPART * tree, SPART ** retvals, ccaddr_t selid, const char *deser_name_or_code,
    int print_asname)
{
  int col_idx;
  const char *opt_deser_name = NULL;
  if ((SSG_PRINT_RETVAL_COLS_RO2XX_IF_BIJECTION == deser_name_or_code)
      || (SSG_PRINT_RETVAL_COLS_RO2XX_IF_NOT_BIJECTION == deser_name_or_code))
    {
...
void
ssg_print_retval_list (spar_sqlgen_t * ssg, SPART * gp, SPART ** retlist, int res_len, int flags, ptrlong * retlist_restr_bits,
    SPART * auto_valmode_gp, ssg_valmode_t needed)
{
  ...
  if ((0 == gp->_.gp.subtype) || (WHERE_L == gp->_.gp.subtype) || (OPTIONAL_L == gp->_.gp.subtype))
	...
	switch (memb->type)
	  {
	  case SPAR_GP:
	    t_set_push (&(ssg->ssg_valid_ret_selids), memb->_.gp.selid);
	    break;
	  case SPAR_TRIPLE:
	    t_set_push (&(ssg->ssg_valid_ret_tabids), memb->_.triple.tabid);
	    break;
	  }
      }
      END_DO_BOX_FAST_REV;
...
void
ssg_print_all_table_fld_restrictions (spar_sqlgen_t * ssg, quad_map_t * qm, caddr_t alias, SPART * gp, SPART * triple,
    int enabled_field_bitmask, int print_outer_filter)
{
...
static void
ssg_collect_aliases_tables_and_conds (qm_atable_array_t qmatables, ccaddr_t * qmconds,
...
ssg_patch_ft_arg1 (spar_sqlgen_t * ssg, SPART * ft_arg1, SPART * g, int contains_in_rdf_quad)
{
  ...
  if ((SPAR_QNAME == g_spart_type) && (SPAR_LIT == ft_arg1_spart_type) && (DV_STRING == DV_TYPE_OF (ft_arg1_str)))
    {
      ccaddr_t boxed_id;
      ...
      if (NULL != boxed_id)
	{
	  ...
	  (tail++)[0] = '^';
	  (tail++)[0] = '\'';
	  memcpy (tail, tmp, idlen);
	  tail += idlen;
	  /*  0         1          2       */
	  /* .012345678901234.567890.12345 */
	  memcpy (tail, "\' AND ([ __enc \"UTF-8\" ] ", 25);
	  tail += 25;
	  memcpy (tail, ft_arg1_str, ft_arg1_strlen);
	  tail += ft_arg1_strlen;
	  strcpy (tail, ")");
  ...
  if ((SPAR_QNAME == g_spart_type) ||
      (SPAR_IS_BLANK_OR_VAR (g) && ((SPART_VARR_FIXED | SPART_VARR_GLOBAL /* Should I add SPART_VARR_EXTERNAL here ? */ ) &
  ...
  if (SPAR_IS_BLANK_OR_VAR (g))
	...
	/* else
      ...
      END_DO_SET ()good_len = dk_set_length (good_expns);
      ...
      if ((0 < good_len) && (16 > good_len) /*&& (4 < bad_len) */ )
	{
	  patched_ft_arg1 = spar_make_funcall (ssg->ssg_sparp, 0, "sql:RDF_OBJ_PATCH_CONTAINS_BY_MANY_GRAPHS",
		  ...
		  spar_make_funcall (ssg->ssg_sparp, 0, "bif:vector", (SPART **) t_list_to_array (good_expns))));
	  return patched_ft_arg1;
	}
    }
...
static void
ssg_print_fake_self_join_subexp (spar_sqlgen_t * ssg, SPART * gp, SPART *** tree_sets, int tree_set_count, int tree_count,
    int inside_breakup, int fld_restrictions_bitmask)
{
  ...
  for (tree_set_ctr = 0; tree_set_ctr < tree_set_count; tree_set_ctr++)
	...
	for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
	  {
	    ...
	    ssg_collect_aliases_tables_and_conds (qmv->qmvATables, qmv->qmvConds, &ata_aliases, &ata_tables, &queued_row_filters);
	...
	ssg_collect_aliases_tables_and_conds (qm->qmATables, qm->qmConds, &ata_aliases, &ata_tables, &queued_row_filters);
	if (tree->_.triple.ft_type)
	  {
	    ...
	    ssg_collect_aliases_tables_and_conds (ft_atables, qmft->qmvftConds, &ata_aliases, &ata_tables, &queued_row_filters);
	  }
      no_extra_ft_tables:;
      }
  if (NULL == colcodes)
...
END_DO_SET ()from_printed:
  ...
  END_DO_SET ()printed_row_filters = queued_row_filters;
  for (tree_set_ctr = 0; tree_set_ctr < tree_set_count; tree_set_ctr++)
    for (tree_ctr = 0; tree_ctr < tree_count; tree_ctr++)
      {
	int condctr;
	...
	if (IS_BOX_POINTER (ft_type))
	  {
	    ...
	    if (NULL == qmft)
	      spar_error (ssg->ssg_sparp,
		  "Special predicate %.100s() for variable %.100s is always false for this specific query on this specific quad storage",
		  ft_type, var_name);
		...
		switch ((ptrlong) (args[argctr]))
		  ...
		  case OFFBAND_L:
		    ssg_puts (", OFFBAND, ");
		    goto contains_prin_id;	/* see below */
		  case SCORE_L:
		    ssg_puts (", SCORE, ");
		    goto contains_prin_id;	/* see below */
		  case SCORE_LIMIT_L:
		    ssg_puts (", SCORE_LIMIT, ");
		  ...
		  default:
		    ...
		    spar_internal_error (ssg->ssg_sparp, "Unsupported option in printing freetext predicate");
		    break;
		  }
	      contains_prin_id:
...
#define SAME_AS__VARIANT_COUNT 6
      ...
      static ptrlong same_as__keys[SAME_AS__VARIANT_COUNT] =
	  { IFP_L, SAME_AS_L, SAME_AS_O_L, SAME_AS_P_L, SAME_AS_S_L, SAME_AS_S_O_L };
      static const char *same_as__names[SAME_AS__VARIANT_COUNT] =
	  { "IFP", "SAME_AS", "SAME_AS_O", "SAME_AS_P", "SAME_AS_S", "SAME_AS_S_O" };
      ssg_putchar (' ');
...
ssg_print_subquery_table_exp (spar_sqlgen_t * ssg, SPART * wrapping_gp)
{
  ...
  ssg_puts_with_comment (" (", "subq begin");
  ssg->ssg_indent++;
  ssg_newline (1);
  ...
  subq_ssg->ssg_comment_sql = ssg->ssg_comment_sql;
  ssg_make_sql_query_text (subq_ssg, 0);
  ssg_free_internals (subq_ssg);
  ssg_newline (1);
  ssg_puts_with_comment (" ) AS", "subq end");
  ssg_putchar (' ');
  ssg->ssg_indent--;
...
void
ssg_prepare_sinv_template (spar_sqlgen_t * parent_ssg, SPART * sinv, SPART * gp, caddr_t * qtext_template_ret,
    caddr_t * qtext_posmap_ret)
{
...
/* There exists a special case of SERVICE that consists of solely one SELECT that is used as a wrapper for aggregates, limits etc. */
  if ((0 == BOX_ELEMENTS (gp->_.gp.filters))
      ...
      && (SPAR_GP == gp->_.gp.members[0]->type) && (SELECT_L == gp->_.gp.members[0]->_.gp.subtype))
    {
      ...
      for (vname_ctr = rset_len; vname_ctr--; /* no step */ )
	{
	  ...
	  for (expn_ctr = rvals_len; expn_ctr--; /* no step */ )
		...
		{
		case SPAR_ALIAS:
		  rval_name = val->_.alias.aname;
		  break;
		case SPAR_VARIABLE:
		case SPAR_BLANK_NODE_LABEL:
		  rval_name = val->_.var.vname;
		  break;
		default:
		  goto failed_single_subq_optimization;	/* see below */
		}
  ...
  if (0 == BOX_ELEMENTS_0 (sinv->_.sinv.rset_varnames))
	...
	ssg_sdprint_tree (ssg, spartlist (ssg->ssg_sparp, 6, SPAR_ALIAS, (ptrlong) 1, stub_varname, SSG_VALMODE_AUTO, (ptrlong) 0,
		(ptrlong) 0));
      else
	{
	  ssg_puts (" ?");
	  ssg_puts (stub_varname);
	}
    }
/*!!!TBD FROM clauses */
...
query_text_is_composed:
  ...
  while (posmap_itm_ctr--)
    qtext_posmap[posmap_itm_ctr] = (ptrlong) (t_set_pop (&(ssg->ssg_param_pos_set)));
...
ssg_print_binv_table_exp (spar_sqlgen_t * ssg, SPART * wrapping_gp, int pass)
{
  ...
  if (SSG_TABLE_SELECT_PASS == pass)
      ...
      else
	{
	  ...
	  ssg_prin_id (ssg, wrapping_gp->_.gp.selid);
	  snprintf (buf, sizeof (buf), " TABLE OPTION (EST_SIZE %ld, EST_TIME %ld)", (long) (binv->_.binv.rows_in_use),
	      (long) (binv->_.binv.rows_in_use * 5));
	  ssg_puts (buf);
	}
    }
      ...
      else
	{
	  ...
	  ssg_newline (0);
	  SPARP_FOREACH_GP_EQUIV (ssg->ssg_sparp, wrapping_gp, equiv_ctr, eq)
	  {
	    ssg_print_equalities_of_vars_with_globals_and_constants (ssg, wrapping_gp, eq, NULL, NULL,
		0 /* print_nice_eq_to_fixed_val */ );
	  }
...
ssg_print_sinv_table_exp (spar_sqlgen_t * ssg, SPART * gp, int pass)
{
...
#if 0
      ssg_puts
	  (" DB.DBA.SPARQL_SINV_IMP (ws_endpoint, ws_params, qtext_template, qtext_posmap, param_row, expected_var_list)(rset any) ");
#else
  ...
  if (SSG_TABLE_PVIEW_PARAM_PASS == pass)
	...
	if (NULL == local_eq)
	  {
		...
		gp->_.gp.selid, NULL, (ptrlong) (0), SPART_BAD_EQUIV_IDX, SPART_RVR_LIST_OF_NULLS, (ptrlong) (0x0));
	...
	if ((SPART_VARR_FIXED | SPART_VARR_NOT_NULL) ==
	    ((SPART_VARR_FIXED | SPART_VARR_NOT_NULL) & local_eq->e_rvr.rvrRestrictions))
	  {
	...
	if ((SPART_VARR_FIXED | SPART_VARR_NOT_NULL) ==
	    ((SPART_VARR_FIXED | SPART_VARR_NOT_NULL) & parent_eq->e_rvr.rvrRestrictions))
	  {
	...
	if (first_sibling_eq->e_gp != gp)
	  {
	    ssg_print_equiv_retval_expn (ssg, first_sibling_eq->e_gp, first_sibling_eq,
		SSG_RETVAL_FROM_GOOD_SELECTED | SSG_RETVAL_MUST_PRINT_SOMETHING, SSG_VALMODE_LONG, NULL_ASNAME);
	    goto param_value_is_printed;	/* see below */
	  }
...
#else
	ssg_puts (" NULL /* runaway ");
	ssg_puts (varname);
	ssg_puts (" after reorder */");
#endif
...
ssg_print_scalar_subquery_exp (spar_sqlgen_t * ssg, SPART * sub_req_top, SPART * wrapping_gp, ssg_valmode_t needed)
{
  ...
  sql_comp_t subq_sc;
  t_NEW_VARZ (spar_sqlgen_t, subq_ssg);
  ssg_puts_with_comment (" (", "scalar subq begin");
  ssg->ssg_indent++;
  ssg_newline (1);
  ssg->ssg_nested_ssg = subq_ssg;
  subq_ssg->ssg_parent_ssg = ssg;
...
ssg_print_table_exp (spar_sqlgen_t * ssg, SPART * gp, SPART ** trees, int tree_count, int pass)
{
  ...
  switch (SPART_TYPE (tree))
	...
	if (SSG_TABLE_SELECT_PASS == pass)
	  {
		...
		(SSG_RETVAL_FROM_FIRST_UNION_MEMBER | SSG_RETVAL_FROM_JOIN_MEMBER |
		    SSG_RETVAL_SET_ALIAS_BY_EQUIV | SSG_RETVAL_USES_ALIAS | SSG_RETVAL_EQUIV_INSTEAD_OF_TREE |
		    SSG_RETVAL_MUST_PRINT_SOMETHING), SSG_VALMODE_AUTO);
    ...
    default:
      spar_sqlprint_error ("ssg_" "print_table_exp(): unsupported type of tree");
    }
}

void
ssg_print_breakup_in_union (spar_sqlgen_t * ssg, SPART * gp, SPART ** retlist, int head_flags, int retval_flags,
    ptrlong * retlist_restr_bits, ssg_valmode_t needed, int first_mcase_idx, int breakup_shift)
{
	  ...
	  for (fld_ctr = 0; fld_ctr < SPART_TRIPLE_FIELDS_COUNT; fld_ctr++)
	      ...
	      if (first_mcase_qmv != mcase_qmv)
		{
		  ...
		  if (first_mcase_qmv->qmvRange.rvrRestrictions != mcase_qmv->qmvRange.rvrRestrictions)
		    goto fld_restrictions_may_vary;	/* see below */
		  if (!sparp_expns_are_equal (ssg->ssg_sparp, (SPART *) (first_mcase_qmv->qmvRange.rvrFixedValue),
			  (SPART *) (mcase_qmv->qmvRange.rvrFixedValue)))
		    goto fld_restrictions_may_vary;	/* see below */
		  if (!sparp_expns_are_equal (ssg->ssg_sparp, (SPART *) (first_mcase_qmv->qmvRange.rvrDatatype),
			  (SPART *) (mcase_qmv->qmvRange.rvrDatatype)))
		    goto fld_restrictions_may_vary;	/* see below */
		  if (!sparp_expns_are_equal (ssg->ssg_sparp, (SPART *) (first_mcase_qmv->qmvRange.rvrLanguage),
			  (SPART *) (mcase_qmv->qmvRange.rvrLanguage)))
		    goto fld_restrictions_may_vary;	/* see below */
		}
  ...
  for (breakup_ctr = 0; breakup_ctr <= /* not '<' */ breakup_shift; breakup_ctr++)
      ...
      for (tc = triples_count; tc--; /* no step */ )
	{
	  SPART *mcase_triple = mcase->_.gp.members[tc];
	  int leftmost_tc = leftmost_tc_of_tabid_reuses[tc];
	  ssg_print_all_table_fld_restrictions (ssg, mcase_triple->_.triple.tc_list[0]->tc_qm, mcase_triple->_.triple.tabid, gp,
	      mcase_triple, ~(common_fld_restrictions_bitmasks[leftmost_tc]), 1);
	}
      ssg->ssg_where_l_printed = save_where_l_printed;
      ssg->ssg_where_l_text = save_where_l_text;
      ssg->ssg_indent--;
  ...
  for (left_tc = 0; left_tc < triples_count; left_tc++)
	...
	continue;
      if (0 != left_tc)
	{
	  ssg_puts_with_comment (",", "table list of next triple starts here");
	  ssg_newline (0);
	}
      ...
      for (tc = left_tc; tc < triples_count; tc++)
	{
      ...
      ssg_print_fake_self_join_subexp (ssg, first_mcase, selected_triples_of_mcases, selected_triples_count, 1 + breakup_shift,
	  1 /* = inside breakup */ , common_fld_restrictions_bitmasks[left_tc]);
    }
  ssg->ssg_indent--;
  save_where_l_printed = ssg->ssg_where_l_printed;
  save_where_l_text = ssg->ssg_where_l_text;
...
ccaddr_t
ssg_print_union_member_item (spar_sqlgen_t * ssg, SPART * member, int *itm_idx_ptr, int itm_count, dk_set_t prev_itm_aliases,
    int print_glued_filters)
{
	  ...
	  for (equiv_ctr = 0; equiv_ctr < member->_.gp.equiv_count; equiv_ctr++)
	    {
	      ...
	      ssg_print_equivalences (ssg, member, eq, prev_itm_aliases, itm_alias, 1 /* print join conds */ ,
		  0 /* do not print inner filters */ );
	    }
	}
      ...
      if (SPAR_GP == itm->type)
	{
	  SPARP_FOREACH_GP_EQUIV (ssg->ssg_sparp, itm, itm_eq_ctr, itm_eq)
	  {
	    ...
	    if (NULL == left_tree)
	      {
		if (itm_eq->
		    e_replaces_filter & (SPART_VARR_FIXED | SPART_VARR_IS_BLANK | SPART_VARR_IS_IRI | SPART_VARR_IS_LIT |
			SPART_VARR_IS_REF | SPART_VARR_NOT_NULL))
		  {
		...
		if (SPAR_IS_BLANK_OR_VAR (left_tree))
		  {
		    ...
		    ssg_print_fld_var_restrictions_ex (ssg, left_qm, left_qmv, left_triple->_.triple.tabid, left_tree, left_triple,
			NULL /*fld_if_outer */ , &tmp_rvr, 0);
...
ssg_print_union (spar_sqlgen_t * ssg, SPART * gp, SPART ** retlist, int head_flags, int retval_flags, ssg_valmode_t needed)
{
  ...
  for (memb_ctr = 0; memb_ctr < memb_count; memb_ctr += (1 + breakup_shift))
	...
	goto retval_list_complete;	/* see below */
      first_breakup_tabids = sparp_gp_may_reuse_tabids_in_union (ssg->ssg_sparp, member, -1);
	  ...
	  while (memb_ctr + breakup_shift + 1 < memb_count)
	    {
	      ...
	      caddr_t *next_breakup_tabids = sparp_gp_may_reuse_tabids_in_union (ssg->ssg_sparp, next_memb, tabids_count);
      ...
      if (0 == itm_count)
	{
      ...
      if ((SPAR_GP == SPART_TYPE (first_itm)) && ((OPTIONAL_L == first_itm->_.gp.subtype)
	      || (SERVICE_L == first_itm->_.gp.subtype)))
	{
	  char buf[105];	/* potentially 100 chars long see sparp_clone_id etc. */
	  ssg_newline (0);
	...
	{
	  ccaddr_t this_alias =
	      ssg_print_union_member_item (ssg, member, &itm_idx, itm_count, prev_itm_aliases,
	      (itm_idx == idx_of_last_optional_in_member));
      ...
      for (equiv_ctr = 0; equiv_ctr < member->_.gp.equiv_count; equiv_ctr++)
	{
	  ...
	  ssg_print_equivalences (ssg, member, eq, NULL, NULL,
	      ('Y' == need_self_joins_in_where) /* print join conds only for 0 or 1 item */ , 1 /* print inner filters */ );
	}
...
ssg_print_t_options_of_select (spar_sqlgen_t * ssg)
{
  ...
  for (ctr = BOX_ELEMENTS (opts); 1 < ctr; ctr -= 2)
	...
	case T_EXISTS_L:
	  ...
	  break;
	case T_NO_CYCLES_L:
	  ssg_puts (" T_NO_CYCLES");
	  break;
	case T_NO_ORDER_L:
	  ssg_puts (" T_NO_ORDER");
	  break;
	case T_SHORTEST_ONLY_L:
	  ssg_puts (" T_SHORTEST_ONLY");
	  break;
	case T_FINAL_AS_L:
	  spar_error (ssg->ssg_sparp, "Option T_FINAL_AS is not supported in SPARQL in this version of Virtuoso");
	  break;
	default:
	  break;
	}
  ...
  for (ctr = BOX_ELEMENTS (opts); 1 < ctr; ctr -= 2)
	...
	case T_STEP_L:
	  ...
	  if (SPAR_VARIABLE == SPART_TYPE (val->_.alias.arg))
	    {
	      int pos =
		  sparp_subexpn_position1_in_retlist (ssg->ssg_sparp, val->_.alias.arg->_.var.vname,
		  tree->_.req_top. /*orig_ */ retvals);
...
ssg_req_top_needs_rb_complete (spar_sqlgen_t * ssg)
{
  ...
  if ((NULL != tree->_.req_top.formatmode_name) && (strcmp (tree->_.req_top.formatmode_name, "_JAVA_")
	  && strcmp (tree->_.req_top.formatmode_name, "_UDBC_") && strcmp (tree->_.req_top.formatmode_name, "_MSACCESS_")))
    return SSG_REQ_TOP_RB_COMPLETE_OFF;
...
ssg_make_sql_query_text (spar_sqlgen_t * ssg, int some_top_retval_flags)
{
  ...
  int top_retval_flags = some_top_retval_flags |
      ...
      SSG_RETVAL_FROM_FIRST_UNION_MEMBER | SSG_RETVAL_MUST_PRINT_SOMETHING | SSG_RETVAL_CAN_PRINT_NULL | SSG_RETVAL_USES_ALIAS;
  ...
  ssg_find_formatter_by_name_and_subtype (tree->_.req_top.formatmode_name, tree->_.req_top.subtype, &formatter, &agg_formatter,
      &agg_meta);
  ...
  switch (subtype)
	...
	ssg_print_t_options_of_select (ssg);
	  ...
	  retvals, BOX_ELEMENTS_INT (retvals), top_retval_flags, NULL, tree->_.req_top.pattern, retvalmode);
      if ((NULL != ssg->ssg_wrapping_gp) && (NULL != ssg->ssg_wrapping_gp->_.gp.options))
	ssg_print_t_steps_of_select (ssg);
      break;
    case CONSTRUCT_L:
    case DESCRIBE_L:
      if ((NULL != tree->_.req_top.formatmode_name) && (!strcmp ("_JAVA_", tree->_.req_top.formatmode_name)
	      || !strcmp ("_UDBC_", tree->_.req_top.formatmode_name) || !strcmp ("_MSACCESS_", tree->_.req_top.formatmode_name)))
	{
      ...
      else if ((NULL == formatter) && (NULL == agg_formatter) && ssg->ssg_sparp->sparp_sparqre->sparqre_direct_client_call)
	{
	  ...
	  if ((NULL != retvalmode) && (SSG_VALMODE_LONG != retvalmode))
	    spar_sqlprint_error
		("'output:valmode' declaration conflicts with TTL output format needed by database client connection'");
	}
      ...
      if (has_limofs || has_ctor_over_groups)
	{
      ...
      ssg_print_retval_list (ssg, tree->_.req_top.pattern, retvals, 1, top_retval_flags, NULL, tree->_.req_top.pattern, retvalmode);
      if (NULL != formatter)
	{
      ...
      if (has_limofs || has_ctor_over_groups)
	{
	  ...
	  ssg_print_limofs_expn_of_top (ssg);
	  ssg_print_retval_list (ssg, tree->_.req_top.pattern,
	      retvals + 1, BOX_ELEMENTS (retvals) - 1, top_retval_flags | SSG_RETVAL_USES_ALIAS, NULL, NULL, retvalmode);
	  ssg->ssg_indent += 1;
	}
      break;
    case ASK_L:
      if (NULL != formatter)
	{
      ...
      break;
    default:
      spar_sqlprint_error ("ssg_make_sql_query_text(): unsupported type of tree");
    }
#if 0				/*!!! TBD */
  if (NULL != final_binv)
    {
    ...
    ssg_puts ("SELECT "}
	  ...
	  top_union_head_flags, top_retval_flags, retvalmode); if (0 < BOX_ELEMENTS_INT_0 (tree->_.req_top.groupings))
      {
/* Bug 14357, part 2 (see ORDER BY below for more explaination) */
	int gby_printed = 0; int fake_gby_expn_printed = 0; DO_BOX_FAST (SPART *, grouping, gby_ctr, tree->_.req_top.groupings)
	{
	  ssg_valmode_t native, needed;
	  int grouping_is_weird = ssg_expn_is_not_int_const_but_printed_as_some_const (ssg, grouping);
	  if (grouping_is_weird && fake_gby_expn_printed) continue; if (gby_printed) ssg_putchar (',');
	  else
	  {
	  ssg_newline (0); ssg_puts ("GROUP BY"); gby_printed = 1; ssg->ssg_indent++;}
	  if (grouping_is_weird)
	  {
	  ssg_puts_with_comment (" 0.0", "fake, instead of grouping by other const"); fake_gby_expn_printed = 1; continue;}
	  native = sparp_expn_native_valmode (ssg->ssg_sparp, grouping);
	  if (IS_BOX_POINTER (native) && native->qmfIsBijection) needed = native;
	  else
	needed = SSG_VALMODE_SQLVAL; ssg_print_retval_simple_expn (ssg, tree->_.req_top.pattern, grouping, needed, NULL_ASNAME);}
      END_DO_BOX_FAST; if (gby_printed) ssg->ssg_indent--;}
      if (NULL != tree->_.req_top.having)
      {
	ssg_newline (0); ssg_puts ("HAVING"); ssg->ssg_indent++;
#if 1
...
#endif
      ssg->ssg_indent--;}
      if ((0 < BOX_ELEMENTS_INT_0 (tree->_.req_top.order)) && ((SELECT_L == tree->_.req_top.subtype)
	      || (DISTINCT_L == tree->_.req_top.subtype) || has_limofs || has_ctor_over_groups))
      {
...
*/
	int oby_printed = 0; DO_BOX_FAST (SPART *, oby_itm, oby_ctr, tree->_.req_top.order)
	{
	  SPART * expn = oby_itm->_.oby.expn;
	  if (ssg_expn_is_not_int_const_but_printed_as_some_const (ssg, expn)) continue; if (oby_printed) ssg_putchar (',');
	  else
	  {
	  ssg_newline (0); ssg_puts ("ORDER BY"); oby_printed = 1; ssg->ssg_indent++;}
	ssg_print_orderby_item (ssg, tree->_.req_top.pattern, oby_itm);}
      END_DO_BOX_FAST; if (oby_printed) ssg->ssg_indent--;}
	...
	ssg->ssg_indent -= 1; if (0 == final_binv->_.binv.rows_in_use) ssg_puts (" WHERE (1=0)");
	else
	{
	  ...
	  ssg_newline (0); DO_BOX_FAST (SPART *, bndvar, bndctr, final_binv->_.binv.vars)
	  {
	    char colname_buf[40];
	    char buf[200];
	    snprintf (colname_buf, sizeof (colname_buf),
		(SPAR_MAX_BINDINGS_VIEW_CN >= width) ? "\"bnd2\".BND%d" : "\"bnd2\".BND[%d]", bndctr);
	    if (final_binv->_.binv.counters_of_unbound[bndctr]) snprintf (buf, sizeof (buf),
		" AND (%s IS NULL OR (%s = \"%.100s\"))", colname_buf, colname_buf, bndvar->_.var.vname);
	    else
	  snprintf (buf, sizeof (buf), " AND (%s = \"%.100s\")", colname_buf, bndvar->_.var.vname); ssg_puts (buf);}
	END_DO_BOX_FAST; ssg_puts (")"); ssg->ssg_indent -= 1; ssg_newline (0);}
      }
...
case ASK_L:
    ...
    void ssg_print_qm_sql (spar_sqlgen_t * ssg, SPART * tree)
    {
      switch (SPART_TYPE (tree))
      {
case SPAR_QM_SQL_FUNCALL:
	{
	  ...
	  ssg_puts (" (");
	  ssg->ssg_indent++; fixedlen = BOX_ELEMENTS_0 (tree->_.qm_sql_funcall.fixed); for (ctr = 0; ctr < fixedlen; ctr++)
	  {
	  SPART * arg = tree->_.qm_sql_funcall.fixed[ctr];
	    int curr_is_long = SPAR_FUNCALL_ARG_IS_LONG (arg);
	    if (0 != ctr)
	    ssg_puts (", ");
	    if (curr_is_long || prev_was_long) ssg_newline (0); ssg_print_qm_sql (ssg, arg); prev_was_long = curr_is_long;}
	  if (NULL != tree->_.qm_sql_funcall.named)
	  {
	    ...
	    prev_was_long = 0;
	    ssg_puts ("vector (");
	    ssg->ssg_indent++; namedlen = BOX_ELEMENTS_0 (tree->_.qm_sql_funcall.named); for (ctr = 0; ctr < namedlen; ctr++)
	    {
	    SPART * arg = tree->_.qm_sql_funcall.named[ctr];
	      ...
	      ssg_puts (", ");
	      if (curr_is_long || prev_was_long) ssg_newline (0); ssg_print_qm_sql (ssg, arg); prev_was_long = curr_is_long;}
	  ssg->ssg_indent--; ssg_puts (" )");}
	ssg->ssg_indent--; ssg_puts (" )"); break;}
case SPAR_LIT:
case SPAR_QNAME:
	if (NULL == tree)
	{
	ssg_puts (" NULL"); return;}
	if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (tree)) ssg_print_box_as_sql_atom (ssg, tree->_.lit.val, SQL_ATOM_UNAME_ALLOWED);
	else
ssg_print_box_as_sql_atom (ssg, (caddr_t) tree, SQL_ATOM_UNAME_ALLOWED); break; default:
	ssg_print_scalar_expn (ssg, tree, SSG_VALMODE_SQLVAL, NULL_ASNAME);
/*
...
*/
    ...
    void ssg_make_qm_sql_text (spar_sqlgen_t * ssg)
    {
...
case SPAR_REQ_TOP:
	  {
	    int need_for_rb_complete; ssg->ssg_sources = ssg->ssg_tree->_.req_top.sources;	/*!!!TBD merge with environment */
	    need_for_rb_complete = ssg_req_top_needs_rb_complete (ssg);
	    if (need_for_rb_complete) ssg_make_rb_complete_wrapped (ssg, need_for_rb_complete);
	    else
	  ssg_make_sql_query_text (ssg, 0); break;}
case SPAR_CODEGEN:
	  {
	  ssg_codegen_callback_t * cbk = ssg->ssg_tree->_.codegen.cgen_cbk[0]; cbk (ssg, ssg->ssg_tree); break;}
default:
	...
	ssg->ssg_sparp->sparp_sparqre->sparqre_catched_error = thr_get_error_code (self); thr_set_error_code (self, NULL);
#ifdef SPARQL_DEBUG
	printf ("\nssg_make_whole_sql_text() caught composing error: %s",
	    ERR_MESSAGE (ssg->ssg_sparp->sparp_sparqre->sparqre_catched_error));
#endif
      }
    END_QR_RESET}
