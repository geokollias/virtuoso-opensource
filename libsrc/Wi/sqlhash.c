...
key_col_from_ssl (dbe_key_t * key, state_slot_t * ssl, int quietcast, int op)
{
  ...
  if (DV_UNKNOWN == col->col_sqt.sqt_dtp || (!dtp_is_fixed (col->col_sqt.sqt_dtp) && !dtp_is_var (col->col_sqt.sqt_dtp)))
    {
      col->col_sqt.sqt_dtp = quietcast ? DV_ANY : DV_LONG_STRING;
      col->col_sqt.sqt_precision = 0;
    }
  if (DV_ANY == ssl->ssl_dc_dtp && (DV_STRING != ssl->ssl_sqt.sqt_dtp && DV_WIDE != ssl->ssl_sqt.sqt_dtp
	  && DV_BIN != ssl->ssl_sqt.sqt_dtp))
    {
  ...
  if (col->col_sqt.sqt_dtp == DV_LONG_STRING || col->col_sqt.sqt_dtp == DV_ANY)
    col->col_sqt.sqt_precision = 0;
...
*/
  ...
  DO_SET (state_slot_t *, ssl, &setp->setp_keys)
  {
    if (!quietcast && IS_BLOB_DTP (ssl->ssl_sqt.sqt_dtp))
	  ...
	  "Long data types not allowed for distinct, order, " "group or join condition columns (%s)", ssl->ssl_name);
  ...
  if (HA_FILL == ha->ha_op)
    {
      ...
      DO_BOX (state_slot_t *, ssl, inx, ha->ha_slots) * (ssl_head_t *) ssl = ssl_save[inx];
      END_DO_BOX;
