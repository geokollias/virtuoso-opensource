...
xp_rdfxml_get_name_parts (xp_node_t * xn, char *name, int use_default, caddr_t * nsuri_ret, char **local_ret)
{
  ...
  local_ret[0] = name;
  if (0 != ns_len)
    xmlparser_logprintf (xn->xn_xp->xp_parser, XCFG_FATAL, 100 + strlen (name),
	"Name '%.1000s' contains undefined namespace prefix", name);
}


void
xp_pop_rdf_locals (xparse_ctx_t * xp)
{
...
xp_rdfxml_locals_t *
xp_push_rdf_locals (xparse_ctx_t * xp)
{
...
#else
  res =
      xml_uri_resolve_like_get (xp->xp_qi, &err, xp->xp_rdfxml_locals->xrl_base, local,
      NULL /* No need to convert into "UTF-8" because it's UTF-8 already */ );
#endif
  dk_free_box (local);
  if (NULL != err)
    sqlr_resignal (err);
...
xp_rdfxml_element (void *userdata, char *name, vxml_parser_attrdata_t * attrdata)
{
  ...
  if (!strcmp ("http://www.w3.org/1999/02/22-rdf-syntax-ns#", tmp_nsuri))
      ...
      else
	{
	  size_t l1, l2;
	  caddr_t full_element_name;
	  if (!strcmp ("Property", tmp_local) ||
	      !strcmp ("Bag", tmp_local) ||
	      !strcmp ("Seq", tmp_local) ||
	      !strcmp ("Alt", tmp_local) ||
	      !strcmp ("List", tmp_local) || !strcmp ("Statement", tmp_local) || !strcmp ("nil", tmp_local))
	    {;
	    }
	  else if (!strcmp ("subject", tmp_local) ||
	      !strcmp ("predicate", tmp_local) ||
	      !strcmp ("object", tmp_local) ||
	      !strcmp ("type", tmp_local) ||
	      !strcmp ("value", tmp_local) || !strcmp ("first", tmp_local) || !strcmp ("rest", tmp_local) || '_' == tmp_local[0])
	    {
	      xmlparser_logprintf (xp->xp_parser, XCFG_WARNING, 200,
		  "Name rdf:%.200s is used for node, not for property (legal, but strange)", tmp_local);
	    }
	  else if (!strcmp ("ID", tmp_local) ||
	      !strcmp ("about", tmp_local) ||
	      !strcmp ("bagID", tmp_local) ||
	      !strcmp ("parseType", tmp_local) ||
	      !strcmp ("resource", tmp_local) ||
	      !strcmp ("nodeID", tmp_local) ||
	      !strcmp ("li", tmp_local) || !strcmp ("aboutEach", tmp_local) || !strcmp ("aboutEachPrefix", tmp_local))
	    {
	      xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 200, "Name rdf:%.200s is used for node", tmp_local);
	    }
	  else
	    {
	      xmlparser_logprintf (xp->xp_parser, XCFG_WARNING, 200, "Unknown name rdf:%.200s is used (legal, but strange)",
		  tmp_local);
	    }
	  l1 = strlen (tmp_nsuri);
	  l2 = strlen (tmp_local);
	  ...
	  if (!strcmp (tmp_local, "about"))
	    {
	      ...
	      if (XRL_PARSETYPE_PROPLIST & outer->xrl_parsetype)
		{
		  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
		      "Attribute 'rdf:about' can not appear in element that is supposed to be property name");
		  return;
		}
	      inner_subj = xp_rdfxml_resolve_iri_avalue (xp, avalue, 0);
	      XRL_SET_NONINHERITABLE (inner, xrl_subject, inner_subj,
		  "Attribute 'rdf:about' conflicts with other attribute that set the subject");
	      inner->xrl_parsetype = XRL_PARSETYPE_PROPLIST;
	    }
		...
		{
		  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
		      "Attribute 'rdf:resource' can appear only in element that is supposed to be property name");
		  return;
		}
	      inner_subj = xp_rdfxml_resolve_iri_avalue (xp, avalue, 0);
	      XRL_SET_NONINHERITABLE (inner, xrl_subject, inner_subj,
		  "Attribute 'rdf:resource' conflicts with other attribute that set the subject");
	  ...
	  else if (!strcmp (tmp_local, "nodeID"))
	    {
	      ...
	      XRL_SET_NONINHERITABLE (inner, xrl_subject, inner_subj,
		  "Attribute 'rdf:nodeID' conflicts with other attribute that set the subject");
	      if (XRL_PARSETYPE_PROPLIST & outer->xrl_parsetype)
		{
		  inner->xrl_parsetype = XRL_PARSETYPE_EMPTYPROP;
		}
	      ...
	      if (XRL_PARSETYPE_PROPLIST & outer->xrl_parsetype)
		{
		  ...
		  XRL_SET_NONINHERITABLE (inner, xrl_reification_id, reif_subj,
		      "Reification ID of the statement is set twice by 'rdf:ID' attribute of a property element");
		}
	      else
		{
		  caddr_t inner_subj = xp_rdfxml_resolve_iri_avalue (xp, avalue, 1);
		  XRL_SET_NONINHERITABLE (inner, xrl_subject, inner_subj,
		      "Attribute 'rdf:ID' conflicts with other attribute that set node ID");
		  inner->xrl_parsetype = XRL_PARSETYPE_PROPLIST;
		}
	    }
		...
		{
		  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
		      "Attribute 'rdf:datatype' can appear only in property elements");
		  return;
		}
	      XRL_SET_NONINHERITABLE (inner, xrl_datatype, xp_rdfxml_resolve_iri_avalue (xp, avalue, 0),
		  "Attribute 'rdf:datatype' is used twice");
		...
		{
		  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
		      "Attribute 'rdf:parseType' can appear only in property elements");
		  return;
		}
	      if (!strcmp (avalue, "Resource"))
		{
		  ...
		  XRL_SET_NONINHERITABLE (inner, xrl_subject, inner_subj,
		      "Attribute parseType='Resource' can not be used if object is set by other attribute");
	    ...
	    {
	      xmlparser_logprintf (xp->xp_parser, XCFG_WARNING, 200, "Unsupported 'rdf:...' attribute");
	      goto push_inner_attr_prop;	/* see below */
	    }
	  continue;
	}
	  ...
	  if ((NULL == inner->xrl_subject) && (XRL_PARSETYPE_RES_OR_LIT & inner->xrl_parsetype))
	    {
	      ...
	      xp_rdfxml_triple_l (xp, inner->xrl_parent->xrl_subject, inner->xrl_predicate, obj, inner->xrl_datatype,
		  inner->xrl_language);
	      if (NULL != inner->xrl_reification_id)
		{
		  xp_rdfxml_triple (xp, inner->xrl_reification_id, uname_rdf_ns_uri_subject, outer->xrl_subject);
		  xp_rdfxml_triple (xp, inner->xrl_reification_id, uname_rdf_ns_uri_predicate, inner->xrl_predicate);
		  xp_rdfxml_triple_l (xp, inner->xrl_reification_id, uname_rdf_ns_uri_object, obj, inner->xrl_datatype,
		      inner->xrl_language);
		  xp_rdfxml_triple (xp, inner->xrl_reification_id, uname_rdf_ns_uri_type, uname_rdf_ns_uri_Statement);
		}
	      dk_free_tree (obj);
...
caddr_t
xp_rdfa_parse_attr_value (xparse_ctx_t * xp, xp_node_t * xn, int attr_id, char **attrvalues, int allowed_syntax,
    caddr_t ** values_ret, int *values_count_ret)
{
...
next_token:
  if (RDFA_ATTRSYNTAX_WS_LIST & allowed_syntax)
    ...
    {
      free_unsaved_token ();
      xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Whitespaces are not allowed for attribute %.20s",
  ...
  if ('\0' == tail[0])
      ...
      if (0 == values_count)
	{
	  ...
	  if (!((RDFA_ATTRSYNTAX_EMPTY_ACCEPTABLE | RDFA_ATTRSYNTAX_EMPTY_MEANS_XSD_STRING) & allowed_syntax))
	    xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Empty value is not allowed for attribute %.20s",
		rdfa_attribute_names[attr_id]);
	  expanded_token =
	      (RDFA_ATTRSYNTAX_EMPTY_MEANS_XSD_STRING & allowed_syntax) ? uname_xmlschema_ns_uri_hash_string : uname___empty;
	  if (NULL != values_ret)
  ...
  if ((1 == values_count) && !(RDFA_ATTRSYNTAX_WS_LIST & allowed_syntax))
    {
      free_unsaved_token ();
      xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Multiple values are not allowed for attribute %.20s",
	  rdfa_attribute_names[attr_id]);
      if (NULL != values_count_ret)
	values_count_ret[0] = values_count;
      return NULL;
  ...
  if ('[' == tail[0])
      ...
      if (!(RDFA_ATTRSYNTAX_SAFECURIE & allowed_syntax))
	{
	  ...
	  xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
	      "\"Safe CURIE\" syntax is not allowed for attribute \"%.20s\", ignored", rdfa_attribute_names[attr_id]);
	  return NULL;
	}
      ...
      if (0 == token_syntax)
	{
	  free_unsaved_token ();
	  xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
	      "The token value should start with a letter or underscore in attribute \"%.20s\", ignored",
	      rdfa_attribute_names[attr_id]);
	  return NULL;
	}
    }
      ...
      if (0 == token_syntax)
	{
	  free_unsaved_token ();
	  xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
	      "The token value should start with a letter, colon or underscore in attribute \"%.20s\", ignored",
	      rdfa_attribute_names[attr_id]);
	  return NULL;
	}
    }
  ...
  if ((RDFA_ATTRSYNTAX_SAFECURIE | RDFA_ATTRSYNTAX_CURIE | RDFA_ATTRSYNTAX_TERM) & token_syntax)
    {
      while (isalnum (tail[0]) || ('_' == tail[0]) || (tail[0] & ~0x7F))
	tail++;
      if (':' == tail[0])
	{
	  curie_colon = tail;
	  ...
	  if (0 == token_syntax)
	    {
	      free_unsaved_token ();
	      xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
		  "The value of attribute \"%.20s\" contains a token that is not a valid NCName, ignored",
		  rdfa_attribute_names[attr_id]);
	      return NULL;
	    }
	}
      if (token_syntax & (RDFA_ATTRSYNTAX_SAFECURIE | RDFA_ATTRSYNTAX_CURIE | RDFA_ATTRSYNTAX_URI))
	while (('\0' != tail[0]) && (']' != tail[0]) && !isspace (tail[0]))
	  tail++;
    }
  ...
  switch (tail[0])
    ...
    case '[':
	  ...
	  ((RDFA_ATTRSYNTAX_SAFECURIE & token_syntax) ?
	      ...
	      "Character '[' is not allowed inside token in the value of attribute \"%.20s\""), rdfa_attribute_names[attr_id]);
      tail = "";
      break;
    case ']':
      ...
      else
	{
	  free_unsaved_token ();
	  xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
	      "Unexpected character ']' in the value of attribute \"%.20s\"", rdfa_attribute_names[attr_id]);
	  tail = "";
	}
      break;
    default:
...
#endif
	      xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
		  "Bad token in the value of attribute \"%.20s\" (undeclared namespace?)", rdfa_attribute_names[attr_id]);
	    }
	}
    }
...
err:
  while (NULL != res)
    dk_free_box (dk_set_pop (&res));
  xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100,
      "Attribute \"prefix\" should be well-formed list of namespace prefixe and URI pairs");
}

void
xp_rdfa_parse_profile (xparse_ctx_t * xp, caddr_t * parsed_attrvalue, caddr_t ** ns_dict_ret, caddr_t ** term_dict_ret,
    caddr_t * vocab_ret, caddr_t * err_ret)
{
  ...
  if (NULL == full_profile_proc_name)
    full_profile_proc_name =
	sch_full_proc_name (wi_inst.wi_schema, "DB.DBA.RDF_RDFA11_FETCH_PROFILES", cli_qual (cli), CLI_OWNER (cli));
  if (NULL == fetch_profile_qr)
    fetch_profile_qr = sch_proc_def (wi_inst.wi_schema, full_profile_proc_name);
  if (NULL == fetch_profile_qr)
    sqlr_new_error ("42001", "SRxxx", "RDFa 1.1 parser needs a procedure named \"%.100s\"", full_profile_proc_name);
...
xp_pop_rdfa_locals (xparse_ctx_t * xp)
{
  ...
  if ((NULL != inner->xrdfal_base) && ((NULL == parent) || (inner->xrdfal_base != parent->xrdfal_base)))
    dk_free_tree (inner->xrdfal_base);
  if ((NULL != inner->xrdfal_language) && ((NULL == parent) || (inner->xrdfal_language != parent->xrdfal_language)))
    dk_free_tree (inner->xrdfal_language);
  if ((NULL != inner->xrdfal_vocab) && ((NULL == parent) || (inner->xrdfal_vocab != parent->xrdfal_vocab)))
    dk_free_tree (inner->xrdfal_vocab);
  if ((NULL != inner->xrdfal_profile_terms) && ((NULL == parent) || (inner->xrdfal_profile_terms != parent->xrdfal_profile_terms)))
...
void
rdfa_feed_or_make_ict (xparse_ctx_t * xp, xp_rdfa_locals_t * xrdfal, caddr_t left, caddr_t pred, caddr_t right, int pred_type,
    caddr_t dt, caddr_t lang)
{
  int ict_is_needed = 0;
  static caddr_t stub_null = NULL;
  ...
  if (ict_is_needed)
	...
	case RDFA_ICT_PRED_PROPERTY:
	  ...
	  tf_triple_l (xp->xp_tf, left, pred, right,
	      (((NULL != dt) && (uname___empty != dt)) ? dt : stub_null), ((NULL != lang) ? lang : stub_null));
	  break;
#ifdef RDFXML_DEBUG
	default:
	  GPF_T1 ("rdfa_" "ict_feed_or_leave(): bad ict->ict_pred_type");
  ...
  if (RDFA_IN_HTML & outer_place_bits)
      ...
      if (!strcmp (local_name, "head"))
	{
	  if ((RDFA_IN_HEAD | RDFA_IN_BODY) & outer_place_bits)
	    xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Element \"head\" can not appear inside %s element",
		(RDFA_IN_HEAD & outer_place_bits) ? "other \"head\"" : "\"body\"");
	  else
	...
	{
	  if ((RDFA_IN_HEAD | RDFA_IN_BODY) & outer_place_bits)
	    xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Element \"body\" can not appear inside %s element",
		(RDFA_IN_BODY & outer_place_bits) ? "other \"body\"" : "\"head\"");
	  else
  ...
  if ((NULL != avalues[RDFA_ATTR_VOCAB]) || (NULL != avalues[RDFA_ATTR_PROFILE]) || (NULL != avalues[RDFA_ATTR_PREFIX]))
      ...
      if (NULL != avalues[RDFA_ATTR_PROFILE])
	{
	  ...
	  xp_rdfa_parse_attr_value (xp, xn, RDFA_ATTR_PROFILE, avalues,
	      RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_EMPTY_ACCEPTABLE | RDFA_ATTRSYNTAX_WS_LIST, &profile_uris, &profile_uri_count);
  ...
  if (NULL != avalues[RDFA_ATTR_DATATYPE])
    {
	  ...
	  RDFA_ATTRSYNTAX_TERM | RDFA_ATTRSYNTAX_CURIE | RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_EMPTY_ACCEPTABLE, NULL, NULL);
    }
  if (NULL != avalues[RDFA_ATTR_HREF])
      ...
      if (RDFA_IN_BASE & inner_place_bits)
	{
	  ...
	  xpt->xpt_href = xp_rdfa_parse_attr_value (xp, xn, RDFA_ATTR_HREF, avalues,
	      RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_EMPTY_ACCEPTABLE | RDFA_ATTRSYNTAX_DIRTY_HREF, NULL, NULL);
  ...
  if (NULL != avalues[RDFA_ATTR_REL])
    {
	  ...
	  RDFA_ATTRSYNTAX_TERM | RDFA_ATTRSYNTAX_CURIE | RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_REL_REV_RESERVED |
	  RDFA_ATTRSYNTAX_WS_LIST, &(xpt->xpt_rel_preds), &rel_pred_count);
      rel_rev_attrcount++;
    }
  ...
  if (NULL != avalues[RDFA_ATTR_REV])
    {
	  ...
	  RDFA_ATTRSYNTAX_TERM | RDFA_ATTRSYNTAX_CURIE | RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_REL_REV_RESERVED |
	  RDFA_ATTRSYNTAX_WS_LIST, &(xpt->xpt_rev_preds), &rev_pred_count);
      rel_rev_attrcount++;
    }
  if (NULL != avalues[RDFA_ATTR_SRC])
      ...
      if (2 <= src_prio)
	{
	  ...
	  xpt->xpt_src = xp_rdfa_parse_attr_value (xp, xn, RDFA_ATTR_SRC, avalues,
	      RDFA_ATTRSYNTAX_URI | RDFA_ATTRSYNTAX_EMPTY_ACCEPTABLE, NULL, NULL);
	  src_prio = 2;
	}
    }
...
/* Setting the [new subject] */
  for (;;)
      ...
      if (!rel_rev_attrcount)
	{
      ...
      if ((RDFA_IN_HTML & outer_place_bits) &&
	  !((RDFA_IN_HEAD | RDFA_IN_BODY) & outer_place_bits) && ((RDFA_IN_HEAD | RDFA_IN_BODY) & inner_place_bits))
	{
	  subj = uname___empty;
	  need_rdfa_local = 1;
	}
...
/* Finally we can make triples, starting from incomplete triples at upper levels */
  if ((NULL != inner->xrdfal_subj) && (inner->xrdfal_subj != outer->xrdfal_obj_res))
      ...
      for (;;)
	{
	  for (ctr = ancestor->xrdfal_ict_count; ctr--; /* no step */ )	/* The order is important */
		...
		{
		  rdfa_feed_or_make_ict (xp, ancestor, box_copy (ict->ict_left), box_copy (ict->ict_pred),
		      box_copy (inner->xrdfal_subj), ict->ict_pred_type, NULL, NULL);
  ...
  if ((NULL != inner->xrdfal_subj) && typeof_count)
      ...
      for (ctr = typeof_count; ctr--; /* no step */ )
	{
	  ...
	  rdfa_feed_or_make_ict (xp, inner, box_copy (inner->xrdfal_subj), uname_rdf_ns_uri_type, type_uri,
	      RDFA_ICT_PRED_REL_OR_TYPEOF, NULL, NULL);
	}
    }
  for (ctr = rel_pred_count; ctr--; /* no step */ )
    {
      ...
      rdfa_feed_or_make_ict (xp, inner, box_copy (inner->xrdfal_subj), p, box_copy (inner->xrdfal_obj_res),
	  RDFA_ICT_PRED_REL_OR_TYPEOF, NULL, NULL);
    }
  for (ctr = rev_pred_count; ctr--; /* no step */ )
    {
      ...
      rdfa_feed_or_make_ict (xp, inner, box_copy (inner->xrdfal_subj), p, box_copy (inner->xrdfal_obj_res), RDFA_ICT_PRED_REV, NULL,
	  NULL);
    }
  ...
  if (!prop_pred_count)
    {
      dk_free_box (xpt->xpt_obj_content);
      xpt->xpt_obj_content = NULL;
      dk_free_box (inner->xrdfal_datatype);
      inner->xrdfal_datatype = NULL;
    }
...
xp_rdfa_character (void *userdata, char *s, int len)
{
  ...
  xp_rdfa_locals_t *inner = xp->xp_rdfa_locals;
  int inner_place_bits = inner->xrdfal_place_bits;
  if (((RDFA_IN_BASE & inner_place_bits) || (RDFA_IN_LITERAL & inner_place_bits)) && !(RDFA_IN_UNUSED & inner_place_bits))
    session_buffered_write (xp->xp_strses, s, len);
}
...
caddr_t
xp_mdata_parse_attr_value (xparse_ctx_t * xp, xp_node_t * xn, int attr_id, char **attrvalues, int allowed_syntax,
    caddr_t ** values_ret, int *values_count_ret)
{
  char *attrvalue = attrvalues[attr_id];
  char *tail = attrvalue;
...
next_token:
  if ((MDATA_ATTRSYNTAX_WS_LIST | MDATA_ATTRSYNTAX_ID) & allowed_syntax)
	...
	return NULL;
      xmlparser_logprintf (xp->xp_parser, XCFG_ERROR, 100, "Whitespaces are not allowed for attribute %.20s",
	  mdata_attribute_lognames[attr_id]);
      return NULL;
    }
  if ('\0' == tail[0])
    {
  ...
  if (MDATA_ATTRSYNTAX_REL & token_syntax)
      ...
      if (NULL == strchr (expanded_token, ':'))
	{
	  caddr_t vocab_token;
	  char *ttail;
	  for (ttail = expanded_token; ('\0' != ttail[0]) && !(ttail[0] & ~0x7f); ttail++)
	    ttail[0] = tolower (ttail[0]);
  ...
  if (MDATA_ATTRSYNTAX_URI & token_syntax)
      ...
      if (NULL == expanded_token)
	{
	  if (MDATA_ATTRSYNTAX_SILENT & allowed_syntax)
	    return NULL;
	  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100, "Bad URI token in the value of attribute \"%.20s\"",
	      mdata_attribute_lognames[attr_id]);
	}
    }
token_done:
...
void
mdata_feed_or_keep (xparse_ctx_t * xp, xp_mdata_locals_t * subj_l, caddr_t prop, xp_mdata_locals_t * inner, caddr_t obj,
    int obj_type)
{
  int obj_is_iri = ((MDATA_ATTR_ITEMID == obj_type) || (MDATA_ATTR_ID == obj_type) || (MDATA_ATTR_OBJ_REF == obj_type)
      || (MDATA_ATTR_OBJ_REF == obj_type));
  ...
  switch (obj_type)
	...
	if (6 == sscanf (obj, "%4u-%2u-%2uT%2u:%2u:%2u", &year, &mnth, &day, &hour, &minute, &second))
	  dt_dtp = DV_DATETIME;
	else if (6 == sscanf (obj, "%4u-%2u-%2u %2u:%2u:%2u", &year, &mnth, &day, &hour, &minute, &second))
	  dt_dtp = DV_DATETIME;
	else if (8 == sscanf (obj, "%9s, %2u-%3s-%2u %2u:%2u:%u %9s",
		weekday, &day, month, &year, &hour, &minute, &second, tzstring))
	  dt_dtp = DV_DATETIME;
	else if (3 == sscanf (obj, "%4u-%2u-%2u", &year, &mnth, &day))
	  dt_dtp = DV_DATE;
	else if (3 == sscanf (obj, "%2u-%3s-%2u", &day, month, &year))
	  dt_dtp = DV_DATE;
	else if (3 == sscanf (obj, "%2u:%2u:%2u", &hour, &minute, &second))
	  dt_dtp = DV_TIME;
	if (0 != dt_dtp)
	  {
...
mdata_process_pending_triples (xparse_ctx_t * xp)
{
  ...
  while (hit_next (&hit, (char **) &id_ptr, (char **) &id_desc_ptr))
	...
	xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
	    "Id '%.200s' is mentioned in an itemref attribute of some itemscope but not found in the whole XHTML/Microdata resource",
	    id_ptr[0]);
    }
...
/* Setting place bits */
  ...
  if ((2 == need_mdata_local) && (NULL != outer->xmdatal_subj) && (0 <= obj_attr_idx))
      ...
      for (prop_ctr = 0; prop_ctr < prop_count; prop_ctr++)
	{
	  ...
	  props[prop_ctr] = NULL;
	  mdata_feed_or_keep (xp, outer, prop, outer /* yes, not inner */ , box_dv_short_string (avalues[obj_attr_idx]),
	      obj_attr_idx);
	}
  ...
  if (outer->xmdatal_prop_count && (NULL != outer->xmdatal_subj) && (NULL != inner->xmdatal_subj)
      && ((NULL != avalues[MDATA_ATTR_ITEMSCOPE]) || (NULL != avalues[MDATA_ATTR_ID])))
    {
      int prop_ctr;
      for (prop_ctr = 0; prop_ctr < outer->xmdatal_prop_count; prop_ctr++)
	{
	  ...
	  mdata_feed_or_keep (xp, outer, box_copy (prop), inner,
	      box_dv_short_string (inner->xmdatal_subj),
	      ((NULL != avalues[MDATA_ATTR_ITEMSCOPE]) ? MDATA_ATTR_ITEMSCOPE : MDATA_ATTR_ID));
	}
    }
  if (NULL != avalues[MDATA_ATTR_ITEMTYPE])
	...
	xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100, "Attribute itemtype without attribute itemscope");
      itemtype = xp_mdata_parse_attr_value (xp, xn, MDATA_ATTR_ITEMTYPE, avalues, MDATA_ATTRSYNTAX_URI, NULL, NULL);
      ...
      if (NULL != old_itemtype_ptr)
	{
	  ...
	  xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100, "The subject '%.200s' has more than one itemtype definition",
	      inner->xmdatal_subj);
	}
      else
	{
  ...
  if ((NULL != avalues[MDATA_ATTR_ITEMPROP]) && !props_connect_local_id_to_local_itemscope)
      ...
      if ((NULL != outer->xmdatal_subj) && (0 <= obj_attr_idx))
	{
	  if (inner->xmdatal_subj != outer->xmdatal_subj)
	    xmlparser_logprintf (xp->xp_parser, XCFG_FATAL, 100,
		"An opening tag with itemscope or id can not contain both attribute itemprop and some object attribute");
	  for (prop_ctr = 0; prop_ctr < prop_count; prop_ctr++)
	    {
	      caddr_t prop = props[prop_ctr];
	      props[prop_ctr] = NULL;
...
rdfxml_parse (query_instance_t * qi, caddr_t text, caddr_t * err_ret,
    ...
    ccaddr_t * cbk_names, caddr_t * app_env, const char *enc, lang_handler_t * lh
