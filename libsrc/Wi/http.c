...
static int
http_acl_check_rate (ws_acl_t * elm, caddr_t name, int check_rate, int rw_flag, acl_hit_t ** hit_ret)
{
  int res;
  id_hash_t *loc_hash;
...
static int
http_acl_match (caddr_t * alist, caddr_t name, ccaddr_t dst, int obj_id, int rw_flag, int check_rate, acl_hit_t ** hit,
    ws_connection_t * ws)
{
  int inx;
  DO_BOX (ws_acl_t *, elm, inx, alist)
  {
	...
	if (dst == NULL && obj_id < 0 && rw_flag < 0)
	  return http_acl_check_rate (elm, name, check_rate, rw_flag, hit);
	    ...
	    && DVC_MATCH == cmp_like (dst, elm->ha_dest ? elm->ha_dest : "*", NULL, 0, LIKE_ARG_CHAR, LIKE_ARG_CHAR))
	  return http_acl_check_rate (elm, name, check_rate, rw_flag, hit);
...
caddr_t *
ws_read_post (dk_session_t * ses, int max, dk_session_t * str, dk_session_t * cont, dk_set_t * parts)
{
  char name[300];
  int inx = 0;
  int reading_name = 1;
...
http_log_file_check (struct tm *now)
{
  ...
  if (http_log_last_opened->tm_mday != now->tm_mday ||
      http_log_last_opened->tm_mon != now->tm_mon || http_log_last_opened->tm_year != now->tm_year)
    {
      ...
      while (ptmp > http_log_name)
	{
	  ...
	  http_log_last_opened->tm_mon + 1, http_log_last_opened->tm_year + 1900);
      strcat_ck (new_name, tstamp);
      if (ext)
	strcat_ck (new_name, ext);
      if (cfg_init2 (&pcfgFile, f_config_file, 1))
	goto error_end;
      if (cfg_write (pcfgFile, "HTTPServer", "HTTPLogFile", new_name) == -1 || cfg_commit (pcfgFile) == -1)
	goto error_end;
      cfg_done (pcfgFile);
...
log_info_http (ws_connection_t * ws, const char *code, OFF_T len)
{
  ...
  snprintf (buf, sizeof (buf),
      ...
      u_id, (tm->tm_mday), monthname[month - 1], year, tm->tm_hour, tm->tm_min, tm->tm_sec, (long) dt_local_tz_for_logs / 36 * 100,
      (ws->ws_req_line
#ifdef WM_ERROR
...
ws_read_multipart_mime_post (ws_connection_t * ws, int *is_stream)
{
  ...
  if (ws->ws_req_len < MIME_POST_LIMIT)
    {
      DO_BOX (caddr_t, line, inx, ws->ws_lines) if (inx)
...
ws_url_rewrite (ws_connection_t * ws)
{
  ...
  err = qr_quick_exec (url_rewrite_qr, cli, NULL, &lc, 3, ":0", ws->ws_path_string, QRP_STR, ":1", ws->ws_map->hm_url_rewrite_rule, QRP_STR, ":2", NULL == ws->ws_params ? list (0) : box_copy_tree ((caddr_t) (ws->ws_params)), QRP_RAW	/* compatibility with old execution sequence */
      );
...
ws_path_and_params (ws_connection_t * ws)
{
  ...
  while ('\x20' == ws->ws_req_line[inx])
    inx++;
  proto = ws->ws_req_line + inx;
  while ((unsigned char) (proto[0]) > 0x20)
    proto++;
  while ('\x20' == proto[0])
    proto++;
  ...
  ws->ws_try_pipeline = (nc_strstr ((unsigned const char *) ws_header_field (ws->ws_lines, "Connection:", ""),
      ...
      (ws->ws_proto_no > 10
	  && !nc_strstr ((const unsigned char *) ws_header_field (ws->ws_lines, "Connection:", ""), (unsigned char *) "close")));
#ifndef VIRTUAL_DIR
  {
...
/*    {
...
#ifdef DEBUG
  {
    ...
    for (i = 0; i < BOX_ELEMENTS (ws->ws_path); i++)
      printf (" '%s'", ws->ws_path[i]);
    printf (" )\n");
  }
#endif
  if (lc == '/' || (lc == '.' && n_fill == 1))
...
{
  const static char *defs[] = { "GET", "HEAD", "POST", "OPTIONS", NULL };
  int inx, m;
...
ws_check_accept (ws_connection_t * ws, char *mime, const char *code, int check_only, OFF_T clen, const char *charset)
{
  ...
  if (!match)
    {
      const char *cname = ws->ws_resource ? ws->ws_resource : (ws->ws_map
	  && ws->ws_map->hm_def_page ? ws->ws_map->hm_def_page : "index.html");
      caddr_t tmpbuf;
...
ws_strses_reply (ws_connection_t * ws, const char *volatile code)
{
  ...
  if (ws->ws_xslt_url)
    {
      ...
      client_connection_t *cli = ws->ws_cli;
      caddr_t url, xslt_url = ws->ws_xslt_url, xslt_parms = ws->ws_xslt_params;
      ...
      if (!err)
	{
	  ...
	  exec_params[0] = box_string ("_XML");
	  exec_params[1] = (caddr_t) & strses;
	  exec_params[2] = box_string ("DOC_URI");
	  exec_params[3] = (caddr_t) & url;
	  exec_params[4] = box_string ("XSLT_URI");
	  exec_params[5] = (caddr_t) & xslt_url;
	  exec_params[6] = box_string ("PARAMS");
	  exec_params[7] = (caddr_t) & xslt_parms;
	  exec_params[8] = box_string ("MEDIATYPE");
	  exec_params[9] = (caddr_t) & media_type;
	  exec_params[10] = box_string ("ENC");
	  exec_params[11] = (caddr_t) & xsl_encoding;
	  IN_TXN;
      ...
      if (err)
	{
	  ...
	  log_error ("%s : [%s] [%s]", err_msg, ERR_STATE (err), ERR_MESSAGE (err));

	  if (DV_STRINGP (ws->ws_status_line) && 0 != strncmp (ws->ws_status_line, "HTTP/1.1 401", 12))
	    {
	      dk_free_tree (ws->ws_status_line);
	      ws->ws_status_line = NULL;
	      ws->ws_status_code = 0;
	  ...
	  while (warnings)
	    {
	      ...
	      SES_PRINT (ws->ws_strses, "\n<br />" " <b>Warning</b>: STATE:<b>");

	      dks_esc_write (ws->ws_strses, ERR_STATE (warn),
		  box_length (ERR_STATE (warn)) - 1, WS_CHARSET (ws, NULL), default_charset, DKS_ESC_PTEXT);

	      SES_PRINT (ws->ws_strses, "</b> MESSAGE=<b>");

	      dks_esc_write (ws->ws_strses, ERR_MESSAGE (warn),
		  box_length (ERR_MESSAGE (warn)) - 1, WS_CHARSET (ws, NULL), default_charset, DKS_ESC_PTEXT);

	      SES_PRINT (ws->ws_strses, "</b><br />\n");
	      dk_free_tree (warn);
	    }
	  len = strses_length (ws->ws_strses);
	}
  ...
  if (IS_CHUNKED_OUTPUT (ws))
  ...
  else if (ws->ws_method != WM_HEAD && ws->ws_body_limit > 0 && ws->ws_body_limit < len)
    {
      ...
      HTTP_SET_STATUS_LINE (ws, code, 1);
      strses_flush (ws->ws_strses);
      ws_http_error (ws, "HTTP/1.1 509 Bandwidth Limit Exceeded", "Bandwidth Limit Exceeded", ws->ws_p_path_string,
	  ws->ws_path_string);
      len = strses_length (ws->ws_strses);
    }
...
/*      fprintf (stdout, "\nREPLY-----\n%s", tmp); */
    /* mime type */
    if (ws->ws_status_code != 101 && (!ws->ws_header
	    || (NULL == nc_strstr ((unsigned char *) ws->ws_header, (unsigned char *) "Content-Type:"))))
      {
#ifdef BIF_XML
	if (media_type)
	  {
...
static caddr_t
ws_get_ranges (ws_connection_t * ws, char *err_text, int err_text_max, volatile OFF_T size, OFF_T * ranges_in, int have_if_range)
{
...
send_multipart_byteranges (ws_connection_t * ws, int fd,
    ...
    const char *last_modify, const char *date_now, const char *http_server_id_string, wcharset_t * volatile charset)
{
  int mp;
  char head[4000];
  ...
  snprintf (head, sizeof (head),
      ...
      head_beg, etag, last_modify, date_now, http_server_id_string, ws->ws_try_pipeline ? "Keep-Alive" : "close");
  ...
  for (mp = 0; mp < n_ranges; mp++)
    {
	  ...
	  (OFF_T_PRINTF_DTP) ranges[mp * 2 + 1], (OFF_T_PRINTF_DTP) (ranges[mp * 2 + 1] - ranges[mp + 2] + 1));
      SES_PRINT (ws->ws_session, head);
...
ws_file (ws_connection_t * ws)
{
  ...
  if (ws->ws_method != WM_HEAD && ws->ws_method != WM_OPTIONS)
    {
      ...
      err_text[0] = 0;
      n_ranges = (int) (ptrlong) ws_get_ranges (ws, err_text, sizeof (err_text), off, &(ranges[0]), if_range ? 1 : 0);
      if (err_text[0])
	{
  ...
  CATCH_WRITE_FAIL (ws->ws_session)
  {
    if (n_ranges > 1)
	  ...
	  ctype, head_beg, etag, last_modify, date_now, http_server_id_string, charset);
    else
      {
	...
	snprintf (head, sizeof (head),
	    ...
	    ws->ws_try_pipeline ? "Keep-Alive" : "close",
	    (MAINTENANCE) ? "Retry-After: 1800\r\n" : "", ws->ws_header ? ws->ws_header : "", ranges_buffer);
	SES_PRINT (ws->ws_session, head);
	if (ws->ws_method == WM_OPTIONS && ws->ws_status_code < 400)
	  {
	    SES_PRINT (ws->ws_session, "Allow: ");
...
ws_post_process (ws_connection_t * ws)
{
  ...
  err = qr_quick_exec (http_ppr_qr, cli, NULL, NULL, 1, ":0", p_proc, QRP_STR);
...
ws_usr_qual (ws_connection_t * ws, int is_soap)
{
  ...
  if (!is_soap && (!ws || !ws->ws_map || !ws->ws_map->hm_vsp_uid || !IS_STRING_DTP (DV_TYPE_OF (ws->ws_map->hm_vsp_uid))))
    goto error_end;

  else if (is_soap && (!ws || !ws->ws_map || !ws->ws_map->hm_soap_uid || !IS_STRING_DTP (DV_TYPE_OF (ws->ws_map->hm_soap_uid))))
    goto error_end;
...
/*  if (!(proc = sch_proc_def (wi_inst.wi_schema, auth_proc)))*/
  ...
  err = qr_quick_exec (http_auth_qr, cli, NULL, &lc, 2, ":0", auth_proc, QRP_STR, ":1", auth_realm, QRP_STR);
...
static void
http_get_def_page (caddr_t fpath, caddr_t * ts2, caddr_t all_str, char *def_page, int def_page_len)
{
  char *tmp, *tok_s = NULL, *tok;
...
ws_check_rdf_accept (ws_connection_t * ws)
{
  ...
  if (NULL == strstr (accept, "application/rdf+xml") && NULL == strstr (accept, "text/rdf+n3")
      && NULL == strstr (accept, "text/turtle"))
    return 0;
  ...
  err = qr_quick_exec (qr, cli, NULL, &lc, 4,
      ...
      ":2", box_copy_tree ((ccaddr_t) (ws->ws_lines)), QRP_RAW, ":3", (ptrlong) http_check_rdf_accept, QRP_INT);
...
request_do_again:
  ...
  if (http_ses_trap &&
      ...
      (!nntp_port || ws->ws_port != nntp_port) && (!ftp_port || ws->ws_port != ftp_port))
#else
  if (http_ses_trap)
#endif
    {
	...
	{
	  err =
	      srv_make_new_error ("42000", "HTL01", "No virtual directory name found in the request line %.1000s", ws->ws_req_line);
	  dk_free_box (vdir);
	  goto rec_err_end;
	}
	      ...
	      !strcmp (save_history_name, "/")))
	{
	  ...
	  err = qr_quick_exec (stmt, ws->ws_cli, NULL, NULL, 2, ":0", save_history_name, QRP_STR, ":1", last_slash, QRP_STR);
  ...
  if (!soap_method)
      ...
      else if (ctype && !strcmp (ctype, SOAP_CTYPE_12))	/*application/soap+xml */
	{
	  ...
	  if (!soap_method)
	    soap_method = box_dv_short_string ("");
	  soap_version = 12;
	}
  ...
  else if (ws->ws_p_path && ((BOX_ELEMENTS (ws->ws_p_path) == 2 && !strcmp (ws->ws_p_path[0], "SOAP") && ((is_dsl = !strcmp (ws->ws_p_path[1], "services.xml")) || (is_wsdl = !strcmp (ws->ws_p_path[1], "services.wsdl")) || (is_wsdl = !strncmp (ws->ws_p_path[1], "services20.", 11)) ||	/* .xml, .rdf, .n3 goes here */
		  (is_vsmx = !strcmp (ws->ws_p_path[1], "services.vsmx")))) ||
	  (BOX_ELEMENTS (ws->ws_p_path) > 2 &&
	      !strcmp (ws->ws_p_path[0], "SOAP") && (is_http_binding = !strcmp (ws->ws_p_path[1], "Http")))))
    {
      SET_SOAP_USER (ws);
      if (is_dsl)
  ...
  else if (is_physical_soap && soap_get_opt_flag ((ws && ws->ws_map ? ws->ws_map->hm_soap_opts : NULL), "XML-RPC"))
    {
      deflt = 1;
  ...
  if (ws->ws_method != WM_ERROR)
      ...
      if (!deflt && !inln)	/* virtual directories features dir index & default page */
	{
	  ...
	  if (!ts_probe)
	      ...
	      else
		{
		  ...
		  if (ts2)
		    {
		      caddr_t l_path = dk_alloc_box (box_length (ws->ws_path_string) + strlen (ws->ws_map->hm_def_page), DV_STRING);
		      if (0 != def_page[0])
			{
			  dk_free_box (ts2);
			  http_get_def_page (fpath, &ts2, ws->ws_map->hm_def_page, def_page, sizeof (def_page));
...
#endif
      if (!sch_proc_def (isp_schema (NULL), p_name) || !ws->ws_map || !ws->ws_map->hm_vsp_uid)
	{
	  strcpy_ck (p_name, "WS.WS.DEFAULT");
	  deflt = 1;
	}
	...
	{
	  caddr_t ts;
	  caddr_t complete_name = dk_alloc_box (strlen (ws->ws_p_path_string) + strlen (www_root) + 2, DV_C_STRING);
...
{
  if (DKSESSTAT_ISSET (ws->ws_session, SST_OK) && !DKSESSTAT_ISSET (ws->ws_session, SST_BROKEN_CONNECTION))
    {
...
check_state:
  ...
  switch (ses->dks_ws_status)
    ...
    case DKS_WS_ACCEPTED:
      ...
      if (try_pipeline && ws_pipeline_ready (ws) && n_consec < 10)
	{
	  mutex_leave (ws_queue_mtx);
	  tws_immediate_reuse++;
	  n_consec++;
  ...
  if (DKS_WS_RUNNING == ses->dks_ws_status)
    {
      ...
      if (LT_PENDING == lt->lt_status && lt->lt_threads > 0)
	{
	  lt->lt_error = LTE_SQL_ERROR;
	  LT_ERROR_DETAIL_SET (lt, box_dv_short_string ("Async cancel on served connection"));
...
http_timeout_keep_alives (int must_kill)
{
  ...
  DO_SET (dk_session_t *, ses, &clients)
  {
	...
	== (io_action_func) ws_keep_alive_ready && ses->dks_last_used && DKS_WS_KEEP_ALIVE == ses->dks_ws_status)
      {
...
dk_session_t *
http_session_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
...
dk_session_t *
http_session_no_catch_arg (caddr_t * qst, state_slot_t ** args, int nth, const char *func)
{
  dk_session_t *res = NULL;
  if (((int) BOX_ELEMENTS (args)) > nth)
    {
  ...
  if (!res)
    {
      query_instance_t *qi = (query_instance_t *) qst;
      if (!qi->qi_client->cli_http_ses)
	sqlr_new_error ("37000", "HT069", "http output function %s() outside of http context and no stream specified", func);
...
void
dks_sqlval_esc_write (caddr_t * qst, dk_session_t * out, caddr_t val, wcharset_t * tgt_charset, wcharset_t * src_charset,
    int dks_esc_mode)
{
  ...
  if (DV_STRINGP (val))
  ...
  else if (DV_BLOB_WIDE_HANDLE == dtp)
    {
      ...
      dks_wide_esc_write (out, (wchar_t *) wstring, box_length (wstring) / sizeof (wchar_t) - 1, tgt_charset, dks_esc_mode);
      dk_free_box (wstring);
    }
#ifdef BIF_XML
  else if (DV_XML_ENTITY == dtp)
    {
      ...
      if (!old_enc)
	((xml_entity_t *) val)->xe_doc.xd->xout_encoding = (caddr_t) (CHARSET_NAME (tgt_charset, NULL));
...
bif_http_xmlelement_impl (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, int is_empty, const char *bifname)
{
  ...
  if ('?' == elt[0])
    {
      if (!is_empty)
	sqlr_new_error ("22023", "SR641",
	    "%s() is only for only plain elements, not for processing instructions like <%.200s ...?>", bifname, elt);
      session_buffered_write_char ('?', out);
    }
  else
    {
...
bif_http_rewrite (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_TYPE_OF (out) != DV_STRING_SESSION)
    sqlr_new_error ("22023", "HT070", "The HTTP output is not an STRING session in http_rewrite");
  strses_flush (out);
  return 0;
}
...
bif_http_pending_req (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (BOX_ELEMENTS (args) > 1)
    {
      ...
      ws = qi->qi_client->cli_ws;
      if (!ws)
	sqlr_new_error ("37000", "HT067", "The http_pending_req with parameter function allowed only inside HTTP request.");
      return (caddr_t) list (3, box_copy (ws->ws_client_ip), box_copy (ws->ws_path_string), box_num ((ptrlong) ws));
    }
  ...
  DO_SET (lock_trx_t *, lt, &all_trxs)
  {
	...
	arr = (caddr_t *) list (3, box_copy (ws->ws_client_ip), box_copy (ws->ws_path_string), box_num ((ptrlong) ws));
...
bif_http_lock (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!www_maintenance_page)
    sqlr_new_error ("22023", "HTERR",
	"The maintenance page is not specified, must have MaintenancePage setting in the HTTPServer section of the INI");
...
bif_http_internal_redirect (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (NULL == ws)
    sqlr_new_error ("42000", "HT067", "http_internal_redirect() function allowed only inside HTTP request");
...
remove_old_cached_sessions (void)
{
  ...
  DO_SET (ws_cached_connection_t *, conn, &ws_proxy_cache)
  {
    ...
    if ((now - ((long) conn->hit)) > MIN ((http_proxy_connection_cache_timeout * 1000L), (conn->timeout * 1000L)))
      {
	void *item = (void *) conn;
...
http_write_req (dk_session_t * ses, char *host, caddr_t * head, caddr_t * body, dk_session_t * ent_ses)
{
  ...
  while (*szContentType && *szContentType <= '\x20')
    szContentType++;
  if (!strnicmp (szContentType, "multipart", 9) || !strnicmp (szContentType, "application/x-www-form-urlencoded", 33))
...
dk_session_t *
http_request (char *host, caddr_t * req, caddr_t * body, caddr_t * err_ret, caddr_t ** head_ret, dk_session_t * ent_ses)
{
    ...
    caddr_t header, caddr_t body, caddr_t proxy, int strses_body)
{
  ...
  *err_ret = NULL;
  ...
  snprintf (req, sizeof (req), "%s %s HTTP/1.1\r\n" "Host: %s\r\n" "%s"
      ...
      (slash && *slash != 0) ? slash : "/",
      host, ua_fld, ((header != NULL) ? header : ""), ((header != NULL) ? "\r\n" : ""), content_type, len_fld);
...
ws_mime_header_field (caddr_t * head, const char *f, const char *subf, int initial_mode)
{
  ...
  DO_BOX (caddr_t, line, inx, head)
  {
    ...
    while (0 <= (offset =
	    mime_get_attr (line,
		(long) offset, ':', &rfc822, &override_to_mime, szName, sizeof (szName), szValue, sizeof (szValue))))
      {
	if (!stricmp (f, szName))
	  {
	    size_t len;
...
bif_http_proxy (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (dtp == DV_STRING_SESSION)
	...
	"Function http_proxy needs an array as argument 3, " "not an arg of type %s (%d)", dv_type_title (dtp), dtp);
...
http_client_cache_get (query_instance_t * qi, caddr_t url, caddr_t header, caddr_t body, state_slot_t ** args, int arg_pos)
{
  ...
  err = qr_rec_exec (qr, qi->qi_client, &lc, qi, NULL, 2, ":0", url, QRP_STR, ":1", http_client_cache_hash (header, body), QRP_RAW);
  if (err)
    {
      log_error ("Error retrieving http client cache : %s: %s", ((caddr_t *) err)[QC_ERRNO], ((caddr_t *) err)[QC_ERROR_STRING]);
      dk_free_tree (err);
    }
  else
    {
...
http_client_cache_register (query_instance_t * qi, caddr_t url, caddr_t header, caddr_t req_body, caddr_t * head, caddr_t body)
{
  ...
  err = qr_rec_exec (qr, qi->qi_client, NULL, qi, NULL, 4,
      ":0", url, QRP_STR,
      ":1", box_copy_tree ((caddr_t) head), QRP_RAW, ":2", body, QRP_STR, ":3", http_client_cache_hash (header, req_body), QRP_RAW);

  if (err)
    {
      log_error ("Error registering http client cache : %s: %s", ((caddr_t *) err)[QC_ERRNO], ((caddr_t *) err)[QC_ERROR_STRING]);
      dk_free_tree (err);
      err = NULL;
    }
}
...
bif_http_get (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (close || err || !SESSTAT_ISSET (ses->dks_session, SST_OK))
    ...
    {
      http_session_used (ses, ((proxy != NULL) ? proxy : host), peer_max_timeout);
    }
#endif
  *err_ret = err;
...
bif_http_output_flush (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (DV_TYPE_OF (strses) != DV_STRING_SESSION)
    sqlr_new_error ("22023", "HT031", "The HTTP output is not an STRING session in http_output_flush");
  strses_flush (strses);
  return (NULL);
}
...
ws_make_chunked (query_instance_t * qi, ws_connection_t * ws)
{
  ...
  if (ws->ws_header && NULL != nc_strstr ((unsigned char *) ws->ws_header, (unsigned char *) "Content-Length:"))
    sqlr_new_error ("42000", "HT064", "Chunked response and Content-Length specified by http_header()");

  if (ws->ws_header && NULL != nc_strstr ((unsigned char *) ws->ws_header, (unsigned char *) "Content-Encoding:"))
...
bif_sys_connected_server_address (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (!tcpses_getsockname (ses, buf, sizeof (buf)))
    {
#ifdef COM_UNIXSOCK
      if (BOX_ELEMENTS (args) && 1 == bif_long_arg (qst, args, 0, "sys_connected_server_address"))
	{
	  int port;
	  ...
	  if (is_default_host)
	    {
	      ...
	      snprintf (l_host_default, box_length (l_host_default), "//%s|%s%s", (lhost[0] == ':' ? "*all*" : lhost), lhost,
		  lpath);
	    }
	}
      dk_free_box (host);
...
https_cert_verify_callback (int ok, void *_ctx)
{
  ...
  if ((errnum == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT
	  || errnum == X509_V_ERR_SELF_SIGNED_CERT_IN_CHAIN || errnum == X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY
#if OPENSSL_VERSION_NUMBER >= 0x00905000
	  || errnum == X509_V_ERR_CERT_UNTRUSTED
#endif
	  || errnum == X509_V_ERR_UNABLE_TO_VERIFY_LEAF_SIGNATURE) && verify == HTTPS_VERIFY_OPTIONAL_NO_CA)
    {
  ...
  if (!ok)
    {
      log_error ("HTTPS Certificate Verification: Error (%d): %s", errnum, X509_verify_cert_error_string (errnum));
    }

  if (errdepth > depth)
    {
      log_error ("HTTPS Certificate Verification: Certificate Chain too long "
	  "(chain has %d certificates, but maximum allowed are only %ld)", errdepth, depth);
      ok = 0;
    }
  return (ok);
}
...
http_listen (char *host, caddr_t * https_opts)
{
  ...
  SESSION_SCH_DATA (listening)->sio_default_read_ready_action = (io_action_func) ws_ready;
...
bif_https_renegotiate (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (ssl)
	...
	verify |= SSL_VERIFY_PEER;
      ...
      SSL_set_session_id_context (ssl, (const unsigned char *) ((void *) &s_server_auth_session_id_context),
...
bif_http_debug_log (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (debug_log && fname_cvt)
      ...
      if (!debug_log)
	{
	  int errn = errno;
	  caddr_t err =
	      srv_make_new_error ("39000", "FA042", "Can't open debug log file '%.1000s', error : %s", fname_cvt, strerror (errn));
...
bif_sysacl_compose (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (src_len % 2)
    sqlr_new_error ("22023", "SA001",
	"The function sysacl_compose() expects get_keyword - style array of roles and permission bits as its argument)");
  gid_count = src_len / 2;
  ...
  for (gid_ctr = 0; gid_ctr < gid_count; gid_ctr++)
	...
	{
	  dk_free_box (res);
	  sqlr_new_error ("22023", "SA004",
	      "User or group should be identified by name or integer id (U_NAME or U_ID from DB.DBA.SYS_USERS)");
	}
      gid = grp->usr_id;
	...
	{
	  dk_free_box (res);
	  sqlr_new_error ("22023", "SA006", "Permission %ld is out of range of valid permisions (from 0 to %ld)", (long) perm,
	      (long) maxperm);
	}
      for (ctr2 = 0; ctr2 < gid_ctr; ctr2++)
	{
	  if (gids[ctr2] != gid)
	    continue;
	  dk_free_box (res);
	  sqlr_new_error ("22023", "SA007",
	      "User or group with id %ld is listed twice, in indicies %ld and %ld of the source vector", (long) gid,
	      (long) (ctr2 * 2), (long) (gid_ctr * 2));
	}
      gids[gid_ctr] = gid;
      perms[gid_ctr] = perm;
    }
  ...
  for (gid_ctr = gid_count; gid_ctr--; /* no step */ )
      ...
      for (ctr2 = 0; ctr2 < gid_ctr; ctr2++)
	{
	  ...
	  if (gids[ctr2] < gids[ctr2 + 1])
	    continue;
	  swap_gid = gids[ctr2];
	  gids[ctr2] = gids[ctr2 + 1];
	  gids[ctr2 + 1] = swap_gid;
	  swap_perm = perms[ctr2];
	  perms[ctr2] = perms[ctr2 + 1];
	  perms[ctr2 + 1] = swap_perm;
	}
    }
...
bif_sysacl_direct_bits_of_user (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (1 < BOX_ELEMENTS (args))
    user =
	bif_user_t_arg (qst, args, 1, "sysacl_direct_bits_of_user",
	(USER_SHOULD_EXIST | USER_NOBODY_IS_PERMITTED | USER_SPARQL_IS_PERMITTED), 1);
  else
...
bif_sysacl_all_bits_of_tree (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (1 < BOX_ELEMENTS (args))
    user =
	bif_user_t_arg (qst, args, 1, "sysacl_all_bits_of_tree",
	(USER_SHOULD_EXIST | USER_NOBODY_IS_PERMITTED | USER_SPARQL_IS_PERMITTED), 1);
  else
    user = ((query_instance_t *) qst)->qi_client->cli_user;
...
bif_sysacl_bit1_of_tree (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  if (1 < BOX_ELEMENTS (args))
    user =
	bif_user_t_arg (qst, args, 1, "sysacl_bit1_of_tree",
	(USER_SHOULD_EXIST | USER_NOBODY_IS_PERMITTED | USER_SPARQL_IS_PERMITTED), 1);
  else
    user = ((query_instance_t *) qst)->qi_client->cli_user;
...
bif_sysacl_bit1_of_tree_vec (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args, state_slot_t * ret)
{
  ...
  SET_LOOP
  {
	...
	switch (DV_TYPE_OF (user_name_or_id))
	  {
	  case DV_LONG_INT:
	    curr_user = sec_id_to_user (unbox (user_name_or_id));
	    break;
	  case DV_STRING:
	    curr_user = sec_name_to_user (user_name_or_id);
	    break;
	  default:
	    bit1 = 0;
	    goto ans_done;
...
ws_serve_client_connection (ws_connection_t * ws)
{
  ...
  err = qr_quick_exec (qr, cli, NULL, NULL, 3, ":0", p_name, QRP_STR, ":1", conn, QRP_RAW, ":2", cd, QRP_RAW);
...
http_init_part_one ()
{
  ...
  if (!ws_default_charset)
      ...
      else
	{
	  ...
	  if (ws_default_charset_name)
	    log_error ("Default HTTP charset %.200s not defined."
		" Reverting to the default database charset", ws_default_charset_name);
	}
    }
  ...
  if (!http_server_id_string)
    {
	  ...
	  build_special_server_model[0] ? " " : "", build_special_server_model);
...
#define WS_MIN_RC 1
  ...
  DO_SET (ws_connection_t *, ws, &ws_threads) ws->ws_thr_cache_clear = 1;
  END_DO_SET ();
  ...
  if (pop3_port)
    {
      ...
      SESSION_SCH_DATA (pop3_listen)->sio_default_read_ready_action = (io_action_func) ws_ready;
  ...
  if (nntp_port)
    {
      ...
      SESSION_SCH_DATA (nntp_listen)->sio_default_read_ready_action = (io_action_func) ws_ready;
  ...
  if (ftp_port)
    {
      ...
      SESSION_SCH_DATA (ftp_listen)->sio_default_read_ready_action = (io_action_func) ws_ready;
...
{
  if (cli->cli_ws && cli->cli_ws->ws_flushed != 1 && !cli->cli_ws->ws_ignore_disconnect)
    {
	  ...
	  cli->cli_ws->ws_session->dks_in_fill < cli->cli_ws->ws_session->dks_in_length)
	{
	  ...
	  if (SER_SUCC == tcpses_is_read_ready (ws->ws_session->dks_session, &to))
	      ...
	      else
		{
		  ...
		  if (rc == -1 || rc == 0 || SESSTAT_ISSET (ws->ws_session->dks_session, SST_BROKEN_CONNECTION))
		    return 1;
		  else
  ...
  *szBoundry = 0;
  ...
  DO_BOX (caddr_t *, part, inx, parts)
  {
    ...
    dk_set_push (&ret_attrs, box_varchar_string ((db_buf_t) (buff2 + body_start_offset),
	    body_end_offset - body_start_offset, DV_SHORT_STRING));
