...
static char base64_vec[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";

int
xenc_encode_base64 (char *input, char *output, size_t len)
{
  unsigned char c;
  int n = 0, i, count = 0, x = 0;
  size_t j = 0;
  unsigned long val = 0;
  unsigned char enc[4];
...
static int
add_algo_to_store (algo_store_t * s, const char *xmln, void *f)
{
...
int
dsig_digest_algo_create (const char *xmln, dsig_digest_f f)
{
  return add_algo_to_store (select_store ("digest"), xmln, (void *) f);
}

int
dsig_sign_algo_create (const char *xmln, dsig_sign_f f)
{
  return add_algo_to_store (select_store ("dsig"), xmln, (void *) f);
}

int
dsig_verify_algo_create (const char *xmln, dsig_verify_f f)
{
  return add_algo_to_store (select_store ("verify"), xmln, (void *) f);
}

int
dsig_canon_algo_create (const char *xmln, dsig_canon_f f)
{
  return add_algo_to_store (select_store ("canon"), xmln, (void *) f);
}

int
dsig_canon_2_algo_create (const char *xmln, dsig_canon_2_f f)
{
  return add_algo_to_store (select_store ("canon_2"), xmln, (void *) f);
}

int
dsig_transform_algo_create (const char *xmln, dsig_transform_f f)
{
  return add_algo_to_store (select_store ("trans"), xmln, (void *) f);
}

int
dsig_single_transform_algo (query_instance_t * qi, dk_session_t * ses_in, long len, dk_session_t * ses_out, caddr_t transform_data)
{
  char buf[1024];
  int read_b;
  int err = 0;
...
int
xml_canon_exc_algo (dk_session_t * ses_in, long len, dk_session_t * ses_out)
{
...
int
xml_c_build_ancessor_ns_link (caddr_t * doc_tree, caddr_t * select_tree, id_hash_t * nss, dk_set_t * parent_link)
{
...
static int
strcmp_1 (const void *n1, const void *n2)
{
  int ret;
  char *ns1 = ((char **) n1)[0];
  char *ns2 = ((char **) n2)[0];
...
void
xml_c_namespaces_sort (caddr_t * namespaces)
{
  if (namespaces)
    qsort (namespaces, box_length (namespaces) / sizeof (caddr_t) / 2, sizeof (caddr_t) * 2, strcmp_1);
}

void
xml_c_attributes_sort (caddr_t * tree)
{
...
void
xml_c_mark_as_busy (xml_c_stack exc_list, caddr_t * exc_ns, int inx)
{
  ...
  new_curr_ns = (caddr_t *) dk_alloc_box (curr_ns ? box_length (curr_ns) : 0 + 2 * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
...
int
xml_c_get_namespace (caddr_t name, xml_c_stack exc_list, caddr_t * prefix, caddr_t * ns, int depth)
{
...
static void
xml_c_fill_exc_list_up (xml_c_stack exc_list, caddr_t * namespaces)
{
...
static void
xmlc_c_clean_exc_list_up (xml_c_stack exc_list)
{
...
static caddr_t *
xml_c_new_namespace (const char *prefix, const char *uri)
{
...
caddr_t *
xml_c_namespaces_sort_2 (caddr_t * tree, xml_c_stack exc_list, int depth)
{
  caddr_t *tag = ((caddr_t **) tree)[0] + 1;
...
void
xml_c_nss_free (id_hash_t * nss)
{
  id_hash_iterator_t iter;
  caddr_t **tree;
  caddr_t **namespaces;

  for (id_hash_iterator (&iter, nss); hit_next (&iter, (char **) &(tree), (char **) &namespaces);
      /* */ )
    {
      if (namespaces)
...
id_hash_t *
xml_c_nss_hash_create (caddr_t * select_tree, id_hash_t * nss, caddr_t * new_namespaces)
{
...
int
xml_canonicalize (query_instance_t * qi, caddr_t * doc_tree, caddr_t * select_tree, id_hash_t * nss, dk_session_t * ses_out)
{
...
caddr_t *
xml_find_any_child (caddr_t * curr, const char *name, const char *uri)
{
  ...
  DO_BOX (caddr_t *, child, inx, curr)
  {
	...
	if (szColon && !uri)
	  ...
	  goto internal_search;
	else if (szColon && uri && ((URILen != (szColon - szName)) || strnicmp (szName, uri, MIN (szColon - szName, URILen))))
	  goto internal_search;
...
int
dsig_tr_enveloped_signature (query_instance_t * qi, dk_session_t * ses_in, long len, dk_session_t * ses_out, caddr_t transform_data)
{
  ...
  xte = (xml_tree_ent_t *) xml_make_tree_with_ns (qi, xml_text, &err_ret, "UTF-8", lh_get_handler ("x-any"), &nss, 0);
  dk_free_box (xml_text);
...
int
dsig_tr_canon_exc_algo (query_instance_t * qi, dk_session_t * ses_in, long len, dk_session_t * ses_out, caddr_t transform_data)
{
...
int
dsig_tr_fake_uri (query_instance_t * qi, dk_session_t * ses_in, long len, dk_session_t * ses_out, caddr_t transform_data)
{
  ...
  xte = (xml_tree_ent_t *) xml_make_tree_with_ns (qi, text, &err_ret, "UTF-8", lh_get_handler ("x-any"), &nss, 0);
...
caddr_t
xenc_alloc_cbc_box (long len, size_t block_size, dtp_t t1)
{
...
int
xenc_write_cbc_buffer (unsigned char *buf, long len, dk_session_t * out)
{
...
#ifdef AES_ENC_ENABLE
int
xenc_aes_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  ...
  AES_cbc_encrypt (text, outbuf, box_length (text), &aes_schedule, (unsigned char *) key->ki.aes.iv, AES_ENCRYPT);
#endif
...
int
xenc_aes_decryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  caddr_t text;
  ...
  if (!EVP_DecryptFinal_ex (&ctx, outbuf + outlen, &tmplen))
    {
      ...
      dk_free_box (text_beg);
      dk_free_box (outbuf);
      xenc_report_error (t, 500, XENC_ENC_ERR, "AES decryption internal error %s", ERR_error_string (err, buf));
    }
  outlen += tmplen;
  EVP_CIPHER_CTX_cleanup (&ctx);
#else
  ...
  AES_cbc_encrypt ((const unsigned char *) text, (unsigned char *) text,
      outlen, &aes_schedule, (unsigned char *) key->ki.aes.iv, AES_DECRYPT);
#endif
...
#endif
int
xenc_dsa_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  if (t)
...
int
xenc_dsa_decryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  if (t)
    xenc_report_error (t, 500, XENC_ENC_ERR, "DSA is for signatures only and is not an encryption algorithm");
  return 0;
...
int
xenc_dh_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  if (t)
...
int
xenc_dh_decryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  if (t)
    xenc_report_error (t, 500, XENC_ENC_ERR, "DH is for signatures only and is not an encryption algorithm");
  return 0;
...
int
xenc_rsa_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  char *buf;
  char *out_buf = NULL;
  int tot_l = 0;
  ...
  if (!seslen)
    return 0;
  if (seslen != 16 && seslen != 24 && seslen != 32)
    return 0;
...
int
xenc_rsa_decryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  char *buf;
  char *out_buf = 0;
  int err = 1;
  ...
  if (key->xek_type != DSIG_KEY_RSA)
    {
      xenc_report_error (t, 500 + strlen (key->xek_name), XENC_ENC_ERR, "could not make RSA decryption [key %s is not RSA]",
	  key->xek_name);
      return 0;
    }
  if (!rsa || !rsa->p || !rsa->q)
      ...
      if (key && key->xek_private_rsa)
	{
      ...
      xenc_report_error (t, 500 + strlen (key->xek_name), XENC_ENC_ERR, "could not make RSA decryption [key %s is not RSA private]",
	  key->xek_name);
      return 0;
    }
cont:
...
#ifdef _KERBEROS
caddr_t krb_seal (gss_ctx_id_t context, caddr_t in_buf_cont);

int
xenc_kerberos_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  caddr_t in_buf = 0;
  caddr_t ret_buf = 0;
  int err = 1;
  ...
  dk_free_box (ret_buf);
  if (err)
    xenc_report_error (t, 500 + strlen (key->xek_name), XENC_ENC_ERR, "could not encrypt by kerberos key %s", key->xek_name);
  return 0;
}
#endif
...
int
xenc_des3_encryptor (dk_session_t * ses_in, long seslen, dk_session_t * ses_out_base64, xenc_key_t * key, xenc_try_block_t * t)
{
  ...
  while (blocks >= 0)
      ...
      if (!blocks)
	{
	  ...
	  (long) DES_BLOCK_LEN,
	  key->ki.triple_des.ks1, key->ki.triple_des.ks2, key->ki.triple_des.ks3, (des_cblock *) _iv, DES_ENCRYPT);
      total_blocks++;
...
#if 0
int
xenc_des3_decryptor (dk_session_t * ses_in_base64, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  int read_b;
  int len, failed = 0;
  char buf[DES_BLOCK_LEN + 1];
  ...
  for (; !failed;)
    {
      des_ede3_cbc_encrypt ((const unsigned char *) buf,
	  (unsigned char *) out_buf,
	  (long) DES_BLOCK_LEN, key->ki.triple_des.ks1, key->ki.triple_des.ks2, key->ki.triple_des.ks3, &iv, DES_DECRYPT);
      CATCH_READ_FAIL (ses_in)
      {
	read_b = session_buffered_read (ses_in, buf, DES_BLOCK_LEN);
      }
...
int
xenc_signature_wrapper (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  return 0;
}

int
xenc_signature_wrapper_1 (dk_session_t * ses_in, long seslen, dk_session_t * ses_out, xenc_key_t * key, xenc_try_block_t * t)
{
  return 0;
}
...
void
xenc_aes_enctest_1 (const char *data)
{
#ifdef AES_ENC_ENABLE
...
xenc_aes_enctest ()
{
  ...
  xenc_aes_enctest_1
      ("The Importers are used by the proxy generator of ASP.NET, which is used by Visual Studio .NET and the wsdl.exe command-line tool. The Importers will pick up any known <<format extensions>> that exist in the WSDL file and will turn them into client");
  xenc_aes_enctest_1
      ("The Importers are used by the proxy generator of ASP.NET, which is used by Visual Studio .NET and the wsdl.exe command-line tool. The Importers will pick up any known");
  xenc_aes_enctest_1
...
void
xenc_asserts_print_report (FILE * stream)
{
...
/* digest test funcs */
int
dsig_test_digest_1 (dk_session_t * ses, long len, caddr_t * out)
{
  return 0;
}

int
dsig_test_digest_2 (dk_session_t * ses, long len, caddr_t * out)
{
  return 0;
}

int
dsig_test_digest_3 (dk_session_t * ses, long len, caddr_t * out)
{
  return 0;
}

int
dsig_test_digest_4 (dk_session_t * ses, long len, caddr_t * out)
{
  return 0;
}

int
dsig_test_digest_5 (dk_session_t * ses, long len, caddr_t * out)
{
  return 0;
}
...
/* trans test func */
int
dsig_test_trans_1 (query_instance_t * qi, dk_session_t * ses, long len, dk_session_t * ses_out, caddr_t tr_data)
{
  return 0;
}

int
dsig_test_trans_2 (query_instance_t * qi, dk_session_t * ses, long len, dk_session_t * ses_out, caddr_t tr_data)
{
  return 0;
}

int
dsig_test_trans_3 (query_instance_t * qi, dk_session_t * ses, long len, dk_session_t * ses_out, caddr_t tr_data)
{
  return 0;
}

int
dsig_test_trans_4 (query_instance_t * qi, dk_session_t * ses, long len, dk_session_t * ses_out, caddr_t tr_data)
{
  return 0;
}

int
dsig_test_trans_5 (query_instance_t * qi, dk_session_t * ses, long len, dk_session_t * ses_out, caddr_t tr_data)
{
  return 0;
}
...
/* dsig */
int
dsig_test_sign_1 (dk_session_t * ses, long len, xenc_key_t * key, caddr_t * out)
{
  return 0;
}

int
dsig_test_sign_2 (dk_session_t * ses, long len, xenc_key_t * key, caddr_t * out)
{
  return 0;
}

int
dsig_test_sign_3 (dk_session_t * ses, long len, xenc_key_t * key, caddr_t * out)
{
  return 0;
}

int
dsig_test_sign_4 (dk_session_t * ses, long len, xenc_key_t * key, caddr_t * out)
{
  return 0;
}

int
dsig_test_sign_5 (dk_session_t * ses, long len, xenc_key_t * key, caddr_t * out)
{
  return 0;
}
...
/* canon */
int
dsig_test_canon_1 (dk_session_t * in, long len, dk_session_t * out)
{
  return 0;
}

int
dsig_test_canon_2 (dk_session_t * in, long len, dk_session_t * out)
{
  return 0;
}

int
dsig_test_canon_3 (dk_session_t * in, long len, dk_session_t * out)
{
  return 0;
}

int
dsig_test_canon_4 (dk_session_t * in, long len, dk_session_t * out)
{
  return 0;
}

int
dsig_test_canon_5 (dk_session_t * in, long len, dk_session_t * out)
{
  return 0;
}
...
static char soap_msg[] =
    ...
    "         </xenc:CipherData>\n" "      </xenc:EncryptedData>\n" "   </S:Body>\n" "</S:Envelope>\n";
void
dsig_transform_test (query_instance_t * qi, dsig_transform_f tr, const char *tr_name, const char *msg_result, caddr_t tr_data)
{
...
dsig_tr_enveloped_signature_test (query_instance_t * qi)
{
  ...
  dsig_transform_test (qi, dsig_tr_fake_uri, "FAKE Uri Transform",
      "<xenc:CipherData Id=\"Id-CD\">\n"
      "                <xenc:CipherValue>d2FpbmdvbGRfE0lm4byV0...\n"
      "                </xenc:CipherValue>\n" "             </xenc:CipherData>", box_dv_short_string ("#Id-CD"));
#endif
...
dsig_sha1_digest_test ()
{
  ...
  if (!digest || strcmp (digest, digest_etalon))
    {
      xenc_assert (0);
...
P_SHA1_init (const char *secret, int secret_len, const char *seed, int seed_len)
{
  ...
  HMAC (EVP_sha1 (), secret, secret_len, (const unsigned char *) seed, seed_len, (unsigned char *) ctx->A, NULL);
  return ctx;
}
...
P_SHA1_block (P_SHA1_CTX * ctx, char *dst)
{
  ...
  HMAC (EVP_sha1 (), psha1_secret (ctx), ctx->secret_len,
      (const unsigned char *) ctx->A, sizeof (ctx->A) + ctx->seed_len, (unsigned char *) dst, NULL);
  ...
  HMAC (EVP_sha1 (), psha1_secret (ctx), ctx->secret_len,
      (const unsigned char *) ctx->A, sizeof (ctx->A), (unsigned char *) ctx->A, NULL);
}
...
generate_algo_accessor (canon_2, "canon_2") generate_algo_accessor (digest, "digest") generate_algo_accessor (transform, "trans")
     void
     algo_stores_init ()
{
  ptrlong idx;
...
dsig_sec_init ()
{
  ...
  xenc_algorithms_create (DSIG_HMAC_SHA1_ALGO, "hmac sha1 algorithm",
      xenc_signature_wrapper, xenc_signature_wrapper_1, DSIG_KEY_KERBEROS);
