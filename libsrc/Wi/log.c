...
err_log_error (caddr_t err)
{
  if (err == (caddr_t) SQL_NO_DATA_FOUND)
    ...
    {
      log_error ("SQL Error: %s : %s", ((caddr_t *) err)[1], ((caddr_t *) err)[2]);
    }
}
...
{
  LT_ERROR_DETAIL_SET (lt, box_sprintf (300, "%.30s %.160s : %.100s", text, name, virt_strerror (eno)));
}
...
log_time (caddr_t * box)
{
  ...
  if (!box && log_in_cl_recov)
    return LTE_OK;
  if (!box && (!last_log_time_written || approx_msec_real_time () - last_log_time_written > 30000))
    {
      char dt[DT_LENGTH];
      dt_now (dt);
...
log_change_if_needed (lock_trx_t * lt, int rewrite)
{
  ...
  if (dbs->dbs_log_length > ls->ls_bytes)
      ...
      if (!ls)
	{
      ...
      if (new_fd < 0)
	{
	  srv_report_errno_trx_error (lt, "Error opening the log segment file", ls->ls_file, errno);
	  return LTE_LOG_FAILED;
	}
      close (old_fd);
...
log_commit (lock_trx_t * lt)
{
  ...
  if (!dbs->dbs_log_session)
	...
	{
	  log_error
	      ("There is a non-zero length log when starting crash dump.  Please move this log away, complete  the crash dump and recovery and then replay this log, using the -R or +restore-crash-dump command line option.  Exiting.\n");
	  exit (1);
	}
  ...
  CATCH_WRITE_FAIL (log_ses)
  {
    ...
    if (LT_PREPARE_PENDING == lt->lt_status)
      {
	lt->lt_2pc._2pc_logged = prev_length + log_ses->dks_bytes_sent + log_ses->dks_out_fill - 1;
...

  {
    ...
    if (autocheckpoint_log_size > 0 && dbs->dbs_log_length >= autocheckpoint_log_size && !auto_cpt_scheduled)
      {
	...
	if (DV_LONG_INT == DV_TYPE_OF (item))
	  {
	    ...
	    if (QFID_HOST (id) == local_cll.cll_this_host && (!last_id || W_ID_GT ((uint32) id, last_id)))
	      last_id = (uint32) id;
	  }
	dk_free_box (item);
      }
...
lt_log_merge (lock_trx_t * lt, int in_txn)
{
  ptrlong plt;
  lock_trx_t *main_lt;
  if (LT_PENDING != lt->lt_status || !lt->lt_rc_w_id || lt->lt_rc_w_id == lt->lt_w_id || !strses_out_bytes (lt->lt_log))
    return LTE_OK;
  if (!in_txn)
    IN_TXN;
  ASSERT_IN_TXN;
...
log_insert (lock_trx_t * lt, row_delta_t * rd, int flag)
{
...
#if LOG_DEBUG_LEVEL>1
  {
	...
	op2 == LOG_INSERT_REPL ? "LOG_INSERT_REPL" : op2 == LOG_INSERT_SOFT ? "LOG_INSERT_SOFT" : "LOG_INSERT";
...
{
  log_text_array (lt, list (2, box_string ("sequence_remove (?)"), box_dv_short_string (text)));
}


void
log_update (lock_trx_t * lt, row_delta_t * rd, update_node_t * upd, caddr_t * qst)
{
  int inx;
  if (!lt || lt->lt_replicate == REPL_NO_LOG || cl_non_logged_write_mode)
    return;
...
/*  log_info ("t%d:n%d sync op %d key_id=%d", rfwd_ctr, op_ctr, flag, row_key_id((caddr_t*)row));  */
  ...
  if (flag == LOG_INSERT_SOFT)
    ...
    st = ins_replay;
  err = qr_quick_exec (st, cli, "x", NULL, 1, ":PL", row, QRP_RAW);
  return err;
}
...
log_replay_key_insert (lock_trx_t * lt, dk_session_t * in, int flag1)
{
  ...
  if (flag == LOG_INSERT_SOFT)
    ...
    flag = INS_NORMAL;
  err = qr_quick_exec (ins_key_replay, lt->lt_client, "x", NULL, 2, ":0", row, QRP_RAW, ":1", (caddr_t) (ptrlong) flag, QRP_INT);
  return err;
}
...
log_replay_key_insert_row (client_connection_t * cli, caddr_t * row, int flag)
{
  ...
  if (flag == LOG_INSERT_SOFT)
    ...
    flag = INS_NORMAL;
  err = qr_quick_exec (ins_key_replay, cli, "x", NULL, 2, ":0", row, QRP_RAW, ":1", (caddr_t) (ptrlong) flag, QRP_INT);
  return err;
}
...
log_replay_delete (lock_trx_t * lt, dk_session_t * in)
{
  ...
  err = qr_rec_exec (qr, lt->lt_client, NULL, CALLER_LOCAL, opts, 1, ":PL", row, QRP_RAW);
...
log_replay_key_delete (lock_trx_t * lt, dk_session_t * in)
{
  ...
  err = qr_rec_exec (qr, lt->lt_client, NULL, CALLER_LOCAL, opts, 1, ":0", row, QRP_RAW);
...
log_replay_text_1 (lock_trx_t * lt, caddr_t * entry, int is_pushback, int use_stmt_cache)
{
  ...
  if (!is_pushback)
	...
	{
	  dbg_printf_1 (("log_replay_text: '%s': unknown statement type, skipping conflict resolution", text));
	  goto cr_done;
	}
	...
	{
	  dk_free_box ((box_t) entry);
	  return srv_make_new_error ("42000", "TR083", "log_replay_text: dtp is not DV_ARRAY_OF_POINTER (%d)", dtp);
	}
      n_args = BOX_ELEMENTS (entry) - 1;
      n_cols = 0;
	  ...
	  if ((dtp = DV_TYPE_OF (entry[n_args])) != DV_LONG_INT)
	    {
	      ...
	      return srv_make_new_error ("42000", "TR084", "log_replay_text: n_cols is not DV_LONG_INT (%d)", dtp);
	    }
	  n_cols = (int) unbox (entry[n_args]);
	  if (n_cols <= 0)
	    {
	      dk_free_tree ((box_t) entry);
	      return srv_make_new_error ("42000", "TR085", "log_replay_text: invalid n_cols (%d)", n_cols);
	    }
	  n_args -= n_cols + 1;
	  if (n_args < 0)
	    {
	      dk_free_tree ((box_t) entry);
	      return srv_make_new_error ("42000", "TR086", "log_replay_text: n_cols (%d) + 1 > n_args (%d)", n_cols, n_args);
	    }
	}
      if (cr_type == 'U')
	...
	n_cr_args = n_cols + 1;
#if 0
      dbg_printf_1 (("cr_type: '%c', n_cols: %d, n_args: %d, n_cr_args: %d", cr_type, n_cols, n_args, n_cr_args));
#endif
	...
	q = strchr (table_name, '\0');
      stmt = dk_alloc_box (sizeof (cr_prefix) + (q - table_name) * 2 + 3 + 1 + (3 * n_cr_args - 2) + 1, DV_LONG_STRING);
      ...
      if (!qr)
	{
      ...
      arr = (caddr_t *) dk_alloc_box (n_cr_args * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
      inx = 0;
      ...
      if (cr_type == 'D' || cr_type == 'U')
	{
      ...
      err = qr_exec (lt->lt_client, qr, CALLER_LOCAL, NULL, NULL, &lc, arr, cr_opts, 0);
	...
	{
	  dk_free_tree ((box_t) entry);
	  return srv_make_new_error ("42000", "TR087", "log_replay_text: NULL lc");
	}
      if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (lc->lc_proc_ret))
	{
	  dk_free_tree ((box_t) entry);
	  lc_free (lc);
	  return srv_make_new_error ("42000", "TR088", "log_replay_text: invalid lc type");
	}
      ...
      if (BOX_ELEMENTS (proc_ret) < 2)
	{
	  ...
	  return srv_make_new_error ("42000", "TR089",
	      "log_replay_text: lc too short (want %d, have %ld)", 2, (long) (BOX_ELEMENTS (proc_ret)));
	}
	  ...
	  if (cr_type == 'I' || cr_type == 'U')
	      ...
	      if (BOX_ELEMENTS_INT (proc_ret) < 2 + n_cols)
		{
		  ...
		  return srv_make_new_error ("42000", "TR090",
		      "log_replay_text: lc too short (want %d, have %ld)", 2 + n_cols, (long) (BOX_ELEMENTS (proc_ret)));
		}
	...
	case 4:		/* reject */
	  lc_free (lc);
	  dk_free_tree ((box_t) entry);
	  return srv_make_new_error ("42000", "TR091", "log_replay_text: conflict resolver returned %d (reject)", retc);
	  break;
	...
	default:
	  lc_free (lc);
	  dk_free_tree ((box_t) entry);
	  return srv_make_new_error ("42000", "TR092", "log_replay_text: conflict resolver returned %d (unknown)", retc);
	  break;
	}
    }
  ...
  for (;;)
    {
      ...
      va_end (args);
      if ((outsize != -1) && (outsize < bufsize - 1))
...
log_key_ins_del_qr (dbe_key_t * key, caddr_t * err_ret, int op, int ins_mode, int is_rfwd)
{
  ...
  switch (op)
	...
	int n_cols, k, need_comma = 0;
	char **names;
	sb_printf (&sb, "DELETE FROM \"%s\".\"%s\".\"%s\"", ESC (key_table->tb_qualifier, 1), ESC (key_table->tb_owner, 2),
...
lre_wait_all (lr_executor_t * executor)
{
  ...
  DO_HT (void *, key_id, lre_queue_t *, lrq, executor->lre_aqs) flush_request (executor, lrq);
  END_DO_HT;
  for (;;)
    {
      mutex_enter (executor->lre_mtx);
...
repl_append_vec_entry_async (lre_queue_t * lq, client_connection_t * cli, lre_request_t * request, caddr_t * row, char flag)
{
  ...
  if (qr_pool == NULL)
	...
	{
	  log_error ("in repl_append_vec_entry_async: key_id=%d, op=%d, query_params=%d, row_params=%d", request->lr_lrq->lr_key_id,
	      request->lr_op, inx, n_pars);
	  GPF_T1 ("repl_append_vec_entry_async param count");
	}
    }
  n_actual_params = dk_set_length (request->lr_qr->qr_parms);
  /* check for blobs */
  if (key && key->key_row_var && request->lr_op != LOG_KEY_DELETE && request->lr_op != LOG_DELETE)	/* delete log only PK */
    {
      DO_CL (cl, key->key_row_var)
      {
	...
	if (IS_BLOB_DTP (dtp))
	  {
		...
		if (-1 == inx_in_rd)
		  GPF_T1 ("rd bad in blob ins replay");
...
log_is_cl_prepared (caddr_t * header)
{
  ...
  return id && (LOG_CL_2PC_PREPARE == id[0] || LOG_CL_2PC_PREPARE_FROM_SYNC == id[0]);
}
...
repl_cycle_message (caddr_t * header)
{
  ...
  if (tc_repl_cycle < 5)
	...
	acct = "<unknown>";
	  ...
	  ":: server %s account %s transaction %ld has this server for origin.", srv, acct, unbox (repl[REPLH_LEVEL]));
    }
}
...
log_handle_double_sync (client_connection_t * repl_cli, caddr_t * repl_header, caddr_t trx_id)
{
  ...
  if (lt->lt_threads || lt->lt_client)
    {
      log_info ("Host %d: Found w_id %d:%d in log sync with a client or thread.  First abort this and then replay the incoming",
	  local_cll.cll_this_host, QFID_HOST (w_id), (int32) w_id);
      if (LT_DELTA_ROLLED_BACK != lt->lt_status)
	lt_kill_other_trx (lt, NULL, NULL, LT_KILL_ROLLBACK);
    }
  else
    {
      log_info ("Host %d: Getting a duplicate sync %d:%d, can happen if cpt relogged a prepared.", local_cll.cll_this_host,
	  QFID_HOST (w_id), (int32) w_id);
      lt->lt_threads = 1;
  ...
  if (!is_repl)
    {
      ...
      if ((LOG_VIRT_2PC_PREPARE == st_2pc) || (LOG_MTS_2PC_PREPARE == st_2pc))
	{
	  ...
	  if (LOG_VIRT_2PC_PREPARE == st_2pc)
	    ...
	    {
	      log_error
		  ("MS DTC unfinished transaction has been found, but no MS DTC support had been loaded. Either remove log file, or load MS DTC support plugin. exit");
	      call_exit (1);
	    }
  ...
  if (is_cl_prepared)
    {
      ...
      log_info ("Host %d:  In log %s got prepared lt %d:%d, will resolve later from other branches", local_cll.cll_this_host,
	  is_repl ? "resync" : "rfwd", QFID_HOST (lt->lt_w_id), (int32) lt->lt_w_id);
...
log_replay_time (caddr_t * header)
{
  if (DV_ARRAY_OF_POINTER == DV_TYPE_OF (header[LOGH_USER]))
      ...
      if (BOX_ELEMENTS (dta) == 2 && DV_DATETIME == DV_TYPE_OF (dta[0]))
	{
	  ...
	  if (QFID_HOST (w_id) == local_cll.cll_this_host && (!log_last_local_w_id || W_ID_GT ((int32) w_id, log_last_local_w_id)))
...
{
  volatile OFF_T pos = file_in->dks_bytes_received - (file_in->dks_in_fill - file_in->dks_in_read);
  int fd = tcpses_get_fd (file_in->dks_session);
  FILE *f;
  int c = 0;

  log_error ("Searching for the next valid header signature starting at " OFF_T_PRINTF_FMT, (OFF_T_PRINTF_DTP) pos);
  ...
  for (;;)
    {
      ...
      B_LOG_CK (0);
      pos -= 5;
      log_error ("Valid looking header found at offset " OFF_T_PRINTF_FMT ".", (OFF_T_PRINTF_DTP) pos);
      break;
    }
...
log_check_trx (int64 trx_no)
{
  ...
  while (DKSESSTAT_ISSET (file_in, SST_OK))
      ...
      if (!log_check_header (header))
	{
      ...
      CATCH_READ_FAIL (file_in) session_buffered_read (file_in, trx_string, bytes);
      FAILED
      {
  ...
  while (DKSESSTAT_ISSET (file_in, SST_OK))
...
#endif
      header = (caddr_t *) read_object (file_in);
      if (!DKSESSTAT_ISSET (file_in, SST_OK) || dbf_stop_rfwd)
	{
	  break;
	}
      if (!log_check_header (header))
	{
	  log_error ("Invalid log entry in replay. Delete transaction log %.500s or truncate at point of error."
	      ...
	      wi_inst.wi_master->dbs_log_name ? wi_inst.wi_master->dbs_log_name : "", (OFF_T_PRINTF_DTP) log_rec_start);
#if LOG_DEBUG_LEVEL >1
	...
	log_error ("Log reading error in replay. The log replay may be incomplete."
	    " Error at offset " OFF_T_PRINTF_FMT, (OFF_T_PRINTF_DTP) log_rec_start);
	END_READ_FAIL (file_in);
	break;
      }
      END_READ_FAIL (file_in);
	  ...
	  if (LTE_OK != rc)
	    {
		  ...
		  rc, (OFF_T_PRINTF_DTP) log_rec_start, (OFF_T_PRINTF_DTP) file_in->dks_bytes_received);
	    }
	  if (dbf_rq_key)
	    log_skip_blobs_1 (file_in);
	}
...
log_checkpoint (dbe_storage_t * dbs, char *new_log, int shutdown)
{
  ...
  DO_SET (lock_trx_t *, lt, &all_trxs)
  {
	...
	if (LTE_OK != rc)
	  log_error
	      ("Checkpoint interrupted 2pc transaction between prepare and final.  The log of the prepared state could not be written to the post-checkpoint transaction log.  This means that, if the transaction commits and ought to be replayed from log, the transaction will be lost. trx no %d:%d",
	      QFID_HOST (lt->lt_w_id), (uint32) lt->lt_w_id);
      }
