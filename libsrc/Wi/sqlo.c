...
so_name_to_table (sqlo_t * so, caddr_t name)
{
  ...
  if (tb)
    return tb;
  tb = sch_name_to_table (wi_inst.wi_schema, name);
...
ot_is_defd (op_table_t * ot, ST * col_ref)
{
  ...
  if (ot->ot_dt)
      ...
      if (ST_P (left, SELECT_STMT))
	{
	  ...
	  {
	    if (ST_P (as, BOP_AS) && 0 == CASEMODESTRCMP (as->_.as_exp.name, col_ref->_.col_ref.name))
	      {
  ...
  if (col && col != (dbe_column_t *) CI_ROW && col != (dbe_column_t *) CI_ANY && col != (dbe_column_t *) CI_INDEX)
    {
...
sco_is_defd (sql_scope_t * sco, ST * col_ref, int mode, int generate)
{
  ...
  {
    if (SCO_UNQUALIFIED == mode && NULL != (col = ot_is_defd (ot, col_ref)))
      {
	if (!def_ot)
	  {
	    n_found++;
    ...
    if (SCO_THIS_QUAL == mode && sqlc_pref_match (sqlo_ot_effective_prefix (ot), col_ref->_.col_ref.prefix))
      {
	...
	if (generate)
	  sqlc_error (sco->sco_so->so_sc->sc_cc, "S0022", "No column %s.%s", col_ref->_.col_ref.prefix, col_ref->_.col_ref.name);
	else
	  return NULL;
      }
    if (SCO_ANY_QUAL == mode && NULL != (col = ot_is_defd (ot, col_ref)))
      {
	if (!def_ot)
	  {
	    def_ot = ot;
	  ...
	  {
	    if (generate)
	      sqlc_error (sco->sco_so->so_sc->sc_cc, "S0022", "Ambiguous col ref %s", col_ref->_.col_ref.name);
	    else
	      return NULL;
	  }
      }
...
next:;
  ...
  if (!n_found && SCO_THIS_QUAL == mode && strchr (col_ref->_.col_ref.prefix, '.'))
    {
      ...
      DO_SET (op_table_t *, ot, &sco->sco_tables)
      {
	if (!ot->ot_prefix && ot->ot_table == prefix_table && ot_is_defd (ot, col_ref))
	  {
	    def_ot = ot;
	    n_found++;
	  }
  ...
  if (n_found > 1)
	...
	sqlc_new_error (sco->sco_so->so_sc->sc_cc, "42S22", "SQ065", "Col ref ambiguous %s.%s.",
	    col_ref->_.col_ref.prefix ? col_ref->_.col_ref.prefix : "", col_ref->_.col_ref.name);
      else
	return NULL;
    }
  if (def_ot)
    {
      ...
      if (def_ot->ot_super && def_ot->ot_super->ot_inside_clause && def_ot->ot_table && def_ot->ot_table->tb_has_cgs)
	{
	  sco->sco_so->so_def_col = tb_name_to_column (def_ot->ot_table, col_ref->_.col_ref.name);
	  sco->sco_so->so_def_ot = def_ot;
	  if (OT_IN_WHERE == def_ot->ot_super->ot_inside_clause)
	    t_set_pushnew (&def_ot->ot_cg_cond_cols, (void *) sco->sco_so->so_def_col);
	}
    }
ok:
	      ...
	      while (sco)
		{
		  DO_SET (op_table_t *, ot, &sco->sco_tables)
		  {
		    if (P_EXACT == sqlc_pref_match (sqlo_ot_effective_prefix (ot), col_ref->_.col_ref.name))
			  ...
			  "It matches both a variable of user defined type and a table correlation name.", col_ref->_.col_ref.name);
		  }
		  END_DO_SET ();
		  sco = sco->sco_super;
		}
...
void
sqlo_natural_join_cond (sqlo_t * so, op_table_t * left_ot, op_table_t * right_ot, ST * tree)
{
  int inx;
  ST *ctree = NULL;
  ST *term;
  if (tree->_.join.is_natural || ST_P (tree->_.join.cond, JC_USING))
    {
      if (!tree->_.join.cond || tree->_.join.cond == (ST *) STAR)
	{
	  ...
	  DO_SET (dbe_column_t *, col, &left_ot->ot_table->tb_primary_key->key_parts)
	  {
	    ...
	    if (!IS_BLOB_DTP (col->col_sqt.sqt_dtp) && col2 && !IS_BLOB_DTP (col2->col_sqt.sqt_dtp))
	      {
	...
	sqlc_new_error (so->so_sc->sc_cc, "37000", "SQ067", "Explicit join condition not allowed in natural join");
	...
	sqlc_new_error (so->so_sc->sc_cc, "37000", "SQ068", "Empty or USING join condition not allowed with non-natural join");
    }
}
...
sqlo_wrap_join_dt (sqlo_t * so, ST * tree)
{
  ...
  selection = sqlp_stars ((ST **) t_list (1, t_list (3, COL_DOTTED, NULL, STAR)), from);
...
sqlo_rls_add_condition (sqlo_t * so, op_table_t * ot, dk_set_t * res, dbe_table_t * tb)
{
  ...
  if (tb->tb_rls_procs[TB_RLS_S] && cli->cli_user && !sec_user_has_group (G_ID_DBA, cli->cli_user->usr_g_id))
    {
      caddr_t err = NULL;
      ...
      static char *select_mask = "select 1 from \"%s\".\"%s\".\"%s\" where %s";
	...
	goto done;
      ...
      snprintf (sql_text, box_length (sql_text) - 1, select_mask, tb->tb_qualifier, tb->tb_owner, tb->tb_name_only, ret_val);
      dk_free_box (ret_val);
      ret_val = NULL;
	...
	goto done;
      ...
      t_set_push (res, t_list (6, JOINED_TABLE, NULL, NULL, NULL, NULL, t_tree->_.select_stmt.table_exp->_.table_exp.where));
    done:
      if (tree)
	dk_free_tree ((box_t) tree);
      if (sql_text)
...
{
  int inx;
  DO_SET (ST **, dts, &so->so_sc->sc_with_list)
  {
    DO_BOX (ST *, dt, inx, dts)
    {
      if (!CASEMODESTRCMP (dt->_.with_dt.name, tree->_.table.name))
	return dt->_.with_dt.dt;
    }
    END_DO_BOX;
  }
  END_DO_SET ();
...
sqlo_add_table_ref (sqlo_t * so, ST ** tree_ret, dk_set_t * res)
{
  ...
  switch (tree->type)
	...
	if (!view || inside_view)
	  {
	    ...
	    ot->ot_inside_outer = so->so_inside_outer;
	...
	else
	  {
		...
		ST_P (view, EXCEPT_ST) || ST_P (view, EXCEPT_ALL_ST) || ST_P (view, INTERSECT_ST) || ST_P (view, INTERSECT_ALL_ST))
	      {
		...
		ot = (op_table_t *) so->so_tables->data;
		ot->ot_prefix =
		    tree->_.table.prefix ? tree->_.table.prefix : tb ? tb->tb_name : with_view ? tree->_.table.
		    name : t_box_string ("none");
		sco_add_table (so->so_scope, ot);
	      }
	    else
	      {
	...
	int is_jtc, is_natural = (tree->_.join.is_natural && (!tree->_.join.cond || tree->_.join.cond == (ST *) STAR));
	...
	old_outer = so->so_inside_outer;
	so->so_inside_outer = OJ_LEFT == tree->_.join.type;
	sqlo_add_table_ref (so, &tree->_.join.right, sco->sco_has_jt ? &res_jt : res);
	so->so_inside_outer = old_outer;
	...
	if (tree->_.join.type == OJ_LEFT || tree->_.join.type == OJ_FULL)
	  right_ot->ot_is_outer = 1;
	    ...
	    !ST_P (left_ot->ot_dt, PROC_TABLE) && !ST_P (right_ot->ot_dt, PROC_TABLE))
	  {
	    t_set_push (res, tree);
	    break;
	  }
...
void
sqlo_replace_col_refs_prefixes (sqlo_t * so, ST * tree, caddr_t old_prefix, ST ** selection, int remove_as_decls)
{
  ...
  if (dtp == DV_ARRAY_OF_POINTER)
	...
	if (ST_COLUMN (elt, COL_DOTTED))
	  {
	    if (elt->_.col_ref.prefix && !CASEMODESTRCMP (elt->_.col_ref.prefix, old_prefix))
	      {
		int inx2;
		ST *real_col = NULL;
...
sqlo_replace_fun_refs_prefixes (ST * tree, caddr_t old_prefix, caddr_t new_prefix)
{
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (tree))
    return;
  if (ST_P (tree, FUN_REF) && tree->_.fn_ref.fn_name && !CASEMODESTRCMP (tree->_.fn_ref.fn_name, old_prefix))
    {
...
sqlo_join_exp_inlineable (ST * exp)
{
  ...
  if (ST_P (exp, JOINED_TABLE) && OJ_FULL == exp->_.join.type)
    return 0;
  if (ST_P (exp, UNION_ST) || ST_P (exp, UNION_ALL_ST)
      || ST_P (exp, INTERSECT_ST) || ST_P (exp, INTERSECT_ALL_ST) || ST_P (exp, EXCEPT_ST) || ST_P (exp, EXCEPT_ALL_ST))
    return 0;
...
sqlo_dt_inlineable (sqlo_t * so, ST * tree, ST * from, op_table_t * ot, int single_only)
{
  ...
  if (ST_P (dtexp, SELECT_STMT) &&
      ...
      !dt_orig->_.table_exp.having && !dt_orig->_.table_exp.order_by && !sqlp_is_union_wrap (dtexp))
    {
      int dt_inx;
	...
	if (single_only && ST_P (dt_from, JOINED_TABLE))
	  return 0;
	if (ST_P (dt_from, JOINED_TABLE) && !sqlo_join_exp_inlineable (dt_from))
	  return 0;
      }
      END_DO_BOX;
      if (sqlo_print_debug_output)
	{
	  sqlo_print (("Expanding %s(%s) [%s]: \n",
		  dot->ot_prefix, dot->ot_new_prefix, dot->ot_table ? dot->ot_table->tb_name : "(NONE)"));
	}
...
sqlo_expand_dt (sqlo_t * so, ST * tree, ST ** from_ret, op_table_t * ot, int is_in_jt, dk_set_t * new_froms)
{
  ...
  sqlo_replace_col_refs_prefixes (so, tree->_.select_stmt.table_exp, dot->ot_new_prefix, (ST **) dtexp->_.select_stmt.selection, 1);
  ...
  if (dtexp->_.select_stmt.table_exp->_.table_exp.where)
	...
	&texp->_.table_exp.having : &texp->_.table_exp.where, dtexp->_.select_stmt.table_exp->_.table_exp.where);
...
sqlo_inline_jt (sqlo_t * so, ST * tree, ST * exp, op_table_t * ot)
{
  ...
  if (ST_P (exp, JOINED_TABLE))
	...
	any += sqlo_inline_jt (so, tree, exp->_.join.left->_.table_ref.table, ot);
      if (OJ_LEFT == exp->_.join.type && sqlo_dt_inlineable (so, tree, exp->_.join.right, ot, 1))
	{
  ...
  for (inx = 2; inx < argcount; inx++)
      ...
      if (!DV_STRINGP (arg))
	{
	  if (inx >= surely_option_idx)
	    sqlc_error (sc->sc_cc, "37000", "Argument %d of %s should be a keyword, i.e. a symbol", inx + 1,
		sqlo_spec_predicate_name (type));
	  continue;
	}
      if (0 == stricmp ((char *) arg, "OFFBAND"))
	{
	  ...
	  DO_BOX (dbe_column_t *, ocol, oinx, ocols)
	  {
		...
		sc_crr = sqlo_virtual_col_crr (so, ot, ocol->col_name, ocol->col_sqt.sqt_dtp, 1);
	  ...
	  END_DO_BOX;
	  sqlc_error (sc->sc_cc, "37000", "No offband column '%.300s' in %.300s", args[inx + 1]->_.col_ref.name,
	      ot->ot_table->tb_name);
	offb_done:
	  inx++;
	  continue;
...
sqlo_implied_columns_of_contains (sqlo_t * so, ST * tree, int add_score)
{
  ...
  if (BOX_ELEMENTS (tree) > 1 && NULL != (args = sqlc_contains_args (tree, &ctype)))
	...
	sqlc_error (so->so_sc->sc_cc, "37000", "The first argument of %s must be a column", sqlo_spec_predicate_name (ctype));

      ot = sco_is_defd (so->so_scope, args[0], args[0]->_.col_ref.prefix ? SCO_THIS_QUAL : SCO_UNQUALIFIED, 1);
      if (!ot || !ot->ot_table)
	...
	case 'q':
	  ...
	  if (BOX_ELEMENTS (args) > 3)
	    sqlc_error (so->so_sc->sc_cc, "37000", "Too many arguments passed to %s", sqlo_spec_predicate_name (ctype));
	  break;
	default:
	  GPF_T;
	}
	...
	if ((*tree)->_.col_ref.prefix &&
	    ...
	    !CASEMODESTRCMP (sel->_.col_ref.name, (*tree)->_.col_ref.name))
	  {
		...
		*tree = t_listst (5, BOP_AS,
		    t_list (3, COL_DOTTED, new_pref, as_exp->_.as_exp.name), NULL, sel->_.col_ref.name, NULL);
	      }
	    else
	      {
...
sqlo_jt_dt_wrap (sqlo_t * so, ST ** jptr, ST * select_stmt, int was_top, int replace_col_refs)
{
  ...
  if (ST_P (*jptr, JOINED_TABLE))
	...
	sqlo_box_print (jtm_prefix);
      ...
      texp = t_listst (9, TABLE_EXP, t_list (1, *jptr), NULL, NULL, NULL, NULL, NULL, NULL, NULL);
      ...
      tb = t_listst (3, DERIVED_TABLE, t_listst (5, SELECT_STMT, NULL, sel, NULL, sqlp_infoschema_redirect (texp)), jtm_prefix);
		...
		if (ST_COLUMN (sel_exp, COL_DOTTED))
		  {
			...
			if (!IS_FOR_XML (select_stmt))
			  {
				...
				t_listst (5, BOP_AS, sel_exp, NULL, sel[col_inx - 1]->_.as_exp.left->_.col_ref.name, NULL);
			  }
			else
			  {
			...
			((ST **) select_stmt->_.select_stmt.selection)[inx] =
			    t_listst (5, BOP_AS, sel_exp, t_box_string (orig_name), sel_exp->_.col_ref.name, NULL);
		      }
		  }
			...
			if (!strcmp (jtm_prefix, as->_.col_ref.prefix))
			  {
			    ...
			    if (!col_inx || col_inx > BOX_ELEMENTS_INT (sel))
			      SQL_GPF_T1 (so->so_sc->sc_cc, "an unknown column detected in jt");
			    ot = sqlo_cname_ot (so, sel[col_inx - 1]->_.as_exp.left->_.col_ref.prefix);
  ...
  if (ST_P (tree, SELECT_STMT))
    {
      ...
      ST_P (tree->_.table_ref.table, JOINED_TABLE) && tree->_.table_ref.table->_.join.type == OJ_FULL && !tree->_.table_ref.range)
    {				/* need to add an alias for the table as it'll be changed to select */
      ...
      sqlo_jt_replace_col_refs (&select_stmt, jtm_prefix, (ST **) left->_.select_stmt.selection, tree, 0);
      if (was_top)
	{
	  int inx;
	  ...
	  DO_BOX (ST *, sel_exp, inx, select_stmt->_.select_stmt.selection)
	  {
	    if (ST_COLUMN (sel_exp, COL_DOTTED) && !CASEMODESTRCMP (sel_exp->_.col_ref.prefix, jtm_prefix))
	      {
		int col_inx = atoi (sel_exp->_.col_ref.name + 5);
		if (!col_inx || col_inx > BOX_ELEMENTS_INT (sel))
      ...
      if (tree->_.join.type == OJ_FULL)
	{
	  ...
	  *ptree = t_listst (5, UNION_ST, left_oj_tree->_.table_ref.table, right_oj_tree->_.table_ref.table, NULL, 0);
	...
	if (ot && !sqlo_join_reffed_outside (tree, (*tbp)->_.join.cond, ot->ot_new_prefix))
	  {
		...
		sqlo_print (("Dropping the joined table %s(%s) [%s]\n",
			ot->ot_prefix, ot->ot_new_prefix, ot->ot_table ? ot->ot_table->tb_name : "(NONE)"));
	      }
	    if (ot->ot_from_ots)
	      {
...
sqlo_expand_group_by (caddr_t * selection, ST *** p_group_by, ptrlong * p_is_distinct)
{
  ...
  if (!selection || !group_by || BOX_ELEMENTS (selection) != BOX_ELEMENTS (group_by) || *p_is_distinct)
    return;
...
st_equal_no_serial (box_t st1, box_t st2)
{
  ...
  if (dtp1 == dtp2 && dtp1 == DV_ARRAY_OF_POINTER && BIN_EXP_P ((ST *) st1) && ((ST *) st1)->type == ((ST *) st2)->type)
    {
  ...
  if (dk_set_length (or_list) > 1)
	...
	DO_SET (dk_set_t, and_list, &and_lists)
	{
	  ...
	  DO_SET_WRITABLE (ST *, and_elt, iter, &and_list)
	  {
		...
		first_and_elt->type != and_elt->type && first_and_elt->type == cmp_op_inverse (and_elt->type))
	      {
	...
	if (!strcmp (tree->_.col_ref.prefix, ot->ot_new_prefix) && !is_not_one_gb)
	  {
		...
		" in either an aggregate function or the GROUP BY clause.", tree->_.col_ref.name);
#endif
	    new_group =
		(ST **) t_alloc_box (((group[0] ? BOX_ELEMENTS (group[0]) : 0) + 1) * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
...
{
  return (ST_COLUMN (tree, COL_DOTTED) && tree->_.col_ref.prefix && !strcmp (tree->_.col_ref.prefix, pref)
      && !strcmp (tree->_.col_ref.name, col));
}
...
sqlo_cols_eq (dk_set_t top_and, caddr_t col, caddr_t pref1, caddr_t pref2)
{
  ...
  END_DO_SET ();
  if (p1 && p2 && box_equal ((cbox_t) val1, (cbox_t) val2))
    return 1;
  return 0;
}
...
sqlo_identity_self_join (sqlo_t * so, ST ** ptree)
{
  ...
  DO_BOX (ST *, t1, inx, texp->_.table_exp.from)
  {
	...
	t2 = tref_table (t2);
	if (ST_P (t2, TABLE_DOTTED) && 0 == strcmp (t1->_.table.name, t2->_.table.name))
	  {
...
sqlo_distinct_redundant (ST * sel, ST * gb)
{
  ...
  DO_BOX (ST *, spec, inx, (ST **) gb)
  {
	...
	exp = exp->_.call.params[0];
      if (box_equal ((cbox_t) exp, (cbox_t) spec))
	{
	  found = 1;
	  break;
	}
...
col_ref_ot (ST * exp)
{
  return (op_table_t *) unbox ((caddr_t) exp->_.col_ref.ot);
}

void
sqlo_cg_mark_gby_oby (sqlo_t * so, op_table_t * ot, ST * texp)
{
  /* if col groups present, mark the oby and gby cols in the ot for sorted proj choices */
  int inx, inx2;
  DO_BOX (ST *, gby, inx, texp->_.table_exp.group_by_full)
  {
    DO_BOX (ST *, exp, inx2, gby)
    {
      if (ST_P (exp, ORDER_BY))
	exp = exp->_.o_spec.col;
      if (ST_P (exp, COL_DOTTED))
	{
	  int len = BOX_ELEMENTS (exp);
	  if (len >= 5)
	    {
	      t_set_pushnew (&col_ref_ot (exp)->ot_cg_group_cols, unbox ((caddr_t) exp->_.col_ref.col));
	    }
	}
    }
    END_DO_BOX;
  ...
  END_DO_BOX;
  DO_BOX (ST *, exp, inx2, texp->_.table_exp.order_by)
  {
    if (ST_P (exp, ORDER_BY))
      exp = exp->_.o_spec.col;
    if (ST_P (exp, COL_DOTTED))
	...
	if (len >= 5)
	  {
	    t_set_pushnew (&col_ref_ot (exp)->ot_cg_order_cols, unbox ((caddr_t) exp->_.col_ref.col));
	  }
      }
  }
  END_DO_BOX;
...
sqlo_select_scope (sqlo_t * so, ST ** ptree)
{
  ...
  if (texp)
    {
      dk_set_t save_const = so->so_const_subqs;
	...
	if (ST_P (as, BOP_AS))
	  {
	    ...
	    t_set_push (&sco->sco_named_vars, t_list (2,
		    t_list (3, COL_DOTTED, NULL, t_box_string (as->_.as_exp.name)), t_box_copy_tree ((caddr_t) as_value)));
	  }
      }
      END_DO_BOX;
      ...
      if (so->so_this_dt->ot_fun_refs || texp->_.table_exp.group_by)
	{
      ...
      sqlo_cg_mark_gby_oby (so, ot, texp);
    }
  else
    sqlo_scope_array (so, (ST **) tree->_.select_stmt.selection);
  ...
  if (texp && texp->_.table_exp.group_by)
    {
      ...
      sqlo_oby_exp_cols (so, tree, texp->_.table_exp.group_by);
      if (!ot->ot_fun_refs)
	sqlo_expand_group_by (tree->_.select_stmt.selection, &tree->_.select_stmt.table_exp->_.table_exp.group_by, &is_distinct);
      ...
      if (texp->_.table_exp.group_by)
	{
	  ...
	  sqlo_check_group_by_cols (so, (ST *) tree->_.select_stmt.selection, &(texp->_.table_exp.group_by), ot, is_not_one_gb);
	  sqlo_check_group_by_cols (so, (ST *) texp->_.table_exp.order_by, &(texp->_.table_exp.group_by), ot, is_not_one_gb);
	  if (!is_not_one_gb && SEL_IS_DISTINCT (tree)
	      && sqlo_distinct_redundant ((ST *) tree->_.select_stmt.selection, (ST *) texp->_.table_exp.group_by))
      ...
      void sqlo_exists_record_org (ST * tree, ST * cond)
      {
	...
	if (BOP_EQ == cond->type)
      ...
      void sqlo_subq_convert_to_exists (sqlo_t * so, ST ** tree_ret)
      {
	ST *tree = *tree_ret;
	  ...
	  case IN_SUBQ_PRED:
	    {
	      ...
	      if (SEL_TOP (select))
		sqlc_new_error (so->so_sc->sc_cc, "37000", "SQ148", "TOP not supported in IN, ANY, SOME, ALL, ONE subqueries");
	      if (texp)
		{
			  ...
			  if (ST_P (tree->_.subq.left, COMMA_EXP))
			    {
			      ...
			      if (BOX_ELEMENTS (comma->_.comma_exp.exps) != BOX_ELEMENTS (select->_.select_stmt.selection))
				sqlc_new_error (so->so_sc->sc_cc, "37000", "SQ139",
			      ...
			      {
				t_st_and (&cond,
				    t_listst (3, tree->_.subq.cmp_op, exp, (ST *) select->_.select_stmt.selection[comma_inx]));
			      }
			      END_DO_BOX;
			    }
			  else
			      ...
			      if (so->so_bin_op_is_negate)
				{
				  BIN_OP (cond, BOP_OR,
				      ...
				      t_listst (3, BOP_NULL, select->_.select_stmt.selection[0], NULL));
				}
			      else
				{
				  BIN_OP (cond, tree->_.subq.cmp_op, tree->_.subq.left, (ST *) select->_.select_stmt.selection[0]);
				  sqlo_exists_record_org (tree, cond);
				}
			    }

			  if ((to_negate && !op_negate) || (!to_negate && op_negate))
	...
	return (SINV_DV_STRINGP (name) && (!stricmp (name, "__ID2I") || !stricmp (name, "__ID2In") || !stricmp (name, "__RO2SQ")));
      ...
      int sqlo_exp_nullable (sqlo_t * so, ST * tree)
      {
	int inx;
	dtp_t dtp = DV_TYPE_OF (tree);
	if (DV_DB_NULL == dtp)
	...
	if (ST_P (tree, COL_DOTTED))
	  {
	    ...
	    if (ot->ot_dt)
	      {
		ST *dt = ot->ot_dt;
		ST *left = sqlp_union_tree_select (dt);
		if (dt == left && ST_P (left, SELECT_STMT))
		  {
		    DO_BOX (ST *, as, inx, left->_.select_stmt.selection)
		    {
		      if (0 == CASEMODESTRCMP (as->_.as_exp.name, tree->_.col_ref.name))
			{
			  return sqlo_exp_nullable (so, as->_.as_exp.left);
			}
		    }
		    END_DO_BOX;
		  }
	    ...
	    if (ot->ot_table)
	      {
		dbe_column_t *col = tb_name_to_column (ot->ot_table, tree->_.col_ref.name);
		return col ? !col->col_sqt.sqt_non_null : 1;
	      }
	    else
	      return 1;
	  }
	...
	return 0;
      ...
      void sqlo_count_unq_preserving (sqlo_t * so, ST * tree)
      {
	while ST_P
	  (tree->_.fn_ref.fn_arg, CALL_STMT && sqlo_is_unq_preserving (tree->_.fn_ref.fn_arg->_.call.name)
	      && 1 <= BOX_ELEMENTS (tree->_.fn_ref.fn_arg->_.call.params))
	      tree->_.fn_ref.fn_arg = tree->_.fn_ref.fn_arg->_.call.params[0];
      }


      void sqlo_scalar_subq_scope (sqlo_t * so, ST ** ptree)
      {
	ST *tree = *ptree;
	dk_set_t s;
	sql_scope_t *sco = so->so_scope;
	...
	for (s = sco->sco_scalar_subqs; s; s = s->next->next)
	  {
	    org = (ST *) s->data;
	    if (box_equal ((cbox_t) org, (cbox_t) tree))
	      {
...
#define ST_ABSENT ((ST*)-1)
	...
	if (2 == enable_rdf_box_const)
	  {
	    *ptree = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase ("__rdflit"), t_list (1, data));
	    sqlo_lit_param (so, &(*ptree)->_.call.params[0]);
	  }
	else
	  {
	    *ptree = data;
	...
	*ptree = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase ("__rdflit"), t_listst (1, copy));
	...
	switch (call_name[0])
      ...
      ST *sqlo_func_opt (sqlo_t * so, ST * tree, caddr_t call_name)
      {
	if (st_is_call (tree, "substr", 3) || st_is_call (tree, "substring", 3))
	  {
	    /* if a substring of a column covers the whole column, take out the substring call */
	    ST *col_ref = tree->_.call.params[0];
	    caddr_t lower = (caddr_t) tree->_.call.params[1];
	    caddr_t upper = (caddr_t) tree->_.call.params[2];
	    if (1 == unbox (lower) && DV_LONG_INT == DV_TYPE_OF (upper) && ST_P (col_ref, COL_DOTTED))
	      {
		op_table_t *ot = sqlo_cname_ot_1 (so, col_ref->_.col_ref.prefix, 0);
		if (ot && ot->ot_table)
		  {
		    dbe_column_t *col = tb_name_to_column (ot->ot_table, col_ref->_.col_ref.name);
		    if (col && col->col_sqt.sqt_precision && col->col_sqt.sqt_precision <= unbox (upper))
		      return col_ref;
		  }
	      }
	  }
	...
	if (SYMBOLP (tree))
	  {
	    if (':' == ((char *) tree)[0] && so->so_stl)
	      so->so_stl->stl_explicit_params = 1;	/* if there are explicit params, lits do not become params */
	    return;
	  }
	if (ST_P (tree, QUOTE))
	  return;
	...
	switch (tree->type)
	  ...
	  case COL_DOTTED:
	    {
	      ...
	      if (so->so_def_col)
		*ptree =
		    t_listst (5, COL_DOTTED, tree->_.col_ref.prefix, tree->_.col_ref.name, box_num ((ptrlong) so->so_def_col),
		    box_num ((ptrlong) so->so_def_ot));
	      break;
	    }
	  case FUN_REF:
	    {
	      ...
	      if (AMMSC_AVG == tree->_.fn_ref.fn_code)
		{
		  ...
		  ST *new_tree, *left, *right;
		  FN_REF (left, AMMSC_SUM, ad, arg);
		  FN_REF (right, AMMSC_COUNT, ad, arg_copy);
		  BIN_OP (new_tree, BOP_DIV, left, right);
		  *ptree = new_tree;
		  tree = new_tree;
	      ...
	      if (AMMSC_COUNT == tree->_.fn_ref.fn_code || AMMSC_COUNTSUM == tree->_.fn_ref.fn_code)
		sqlo_count_unq_preserving (so, tree);
	      if (AMMSC_COUNT == tree->_.fn_ref.fn_code
		  && !tree->_.fn_ref.all_distinct && !sqlo_has_col_ref (tree->_.fn_ref.fn_arg))
		{
	      ...
	      else
		{
	      ...
	      t_NCONCF1 (so->so_this_dt->ot_fun_refs, (void *) tree);
	      if (so->so_scope)
		so->so_scope->sco_fun_refs_allowed = old_fun_refs_allowed;
	      break;
	    }
	  case ANALYTICAL_EXP:
	    sqlo_scope (so, &tree->_.anf.call);
	    if (tree->_.anf.partition)
	      sqlo_scope_array (so, tree->_.anf.partition);
	    if (tree->_.anf.order)
	      sqlo_scope_array (so, tree->_.anf.order);
	    break;
	  case SCALAR_SUBQ:
	  ...
	  case CALL_STMT:
		...
		sqlo_scope (so, &(tree->_.call.params[inx]));
	      ...
	      END_DO_BOX;
	      so->so_stl = save;
	      res = sqlo_func_opt (so, tree, call_name);
	      res = sinv_check_inverses (res, sqlc_client ());
	      if (call_name == uname_one_of_these)
		res = sqlo_iri_in_opt (so, res);
	      if (res != tree)
		{
		  *ptree = res;
		  tree = res;
	  ...
	  case WITH_STMT:
	    {
	      sql_comp_t *sc = so->so_sc;
	      dk_set_t save = sc->sc_with_list;
	      t_set_push (&sc->sc_with_list, tree->_.with_stmt.dts);
	      *ptree = tree->_.with_stmt.body;
	      sqlo_select_scope (so, ptree);
	      sc->sc_with_list = save;
	      break;
	    }
	  case SELECT_STMT:
	    sqlo_select_scope (so, ptree);
		...
		if (res != tree)
		  {
		    *ptree = res;
		    tree = res;
		    sqlo_check_rdf_lit (so, &tree->_.bin_exp.left);
		    sqlo_check_rdf_lit (so, &tree->_.bin_exp.right);
		  }
		if (ST_P (tree, BOP_OR))
      ...
      int sqlo_refd_col_cb (ST * tree, void *cd)
      {
	if (ST_P (tree, TABLE_REF))
	  return 1;
	if (ST_P (tree, COL_DOTTED))
	  {
	    t_id_hash_set ((id_hash_t *) cd, (caddr_t) & tree, NULL);
	    return 1;
	  }
	return 0;
      }
	...
	END_DO_BOX;
      ...
      {
	/* intermediate, for removing unrefd cols of derived tables */
	id_hash_t *ht = t_id_hash_allocate (31, sizeof (caddr_t), 0, treehash, treehashcmp);
	ST *texp = tree->_.select_stmt.table_exp;
	sqlo_map_st ((ST *) tree->_.select_stmt.selection, sqlo_refd_col_cb, ht);
	if (texp)
	  {
	...
	DO_BOX (ST *, exp, inx, ((caddr_t *) tree))
	{
	  sqlo_replace_tree (&((ST **) tree)[inx], old_tree, new_tree);
	}
	END_DO_BOX;
      }


      uint32 sqlo_subq_hash_cb (ST * tree, void *cd)
      {
	uint32 *hp = (uint32 *) cd;
	if (ST_P (tree, TABLE_DOTTED))
	  {
	    *hp = ((*hp) << 1) ^ box_hash (tree->_.table.name);
	    if (1 == *hp)
	      *hp = 2;
	    return 1;
	  }
	return 0;
      ...
      {
	uint32 hno = 1;
	sqlo_map_st (tree, sqlo_subq_hash_cb, &hno);
	return hno;
      }
