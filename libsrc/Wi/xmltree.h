...
struct xml_lazy_doc_s
{
  XD_MEMBERS union
  {
    struct
    {
    XTD_MEMBERS} xtd_stub;
    struct
    {
    XPER_MEMBERS} xper_stub;
  } stub;
...
{
  XE_MEMBERS xte_bmk_t * xte_stack_buf;	/*!< Buffer for stack of ancestors of current subtree (and for current subtree) */
...
{
  XE_MEMBERS char xper_type;	/*!< type of this entity, one of XML_MKUP_XXX constants */
...
typedef struct xslt_sheet_s
{
  ...
  caddr_t xsh_sparql_preamble;	/*!< A string that is inserted at the front of any SPARQL query that is executed by for-each-row */
   XOUT_MEMBERS xslt_sheet_stats_t xsh_total_uses;
...
extern xp_query_t *xp_query_parse (query_instance_t * qi, char *str, ptrlong predicate_type, caddr_t * err_ret,
    xp_query_env_t * xqre);
...
extern xslt_sheet_t *xslt_sheet (query_instance_t * qi, caddr_t base, caddr_t ref, caddr_t * err_ret, shuric_t * loaded_by);
