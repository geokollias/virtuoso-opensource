...
typedef void jso_validation_cbk_t (void * /* actually jso_rtti_t * */ inst_rtti, dk_set_t * errors_log_ptr,
...
extern int jso_get_pinned_cd_and_rtti (ccaddr_t jclass, ccaddr_t jinstance, jso_class_descr_t ** cd_ptr,
    jso_rtti_t ** inst_rtti_ptr);

/*! The function searches for an draft instance such that { ?jinstance rdf:type ?jclass }, does not signal an error if \c quiet */
extern int jso_get_draft_cd_and_rtti (ccaddr_t jclass, ccaddr_t jinstance, jso_class_descr_t ** cd_ptr, jso_rtti_t ** inst_rtti_ptr,
    int quiet);
