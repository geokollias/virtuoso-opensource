...
#ifdef MALLOC_DEBUG
  tree =
      (XT *) dbg_dk_alloc_box (xtlist_impl_file, xtlist_impl_line, sizeof (caddr_t) * (1 + length + tail_len), DV_ARRAY_OF_POINTER);
#else
...
*/
  ...
  if (!xp_env ()->xe_for_interp && step->_.step.preds)
    {
      int inx;
    ...
    char **err_ret, caddr_t * q_ret, caddr_t * o_ret, caddr_t * n_ret)
{
  ...
  if (q && (strlen (q) > MAX_NAME_LEN))
    {
      err_ret[0] = "Database qualifier is too long";
      return NULL;
    }
  if (o && (strlen (o) > MAX_NAME_LEN))
    {
      err_ret[0] = "Owner name is too long";
      return NULL;
    }
  if (o && strchr (o, '.'))
    {
      err_ret[0] = "Invalid owner name (it contains dot char)";
      return NULL;
    }
  if (n && (strlen (n) > MAX_NAME_LEN))
    {
      err_ret[0] = "Name is too long";
      return NULL;
    }
  if (n && strchr (n, '.'))
    {
      err_ret[0] = "Invalid local part of the name (it contains dot char)";
      return NULL;
    }
  if (NULL == o && NULL == n)
    {
      ...
      if ((NULL == q) || ('\0' == q[0]))
	{
	  err_ret[0] = "Invalid name (empty string)";
	  return NULL;
	}
      if (NULL != part_tok (&xx))
	{
	  err_ret[0] = "Invalid name (too many dots)";
	  return NULL;
	}
      while (NULL == n)
	{
      ...
      if ('\0' == n[0])
	{
	  err_ret[0] = "Invalid name (no local part after dot)";
	  return NULL;
	}
      if ((NULL != o) && ('\0' == o[0]))
	o = NULL;
  ...
  if (NULL == newest_schema)
    newest_schema = wi_inst.wi_schema;
...
caddr_t
xp_xml_view_name (xpp_t * xpp, char *q, char *o, char *n)
{
  char *err = NULL;
...
caddr_t *
dk_set_assoc (dk_set_t list, ccaddr_t key)
{
  while (list)
    {
      if ((!key && !list->data) || (key && list->data && 0 == strcmp (key, (char *) list->data)))
	{
...
caddr_t
xp_make_extfunction_name (xpp_t * xpp, caddr_t pref, caddr_t qname)
{
      ...
      if (xp_is_join_step (axis))
	{
	  ...
	  if (((XT *) XP_NODE == node) || ((XT *) XP_ELT == node))
	  ...
	  else if (ARRAYP (node))
	    {
	      ...
	      int pred;
	      dbe_table_t *tb;
	      xp_q_name (xpp, exp_name, sizeof (exp_name), node->_.name_test.nsuri, node->_.name_test.local,
		  NULL /* not xp_env()->xe_dflt_elt_namespace */ );
	      ...
	      else
		{
		  ...
		  xc->xc_node_test = stlist (4,
		      pred, box_copy_tree ((box_t) st_col_dotted (cn, "E_NAME")), box_dv_short_string (exp_name), NULL);
		}
	    }
	  else
...
{
  return (!(xt_is_ret_boolean (exp) || xt_is_ret_node_set (exp)));
}
#endif
...
XT *
xp_make_flwr (xpp_t * xpp, dk_set_t forlets, XT * where_expn, dk_set_t ordering, XT * return_expn)
{
#ifdef XPATH_DEBUG
  ...
  if (NULL != where_expn)
    {
      ...
      return_expn = xp_make_call (xpp, box_dv_uname_string ("if"), list (3, where_expn, return_expn, false_expn));
    }
  while (NULL != forlets)
    {
	...
	case XQ_FOR:
	  {
	    ...
	    if (is_last_for_forlet)
	      {
		return_expn =
		    xp_make_call (xpp, box_dv_uname_string ("ORDER BY operator"), list (3, return_expn, ordering_array,
			xp_make_literal_tree (xpp, box_num (1), 0)));
	      }
	    break;
	  }
	case XQ_LET:
	...
	default:
	  xp_error (xpp, "internal XQuery error in compilation of FLWR expression");
	}
    }
#ifdef XPATH_DEBUG
...
XT *
xp_make_direct_el_ctor (xpp_t * xpp, XT * el_name, dk_set_t attrs, dk_set_t subel_expns)
{
...
XT *
xp_make_direct_comment_ctor (xpp_t * xpp, XT * content)
{
...
XT *
xp_make_direct_pi_ctor (xpp_t * xpp, XT * name, XT * content)
{
...
XT *
xp_make_deref (xpp_t * xpp, XT * step, XT * name_test)
{
...
XT *
xp_make_cast (xpp_t * xpp, ptrlong cast_or_treat, XT * type, XT * arg_tree)
{
...
XT *
xp_make_sortby (xpp_t * xpp, XT * arg_tree, dk_set_t criterions)
{
...
xp_join_pred (xp_ctx_t * start, xp_ctx_t * next)
{
  ...
  p = t_stlist (3, CALL_STMT, t_box_string ("ancestor_of"),
      t_list (5, st_col_dotted (cn1, "E_ID"), st_col_dotted (cn2, "E_ID"), t_box_num (next->xc_axis), NULL, NULL));
...
char *wildcard_doc_text =
    "select 1 from (select _D.E_ID, (select N.E_ID from DB.DBA.VXML_DOCUMENT N where N.E_ID > _D.E_ID) as D_NEXT "
    "  from DB.DBA.VXML_DOCUMENT _D where D_URI like ':0'and _D.D_URI >= ':1' and _D.D_URI < ':2' " "order by D_URI) __D";
...
xp_init_select (xpp_t * xpp, xp_ctx_t * xc, XT * step, xp_ret_t * xr)
{
  ...
  texp->_.table_exp.from =
      (ST **) t_list (1, t_list (3, TABLE_REF, t_list (5, TABLE_DOTTED, t_box_string (step_ctx->xc_table->tb_name),
	      t_box_string (cn), t_box_num (SC_U_ID (xp_env ()->xe_sc)), t_box_num (SC_G_ID (xp_env ()->xe_sc))), NULL));
...
{
  char exp_name[MAX_XML_QNAME_LENGTH + 1];
  xp_q_name (xpp, exp_name, sizeof (exp_name), tree->_.name_test.nsuri, tree->_.name_test.local,
      NULL /* not xp_env()->xe_dflt_elt_namespace */ );
...
/*  dk_free_tree ((caddr_t) sel->_.select_stmt.selection);*/
  ...
  if (mode == GM_TOP)
	...
	ref = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase ("http_entity"),
	    t_list (3, ref, t_box_copy (xe->xe_result_tag), t_box_num (xe->xe_is_shallow != 0 ? 4 : 0)));
      else if (xe->xe_is_sax)
	ref = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase ("DB.DBA.sax_entity"),
	    t_list (3, ref, t_box_copy (xe->xe_result_tag), t_box_num (xe->xe_is_shallow != 0 ? 4 : 0)));
      else if (!xe->xe_is_for_key)
	ref = t_listst (3, CALL_STMT, t_sqlp_box_id_upcase ("value_of"), t_list (1, ref));
  ...
  if ((dot = strchr (sym, '.')))
    {
      ...
      return (t_stlist (3, COL_DOTTED, t_box_substr (sym, 1, dotp), t_box_substr (sym, dotp + 1, (int) strlen (sym))));
    }
...
xp_sql (xpp_t * xpp, xp_ctx_t * start_ctx, XT * tree, xp_ret_t * xr, int mode)
{
  ...
  if (DV_ARRAY_OF_POINTER != DV_TYPE_OF (tree))
	...
	xr->xr_tree = (ST *) t_full_box_copy_tree ((caddr_t) tree);
      ...
      ((tree->_.step.axis == XP_ATTRIBUTE) || (tree->_.step.axis == XP_ATTRIBUTE_WR)) && !tree->_.step.input)
    {
...
/*end mapping schema*/
      ...
      case XP_FILTER:
	{
	  ...
	  if (SELECT_STMT != path.xr_tree->type)
	    xp_error (xpp,
		"Unsupported use of XPATH filter subexpression, please rewrite the query to avoid filters on attributes");
...
xt_node_test_match_impl (XT * node_test, caddr_t qname)
{
  ...
  switch ((ptrlong) node_test)
    ...
    case XP_NODE:
      return 1;
    case XP_TEXT:
      return qname == uname__txt;
    case XP_PI:
      return qname == uname__pi;
    case XP_COMMENT:
      return qname == uname__comment;
    case XP_ELT:
      return (' ' != qname[0]);
    case XP_ELT_OR_ROOT:
      return (' ' != qname[0]) || (qname == uname__root);
    default:
      if (!ARRAYP (node_test))
	GPF_T;
...
xt_node_test_match_parts (XT * node_test, char *local, size_t local_len, caddr_t ns)
{
  ...
  switch ((ptrlong) node_test)
    {
    case 0:
      return (local_len && (' ' != local[0]));
    case XP_NODE:
      return 1;
    case XP_TEXT:
      return ((4 /* strlen(" txt") */  == local_len) && !memcmp (local, uname__txt, 4));
    case XP_PI:
      return ((3 /* strlen(" pi") */  == local_len) && !memcmp (local, uname__pi, 3));
    case XP_COMMENT:
      return ((8 /* strlen(" comment") */  == local_len) && !memcmp (local, uname__comment, 8));
    case XP_ELT:
      return (local_len && (' ' != local[0]));
    case XP_ELT_OR_ROOT:
      return ((local_len && (' ' != local[0])) || ((5 /* strlen(" root") */  == local_len) && !memcmp (local, uname__root, 5)));
    default:
      if (!ARRAYP (node_test))
	GPF_T;
      switch (node_test->type)
...
/ *end mapping schema*/
    ...
    step = xp_make_step (xpp, XP_CHILD, xp_make_name_test_from_qname (xpp, st->xs_xj->xj_element, 1), NULL);
...
xp_parent_step (xpp_t * xpp, xv_join_elt_t * start)
{
  ...
  return xtlist (xpp, 6, XP_STEP, NULL, XP_PARENT,
      xp_make_name_test_from_qname (xpp, start->xj_element, 1), box_xc (parent_xc), NULL);
}
  ...
  DO_BOX (xj_col_t *, xc, inx, start->xj_cols)
  {
	...
	if (!IS_BOX_POINTER (xc->xc_relationship))	/*mapping schema */
	  NCONCF1 (*paths, xtlist (xpp, 6, XP_STEP, NULL, XP_ATTRIBUTE,
		  xp_make_name_test_from_qname (xpp, col_name, 1), NULL, NULL));
/*mapping schema*/
	else
	  {
	    step = xp_make_step (xpp, XP_CHILD, xp_make_name_test_from_qname (xpp, col_name, 1), NULL);
...
xv_paths (xpp_t * xpp, xv_join_elt_t * start, XT * step, XT * end_step, xv_step_t * prev, dk_set_t * paths)
{
  ...
  if (!prev)
    {
      if (XP_ABS_CHILD == axs || XP_ABS_CHILD_WR == axs || XP_ABS_DESC == axs || XP_ABS_DESC_WR == axs || XP_ABS_DESC_OR_SELF == axs
	  || XP_ABS_DESC_OR_SELF_WR == axs)
	{
	  start = xj_top (start);
	  prev = NULL;
	}
...
/ *end mapping schema*/
    ...
    if (XP_DESCENDANT == axs || XP_DESCENDANT_WR == axs || XP_ABS_DESC == axs || XP_ABS_DESC_WR == axs || XP_ABS_DESC_OR_SELF == axs
	|| XP_ABS_DESC_OR_SELF_WR == axs)
      {
...
#else
  *paths = CONS (xtlist_with_tail (xpp, 8, (caddr_t) args, CALL_STMT,
	  box_copy (tree->_.xp_func.qname), NULL, (ptrlong) (tree->_.xp_func.res_dtp), (ptrlong) 0, (ptrlong) 0, NULL), NULL);
...
/*to create xmlview*/
  if (xe->xe_outputxml)
    {
      sprintf (tmp, "%s.%s.xte_%s_%s_%s", xv->xv_schema, xv->xv_user, xv->xv_local_name, xj->xj_element, xj->xj_prefix);
    }
/*end*/
  else
    sprintf (tmp, "%s.%s.http_%s_%s_%s", xv->xv_schema, xv->xv_user, xv->xv_local_name, xj->xj_element, xj->xj_prefix);
  if (xe->xe_is_for_attrs)
	...
	sel = (ST **) t_list (1,
		...
		t_list (3, CALL_STMT,
		    ...
		    t_list (1, t_list (3, CALL_STMT, t_box_string (tmp), cols))), NULL, t_box_string ("ENTITY"), NULL));
      else
	sel = (ST **) t_list (1,
	    t_list (5, BOP_AS, t_list (3, CALL_STMT, t_box_string (tmp), cols), NULL, t_box_string ("ENTITY"), NULL));
...
xp_error (xpp_t * xpp, const char *strg)
{
  ...
  xpp->xpp_err = srv_make_new_error ("37000", "XM028",
      "%.400s, line %d: %.1200s\nin the following expression:\n%.210s", xpp->xpp_err_hdr, lineno, strg, buf);
...
xpyyerror_impl (xpp_t * xpp, char *raw_text, const char *strg)
{
  ...
  xpp->xpp_err = srv_make_new_error ("37000", "XM029",
      ...
      xpp->xpp_err_hdr, lineno, strg, ((NULL == raw_text) ? "" : " at "), ((NULL == raw_text) ? "" : raw_text));
  longjmp (xpp->xpp_reset, 1);
}
...
static int
charref_to_unichar (const char **src_tail_ptr, const char *src_end, const char **err_msg_ret)
{
  ...
  if (!strncmp ("apos;", src_tail, 5))
    {
      src_tail_ptr[0] += 5;
      return '\'';
    }
  if (!strncmp ("quot;", src_tail, 5))
    {
      src_tail_ptr[0] += 5;
      return '\"';
    }
  if (!strncmp ("amp;", src_tail, 4))
    {
      src_tail_ptr[0] += 4;
      return '&';
    }
  if (!strncmp ("lt;", src_tail, 3))
    {
      src_tail_ptr[0] += 3;
      return '<';
    }
  if (!strncmp ("gt;", src_tail, 3))
    {
      src_tail_ptr[0] += 3;
      return '>';
    }
...
caddr_t
xp_charref_to_strliteral (xpp_t * xpp, const char *strg)
{
...
caddr_t
xp_strliteral (xpp_t * xpp, const char *strg, char delimiter, int attr_cont)
{
  caddr_t tmp_buf;
  caddr_t res;
  const char *err_msg;
...
void
xp_reject_option_if_not_allowed (xpp_t * xpp, int type)
{
  ...
  switch (type)
    {
    case XP_XPATH_OPTS:
      xp_error (xpp, "XPATH-specific option listed in '[' ... ']' list before non-XPATH expression");
    case XP_XQUERY_OPTS:
      xp_error (xpp, "XQUERY-specific option listed in '[' ... ']' list before non-XQUERY expression");
    case XP_FREETEXT_OPTS:
      xp_error (xpp, "Freetext-specific option listed in '[' ... ']' list before non-freetext expression");
    default:
      GPF_T;
    }
}
...
void
xp_register_default_namespace_prefixes (xpp_t * xpp)
{
...
void
xp_register_namespace_prefix (xpp_t * xpp, ccaddr_t ns_prefix, ccaddr_t ns_uri)
{
...
void
xp_register_namespace_prefix_by_xmlns (xpp_t * xpp, ccaddr_t xmlns_attr_name, ccaddr_t ns_uri)
{
  if (!strcmp (xmlns_attr_name, "xmlns"))
    xp_env ()->xe_dflt_elt_namespace = box_dv_uname_string (ns_uri);
  else if (!strncmp (xmlns_attr_name, "xmlns:", 6))
...
dk_set_t
xp_bookmark_namespaces (xpp_t * xpp)
{
  dk_set_t *list_ptr = &(xp_env ()->xe_namespace_prefixes);
  return list_ptr[0];
}
...
void
xp_unregister_local_namespaces (xpp_t * xpp, dk_set_t start_state)
{
  dk_set_t *list_ptr = &(xp_env ()->xe_namespace_prefixes);
...
void
xp_set_encoding_option (xpp_t * xpp, caddr_t enc_name)
{
...
ST *
xp_to_sql_tree (sql_comp_t * sc, char *str, caddr_t * err_ret, int is_in_sql)
{
  XT *exp;
  xp_ctx_t *xc;
  xp_ret_t xr;
...
void
xp_copy_lexem_bufs (xpp_t * xpp, xp_lexbmk_t * begin, xp_lexbmk_t * end, int skip_last_n)
{
...
XT *
xp_sql_xp_tree (xpp_t * caller_xpp, xp_lexbmk_t * begin, sql_comp_t * sc, caddr_t viewname, caddr_t * err_ret, int is_in_sql)
{
...
/*  xp_ret_t xr;*/
  ...
  CATCH (CATCH_XP)
  {
    ...
    xq_xml = xtlist (caller_xpp, 10, XQ_FOR_SQL, qr_mem_wrapper,	/* qr_mem_wrapper               */
...
#if 0
	XT *call_stmt =
	    xp_make_call (caller_xpp, box_dv_uname_string ("document-literal"), list (3, xq_xml, NULL, (ptrlong) GE_XML));
...
{
  if ((strstr (str, "EXEC SQL XPATH") == str) || (strstr (str, "EXEC SQL xpath") == str) || (strstr (str, "EXEC SQL XPath") == str))
    {
...
{
  caddr_t err;
  fixed_doc = (caddr_t) sql_compile_static (fixed_doc_text, bootstrap_cli, &err, SQLC_DEFAULT);
  wildcard_doc = (caddr_t) sql_compile_static (wildcard_doc_text, bootstrap_cli, &err, SQLC_DEFAULT);
}
#endif
  ...
  if (start)
    {
      ...
      caddr_t *p2 =
	  (caddr_t *) dk_alloc_box (sizeof (caddr_t) * (1 /*operation */  + 1 /*flags */  + 1 /*<tag> */  + n_words +
	      1 /*</tag> */ ),
	  DV_ARRAY_OF_POINTER);
...
xpt_eq (XT * tree, XT * ctx_step)
{
  ...
  if ((right->type != XP_LITERAL) || (!DV_STRINGP (right->_.literal.val)) || (XP_STEP != left->type))
    return NULL;
  ...
  if (XP_ATTRIBUTE == left->_.step.axis)
    {
      word_or_phrase =
	  xp_word_or_phrase_from_string (NULL /* no xp_error */ , right->_.literal.val, &eh__UTF8,
	  xpt_lhptr (right->_.literal.lhptr), 0);
  ...
  if (!(ARRAYP (node) || ((XT *) XP_TEXT == node) || ((XT *) XP_STAR == node) || ((XT *) XP_ELT == node)
	  || ((XT *) XP_ELT_OR_ROOT == node)))
    return NULL;
  word_or_phrase =
      xp_word_or_phrase_from_string (NULL /* no xp_error */ , right->_.literal.val, &eh__UTF8, xpt_lhptr (right->_.literal.lhptr),
      0);
  if (NULL == word_or_phrase)
    return NULL;
...
ptrlong
xpt_range_flags_of_step (XT * tree, XT * ctx_node)
{
...
*/
  if (0 == stricmp (tree->_.xp_func.qname, "text-contains"))
	...
	return in;
      text_tree =
	  xp_text_parse (args[1]->_.literal.val, &eh__UTF8, xpt_lhptr (args[1]->_.literal.lhptr), NULL /* no runtime options */ ,
	  &err);
      if (err)
	dk_free_tree (err);
      if (NULL != text_tree)
...
xpt_text_exp (XT * tree, XT * ctx_node)
{
  ...
  switch (tree->type)
    ...
    case XP_FILTER:
      return (xpt_combine (BOP_AND, xpt_text_exp (tree->_.filter.path, NULL), xpt_text_exp (tree->_.filter.pred, NULL)));
    case BOP_PLUS:
    case BOP_MINUS:
    case BOP_TIMES:
    case BOP_DIV:
    ...
    case BOP_LTE:
    case BOP_NEQ:
      return (xpt_combine (BOP_OR, xpt_text_exp (tree->_.bin_exp.left, ctx_node), xpt_text_exp (tree->_.bin_exp.right, ctx_node)));
    case BOP_EQ:
...
xp_text_parse (char *str2, encoding_handler_t * enc, lang_handler_t * lang, caddr_t * ret_dtd_config, caddr_t * err_ret)
{
  ...
  if (NULL == str2)
    {
      err_ret[0] =
	  srv_make_new_error ("22023", "FT041", "text criteria is NULL, maybe the value is required before it is calculated");
      return NULL;
    }
...
void
xp_env_push (xpp_t * xpp, char *context_type, char *obj_name, int copy_static_context)
{
...
XT *
xp_make_module (xpp_t * xpp, caddr_t ns_prefix, caddr_t ns_uri, XT * expn)
{
  ...
  res = xtlist (xpp, 7, XQ_QUERY_MODULE,
      ...
      copy_list_to_array (xpp->xpp_xp_env->xe_modules), fundefs, defglobals, expn);
  return res;
}

void
xp_import_schema (xpp_t * xpp, caddr_t ns_prefix, caddr_t ns_uri, caddr_t * at_hints)
{
  caddr_t sch_uri;
  shuric_t *shu;
  caddr_t err = NULL;
  ...
  if (NULL == xpp->xpp_qi)
    {
      xpp->xpp_err =
	  srv_make_new_error ("37XQR", "SQ196", "Unable to retrieve '%.1000s' from SQL compiler due to danger of fatal deadlock",
	  sch_uri);
      longjmp (xpp->xpp_reset, 1);
    }
...
void
xp_import_module (xpp_t * xpp, caddr_t ns_prefix, caddr_t ns_uri, caddr_t * at_hints)
{
  caddr_t uri;
...
/* "i_..." stands for "import_..." */
  ...
  dk_set_push (&(xpp->xpp_preamble_decls), xtlist (xpp, 4, XQ_IMPORT_MODULE, box_dv_short_string (ns_prefix),
	  box_dv_short_string (ns_uri), at_hints));
  if (NULL == xpp->xpp_qi)
...
void
xp_var_decl (xpp_t * xpp, caddr_t var_name, XT * var_type, XT * init_expn)
{
...
XT *
xp_make_name_test_from_qname (xpp_t * xpp, caddr_t qname, int qname_is_expanded)
{
  char *colon;
  ...
  if (('(' == qname[0]) && !qname_is_expanded)
      ...
      if ((4 < len) && ('!' == qname[1]) && ('!' == qname[len - 2]) && (')' == qname[len - 1]))
	{
	  ...
	  if (NULL == colon)
		...
		box_dv_uname_nchars (qname, colon - fullname), box_dv_uname_string (colon + 1), fullname);
	}
    }
  colon = strrchr (qname, ':');
...
XT *
xp_make_seq_type (xpp_t * xpp, ptrlong first_token, caddr_t top_name, XT * type, ptrlong is_nilable, ptrlong n_occurrences)
{
...
*/
  switch (first_token)
    ...
    case PI_LPAR_L:
      ...
      break;
    case COMMENT_LPAR_L:
      mode = XQ_SEQTYPE_NODE;
      top_name = (caddr_t) XP_COMMENT;
      break;
    case TEXT_LPAR_L:
      mode = XQ_SEQTYPE_NODE;
      top_name = (caddr_t) XP_TEXT;
      break;
    case NODE_LPAR_L:
      mode = XQ_SEQTYPE_NODE;
      top_name = (caddr_t) XP_NODE;
      break;
    case ELEMENT_LPAR_L:
      if (top_name)
	top_name = xp_make_expanded_name (xpp, top_name, 0);
      component_name = ((NULL != type) ? xp_make_expanded_name (xpp, (caddr_t) type, 0) : NULL /* not top_name */ );
      mode = XQ_SEQTYPE_ELEMENT;
      break;
  ...
  if (NULL != component_name)
	...
	{
	  if (NULL == type_ns_uri)
	    xp_error_printf (xpp, "No schema loaded for the default namespace specified by name '%s' in sequence type declaration",
		component_name);
	  if (strcmp (type_ns_uri, XS_NS_URI))
	    xp_error_printf (xpp, "No schema loaded for the namespace '%s' specified by name '%s' in sequence type declaration",
		type_ns_uri, component_name);
	  if (XS_SP_TYPES != dict_to_search)
	    xp_error_printf (xpp, "No XMLSchema component definition found for name '%s' in sequence type declaration",
		component_name);
...
component_name_resolved:
  if ((XQ_SEQTYPE_REQ_ONE == n_occurrences) && (NULL == comp))
	...
	{
	  if (NULL != top_name)
	    return xtlist (xpp, 2, XP_PI, box_copy (top_name));
	  else
	    return (XT *) XP_PI;
	}
...
xp_make_sqlcolumn_ref (xpp_t * xpp, caddr_t name)
{
  ...
  if (NULL == xpp->xpp_sql_columns)
    xp_error (xpp,
	"sql:column() can be used only if the text of the query is the constant string that is the first argument of a direct call of xquery_eval()");
  ...
  END_DO_SET ()dk_set_push (xpp->xpp_sql_columns, box_dv_short_string (name));
...
static caddr_t /* XT ** actually */
xp_make_call_args (xpp_t * xpp, char *name, XT ** args)
{
  xp_query_t *xqr;
  xp_env_t *xe = xp_env ();
  xqr = xe->xe_xqr;
...
xp_make_call (xpp_t * xpp, const char *qname, caddr_t arg_array)
{
  ...
  if (NULL == metas->xpfm_executable)
    {
	  ...
	  || !strcmp (XFN_NS_URI ":collection", metas->xpfm_name) || !strcmp (XXF_NS_URI ":collection", metas->xpfm_name))
	{
  ...
  res = xtlist_with_tail (xpp, 8, arg_array, CALL_STMT,
      ...
      (ptrlong) (metas->xpfm_res_dtp), xe_new_xqst (xpp, XQST_REF), xe_new_xqst (xpp, XQST_REF), var);
  if (xp_env ()->xe_for_interp)
    {
      ...
      if (itercount != 0)
	{
	  ...
	  cart = xtlist_with_tail (xpp, 8, (caddr_t) iter_vars, CALL_STMT,
	      ...
	      DV_ARRAY_OF_XQVAL, xe_new_xqst (xpp, XQST_REF), xe_new_xqst (xpp, XQST_REF), cart_var);
	  for (iterctr = allctr = 0; allctr < argc; allctr++)
	      ...
	      if (descr->xpfma_is_iter)
		{
		  ...
		  cart->_.xp_func.argtrees[iterctr * 2 + 1] = res->_.xp_func.argtrees[allctr];
...
static XT *
xp_find_defun_in_includes (shuric_t * shu, caddr_t name, caddr_t * uri_ret)
{
...
XT *
xp_make_call_or_funcall (xpp_t * xpp, caddr_t qname, caddr_t arg_array)
{
  xp_env_t *env = xp_env ();
  xp_env_t *top_env;
  XT *defun = NULL;
  ...
  if (defun->_.defun.argcount != BOX_ELEMENTS (arg_array))
    {
	  ...
	  ((NULL != import_uri) ? import_uri : ""), (long) (defun->_.defun.argcount), (long) (BOX_ELEMENTS (arg_array)));
      dk_free_box (name);
      xp_error (xpp, buf);
    }
  sprintf (buf, "<result of %s() at line %d>", name, (int) (xpp->xpp_curr_lexem->xpl_lineno));
  ...
  res = xtlist_with_tail (xpp, 8, arg_array, CALL_STMT,
      ...
      box_num ((ptrlong) (xpf_call_udf)), (ptrlong) XPDV_NODESET, xe_new_xqst (xpp, XQST_REF), xe_new_xqst (xpp, XQST_REF), var);
  dk_free_box (name);
  return res;
}
...
XT *
xp_make_defun (xpp_t * xpp, caddr_t name, caddr_t param_array, XT * ret_type, xp_query_t * body_xqr)
{
  ptrlong argctr;
  XT *defun, **defun_ptr;
...
caddr_t
shuric_uri_to_text__xqr (caddr_t uri, query_instance_t * qi, void *env, caddr_t * err_ret)
{
...
void
shuric_parse_text__xqr (shuric_t * shuric, caddr_t uri_text_content, query_instance_t * qi, void *env, caddr_t * err_ret)
{
  ptrlong *map;
  int n_slots, fill = 0;
...
void
shuric_destroy_data__xqr (struct shuric_s *shuric)
{
...
caddr_t
shuric_get_cache_key__xqr (struct shuric_s *shuric)
{
  xp_query_t *xqr = (xp_query_t *) shuric->shuric_data;
  return xqr->xqr_key;
}
...
shuric_t *
xqr_shuric_retrieve (query_instance_t * qi, caddr_t uri, caddr_t * err_ret, shuric_t * loaded_by)
{
...
xqr_stub_for_funcall (xpf_metadata_t * metas, int argcount)
{
  ...
  call = xtlist_with_tail (&l_xpp, 8, (caddr_t) arg_array, CALL_STMT,
      ...
      box_num ((ptrlong) (metas->xpfm_executable)),
      (ptrlong) (metas->xpfm_res_dtp), xe_new_xqst (&l_xpp, XQST_REF), xe_new_xqst (&l_xpp, XQST_REF), var);
  ...
  if (itercount != 0)
      ...
      if (NULL != var)
	{
	  ...
	  box_dv_uname_string ("(internal) Cartesian product loop"),
	  box_num ((ptrlong) (xpf_cartesian_product_loop)),
	  DV_ARRAY_OF_XQVAL, xe_new_xqst (&l_xpp, XQST_REF), xe_new_xqst (&l_xpp, XQST_REF), cart_var);
	  ...
	  if (descr->xpfma_is_iter)
	    {
	      ...
	      cart->_.xp_func.argtrees[iterctr * 2 + 1] = call->_.xp_func.argtrees[allctr];
...
xp_query_t *
xp_query_parse (query_instance_t * qi, char *str, ptrlong predicate_type, caddr_t * err_ret, xp_query_env_t * xqre)
{
  if (NULL == str)
    {
  ...
  if (!DV_STRINGP (str))
	...
	for (ctr = 0; ctr < buflen; ctr++)
	  {
	    ...
	    dk_set_push (&lexems, list (5,
		    curr->xpl_lineno, curr->xpl_depth, box_copy (curr->xpl_raw_text), curr->xpl_lex_value, curr->xpl_state));
#else
	    dk_set_push (&lexems, list (4, curr->xpl_lineno, curr->xpl_depth, box_copy (curr->xpl_raw_text), curr->xpl_lex_value));
#endif
	  }
      }
      END_DO_SET ();
      if (NULL != xpp->xpp_err)
	{
	  dk_set_push (&lexems, list (3,
		  ...
		  xpp->xpp_lexdepth, box_copy (ERR_MESSAGE (xpp->xpp_err))));
	}
...
static long
xp_axis_to_ap_axis_wr (long axs)
{
  switch (axs)
    {
    case XP_ABS_CHILD:
      return XP_ABS_CHILD_WR;
    case XP_ABS_DESC:
      return XP_ABS_DESC_WR;
    case XP_CHILD:
      return XP_CHILD_WR;
    case XP_DESCENDANT:
      return XP_DESCENDANT_WR;
    case XP_DESCENDANT_OR_SELF:
      return XP_DESCENDANT_OR_SELF_WR;
...
xp_query_enable_wr (xp_query_t * xqr, XT * tree, int target_is_wr)
{
  ...
  switch (tree->type)
      ...
      if (NULL != tree->_.step.preds && 1 <= BOX_ELEMENTS (tree->_.step.preds))
	{
	  ...
	  if ((BOP_AND == step_pred->type) || (BOP_LIKE == step_pred->type))
	    {
	      ...
	      if ((XP_STEP == l_exp->type) && (XP_SELF == l_exp->_.step.axis) && (XP_LITERAL == exp_r->type))
		{
		  lang_handler_t *lh = xpt_lhptr (exp_r->_.literal.lhptr);
		  int word_count = lh_count_words (&eh__UTF8, lh,
      ...
      if (target_is_wr && (((XT *) XP_ELT == step_node) || ((XT *) XP_ELT_OR_ROOT == step_node) || (ARRAYP (step_node)
		  && (XP_NAME_EXACT == step_node->type))))
	{
	  goto enable_wr_here;
	}
...
xp_wordstack_from_string (char *str, encoding_handler_t * eh, lang_handler_t * lh, dk_set_t * wordstack_ptr)
{
  ...
  ret = lh_iterate_patched_words (eh, lh->lh_ftq_language,
      str, strlen (str),
      lh->lh_ftq_language->lh_is_vtb_word, lh->lh_ftq_language->lh_normalize_word, push_string_into_set_callback, wordstack_ptr);
  return ret;
}
...
xp_word_from_exact_string (xpp_t * xpp, const char *str, encoding_handler_t * eh, int allow_xp_error)
{
  ...
  word_end =
      (utf8char *) eh_encode_buffer__UTF8 (unidata, unidata + unidatalen, (char *) (word_buf), (char *) (word_buf + word_buflen));
