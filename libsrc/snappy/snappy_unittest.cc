...
static void Measure(const char* data,
  ...
  const float ctimeMean=ctime[med]/repeats;
  const float utimeMean=utime[med]/repeats;

  float comp_rate = (length / ctimeMean) / 1048576.0;
  float uncomp_rate = (length / utimeMean) / 1048576.0;
  ...
  printf("%-7s [b %dM] bytes %6d -> %6d %4.1f%%  "
         "comp %5.1f MB/s  uncomp %5s MB/s sum_time=%.6f \n",
         x.c_str(),
         block_size/(1<<20),
